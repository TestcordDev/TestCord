...
function isOutgoingFriendRequest(userId) {
    return RelationshipStore.getRelationshipType(userId) === 4 /* RelationshipType.OUTGOING_REQUEST */;
}
function cancelOutgoingFriendRequest(userId) {
    if (!isOutgoingFriendRequest(userId))
        return;
    return RelationshipActions.cancelFriendRequest(userId);
}
const userContextPatch = (children, { user }) => {
    if (!user || !isOutgoingFriendRequest(user.id))
        return;
    children.push(<Menu.MenuItem id="vc-cancel-outgoing-friend-request" label="Cancel Outgoing Friend Request" action={() => cancelOutgoingFriendRequest(user.id)}/>);
};
export default definePlugin({
    ...
    isOutgoingButton({ relationshipType, hasOutgoingPendingGameFriends }) {
        return relationshipType === 4 /* RelationshipType.OUTGOING_REQUEST */ || hasOutgoingPendingGameFriends === true;
    },
    getCancelFriendRequestTextButtonProps(userId) {
        return {
    ...
    getCancelFriendRequestIconButtonProps(context, Icon) {
        if (!this.isOutgoingButton(context))
            return {};
        return {
            "aria-label": "Cancel Outgoing Friend Request",
            disabled: false,
            icon: Icon ? (iconProps) => <Icon {...iconProps} color="var(--status-danger)"/> : undefined,
