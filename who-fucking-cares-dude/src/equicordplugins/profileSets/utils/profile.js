...
function setPendingChanges(payload, guildId) {
    dispatch("USER_PROFILE_SETTINGS_SET_PENDING_CHANGES", guildId ? { guildId, ...payload } : payload);
}
function openProfileImagePreview(uploadType, image, guildId) {
...
function hasAvatarDecoration(value) {
...
function normalizeDisplayNameStyles(value) {
    if (!value)
        return null;
    const fontId = value.fontId ?? value.font_id;
    const effectId = value.effectId ?? value.effect_id;
    if (typeof fontId !== "number" || typeof effectId !== "number")
        return null;
...
async function processImage(imageData, userId, type, guildId, useGuildPath) {
    if (!imageData)
        return null;
...
export async function getCurrentProfile(guildId, options = {}) {
    ...
    const pendingChangesDefault = UserProfileSettingsStore.getPendingChanges() ?? {};
    const pendingChangesForGuild = UserProfileSettingsStore.getPendingChanges(effectiveGuildId) ?? {};
    const pendingChanges = isGuildProfile && Object.keys(pendingChangesForGuild).length > 0
    ...
    const avatarToUse = hasImageInput(pendingAvatar)
        ? pendingAvatar
    ...
    const avatarInput = hasImageInput(avatarToUse)
        ? avatarToUse
    ...
    const bannerToUse = hasImageInput(pendingBanner)
        ? pendingBanner
...
function resolvePendingAvatar(pendingChanges) {
    if (!pendingChanges)
        return null;
...
function collectibleEqBySku(a, b) {
    if (a == null || b == null)
        return a == null && b == null;
    return String(a.skuId ?? "") === String(b.skuId ?? "");
}
function avatarDecorationEq(a, b) {
    if (a == null || b == null)
        return a == null && b == null;
    return String(a.skuId ?? "") === String(b.skuId ?? "") && String(a.asset ?? "") === String(b.asset ?? "");
}
function nameplateEq(a, b) {
    if (a == null || b == null)
        return a == null && b == null;
    return String(a.skuId ?? "") === String(b.skuId ?? "") && String(a.asset ?? "") === String(b.asset ?? "");
}
export async function loadPresetAsPending(preset, guildId, options = {}) {
    try {
