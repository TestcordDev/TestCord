...
function encryptBuffer(key, iv, data) {
...
function getUploadBody(encryptedData, encryptedName, fileIv, mimeType) {
    return {
...
export async function uploadDrop(_event, apiKey, fileName, mimeType, data, expiryDays, maxDownloads) {
    ...
    if (!normalizedApiKey)
        return { success: false, error: "Anon.li API key is missing." };
    if (!normalizedFileName)
        return { success: false, error: "File name is invalid." };
    ...
    try {
        ...
        if (normalizedMaxDownloads > 0)
            createBody.maxDownloads = normalizedMaxDownloads;
        const drop = await requestJson(`${API_BASE}/drop`, normalizedApiKey, createBody);
        const dropId = getDropId(drop);
        if (!dropId)
            return { success: false, error: "Anon.li did not return a drop id." };
        const file = await requestJson(`${API_BASE}/drop/${encodeURIComponent(dropId)}/file`, normalizedApiKey, getUploadBody(encryptedData, encryptedName, fileIv, normalizedMimeType));
        const fileId = getFileId(file);
        const uploadUrl = getUploadUrl(file);
        if (!fileId || !uploadUrl)
            return { success: false, error: "Anon.li did not return an upload URL." };
        ...
        if (!uploadResponse.ok)
            return { success: false, error: `Storage upload failed with HTTP ${uploadResponse.status}.` };
        if (!etag)
            return { success: false, error: "Storage upload did not return an ETag." };
        await requestJson(`${API_BASE}/drop/${encodeURIComponent(dropId)}?action=finish`, normalizedApiKey, {
            files: [{
                    fileId,
