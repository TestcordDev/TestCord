...
function shouldReply(message, currentUserId) {
    if (!settings.store.active)
        return false;
    if (message.author?.id === currentUserId)
        return false;
    if (settings.store.ignoreBots && message.author?.bot)
        return false;
...
async function sendAutoReply(message) {
    ...
    if (settings.store.replyWithMention)
        text = `<@${message.author?.id}> ${text}`;
    lastReplied.set(message.author?.id, Date.now());
    const body = { content: text, tts: false, flags: 0 };
    if (settings.store.replyAsQuote && message.id) {
        body.message_reference = { message_id: message.id, channel_id: channelId };
        if (message.guild_id)
            body.message_reference.guild_id = message.guild_id;
...
function AutoReplyIcon({ active, height = 20, width = 20, className }) {
    return (<svg className={className} aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24" style={{ color: active ? "var(--status-positive)" : "currentColor" }}>
            <path fill="currentColor" d="M21 2a1 1 0 0 1 1 1v6a1 1 0 0 1-1 1h-6a1 1 0 1 1 0-2h3.93A8 8 0 0 0 6.97 5.78a1 1 0 0 1-1.26-1.56A9.98 9.98 0 0 1 20 6V3a1 1 0 0 1 1-1ZM3 22a1 1 0 0 1-1-1v-6a1 1 0 0 1 1-1h6a1 1 0 1 1 0 2H5.07a8 8 0 0 0 11.96 2.22 1 1 0 1 1 1.26 1.56A9.99 9.99 0 0 1 4 18v3a1 1 0 0 1-1 1Z"/>
        </svg>);
}
// ── Chat Bar Button ────────────────────────────────────────────────────────────
const AutoReplyButton = ({ isMainChat }) => {
    ...
    return (<ChatBarButton tooltip={tooltip} onClick={toggle}>
...
export default definePlugin({
    ...
    flux: {
        async MESSAGE_CREATE({ message, optimistic }) {
            if (optimistic)
                return;
            if (!message?.author?.id || !message?.channel_id)
                return;
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("AutoReply", () => (<HeaderBarButton icon={() => <AutoReplyIcon active={settings.store.active}/>} tooltip={`AutoReply: ${settings.store.active ? "ON" : "OFF"}`} onClick={() => { settings.store.active = !settings.store.active; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("AutoReply", () => (<ChannelToolbarButton icon={() => <AutoReplyIcon active={settings.store.active}/>} tooltip={`AutoReply: ${settings.store.active ? "ON" : "OFF"}`} onClick={() => { settings.store.active = !settings.store.active; }}/>), 5);
        }
    },
    stop() {
        for (const timeout of pendingReplies)
