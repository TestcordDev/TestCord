...
const PLUGIN_ENABLED_PATHS = {};
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew }) {
    ...
    const stability = React.useMemo(() => PluginHealth.getStability(plugin.name), [plugin.name, stabilityTick]);
    ...
    const stabilityTooltip = showStabilityBadge
        ...
        : undefined;
    const footer = (<div className={cl("card-meta")}>
            <span className={cl("card-source")}>
                {pluginDetails && (<img src={pluginDetails.src} alt={pluginDetails.alt} className={cl("source")}/>)}
                {tooltip}
            </span>
            {showStabilityBadge && (<span className={cl("card-stability")} data-badge={stability.badge} title={stabilityTooltip}>
                    {stability.badge === "unstable" ? "Unstable" : "Flaky"}
                </span>)}
            {!!plugin.tags?.length && (<div className={cl("card-tags")}>
                    {plugin.tags.slice(0, 2).map(tag => (<span key={tag} className={cl("card-tag")}>{tag}</span>))}
                    {plugin.tags.length > 2 && <span className={cl("card-tag")}>+{plugin.tags.length - 2}</span>}
                </div>)}
        </div>);
    return (<AddonCard name={plugin.name} tooltip={tooltip} description={plugin.description} isNew={isNew} enabled={isEnabled()} setEnabled={toggleEnabled} disabled={disabled} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} infoButton={<button role="switch" onClick={() => openPluginModal(plugin, onRestartNeeded)} className={cl("info-button")}>
