...
export function HashLink({ repo, hash, disabled = false }) {
    return (<Link href={`${repo}/commit/${hash}`} disabled={disabled}>
            {hash.slice(0, 7)}
        </Link>);
}
export function Changes({ updates, repo, repoPending }) {
    return (<Card className={Margins.top16} style={{ padding: 0 }} defaultPadding={false}>
            {updates.map(({ hash, author, message }, i) => (<div key={hash} style={{
                padding: "12px 16px",
...
export function Updatable(props) {
    ...
    function showDiscardForError(error) {
        if (!IS_DEV)
            return;
    ...
    function confirmDiscardLocalChanges() {
        return new Promise(resolve => {
            openModal(props => (<ConfirmModal {...props} title="Discard local changes?" subtitle="This resets your local Testcord files to the selected remote branch and removes untracked files before updating." confirmText="Discard and Update" cancelText="Cancel" variant="primary" onConfirm={() => resolve(true)} onCancel={() => resolve(false)}/>));
        });
    }
    return (<>
            <Flex className={Margins.bottom8} gap="8px">
                <Button disabled={isUpdating || isChecking} onClick={runWithDispatch(setIsChecking, async () => {
            ...
            if (outdated) {
                ...
                {isOutdated && (<Button size="small" variant="primary" disabled={isUpdating || isChecking} onClick={runWithDispatch(setIsUpdating, async () => {
                if (await update()) {
                    setShowDiscardLocalChanges(false);
                    setUpdates([]);
                    await new Promise(r => {
                        openModal(props => (<ConfirmModal {...props} title="Update Success!" subtitle="Successfully updated. Restart now to apply the changes?" confirmText="Restart" cancelText="Not now!" variant="primary" onConfirm={() => {
                                relaunch();
                                r();
                            }} onCancel={r}/>));
                    });
                ...
                {IS_DEV && isOutdated && showDiscardLocalChanges && (<Button size="small" variant="secondary" disabled={isUpdating || isChecking} onClick={runWithDispatch(setIsUpdating, async () => {
                if (!await confirmDiscardLocalChanges())
                    return;
