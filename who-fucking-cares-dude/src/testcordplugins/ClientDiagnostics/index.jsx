...
const SETTINGS_KEYS = ["sortBy", "showDisabled", "showApiPlugins", "refreshMs"];
const REFRESH_SETTING_KEYS = ["refreshMs"];
...
let probeDisposers = [];
let lagNotificationInterval;
const lagNotificationTimes = new Map();
...
function getSurfaceStats(stat, surface) {
...
function recordCall(pluginName, surface, duration, failed, beforeHeap, afterHeap) {
    ...
    if (beforeHeap !== undefined && afterHeap !== undefined) {
        ...
        if (delta > 0)
            stat.heapIncrease += delta;
        else
            stat.heapDecrease -= delta;
    }
}
function recordAsync(pluginName, surface, duration) {
    const stat = getPluginStats(pluginName);
    const surfaceStat = getSurfaceStats(stat, surface);
    stat.asyncCalls++;
...
function stringifyCodePart(value) {
    if (typeof value === "function")
        return normalizeCodeSnippet(value.toString());
...
function rememberSourceSnippet(pluginName, surface, label, source) {
    if (typeof source !== "function") {
        const code = normalizeCodeSnippet(stringifyCodePart(source));
        if (!code)
            return;
        const snippets = sourceSnippets.get(pluginName) ?? [];
        if (snippets.some(s => s.surface === surface && s.label === label))
            return;
    ...
    const snippets = sourceSnippets.get(pluginName) ?? [];
    if (snippets.some(s => s.surface === surface && s.fn === source))
        return;
    const snippet = { surface, label, code: "", fn: source };
    snippets.push(snippet);
    if (snippets.length > 30)
        snippets.shift();
    sourceSnippets.set(pluginName, snippets);
}
function runMeasured(pluginName, surface, callback) {
...
function wrapMeasured(pluginName, surface, original) {
    const wrapped = function (...args) {
        return runMeasured(pluginName, surface, () => original.apply(this, args));
    };
    measuredFunctions.add(wrapped);
    return wrapped;
}
function wrapObjectMethod(owner, key, pluginName, surface) {
    const original = owner[key];
    if (typeof original !== "function" || measuredFunctions.has(original))
        return;
    rememberSourceSnippet(pluginName, surface, `${surface} callback`, original);
    owner[key] = wrapMeasured(pluginName, surface, original);
}
function asRecord(value) {
    return isObject(value) ? value : null;
}
function rememberPatchSnippets(plugin) {
...
function instrumentPlugin(plugin) {
    ...
    for (const command of plugin.commands ?? []) {
        ...
        if (commandRecord)
            wrapObjectMethod(commandRecord, "execute", plugin.name, "command");
    }
    ...
    for (const [field, key, surface] of renderFields) {
        ...
        if (owner)
            wrapObjectMethod(owner, key, plugin.name, surface);
    }
...
function changeResource(pluginName, key, delta) {
    const stat = getPluginStats(pluginName);
    stat[key] = Math.max(0, stat[key] + delta);
    stat.lastSeen = Date.now();
}
function rememberResource(map, id, pluginName, key) {
    map.set(id, pluginName);
    changeResource(pluginName, key, 1);
}
function forgetResource(map, id, key) {
...
function getEventKey(type, options) {
    const capture = typeof options === "boolean" ? options : options?.capture === true;
    return `${type}:${capture}`;
}
function changeListenerCount(pluginName, delta) {
    const count = Math.max(0, (listenerCountByPlugin.get(pluginName) ?? 0) + delta);
    if (count === 0)
        listenerCountByPlugin.delete(pluginName);
    else
        listenerCountByPlugin.set(pluginName, count);
    changeResource(pluginName, "listeners", delta);
}
function rememberListener(target, type, listener, pluginName, options) {
...
function forgetListener(target, type, listener, options) {
...
function installGlobalProbes() {
    ...
    window.setInterval = ((handler, timeout, ...args) => {
        ...
        const wrapped = (...callbackArgs) => runMeasured(context.pluginName, "interval", () => handler(...callbackArgs));
    ...
    try {
        ...
        for (const dispose of probeDisposers.reverse())
            dispose();
        probeDisposers = [];
        restoreGlobalProbes();
        throw error;
...
function rebuildResourceCounters() {
    ...
    for (const pluginName of intervalOwners.values())
        changeResource(pluginName, "intervals", 1);
    for (const pluginName of timeoutOwners.values())
        changeResource(pluginName, "pendingTimeouts", 1);
    for (const pluginName of frameOwners.values())
        changeResource(pluginName, "animationFrames", 1);
...
function buildRows(showDisabled, showApiPlugins, sortBy) {
    const rows = new Map();
    for (const [name, stat] of stats) {
        const plugin = plugins[name];
        const api = name.endsWith("API");
        if (!showApiPlugins && api)
            continue;
        rows.set(name, {
            ...stat,
...
function formatBytes(value) {
    ...
    if (absValue >= 1024 * 1024 * 1024)
        return `${sign}${(absValue / 1024 / 1024 / 1024).toFixed(2)} GB`;
    if (absValue >= 1024 * 1024)
        return `${sign}${(absValue / 1024 / 1024).toFixed(1)} MB`;
    if (absValue >= 1024)
        return `${sign}${(absValue / 1024).toFixed(1)} KB`;
...
function matchesPluginSearch(pluginName, query) {
...
function getRecommendation(row, reasons) {
...
function getRelevantSnippets(row) {
    const snippets = sourceSnippets.get(row.name) ?? [];
    return [...snippets]
        .sort((a, b) => {
        if (a.surface === row.hotSurface)
            return -1;
        if (b.surface === row.hotSurface)
            return 1;
        if (a.surface === "patch")
            return -1;
        if (b.surface === "patch")
...
function buildImpactAnalysis(rows) {
    return rows
...
function maybeSendLagNotification() {
    if (!settings.store.lagNotifications)
        return;
    if (Date.now() - startedAt < LAG_NOTIFICATION_MIN_COLLECTION_MS)
        return;
    const now = Date.now();
...
function startLagNotifications() {
    if (lagNotificationInterval !== undefined)
        return;
...
function copyPluginMonitorReport(selectedRow, rows) {
    const memory = getMemory();
...
function SearchControls({ query, onChange, onSearch, disabled }) {
    return (<div className={cl("search-controls")}>
...
function PluginRow({ row }) {
    ...
    return (<div className={cl("row", "body")}>
...
function SurfaceBreakdown({ row }) {
    ...
    return (<div className={cl("surface-list")}>
            {surfaces.map(([surface, stat]) => (<div className={cl("surface-row")} key={surface}>
...
function SnippetList({ item }) {
    if (item.snippets.length === 0) {
        return (<BaseText size="sm" color="text-muted">
    ...
    return (<div className={cl("snippet-list")}>
            {item.snippets.map((snippet, index) => (<div className={cl("snippet")} key={`${snippet.surface}-${index}`}>
...
function ImpactDetails({ item }) {
    const { row } = item;
    return (<div className={cl("details")}>
            ...
            <div className={cl("detail-section")}>
                ...
                <div className={cl("reason-list")}>
                    {item.reasons.map(reason => (<div className={cl("reason-line")} key={`${reason.label}-${reason.value}`}>
...
function ImpactAnalysisCard({ item, selected, onSelect }) {
    const { row } = item;
    const impactClass = getImpactClass(row.impact);
    return (<div className={cl("analysis-card", selected && "selected")}>
...
function ImpactAnalysisPage() {
    ...
    return (<div className={cl("page")}>
            ...
            {items.length === 0 ? (<div className={cl("empty")}>
                ...
                </div>) : filteredItems.length === 0 ? (<div className={cl("empty")}>
                ...
                </div>) : (<div className={cl("analysis-list")}>
                    {filteredItems.map(item => (<ImpactAnalysisCard key={item.row.name} item={item} selected={selectedPlugin === item.row.name} onSelect={() => setSelectedPlugin(selectedPlugin === item.row.name ? undefined : item.row.name)}/>))}
                </div>)}
        </div>);
}
function PluginMonitorPage() {
    ...
    const filteredRows = React.useMemo(() => (normalizedQuery
        ...
        : rows).slice(0, 16), [normalizedQuery, rows]);
    const measuredRows = rows.filter(row => row.calls > 0);
    ...
    return (<div className={cl("page")}>
            ...
            <div className={cl("monitor-layout")}>
                <div className={cl("monitor-results")}>
                    {filteredRows.length === 0 ? (<BaseText size="sm" color="text-muted">No plugin matches that search.</BaseText>) : filteredRows.map(row => (<div className={cl("monitor-result", selectedRow?.name === row.name && "selected")} key={row.name}>
                            <div className={cl("plugin-cell")}>
                                <BaseText size="sm" weight="semibold" lineClamp={1}>{row.name}</BaseText>
                ...
                {selectedRow ? (<div className={cl("monitor-panel")}>
                        ...
                        <div className={cl("detail-section")}>
                            ...
                            {reasons.length === 0 ? (<BaseText size="sm" color="text-muted">No risk signal for this plugin yet.</BaseText>) : (<div className={cl("reason-list")}>
                                    {reasons.map(reason => (<div className={cl("reason-line")} key={`${reason.label}-${reason.value}`}>
                                            <ReasonPill reason={reason}/>
                                            <BaseText size="sm">{reason.description}</BaseText>
                                        </div>))}
                                </div>)}
...
function DiagnosticsPage() {
    ...
    const elapsed = useFixedTimer({ interval: refreshMs });
    const memory = getMemory();
    const rows = React.useMemo(() => buildRows(showDisabled, showApiPlugins, sortBy), [elapsed, showDisabled, showApiPlugins, sortBy]);
    ...
    return (<div className={cl("page")}>
            ...
            {filteredRows.length === 0 && (<div className={cl("empty")}>
                    <BaseText size="md" weight="semibold">No plugin matches that search.</BaseText>
                    <BaseText size="sm" color="text-muted">Try another plugin name or clear the search field.</BaseText>
                </div>)}
        </div>);
    ...
    return (<div className={cl("page")}>
            ...
            <div className={cl("guide-section")}>
                ...
                <div className={cl("guide-grid")}>
                    {GUIDE_ITEMS.map(item => (<div className={cl("guide-item")} key={item.label}>
            ...
            <div className={cl("guide-section")}>
                ...
                <div className={cl("guide-grid")}>
                    {MONITOR_GUIDE_ITEMS.map(item => (<div className={cl("guide-item")} key={item.label}>
                            <BaseText size="sm" weight="semibold">{item.label}</BaseText>
                            <BaseText size="sm" color="text-muted">{item.description}</BaseText>
                        </div>))}
                </div>
