...
const AppStartPerformance = findByPropsLazy("markWithDelta", "markAndLog", "markAt");
function TimerItem({ emoji, prefix, log, delta, instance }) {
...
function TimingSection({ title, logs, traceEnd }) {
    ...
    return (<section>
            ...
            <code>
                {traceEnd && (<div style={{ color: "var(--text-strong)", marginBottom: 5, userSelect: "text" }}>
                ...
                <div style={{ color: "var(--text-strong)", display: "grid", gridTemplateColumns: "repeat(3, auto) 1fr", gap: "2px 10px", userSelect: "text" }}>
                    ...
                    {AppStartPerformance.logs.map((log, i) => (<TimerItem key={i} {...log} instance={timings[i]}/>))}
                </div>
            </code>
        </section>);
}
function ServerTrace({ trace }) {
...
function StartupTimingPage() {
    if (!AppStartPerformance?.logs)
        return <div>Loading...</div>;
    const serverTrace = AppStartPerformance.logGroups.find(g => g.serverTrace)?.serverTrace;
    return (<React.Fragment>
            <TimingSection title="Startup Timings" logs={AppStartPerformance.logs} traceEnd={AppStartPerformance.endTime_}/>
            {/* Lazy Divider */}
