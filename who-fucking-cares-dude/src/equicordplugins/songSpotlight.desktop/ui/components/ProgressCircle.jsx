...
const SIZE = 50;
export default function ProgressCircle({ border, audioRef, playingRef, ...props }) {
    ...
    return (<svg {...props} viewBox={`0 0 ${SIZE * 2} ${SIZE * 2}`}>
            <circle cx={SIZE} cy={SIZE} r={radius} fill="none" stroke="currentColor" strokeWidth={stroke} strokeDasharray={circumference} strokeDashoffset={circumference * (1 - progress)} strokeLinecap="round" transform={`rotate(-90 ${SIZE} ${SIZE})`} data-empty={progress === 0}/>
        </svg>);
}
