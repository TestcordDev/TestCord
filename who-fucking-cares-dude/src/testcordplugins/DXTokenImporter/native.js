...
import { ENCRYPTION_PREFIX, FALLBACK_PREFIX, TOKEN_REGEX_SOURCE } from "./common";
...
export async function checkToken(_, token) {
    if (!validToken(token))
...
export async function encryptToken(_, token) {
    if (!validToken(token))
        return null;
    return encryptSecret(_, token);
}
export async function decryptStoredToken(_, payload) {
    if (typeof payload !== "string" || payload.length > MAX_ENCRYPTED_LENGTH)
        return null;
...
export async function encryptSecret(_, value) {
    ...
    try {
        ...
        console.warn("[DXTokenImporter] safeStorage encrypt failed, using built-in cipher:", e.message);
    }
    ...
    try {
        ...
        console.error("[DXTokenImporter] built-in cipher failed:", e.message);
        return null;
    }
}
export async function decryptSecret(_, payload) {
...
export async function exportVault(_, json, passphrase) {
...
export async function importVault(_, payload, passphrase) {
...
export async function makeLockSalt() {
    return crypto.randomBytes(16).toString("hex");
}
export async function deriveLockVerifier(_, passphrase, saltHex) {
    if (!validPassphrase(passphrase) || typeof saltHex !== "string" || !/^[0-9a-f]{32}$/.test(saltHex))
        return null;
    return crypto.scryptSync(passphrase, Buffer.from(saltHex, "hex"), 32).toString("hex");
}
export async function verifyLock(_, passphrase, saltHex, verifierHex) {
    if (typeof verifierHex !== "string" || !/^[0-9a-f]{64}$/.test(verifierHex))
        return false;
...
function decryptDPAPIKeys(hexKeys) {
...
function decryptToken(encryptedBase64, masterKey) {
...
export async function findLocalTokens() {
    ...
    if (process.platform === "win32")
        return findLocalTokensWindows();
    if (process.platform === "darwin")
        return findLocalTokensUnix("darwin");
    if (process.platform === "linux")
        return findLocalTokensUnix("linux");
    return [];
}
function findLocalTokensWindows() {
    ...
    for (const app of apps) {
        ...
        try {
            ...
            if (typeof encryptedKey === "string" && encryptedKey)
                installs.push({ appPath, encryptedKey });
        }
        catch (e) {
            console.warn("[DXTokenImporter] Failed to read Local State of", app, e.message);
        }
    }
    ...
    try {
        ...
        console.warn("[DXTokenImporter] DPAPI unwrap failed:", e.message);
        return [];
    }
...
function findLocalTokensUnix(platform) {
