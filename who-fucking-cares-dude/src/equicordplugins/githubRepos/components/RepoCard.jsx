...
export function RepoCard({ repo, showStars, showLanguage }) {
    const handleClick = () => window.open(repo.html_url, "_blank");
    const langColor = getLanguageColor(repo?.language);
    return (<div className={cl("card")} onClick={handleClick}>
            ...
            <div className={cl("info")}>
                ...
                {repo.description && (<BaseText size="sm" className={cl("description")}>
                        {repo.description}
                    </BaseText>)}
                {(showLanguage || showStars) && (<div className={cl("meta")}>
                        {showLanguage && repo.language && (<span className={cl("language")}>
                                <span className={cl("language-color")} style={{ backgroundColor: langColor }}/>
                                {repo.language}
                            </span>)}
                        {showStars && (<span className={cl("stars")}>
                                <Star className={cl("star-icon")} width={12} height={12}/>
                                {repo.stargazers_count.toLocaleString()}
                            </span>)}
                    </div>)}
            </div>
            <svg className={cl("link")} aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24">
