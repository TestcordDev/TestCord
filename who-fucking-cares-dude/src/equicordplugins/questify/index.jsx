...
function stopPerAccountTasks(source, preserveResume = true) {
...
function resumeAutoCompletesIfReady() {
...
function enrolledIncompleteButton(args) {
...
export default definePlugin({
    ...
    startAt: "Init" /* StartAt.Init */, // Needed in order to beat Read All Messages to inserting above the server list.
    managedStyle,
    settings,
    ...
    flux: {
        ...
        QUESTS_USER_STATUS_UPDATE(data) {
            ...
            const userStatus = snakeToCamel(data).userStatus;
            ...
            if (completedRecently && !claimedAt && !notifiedCompletedQuests.has(userStatus.questId)) {
                ...
                if (getQuestifySettings().questCompletedAlertSound) {
                    playAudio(getQuestifySettings().questCompletedAlertSound, { volume: Math.max(0, Math.min(100, getQuestifySettings().questCompletedAlertVolume)) });
                }
            }
        },
    ...
    start() {
        ...
        if (enabledOnStartup) {
            addServerListElement(0 /* ServerListRenderPosition.Above */, this.renderQuestifyButton);
        }
        onceReady.then(() => {
            ...
            if (!getQuestifySettings().disableQuestsEverything) {
                ...
                removeServerListElement(0 /* ServerListRenderPosition.Above */, this.renderQuestifyButton);
            }
        });
    },
    stop() {
