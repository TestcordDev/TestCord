...
function Icon(iconProps) {
    return (<Tooltip text={iconProps.tooltip ?? ""}>
            {props => (<svg aria-hidden="true" role="img" width="1em" height="1em" {...props} fill="none" viewBox={iconProps.viewBox ?? "0 0 24 24"}>
                    {iconProps.paths.map(path => (<path key={path.path} fill={path.fill} d={path.path}/>))}
                </svg>)}
        </Tooltip>);
}
export function FriendIcon() {
    return (<Icon tooltip={"Friend"} paths={[
...
export function BlockedIcon() {
    return (<Icon tooltip={"Blocked"} paths={[
