...
function Tag({ children, color }) {
    return (<span style={{ background: color, color: "#fff", borderRadius: 3, padding: "1px 6px", fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>
            {children}
        </span>);
}
function Field({ label, value }) {
    return (<div style={{ display: "flex", gap: 8, fontSize: 13, marginBottom: 2 }}>
...
function SanctionCard({ s }) {
    return (<div style={{ borderLeft: "3px solid var(--status-danger)", background: "var(--background-secondary)", borderRadius: 4, padding: "8px 12px", marginBottom: 8 }}>
...
function BreachCard({ b }) {
    return (<div style={{ borderLeft: "3px solid var(--text-warning)", background: "var(--background-secondary)", borderRadius: 4, padding: "8px 12px", marginBottom: 6 }}>
...
function SectionTitle({ children }) {
    return (<div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: TEXT_MUTED, borderBottom: "1px solid var(--background-modifier-accent)", paddingBottom: 3, marginBottom: 8, marginTop: 4 }}>
            {children}
        </div>);
}
export function CordCatModal({ data }) {
    ...
    const breachCount = data.breach?.resultsCount ?? breachResults.length ?? 0;
    ...
    return (<div style={{ padding: "4px 2px", color: TEXT_NORMAL }}>

            {banner && (<img src={banner} alt="" style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 4, marginBottom: 10 }}/>)}
            ...
            {breachError ? (<div style={{ color: "var(--status-danger)", fontSize: 13, marginBottom: 8, padding: 8, background: "var(--background-secondary)", borderRadius: 4 }}>
                    Error fetching breach data: {breachError}
                </div>) : uniqueIPs.length > 0 && (<div style={{ marginBottom: 8, fontSize: 13 }}>
