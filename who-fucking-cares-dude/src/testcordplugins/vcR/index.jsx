...
function TokenDisplay({ token, onRemove }) {
    ...
    return (<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 8px", border: "1px solid var(--background-modifier-accent)", borderRadius: "6px", marginBottom: "6px" }}>
...
function TokenManager() {
    ...
    const [tokenEntries, setTokenEntries] = React.useState([]);
    ...
    const addToken = async () => {
        ...
        if (trimmedToken.length === 0 || tokenEntries.includes(trimmedToken))
            return;
    ...
    return (<>
            ...
            <ul style={{ paddingLeft: "0", listStyleType: "none", marginTop: "8px" }}>
                {tokenEntries.map(token => (<li key={token}>
...
async function executeVoiceChannelAction(guildId, channelId, duration, rejoin, delay, randomDelay, amount) {
    if (randomDelay <= 1) {
        if (shouldStop)
            return;
        ...
        await sleep(delay);
        if (rejoin > 0)
            scheduleRejoin(guildId, channelId, duration, rejoin - 1, delay, randomDelay, amount);
    }
}
...
function initializeWebSockets() {
    ...
    currentTokens.forEach((token, index) => {
        ...
        const heartbeatInterval = { id: null };
        const sequenceNumber = { seq: null };
...
function VCRaperModal(props) {
    ...
    return (<ModalRoot {...props} size={"dynamic" /* ModalSize.DYNAMIC */}>
            ...
            color: "#ffffff",
            fontSize: "14px"
        }} value={duration.toString()} placeholder="How long to stay in VC" onChange={value => setDuration(Number(value))}/>
                            <span style={{
            display: "block",
            color: "#64748b",
            fontSize: "11px",
            ...
            color: "#ffffff",
            fontSize: "14px"
        }} value={delay.toString()} placeholder="Initial delay before action" onChange={value => setDelay(Number(value))}/>
                            <span style={{
            display: "block",
            color: "#64748b",
            fontSize: "11px",
            ...
            color: "#ffffff",
            fontSize: "14px"
        }} value={rejoin.toString()} placeholder="Number of rejoins" onChange={value => setRejoin(Number(value))}/>
                            <span style={{
            display: "block",
            color: "#64748b",
            fontSize: "11px",
            ...
            color: "#ffffff",
            fontSize: "14px"
        }} value={randomDelay.toString()} placeholder="Randomization factor" onChange={value => setRandomDelay(Number(value))}/>
                            <span style={{
            display: "block",
            color: "#64748b",
            fontSize: "11px",
            ...
            fontSize: "14px",
            textAlign: "center"
        }} value={amount.toString()} placeholder="Number of bots" onChange={value => setAmount(Number(value))}/>
                        <div style={{
            backgroundColor: "#0f172a",
            padding: "8px 16px",
            borderRadius: "6px",
...
function createVCMenuItem(guildId, id) {
    return (<Menu.MenuItem id="VC-RAPER" label="RAPE VC" color="danger" action={() => {
