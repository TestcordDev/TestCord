...
const Native = IS_DISCORD_DESKTOP
    ? VencordNative.pluginHelpers.PerformanceBoost
    : null;
let active = false;
let ready = false; // Becomes true on CONNECTION_OPEN (or fallback timeout) — ignores startup game reports
let readyFallbackTimer = null; // Fallback timer if CONNECTION_OPEN is not received
...
const NOTICE_COLORS = { success: "#3ba55c", warning: "#faa81a", error: "#ed4245", info: "#5865f2" };
...
let disposePassiveListeners = null;
let springs = [];
...
function clearStoreCaches() {
    ...
    for (const name of CACHE_STORE_NAMES) {
        try {
            ...
            logger.warn(`clearCache ${name} failed`, e);
        }
    }
...
function applyRuntimeOpts() {
    ...
    if (settings.store.skipSpringAnimations && springs.length === 0) {
        springs = findAll(m => typeof m?.Globals === "object" && typeof m?.Springs === "object");
        for (const s of springs)
            s.Globals?.assign?.({ skipAnimation: true });
    }
    ...
    if (settings.store.lazyImages) {
        const isChatImage = (img) => img.closest('[class*="scrollerInner_"], [class*="messageListItem_"]') !== null;
    ...
    try {
        ...
        if (compactSetting?.updateSetting && typeof compactSetting.getSetting === "function") {
            ...
            if (settings.store.compactMode)
                compactSetting.updateSetting(true);
        }
    }
    catch (e) {
        logger.warn("Failed to set compact mode", e);
    }
    try {
        ...
        if (gifSetting?.updateSetting && typeof gifSetting.getSetting === "function") {
            ...
            if (settings.store.disableGifAutoplay)
                gifSetting.updateSetting(false);
        }
    }
    catch (e) {
        logger.warn("Failed to set GIF autoplay", e);
    }
}
async function revertUserSettings() {
    try {
        const originalCompact = await DataStore.get(ORIG_COMPACT_KEY);
        if (originalCompact !== undefined) {
            const compactSetting = getUserSettingLazy("textAndImages", "messageDisplayCompact");
            if (compactSetting?.updateSetting)
                await compactSetting.updateSetting(originalCompact);
            await DataStore.del(ORIG_COMPACT_KEY);
        }
    }
    catch (e) {
        logger.warn("Failed to revert compact mode", e);
    }
    try {
        const originalGif = await DataStore.get(ORIG_GIF_KEY);
        if (originalGif !== undefined) {
            const gifSetting = getUserSettingLazy("textAndImages", "gifAutoPlay");
            if (gifSetting?.updateSetting)
                await gifSetting.updateSetting(originalGif);
            await DataStore.del(ORIG_GIF_KEY);
        }
    }
    catch (e) {
        logger.warn("Failed to revert GIF autoplay", e);
    }
}
// ── Process Priority & Cache ──
async function setPriority(level) {
    if (!Native) {
        notice(t("تغيير الأولوية يتطلب نسخة سطح المكتب.", "Changing priority requires the desktop app."), "warning");
        return;
    }
    try {
        const res = await Native.setProcessPriority(level);
        if (res.ok && level === "belowNormal")
            notice(t(`تم خفض أولوية ${res.changed} عملية`, `Lowered priority for ${res.changed} process(es)`), "success");
        else if (!res.ok)
            notice(t("تغيير الأولوية غير متاح: " + res.reason, "Priority change unavailable: " + res.reason), "warning");
    }
    catch (e) {
        logger.error("setPriority failed", e);
    }
}
async function cleanCache() {
    if (!Native) {
        notice(t("تنظيف الكاش يتطلب نسخة سطح المكتب.", "Cache cleaning requires the desktop app."), "warning");
        return;
    }
    try {
        ...
        logger.error("cleanCache failed", e);
    }
}
...
async function promptHardwareRestart() {
    if (await DataStore.get(HW_ACK_KEY))
        return;
    await DataStore.set(HW_ACK_KEY, true);
    showNotice(t("لتطبيق تعطيل تسريع العتاد: عطّله يدوياً من إعدادات Discord ← متقدّم، ثم أعد التشغيل.", "To disable hardware acceleration: turn it off manually in Discord Settings → Advanced, then restart."), t("أعد التشغيل الآن", "Restart now"), doRestart);
}
...
async function sampleLoad() {
    if (!Native || !settings.store.autoHighLoad)
        return;
    try {
...
async function applyMode() {
    ...
    if (settings.store.changeProcessPriority)
        await setPriority("belowNormal");
    if (settings.store.cleanCacheOnStart)
        await cleanCache();
    if (settings.store.reduceHardwareAcceleration)
        await promptHardwareRestart();
    refreshButtons();
...
// ── Bolt Icon ──
function BoltIcon({ active: isActive }) {
    const color = isActive ? "#3ba55c" : "#ed4245"; // Green when enabled, red when disabled
    return (<svg width={20} height={20} viewBox="0 0 24 24" fill={color} stroke={color} strokeWidth="1">
...
function PerfHeaderButton() {
    ...
    return (<HeaderBarButton icon={() => <BoltIcon active={active}/>} tooltip={active ? t("إيقاف وضع الأداء", "Disable performance mode") : t("تفعيل وضع الأداء", "Enable performance mode")} onClick={toggle}/>);
}
export default definePlugin({
    ...
    flux: {
        ...
        RUNNING_GAMES_CHANGE({ games }) {
            ...
            if (!settings.store.autoDetectGames || !ready)
                return;
    ...
    async start() {
        ...
        manualOff = (await DataStore.get(MANUAL_OFF_KEY)) ?? false;
        if (settings.store.gameMode)
            await applyMode();
        else
            await revertUserSettings(); // Ensure settings revert if disabled
