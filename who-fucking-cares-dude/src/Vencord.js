...
async function syncSettings() {
    ...
    if (Settings.cloud.settingsSync && // if it's enabled
...
async function init() {
    await onceReady;
    startAllPlugins("WebpackReady" /* StartAt.WebpackReady */);
    syncSettings();
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn("Webpack has finished initialising, but some patches haven't been applied yet.", "This might be expected since some Modules are lazy loaded, but please verify", "that all plugins are working as intended.", "You are seeing this warning because this is a Development build of TestCord.", "\nThe following patches have not been applied:", "\n\n" + pendingPatches.map(p => `${p.plugin}: ${p.find}`).join("\n"));
    }
...
if (!IS_DEV) {
    window.onerror = (message, source, lineno, colno, error) => {
        ...
        if (msg.includes("startsWith") || msg.includes("Cannot read properties"))
            return true;
    };
    window.addEventListener("unhandledrejection", e => {
        const { reason } = e;
        if (reason?.message?.includes?.("startsWith"))
            e.preventDefault();
    });
}
function initNotificationSafeguard() {
    if (typeof window === "undefined")
        return;
    if (window.Notification && !window.Notification._safePatched) {
        const OrigNotification = window.Notification;
        const SafeNotification = function (title, options) {
    ...
    const patchDiscordNative = () => {
        try {
            ...
            if (dn?.notifications?.sendNotification && !dn.notifications._safePatched) {
                ...
                dn.notifications.sendNotification = function (opts, ...rest) {
...
initStyles();
startAllPlugins("Init" /* StartAt.Init */);
init();
window.addEventListener("keydown", e => {
    ...
    if ((isCtrl && e.shiftKey && key === "i") || (isCtrl && e.altKey && key === "i") || key === "f12") {
        try {
            const winApi = window.DiscordNative?.window || window.VencordNative?.window;
...
document.addEventListener("DOMContentLoaded", () => {
    startAllPlugins("DOMContentLoaded" /* StartAt.DOMContentLoaded */);
    // FIXME
