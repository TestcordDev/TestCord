...
function parseIPv6(addr) {
    ...
    if (s.includes("::")) {
        ...
        if (missing < 0)
            return null;
        s = [...headGroups, ...Array(missing).fill("0"), ...tailGroups].join(":");
    }
...
function isPrivateAddress(address) {
    ...
    if (asV4 !== null)
        return privateV4.some(inRange => inRange(asV4));
    const asV6 = parseIPv6(normalized);
    if (asV6 !== null)
        return privateV6.some(inRange => inRange(asV6));
    return false;
}
export async function assertSafeUrl(rawUrl, options = {}) {
    ...
    if (options.allowedHosts && !options.allowedHosts.some(host => url.hostname === host || url.hostname.endsWith(`.${host}`))) {
...
export async function readCappedText(response, maxBytes = 8 * 1024 * 1024) {
