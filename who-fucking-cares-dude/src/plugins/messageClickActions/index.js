...
const doubleClickOthersActions = [
    { label: "None", value: "NONE" },
    { label: "Delete", value: "DELETE" },
    { label: "Reply", value: "REPLY" },
    { label: "Quote", value: "QUOTE" },
...
const keydown = (e) => {
    ...
    if (e.key === "Backspace")
        pressedModifiers.add("BACKSPACE");
    if (e.key === "Delete")
        pressedModifiers.add("DELETE");
};
const keyup = (e) => {
    const mod = modifierFromKey(e);
    if (mod)
    ...
    if (e.key === "Backspace")
        pressedModifiers.delete("BACKSPACE");
    if (e.key === "Delete")
        pressedModifiers.delete("DELETE");
};
const focusChanged = () => {
...
let pendingDoubleClickAction = null;
...
function getConfiguredReactionEmojis() {
    ...
    return Array.from(new Set(configured.filter((emoji) => Boolean(emoji))));
}
const canSend = (channel) => !channel.guild_id || PermissionStore.can(PermissionsBits.SEND_MESSAGES, channel);
const canDelete = (msg, channel) => {
    const myId = AuthenticationStore.getId();
    return msg.author.id === myId ||
        PermissionStore.can(PermissionsBits.MANAGE_MESSAGES, channel) ||
        msg.interactionMetadata?.authorizing_integration_owners?.[1 /* ApplicationIntegrationType.USER_INSTALL */] === myId;
};
const canReply = (msg) => MessageTypeSets.REPLYABLE.has(msg.type) && !msg.hasFlag(64 /* MessageFlags.EPHEMERAL */);
async function toggleReaction(channelId, messageId, emoji, channel, msg) {
...
async function addReaction(channelId, messageId, emoji, channel) {
    const emojiParam = normalizeEmoji(emoji);
    if (!emojiParam)
        return;
    if (channel.guild_id && (!PermissionStore.can(PermissionsBits.ADD_REACTIONS, channel) || !PermissionStore.can(PermissionsBits.READ_MESSAGE_HISTORY, channel))) {
...
async function reactWithConfiguredEmojis(channel, msg) {
...
function quoteMessage(channel, msg) {
    if (!canReply(msg)) {
...
function openInNewTab(msg, channel) {
    VencordNative.native.openExternal(getMessageLink(msg, channel));
}
function openInThread(msg, channel) {
    FluxDispatcher.dispatch({
...
async function executeAction(action, msg, channel, event, triggerModifier = "NONE") {
    const myId = AuthenticationStore.getId();
    ...
    switch (action) {
        ...
        case "EDIT":
            if (!isMe)
                return;
            if (EditMessageStore.isEditing(channel.id, msg.id) || msg.state !== "SENT")
                return;
...
export default definePlugin({
    ...
    onMessageClick(msg, channel, event) {
        ...
        if ((settings.store.disableInDms && isDM) || (settings.store.disableInSystemDms && isSystemDM))
            return;
        const singleClickAction = isMe
            ? settings.store.singleClickAction
            : settings.store.singleClickOthersAction;
        const doubleClickAction = isMe
            ? settings.store.doubleClickAction
            : settings.store.doubleClickOthersAction;
        const tripleClickAction = settings.store.tripleClickAction;
        const singleClickModifier = isMe
            ? settings.store.singleClickModifier
            : settings.store.singleClickOthersModifier;
        const doubleClickModifier = settings.store.doubleClickModifier;
        const tripleClickModifier = settings.store.tripleClickModifier;
        ...
        const canTripleClick = settings.store.deferDoubleClickForTriple &&
            isModifierPressed(tripleClickModifier) &&
            tripleClickAction !== "NONE";
        const shouldDeferDoubleClick = canDoubleClick &&
            canTripleClick &&
        ...
        if (isSingleClick) {
            ...
            const canDoubleClickWithCurrentModifier = doubleClickAction !== "NONE" &&
