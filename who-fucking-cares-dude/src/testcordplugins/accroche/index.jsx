...
const settings = definePluginSettings({
    ...
    enableAnchor: {
        type: 3 /* OptionType.BOOLEAN */,
        description: "Enable anchoring feature (automatically return to the anchored person's channel)",
        default: true,
    },
    anchorDelay: {
        type: 1 /* OptionType.NUMBER */,
...
// Global variables
let hookedUserInfo = null;
let anchoredUserInfo = null;
let originalSelectVoiceChannel = null;
const isPreventingMove = false;
let anchorMonitoringInterval = null;
...
async function moveUserToVoiceChannel(userId, channelId) {
    ...
    try {
        ...
        if (settings.store.showNotifications) {
            ...
            showNotification({
                title: "🔗 Hook - Success",
                body: `${user?.username || "User"} has been brought back to your voice channel`,
            });
        }
    }
    catch (error) {
...
async function hookUser(userId, username) {
    verboseLog(`🚀 Starting hookUser function for ${username} (${userId})`);
    const currentUser = UserStore.getCurrentUser();
    if (!currentUser) {
        log("❌ Current user not available", "error");
        return;
    }
    verboseLog(`✅ Current user found: ${currentUser.username} (${currentUser.id})`);
    ...
    verboseLog(`🔍 Initial voice state - User: ${userVoiceState?.channelId || "null"}, You: ${currentVoiceState?.channelId || "null"}`);
    ...
    if (!userVoiceState?.channelId || !currentVoiceState?.channelId) {
        ...
        verboseLog(`🔍 Voice state after wait - User: ${userVoiceState?.channelId || "null"}, You: ${currentVoiceState?.channelId || "null"}`);
    }
...
async function anchorUser(userId, username) {
    verboseLog(`🚀 Starting anchorUser function for ${username} (${userId})`);
    const currentUser = UserStore.getCurrentUser();
    if (!currentUser) {
        log("❌ Current user not available", "error");
        return;
    ...
    let userVoiceState = VoiceStateStore.getVoiceStateForUser(userId);
    let currentVoiceState = VoiceStateStore.getVoiceStateForUser(currentUserId);
    verboseLog(`🔍 Initial voice state (anchoring) - User: ${userVoiceState?.channelId || "null"}, You: ${currentVoiceState?.channelId || "null"}`);
    // If voice state is not immediately available, wait a bit
    if (!userVoiceState?.channelId || !currentVoiceState?.channelId) {
        verboseLog("⏳ Voice state not immediately available for anchoring, waiting 500ms...");
        await sleep(500);
        userVoiceState = VoiceStateStore.getVoiceStateForUser(userId);
        currentVoiceState = VoiceStateStore.getVoiceStateForUser(currentUserId);
        verboseLog(`🔍 Voice state after wait (anchoring) - User: ${userVoiceState?.channelId || "null"}, You: ${currentVoiceState?.channelId || "null"}`);
    }
    if (!userVoiceState?.channelId) {
        log(`❌ User ${username} is not in a voice channel`, "warn");
        if (settings.store.anchorNotifications) {
...
function startAnchorMonitoring() {
    ...
    anchorMonitoringInterval = setInterval(() => {
        ...
        if (!myVoiceState?.channelId || !anchoredUserVoiceState?.channelId) {
            verboseLog(`🔍 Anchor monitoring: One of the users is not in a voice channel - You: ${myVoiceState?.channelId || "null"}, Anchored: ${anchoredUserVoiceState?.channelId || "null"}`);
            return;
        }
        // Periodic log to check state
        if (Math.random() < 0.1) {
            // 10% chance on each check
            verboseLog(`🔍 Anchor monitoring: You: ${myVoiceState.channelId}, ${anchoredUserInfo.username}: ${anchoredUserVoiceState.channelId}`);
        }
        ...
        if (myVoiceState.channelId !== anchoredUserVoiceState.channelId) {
            ...
            log(`⚠️ Monitoring: You have been moved, automatic return to ${anchoredUserInfo.username}'s channel`);
...
async function moveCurrentUserToVoiceChannel(channelId) {
    ...
    try {
        console.log(`🔄 Attempting to move ${currentUser.username} to channel ${channelId}`);
...
const UserContextMenuPatch = (children, { user }) => {
    ...
    if (!settings.store.enabled || !user) {
        verboseLog(`❌ Plugin disabled or user missing - enabled: ${settings.store.enabled}, user: ${!!user}`);
        return;
    }
    const currentUser = UserStore.getCurrentUser();
    if (!currentUser || user.id === currentUser.id) {
        verboseLog(`❌ Current user missing or same user - currentUser: ${!!currentUser}, sameUser: ${user.id === currentUser?.id}`);
        return;
    }
    ...
    children.push(React.createElement(Menu.MenuSeparator, {}), React.createElement(Menu.MenuItem, {
        id: "hook-user",
...
export default definePlugin({
    name: "Hook",
    description: "Hooks a user to prevent them from changing voice channels or anchors to a user to automatically return to their channel",
    tags: ["Voice", "Utility"],
    authors: [
        {
            name: "Bash",
    ...
    flux: {
        async VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            if (!currentVoiceState?.channelId) {
                verboseLog("🔇 You are not in a voice channel, hooking/anchoring suspended");
                return;
            }
            // Anchoring logic (automatically return to the anchored person's channel)
            if (anchoredUserInfo) {
                console.log("🔍🔍🔍 ANCHORING ACTIVE - Checking channel changes 🔍🔍🔍");
                verboseLog(`⚓ Anchoring active for ${anchoredUserInfo.username} (${anchoredUserInfo.userId})`);
                for (const voiceState of voiceStates) {
                    ...
                    if (userId === currentUserId &&
                        ...
                        if (channelId !== anchoredUserVoiceState.channelId) {
                            log(`⚠️ You have been moved, automatic return to ${anchoredUserInfo.username}'s channel`);
                            ...
                            setTimeout(async () => {
                                try {
                                    ...
                                    if (!anchoredUserInfo || !currentAnchoredState?.channelId) {
                                        verboseLog("🔍 User no longer anchored or anchored person no longer in a voice channel");
                                        return;
                                    }
                                    ...
                                    log(`❌ Error during return to ${anchoredUserInfo.username}: ${error}`, "error");
                                    if (settings.store.anchorNotifications) {
                                        showNotification({
                                            title: "⚓ Anchor - Error",
            ...
            for (const voiceState of voiceStates) {
                ...
                if (userId === hookedUserInfo.userId &&
                    channelId !== hookedUserInfo.lastChannelId) {
                    verboseLog(`🔄 Channel change detected for ${hookedUserInfo.username}: ${oldChannelId} -> ${channelId}`);
                    ...
                    if (channelId !== currentVoiceState.channelId) {
                        log(`⚠️ ${hookedUserInfo.username} changed channels, attempting to bring back to your channel`);
                        // Wait for a delay before bringing the user back
                        setTimeout(async () => {
                            try {
                                // Check that the user is still hooked and in a different channel
                                const currentHookedState = VoiceStateStore.getVoiceStateForUser(hookedUserInfo.userId);
                                const myCurrentState = VoiceStateStore.getVoiceStateForUser(currentUserId);
                                if (!hookedUserInfo || !myCurrentState?.channelId) {
                                    verboseLog("🔍 User no longer hooked or you are no longer in a voice channel");
                                    return;
                                }
                                if (currentHookedState?.channelId === myCurrentState.channelId) {
                                    verboseLog("✅ User is already in your channel");
                                    return;
                                }
                                // Bring the user back to your channel
                                await moveUserToVoiceChannel(hookedUserInfo.userId, myCurrentState.channelId);
                            }
                            catch (error) {
                                log(`❌ Error during move of ${hookedUserInfo.username}: ${error}`, "error");
                                if (settings.store.showNotifications) {
                                    showNotification({
                                        title: "🔗 Hook - Error",
                                        body: `Unable to bring ${hookedUserInfo.username} back to your channel`,
                                    });
                                }
                            }
                ...
                if (userId === currentUserId &&
                    channelId !== currentVoiceState.channelId) {
                    verboseLog(`🔄 You changed channels: ${currentVoiceState.channelId} -> ${channelId}`);
                    // If we have a hooked user and we join a new channel
                    if (channelId && hookedUserInfo) {
                        const hookedUserVoiceState = VoiceStateStore.getVoiceStateForUser(hookedUserInfo.userId);
                        // If the hooked user is in a different voice channel
                        if (hookedUserVoiceState?.channelId &&
                            hookedUserVoiceState.channelId !== channelId) {
                            log(`🔄 You changed channels, moving ${hookedUserInfo.username} to your new channel`);
                            setTimeout(async () => {
                                try {
                                    await moveUserToVoiceChannel(hookedUserInfo.userId, channelId);
                                }
                                catch (error) {
                                    log(`❌ Error during move of ${hookedUserInfo.username}: ${error}`, "error");
                                }
...
- Anchor notifications: ${settings.store.anchorNotifications ? "ON" : "OFF"}`);
        ...
        if (settings.store.preventSelfMove && ChannelActions) {
            ...
            ChannelActions.selectVoiceChannel = function (channelId) {
                if (hookedUserInfo && !isPreventingMove) {
                    const currentUser = UserStore.getCurrentUser();
                    if (currentUser && hookedUserInfo.userId === currentUser.id) {
                        log(`🚫 Manual move attempt blocked for ${hookedUserInfo.username}`);
                        if (settings.store.showNotifications) {
                            showNotification({
                                title: "🔗 Hook - Blocked",
