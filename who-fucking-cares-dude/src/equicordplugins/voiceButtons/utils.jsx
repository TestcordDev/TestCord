...
const DeafenIconSelf = findComponentByCodeLazy("M22.7 2.7a1", "1.4l20-20ZM17");
const DeafenIconOther = findComponentByCodeLazy("M21.76.83a5.02", "M12.38");
const ChatIcon = findComponentByCodeLazy(".css,d:\"M12 22a10");
const MuteIconSelf = findComponentByCodeLazy("d:\"m2.7 22.7 20-20a1", "1.4ZM10.8");
const MuteIconOther = findComponentByCodeLazy("M21.76.83a5.02", "M12 2c.33 0");
function VoiceUserButton({ user, tooltip, icon, onClick }) {
    const isCurrent = (user.id === UserStore.getCurrentUser().id);
    if (isCurrent && settings.store.showButtonsSelf === "hide")
        return null;
    const disabled = isCurrent && settings.store.showButtonsSelf === "disable";
    return (<div className="voice-user-button-container">
            <Tooltip text={tooltip} shouldShow={!disabled}>
                {({ onMouseEnter, onMouseLeave }) => (<Button size={Button.Sizes.MIN} color={Button.Colors.TRANSPARENT} look={Button.Looks.FILLED} disabled={disabled} onClick={e => {
                e.preventDefault();
                e.stopPropagation();
                onClick();
...
function canServerMuteDeafen(userId) {
    if (!settings.store.useServer)
        return { canMute: false, canDeafen: false };
...
function getServerMuteDeafenState(userId) {
    const voiceState = VoiceStateStore.getVoiceStateForUser(userId);
    if (!voiceState)
        return { isServerMuted: false, isServerDeafened: false };
    return {
        isServerMuted: voiceState.mute ?? false,
        isServerDeafened: voiceState.deaf ?? false
    };
}
export function UserChatButton({ user }) {
    const isCurrent = (user.id === UserStore.getCurrentUser().id);
    return (<VoiceUserButton user={user} tooltip={isCurrent ? "Navigate to DMs" : `Open DMs with ${getUserName(user)}`} icon={<ChatIcon size="sm"/>} onClick={() => {
            if (isCurrent) {
...
export function UserMuteButton({ user }) {
    const isCurrent = (user.id === UserStore.getCurrentUser().id);
    ...
    return (<VoiceUserButton user={user} tooltip={`${tooltipAction} ${isCurrent ? "yourself" : `${getUserName(user)}`}`} icon={isCurrent ? <MuteIconSelf muted={isMuted} size="sm" color={color}/> : <MuteIconOther muted={isMuted} size="sm" color={color}/>} onClick={() => {
            if (canServerMute) {
...
export function UserDeafenButton({ user }) {
    const isCurrent = (user.id === UserStore.getCurrentUser().id);
    ...
    return (<VoiceUserButton user={user} tooltip={`${tooltipAction} ${isCurrent ? "yourself" : `${getUserName(user)}`}`} icon={isCurrent ? <DeafenIconSelf muted={isDeafened} size="sm" color={color}/> : <DeafenIconOther muted={isDeafened} size="sm" color={color}/>} onClick={() => {
            if (canServerDeafen) {
