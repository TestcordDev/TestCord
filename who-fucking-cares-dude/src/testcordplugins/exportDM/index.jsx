...
function ExportIcon({ width = 20, height = 20 }) {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24">
...
async function fetchAllMessages(channelId, token, onProgress) {
            ...
            for (const m of raw) {
                if (m && (m.deleted || m.state === "DELETED" || m.mlDeleted)) {
                    messageMap.set(m.id, {
                        ...
                        stickers: (m.sticker_items ?? m.stickers ?? []).map((s) => ({ name: s.name, id: s.id })),
                        reactions: (m.reactions ?? []).map((r) => ({ emoji: r.emoji?.name ?? r.emoji?.id, count: r.count })),
    ...
    try {
        ...
        for (const record of idbRecords) {
            ...
            messageMap.set(record.message_id, {
                ...
                stickers: (m.sticker_items ?? m.stickerItems ?? []).map((s) => ({ name: s.name, id: s.id })),
                reactions: (m.reactions ?? []).map((r) => ({ emoji: r.emoji?.name ?? r.emoji?.id, count: r.count })),
                referencedMessage: m.referenced_message ? {
...
function buildTxt(messages, channelName) {
    ...
    for (const m of messages) {
        ...
        for (const e of m.embeds) {
            if (e.url)
                lines.push(`  [LIEN] ${e.title ?? "Embed"}: ${e.url}`);
        }
...
function buildJson(messages, channelName) {
...
function buildMd(messages, channelName) {
...
function buildHtml(messages, channelName) {
    const rows = messages.map(m => {
        ...
        const mediaHtml = m.attachments.map(a => {
            ...
            if (t_type === "image")
                return `<div class="media"><img src="${a.url}" alt="${a.filename}" loading="lazy"><div class="media-name">${a.filename} (${formatSize(a.size)})</div></div>`;
            if (t_type === "video")
                return `<div class="media"><video src="${a.url}" controls preload="none"></video><div class="media-name">${a.filename}</div></div>`;
            if (t_type === "audio")
                return `<div class="media"><audio src="${a.url}" controls></audio><div class="media-name">${a.filename}</div></div>`;
        ...
        const embedHtml = m.embeds.map(e => {
            ...
            if (e.title)
                html += `<div class="embed-title">${e.title.replace(/</g, "&lt;")}</div>`;
            if (e.description)
                html += `<div class="embed-desc">${e.description.slice(0, 300).replace(/</g, "&lt;")}</div>`;
            if (e.image)
                html += `<img src="${e.image}" class="embed-img" loading="lazy">`;
            if (e.url)
                html += `<a href="${e.url}" target="_blank" class="embed-url">${e.url}</a>`;
            return html + "</div>";
        }).join("");
...
// ── SVG icon for search ───────────────────────────────────────────────
function SearchIcon() {
    return (<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ opacity: 0.5, flexShrink: 0 }}>
...
// ── Modal ─────────────────────────────────────────────────────────────────
function ExportDMModal({ rootProps }) {
    ...
    async function doExport() {
        ...
        for (let i = 0; i < selectedChannels.length; i++) {
            ...
            switch (format) {
                case "json":
                    content = buildJson(msgs, ch.name);
                    ext = "json";
                    mime = "application/json";
                    break;
                case "csv":
                    content = buildCsv(msgs);
                    ext = "csv";
                    mime = "text/csv";
                    break;
                case "md":
                    content = buildMd(msgs, ch.name);
                    ext = "md";
                    mime = "text/markdown";
                    break;
                case "txt":
                    content = buildTxt(msgs, ch.name);
                    ext = "txt";
                    mime = "text/plain";
                    break;
                default:
                    content = buildHtml(msgs, ch.name);
                    ext = "html";
                    mime = "text/html";
                    break;
            }
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="edm-content">
                ...
                <div className="edm-search-bar">
                    <SearchIcon />
                    <input className="edm-search-input" type="text" placeholder={t("Search for a friend or a DM...")} value={search} onChange={e => setSearch(e.currentTarget.value)} autoFocus/>
                ...
                <div className="edm-channel-list">
                    {filtered.length === 0 && (<div className="edm-empty">
                            {search ? `${t("No results for")} "${search}"` : t("No DMs found")}
                        </div>)}
                    {filtered.map(c => {
            const av = getChAvatar(c);
            const isSel = selected.has(c.id);
            return (<div key={c.id} className={`edm-channel-row ${isSel ? "edm-channel-row--selected" : ""}`} onClick={() => toggleSelected(c.id)}>
                                {av
                ...
                <div className="edm-format-row">
                    {FORMATS.map(f => (<button key={f.key} className={`edm-format-btn ${format === f.key ? "edm-format-btn--active" : ""}`} onClick={() => setFormat(f.key)} title={f.desc}>
...
function ExportButton() {
    return (<HeaderBarButton icon={ExportIcon} tooltip={t("Export DMs")} onClick={() => openModal(props => <ExportDMModal rootProps={props}/>)}/>);
}
