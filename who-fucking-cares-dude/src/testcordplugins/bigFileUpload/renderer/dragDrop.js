...
let uploadBufferFn = null;
// Nitro limit checker function - injected by index.tsx
let shouldUseNativeUploadFn = null;
...
export function setNitroLimitChecker(fn) {
    ...
    for (const file of files) {
        try {
            if (file.size > DRAG_DROP_MAX_SIZE) {
                showToast(`'${file.name}' is too large (${formatFileSize(file.size)}). Drag-and-drop limit is ${formatFileSize(DRAG_DROP_MAX_SIZE)}. Use the Upload button for larger files.`, Toasts.Type.FAILURE);
                continue;
            }
