...
function Icon(props) {
    ...
    return (<div {...restProps} className={classes(cl("speaker"), className)}>
...
function LockedSpeakerIcon(props) {
    return (<Icon {...props}>
...
function VoiceChannelTooltip({ channel, isLocked }) {
    const voiceStates = useStateFromStores([VoiceStateStore], () => VoiceStateStore.getVoiceStatesForChannel(channel.id));
    const users = useMemo(() => Object.values(voiceStates).map(voiceState => UserStore.getUser(voiceState.userId)).filter(user => user != null), [voiceStates]);
    ...
    return (<>
            ...
            <div className={cl("vc-members")}>
                <Icon size={18}/>
                <UserSummaryItem users={users} renderIcon={false} max={13} size={18}/>
            </div>
        </>);
}
const clickTimers = new Map();
export const VoiceChannelIndicator = ErrorBoundary.wrap(({ userId, isProfile, isActionButton, shouldHighlight }) => {
    ...
    if (!isDM && !PermissionStore.can(PermissionsBits.VIEW_CHANNEL, channel) && !isPluginEnabled(ShowHiddenChannelsPlugin.name))
        return null;
    ...
    const IconComponent = isLocked
        ...
        : isDeaf
            ...
            : isMuted
                ? MutedIcon
                : SpeakerIcon;
    return (<Tooltip text={<VoiceChannelTooltip channel={channel} isLocked={isLocked}/>} tooltipClassName={cl("tooltip-container")} tooltipContentClassName={cl("tooltip-content")}>
            {props => (<IconComponent {...props} role="button" onClick={onClick} className={classes(cl("clickable"), isActionButton && ActionButtonClasses.actionButton, isActionButton && shouldHighlight && ActionButtonClasses.highlight, cl(isProfile && "profile-speaker"))} size={isActionButton ? 20 : 16}/>)}
        </Tooltip>);
}, { noop: true });
