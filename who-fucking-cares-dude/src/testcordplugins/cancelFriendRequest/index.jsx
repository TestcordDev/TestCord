...
function getUserIdFromOutgoingRelationships() {
    try {
        const rels = RelationshipStore.getRelationships?.() ?? {};
...
let observer = null;
const patchedButtons = new Set();
function patchBtn(btn, userId) {
    if (btn.dataset.cfp)
        return;
    btn.dataset.cfp = "1";
    const handler = (e) => {
...
function scan(root = document) {
    ...
    root.querySelectorAll('button[aria-label="Outgoing Friend Request"]').forEach(btn => {
    ...
    root.querySelectorAll('button[disabled][class*="secondary"]').forEach(btn => {
        ...
        if (avatarImg) {
            const m = avatarImg.getAttribute("src")?.match(/avatars\/(\d+)\//);
            if (m) {
                const relType = RelationshipStore.getRelationshipType(m[1]);
                if (relType === OUTGOING_REQUEST) {
                    patchBtn(btn, m[1]);
                    return;
                }
            }
        ...
        const uid = getUserIdFromOutgoingRelationships();
        if (uid) {
            const relType = RelationshipStore.getRelationshipType(uid);
            if (relType === OUTGOING_REQUEST)
                patchBtn(btn, uid);
        }
    });
