...
const knownNicks = new Map();
async function resetNick(guildId, forcedNick, currentUserId) {
    if (resettingGuilds.has(guildId))
        return;
...
export default definePlugin({
    ...
    flux: {
        GUILD_MEMBER_UPDATE({ guildId, user, nick }) {
            ...
            if (prevNick !== undefined && prevNick === currentNick)
                return;
