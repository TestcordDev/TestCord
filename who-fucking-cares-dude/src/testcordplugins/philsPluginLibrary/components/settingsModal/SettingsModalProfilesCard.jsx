...
export const SettingsModalProfilesCard = (props) => {
    const { profileableStore: { use } } = props;
    const { currentProfile, setCurrentProfile, deleteProfile, getCurrentProfile, getDefaultProfiles, getProfile, getProfiles, saveProfile, isCurrentProfileADefaultProfile } = use();
    ...
    return (<SettingsModalCard title="Profile" {...props}>
            <SettingsModalCardItem>
                <Flex style={{ alignItems: "center" }}>
                    <div style={{ flex: 1 }}>
                        {isSaving
            ? <TextInput style={{ width: "100%" }} placeholder="Insert name" value={profileNameInput} onChange={setProfileNameInput}/>
            : <Select isSelected={value => name === value} options={getProfiles(true).map(profile => ({
                    label: profile.name,
                    value: profile.name
                }))} select={value => setCurrentProfile(getProfile(value) || { name: "" })} serialize={() => ""}/>}
                    </div>
