...
const EMPTY_META = { names: "", url: "", durationMs: 0 };
const cache = new Map();
const lookups = new Set();
export function trackMeta(trackId) {
    return cache.get(trackId);
}
export function isTrackResolved(trackId) {
...
function wantedIds(ynisonState) {
    ...
    for (const offset of [0, 1, -1]) {
        ...
        if (!id || wanted.includes(id) || cache.has(id) || lookups.has(id))
            continue;
        wanted.push(id);
    }
    return wanted;
}
export function resolveTracks(ynisonState, onResolved) {
    if (!state.token)
        return;
    ...
    void fetchTracks(wanted)
        .then(resolved => {
        for (const id of wanted)
            lruSet(cache, id, resolved.get(id) ?? EMPTY_META, MAX_ARTIST_CACHE_ENTRIES);
    })
        .catch(error => {
        for (const id of wanted)
            lruSet(cache, id, EMPTY_META, MAX_ARTIST_CACHE_ENTRIES);
