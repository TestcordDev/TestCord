...
import { SettingsTabsKey } from "@equicordplugins/moreStickers/types";
...
const StickerPackMetadata = ({ meta, hoveredStickerPackId, setHoveredStickerPackId, refreshStickerPackMetas }) => {
    return (<div className="sticker-pack" onMouseEnter={() => setHoveredStickerPackId(meta.id)} onMouseLeave={() => setHoveredStickerPackId(null)}>
            <div className={[
            ...
            <button className={hoveredStickerPackId === meta.id ? "show" : ""} onClick={async () => {
            try {
...
export const Packs = () => {
    const [stickerPackMetas, setstickerPackMetas] = React.useState([]);
    const [addStickerUrl, setAddStickerUrl] = React.useState("");
    const [addStickerHtml, setAddStickerHtml] = React.useState("");
    const [tab, setTab] = React.useState(SettingsTabsKey.ADD_STICKER_PACK_URL);
    const [hoveredStickerPackId, setHoveredStickerPackId] = React.useState(null);
    const [_isV1, setV1] = React.useState(false);
    ...
    return (<div className={cl("settings")}>
            <TabBar type="top" look="brand" selectedItem={tab} onItemSelect={setTab} className="tab-bar">
                {Object.values(SettingsTabsKey).map(k => (<TabBar.Item key={k} id={k} className="tab-bar-item">
                            {k}
                        </TabBar.Item>))}
            </TabBar>
            ...
            <div className="section">
                    ...
                    flexGrow: 1
                            ...
                            <CheckedTextInput initialValue={addStickerUrl} onChange={setAddStickerUrl} validate={(v) => {
                    try {
                    ...
                    return "Invalid URL";
                        ...
                        <Button size={Button.Sizes.SMALL} onClick={async (e) => {
                    e.preventDefault();
                    let type = "";
                    try {
            ...
            <div className="section">
                    ...
                    flexGrow: 1
                            ...
                            <TextArea value={addStickerHtml} onChange={setAddStickerHtml} placeholder="Paste HTML here" rows={1}/>
                        </span>
                        <Button size={Button.Sizes.SMALL} onClick={async (e) => {
                    e.preventDefault();
                    let errorMessage = "";
            ...
            gap: "8px"
                    ...
                    {stickerPackMetas.map(meta => (<StickerPackMetadata key={meta.id} meta={meta} hoveredStickerPackId={hoveredStickerPackId} setHoveredStickerPackId={setHoveredStickerPackId} refreshStickerPackMetas={refreshStickerPackMetas}/>))}
                </div>
            </div>
...
export async function getRecentStickers(key = KEY) {
    return (await DataStore.get(key)) ?? [];
}
export async function setRecentStickers(stickers, key = KEY) {
...
export async function addRecentSticker(sticker) {
...
export async function removeRecentStickerByPackId(packId) {
    const stickers = await getRecentStickers();
