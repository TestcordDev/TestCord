...
const customSongDelays = {};
const DATASTORE_KEY = "vc-spotify-custom-song-delays";
// Load saved data immediately
DataStore.get(DATASTORE_KEY).then(saved => {
    if (saved) {
        Object.assign(customSongDelays, saved);
        FluxDispatcher?.dispatch?.({ type: "SPOTIFY_LYRICS_DELAYS_LOADED" });
    }
});
function CustomDelayModal({ modalProps, trackKey, trackName }) {
    const [delay, setDelay] = React.useState(customSongDelays[trackKey] ?? 0);
    ...
    return (<ModalRoot {...modalProps} size="small" class="musicControlsModal">
            <ModalHeader>
            ...
            <ModalContent>
                <Flex flexDirection="column" style={{ padding: "8px 0" }}>
                    ...
                    <Slider minValue={-2500} maxValue={2500} markers={makeRange(-2500, 2500, 250)} stickToMarkers={true} initialValue={delay} onValueChange={handleDelayChange}/>
                </Flex>
            </ModalContent>
            <ModalFooter gap={12}>
...
export function LyricsContextMenu() {
    ...
    return (<Menu.Menu navId="spotify-lyrics-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Spotify Lyrics Menu">

            <Menu.MenuItem key="copy-lyric" id="copy-lyric" label="Copy current lyric" disabled={!currLyric?.text} action={() => copyWithToast(currLyric.text, "Lyric copied!")} icon={CopyIcon}/>

            <Menu.MenuItem key="spotify-custom-lyric-delay" id="spotify-custom-lyric-delay" label="Custom delay for current song" disabled={!trackKey} action={() => {
            ...
            <Menu.MenuItem navId="spotify-lyrics-provider" id="spotify-lyrics-provider" label="Lyrics Provider">
                {[...providers, ...lyricsAlternative].map(provider => <Menu.MenuRadioItem key={`lyrics-provider-${provider}`} id={`switch-provider-${provider.toLowerCase()}`} group="vc-spotify-lyrics-switch-provider" label={`${provider}${lyricsInfo?.lyricsVersions[provider] ? " (saved)" : ""}`} checked={provider === lyricsInfo?.useLyric} disabled={lyricsAlternative.includes(provider) && !hasLyrics} action={() => {
                FluxDispatcher.dispatch({
                    // @ts-ignore
