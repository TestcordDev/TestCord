...
import { abortActiveClipUploads, getClipCreatedAt, getClipTitleFromName, getDefaultClipTitle, getDefaultFileName, getErrorMessage, getParticipantIds, getString, isValidDate, pickClipFile, uploadClipFile } from "./upload";
export function openUploadClipFileModal(channelId, clip) {
    openModal(modalProps => (<UploadClipFileModal modalProps={modalProps} channelId={channelId} clip={clip}/>));
}
function UploadClipFileModal({ modalProps, channelId, clip }) {
    ...
    const notice = createdAt && !isValidDate(createdAt)
        ? { message: "Created at must be a valid date.", type: "critical" }
        : undefined;
    ...
    async function chooseClipFile() {
        ...
        if (appUsers?.length)
            setParticipants(current => current.length ? current : appUsers);
        if (appId)
            setApplicationId(current => current || appId);
    }
    ...
    return (<Modal {...modalProps} title="Upload Clip File" notice={notice} actions={[
            {
                text: "Cancel",
                variant: "secondary",
            ...
            <Flex flexDirection="column" gap={12}>
                <section>
                    ...
                    <Checkbox value={parseMetadata} onChange={(_event, checked) => setParseMetadata(checked)} disabled={uploading} type="row">
