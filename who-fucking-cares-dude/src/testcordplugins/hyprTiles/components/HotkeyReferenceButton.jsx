...
import { actionLabels, getKeybindSettingKey, hotkeySections } from "../utils/keybinds";
const cl = classNameFactory("vc-hyprtiles-");
function KeyboardIcon(props) {
    return (<svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor" {...props}>
...
function HotkeyReferenceButtonInner() {
    ...
    return (<span ref={buttonRef} style={{ display: "inline-flex", alignItems: "center" }}>
            <Popout position="bottom" align="right" spacing={8} animation={Popout.Animation.NONE} shouldShow={isOpen} onRequestClose={() => setIsOpen(false)} targetElementRef={buttonRef} renderPopout={() => (<ErrorBoundary noop>
                        <HotkeyPopoutContent />
                    </ErrorBoundary>)}>
                {(_, { isShown }) => (<HeaderBarButton icon={KeyboardIcon} tooltip={isShown ? null : "HyprTiles Keybinds"} selected={isShown} onClick={() => setIsOpen(v => !v)}/>)}
            </Popout>
        </span>);
}
