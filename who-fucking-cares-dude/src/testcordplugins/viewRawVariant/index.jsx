...
function openViewRawModal(obj, type, isMessage) {
    openModal(props => (<ErrorBoundary>
            <Modal {...props} size="lg" title={`View Raw ${type}`}>
...
function makeContextCallback(name, action) {
    return (children, props) => {
        ...
        children.push(<Menu.MenuItem id={`c98-view-${name}-raw`} label="View Raw Variant" action={() => action(value)} icon={CopyIcon}/>);
    };
}
