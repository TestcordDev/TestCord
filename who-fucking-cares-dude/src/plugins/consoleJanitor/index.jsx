...
import definePlugin, { defineDefault } from "@utils/types";
...
function AllowLevelSetting({ settingKey }) {
    const { allowLevel } = settings.use(["allowLevel"]);
    const value = allowLevel[settingKey];
    return (<Checkbox value={value} onChange={(_, newValue) => settings.store.allowLevel[settingKey] = newValue} size={20}>
            <BaseText size="sm">{settingKey[0].toUpperCase() + settingKey.slice(1)}</BaseText>
        </Checkbox>);
}
const AllowLevelSettings = ErrorBoundary.wrap(() => {
    return (<SettingsSection name="Filter List" id="filterList" description="Always allow loggers of these types">
            <div style={{ display: "flex", flexDirection: "row" }}>
                {Object.keys(settings.store.allowLevel).map(key => (<AllowLevelSetting key={key} settingKey={key}/>))}
            </div>
        </SettingsSection>);
});
