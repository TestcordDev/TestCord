...
function findGroupChildrenByChildIdImpl(id, children, matchSubstring = false) {
...
function getPatchesForNavId(navId) {
    ...
    if (navId === "message-actions" || navId === "message") {
        ...
        if (messagePatches && messageActionPatches) {
            ...
            for (const p of messageActionPatches)
                combined.add(p);
            return combined;
        }
...
function applyAllPatches(navId, sourceChildren, args) {
        ...
        if (contextMenuPatches?.size) {
            ...
            for (const patch of contextMenuPatches) {
                ...
                if (timed) {
                    ...
                    if (dt >= SLOW_PATCH_MS)
                        slowPatches.push(`nav#${i}=${dt.toFixed(1)}ms`);
                }
                i++;
            }
        }
        if (globalPatches.size) {
            ...
            for (const patch of globalPatches) {
                ...
                if (timed) {
                    const dt = performance.now() - t0;
                    if (dt >= SLOW_PATCH_MS)
                        slowPatches.push(`global#${i}=${dt.toFixed(1)}ms`);
                }
                i++;
            }
        }
    ...
    if (timed) {
        ...
        if (total >= SLOW_TOTAL_MS || slowPatches.length) {
            ContextMenuLogger.debug(`Patched ${navId} in ${total.toFixed(1)}ms` +
...
export function _usePatchContextMenu(props) {
    ...
    React.useEffect(() => {
        ...
        if (state.sourceChildren === props.children)
            return;
        try {
    ...
    if (!Menu.MenuItem)
        return props; // Prevent crashes if menu items failed to resolve
    const state = mountRef.current;
