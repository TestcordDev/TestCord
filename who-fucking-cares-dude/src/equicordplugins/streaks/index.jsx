...
const colorFor = (streak) => {
    if (streak >= STREAK_THRESHOLDS.ELITE)
        return settings.store.eliteColor;
    if (streak >= STREAK_THRESHOLDS.DIAMOND)
        return settings.store.diamondColor;
    if (streak >= STREAK_THRESHOLDS.PLATINUM)
        return settings.store.platinumColor;
    if (streak >= STREAK_THRESHOLDS.GOLD)
        return settings.store.goldColor;
    if (streak >= STREAK_THRESHOLDS.SILVER)
        return settings.store.silverColor;
    if (streak >= STREAK_THRESHOLDS.BRONZE)
        return settings.store.bronzeColor;
...
const StreakBadge = ({ userId }) => {
    ...
    return (<Tooltip text={`${streak.count} day streak`}>
            {tooltipProps => (<span {...tooltipProps} className={cl("badge")} style={{ color }}>
...
export default definePlugin({
    ...
    renderMemberListDecorator({ user, type }) {
        if (type !== "dm" || !user || user.id === UserStore.getCurrentUser()?.id)
            return null;
