...
const Native = typeof VencordNative !== "undefined"
    ? VencordNative.pluginHelpers.MetadataScanner
    : null;
const CopyButton = ({ text }) => {
    ...
    return (<button onClick={handleCopy} className="vc-mds-copy-btn" title="Copy to clipboard" type="button">
            {copied ? (<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#23a55a" strokeWidth="2.5">
...
export function MetadataScannerModal({ rootProps, url, name, mimeType, size }) {
        ...
        async function fetchAndParse() {
            try {
                ...
                if (Native) {
                    ...
                    if (uint8.byteLength > MAX_SCAN_BYTES)
                        throw new Error("File exceeds size limit (50MB)");
                    ...
                    if (contentLength > MAX_SCAN_BYTES)
                        throw new Error("File exceeds size limit (50MB)");
    ...
    if (confirmScanLargeFile) {
        return (<ModalRoot {...rootProps} className={`vc-mds-modal ${themeClass}`} size={"small" /* ModalSize.SMALL */}>
                ...
                <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "16px", alignItems: "center", textAlign: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: "12px", width: "100%", marginTop: "8px" }}>
                        <Button size={Button.Sizes.MEDIUM} color={Button.Colors.PRIMARY} style={{ flex: 1 }} onClick={rootProps.onClose}>
                            Cancel
                        </Button>
                        <Button size={Button.Sizes.MEDIUM} color={Button.Colors.RED} style={{ flex: 1 }} onClick={() => setConfirmScanLargeFile(false)}>
                            Scan Anyway
                        </Button>
                    </div>
                </div>
    ...
    const renderPreview = () => {
        ...
        if (isImage) {
            return (<div style={{
                ...
                }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.8)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; }} title="Copy Image" type="button">
                    ...
                    color: "#fff",
                    transition: "background 0.2s"
                }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.8)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; }} title="Download Image" type="button">
                    ...
                    color: "#fff",
                    transition: "background 0.2s"
                }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.8)"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; }} title="Fullscreen" type="button">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        return (<div className="vc-mds-preview-fallback">
    ...
    const renderTabContent = () => {
        if (!data)
            return null;
        const renderSearchInput = () => (<div style={{ marginBottom: "12px", width: "100%" }}>
                <input type="text" placeholder="Search tags or values..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{
                width: "100%",
                padding: "8px 12px",
        ...
        switch (activeTab) {
            case "file":
                return (<table className="vc-mds-table">
                        <tbody>
                            <tr>
                            ...
                            {(data.fileInfo.dimensions || (dynamicMedia.width && dynamicMedia.height)) && (<tr>
                            ...
                            {dynamicMedia.duration !== undefined && (<tr>
            ...
            case "exif": {
                ...
                return (<div>
                        {renderSearchInput()}
                        {filtered.length === 0 ? (<div style={{ padding: "16px", textAlign: "center", color: "var(--text-muted)" }}>No tags matching search query.</div>) : (<table className="vc-mds-table">
                                <tbody>
            ...
            case "chunks": {
                ...
                return (<div>
                        {renderSearchInput()}
                        {filtered.length === 0 ? (<div style={{ padding: "16px", textAlign: "center", color: "var(--text-muted)" }}>No chunks matching search query.</div>) : (<table className="vc-mds-table">
                                <tbody>
                                    {filtered.map((chunk, i) => (<tr key={i}>
            ...
            case "sd":
                if (!data.sdParams)
                    return null;
                const { sdParams } = data;
                return (<div className="vc-mds-sd-container">
                        <div>
            ...
            case "gps": {
                ...
                return (<div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                        <div className="vc-mds-gps-card">
                            ...
                            {gps.altitude !== undefined && (<div style={{ fontSize: "13px", color: "var(--text-muted)" }}>
                                    Altitude: <strong>{gps.altitude.toFixed(1)} meters</strong> (above sea level)
                                </div>)}
                            <div className="vc-mds-gps-actions">
                                <button onClick={() => window.open(gps.googleMapsUrl, "_blank")} className="vc-mds-button" type="button">
                                    Open in Google Maps
                                </button>
                                <button onClick={() => window.open(gps.osmUrl, "_blank")} className="vc-mds-button vc-mds-button-secondary" type="button">
                        ...
                        <div style={{ borderRadius: "8px", overflow: "hidden", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", height: "300px", width: "100%" }}>
                            <iframe width="100%" height="100%" frameBorder="0" scrolling="no" marginHeight={0} marginWidth={0} src={`https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${gps.latitude},${gps.longitude}`} style={{ border: 0 }}/>
                        </div>
                    </div>);
            }
            case "raw": {
    ...
    return (<>
            <ModalRoot {...rootProps} className={`vc-mds-modal ${themeClass}`} size={"large" /* ModalSize.LARGE */}>
                <ModalHeader className="vc-mds-modal-header">
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                ...
                <div className="vc-mds-layout">
                    <div className="vc-mds-sidebar">
                        ...
                        <div style={{ marginTop: "auto" }}>
                            <Button size={Button.Sizes.MEDIUM} color={Button.Colors.BRAND} style={{ width: "100%" }} onClick={() => window.open(url, "_blank")}>
                                Open in Browser
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="vc-mds-main">
                        {loading ? (<div className="vc-mds-loading-state">
                                <div className="vc-mds-spinner"/>
                                <span>Reading file structures…</span>
                            </div>) : error ? (<div className="vc-mds-error-state">
                                <span className="vc-mds-error-title">Analysis Failed</span>
                                <span style={{ fontSize: "13px" }}>{error}</span>
                            </div>) : (<>
                                <div className="vc-mds-tabs">
                                    {tabs.map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`vc-mds-tab ${activeTab === tab.id ? "active" : ""}`} type="button">
                                            {tab.label}
                                        </button>))}
                                </div>
            ...
            {isFullscreen && ReactDOM.createPortal(<div ref={fullscreenRef} style={{
                position: "fixed",
                top: 0,
                left: 0,
                width: "100vw",
                ...
                zIndex: 100000,
                overflow: "hidden"
            }} onWheel={handleWheel} onMouseDown={e => {
                e.stopPropagation();
                handleMouseDown(e);
            }} onMouseMove={handleMouseMove} onMouseUp={e => {
                e.stopPropagation();
                handleMouseUp();
                ...
                if (e.target === e.currentTarget) {
                    ...
                    <img src={url} alt="Fullscreen Preview" style={{
                maxWidth: "90%",
                maxHeight: "90%",
                objectFit: "contain",
                transform: `translate(${dragOffset.x}px, ${dragOffset.y}px) scale(${zoomScale})`,
                ...
                cursor: zoomScale > 1 ? (isDragging ? "grabbing" : "grab") : "default"
                    ...
                    <div onMouseDown={e => e.stopPropagation()} onMouseUp={e => e.stopPropagation()} onClick={e => e.stopPropagation()} onWheel={e => e.stopPropagation()} style={{
                position: "absolute",
                bottom: "30px",
                left: "50%",
            ...
            }} onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "0.8"; }} title="Copy Image" type="button">
                ...
                opacity: 0.8,
                transition: "opacity 0.2s"
            }} onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "0.8"; }} title="Download Image" type="button">
                ...
                opacity: 0.8,
                transition: "opacity 0.2s"
            }} onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "0.8"; }} title="Close Fullscreen" type="button">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
