...
export function ReplacementInput({ replacement, setReplacement, replacementError }) {
    ...
    return (<>
            ...
            {!isFunc && (<div>
                    ...
                    {Object.entries(RegexGuide).map(([placeholder, desc]) => (<Paragraph key={placeholder}>
                            {Parser.parse("`" + placeholder + "`")}: {desc}
                        </Paragraph>))}
                </div>)}

            <FormSwitch className={Margins.top16} value={isFunc} onChange={setIsFunc} title={"Treat Replacement as function"} description='"Replacement" will be evaluated as a function if this is enabled' hideBorder/>
        </>);
}
