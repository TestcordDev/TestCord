...
function isAuthorized(req, body) {
    ...
    if (RendererSettings.store.plugins?.TestcordHelper?.liveFixRequireToken === false)
        return true;
    if (authToken === null)
        return false;
...
export function startLiveFixServer(_) {
    if (server)
...
export function stopLiveFixServerCleanup(_) {
    queue.length = 0;
    inFlight = false;
}
export function stopLiveFixServer(_) {
    if (server) {
        server.close();
        server = null;
    }
...
export function writeResponse(_, data) {
