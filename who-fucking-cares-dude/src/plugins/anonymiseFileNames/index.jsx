...
const settings = definePluginSettings({
    ...
    method: {
        ...
        options: [
            { label: "Random Characters", value: 0 /* Methods.Random */, default: true },
            { label: "Consistent", value: 1 /* Methods.Consistent */ },
            { label: "Timestamp", value: 2 /* Methods.Timestamp */ },
            { label: "Date", value: 3 /* Methods.Date */ },
        ],
    },
    randomisedLength: {
    ...
    randomisedLength: {
        disabled() { return this.store.method !== 0 /* Methods.Random */; },
    },
    consistent: {
        disabled() { return this.store.method !== 1 /* Methods.Consistent */; },
    },
    dateFormat: {
        disabled() { return this.store.method !== 3 /* Methods.Date */; },
    },
});
export default definePlugin({
    ...
    AnonymiseUploadButton: ErrorBoundary.wrap(({ upload }) => {
        ...
        return (<ActionBarIcon tooltip={anonymise ? "Using anonymous file name" : "Using normal file name"} onClick={onToggleAnonymise}>
                {anonymise
    ...
    anonymise(upload) {
        ...
        const addSpoilerPrefix = (str) => settings.store.spoilerMessages ? "SPOILER_" + str : str;
        ...
        if ((upload[ANONYMISE_UPLOAD_SYMBOL] ?? settings.store.anonymiseByDefault) === false)
            return addSpoilerPrefix(originalFileName + ext);
        const newFilename = (() => {
            switch (settings.store.method) {
                case 0 /* Methods.Random */:
                    const chars = "0123456789bdfhjkmnpqrstvwxz";
                    const returnedName = Array.from({ length: settings.store.randomisedLength }, () => chars[Math.floor(Math.random() * chars.length)]).join("") + ext;
                    return addSpoilerPrefix(returnedName);
                case 1 /* Methods.Consistent */:
                    return addSpoilerPrefix(settings.store.consistent + ext);
                case 2 /* Methods.Timestamp */:
