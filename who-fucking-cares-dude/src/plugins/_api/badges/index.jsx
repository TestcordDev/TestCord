...
const EquicordContributorBadge = {
...
const TestcordContributorBadge = {
...
const HEART_BADGE_ICON = `data:image/svg+xml;charset=utf-8,${encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 16 16\"><path fill=\"#db61a2\" fill-rule=\"evenodd\" d=\"M4.25 2.5c-1.336 0-2.75 1.164-2.75 3 0 2.15 1.58 4.144 3.365 5.682A20.565 20.565 0 008 13.393a20.561 20.561 0 003.135-2.211C12.92 9.644 14.5 7.65 14.5 5.5c0-1.836-1.414-3-2.75-3-1.373 0-2.609.986-3.029 2.456a.75.75 0 01-1.442 0C6.859 3.486 5.623 2.5 4.25 2.5zM8 14.25l-.345.666-.002-.001-.006-.003-.018-.01a7.643 7.643 0 01-.31-.17 22.075 22.075 0 01-3.434-2.414C2.045 10.731 0 8.35 0 5.5 0 2.836 2.086 1 4.25 1 5.797 1 7.153 1.802 8 3.02 8.847 1.802 10.203 1 11.75 1 13.914 1 16 2.836 16 5.5c0 2.85-2.045 5.231-3.885 6.818a22.08 22.08 0 01-3.744 2.584l-.018.01-.006.003h-.002L8 14.25zm0 0l.345.666a.752.752 0 01-.69 0L8 14.25z\"/></svg>")}`;
const TestcordUserBadge = {
    id: "testcord_user",
...
const UserPluginContributorBadge = {
...
let DonorBadges = {};
let EquicordDonorBadges = {};
let TestcordCustomBadges = {};
async function loadBadges(url, noCache = false) {
    const init = {};
    if (noCache)
        init.cache = "no-cache";
...
async function loadAllBadges(noCache = false) {
    ...
    const results = await Promise.allSettled(urls.map(({ url }) => fetch(url, init).then(r => r.ok ? r.json() : {})));
...
async function ensurePronounsMarker() {
    if (Settings.plugins?.TestcordHelper?.pronounsBadge === false)
        return;
    const selfId = UserStore.getCurrentUser()?.id;
    if (!selfId)
        return;
    if (UserProfileStore.getUserProfile(selfId)?.pronouns?.includes(ZWSP))
        return;
    try {
        const { body } = await RestAPI.get({ url: Constants.Endpoints.USER_PROFILE(selfId) });
        const pronouns = body?.user_profile?.pronouns ?? body?.pronouns;
        if (pronouns?.includes(ZWSP))
            return;
...
export function BadgeContextMenu({ badge }) {
    return (<Menu.Menu navId="vc-badge-context" onClose={ContextMenuApi.closeContextMenu} aria-label="Badge Options">
            {badge.description && (<Menu.MenuItem id="vc-badge-copy-name" label="Copy Badge Name" action={() => copyWithToast(badge.description)}/>)}
            {badge.iconSrc && (<Menu.MenuItem id="vc-badge-copy-link" label="Copy Badge Image Link" action={() => copyWithToast(badge.iconSrc)}/>)}
        </Menu.Menu>);
}
export default definePlugin({
    ...
    renderBadgeComponent: ErrorBoundary.wrap((badge) => {
        const Component = badge.component;
        return <Component {...badge}/>;
    }, { noop: true }),
    getBadgeMouseEventHandlers(badge) {
        const handlers = {};
        if (!badge)
            return handlers; // sanity check
        const { onClick, onContextMenu } = badge;
        if (onClick)
            handlers.onClick = e => onClick(e, badge);
        if (onContextMenu)
            handlers.onContextMenu = e => onContextMenu(e, badge);
        return handlers;
    },
    getDonorBadges(userId) {
    ...
    getTestCordCustomBadges(userId) {
    ...
    getTestCordDonorBadges: function (userId) {
