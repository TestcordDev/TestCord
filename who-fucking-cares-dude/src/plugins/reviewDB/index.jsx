...
const guildPopoutPatch = (children, { guild }) => {
    if (!guild)
        return;
    children.push(<Menu.MenuItem label="View Reviews" id="vc-rdb-server-reviews" icon={OpenExternalIcon} action={() => openReviewsModal(guild.id, guild.name, 1 /* ReviewType.Server */)}/>);
};
const userContextPatch = (children, { user }) => {
    if (!user)
        return;
    children.push(<Menu.MenuItem label="View Reviews" id="vc-rdb-user-reviews" icon={OpenExternalIcon} action={() => openReviewsModal(user.id, user.username, 0 /* ReviewType.User */)}/>);
};
export default definePlugin({
    ...
    async start() {
        ...
        setTimeout(async () => {
            ...
            if (user) {
                ...
                if (notification) {
                    const props = notification.type === 1 /* NotificationType.Ban */ ? {
                        cancelText: "Appeal",
                        confirmText: "Ok",
                        onCancel: async () => VencordNative.native.openExternal("https://reviewdb.mantikafasi.dev/api/redirect?"
                            + new URLSearchParams({
                                token: Auth.token,
                                page: "dashboard/appeal"
                            }))
                    } : {};
                    openModal(modalProps => (<ConfirmModal {...modalProps} title={notification.title} confirmText={props.confirmText ?? "OK"} cancelText={props.cancelText} variant="primary" onCancel={props.onCancel}>
    ...
    renderProfileCollection: {
        ...
        render: ({ user, isSideBar = false }) => {
            ...
            const reviewsSection = (<section className={ProfileCardClasses.container}>
                    <ul className={ProfileCardClasses.cardsList} tabIndex={-1}>
                        <li className={ProfileCardClasses.firstCardContainer}>
                            <Clickable className={classes(ProfileCardContainerClasses.breadcrumb, reviewData?.hasOptedOut && cl("profile-popout-disabled"))} onClick={() => !reviewData?.hasOptedOut && openReviewsModal(user.id, user.username, 0 /* ReviewType.User */)}>
                    ...
                    ? (<div className={ProfileCardContainerClasses.icons}>
                            ...
                            return (<div className={ProfileCardContainerClasses.icon} key={review.id}>
                                                                <img src={review.sender.profilePhoto} alt={review.sender.username} className={showCount ? ProfileCardContainerClasses.displayCount : undefined} onError={e => e.currentTarget.src = IconUtils.getDefaultAvatarURL(review.sender.discordID)}/>
                                                                {showCount && (<div className={ProfileCardContainerClasses.displayCountText}>
