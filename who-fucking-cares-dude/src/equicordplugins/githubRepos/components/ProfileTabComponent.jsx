...
export function ProfileTabComponent({ id }) {
    const [groups, setGroups] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [activeKey, setActiveKey] = useState(PERSONAL_GROUP_KEY);
    ...
    if (!groups.length)
        return null;
    return (<div className={cl("container", "tab")}>
            <RepoSubTabs groups={sortedGroups} activeKey={activeGroup?.key ?? PERSONAL_GROUP_KEY} onSelect={setActiveKey} sortMode={sortMode} onToggleSort={() => setSortMode(mode => mode === "count" ? "alpha" : "count")} canSort={sortedGroups.length > 2}/>
            <div className={cl("list")}>
                {activeGroup?.repos.map(repo => (<RepoCard key={repo.id} repo={repo} showStars={settings.store.showStars} showLanguage={settings.store.showLanguage}/>))}
            </div>
        </div>);
}
