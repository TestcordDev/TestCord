...
export default definePlugin({
    name: "AutoDeleter",
    description: "Automatically deletes your messages after a configurable delay (persists after restart)",
    tags: ["Chat", "Utility"],
    authors: [
        {
            name: "Bash",
    ...
    async start() {
        ...
        this.log(`Restoration completed: ${this.stats.restoredFromStorage} messages restored`);
    },
    stop() {
        ...
        try {
            ...
            for (const savedMsg of savedMessages) {
                ...
                if (timeUntilDeletion <= 0) {
                    this.debug(`Message ${savedMsg.id} should have been deleted, immediate deletion`);
    ...
    onMessageCreate(event) {
        try {
            if (!settings.store.enabled || settings.store.emergencyStop)
                return;
    ...
    shouldPreserveMessage(message) {
        ...
        if (settings.store.maxMessageLength > 0 &&
            content.length > settings.store.maxMessageLength) {
            return true;
        }
        if (settings.store.minMessageLength > 0 &&
            content.length < settings.store.minMessageLength) {
            return true;
        }
        // Check embeds
        if (settings.store.preserveEmbeds &&
            message.embeds &&
            message.embeds.length > 0) {
            return true;
        }
        // Check attachments
        if (settings.store.preserveAttachments &&
            message.attachments &&
            message.attachments.length > 0) {
            return true;
        }
        // Check reactions
        if (settings.store.preserveReactions &&
            message.reactions &&
    ...
    shouldDeleteImmediately(content) {
        if (!content)
            return false;
    ...
    calculateSmartDelay(message) {
        ...
        const mediaMultiplier = (message.embeds?.length || 0) > 0 ||
    ...
    checkThrottlingLimits() {
        if (!settings.store.aggressiveThrottling)
            return true;
        const now = Date.now();
    ...
    scheduleMessageDeletion(message, customDelay) {
        const delay = customDelay || this.getDelayInMs();
        const scheduledTime = Date.now() + delay;
        const { deletionMode } = settings.store;
        this.debug(`Scheduling message deletion ${message.id} in ${delay}ms (${new Date(scheduledTime).toLocaleString()}) - Mode: ${deletionMode}`);
    ...
    addToDeletionQueue(messageId, channelId, mode) {
    ...
    scheduleMessageDeletionFromRestore(savedMsg, delay) {
        this.debug(`Rescheduling message deletion ${savedMsg.id} in ${delay}ms`);
        const timeoutId = setTimeout(() => {
    ...
    async deleteMessage(messageId, channelId, mode, attempt = 1) {
        try {
            const deletionMode = mode || settings.store.deletionMode;
            this.debug(`Attempting to delete message ${messageId} - Mode: ${deletionMode} (attempt ${attempt})`);
            ...
            if (this.rateLimitInfo.isRateLimited &&
                ...
                if (timeSinceLastRateLimit < this.rateLimitInfo.retryAfter * 1000) {
                    this.debug(`Rate limit active, adding to retry queue: ${messageId}`);
    ...
    handleDeletionError(error, messageId, channelId, mode, attempt) {
        ...
        if (error.status === 429 && settings.store.rateLimitHandling) {
            ...
            // Too many attempts
            this.error(`Giving up on deleting message ${messageId} after ${attempt} attempts`);
            this.trackedMessages.delete(messageId);
            this.messageCache.delete(messageId);
        }
        if (settings.store.notifications) {
    ...
    handleRateLimit(error, messageId, channelId, mode, attempt) {
        ...
        this.log(`Rate limit detected - Retry after ${retryAfter}s (attempt ${attempt}) - Total: ${this.rateLimitInfo.totalRateLimits}`);
    ...
    addToRetryQueue(messageId, channelId, mode, attempts, delay = 0) {
        ...
        this.debug(`Added to retry queue: ${messageId} (retry in ${baseDelay}ms)`);
        if (!this.retryProcessor) {
    ...
    async performNormalDeletion(messageId, channelId) {
        await RestAPI.del({
            url: `/channels/${channelId}/messages/${messageId}`,
        });
    },
    async performSilentDeletion(messageId, channelId) {
    ...
    async performEditThenDelete(messageId, channelId) {
        try {
    ...
    async performAntiLogDeletion(messageId, channelId) {
        try {
            ...
            const buggedMsgResponse = await messageSendWrapper(settings.store.blockMessage, messageId, channelId);
            ...
            this.error(`Error during AntiLog deletion of message ${messageId}:`, error);
    ...
    updateAverageMessageLength(newLength) {
    ...
    getPendingMessages() {
        return Array.from(this.trackedMessages.values()).map((msg) => ({
            id: msg.id,
            channelId: msg.channelId,
    ...
    optimizePerformance() {
        ...
        this.debug(`Optimization completed - Cache: ${this.messageCache.size} messages, ${this.channelCache.size} channels`);
    },
});
