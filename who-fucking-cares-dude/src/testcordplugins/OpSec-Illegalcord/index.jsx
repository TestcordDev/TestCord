...
function getContextVocabulary(message) {
    const vocab = new Set();
    if (message?.content) {
    ...
    if (message?.messageReference?.message_id) {
        const MessageStore = window.DiscordModules?.MessageStore;
        if (MessageStore) {
            try {
...
function findContextCorrections(text, vocab) {
    ...
    for (const word of words) {
        ...
        if (knownMisspellings.has(lowerWord))
            continue;
        ...
        for (const known of vocab) {
            if (known.length < 2 || lowerWord.length < 2)
                continue;
            const maxDist = Math.max(1, Math.floor(Math.min(known.length, lowerWord.length) / 3));
            if (Math.abs(known.length - lowerWord.length) > maxDist)
                continue;
...
const correctionRegexCache = new Map();
function getCorrectionRegex(word) {
...
function applyContextCorrections(text, corrections) {
    let result = text;
...
function getCustomReplacements() {
    ...
    return lines.map(line => {
        ...
        if (word && replacement) {
            return [new RegExp(`\\b${word}\\b`, "gi"), replacement];
        }
        return null;
    }).filter(Boolean);
}
...
const COMMON_MISSPELLINGS = [
    ...
    [/\bgonna([a-z])/gi, m => "gonna " + m.slice(5)],
    ...
    [/\bwanna([a-z])/gi, m => "wanna " + m.slice(5)],
    [/\bgotta([a-z])/gi, m => "gotta " + m.slice(5)],
...
const COMMON_PUNCTUATION_ERRORS = [
    [/\.\.+/g, "."],
    [/!!+/g, "!"],
    [/\?\?+/g, "?"],
    [/,,+/g, ","],
...
const STRAIGHT_TO_CURLY_START = {
    "'": "'",
    "\"": "\"",
    "`": "`"
};
const STRAIGHT_TO_CURLY_END = {
    "'": "'",
    "\"": "\"",
    "`": "`"
};
...
function isProbablyItalian(text) {
...
function fixItalianContractions(text) {
    let result = text;
...
function fixPunctuation(text, allCaps = false) {
...
function addPeriodIfNeeded(text) {
