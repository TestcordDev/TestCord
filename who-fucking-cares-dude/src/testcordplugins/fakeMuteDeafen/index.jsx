 ...
 */
import { registerCommand, sendBotMessage, unregisterCommand } from "@api/Commands";
...
let buttonRerender = null;
function refreshVoiceState() {
    buttonRerender?.();
    if (!wsModule || !SelectedChannelStore || !ChannelStore || !MediaEngineStore)
        return;
...
function FakeDeafenIcon({ className }) {
    ...
    return (<svg className={className} width="28" height="28" viewBox="0 0 32 32" fill="none">
            <mask id="fakeMuteDeafenLine">
                <rect width="100%" height="100%" fill="#ffffff"/>
                <line className="blackLine" x1="29" y1="2" x2="2" y2="29" stroke="#000000" strokeWidth="8" strokeLinecap="round" style={lineStyle}/>
            </mask>

            <g mask="url(#fakeMuteDeafenLine)">
                <rect x="6" y="8" width="20" height="4" rx="2" fill={!enabled ? "var(--status-danger)" : "currentColor"}/>
                <rect x="11" y="3" width="10" height="8" rx="3" fill={!enabled ? "var(--status-danger)" : "currentColor"}/>
                <circle cx="10" cy="21" r="4" stroke={!enabled ? "var(--status-danger)" : "currentColor"} strokeWidth="2" fill="none"/>
                <circle cx="22" cy="21" r="4" stroke={!enabled ? "var(--status-danger)" : "currentColor"} strokeWidth="2" fill="none"/>
                <path d="M14 21c1 1 3 1 4 0" stroke={!enabled ? "var(--status-danger)" : "currentColor"} strokeWidth="2" strokeLinecap="round"/>
            </g>

            <line x1="29" y1="2" x2="2" y2="29" stroke="var(--status-danger, currentColor)" strokeWidth="2" strokeLinecap="round" style={lineStyle}/>
        </svg>);
}
function FakeMuteDeafenButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton className="button__201d5 wrapper__201d5" tooltipText={hideTooltips ? void 0 : isEnabled ? "Disable Fake Mute & Deafen" : "Enable Fake Mute & Deafen"} icon={<FakeDeafenIcon className={iconForeground}/>} role="switch" aria-checked={isEnabled} redGlow={!isEnabled} plated={nameplate != null} onClick={handleClick}/>);
}
let lastVoiceChannelId = null;
let refreshVoiceStateTimer = null;
...
export default definePlugin({
    ...
    start() {
        ...
        if (socket && !originalSend) {
            ...
            socket.send = function (op, data, ...args) {
                if (op === 4 && data) {
                    if (fakeVoiceState.selfMute)
                        data.self_mute = true;
                    if (fakeVoiceState.selfDeaf)
                        data.self_deaf = true;
                    if (fakeVoiceState.selfVideo)
        ...
        if (settings.store.userAreaButton) {
            try {
                window.Vencord?.Api?.UserArea?.addUserAreaButton("fake-mute-deafen", FakeMuteDeafenButton);
            }
            catch (e) {
    ...
    stop() {
        ...
        try {
            window.Vencord?.Api?.UserArea?.removeUserAreaButton("fake-mute-deafen");
        }
        catch (e) { }
    ...
    contextMenus: {
        "audio-device-context"(children, d) {
            ...
            if (settings.store.combinedContextItem) {
                children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-mute-deafen-both" label="Fake Mute & Deafen" checked={fakeVoiceState.selfMute && fakeVoiceState.selfDeaf} action={() => {
                        const next = !(fakeVoiceState.selfMute && fakeVoiceState.selfDeaf);
                        fakeVoiceState.selfMute = next;
                        fakeVoiceState.selfDeaf = next;
                        refreshVoiceState();
            ...
            if (d.renderInputDevices) {
                children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-mute" label="Fake Mute" checked={fakeVoiceState.selfMute} action={() => {
                        fakeVoiceState.selfMute = !fakeVoiceState.selfMute;
                        refreshVoiceState();
                    }}/>);
            }
            if (d.renderOutputDevices) {
                children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-deafen" label="Fake Deafen" checked={fakeVoiceState.selfDeaf} action={() => {
                        fakeVoiceState.selfDeaf = !fakeVoiceState.selfDeaf;
                        if (settings.store.autoMute) {
                            fakeVoiceState.selfMute = fakeVoiceState.selfDeaf;
                        }
        ...
        "video-device-context"(children) {
            if (!settings.store.contextMenu)
                return;
            children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-video" label="Fake Camera" checked={fakeVoiceState.selfVideo} action={() => {
