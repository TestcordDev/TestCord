...
export function t(arabic, english = arabic) { return english; }
import definePlugin from "@utils/types";
import { findByPropsLazy } from "@webpack";
import { createRoot, IconUtils, MediaEngineStore, React, showToast, Toasts, useEffect, useRef, UserStore, useState, VoiceActions } from "@webpack/common";
...
// The same lock glyph as a data URI, so the lock screen and the masked notification can use
// an <img> without any network request.
const LOCK_ICON_DATA_URI = "data:image/svg+xml;utf8," + encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\" width=\"64\" height=\"64\">" +
    `<path fill="#ffffff" d="${LOCK_ICON_PATH}"/></svg>`);
/** Restart an icon animation by swapping the variant class. The reflow read is required:
 *  re-adding a class the element already has does NOT replay a CSS animation. */
function setIconVariant(img, variant) {
    img.classList.remove("pcl-lockicon-intro", "pcl-lockicon-shake", "pcl-lockicon-rotate");
    void img.offsetWidth;
    img.classList.add(`pcl-lockicon-${variant}`);
}
function LockIcon(props) {
    const size = props.width ?? props.height ?? props.size ?? 20;
    return (<svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" height={size} width={size}>
            <path fill={props.color ?? "currentColor"} d={LOCK_ICON_PATH}/>
        </svg>);
}
// ─── Crypto helpers (ported verbatim, pure Web Crypto, no BD dependency) ───
const str2binb = (s) => new TextEncoder().encode(s);
const buf2hex = (buf) => Array.prototype.map.call(new Uint8Array(buf), (x) => ("00" + x.toString(16)).slice(-2)).join("");
const hex2binb = (hex) => Uint8Array.from(hex.match(/.{1,2}/g) ?? [], byte => parseInt(byte, 16));
...
// Upstream generated the salt as HEX but decoded it with atob() as BASE64 — a real mismatch.
// It happened to "work" (hex characters are all valid base64 symbols and the length divides by
// four) and the salt stayed unique, but it silently derived from the wrong bytes. Hex in, hex
// out. SHA-1 is also replaced by SHA-256, and the iteration count raised from 4,000 to OWASP's
// current PBKDF2-HMAC-SHA256 guidance — a short numeric passcode is brute-forceable offline
// otherwise, and people reuse passcodes elsewhere.
async function pbkdf2(str, salt, iterations) {
    const key = await crypto.subtle.importKey("raw", str2binb(str), { name: "PBKDF2" }, false, ["deriveKey"]);
    const derived = await crypto.subtle.deriveKey({ name: "PBKDF2", salt: hex2binb(salt), iterations, hash: { name: "SHA-256" } }, key, { name: "HMAC", hash: "SHA-256", length: 256 }, true, ["sign", "verify"]);
...
async function hashCode(code) {
    ...
    const hash = await pbkdf2(code, salt, KDF_ITERATIONS);
    return { hash, salt, iterations: KDF_ITERATIONS, kdf: KDF_VERSION };
}
async function hashCheck(code, salt, iterations, expected) {
    return (await pbkdf2(code, salt, iterations)) === expected;
}
/**
 * Is there a passcode this build can actually verify?
 *
 * A hash produced by a different KDF can never match, so treating it as "set" would lock the
 * user out permanently with no way back in through the UI. Treating it as "not set" instead
 * degrades to the unconfigured state — the setup screen — which is recoverable. Deliberate
 * trade-off: fail open on an unverifiable hash rather than brick the client.
 */
function hasPasscode(d) {
    return Boolean(d.hash) && d.kdf === KDF_VERSION;
}
// ─── Settings ───
...
async function loadData() {
    const oldData = await DataStore.get(DS_KEY);
...
function buildAnimatedIcon(container, variant) {
    const img = document.createElement("img");
    img.alt = "";
    img.width = 64;
    img.height = 64;
    img.className = `pcl-lockicon pcl-lockicon-${variant}`;
    img.style.opacity = "0";
    container.appendChild(img);
    setTimeout(() => {
        img.style.transition = ".3s opacity";
        img.style.opacity = "1";
        img.src = LOCK_ICON_DATA_URI;
    }, 0);
    return img;
}
function PasscodeBtn({ number, dec, click, code, children }) {
    return (<div className="pcl-btn pcl-animate" id={`PCLBtn-${code ?? number}`} onClick={() => click?.(number)}>
            {!children ? (<>
...
function BackspaceIcon() {
    return (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" height="22" width="22">
...
function CancelIcon() {
    return (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" height="30" width="30">
...
function EnterIcon() {
    return (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" height="34" width="34">
...
function PasscodeLocker({ type, button, onDone }) {
    ...
    const getBg = () => rootRef.current?.querySelector(".pcl-layout-bg");
    ...
    const close = (success) => {
        try {
            const controls = rootRef.current?.querySelector(".pcl-controls");
            if (controls)
    ...
    const fail = () => {
        if (inputRef.current) {
            ...
            setTimeout(() => {
                if (inputRef.current)
                    inputRef.current.classList.remove("vcl-err");
            }, 2800);
        }
        if (iconRef.current) {
            setIconVariant(iconRef.current, type === "default" ? "shake" : "rotate");
        }
        if (type !== "default")
            return;
    ...
    const accept = async (submittedCode = inputRef.current?.value || "") => {
        try {
            if (submittedCode === "")
                return;
            if (type === "editor") {
                if (!confirmStage) {
                    ...
                    if (iconRef.current)
                        setIconVariant(iconRef.current, "rotate");
                }
    ...
    useEffect(() => {
        ...
        const finishEntrance = () => {
            ...
            try {
                if (onTransEnd)
                    bgRef?.removeEventListener("transitionend", onTransEnd);
                if (tick)
                    clearInterval(tick);
                        ...
                        animEls().forEach(el => {
                            ...
                            if (Math.hypot(Math.abs(eTop - cTop), Math.abs(eLeft - cLeft)) <= radius) {
                                if (el.classList.contains("pcl-icon") && !iconRef.current) {
                                    iconRef.current = buildAnimatedIcon(el, "intro");
    ...
    useEffect(() => {
        const onKeyDown = (e) => {
            if (e.key === "Escape" && type !== "default")
                close(false);
        };
        window.addEventListener("keydown", onKeyDown);
        return () => window.removeEventListener("keydown", onKeyDown);
    }, [type]);
    const title = type === "editor" ? (confirmStage ? t("أعد إدخال رمزك", "Re-enter your passcode") : t("أدخل رمزك الجديد", "Enter your new passcode")) : t("أدخل رمز ديسكورد", "Enter your Discord passcode");
    const { avatarUrl, username } = getUserAssets();
    return (<div className="pcl-layout" ref={rootRef}>
            ...
            <div className="pcl-controls">
                ...
                <div id="vcl-input-wrap" className="pcl-animate">
                    ...
                    <input id="vcl-input" ref={inputRef} type="password" placeholder={t("كلمة المرور", "Password")} autoComplete="off" spellCheck={false} autoFocus defaultValue="" onChange={e => {
            ...
            if (e.key === "Enter")
        ...
        }} onKeyUp={e => e.stopPropagation()} onKeyPress={e => e.stopPropagation()} className={delay ? "vcl-err" : ""} disabled={delay}/>
                ...
                <div className={`pcl-delay ${delay ? "pcl-delay-visible" : ""}`}>
                    ...
                    {`${t("محاولات كثيرة.\nحاول مجدّداً بعد", "Too many tries.\nPlease try again in")} ${delayLeft} ${delayLeft > 1 ? t("ثوانٍ", "seconds") : t("ثانية", "second")}.`}
                </div>
            </div>
        </div>);
}
...
// through the cracks: window.__testcordPasscodeReset()
function forceReset(reason) {
    if (reason)
        console.warn(`[PasscodeLock] Force reset triggered: ${reason}`);
    try {
        root?.unmount();
    }
    catch (e) {
...
function openLocker(type, button, onSuccess) {
    if (root)
        return;
    ...
    root.render(<PasscodeLocker type={type} button={button} onDone={(success, newCode) => {
            clearTimeout(watchdogTimeout);
            try {
                root?.unmount();
            }
            ...
            if (type === "default") {
                ...
                if (deafened) {
                    try {
                        ...
                        console.error("[PasscodeLock] re-deafen failed", e);
                    }
                }
            }
            if (success && type === "editor" && newCode) {
                hashCode(newCode).then(({ hash, salt, iterations, kdf }) => {
                    ...
                    showToast(t("تعذّر حفظ الرمز، حاول مجدداً.", "Failed to save passcode, please try again."), Toasts.Type.FAILURE);
                });
            }
            if (success)
                onSuccess?.(newCode);
...
function lock(button = document.body) {
    if (isLocked)
        return;
    try {
        ...
        showToast(t("تعذّر فتح قفل الرمز — راجع الطرفية للتفاصيل.", "PasscodeLock failed to open — check the console for details."), Toasts.Type.FAILURE);
    }
}
function resetAutolock() {
    clearTimeout(autolockTimeout);
    if (!settings.store.autolockSeconds)
        return;
...
export default definePlugin({
    ...
    headerBarButton: {
        ...
        render: () => {
            ...
            return (<HeaderBarButton ref={ref} icon={LockIcon} iconSize={20} tooltip="Lock Discord" onClick={() => {
    ...
    settingsAboutComponent: () => {
        ...
        return (<div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
                <button style={{ padding: "8px 14px", borderRadius: "4px", background: "var(--brand-experiment, #5865F2)", color: "#fff", border: "none", cursor: "pointer" }} onClick={() => {
                if (root && !document.querySelector(".pcl-layout")) {
                ...
                <button style={{ padding: "8px 14px", borderRadius: "4px", background: "transparent", color: "var(--text-normal, #fff)", border: "1px solid var(--background-modifier-accent, #555)", cursor: "pointer" }} onClick={() => {
                if (root && !document.querySelector(".pcl-layout")) {
                    forceReset("settings button clicked while root was set but no overlay was actually in the DOM");
                }
                lock(document.body);
    ...
    async start() {
        ...
        window.__esharqPasscodeReset = () => forceReset("manually triggered via window.__testcordPasscodeReset()");
        if (settings.store.hideNotifications && NotificationModule) {
            originalShowNotification = NotificationModule.showNotification;
            NotificationModule.showNotification = function (...args) {
        ...
        window.addEventListener("mousemove", this._activityListener);
        window.addEventListener("keydown", this._activityListener);
        window.addEventListener("mousedown", this._activityListener);
        resetAutolock();
    ...
    stop() {
        ...
        window.removeEventListener("mousemove", this._activityListener);
        window.removeEventListener("keydown", this._activityListener);
        window.removeEventListener("mousedown", this._activityListener);
        clearTimeout(autolockTimeout);
        clearTimeout(watchdogTimeout);
        delete window.__esharqPasscodeReset;
        root?.unmount();
        container?.remove();
        root = null;
        container = null;
