...
import { ANIM_KEYS, ANIM_LABELS, applyPreset, deletePreset, duplicatePreset, exportAllPresets, getAnim, getAnimMaster, getForceApplyDefault, getHideDuplicate, getRestoreDefault, hasPreset, listPresets, loadPresets, renamePreset, reorderPresets, savePreset, setAnim, setAnimMaster, setForceApplyDefault, setHideDuplicate, setRestoreDefault } from "./presets";
...
const saveJson = (data, filename) => saveFile(new File([data], filename, { type: "application/json" }));
function formatDate(ts) {
    if (!ts)
        return "";
    return new Intl.DateTimeFormat(navigator.language, { dateStyle: "medium", timeStyle: "short" }).format(ts);
}
function PresetDragPreview({ preset, point, offset }) {
    if (!preset || !point)
        return null;
    const count = Object.values(preset.plugins).filter(p => p.enabled).length;
    return (<div className="vc-presets-row vc-presets-drag-preview" style={{ transform: `translate3d(${point.x - offset.x}px, ${point.y - offset.y}px, 0)` }}>
...
function PresetRow({ preset, globalDefault, forceDefault, hideDuplicate, draggingName, dragOverName, dropIntent, setDraggingName, setDragOverName, setDropIntent, setDragPreviewPoint, setDragPreviewOffset, onChange }) {
    ...
    const onRename = () => openNameModal("Rename preset", preset.name, to => {
        if (hasPreset(to) && to !== preset.name)
            return showToast(`A preset named "${to}" already exists.`, Toasts.Type.FAILURE);
    ...
    const onDuplicate = () => {
        ...
        let i = 2;
        while (hasPreset(copy))
            copy = `${preset.name} (copy ${i++})`;
    ...
    const [{ isOver }, drop] = useDrop({
        ...
        hover: (item, monitor) => {
            if (!rowRef.current || item.name === preset.name)
                return;
        ...
        drop: (item, monitor) => {
            if (monitor.didDrop() || item.name === preset.name)
                return;
            const clientOffset = monitor.getClientOffset();
            if (clientOffset)
                setDragPreviewPoint(clientOffset);
            ...
            clearDragTarget();
            if (intent === "merge")
                openMergeModal(item.name, preset.name, onChange);
        },
    });
    React.useEffect(() => {
        if (!isOver && dragOverName === preset.name)
            clearDragTarget();
    }, [isOver, dragOverName, preset.name]);
    drag(drop(rowRef));
    const count = Object.values(preset.plugins).filter(p => p.enabled).length;
    return (<Card ref={rowRef} className={`vc-presets-row${(draggingName === preset.name || isDragging) ? " vc-presets-row-dragging" : ""}${dragOverName === preset.name ? ` vc-presets-row-drop-target vc-presets-row-drop-${dropIntent}` : ""}`}>
...
function PresetsTab() {
    ...
    const [draggingName, setDraggingName] = React.useState(null);
    const [dragOverName, setDragOverName] = React.useState(null);
    const [dropIntent, setDropIntent] = React.useState(null);
    const [dragPreviewPoint, setDragPreviewPoint] = React.useState(null);
    const [dragPreviewOffset, setDragPreviewOffset] = React.useState({ x: 16, y: 16 });
    ...
    return (<SettingsTab>
            ...
            <FormSwitch value={globalDefault} onChange={v => { setRestoreDefault(v); forceUpdate(); }} title="Default: restore plugin settings on apply (overwrites current)" description="The default for presets set to 'Restore: Default'. When on, applying such a preset also overwrites each plugin's settings with its saved values. Per-preset 'Restore: On/Off' overrides this."/>

            <FormSwitch value={forceDefault} onChange={v => { setForceApplyDefault(v); forceUpdate(); }} title="Default: force apply presets (full overwrite)" description="The default for presets set to 'Force: Default'. When on, applying such a preset disables every plugin the preset doesn't include and always overwrites plugin settings, making the preset the source of truth. Per-preset 'Force: On/Off' overrides this."/>

            <FormSwitch value={hideDuplicate} onChange={v => { setHideDuplicate(v); forceUpdate(); }} title="Hide the Duplicate button on preset rows" description="Removes the per-preset Duplicate button from the list. You can still duplicate by exporting and re-importing."/>
            ...
            <div className="vc-presets-anim-section">
                <div className="vc-form-switch-container">
                    <div className="vc-form-switch vc-presets-anim-master-row">
                        <button type="button" className="vc-form-switch-text vc-presets-anim-master-button" aria-expanded={animOpen} onClick={() => setAnimOpen(o => !o)}>
                        ...
                        <div className="vc-form-switch-control">
                            <Switch checked={animMaster} onChange={v => { setAnimMaster(v); syncAnimClasses(); forceUpdate(); }}/>
                        </div>
                    </div>
                </div>
                <div className={`vc-presets-anim-list${animOpen ? " vc-presets-anim-list-open" : ""}${animMaster ? "" : " vc-presets-anim-list-disabled"}`}>
                    {ANIM_KEYS.map(key => (<FormSwitch key={key} value={getAnim(key)} disabled={!animMaster} onChange={v => { setAnim(key, v); syncAnimClasses(); forceUpdate(); }} title={ANIM_LABELS[key].title} description={ANIM_LABELS[key].description}/>))}
                </div>
            </div>
