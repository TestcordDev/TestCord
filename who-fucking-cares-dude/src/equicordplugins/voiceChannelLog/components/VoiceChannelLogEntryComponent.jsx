...
export function VoiceChannelLogEntryComponent({ logEntry, channel }) {
    ...
    return (<li className={cl("entry")}>
            ...
            {(logEntry.type === "activity" || logEntry.type === "activity_stop") && logEntry.applicationId ? (<img className={cl("app-icon")} src={IconUtils.getApplicationIconURL({ id: logEntry.applicationId, icon: ApplicationStore.getApplication(logEntry.applicationId)?.icon })} alt={logEntry.activityName ?? "Activity"}/>) : (<EventIcon type={logEntry.type}/>)}
            <Tooltip text={username}>
                {(tooltipProps) => (<img {...tooltipProps} className={cl("avatar")} onClick={() => openUserProfile(logEntry.userId)} src={user ? user.getAvatarURL(channel.getGuildId()) : IconUtils.getDefaultAvatarURL(logEntry.userId)} alt={username}/>)}
            </Tooltip>
            <div className={cl("content")}>
                <span className={cl("username")} onClick={() => openUserProfile(logEntry.userId)}>{username}</span>
                {logEntry.type === "soundboard" ? (<div className={cl("soundboard-info")}>
                        ...
                        <div className={cl("soundboard-actions")}>
                            <Tooltip text="Play sound">
                                {tooltipProps => (<Clickable {...tooltipProps} className={cl("soundboard-action")} onClick={() => playSound(logEntry.soundId)}>
                                        <svg width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M8 5v14l11-7z"/></svg>
                                    </Clickable>)}
                            </Tooltip>
                            <Tooltip text="Save sound">
                                {tooltipProps => (<Clickable {...tooltipProps} className={cl("soundboard-action")} onClick={() => downloadSound(logEntry.soundId)}>
                    ...
                    </div>) : (logEntry.type === "activity" || logEntry.type === "activity_stop") && logEntry.applicationId ? (<div className={cl("soundboard-info")}>
                        <span className={cl("soundboard-id")}>{description}</span>
                        <div className={cl("soundboard-actions")}>
                            <Tooltip text="View activity">
                                {tooltipProps => (<Clickable {...tooltipProps} className={cl("soundboard-action")} onClick={() => {
                    closeAllModals();
                    ...
                    </div>) : (<span className={cl("description")}>{description}</span>)}
            </div>
        </li>);
}
