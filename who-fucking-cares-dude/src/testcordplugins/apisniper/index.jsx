...
export const Native = VencordNative.pluginHelpers.ApiSniper;
...
function isLikelyFalsePositive(match, fullContent) {
    // Discord emoji format: <:name:id> or <a:name:id>
    if (/<a?:\w+:\d+>/.test(fullContent) && fullContent.trim().match(/^<a?:\w+:\d+>$/))
        return true;
    // Tenor/gif URLs
    if (/tenor\.com/i.test(fullContent))
        return true;
    if (/discord(app)?\.com/i.test(fullContent) && /https?:\/\//i.test(fullContent))
        return true;
    if (/cdn\.discordapp\.com/i.test(fullContent))
        return true;
    if (/reddit\.com/i.test(fullContent))
        return true;
    // Message content that's clearly not a credential
    if (/\btimed out\b/i.test(fullContent))
        return true;
    if (/\bgif\b/i.test(match) && /https?:\/\//i.test(fullContent))
        return true;
    if (/\bimage\d*\.\w+\b/i.test(fullContent))
        return true;
    // Tenor slug format: word-word-number
    if (/^[a-z]+-[a-z]+-[a-z]+-\d+$/i.test(match))
        return true;
    // Domain-like patterns (example.com, site.org, etc.)
    if (/\b[a-z]+\.(?:com|org|net|io|dev|app|co|gg|me)\b/i.test(match))
        return true;
    // Natural language with common short words separated by dots
    // Real base64 tokens don't have lowercase dictionary words
    if (/^[a-z]{2,10}\.[a-z]{2,10}\.[a-z]{2,10}$/i.test(match))
        return true;
    ...
    if (parts.length === 3) {
        ...
        if (parts.some(p => commonWords.test(p)))
            return true;
    }
    return false;
}
...
function checkForCredentials(content) {
    if (!couldContainCredential(content))
        return [];
    const findings = [];
    for (const [type, pattern] of PATTERN_ENTRIES) {
        ...
        if (match) {
            ...
            if (isLikelyFalsePositive(matchedValue, content))
                continue;
...
function shouldIgnoreMessage(msg) {
    ...
    if (msg.author.id === UserStore.getCurrentUser()?.id && !settings.store.snipeOwnMessages)
        return true;
    if (getBlacklistSet().has(msg.author.id))
        return true;
    return false;
}
let blacklistCacheKey;
...
function processMessage(msg, channelId) {
    if (shouldIgnoreMessage(msg))
        return;
    if (!couldContainCredential(msg.content))
        return;
    const msgId = msg.id;
    const now = Date.now();
...
function messageUpdateHandler(payload) {
    const msg = payload.message;
    if (!msg)
        return;
