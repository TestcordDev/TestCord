...
function PatchHelper() {
    ...
    const [parsedFind, setParsedFind] = useState("");
    ...
    function onFindChange(v) {
        setFind(v);
        try {
            let parsedFind = v;
            if (/^\/.+?\/$/.test(v))
                parsedFind = new RegExp(v.slice(1, -1));
            setFindError(void 0);
    ...
    return (<SettingsTab>
            ...
            <FullPatchInput setFind={onFindChange} setParsedFind={setParsedFind} setMatch={onMatchChange} setReplacement={setReplacement}/>
            ...
            <div className={Margins.top20}>
                <ReplacementInput replacement={replacement} setReplacement={setReplacement} replacementError={replacementError}/>
            </div>

            {module && (<>
                    <Divider className={Margins.top16 + " " + Margins.bottom16}/>
                    <Span size="md" weight="medium" color="text-strong">Preview</Span>
                    <PatchPreview module={module} match={match} replacement={replacement} setReplacementError={setReplacementError}/>
                </>)}
