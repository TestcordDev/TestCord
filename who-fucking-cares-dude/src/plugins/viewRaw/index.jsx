...
const CopyRawIcon = ({ height = 20, width = 20, className }) => {
    return (<svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" width={width} height={height} className={className}>
...
function openViewRawModal(json, type, msgContent) {
    openModal(props => (<ErrorBoundary>
            <Modal {...props} title={`Raw ${type} Data`} size="xl" actions={[
            {
                text: `Copy ${type} Data`,
                variant: "secondary",
...
function openViewRawModalMessage(msg) {
...
function MakeContextCallback(name, getData) {
    return (children, props) => {
        ...
        if (isMessage && !settings.store.messageContextMenu)
            return;
        ...
        (devContainer ?? children).splice(-1, 0, <Menu.MenuItem id={id} label="View Raw" action={action} icon={CopyRawIcon}/>);
    };
}
const devContextCallback = (children, { id }) => {
    ...
    if (!role)
        return;
    children.push(<Menu.MenuItem id={"vc-view-role-raw"} label="View Raw" action={() => openViewRawModal(JSON.stringify(role, null, 4), "Role")} icon={CopyRawIcon}/>);
};
