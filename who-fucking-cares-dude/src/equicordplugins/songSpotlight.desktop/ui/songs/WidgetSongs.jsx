...
import Song from ".";
export default function WidgetSongs({ user }) {
    ...
    useEffect(() => {
        if (isAuthorized() && !data)
            listData(user.id).catch(() => setFailed(true));
    }, [isAuthorized()]);
    const owned = UserStore.getCurrentUser().id === user.id;
    let full;
    if (failed) {
        full = (<BaseText size="md" weight="semibold" className={cl("errored")}>
        ...
        full = (<>
                ...
                {owned && (<Button variant="secondary" style={{ marginTop: "14px", flexShrink: 1 }} onClick={() => openSettingsModal()}>
                        Edit songs
                    </Button>)}
            </>);
    }
    return (<ScrollerThin fade className={classes(WidgetClasses.tabPanelScroller, cl("widget-container"))} key="song-spotlight-widget-songs">
            <Flex flexDirection="column" justifyContent={full && "center"} alignItems={full && "center"} gap={full ? "8px" : "10px"} style={full && { flex: 1, textAlign: "center" }}>
                {full
