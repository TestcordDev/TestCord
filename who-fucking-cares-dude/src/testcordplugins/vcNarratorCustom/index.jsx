...
import { ChannelStore, Forms, GuildMemberStore, IconUtils, Menu, Modal, openModal, React, SearchableSelect, SelectedChannelStore, SelectedGuildStore, showToast, useMemo, UserStore, VoiceStateStore, } from "@webpack/common";
import { addUserToList, addUserToStateChangeFilterList, clean, clearTtsCache, formatText, getCachedVoiceFromDB, getPersistentTtsCacheStats, getVoiceForUser, parseStateChangeFilterList, parseUserIdList, parseUserVoiceMap, removeUserFromList, removeUserFromStateChangeFilterList, removeUserVoiceFromMap, setCachedVoiceInDB, ttsCache, upsertUserVoiceMap, VOICE_OPTIONS, } from "./util";
...
let lastApiCallStatus = { at: 0, message: "No API calls yet" };
function setLastApiCallStatus(next) {
...
function TroubleshootingSettings() {
    ...
    const [apiStatus, setApiStatus] = React.useState(lastApiCallStatus);
    ...
    const apiStatusClass = apiStatus.status
        ? apiStatus.status >= 200 && apiStatus.status < 300
            ...
            : cl("api-status-error")
        : cl("api-status-muted");
    return (<div className={cl("troubleshooting")}>
...
function CustomSoundSettings({ soundKey, nameKey }) {
    ...
    return (<div className={cl("custom-sound")}>
            ...
            <input ref={fileInputRef} type="file" accept=".mp3,.wav,.ogg,.m4a,.aac,.flac,.webm,.wma,.mp4" className={cl("file-input")} onChange={uploadSound}/>
            <div className={cl("buttons")}>
                ...
                <Button variant="dangerPrimary" size="small" disabled={busy || !hasCustomSound} onClick={() => {
...
function interruptPlayback(key) {
    if (currentInterruptKey !== key || !currentStop)
        return;
    currentAudio?.pause();
    currentStop();
}
...
function queueSpeak(text, userId, interruptKey, queue = "main", useDefaultVoice, sound) {
...
async function speak(text, userId, interruptKey, useDefaultVoice, sound) {
    return new Promise(resolve => {
        const playAudio = (url, playbackRate) => new Promise(resolveAudio => {
        ...
        const getTtsUrl = async (speechText, voice) => {
        ...
        const playSpeech = async (speechText, voice) => {
        ...
        void (async () => {
            ...
            if (await playSpeech(beforeSound, voice)) {
                ...
                if (soundCompleted)
                    await playSpeech(afterSound, voice);
            }
            resolve();
        })().catch(() => resolve());
    });
...
let myLastChannelId;
const lastJoinLeaveTimestamp = new Map();
let trackedChannelId = null;
let baselineReady = false;
let baselineUpdateInProgress = false;
const voiceStateSnapshot = new Map();
const lastAnnounced = new Map();
const lastIntroSpokenAt = new Map();
...
function buildStateSegments(type, isOn, preferredName, isSelf) {
...
function queueStateSplitAnnouncement(userId, intro, actionText) {
...
async function refreshBaseline(channelId) {
    if (baselineUpdateInProgress)
        return;
    ...
    for (let i = 0; i < 15; i++) {
        ...
        if (states) {
            ...
            for (const s of Object.values(states)) {
                if (!s?.userId)
                    continue;
                voiceStateSnapshot.set(s.userId, normalizeState(s));
            }
            baselineReady = true;
            break;
        }
...
function getTypeAndChannelId({ channelId, oldChannelId }, isMe) {
    ...
    if (channelId !== oldChannelId) {
        if (channelId)
            return [oldChannelId ? "move" : "join", channelId];
        if (oldChannelId)
            return ["leave", oldChannelId];
    }
    return ["", ""];
}
function playSample(tempSettings, type) {
    const settingsobj = Object.assign({}, settings.store, tempSettings);
    const currentUser = UserStore.getCurrentUser();
    const myGuildId = SelectedGuildStore.getGuildId();
    queueSpeak(formatText(settingsobj[type + "Message"], currentUser.username, "general", currentUser.globalName ?? currentUser.username, (myGuildId ? GuildMemberStore.getNick(myGuildId, currentUser.id) : null) ?? currentUser.username, settingsobj.latinOnly), undefined, undefined, "main", undefined, settingsobj[type + "Sound"]);
}
...
function VoiceSelectModal({ modalProps, user }) {
    ...
    return (<Modal {...modalProps} size="md" title="VC Narrator Voice" actions={[
            {
                text: "Cancel",
                variant: "secondary",
            ...
            <section className={Margins.bottom16}>
                ...
                <SearchableSelect options={options} value={options.find(o => o.value === currentValue)?.value} placeholder="Select a voice" maxVisibleItems={6} closeOnSelect={true} onChange={v => setCurrentValue(v)}/>

                <Forms.FormText className={cl("preview-hint")}>
                    Preview how this voice sounds with their name:
                </Forms.FormText>
                <div className={cl("preview-buttons")}>
                    <Button variant="secondary" size="small" disabled={busy} onClick={() => previewVoice(clean(displayName, settings.store.latinOnly) || "Someone")}>
                        {busy ? "Playing..." : `"${clean(displayName, settings.store.latinOnly) || "Someone"}"`}
                    </Button>
                    <Button variant="secondary" size="small" disabled={busy} onClick={() => previewVoice(`${clean(displayName, settings.store.latinOnly) || "Someone"} joined`)}>
                        Joined
                    </Button>
                    <Button variant="secondary" size="small" disabled={busy} onClick={() => previewVoice(`${clean(displayName, settings.store.latinOnly) || "Someone"} left`)}>
                        Left
                    </Button>
                </div>
            </section>
...
function openVoiceSelectModal(user) {
    openModal(modalProps => (<ErrorBoundary>
...
const UserContextMenuPatch = (children, { user }) => {
    ...
    const filterLabel = filterMode === "whitelist"
    ...
    children.push(<Menu.MenuItem id="vc-narrator-submenu" label="VC Narrator">
            <Menu.MenuItem key="voice" id="vc-narrator-voice" label={`Voice: ${voiceLabel}`} action={() => openVoiceSelectModal(user)}/>
            <Menu.MenuItem key="ignore" id="vc-narrator-ignore" label={isIgnored ? "Unignore User" : "Ignore User"} action={() => {
            if (isIgnored) {
            ...
            <Menu.MenuItem key="filter" id="vc-narrator-state-filter" label={filterLabel} disabled={!filterEnabled} action={filterAction}/>
        </Menu.MenuItem>);
};
export default definePlugin({
    ...
    authors: [Devs.Ven, Devs.Nyako, TestcordDevs.Loukios, TestcordDevs.examplegit, TestcordDevs.qdnx],
    settings,
    contextMenus: {
    ...
    flux: {
        async VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            if (!myChanId) {
                ...
                if (selectedChanId) {
                    const states = VoiceStateStore.getVoiceStatesForChannel?.(selectedChanId);
            ...
            const allowStateChange = (targetId, isSelf) => {
                if (isSelf)
                    return true;
                if (filterMode === "off")
                    return true;
            ...
            if (myChanId && ChannelStore.getChannel(myChanId)?.type === 13 /* ChannelType.GUILD_STAGE_VOICE */)
                return;
            if (!myChanId) {
            ...
            const isBatchUpdate = voiceStates.length > 1 && voiceStates.some(s => !("oldChannelId" in s));
            ...
            for (const state of voiceStates) {
                ...
                if (type) {
                    if (!shouldSkipAnnouncement) {
                        ...
                        if (!throttled) {
                            ...
                            queueSpeak(formatText(template, u, channel, displayName, nickname, settings.store.latinOnly), userId, undefined, "main", undefined, settings.store[type + "Sound"]);
                        }
                    }
                ...
                if (shouldSkipAnnouncement)
                    continue;
                if (!allowStateChange(userId, isMe))
                    continue;
    ...
    settingsAboutComponent({ tempSettings: s }) {
        ...
        const authorUser = UserStore.getUser(String(TestcordDevs.examplegit.id));
        const authorAvatar = authorUser ? IconUtils.getUserAvatarURL(authorUser, false, 64) : null;
        return (<>
                <div className={cl("author-note")}>
                    {authorAvatar && (<img src={authorAvatar} alt="Author avatar" className={cl("author-avatar")}/>)}
                ...
                <div className={cl("preview-section")}>
                    ...
                    <div className={cl("preview-grid")}>
                        {allTypes.map(t => (<Button key={t} variant="secondary" size="small" disabled={busy} onClick={() => playSample(s, t)}>
                                {wordsToTitle([t])}
                            </Button>))}
                    </div>
                </div>
