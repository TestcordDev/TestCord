...
function getConfiguredKeybind() {
    ...
    if (Array.isArray(raw) && raw.some(key => !isModifierKey(key)))
        return raw;
...
function matchesKeybind(event) {
    ...
    if (!matchesModifiers(event, keybind))
        return false;
    for (const key of keybind) {
        if (isModifierKey(key))
            continue;
        if (pressed !== key && code !== key)
            return false;
...
function matchesModifiers(event, keybind) {
...
function addPressedModifiers(event, keys) {
    if (event.metaKey)
        keys.push("Meta");
    if (event.ctrlKey)
        keys.push("Control");
...
function shouldIgnoreKeybindTarget(target) {
...
function RandomVoiceIcon({ className }) {
    return (<svg className={className} width="18" height="18" viewBox="0 0 24 24">
            <g fill="currentColor">
...
function setServerIds(serverIds) {
...
function getGuildVoiceStates() {
    return getRenderableGuilds().flatMap(({ id }) => Object.entries(VoiceStateStore.getVoiceStates(id) ?? {}));
}
function hasStateFilters(store = settings.store) {
    return stateFilters.some(({ key }) => store[key]);
}
function matchesOperation(operation, left, right) {
    if (operation === "==")
        return left === right;
    if (operation === ">")
        return left < right;
...
function isAfkChannel(channel) {
    const guildId = channel.getGuildId();
    if (!guildId)
        return false;
    return GuildStore.getGuild(guildId)?.afkChannelId === channel.id;
}
function matchesStateFilters(state, store = settings.store) {
    if (store.mute && !state.selfMute)
        return false;
    if (store.deafen && !state.selfDeaf)
        return false;
    if (store.video && !state.selfVideo)
        return false;
    if (store.stream && !state.selfStream)
        return false;
    return true;
}
function isJoinableChannel(channelId, store = settings.store) {
    const channel = ChannelStore.getChannel(channelId);
    if (!channel)
        return false;
    const selectedServerIds = getSelectedServerIds(store);
    const guildId = channel.getGuildId();
    if (selectedServerIds != null && (!guildId || !selectedServerIds.includes(guildId)))
        return false;
    if (store.avoidStages && isStageChannel(channel))
        return false;
    if (store.avoidAfk && isAfkChannel(channel))
        return false;
    ...
    if (channel.userLimit > 0 && usersInChannel >= channel.userLimit)
        return false;
    if (currentUserId && voiceStates && Object.prototype.hasOwnProperty.call(voiceStates, currentUserId))
        return false;
    return true;
}
function matchesChannelFilters(channelId, store = settings.store) {
    const channel = ChannelStore.getChannel(channelId);
    if (!channel)
        return false;
    const voiceStates = VoiceStateStore.getVoiceStatesForChannel(channelId);
    ...
    if (!matchesOperation(store.spacesLeftOperation, spacesLeft, store.spacesLeft))
        return false;
    if (!matchesOperation(store.UserAmountOperation, usersInChannel, store.UserAmount))
        return false;
    if (!matchesOperation(store.vcLimitOperation, channelLimit, store.vcLimit))
        return false;
    if (!hasStateFilters(store) || !voiceStates)
        return true;
    ...
    if (store.includeStates && !hasMatch)
        return false;
    if (store.avoidStates && hasMatch)
        return false;
    return true;
}
function getCandidateChannelIds(store = settings.store) {
    ...
    for (const [, state] of getGuildVoiceStates()) {
        ...
        if (!channelId || candidates.has(channelId))
            continue;
        if (!isJoinableChannel(channelId, store))
            continue;
        if (!matchesChannelFilters(channelId, store))
            continue;
...
function getFriendChannelIds() {
    const friendChannelIds = new Set();
...
async function startChannelStream(channel) {
    if (isStageChannel(channel) || !PermissionStore.can(PermissionsBits.STREAM, channel))
        return;
...
function runAfterVoiceJoin(channelId, callbacks) {
    let attempts = 0;
...
function RandomVoiceButton({ iconForeground, hideTooltips, nameplate }) {
    return (<UserAreaButton className="button__201d5 wrapper__201d5" onClick={() => void joinRandomVoice()} onContextMenu={event => ContextMenuApi.openContextMenu(event, () => <RandomVoiceMenu onClose={ContextMenuApi.closeContextMenu}/>)} role="switch" tooltipText={hideTooltips ? void 0 : "Random Voice"} icon={<RandomVoiceIcon className={iconForeground}/>} plated={nameplate != null}/>);
}
function RandomVoiceMenu({ onClose }) {
    ...
    const toggle = (key) => update(key, !store[key]);
    ...
    const toggleServer = (guildId) => {
        setServerIds(selectedServerIds.includes(guildId)
            ? selectedServerIds.filter(id => id !== guildId)
            : [...selectedServerIds, guildId]);
        rerender();
    };
    const setSlider = (key) => debounce((value) => {
        store[key] = Math.round(value);
        rerender();
    }, 50);
    return (<Menu.Menu navId="random-voice" onClose={onClose} aria-label="Random Voice">
            <Menu.MenuItem id="random-voice-servers" label="Servers">
                <>
                    <Menu.MenuCheckboxItem id="random-voice-select-all-servers" label="Select All" checked={selectedServerIds.length === allServerIds.length} disabled={selectedServerIds.length === allServerIds.length} action={selectAllServers}/>
                    <Menu.MenuCheckboxItem id="random-voice-reset-servers" label="Reset" checked={selectedServerIds.length === 0} disabled={!selectedServerIds.length} action={resetServers}/>
                    <Menu.MenuSeparator />
                    {guilds.map(guild => (<Menu.MenuCheckboxItem key={guild.id} id={`random-voice-server-${guild.id}`} label={guild.name} checked={selectedServerIds.includes(guild.id)} action={() => toggleServer(guild.id)}/>))}
                </>
            </Menu.MenuItem>

            <Menu.MenuItem id="random-voice-state-filters" label="State Filters">
                <>
                    {stateFilters.map(({ key, label }) => (<Menu.MenuCheckboxItem key={key} id={`random-voice-filter-${key}`} label={label} checked={store[key]} action={() => toggle(key)}/>))}
                    <Menu.MenuSeparator />
                    <Menu.MenuCheckboxItem id="random-voice-include-states" label="Include Filters" checked={store.includeStates} disabled={store.avoidStates || !hasStateFilters(store)} action={() => toggle("includeStates")}/>
                    <Menu.MenuCheckboxItem id="random-voice-avoid-states" label="Avoid Filters" checked={store.avoidStates} disabled={store.includeStates || !hasStateFilters(store)} action={() => toggle("avoidStates")}/>
                </>
            </Menu.MenuItem>
            ...
            <Menu.MenuItem id="random-voice-self-settings" label="Join Settings">
                <>
                    {selfSettings.map(({ key, label }) => (<Menu.MenuCheckboxItem key={key} id={`random-voice-setting-${key}`} label={label} checked={store[key]} action={() => toggle(key)}/>))}
                </>
            </Menu.MenuItem>
        </Menu.Menu>);
}
function renderOperationGroup({ id, label, sliderKey, operationKey, sliderValue, operationValue, onOperationChange, onSliderChange, }) {
    return (<Menu.MenuGroup label={label.toUpperCase()}>
            <Menu.MenuControlItem id={`random-voice-slider-${sliderKey}`} label={label} control={(props, ref) => (<Menu.MenuSliderControl ref={ref} {...props} minValue={1} maxValue={15} value={sliderValue} onChange={onSliderChange} renderValue={value => `${Math.round(value)} user${Math.round(value) === 1 ? "" : "s"}`}/>)}/>
            <Menu.MenuItem id={`random-voice-operation-${operationKey}`} label="Parameters">
                <>
                    {operationOptions.map(option => (<Menu.MenuRadioItem key={option.value} id={`random-voice-operation-${id}-${option.value}`} group={`random-voice-${id}`} label={option.label} checked={operationValue === option.value} action={() => onOperationChange(option.value)}/>))}
                </>
            </Menu.MenuItem>
        </Menu.MenuGroup>);
}
export default definePlugin({
    ...
    onKeyDown(event) {
        if (isRecordingKeybind)
            return;
        if (!settings.store.keybindEnabled)
            return;
        if (shouldIgnoreKeybindTarget(event.target) && !keybindUsesModifier())
            return;
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }) {
            const currentUserId = getCurrentUserId();
            if (!currentUserId || !settings.store.leaveEmpty)
                return;
            const myChannelId = getCurrentVoiceChannelId(currentUserId);
            if (!myChannelId)
                return;
            const touchedCurrentChannel = voiceStates.some(state => state.userId === currentUserId ||
                state.channelId === myChannelId ||
                state.oldChannelId === myChannelId);
            if (!touchedCurrentChannel)
                return;
            const myOwnJoin = voiceStates.some(state => state.userId === currentUserId &&
                state.channelId === myChannelId &&
                state.oldChannelId !== myChannelId);
            if (myOwnJoin)
                return;
            const channelStates = VoiceStateStore.getVoiceStatesForChannel(myChannelId);
