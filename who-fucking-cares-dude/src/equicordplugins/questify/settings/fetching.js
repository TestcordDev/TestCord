...
export function autoFetchCompatible() {
    const settings = getQuestifySettings();
    const { newExcludedQuestAlertSound, newQuestAlertSound, questFetchInterval, notifyOnNewExcludedQuests, notifyOnNewQuests, questButtonDisplay: displayMode, questButtonIndicator: indicatorMode } = settings;
...
export function startAutoFetchingQuests(force = false) {
...
export function stopAutoFetchingQuests() {
    if (autoFetchInterval) {
...
export function resetQuestsToResume(quest, userId) {
