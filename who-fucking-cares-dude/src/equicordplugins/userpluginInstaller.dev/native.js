...
export async function ensurePluginsDirectory(_) {
    if (!IS_DEV)
        return;
    try {
...
export async function rmPlugin(_, name) {
...
export async function isUpdateAvailableForPlugin(_, name) {
...
export function initPluginInstall(_, link, source, owner, repo) {
    // eslint-disable-next-line
    return new Promise(async (resolve, reject) => {
        ...
        win.on("page-title-updated", async (e) => {
            switch (win.webContents.getTitle()) {
                case "abortInstall": {
                    win.close();
...
async function getPluginMeta(path, extra = {}) {
    return new Promise((resolve, reject) => {
        ...
        files.forEach(f => {
            if (f === "index.ts")
                fileToRead = "index.ts";
            if (f === "index.tsx")
                fileToRead = "index.tsx";
            if (f === "index.js")
                fileToRead = "index.js";
            if (f === "index.jsx")
                fileToRead = "index.jsx";
        });
        if (!fileToRead)
...
async function cloneRepo(link, repo) {
    return new Promise((resolve, reject) => {
...
export async function getUserplugins() {
    ...
    try {
        ...
        const plugins = await Promise.allSettled(folderContents
...
export async function updatePlugin(_, directory) {
    return new Promise((resolve, reject) => {
        // directory is renderer-controlled and lands in a cwd — never let it traverse
        if (basename(directory) !== directory || directory.includes(".."))
                    ...
                    switch (win.webContents.getTitle()) {
                        ...
                        case "install": {
                            ...
                            try {
                                ...
                                let errored = "";
                                otherProc.stderr?.on("data", d => { if (String(d).includes("Success"))
                                    return; errored += String(d); });
                                otherProc.once("close", () => {
                                    if (errored)
                                        if (!errored.includes("Success"))
                                            return reject(`Failed to apply the update, chances are you have local changes that conflict with your remote. Git errors:\n\n${errored}`);
...
export async function openGitPathModal(_) {
    const gitPathSet = NativeSettings.store.plugins.UserpluginInstaller?.gitPath;
    const win = new BrowserWindow({
        maximizable: false,
        minimizable: false,
        width: 560,
