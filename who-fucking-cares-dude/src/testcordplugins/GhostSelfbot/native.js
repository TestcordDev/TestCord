...
let ghostPluginPath = null;
function detectGhostPluginPath() {
    if (ghostPluginPath)
...
function getSafePythonPath(pythonPath) {
...
function recreateDirectory(pluginDir, targetPath) {
...
async function getLatestRelease() {
    ...
    if (Array.isArray(assets)) {
        for (const asset of assets) {
            if (!isRecord(asset))
                continue;
            if (getStringProperty(asset, "name") !== GHOST_WINDOWS_ASSET)
                continue;
    ...
    if (!sourceZipUrl)
        throw new Error("GitHub release source zip was not found");
    return { tagName, sourceZipUrl, windowsZipUrl };
}
function getZipEntries(zipData) {
    return unzipSync(new Uint8Array(zipData));
}
function shouldStripRoot(entryNames) {
    let root;
...
function extractZip(zipData, targetDir, stripRoot) {
...
export function getGhostExePath() {
...
export function checkPythonInstalled(_event, pythonPath) {
    try {
...
export function installRequirements(_event, pythonPath) {
    try {
...
export function updateGhostConfig(_event, token, userId = "unknown", username = "unknown") {
    try {
        ...
        if (existsSync(ghostTokensPath)) {
            const tokens = JSON.parse(readFileSync(ghostTokensPath, "utf-8"));
            const existingIndex = tokens.findIndex((t) => t.id === userId);
            if (existingIndex >= 0) {
...
export function launchGhostExe(_event, autoFillToken, token, userId, username, nitroWebhookUrl, privnoteWebhookUrl, autoSetupWebhooks) {
    const ghostExePath = getGhostExePath();
...
export function launchGhostSource(_event, autoFillToken, autoInstallRequirements, pythonPath, token, userId, username, nitroWebhookUrl, privnoteWebhookUrl, autoSetupWebhooks) {
    const ghostSourcePath = getGhostSourcePath();
...
export function checkGhostSetup(_event, pythonPath) {
    const ghostExePath = getGhostExePath();
    const ghostSourcePath = getGhostSourcePath();
    const requirementsPath = getGhostRequirementsPath();
    return {
...
export function updateGhostWebhooks(_event, nitroWebhookUrl, privnoteWebhookUrl) {
    try {
        const ghostConfigPath = getGhostConfigPath();
        if (!existsSync(ghostConfigPath)) {
            logger.error("Ghost config not found. Please run Ghost.exe first to create config.");
        ...
        if (nitroWebhookUrl) {
            if (!config.snipers)
                config.snipers = {};
            if (!config.snipers.nitro)
                config.snipers.nitro = {};
        ...
        if (privnoteWebhookUrl) {
            if (!config.snipers)
                config.snipers = {};
            if (!config.snipers.privnote)
                config.snipers.privnote = {};
...
export function enableWebhookSetup(_event) {
    try {
