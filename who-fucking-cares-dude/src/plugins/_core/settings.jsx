...
import { BackupAndRestoreTab, ChangelogTab, CloudTab, HealthTab, PatchHelperTab, PluginsTab, PresetsTab, PrivacySecurityPanel, StatsTab, ThemesTab, UpdaterTab, VencordTab, } from "@components/settings";
...
function readVisibleTestcordTabs(validKeys) {
    const stored = settings.store.visibleSettingsTabs;
    // No customization yet -> everything visible.
    if (!Array.isArray(stored) || stored.length === 0)
        return validKeys;
    const validKeySet = new Set(validKeys);
    const visible = stored.filter((key) => typeof key === "string" && validKeySet.has(key));
...
function writeVisibleTestcordTabs(keys) {
...
function readTestcordTabOrder(validKeys) {
    const stored = settings.store.settingsTabOrder;
    const validKeySet = new Set(validKeys);
    const ordered = Array.isArray(stored)
        ? stored.filter((key) => typeof key === "string" && validKeySet.has(key))
        : [];
    return [...ordered, ...validKeys.filter(key => !ordered.includes(key))];
}
function writeTestcordTabOrder(keys) {
    settings.store.settingsTabOrder = keys;
}
function readPinnedTestcordTabs(validKeys) {
...
function writePinnedTestcordTabs(keys) {
    settings.store.pinnedSettingsTabs = keys;
}
function orderTestcordTabs(tabs, orderedKeys, pinnedKeys) {
    const orderIndex = new Map(orderedKeys.map((key, index) => [key, index]));
    const pinnedKeySet = new Set(pinnedKeys);
    return [...tabs].sort((a, b) => {
        if (a.key === TESTCORD_MAIN_ENTRY_KEY)
            return -1;
        if (b.key === TESTCORD_MAIN_ENTRY_KEY)
            return 1;
...
function showTestcordUndoToast(message, undo) {
    Toasts.show({
        message,
        id: Toasts.genId(),
        ...
        options: {
            ...
            component: (<div className="vc-testcord-tab-editor-toast">
                    <span>{message}</span>
...
function getTestcordTabDescriptors(entries) {
...
function syncVisibleTestcordSidebarTabs(tabs, visibleKeys, orderedKeys = readTestcordTabOrder(tabs.map(tab => tab.key)), pinnedKeys = readPinnedTestcordTabs(tabs.map(tab => tab.key))) {
    ...
    let attempts = 0;
    function sync() {
        const candidates = Array.from(document.querySelectorAll('[role="button"], [role="tab"], [class*="item"]'));
        let matched = 0;
        for (const candidate of candidates) {
            if (candidate.closest(".vc-testcord-tab-editor"))
                continue;
        ...
        if (matched < tabs.length && attempts++ < 10)
            requestAnimationFrame(sync);
    }
    requestAnimationFrame(sync);
}
function TestcordTabsEditor({ tabs }) {
    ...
    function saveVisibleTabs(nextKeys, undoMessage) {
    ...
    function toggleAllTabs() {
        saveVisibleTabs(areAllEditableTabsVisible ? [TESTCORD_MAIN_ENTRY_KEY] : validKeys, areAllEditableTabsVisible ? "All editable tabs hidden" : undefined);
    }
    function togglePinnedTab(key) {
        savePinnedTabs(pinnedKeySet.has(key)
            ? pinnedKeys.filter(pinnedKey => pinnedKey !== key)
            : [...pinnedKeys, key]);
    }
    function reorderTab(targetKey) {
    ...
    function markDropTarget(event, key, locked) {
        if (locked || !draggingKey)
            return;
    ...
    function setDragImage(event) {
        const chip = event.currentTarget.closest(".vc-testcord-tab-editor-chip");
        if (!chip)
            return;
        ...
        clone.querySelectorAll(".vc-testcord-tab-editor-chip-state, .vc-testcord-tab-editor-pin").forEach(element => {
            element.style.color = "#fff";
    ...
    return (<div className={`vc-testcord-tab-editor ${isEditing ? "vc-testcord-tab-editor-open" : ""} ${draggingKey ? "vc-testcord-tab-editor-is-dragging" : ""}`}>
            <div className="vc-testcord-tab-editor-title-row">
                ...
                <button type="button" className={`vc-testcord-tab-editor-toggle ${isEditing ? "vc-testcord-tab-editor-toggle-on" : ""}`} onClick={toggleEditorMode}>
                ...
                <div className="vc-testcord-tab-editor-panel-inner">
                    ...
                    <div className="vc-testcord-tab-editor-preview">
                        ...
                        <div className="vc-testcord-tab-editor-preview-list">
                            {previewTabs.map(({ key, title, Icon }) => (<span key={key} className="vc-testcord-tab-editor-preview-item">
            ...
            return (<div key={key} role="button" tabIndex={locked ? -1 : 0} draggable={!locked} className={`vc-testcord-tab-editor-chip ${enabled ? "vc-testcord-tab-editor-chip-on" : ""} ${draggingKey === key ? "vc-testcord-tab-editor-chip-dragging" : ""} ${dragOverKey === key && draggingKey !== key ? "vc-testcord-tab-editor-chip-drop-target" : ""}`} aria-disabled={locked} onClick={() => {
                    ...
                    markDropTarget(event, key, locked);
                }} onDragLeave={() => dragOverKey === key && setDragOverKey(null)} onDrop={event => dropOnTab(event, key)} onDragEnd={() => {
                    setDraggingKey(null);
                    setDragOverKey(null);
                }}>
                                    <span className="vc-testcord-tab-editor-drag-handle" aria-hidden onClick={event => event.stopPropagation()} onDragOver={event => markDropTarget(event, key, locked)} onDragEnter={event => markDropTarget(event, key, locked)} onDrop={event => dropOnTab(event, key)}>
                                        ::
                                    </span>
                                    {Icon && <Icon width={18} height={18}/>}
                                    <span className="vc-testcord-tab-editor-chip-title">{title}</span>
                                    {!locked && (<button type="button" className={`vc-testcord-tab-editor-pin ${pinned ? "vc-testcord-tab-editor-pin-on" : ""}`} onClick={event => {
                        event.stopPropagation();
                        togglePinnedTab(key);
                    }} onDragOver={event => markDropTarget(event, key, locked)} onDragEnter={event => markDropTarget(event, key, locked)} onDrop={event => dropOnTab(event, key)}>
                                            {pinned ? "Pinned" : "Pin"}
                                        </button>)}
                                    <span className="vc-testcord-tab-editor-chip-state" onDragOver={event => markDropTarget(event, key, locked)} onDragEnter={event => markDropTarget(event, key, locked)} onDrop={event => dropOnTab(event, key)}>
                                        {locked ? "Locked" : enabled ? "On" : "Off"}
                                    </span>
                                </div>);
        })}
                    </div>
                    <button type="button" className="vc-testcord-tab-editor-show-all" onClick={toggleAllTabs}>
                        {areAllEditableTabsVisible ? "Toggle all off" : "Add all tabs back"}
                    </button>
                    <button type="button" className="vc-testcord-tab-editor-restore" onClick={restoreOriginalLayout}>
...
function TestCordDiscordIcon({ original }) {
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder) {
        const layout = originalLayoutBuilder.buildLayout();
        if (originalLayoutBuilder.key !== "$Root")
            return layout;
        if (!Array.isArray(layout))
            return layout;
        if (layout.some(s => s?.key === "equicord_section"))
            return layout;
        const { buildEntry } = this;
        const equicordEntries = [
            buildEntry({
                key: TESTCORD_MAIN_ENTRY_KEY,
                title: "Testcord",
                panelTitle: "Testcord Settings",
                Component: () => <>
                    <TestcordTabsEditor tabs={getTestcordTabDescriptors(equicordEntries)}/>
                    <VencordTab />
                </>,
                Icon: TestCordIcon
            }),
        ...
        const orderedEquicordEntries = orderTestcordTabs(equicordEntries, readTestcordTabOrder(equicordEntries.map(entry => entry.key)), readPinnedTestcordTabs(equicordEntries.map(entry => entry.key)));
        syncVisibleTestcordSidebarTabs(getTestcordTabDescriptors(equicordEntries), readVisibleTestcordTabs(equicordEntries.map(entry => entry.key)));
        const equicordSection = {
    ...
    getVersionInfo(support = true) {
        ...
        if (IS_WEB)
            version = "Web";
        if (IS_VESKTOP)
            version = `Vesktop v${VesktopNative.app.getVersion()}`;
        if (IS_EQUIBOP)
            version = `Equibop v${VesktopNative.app.getVersion()}`;
        if (IS_STANDALONE)
            version = "Standalone";
    ...
    getInfoRows() {
        ...
        if (chromiumVersion)
            rows.push(`Chromium ${chromiumVersion}`);
        return rows;
    },
    getInfoString() {
        if (!settings.store.includeVencordInfoWhenCopying)
            return "";
        return "\n" + this.getInfoRows().join("\n");
    },
    makeInfoElements(Component, props) {
        return this.getInfoRows().map((text, i) => (<Component key={i} {...props}>
                {text}
            </Component>));
    },
    renderDiscordIcon(originalIcon) {
