...
const DraggableItem = ({ uploadItem, moveItem, children }) => {
    ...
    return (<div key={uploadItem.id} ref={node => {
            drag(drop(node));
        }} className={classes(cl("item"), isDragging && cl("dragging"), isOver && cl("drop-target"))}>
            {children}
        </div>);
};
const DraggableList = ({ channelId, draftType, keyboardModeEnabled, size, attachments, ignoredFilename }) => {
    ...
    const moveItem = (fromId, toId) => {
        ...
        let itemIndex = 0;
        const next = attachments.map(attachment => attachment.filename === ignoredFilename ? attachment : nextItems[itemIndex++]);
        attachments.splice(0, attachments.length, ...next);
        UploadManager.setUploads({ uploads: next, channelId, draftType });
        forceUpdate();
    };
    return items.map(uploadItem => (<DraggableItem key={uploadItem.id} uploadItem={uploadItem} moveItem={moveItem}>
            <AttachmentItem channelId={channelId} upload={uploadItem} draftType={draftType} keyboardModeEnabled={keyboardModeEnabled} clip={uploadItem.clip} size={size}/>
        </DraggableItem>));
};
