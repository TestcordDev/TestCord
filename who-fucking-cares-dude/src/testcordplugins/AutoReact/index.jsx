...
function parseSingleEmoji(emojiStr) {
...
function parseContentRules(rulesStr) {
    if (!rulesStr?.trim())
        return [];
    const rules = [];
    for (const part of rulesStr.split("|")) {
        ...
        if (!word || !emojiPart)
            continue;
        const reactions = emojiPart.split(",").map(parseSingleEmoji).filter((e) => e !== null);
        if (reactions.length > 0)
            rules.push({ word, reactions });
    }
    return rules;
}
function parseChannelRules(rulesStr) {
    try {
        const parsed = JSON.parse(rulesStr);
        return Array.isArray(parsed) ? parsed : [];
    }
...
function saveChannelRules(rules) {
    settings.store.channelRules = JSON.stringify(rules);
}
function parseSelfReactEmojis(emojisStr) {
    try {
...
async function addReactionsSequentially(channelId, messageId, reactions) {
    ...
    if (isOwnMessage) {
        ...
        if (settings.store.selfReactEnabled) {
            ...
            if (isTarget) {
                ...
                if (emojis.length > 0)
                    addReactionsSequentially(channelId, messageId, emojis);
            }
        }
        return;
    }
    ...
    if (content) {
        ...
        for (const rule of parseContentRules(settings.store.contentRules)) {
            ...
            if (haystack.includes(needle))
                matched.push(...rule.reactions);
        }
        if (matched.length > 0)
            addReactionsSequentially(channelId, messageId, matched);
    }
...
function EmojiPickerModal(props) {
    ...
    return (<ModalRoot {...props} size={"dynamic" /* ModalSize.DYNAMIC */}>
            <ModalHeader>
            ...
            alignItems: "center"
                    ...
                    {selectedEmojis.length === 0 && (<Forms.FormText style={{ color: "var(--text-muted)" }}>No emojis selected</Forms.FormText>)}
                    {selectedEmojis.map((emoji, i) => (<div key={i} style={{
                position: "relative",
                cursor: "pointer",
            }} onClick={() => removeEmoji(i)} title="Click to remove">
                            {emoji.id
                ? <img src={`https://cdn.discordapp.com/emojis/${emoji.id}.${emoji.animated ? "gif" : "png"}?size=32`} alt={emoji.name} style={{ width: "32px", height: "32px" }}/>
                ...
                <div style={{ marginBottom: "16px" }}>
                    ...
                    <TextInput value={inputValue} onChange={text => {
            setInputValue(text);
            ...
            if (trimmed.length > 0 && trimmed.length <= 10) {
                ...
                setInputValue("");
                    ...
                    <div style={{ marginTop: "8px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        {["👍", "❤️", "😂", "🔥", "👀", "💯", "🎉", "😎", "👌", "💪", "🙌", "✨"].map(emoji => (<button key={emoji} type="button" onClick={() => addEmoji({ name: emoji, id: null, animated: false })} style={{
                fontSize: "20px",
                padding: "4px 8px",
                cursor: "pointer",
                background: "var(--background-secondary)",
            ...
            <ModalFooter>
                <Button color={selectedEmojis.length > 0 ? Button.Colors.GREEN : Button.Colors.RED} onClick={saveRule} style={{ margin: "0 0 0 8px" }}>
                    {selectedEmojis.length > 0 ? "Save Rule" : "Remove Rule"}
                </Button>
                <Button color={Button.Colors.TRANSPARENT} look={Button.Looks.LINK} onClick={onClose}>
                    Cancel
                </Button>
            </ModalFooter>
        </ModalRoot>);
...
function UserContextMenuPatch() {
    return (children, props) => {
        ...
        const openEmojiPicker = () => {
            openModal(modalProps => (<EmojiPickerModal {...modalProps} userId={user.id} username={user.globalName || user.username}/>));
        };
        children.splice(-1, 0, (<Menu.MenuGroup>
                <Menu.MenuItem id="userreact-toggle" label={hasRule ? "Edit AutoReact" : "AutoReact"} action={openEmojiPicker}/>
            </Menu.MenuGroup>));
    };
}
// Emoji Picker Modal for AutoReact Channel
function ChannelEmojiPickerModal(props) {
    ...
    React.useEffect(() => {
        ...
        if (existing)
            setSelectedEmojis([...existing.reactions]);
    }, []);
    const addEmoji = (emoji) => {
        if (!selectedEmojis.find(e => e.name === emoji.name && e.id === emoji.id)) {
            setSelectedEmojis([...selectedEmojis, emoji]);
    ...
    const save = () => {
        ...
        if (selectedEmojis.length === 0) {
            ...
            const rule = { channelId, channelName, reactions: selectedEmojis };
            if (idx !== -1)
                rules[idx] = rule;
            else
                rules.push(rule);
    ...
    return (<ModalRoot {...props} size={"dynamic" /* ModalSize.DYNAMIC */}>
            ...
            alignItems: "center"
                    ...
                    {selectedEmojis.length === 0 && (<Forms.FormText style={{ color: "var(--text-muted)" }}>No emojis selected</Forms.FormText>)}
                    {selectedEmojis.map((emoji, i) => (<div key={i} style={{ position: "relative", cursor: "pointer" }} onClick={() => setSelectedEmojis(selectedEmojis.filter((_, idx) => idx !== i))} title="Click to remove">
                            {emoji.id
                ? <img src={`https://cdn.discordapp.com/emojis/${emoji.id}.${emoji.animated ? "gif" : "png"}?size=32`} alt={emoji.name} style={{ width: "32px", height: "32px" }}/>
                : <span style={{ fontSize: "28px" }}>{emoji.name}</span>}
                            <div style={{
            ...
            <ModalFooter>
                <Button color={selectedEmojis.length > 0 ? Button.Colors.GREEN : Button.Colors.RED} onClick={save} style={{ margin: "0 0 0 8px" }}>
                    {selectedEmojis.length > 0 ? "Save Rule" : "Remove Rule"}
                </Button>
                <Button color={Button.Colors.TRANSPARENT} look={Button.Looks.LINK} onClick={onClose}>
                    Cancel
...
function ChannelContextMenuPatch() {
    return (children, props) => {
        ...
        children.splice(-1, 0, (<Menu.MenuGroup>
                <Menu.MenuItem id="userreact-channel-toggle" label={hasRule ? "Edit AutoReact Channel" : "AutoReact Channel"} action={() => openModal(modalProps => (<ChannelEmojiPickerModal {...modalProps} channelId={channel.id} channelName={channel.name || channel.id}/>))}/>
            </Menu.MenuGroup>));
    };
}
...
function ContentRulesSection() {
    ...
    return (<div style={{ marginTop: "24px" }}>
            ...
            <TextArea style={{ marginBottom: "12px", minHeight: "80px", fontFamily: "monospace" }} value={text} onChange={(value) => {
            setText(value);
            settings.store.contentRules = value;
        }} placeholder="happy:😀,🔥|ok:👌"/>
            {rules.length > 0 && (<div>
                    {rules.map((rule, i) => (<div key={i} style={{
                    padding: "8px",
                    background: "var(--background-secondary)",
                    borderRadius: "4px",
                    marginBottom: "4px"
    ...
    commands: [
        {
            ...
            execute: (args, ctx) => {
                const state = findOption(args, "state", "");
                const emojisStr = findOption(args, "emojis", "");
                const targetUserId = findOption(args, "user", "");
                if (state === "off") {
