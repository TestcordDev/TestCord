...
function SimplePicker({ voice, voices }) {
    const options = voices.map(voice => ({
        label: voice.name,
        value: voice.voiceURI,
        default: voice.default,
    }));
    return (<SearchableSelect placeholder="Select a voice" maxVisibleItems={5} options={options} value={options.find(o => o.value === voice)?.value} onChange={v => settings.store.voice = v} closeOnSelect/>);
}
const languageNames = new Intl.DisplayNames(["en"], { type: "language" });
function ComplexPicker({ voice, voices }) {
    ...
    return (<>
            ...
            <SearchableSelect placeholder="Select a language" options={languageOptions} value={languageOptions.find(l => l.value === selectedLanguage)?.value} onChange={v => setSelectedLanguage(v)} maxVisibleItems={5} closeOnSelect/>
