...
let soloedUserId = null;
let soloedGroupId = null;
let groups = [];
const modifiedUsers = new Set(); // Tracks users we've altered so we can safely restore them even if they leave
...
function isInSameVoiceChannel(userId) {
    const me = getCurrentUserId();
    if (!me)
        return false;
...
function shouldKeepAudible(userId, keepIds) {
    if (keepIds.has(userId))
        return true;
    if (settings.store.friendsOnly && isFriend(userId))
        return true;
    return false;
}
function applyVolumeOrMute(userId, isKept, targetKeptVolume = 100) {
...
function reapplyCurrentSolo() {
    if (soloedUserId) {
        const keep = new Set([soloedUserId]);
        ...
        if (!group)
            return;
        const keep = new Set(group.userIds);
...
function applySoloUser(targetUserId) {
    loadGroups();
...
function applySoloGroup(groupId) {
    loadGroups();
...
function createGroup(name, initialUserId) {
    loadGroups();
    const group = { id: `${Date.now()}`, name, userIds: initialUserId ? [initialUserId] : [], volume: 100 };
    groups.push(group);
    saveGroups();
    return group;
}
...
function SliderRow({ label, value, min, max, step, unit = "px", onChange }) {
    return (<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            ...
            <Slider initialValue={value} minValue={min} maxValue={max} keyboardStep={step} onValueChange={onChange}/>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
...
function Toggle({ label, desc, value, onChange }) {
    return (<div onClick={() => onChange(!value)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", gap: "16px" }}>
...
function SoloAudioModal({ modalProps }) {
    ...
    return (<ModalRoot {...modalProps} size={"small" /* ModalSize.SMALL */}>
            ...
            <ModalContent style={{ padding: "24px 24px 0 24px" }}>
                    ...
                    {tab === "solo" && <>
                        ...
                        <Card>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                {(soloedUserId || soloedGroupId) && (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={() => { clearSolo(); forceUpdate(); }}>⏹ Clear</Button>)}
                            </div>
                        </Card>
                ...
                return (<div key={uid} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                if (name?.trim()) {
                    ...
                    forceUpdate();
                        ...
                        {groups.map(group => (<Card key={group.id}>
                                ...
                                <SliderRow label="Group Volume" value={group.volume ?? 100} min={0} max={200} step={5} unit="%" onChange={v => {
                    group.volume = v;
                    saveGroups();
                    if (soloedGroupId === group.id)
                        reapplyCurrentSolo();
                    forceUpdate();
                }}/>
                    ...
                    return (<Toggle key={uid} label={<div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                                        <img src={pfp} width={24} height={24} style={{ borderRadius: "50%" }}/>
                                                        <span style={{ fontSize: 15, fontWeight: 500 }}>{name}</span>
                                                    </div>} value={inGroup} onChange={v => {
                            if (v)
                                addUserToGroup(group.id, uid);
                            else
                    ...
                    {tab === "settings" && <>
                        ...
                        <Card>
                            <SliderRow label="Background Users Volume" value={s.backgroundVolume} min={0} max={100} step={5} unit="%" onChange={v => set("backgroundVolume", v)}/>
                        ...
                        <Card>
                            <Toggle label="Keep Friends Audible" desc="Your friends' volume will not be lowered when you solo someone else." value={s.friendsOnly} onChange={v => set("friendsOnly", v)}/>
                            <Toggle label="Toggle Off on Re-click" desc="Clicking an active solo target again will disable soloing and restore all audio." value={s.unmuteOnDeselect} onChange={v => set("unmuteOnDeselect", v)}/>
                        </Card>
                    </>}
...
function QuickSoloAudioMenu({ onClose }) {
    ...
    loadGroups();
    const peers = getVoiceChannelPeers();
    return (<Menu.Menu navId="solo-audio" onClose={onClose} aria-label="Solo Audio Quick Menu">
            ...
            {groups.length > 0 && (<Menu.MenuItem id="solo-audio-groups" label="Solo Group">
                    {groups.map(g => (<Menu.MenuItem key={g.id} id={`solo-audio-group-${g.id}`} label={soloedGroupId === g.id ? `✓ ${g.name}` : g.name} action={() => { applySoloGroup(g.id); rerender(); }}/>))}
                </Menu.MenuItem>)}

            {peers.length > 0 && (<Menu.MenuItem id="solo-audio-users" label="Solo User">
                    {peers.map(userId => {
...
function SoloAudioIcon({ className, style, active }) {
    const lineLength = 30;
    const lineStyle = {
    ...
    return (<svg className={className} style={style} width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
...
function SoloAudioButtonFinal(props) {
    const isActive = soloedUserId !== null || soloedGroupId !== null;
    return (<UserAreaButton onClick={() => openModal(modalProps => <SoloAudioModal modalProps={modalProps}/>)} onContextMenu={event => ContextMenuApi.openContextMenu(event, () => <QuickSoloAudioMenu onClose={ContextMenuApi.closeContextMenu}/>)} role="switch" aria-checked={isActive} redGlow={!isActive} tooltipText={props.hideTooltips ? void 0 : isActive ? "Solo Active (Right-click for options)" : "Solo Audio"} icon={<SoloAudioIcon className={props.iconForeground} active={isActive}/>} plated={props.nameplate != null}/>);
}
const userContextMenuPatch = (children, { user }) => {
    ...
    if (user.id === me)
        return;
    if (!isInSameVoiceChannel(user.id))
        return;
    loadGroups();
    ...
    children.push(<Menu.MenuSeparator />, <Menu.MenuItem id="vc-solo-audio" label={isSoloed ? "✓ Solo Audio" : "Solo Audio"} action={() => applySoloUser(user.id)}/>, <Menu.MenuItem id="vc-solo-groups" label="Audio Groups">
            {notInGroups.length > 0 && (<Menu.MenuItem id="vc-solo-group-add" label="Add to Group">
                    {notInGroups.map(g => (<Menu.MenuItem key={g.id} id={`vc-solo-group-add-${g.id}`} label={g.name} action={() => addUserToGroup(g.id, user.id)}/>))}
                </Menu.MenuItem>)}
            {inGroups.length > 0 && (<Menu.MenuItem id="vc-solo-group-remove" label="Remove from Group">
                    {inGroups.map(g => (<Menu.MenuItem key={g.id} id={`vc-solo-group-remove-${g.id}`} label={g.name} action={() => removeUserFromGroup(g.id, user.id)}/>))}
                </Menu.MenuItem>)}
            <Menu.MenuItem id="vc-solo-group-create" label="＋ New Group with this User" action={() => {
            const name = prompt("Group name:");
            if (name?.trim())
                createGroup(name.trim(), user.id);
        }}/>
        </Menu.MenuItem>);
};
