...
import { getDiscordFolderIcon, isBookmarkFolder, isTabSelected, navigateToBookmark, openedTabs, settings, switchChannel, useBookmarks } from "@equicordplugins/channelTabs/util";
...
function LibraryIcon({ height = 16, width = 16 }) {
    return (<svg viewBox="0 0 24 24" height={height} width={width} fill="none">
...
function FolderIcon({ fill }) {
    return (<path fill={fill} d="M20 7H12L10.553 5.106C10.214 4.428 9.521 4 8.764 4H3C2.447 4 2 4.447 2 5V19C2 20.104 2.895 21 4 21H20C21.104 21 22 20.104 22 19V9C22 7.896 21.104 7 20 7Z"/>);
}
function BookmarkIcon({ bookmark }) {
    if (isBookmarkFolder(bookmark)) {
        ...
        return IconComponent
            ? <IconComponent height={customIconSize} width={customIconSize} color={bookmark.iconColor} fill={fill} style={{ transform: "scale(0.6666667)", transformOrigin: "center" }}/>
    ...
    if (guild)
        return guild.icon
            ? <img src={`https://${window.GLOBAL_ENV.CDN_HOST}/icons/${guild?.id}/${guild?.icon}.png`} className={cl("bookmark-icon")}/>
            : <div className={cl("bookmark-icon")}>
            <BaseText size="xxs" weight="semibold" tag="span">{getGuildAcronym(guild)}</BaseText>
        </div>;
    if (channel?.recipients?.length) {
        if (channel.recipients.length === 1)
            return (<Avatar size="SIZE_16" src={UserStore.getUser(channel.recipients[0]).getAvatarURL(undefined, 128)}/>);
        else
            return (<img src={channel.icon
...
function BookmarkFolderOpenMenu(props) {
    const { bookmarks, index, methods } = props;
    const bookmark = bookmarks[index];
    const { bookmarkNotificationDot } = settings.use(["bookmarkNotificationDot"]);
    return (<Menu.Menu navId="bookmark-folder-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Bookmark Folder Menu">
            {bookmark.bookmarks.map((b, i) => (<Menu.MenuItem key={`bookmark-folder-entry-${b.channelId}`} id={`bookmark-folder-entry-${b.channelId}`} label={<div style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "0.25rem"
                }}>
                    ...
                    textOverflow: "ellipsis"
                        ...
                        </div>} icon={() => <BookmarkIcon bookmark={b}/>} showIconFirst={true} action={() => switchChannel(b)}>
                    {bookmarkNotificationDot && (<Menu.MenuGroup>
                            <Menu.MenuItem key="mark-as-read" id="mark-as-read" label={getIntlMessage("MARK_AS_READ")} disabled={!ReadStateStore.hasUnread(b.channelId)} action={() => ReadStateUtils.ackChannel(ChannelStore.getChannel(b.channelId))}/>
                        </Menu.MenuGroup>)}
                    <Menu.MenuGroup key="bookmarks">
                        <Menu.MenuItem key="edit-bookmark" id="edit-bookmark" label="Edit Bookmark" action={() => {
                const key = openModal(modalProps => <EditModal modalProps={modalProps} modalKey={key} bookmark={b} onSave={name => {
                        ...
                        <Menu.MenuItem key="delete-bookmark" id="delete-bookmark" label="Delete Bookmark" action={() => {
                methods.deleteBookmark(i, index);
            }}/>
                        <Menu.MenuItem key="remove-bookmark-from-folder" id="remove-bookmark-from-folder" label="Remove Bookmark from Folder" action={() => {
                const newBookmarks = [...bookmark.bookmarks];
...
function FolderBookmarkItem({ bookmark, bookmarks, folderIndex, bookmarkIndex, methods, isCurrentChannel, searchActive }) {
    ...
    const [{ isOver: isReorderOver, canDrop: canReorderDrop }, drop] = useDrop(() => ({
        ...
        hover: (item, monitor) => {
            if (!ref.current || searchActive || !item.isFromFolder || item.folderIndex !== folderIndex)
                return;
    ...
    useEffect(() => {
        if (!isReorderOver && dropSide)
            setDropSide(null);
    }, [dropSide, isReorderOver]);
    drag(drop(ref));
    return (<div ref={ref} className={classes(cl("bookmark", "folder-item", "hoverable"), isCurrentChannel && cl("bookmark-active"), isDragging && cl("bookmark-dragging"), isReorderOver && canReorderDrop && cl("bookmark-drop-target"), isReorderOver && canReorderDrop && dropSide === "before" && cl("bookmark-drop-before"), isReorderOver && canReorderDrop && dropSide === "after" && cl("bookmark-drop-after"))} onClick={() => navigateToBookmark(bookmark)} onContextMenu={e => {
            e.stopPropagation();
            ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="folder-item-context" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })}>
                        <Menu.MenuItem key="edit-bookmark" id="edit-bookmark" label="Edit Bookmark" action={() => {
                    const key = openModal(modalProps => <EditModal modalProps={modalProps} modalKey={key} bookmark={bookmark} onSave={name => {
                            const folder = bookmarks[folderIndex];
                            if (!isBookmarkFolder(folder))
                                return;
                            const newBookmarks = [...folder.bookmarks];
                        ...
                        <Menu.MenuItem key="delete-bookmark" id="delete-bookmark" label="Delete Bookmark" action={() => methods.deleteBookmark(bookmarkIndex, folderIndex)}/>
                        <Menu.MenuItem key="remove-from-folder" id="remove-from-folder" label="Remove from Folder" action={() => {
                    methods.addBookmark(bookmark);
                    methods.deleteBookmark(bookmarkIndex, folderIndex);
                }}/>
                        {bookmarkNotificationDot && ReadStateStore.hasUnread(bookmark.channelId) && (<Menu.MenuItem key="mark-as-read" id="mark-as-read" label={getIntlMessage("MARK_AS_READ")} action={() => ReadStateUtils.ackChannel(ChannelStore.getChannel(bookmark.channelId))}/>)}
                    </Menu.Menu>));
        }}>
...
function Bookmark(props) {
    ...
    const [{ isDragging }, drag] = useDrag(() => ({
        ...
        item: () => {
            ...
            const currentIndex = bookmarks.findIndex(b => isBookmarkFolder(b) ?
    ...
    const [{ isOver: isReorderOver, canDrop: canReorderDrop }, drop] = useDrop(() => ({
        ...
        hover: (item, monitor) => {
            if (!ref.current || searchActive || item.isFromFolder)
                return;
            const dragIndex = item.index;
    ...
    const [{ isOver: isFolderDropTarget }, folderDrop] = useDrop(() => ({
        ...
        drop: (item, monitor) => {
            ...
            if (itemType === "vc_Bookmark") {
                ...
                if (isBookmarkFolder(sourceBookmark))
                    return;
                // skip if already in this folder
                if (item.isFromFolder && item.folderIndex === index)
                    return;
    ...
    return (<div className={classes(cl("bookmark", "hoverable", { wider: settings.store.widerTabsAndBookmarks }), isBookmarkFolder(bookmark) && cl("bookmark-folder"), isFolderDropTarget && cl("folder-drop-target"), isExpanded && cl("folder-expanded"), isCurrentChannel && cl("bookmark-active"), isDragging && cl("bookmark-dragging"), isReorderOver && canReorderDrop && cl("bookmark-drop-target"), isReorderOver && canReorderDrop && dropSide === "before" && cl("bookmark-drop-before"), isReorderOver && canReorderDrop && dropSide === "after" && cl("bookmark-drop-after"))} ref={ref} onClick={e => {
            if (isBookmarkFolder(bookmark)) {
                onToggleFolder?.();
            }
            else {
            ...
            <BookmarkIcon bookmark={bookmark}/>
            {isBookmarkFolder(bookmark) && (<svg width="12" height="12" viewBox="0 0 24 24" style={{ marginLeft: "0.25rem" }}>
...
export function HorizontalScroller({ children, className, customRef }) {
    ...
    return (<div className={classes(cl("scroller"), className)} ref={combinedRef}>
            {children}
        </div>);
}
export default function BookmarkContainer(props) {
    ...
    bookmarks?.forEach((bookmark, i) => {
        ...
        else if (bookmark.channelId === channelId)
            isCurrentChannelBookmarked = true;
    });
    ...
    const matchesSearch = (value) => value?.toLowerCase().includes(normalizedSearchQuery) ?? false;
    return (<div className={cl("bookmark-container")}>
            ...
            bookmarks.flatMap((bookmark, i) => {
                if (!isBookmarkFolder(bookmark)) {
                    if (searchActive && !matchesSearch(bookmark.name))
                        return [];
                    return [
                        <Bookmark key={i} index={i} bookmarks={bookmarks} methods={methods} searchActive={searchActive} isExpanded={false} isCurrentChannel={bookmarksIndependentFromTabs &&
                ...
                if (searchActive && !folderMatches && !visibleFolderBookmarks.length)
                    return [];
                const isExpanded = searchActive
                    ? visibleFolderBookmarks.length > 0
                    : expandedFolders.has(i);
                const folderItems = [
                    <Bookmark key={i} index={i} bookmarks={bookmarks} methods={methods} searchActive={searchActive} isExpanded={isExpanded} onToggleFolder={searchActive ? undefined : () => toggleFolder(i)} isCurrentChannel={false}/>
                ];
                if (isExpanded) {
                    visibleFolderBookmarks.forEach(({ childBookmark, childIndex }) => {
                        folderItems.push(<FolderBookmarkItem key={`${i}-${childIndex}`} bookmark={childBookmark} bookmarks={bookmarks} folderIndex={i} bookmarkIndex={childIndex} methods={methods} searchActive={searchActive} isCurrentChannel={bookmarksIndependentFromTabs &&
                                !channelMatchesActiveTab &&
            ...
            <div className={classes(cl("bookmark-search-shell"), isSearchOpen && cl("bookmark-search-shell-open"))}>
                <div className={cl("bookmark-search-field")}>
                    <TextInput inputRef={searchInputRef} inputClassName={cl("bookmark-search-input")} style={{ width: "100%" }} value={searchQuery} onChange={setSearchQuery} placeholder="Search bookmarks" onBlur={() => {
            if (!searchQuery.trim())
            ...
            if (searchQuery.trim()) {
                ...
                <Tooltip text="Search bookmarks" position="left">
                    {p => <button className={classes(cl("button"), cl("bookmark-search-button"))} {...p} onClick={() => {
            ...
            <Tooltip text={isCurrentChannelBookmarked ? "Remove from Bookmarks" : "Add to Bookmarks"} position="left">
                ...
                if (isCurrentChannelBookmarked) {
                    if (currentChannelFolderIndex === -1)
                        methods.deleteBookmark(bookmarks.findIndex(b => !(isBookmarkFolder(b)) && b.channelId === channelId));
                    else
                        methods.deleteBookmark(bookmarks[currentChannelFolderIndex].bookmarks
                            .findIndex(b => b.channelId === channelId), currentChannelFolderIndex);
                }
                else
                    methods.addBookmark({
                        guildId,
                    ...
                    <StarIcon height={20} width={20} colorClass={isCurrentChannelBookmarked ? cl("bookmark-star-checked") : cl("bookmark-star")}/>
                </button>}
            </Tooltip>
        </div>);
}
