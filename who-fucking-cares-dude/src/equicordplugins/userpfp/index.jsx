...
export default definePlugin({
    ...
    settingsAboutComponent: () => (<>
            ...
            <Flex className={cl("settings")}>
                ...
                <Button variant="secondary" className={cl("settings-button")} onClick={() => VencordNative.native.openExternal(DONO_URL)}>
    ...
    contextMenus: {
        "user-context": (children, { user }) => {
            if (!user?.id)
                return;
            children.push(<Menu.MenuSeparator />, <Menu.MenuItem label="Set Avatar" id="set-avatar" icon={PencilIcon} action={async () => {
    ...
    getAvatarHook: (original) => (user, animated, size) => {
        if (settings.store.preferNitro && user.avatar?.startsWith("a_"))
            return original(user, animated, size);
        if (!data.avatars[user.id])
            return original(user, animated, size);
    ...
    getAvatarServerHook: (original) => (config) => {
    ...
    async start() {
        data.avatars = await get(KEY_DATASTORE) || {};
