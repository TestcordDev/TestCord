...
const DEBOUNCE = 1500;
function PinNotice() {
    return (<div className="vc-voiceguard-note" dir={isArabicMode() ? "rtl" : "ltr"}>
            {t("⚠️ خيار «العودة للقناة المثبّتة إن تم نقلك» لا يعمل إلا بعد تثبيت القناة الصوتية: انقر عليها بالزرّ الأيمن ثم اختر «تثبيت القناة».", "⚠️ \"Jump back to the pinned channel if you get moved\" only works once you pin the voice channel: right-click it and choose \"Pin channel\".")}
        </div>);
}
...
function undoServerState(guildId, myId, key) {
...
function rejoin(channelId, message) {
...
function handleVoiceStateUpdate(voiceStates) {
    ...
    if (!mine)
        return; // the event is about other people
    // Remember where we are the moment we see it, so a later disconnect always
    // has somewhere to return to.
    if (mine.channelId)
        lastChannelId = mine.channelId;
    if (busy || Date.now() - lastActionAt < DEBOUNCE)
        return;
    ...
    if (settings.store.stayInChannel && pinnedChannelId && mine.channelId !== pinnedChannelId) {
        rejoin(pinnedChannelId, t("عُدتَ إلى القناة المثبّتة.", "Moved back to the pinned channel."));
        return;
    }
...
function pinChannelContextMenu(children, { channel }) {
    // 2 = guild voice, 13 = stage
    if (!channel || (channel.type !== 2 && channel.type !== 13))
        return;
    const pinned = pinnedChannelId === channel.id;
    children.push(<Menu.MenuItem id="voiceguard-pin" label={pinned ? t("إلغاء تثبيت القناة", "Unpin channel") : t("تثبيت القناة", "Pin channel")} action={() => {
