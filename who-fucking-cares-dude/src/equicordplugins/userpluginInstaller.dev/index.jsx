...
// @ts-ignore
export const Native = VencordNative.pluginHelpers.UserpluginInstaller;
...
export default definePlugin({
    ...
    tags: ["Developers"],
    settingsAboutComponent: () => (<Notice.Warning>
    ...
    async start() {
        if (!VencordNative.pluginHelpers.UserpluginInstaller)
            return void Alerts.show({
    ...
    renderMessageAccessory: props => {
        if (!props?.message?.content)
            return null;
        if (!CLONE_LINK_REGEX.test(props.message.content))
            return null;
