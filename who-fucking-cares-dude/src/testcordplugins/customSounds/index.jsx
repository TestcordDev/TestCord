...
import { dataUriCache, deleteAudio, ensureDataURICached, getAllAudio, getAudioMeta, importAudio, playAudio as playSound, saveAudio } from "./audio";
import { makeEmptyOverride, soundTypes } from "./types";
const cap = (s) => s.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
const audioType = (a) => a.startsWith("data:") || a.startsWith("http") || a.startsWith("blob:") ? "url" : "discord";
const seasonalUrls = Object.fromEntries(soundTypes.flatMap(t => t.seasonal ? Object.entries(t.seasonal) : []));
let audioCtx = null;
...
function setBoost(audio, volume) {
    const factor = Math.max(1, volume);
    if (factor <= 1.001 && !boostNodes.has(audio))
        return;
    try {
    ...
    if (!nodes) {
        try {
            ...
            console.error("[CustomSounds] Web Audio attach failed:", e);
            return;
        }
    }
    nodes.gain.gain.value = factor;
}
function clearBoost(audio) {
...
function setOverride(id, o) { settings.store[id] = JSON.stringify(o); }
async function cacheCustom(id) {
    if (!id)
        return;
    try {
        ...
        showToast("Custom sound load error");
    }
}
const soundSettings = Object.fromEntries(soundTypes.map(t => [t.id, { type: 0 /* OptionType.STRING */, description: `Override for ${t.name}`, default: JSON.stringify(makeEmptyOverride()), hidden: true }]));
const settings = definePluginSettings({ ...soundSettings, overrides: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsUI /> } });
function SoundCard({ type, override, files, onFilesChange, onChange }) {
    ...
    const previewSound = async () => {
        ...
        if (selectedSound === "custom" && selectedFileId) {
            ...
            if (!dataUri?.startsWith("data:audio/")) {
                showToast("No custom sound file available");
                return;
            }
            sound.current = playSound(dataUri, { volume });
        }
        else
            sound.current = playSound(selectedSound === "default" ? type.id : selectedSound, { volume });
    };
    const uploadFile = async (event) => {
        ...
        try {
            ...
            console.error("[CustomSounds] Upload failed:", e);
    ...
    const deleteFile = async (id) => {
        try {
            ...
            console.error("[CustomSounds] Delete failed:", e);
    ...
    return (<Card style={{ padding: "1em 1em 0" }}>
            <FormSwitch title={type.name} value={override.enabled || false} className={Margins.bottom16} hideBorder onChange={async (val) => { override.enabled = val; if (val && override.selectedSound === "custom")
        await cacheCustom(override.selectedFileId); await saveAndNotify(); }}/>
            {override.enabled && <>
                ...
                <Slider minValue={0} maxValue={500} markers={makeRange(0, 500, 50)} initialValue={override.volume} onValueRender={(v) => `${Math.round(v)}%`} className={Margins.bottom16} onValueChange={val => { override.volume = val; setOverride(type.id, override); if (sound.current)
            sound.current.volume = val; saveAndNotify(); }}/>
                <Heading className={Margins.bottom8}>Sound Source</Heading>
                <div style={{ marginBottom: 16 }}>
                    <Select closeOnSelect serialize={v => v} isSelected={v => v === override.selectedSound} options={sourceOpts} select={async (v) => { override.selectedSound = v; if (v === "custom")
            await cacheCustom(override.selectedFileId); await saveAndNotify(); }}/>
                </div>
                {override.selectedSound === "custom" && <>
                    <Heading className={Margins.bottom8}>Custom File</Heading>
                    <div style={{ marginBottom: 16 }}>
                        <Select closeOnSelect serialize={v => v} isSelected={v => v === (override.selectedFileId || "")} options={[{ value: "", label: "Select a file..." }, ...fileOpts]} select={async (id) => { override.selectedFileId = id || undefined; if (id)
                await ensureDataURICached(id); await saveAndNotify(); }}/>
                    </div>
...
function SettingsUI() {
    ...
    React.useEffect(() => {
        soundTypes.forEach(t => { if (!settings.store[t.id])
            setOverride(t.id, makeEmptyOverride()); });
        loadFiles();
    }, []);
    ...
    const handleSettingsUpload = (e) => {
        ...
        reader.onload = async (ev) => {
            try {
                ...
                console.error("[CustomSounds] Import error:", er);
    ...
    const downloadSettings = async () => {
        ...
        const refs = new Set(overrides.map(o => o.selectedFileId).filter(Boolean));
        const all = await getAllAudio();
        const bundled = [...refs].map(id => all[id]).filter(f => f?.dataUri).map(f => ({ id: f.id, name: f.name, type: f.type, dataUri: f.dataUri }));
    ...
    return (<div>
            ...
            return <SoundCard key={`${type.id}-${resetTrigger}`} type={type} override={o} files={files} onFilesChange={loadFiles} onChange={async () => { setOverride(type.id, o); if (o.enabled && o.selectedSound === "custom")
                await cacheCustom(o.selectedFileId); }}/>;
        })}
            </div>
        </div>);
}
export default definePlugin({
    ...
    buildPlayer(player, options = {}, audio, _u, internalVolume, channel) {
    ...
    processAudio(player) {
        ...
        if (cur.volume !== player.preprocessDataPrevious?.volume)
            player._audio?.then(audio => { audio.volume = player._volume; setBoost(audio, cur.volume); });
        if (cur.speed !== player.preprocessDataPrevious?.speed)
            player._audio?.then(audio => { audio.playbackRate = player._speed; });
    },
    stopAudio(player, restart) {
        if (restart)
            player.ensureAudio().then(audio => { audio.currentTime = 0; audio.play(); });
        else if (!player.persistent)
            player.destroyAudio();
        else
            player._audio?.then(audio => { audio.pause(); audio.currentTime = 0; });
    },
    playAudio(audio) { playSound(audio); },
    applyBoost(player, audio) { setBoost(audio, player.preprocessDataCurrent?.volume ?? 1); },
    cleanupBoost(audio) { clearBoost(audio); },
    async start() {
        for (const t of soundTypes) {
            const o = getOverride(t.id);
            if (o?.enabled && o.selectedSound === "custom" && o.selectedFileId) {
                try {
                    await ensureDataURICached(o.selectedFileId);
                }
                catch (e) {
                    console.error("[CustomSounds]", e);
                }
