...
import { getNewPlugins, getNewSettings, writeKnownSettings } from "./knownSettings";
const cl = classNameFactory("vc-new-plugins-");
let hasSeen = false;
function NewPluginsModal({ modalProps, newPlugins, newSettings }) {
    const settings = useSettings();
    const changes = useMemo(() => new ChangeList(), []);
    ...
    const sortedPlugins = useMemo(() => {
        const mapPlugins = (array) => array.map(pn => Plugins[pn]).sort((a, b) => a.name.localeCompare(b.name));
        return [
            ...mapPlugins([...newPlugins]),
            ...mapPlugins([...newSettings.keys()].filter(p => !newPlugins.has(p)))
        ];
    }, []);
    const onRestartNeeded = (name) => {
        changes.handleChange(name);
        forceUpdate();
    };
    const pluginCards = [];
    const requiredPluginCards = [];
    for (const p of sortedPlugins) {
        ...
        if (isRequired) {
            ...
            requiredPluginCards.push(<Tooltip text={tooltipText} key={p.name}>
                    {({ onMouseLeave, onMouseEnter }) => (<PluginCard onMouseLeave={onMouseLeave} onMouseEnter={onMouseEnter} onRestartNeeded={onRestartNeeded} disabled={true} plugin={p} isNew={newPlugins.has(p.name)}/>)}
                </Tooltip>);
        }
        else {
            pluginCards.push(<PluginCard onRestartNeeded={onRestartNeeded} disabled={false} plugin={p} key={p.name} isNew={newPlugins.has(p.name)}/>);
        }
    }
    ...
    return (<Modal {...modalProps} size="md" title={<div className={cl("header-content")}>
                    ...
                    <Notice.Info className={cl("notice")}>
                        Testcord is Open Source Software. If you enjoy using it, consider supporting us <Link href="https://ko-fi.com/x2bkaneki" target="_blank" rel="noopener noreferrer">here</Link>.
                    </Notice.Info>
                </>} actions={[
            {
