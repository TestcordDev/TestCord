...
export function extractSnowflakeFromString(value) {
    const match = value.match(/\d{17,20}/);
    return match?.[0] ?? null;
}
export function extractSnowflakes(values) {
    const ids = new Set();
...
export function extractStrings(dataTransfer) {
    const collected = new Set();
    const add = (v) => v && collected.add(v);
...
export async function collectPayloadStrings(dataTransfer) {
    const sync = extractStrings(dataTransfer);
    const asyncValues = [];
    const itemPromises = Array.from(dataTransfer.items ?? [])
        .filter(item => item.kind === "string")
        .map(item => new Promise(resolve => item.getAsString(val => {
        if (val)
            asyncValues.push(val);
        resolve();
    })));
    await Promise.all(itemPromises);
    return Array.from(new Set([...sync, ...asyncValues]));
}
export function serializeDragEntity(entity) {
    return JSON.stringify(entity);
}
export function parseDragifyPayload(value) {
...
export function parseFromStrings(payloads, stores) {
    ...
    for (const candidate of candidates) {
        if (stores.ChannelStore.getChannel(candidate))
            return { kind: "channel", id: candidate };
        if (stores.GuildStore.getGuild(candidate))
            return { kind: "guild", id: candidate };
        if (stores.UserStore.getUser(candidate))
            return { kind: "user", id: candidate };
    }
    return null;
}
export function extractChannelPath(value) {
...
export function extractChannelFromUrl(value) {
    const channelFromUrl = channelUrlRegex.exec(value);
    if (!channelFromUrl)
        return null;
...
export function extractUserFromAvatar(value) {
    const guildUserAvatar = guildUserAvatarRegex.exec(value);
    if (guildUserAvatar?.[1])
        return guildUserAvatar[1];
    const userAvatar = userAvatarRegex.exec(value);
    if (userAvatar?.[1])
        return userAvatar[1];
    return null;
}
export function extractUserFromProfile(value) {
