...
function processQueue() {
    if (!pluginActive || claiming || !codeQueue.length)
        return;
    claiming = true;
    ...
    GiftActions.redeemGiftCode({
        code,
        onRedeemed: (gift) => {
            if (!pluginActive || claimGeneration !== generation)
                return;
...
export default definePlugin({
    ...
    flux: {
        MESSAGE_CREATE({ message }) {
            if (!message.content)
                return;
            if (!message.content.includes("discord.gift") && !message.content.includes("discord.com/gift"))
                return;
            const match = message.content.match(/(?:discord\.gift\/|discord\.com\/gifts?\/)([a-zA-Z0-9]{16,24})/);
            if (!match)
                return;
            if (new Date(message.timestamp).getTime() < startTime)
                return;
