...
function FileSplitterModal({ modalProps }) {
    ...
    const splitFile = async () => {
        ...
        try {
            ...
            const newChunks = [];
            console.log(`🎯 File Splitter: Splitting ${file.name} (${(file.size /
                1024 /
                1024).toFixed(2)} MB) into ${totalChunks} chunks`);
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            <ModalContent>
                <div style={{ padding: "16px" }}>
                    ...
                    <div style={{ marginBottom: "16px" }}>
                        <input type="file" onChange={handleFileSelect} style={{ marginBottom: "8px" }}/>
                        {file && (<BaseText size="sm" style={{ color: "var(--text-muted)" }}>
                                Selected file: {file.name} (
                                {(file.size / 1024 / 1024).toFixed(2)} MB)
                            </BaseText>)}
                    </div>

                    {file && (<div style={{ marginBottom: "16px" }}>
                            <Button onClick={splitFile} disabled={isProcessing} color={Button.Colors.BRAND}>
                                {isProcessing ? "Splitting in progress..." : "Split file"}
                            </Button>
                        </div>)}

                    {isProcessing && (<div style={{ marginBottom: "16px" }}>
                ...
                transition: "width 0.3s ease",
                    ...
                    {chunks.length > 0 && (<div>
                            ...
                            <div style={{ marginBottom: "16px" }}>
                                <Button onClick={downloadAllChunks} color={Button.Colors.GREEN} style={{ marginRight: "8px" }}>
                                    📥 Download all
                                </Button>
                                <Button onClick={generateRejoinScript} color={Button.Colors.PRIMARY} look={Button.Looks.OUTLINED}>
                                    🔧 Rejoin script
                                </Button>
                            </div>

                            <div style={{ maxHeight: "200px", overflowY: "auto" }}>
                                {chunks.map(chunk => (<div key={chunk.index} style={{
                    display: "flex",
                    ...
                    marginBottom: "4px",
                                        ...
                                        <Button onClick={() => downloadChunk(chunk)} size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} look={Button.Looks.LINK}>
                                            Download
                                        </Button>
                                    </div>))}
                            </div>
            ...
            <ModalFooter>
                <Flex justifyContent="flex-end">
                    <Button onClick={modalProps.onClose} color={Button.Colors.PRIMARY} look={Button.Looks.OUTLINED}>
                        Close
                    </Button>
                </Flex>
            </ModalFooter>
...
export default definePlugin({
    name: "File Splitter",
    description: "Split large files into multiple parts to bypass size limits and share them easily.",
    tags: ["Utility", "Chat"],
