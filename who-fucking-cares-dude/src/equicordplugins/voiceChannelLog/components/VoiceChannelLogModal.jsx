...
export function openVoiceChannelLog(channel) {
    return openModal(props => (<VoiceChannelLogModal props={props} channel={channel}/>));
}
export function VoiceChannelLogModal({ channel, props }) {
    const logs = React.useSyncExternalStore(vcLogSubscribe, () => getVcLogs(channel.id));
    return (<Modal {...props} size="lg" title={`${channel.name} logs`} actions={[
            {
                text: "Clear logs",
            ...
            if (i === 0 || entry.timestamp.toDateString() !== logs[i - 1].timestamp.toDateString()) {
                elements.push(<div key={`sep-${i}`} className={cl("date-separator")} role="separator" aria-label={entry.timestamp.toDateString()}>
                                <span>{entry.timestamp.toDateString()}</span>
                            </div>);
            }
            elements.push(<VoiceChannelLogEntryComponent key={`entry-${i}`} logEntry={entry} channel={channel}/>);
            return elements;
