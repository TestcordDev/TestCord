...
const removedBadgesMap = new Map();
...
async function loadData() {
    ...
    if (savedRemovedBadges) {
        Object.entries(savedRemovedBadges).forEach(([userId, badgeIds]) => {
            removedBadgesMap.set(userId, new Set(badgeIds));
        });
    }
}
async function saveData() {
    ...
    const removedBadgesObject = Object.fromEntries(Array.from(removedBadgesMap.entries()).map(([userId, badgeIds]) => [userId, Array.from(badgeIds)]));
    await DataStore.set("removedBadges", removedBadgesObject);
}
function addBadgesToUser(userId, badges) {
...
function hideRealBadge(userId, badgeId) {
...
function unhideRealBadge(userId, badgeId) {
    if (removedBadgesMap.has(userId)) {
...
function isRealBadgeHidden(userId, badgeId) {
    return removedBadgesMap.has(userId) && removedBadgesMap.get(userId).has(badgeId);
}
function toggleBadge(userId, badge, hasActualBadge) {
...
export default definePlugin({
    ...
    contextMenus: {
        "user-context"(children, { user }) {
            ...
            children.push(<Menu.MenuItem label="Add Badge" key="add-custom-badge" id="user-context-add-custom" color="brand" action={() => addUserBadge(user.id)}/>, <Menu.MenuItem label="Manage Badges" key="custom-badges" id="user-context-custom-badges">
                    ...
                    return (<Menu.MenuCheckboxItem key={badge.id} id={`badge-${badge.id}`} label={badge.title} checked={isChecked} action={() => toggleBadge(user.id, badge, !!hasActualBadge)}/>);
                })}
                    <Menu.MenuSeparator />
                    <Menu.MenuItem label="Give All Badges" id="give-all-badges" color="brand" action={() => giveAllBadges(user.id)}/>
                    <Menu.MenuItem label="Remove All Badges" id="remove-all-badges" color="danger" action={() => {
                    ...
                    <Menu.MenuItem label="Reset to Original" id="reset-badges" color="default" action={() => {
                    userBadgesMap.delete(user.id);
