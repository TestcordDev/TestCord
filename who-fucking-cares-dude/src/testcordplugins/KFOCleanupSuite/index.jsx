...
const deleteStates = {};
function getDeleteState(channelId) {
...
async function searchMessages(channelId, authorId, guildId, offset = 0) {
    const params = {
        include_nsfw: true,
        offset,
...
async function deleteOne(channelId, msgId) {
...
async function runCleanup(ctx, mode, extra) {
    ...
    try {
        ...
        while (state.running && empty < maxRounds) {
            if (maxTotal > 0 && state.count >= maxTotal)
                break;
            ...
            for (const msg of msgs.slice(0, settings.store.maxBatch)) {
                ...
                if (isOld(msg))
                    continue;
                if (isFromProtected(msg))
                    continue;
                if (isFromIgnoredBot(msg))
                    continue;
...
async function runDelete(ctx, mode) {
    const channelId = ctx.channel.id;
    ...
    try {
        ...
        while (state.running && emptyCount < maxRounds) {
            ...
            let msgs;
            try {
                msgs = await searchMessages(channelId, mode === "mine" ? UserStore.getCurrentUser().id : undefined, guildId, offset);
            }
            catch (e) {
            ...
            for (const msg of msgs.slice(0, settings.store.maxBatch)) {
                ...
                if (msg.channel_id !== channelId)
                    continue;
                if (mode === "mine" && msg.author?.id !== UserStore.getCurrentUser().id)
                    continue;
                await sleep(settings.store.deleteDelay);
                try {
                    await deleteOne(channelId, msg.id);
