...
const TOGGLE_KEYS = ["rewriteEnabled"];
...
async function rewriteMessage(systemPrompt, text) {
    return testcordChat({
...
async function tsunderify(text) {
    ...
    try {
        ...
        if (!rewritten || rewritten.length > 1900 || rewritten.length > text.length * 3)
            return text;
        return rewritten;
    }
    catch (error) {
...
function TsundereTalkIcon({ height = 20, width = 20, className }) {
...
const TsundereTalkButton = ErrorBoundary.wrap(function TsundereTalkButton() {
    ...
    return (<HeaderBarButton icon={TsundereTalkIcon} tooltip={rewriteEnabled ? "Tsundere Talk: On" : "Tsundere Talk: Off"} aria-label="Toggle Tsundere Talk" selected={rewriteEnabled} onClick={() => {
...
export default definePlugin({
    ...
    async onBeforeMessageSend(_channelId, message) {
        if (!settings.store.rewriteEnabled)
            return;
