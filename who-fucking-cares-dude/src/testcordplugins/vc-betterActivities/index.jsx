...
const ActivityTooltip = ({ activity, application, user }) => {
...
function getActivityApplication(activity) {
    if (!activity)
        return undefined;
...
function getApplicationIcons(activities, preferSmall = false) {
...
export default definePlugin({
    ...
    patchActivityList: ({ activities, user }) => {
        ...
        const addActivityIcon = (activityName, IconComponent) => {
        ...
        if (icons.length) {
            ...
            return <ErrorBoundary noop>
                <div className={cl("row")}>
                    {icons.map(({ iconElement, tooltip }, i) => (<div key={i} className={cl("icon")} style={iconStyle}>
                            {tooltip ? <Tooltip text={tooltip}>
                                {({ onMouseEnter, onMouseLeave }) => (<div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
                                        {iconElement}
                                    </div>)}
                            </Tooltip> : iconElement}
                        </div>))}
    ...
    showAllActivitiesComponent({ activity, user, ...props }) {
        const currentUser = UserStore.getCurrentUser();
        if (!currentUser)
            return null;
        const [currentActivity, setCurrentActivity] = useState(activity?.type !== 4 ? activity : null);
        const activities = useStateFromStores([PresenceStore], () => PresenceStore.getActivities(user.id).filter((activity) => activity.type !== 4)) ?? [];
        useEffect(() => {
        ...
        const generalProps = useMemo(() => Object.keys(props).reduce((acc, key) => {
            ...
            if (key !== "renderActions" && key !== "application")
                acc[key] = props[key];
            return acc;
        }, {}), [props]);
        if (!activities.length)
        ...
        if (settings.store.allActivitiesStyle === "carousel") {
            return (<div style={{ display: "flex", flexDirection: "column" }}>
                    {activity && currentActivity?.id === activity.id ? (<ActivityView activity={currentActivity} user={user} currentUser={currentUser} {...props}/>) : (<ActivityView activity={currentActivity} user={user} 
                // fetch optional application
                application={getActivityApplication(currentActivity)} currentUser={currentUser} {...generalProps}/>)}
                    {activities.length > 1 &&
                    <div className={cl("controls")} style={{
                            display: "flex",
                            flexDirection: "row",
                            justifyContent: "space-between",
                        }}>
                            <Tooltip text="Left" tooltipClassName={cl("controls-tooltip")}>{({ onMouseEnter, onMouseLeave }) => {
                            return <span onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={() => {
                                    const index = activities.indexOf(currentActivity);
                                    if (index - 1 >= 0)
                                        setCurrentActivity(activities[index - 1]);
                                }}>
                                    <Caret disabled={activities.indexOf(currentActivity) < 1} direction="left"/>
                                </span>;
                        }}</Tooltip>

                            <div className="carousel">
                                {activities.map((activity, index) => (<div key={"dot--" + index} onClick={() => setCurrentActivity(activity)} className={`dot ${currentActivity === activity ? "selected" : ""}`}/>))}
                            </div>
                            ...
                            return <span onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={() => {
                                    ...
                                    <Caret disabled={activities.indexOf(currentActivity) >= activities.length - 1} direction="right"/>
                                </span>;
                        }}</Tooltip>
                        </div>}
                </div>);
            ...
            return (<div style={{
                    ...
                    {activities.map((activity, index) => index === 0 ? (<ActivityView key={index} activity={activity} user={user} currentUser={currentUser} {...props}/>) : (<ActivityView key={index} activity={activity} user={user} application={getActivityApplication(activity)} currentUser={currentUser} {...generalProps}/>))}
                </div>);
        }
    },
    patches: [
