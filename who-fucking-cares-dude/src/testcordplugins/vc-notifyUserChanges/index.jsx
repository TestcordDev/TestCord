...
function Icon(path, opts) {
    return ({ color, tooltip, small }) => (<Tooltip text={tooltip}>
            {(tooltipProps) => (<svg {...tooltipProps} height={(opts?.height ?? 20) - (small ? 3 : 0)} width={(opts?.width ?? 20) - (small ? 3 : 0)} viewBox={opts?.viewBox ?? "0 0 24 24"} fill={color}>
                    <path d={path}/>
                </svg>)}
        </Tooltip>);
}
...
const PlatformIndicator = ({ user, wantMargin = true, wantTopMargin = false, small = false, style = {} }) => {
    if (!user || user.bot)
        return null;
    ...
    const status = PresenceStore.getState()?.clientStatuses?.[user.id];
    if (!status)
        return null;
    const icons = Object.entries(status).map(([platform, status]) => (<PlatformIcon key={platform} platform={platform} status={status} small={small}/>));
    if (!icons.length)
        return null;
    return (<span className="vc-platform-indicator" style={{
            display: "inline-flex",
...
const getRichBody = (user, text) => <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "10px" }}>
    <div style={{ position: "relative" }}>
        <img src={user.getAvatarURL(void 0, 80, true)} style={{ width: "80px", height: "80px", borderRadius: "15%" }} alt={`${user.username}'s avatar`}/>
        <PlatformIndicator user={user} style={{ position: "absolute", top: "-8px", right: "-10px" }}/>
    </div>
    <span>{text}</span>
</div>;
function triggerVoiceNotification(userId, userChannelId) {
...
function toggleUserNotify(userId) {
...
const UserContext = (children, { user }) => {
    ...
    children.splice(-1, 0, (<Menu.MenuGroup>
            <Menu.MenuItem id="toggle-notify-user" label={label} action={() => toggleUserNotify(user.id)} icon={icon}/>
        </Menu.MenuGroup>));
};
