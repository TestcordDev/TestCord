...
async function fetchExcludedQuestConfigs(questIds) {
    const quests = [];
...
export function canOpenDevToolsWindow() {
...
function getQuestNotificationText(quests, excluded) {
...
function notifyNewQuests(quests, excluded) {
    if (quests.length === 0)
        return;
    const firstQuest = quests[0];
...
export async function fetchAndAlertQuests(source) {
