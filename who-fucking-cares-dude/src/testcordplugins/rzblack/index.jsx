...
function kickUserWithSettings(guildId, userId, channelId) {
    ...
    if (settings.store.muteUser || settings.store.deafenUser) {
        const body = {};
        if (settings.store.muteUser)
            body.mute = true;
        if (settings.store.deafenUser)
            body.deaf = true;
        actions.push(RestAPI.patch({
...
const UserContext = (children, { user }) => {
    ...
    if (!voiceChannel || voiceChannel.type !== 2)
        return;
    if (!hasPerms(channelId))
        return;
    const blacklisted = isBlacklisted(user.id, channelId);
    children.splice(-1, 0, (<Menu.MenuGroup>
            <Menu.MenuItem id="vc-blacklist" label={blacklisted ? "Remove Blacklist" : "Add to Blacklist"} color={blacklisted ? "danger" : undefined} action={() => {
            if (blacklisted) {
                Alerts.show({
...
const ChannelContext = (children, { channel }) => {
    ...
    const items = list.map(entry => {
        ...
        return (<Menu.MenuItem key={entry.userId} id={`bl-${entry.userId}`} label={name} color="danger" action={() => {
                Alerts.show({
    ...
    items.push(<Menu.MenuItem key="rm-all" id="rm-all" label="Remove Blacklist" color="danger" action={() => {
            Alerts.show({
                title: "Remove All",
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            for (const { userId, channelId } of voiceStates) {
                if (!channelId || userId === UserStore.getCurrentUser()?.id)
                    continue;
                if (isBlacklisted(userId, channelId)) {
                    const channel = ChannelStore.getChannel(channelId);
                    if (channel && hasPerms(channelId)) {
