 ...
 */
export let OptionalMessageOption = OptPlaceholder;
/**
 * Required message option named "message" you can use in commands.
 * Used in "me"
 * @see {@link OptionalMessageOption}
 */
export let RequiredMessageOption = ReqPlaceholder;
let idCounter = 99;
export const _init = function (cmds) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd, args, ctx) {
 ...
 * @param opt
 */
export function prepareOption(opt) {
...
const isSubCommandParent = (cmd) => cmd.options?.[0]?.type === 1 /* ApplicationCommandOptionType.SUB_COMMAND */;
const getSubCommandName = (cmd, option) => `${cmd.name} ${option.name}`;
// Yes, Discord registers individual commands for each subcommand
function registerSubCommands(cmd, plugin) {
    cmd.options?.forEach(o => {
        if (o.type !== 1 /* ApplicationCommandOptionType.SUB_COMMAND */)
...
function unregisterSubCommands(cmd) {
    const results = BUILT_IN
        .filter(c => c.rootCommand === cmd)
        .map(c => unregisterCommand(c.name));
    return results.length > 0 && results.every(x => x);
}
export function registerCommand(command, plugin) {
    if (!BUILT_IN) {
        console.warn("[CommandsAPI]", `Not registering ${command.name} as the CommandsAPI hasn't been initialised.`, "Please restart to use commands");
        return;
    }
    ...
    command.type ??= 1 /* ApplicationCommandType.CHAT_INPUT */;
    command.inputType ??= 1 /* ApplicationCommandInputType.BUILT_IN_TEXT */;
...
export function unregisterCommand(name, isSubCommands = false) {
