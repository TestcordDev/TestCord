...
import { stringMatches, wreq } from "@webpack";
 ...
 */
export function extractModule(id, patched = companionSettings.store.usePatchedModule) {
    if (patched) {
        try {
...
function getNormalizedModuleText(id_) {
    const id = String(id_);
...
function extractUnpatchedModule(id) {
    if (!wreq.m[id]) {
 ...
 */
export function getModulePatchedBy(id, usePatched = companionSettings.store.usePatchedModule) {
...
export function findModuleId(find) {
    const matches = [];
    for (const id in wreq.m) {
        if (stringMatches(wreq.m[id].toString(), find))
            matches.push(id);
    }
    if (matches.length === 0) {
...
export function mkRegexFind(idOrSearch) {
...
export function toggleEnabled(name, beforeReload) {
...
export function findAllModuleIds(filter, { topLevelOnly = false } = {}) {
    const { c } = wreq;
    const ret = [];
