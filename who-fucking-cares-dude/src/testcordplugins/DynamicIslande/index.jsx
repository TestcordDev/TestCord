...
const runtime = Reflect.get(globalThis, RUNTIME_KEY) ?? {
...
const SETTINGS_KEYS = ["islandColor", "keepIslandVisible", "showSpotifyIsland", "showVoiceIsland", "showScreenShareIsland", "morphNotifications", "showCallControls", "showCallParticipants", "showParticipantButtons"];
function setIslandNotification(notification) {
    if (runtime.notificationTimeoutId !== undefined)
        clearTimeout(runtime.notificationTimeoutId);
...
function Glyph({ path, size: _, ...props }) {
    return (<svg {...props} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d={path}/>
        </svg>);
}
function IslandIcon(props) {
...
function ControlButton({ active, children, compact, danger, label, onClick }) {
    return (<Button aria-label={label} className={cl("control", { "control-active": active, "control-compact": compact, "control-danger": danger })} size="iconOnly" variant="none" onClick={(event) => {
...
function ScreenShareTimer({ startedAt }) {
    const elapsed = Date.now() - startedAt;
    const time = useFixedTimer({ initialTime: startedAt });
    return <>{formatDurationMs(time > 0 ? time : elapsed)}</>;
}
function VoiceIcon({ children, slashed }) {
    return (<span className={cl("voice-icon", { "voice-icon-slashed": slashed })}>
            {children}
...
function SpotifySection() {
    ...
    if (!track)
        return null;
    return (<section className={cl("section")} aria-label={t("عناصر تحكّم Spotify", "Spotify controls")}>
...
function ParticipantRow({ p, channel, currentUser }) {
    ...
    const avatarUrl = typeof p.user.getAvatarURL === "function"
        ...
        : p.user.avatarURL;
    return (<div className="vc-testcord-dynamic-island-participant-row" onContextMenu={e => {
            e.preventDefault();
            const copy = window.DiscordNative?.clipboard?.copy;
            if (copy)
                copy(p.user.id);
            else
                navigator.clipboard.writeText(p.user.id);
        }} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "6px 8px", borderRadius: "8px", cursor: "context-menu", transition: "background-color 0.15s ease" }} title={t("انقر بالزر الأيمن لنسخ المعرّف", "Right click to copy ID")}>
            ...
            {showParticipantButtons && (<div style={{ display: "flex", alignItems: "center", gap: "4px" }} onClick={e => e.stopPropagation()} onContextMenu={e => e.stopPropagation()}>
                    <ControlButton label={isMuted ? t("إلغاء الكتم", "Unmute") : t("كتم", "Mute")} danger={isMuted} compact onClick={() => {
                if (isSelf)
                    IslandVoiceActions.toggleSelfMute();
                else
                    IslandVoiceActions.toggleLocalMute(p.user.id);
            }}>
                        <VoiceIcon slashed={isMuted}><Microphone width={14} height={14}/></VoiceIcon>
                    </ControlButton>

                    {!isSelf && (<>
                            <ControlButton label={isFollowingUser ? t("إلغاء المتابعة", "Unfollow") : t("متابعة", "Follow")} compact active={isFollowingUser} danger={isFollowingUser} onClick={() => {
                    if (isFollowingUser)
                        unfollow();
                    else
                        follow(p.user.id);
                }}>
                                {isFollowingUser ? (<svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                            ...
                            <ControlButton label={isFriend ? t("إزالة صديق", "Remove Friend") : (isOutgoing || isIncoming ? t("إلغاء الطلب", "Cancel Request") : t("إضافة صديق", "Add Friend"))} compact onClick={() => {
                    if (isFriend)
                        RelationshipActions.removeFriend(p.user.id);
                    else if (isOutgoing)
                        RelationshipActions.cancelFriendRequest(p.user.id);
                    else if (isIncoming)
                        RelationshipActions.removeFriend(p.user.id);
                    else
                        RelationshipActions.addRelationship({ userId: p.user.id, context: { location: "Dynamic Island" } });
                }}>
                                {isFriend || isOutgoing || isIncoming ? (<svg width="14" height="14" viewBox="0 0 24 24" fill="none">
...
function VoiceSection({ channelId }) {
    ...
    return (<div className={cl("section")} style={{ display: "flex", flexDirection: "column", alignItems: "stretch", padding: 0, gap: 0 }}>
            <div aria-label={t("عناصر تحكّم مكالمة ديسكورد", "Discord call controls")} style={{ display: "flex", minWidth: 0, padding: "8px", alignItems: "center", gap: "10px" }}>
                <div className={cl("section-info")} onClick={() => { if (showCallParticipants)
        setExpanded(!expanded); }} style={{ cursor: showCallParticipants ? "pointer" : "default", flex: 1 }} title={showCallParticipants ? t("انقر لإظهار/إخفاء قائمة المشاركين", "Click to toggle participants list") : undefined}>
            ...
            {showCallParticipants && expanded && participants.length > 0 && (<div className="vc-testcord-dynamic-island-voice-participants-list" style={{ display: "flex", flexDirection: "column", gap: "6px", padding: "4px 8px 8px 8px", maxHeight: "220px", overflowY: "auto", borderTop: "1px solid rgba(255, 255, 255, 0.05)" }}>
                    {participants.map(p => (<ParticipantRow key={p.user.id} p={p} channel={channel} currentUser={currentUser}/>))}
                </div>)}
        </div>);
}
function ScreenShareSection({ startedAt, stream }) {
    ...
    return (<section className={cl("section", "screen-section")} aria-label={t("عناصر تحكّم مشاركة الشاشة", "Screen sharing controls")}>
            <div className={cl("section-info")}>
...
function DynamicIsland() {
    ...
    const [primaryIsland, setPrimaryIsland] = useState(IslandType.ScreenShare);
    ...
    const activeIslands = [];
    if (stream)
        activeIslands.push(IslandType.ScreenShare);
    if (track)
        activeIslands.push(IslandType.Spotify);
    if (channelId)
        activeIslands.push(IslandType.Voice);
    ...
    useEffect(() => {
        ...
        if (!showSpotifyIsland || !spotifyTrackId || isPlaying)
            return;
    ...
    useEffect(() => {
        ...
        const handleMessage = ({ message }) => {
            ...
            if (!channel || message.author.id === me.id || storedMessage?.blocked)
                return;
    ...
    if (idle && !notification && !keepIslandVisible)
        return null;
    ...
    const cyclePrimary = (direction) => {
        if (activeIslands.length < 2 || !primary)
            return;
    ...
    const finishSwipe = (event) => {
        ...
        if (Date.now() - start.startedAt < SWIPE_MIN_DURATION || Math.abs(distanceX) < SWIPE_MIN_DISTANCE || Math.abs(distanceX) <= Math.abs(distanceY))
            return;
        suppressClickRef.current = true;
        cyclePrimary(distanceX > 0 ? 1 : -1);
    };
    const primaryIsPlaying = primary === IslandType.Spotify ? isPlaying : false;
    return (<div className={cl("root", `color-${islandColor}`, {
            ...
            <Clickable className={cl("summary")} aria-expanded={expanded} aria-label={t("الجزيرة الديناميكية", "Dynamic Island")} onClick={activateSummary} onPointerCancel={() => { swipeStartRef.current = null; }} onPointerDown={beginSwipe} onPointerUp={finishSwipe}>
                {notification
            ...
            : primaryTrack
                ...
                {!notification && primaryTrack && (<span className={cl("visualizer")} aria-label={primaryIsPlaying ? t("يعمل", "Playing") : t("متوقّف", "Paused")}>
                        <span /><span /><span />
                    </span>)}
                {!notification && primaryStream && (<ControlButton compact label={t("إيقاف المشاركة", "Stop sharing")} danger onClick={() => stopScreenShare(primaryStream)}>
                        <Glyph path="M7 7h10v10H7V7Z"/>
                    </ControlButton>)}
                {!notification && primaryChannelId && <span className={cl("live-dot")} aria-label={t("مكالمة نشطة", "Call active")}/>}
                {!notification && activeIslands.length > 1 && (<span className={cl("pages")} aria-label={t(`${activeIslands.length} جزر نشطة`, `${activeIslands.length} active Islands`)}>
...
export function DynamicIslandPortal() {
    ...
    useEffect(() => {
        const syncPortal = () => {
            if (runtime.activeModule === portalModule && runtime.owner == null)
                runtime.owner = owner;
