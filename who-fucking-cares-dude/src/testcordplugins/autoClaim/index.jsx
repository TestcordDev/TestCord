...
function isCustomVoiceChannel(channelId) {
...
function detectVcOwner(channelId) {
    const channel = ChannelStore.getChannel(channelId);
...
function isOwnerInChannel(channelId, ownerId) {
    const voiceStates = VoiceStateStore.getVoiceStatesForChannel(channelId);
    if (!voiceStates)
        return false;
    return Object.keys(voiceStates).includes(ownerId);
}
function updateVcOwnerTracking(channelId) {
    if (!channelId)
        return;
...
function attemptClaim(channelId, reason, isManual = false) {
    const generation = pluginGeneration;
    if (!isManual && !settings.store.autoClaimEnabled)
        return;
    ...
    RestAPI.post({
        ...
        if (!pluginActive || generation !== pluginGeneration)
            return;
...
let ownershipMonitorInterval = null;
function autoDetectVcOwnerOnJoin(channelId, guildId) {
...
function makeContextMenuPatch() {
    return (children, props) => {
        if (!props)
            return;
        if (settings.store.autoClaimEnabled)
            return;
...
function MenuItem(id) {
    if (UserStore.getCurrentUser().id === id)
        return;
    return (<Menu.MenuItem id="manual-claim-vc" label="Claim Voice Channel" action={async () => {
...
export default definePlugin({
    ...
    renderClaimButton() {
        if (settings.store.autoClaimEnabled)
            return null;
        return (<Tooltip text="Claim Voice Channel">
                {tooltipProps => (<Button style={{ backgroundColor: "transparent" }} {...tooltipProps} size={"25"} className={"vc-manual-claim-btn"} onClick={() => {
                    const currentChannelId = VoiceStateStore.getVoiceStateForUser(UserStore.getCurrentUser().id)?.channelId;
                    if (currentChannelId) {
                        updateVcOwnerTracking(currentChannelId);
