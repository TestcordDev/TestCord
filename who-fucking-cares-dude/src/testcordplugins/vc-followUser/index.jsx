...
export function getChannelId(userId) {
    if (!userId) {
        return null;
    }
    try {
...
export function triggerFollow(userChannelId = getChannelId(settings.store.followUserId)) {
...
export function getPinnedUserIds() {
    try {
...
export function isPinned(userId) {
...
const UserContext = (children, { user }) => {
    ...
    children.splice(-1, 0, (<Menu.MenuGroup>
            <Menu.MenuItem id="follow-user" label={label} action={() => toggleFollow(user.id)} icon={icon}/>
            <Menu.MenuItem id="follow-user-pin" label={pinned ? "Unpin from Follow Panel" : "Pin to Follow Panel"} action={() => togglePin(user.id)} icon={icon}/>
        </Menu.MenuGroup>));
};
...
export default definePlugin({
    ...
    FollowIndicator() {
        ...
        if (followUserId) {
            return (<HeaderBarIcon tooltip={`Following ${UserStore.getUser(followUserId).username} (click to trigger manually, right-click to unfollow)`} icon={UnfollowIcon} className="vc-plugin-icon-button" iconClassName="vc-plugin-icon-button" onClick={() => {
                    triggerFollow();
                }} onContextMenu={() => {
                    settings.store.followUserId = "";
                    settings.store.followedUsername = "";
    ...
    addIconToToolBar(e) {
        if (Array.isArray(e.toolbar)) {
            return e.toolbar.push(<ErrorBoundary noop={true} key="follow-indicator">
