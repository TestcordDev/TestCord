...
import type { CoveredSurfacesState } from "./PrivacySecurityPanel";

type ExperimentalShieldKey = "experimentalTracing" | "experimentalRtcDiagnostics" | "experimentalRemoteLogging";

interface ExperimentalPrivacyData {
    shields?: Partial<Record<ExperimentalShieldKey, boolean>>;
}

interface ExperimentDefinition {
    key: ExperimentalShieldKey;
    title: string;
    description: string;
    test: string;
}

const DEFAULT_SHIELDS: Pick<CoveredSurfacesState, ExperimentalShieldKey> = {
...
export function ExperimentalPanel() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const data: ExperimentalPrivacyData = await VencordNative.privacy.getData();
                ...
                if (active) setError("Could not load the current experiment settings.");
            } finally {
    ...
    const setShield = async (key: ExperimentalShieldKey, value: boolean) => {
        const previous = shields[key];
        setError(null);
        setShields(current => ({ ...current, [key]: value }));

        try {
            const updated: ExperimentalPrivacyData["shields"] = await VencordNative.privacy.toggleShield(key, value);
            if (updated) setShields(current => ({ ...current, [key]: updated[key] === true }));
        } catch {
                    ...
                    {EXPERIMENTS.map(experiment => {
                        ...
                        return (
                            <div className="ps-toggle-row" key={experiment.key}>
                                ...
                                <Switch
                                    checked={shields[experiment.key]}
                                    disabled={!loaded}
                                    onChange={value => void setShield(experiment.key, value)}
