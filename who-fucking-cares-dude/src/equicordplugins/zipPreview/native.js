...
export async function fetchDiscordAttachment(_, pathAndQuery) {
    try {
        const attachmentUrl = getDiscordAttachmentUrl(pathAndQuery);
        if (!attachmentUrl)
            return { success: false, error: "Invalid Discord attachment path" };
        ...
        if (previewDialog.response !== 1)
            return { success: false, error: "ZIP preview was cancelled." };
...
function getAttachmentFileName(pathAndQuery) {
    const path = pathAndQuery.split("?", 1)[0];
    const fileName = path.split("/").at(-1);
    return fileName ? decodeURIComponent(fileName) : "Unknown attachment";
}
function getDiscordAttachmentUrl(pathAndQuery) {
    if (!isValidDiscordAttachmentPathAndQuery(pathAndQuery))
        return null;
...
function isValidDiscordAttachmentPathAndQuery(pathAndQuery) {
