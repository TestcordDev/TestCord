...
function Svg(path, label) {
    return () => (<svg className={cl("button-icon", label)} height="24" width="24" viewBox="0 0 24 24" fill="currentColor" aria-label={label} focusable={false}>
            <path d={path}/>
        </svg>);
}
...
function CopyContextMenu({ name, type, path }) {
    return (<Menu.Menu navId="vc-spotify-menu" onClose={ContextMenuApi.closeContextMenu} aria-label={`Spotify ${type} Menu`}>
...
function Controls() {
    ...
    const [isPlaying, shuffle, repeat] = useStateFromStores([SpotifyStore], () => [SpotifyStore.isPlaying, SpotifyStore.shuffle, SpotifyStore.repeat]);
    ...
    return (<Flex className={cl("button-row")} gap="0" style={{ position: "relative" }}>
            <Button className={classes(cl("button"), cl("shuffle"), cl(shuffle ? "shuffle-on" : "shuffle-off"))} onClick={() => SpotifyStore.setShuffle(!shuffle)}>
                <Shuffle />
            </Button>
            <Button onClick={() => {
            ...
            <Button className={classes(cl("button"), cl("repeat"), cl(repeatClassName))} onClick={() => SpotifyStore.setRepeat(nextRepeat)} style={{ position: "relative" }}>
            ...
            <div className={cl("lyrics-toggle-wrapper")}>
                <TooltipContainer text={showSpotifyLyrics ? "Disable Lyrics" : "Enable Lyrics"}>
                    <Button className={classes(cl("button"), cl("lyrics"), cl(showSpotifyLyrics ? "repeat-context" : ""))} onClick={() => { settings.store.showSpotifyLyrics = !showSpotifyLyrics; }}>
                        <LyricsButtonIcon />
                    </Button>
                </TooltipContainer>
            </div>
...
function SpotifySeekBar() {
    ...
    const [storePosition, isSettingPosition, isPlaying] = useStateFromStores([SpotifyStore], () => [SpotifyStore.mPosition, SpotifyStore.isSettingPosition, SpotifyStore.isPlaying]);
    ...
    return (<div id={cl("progress-bar")}>
            ...
            <SeekBar initialValue={position} minValue={0} maxValue={duration} onValueChange={onChange} asValueChanges={onChange} onValueRender={formatDuration}/>
...
function AlbumContextMenu({ track }) {
    ...
    return (<Menu.Menu navId="spotify-album-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Spotify Album Menu">
            ...
            <Menu.MenuItem key="view-cover" id="view-cover" label="View Album Cover" 
    // trolley
    action={() => openImageModal(track.album.image)} icon={ImageIcon}/>
            <Menu.MenuControlItem id="spotify-volume" key="spotify-volume" label="Volume" control={(props, ref) => (<Menu.MenuSliderControl {...props} ref={ref} value={volume} minValue={0} maxValue={100} onChange={debounce((v) => SpotifyStore.setVolume(v))}/>)}/>
        </Menu.Menu>);
}
function makeLinkProps(type, condition, name, path) {
    if (!condition)
        return {};
    return {
        role: "link",
...
function Info({ track }) {
    ...
    const i = (<>
            {img && (<img id={cl("album-image")} src={img.url} alt="Album Image" onClick={() => setCoverExpanded(!coverExpanded)} onContextMenu={e => {
    ...
    return (<div id={cl("info-wrapper")}>
            ...
            <div id={cl("titles")}>
                ...
                {track.artists.some(a => a.name) && (<Paragraph className={cl(["ellipoverflow", "secondary-song-info"])}>
                        <span className={cl("song-info-prefix")}>by&nbsp;</span>
                        {track.artists.map((a, i) => (<React.Fragment key={a.name}>
                ...
                {track.album.name && (<Paragraph className={cl(["ellipoverflow", "secondary-song-info"])}>
...
export function SpotifyPlayer() {
    const track = useStateFromStores([SpotifyStore], () => SpotifyStore.track, null, (prev, next) => prev?.id ? (prev.id === next?.id) : prev?.name === next?.name);
    const device = useStateFromStores([SpotifyStore], () => SpotifyStore.device, null, (prev, next) => prev?.id === next?.id);
