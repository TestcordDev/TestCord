...
export function Switch({ checked, onChange, disabled, id, hasIcon = false, innerRef, ...ariaProps }) {
    ...
    return (<label className={switchCls("container", { checked, disabled, focusVisible })}>
            <div className={switchCls("indicator")} data-mana-component="switch">
                <svg className={switchCls("thumb")} viewBox="0 0 24 24" preserveAspectRatio="xMidYMin meet" aria-hidden="true">
                    <rect fill="var(--switch-thumb-background-default)" x={4} y={4} width={16} height={16} rx={8}/>
                    {hasIcon && (<svg viewBox="0 0 24 24" fill="none">
            ...
            <input ref={innerRef} id={id} type="checkbox" role="switch" className={switchCls("input")} tabIndex={0} checked={checked} disabled={disabled} onChange={e => onChange(e.currentTarget.checked)} onFocus={handleFocusChange} onBlur={handleFocusChange} {...ariaProps}/>
        </label>);
}
