...
import { attachKeyListener, buildRouteTargetFromRouteProps, detachKeyListener, isHyprTilesRunning, makeTargetFromChannel, makeTargetFromGuild, makeTargetFromUser, openTargetAsTile, reloadHyprTilesRules, sendFocusedToNamedGroup, setHyprTilesRunning, toggleAutoLayoutMode, toggleScratchpadById, } from "./controller";
...
if (migrateHyprTilesSettings(SettingsStore.plain.plugins))
    SettingsStore.markAsChanged();
function insertItem(children, item, fallbackAtEnd = true) {
    const ids = [
        "channel-copy-link",
        "thread-copy-link",
        "gdm-invite-people",
...
const channelContextPatch = (children, props) => {
    const target = makeTargetFromChannel(props.channel);
    if (!target)
        return;
    if (props.channel?.type === 4)
        return;
    insertItem(children, (<Menu.MenuItem id="vc-hyprtiles-open-channel" label="Open as Tile" action={() => void openTargetAsTile(target, "contextMenu")}/>));
};
const userContextPatch = (children, props) => {
    if (!props.user)
        return;
    insertItem(children, (<Menu.MenuItem id="vc-hyprtiles-open-user" label="Open DM as Tile" action={() => {
            void makeTargetFromUser(props.user).then(target => {
                if (target)
                    openTargetAsTile(target, "contextMenu");
            });
        }}/>));
};
const guildContextPatch = (children, props) => {
    ...
    if (!target)
        return;
    insertItem(children, (<Menu.MenuItem id="vc-hyprtiles-open-guild" label="Open Server as Tile" action={() => void openTargetAsTile(target, "contextMenu")}/>));
};
export default definePlugin({
    ...
    wrappedChannelRenderers: new WeakMap(),
    isVoiceChannelInTiles(voiceChannelId) {
        if (!isHyprTilesRunning() || !voiceChannelId)
            return false;
        const workspace = HyprTilesStore.getActiveWorkspace();
        return Object.values(workspace.nodesById).some(node => node.kind === "leaf" && node.tileIds.some(id => HyprTilesStore.getTile(id)?.channelId === voiceChannelId));
    },
    renderWorkspaceSwitcher() {
        return <WorkspaceSwitcherButton />;
    },
    wrapChannelRoute(originalRender) {
        ...
        const wrapped = routeProps => {
            ...
            if (!isHyprTilesRunning())
                return routeElement;
            const routeTarget = buildRouteTargetFromRouteProps(routeProps);
            return (<WorkspaceHost routeTarget={routeTarget} routeElement={routeElement}/>);
        };
        this.wrappedChannelRenderers.set(originalRender, wrapped);
        return wrapped;
    },
    _applyBordersKey: null,
    _applyBordersListener: null,
    start() {
    ...
    removeBorders() {
        const body = document.body.classList;
        body.remove("vc-hyprtiles-premium-borders", "vc-hyprtiles-premium-gradient", "vc-hyprtiles-premium-animated", "vc-hyprtiles-premium-hide-channel-name");
    }
});
