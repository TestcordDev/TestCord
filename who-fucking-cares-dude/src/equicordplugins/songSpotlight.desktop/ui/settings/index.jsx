...
function ImportButton({ overwrite, pending, setPending, onImport }) {
...
export default function Settings({ templateData }) {
    ...
    useEffect(() => {
        ...
        if (ticked.current)
            setLocalData(self?.data);
        else
            ticked.current = true;
    }, [self?.data]);
    const [pending, setPending] = useState(!localData);
    const isSame = useMemo(() => self?.data && localData
    ...
    useEffect(() => {
        if (isAuthorized() && !localData)
            getData().then(() => setPending(false));
    }, [isAuthorized()]);
    if (!isAuthorized())
        return <Button onClick={() => presentOAuth2Modal()}>Sign in to Song Spotlight</Button>;
    return (<Flex flexDirection="column" gap="20px">
                    ...
                    && (<BaseText size="xs" weight="normal" className={cl("sub")}>
                        ...
                        <Flex flexDirection="column" gap="8px">
                            <div className={cl("twin-buttons")}>
                                <Button variant="secondary" onClick={() => copyWithToast(JSON.stringify(localData))} disabled={pending}>
                                    Copy to clipboard
                                </Button>
                                <ImportButton overwrite={!!localData[0]} pending={pending} setPending={setPending} onImport={setLocalData}/>
                            </div>
            ...
            showToast("Successfully signed out!", Toasts.Type.SUCCESS);
                    ...
                    <Button variant="dangerSecondary" onClick={() => Alerts.show({
            title: "Are you sure?",
...
export function SettingsModal({ modalProps, ...props }) {
...
export function openSettingsModal(templateData) {
