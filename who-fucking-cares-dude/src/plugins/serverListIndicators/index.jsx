...
function FriendsIndicator() {
    ...
    return (<div id="vc-friendcount">
            {!settings.store.useCompact &&
            <svg id="vc-friendcount-icon" xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
            ...
            <BaseText size="xs" id="vc-friendcount-text">{onlineFriendsCount}
            </BaseText>
...
function ServersIndicator() {
    guildCount = useStateFromStores([GuildStore, UserGuildJoinRequestStore], () => {
        const guildJoinRequests = UserGuildJoinRequestStore.computeGuildIds();
    ...
    return (<div id="vc-guildcount">
            {!settings.store.useCompact &&
            <svg id="vc-guildcount-icon" aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
            ...
            <BaseText size="xs" id="vc-guildcount-text">{guildCount}
            </BaseText>
...
export const settings = definePluginSettings({
    mode: {
        ...
        options: [
            { label: "Only online friend count", value: 2 /* IndicatorType.FRIEND */, default: true },
            { label: "Only server count", value: 1 /* IndicatorType.SERVER */ },
            { label: "Both server and online friend counts", value: 3 /* IndicatorType.BOTH */ },
        ],
...
export default definePlugin({
        ...
        return <ErrorBoundary noop>
            <div id={cl("container")}>
                <Tooltip text={text} position="right">
                    {({ onMouseEnter, onMouseLeave }) => (<div id={cl("indicator-items")} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
                            {!!(mode & 2 /* IndicatorType.FRIEND */) && <FriendsIndicator />}
                            {!!(mode & 1 /* IndicatorType.SERVER */) && <ServersIndicator />}
                        </div>)}
                </Tooltip>
            </div>
        </ErrorBoundary>;
    ...
    start() {
        addServerListElement(0 /* ServerListRenderPosition.Above */, this.renderIndicator);
    },
    stop() {
        removeServerListElement(0 /* ServerListRenderPosition.Above */, this.renderIndicator);
    }
});
