...
function parseUserInfo(payload) {
    ...
    if (!isRecord(raw))
        return null;
    const parseGuildIdentity = (v) => {
        if (!isRecord(v))
            return null;
        if (typeof v.tag !== "string" || typeof v.identity_guild_id !== "string")
            return null;
        return {
            tag: v.tag,
...
export function getActiveRestrictionLabels(action) {
    const labels = [];
    const decisionVisibility = Array.isArray(action.decisionVisibility)
        ? action.decisionVisibility.filter((v) => typeof v === "string" && v.length > 0)
...
function setCache(parsedId, result) {
...
export function invalidateWarnings(parsedId) {
    if (parsedId) {
...
function parseReadyResponse(parsedId, body) {
    ...
    if (breachSuccess) {
        ...
        const errObj = breachObj.error;
        breachError = `${errObj.status ?? "unknown"}: ${errObj.message ?? "unknown error"}`;
    }
    const breachStatus = breachSuccess ? "ready"
        : breachFailed ? "error"
            : "unavailable";
    return {
        kind: "ready",
...
export async function fetchActiveWarnings(parsedId) {
    ...
    try {
        ...
        if (!nativeResult?.ok) {
            const msg = nativeResult?.error ?? "Native fetch returned no result";
        ...
        if (nativeResult.status === 401 || nativeResult.status === 403) {
            ...
            try {
                ...
                if (retryResult?.ok && retryResult.status >= 200 && retryResult.status < 300) {
                    ...
                    if (parsed)
                        return setCache(parsedId, parsed);
                }
