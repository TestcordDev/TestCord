...
// ── Global State ────────────────────────────────────────────────────────────
let fakeAccounts = [];
let activeFakeId = null;
let realUserSnapshot = null;
let _store = null;
let running = false;
let emitChangeTimer = null;
let _origGetUsers = null;
let _origGetValidUsers = null;
// ── Store Validation ────────────────────────────────────────────────────
...
function isMultiAccountStore(mod) {
    try {
...
function unpatchStore() {
    ...
    if (_origGetValidUsers)
        _store.getValidUsers = _origGetValidUsers;
    _origGetUsers = null;
    _origGetValidUsers = null;
    _store.emitChange?.();
}
// ── simulateSwitch ─────────────────────────────────────────────────────────
function simulateSwitch(fake) {
    ...
    try {
        ...
        if (updated)
            FluxDispatcher.dispatch({ type: "CURRENT_USER_UPDATE", user: { ...updated } });
...
function onSwitchFailure(action) {
...
function onSwitchAttempt(action) {
...
function onRemoveAccount(action) {
    ...
    if (idx === -1)
        return; // Not a fake account, ignore
...
function addToSwitcher(userId) {
    if (fakeAccounts.find(f => f.id === userId))
        return;
    ...
    fakeAccounts.push({
        ...
        _publicFlags: user?.publicFlags ?? 0,
        _flags: user?.flags ?? 0,
        _premiumType: user?.premiumType ?? 0,
    });
    DataStore.set(DS_KEY, fakeAccounts.map(f => f.id));
    patchStore();
    _store?.emitChange?.();
...
function RestoreButton() {
    ...
    React.useEffect(() => {
        const t = setInterval(() => setActive(!!activeFakeId), 2000);
        return () => clearInterval(t);
    }, []);
    if (!active)
        return null;
    return (<HeaderBarButton icon={RestoreIcon} tooltip="Fake account active — click to restore your real account" onClick={() => { restoreRealAccount(); setActive(false); }}/>);
}
...
const ctxPatch = (children, { user }) => {
    if (!children || !Array.isArray(children))
        return;
    try {
        if (!user || user.id === UserStore.getCurrentUser()?.id)
            return;
        children.push(<Menu.MenuItem id="fake-account-add" label="Add to Switcher (Fake)" icon={FakeAccountIcon} action={() => addToSwitcher(user.id)}/>);
    }
    catch (e) {
...
export default definePlugin({
    ...
    async start() {
        ...
        waitFor(["getUsers", "getValidUsers", "getHasLoggedInAccounts"], async (mod) => {
            if (!running)
                return;
            ...
            _store = mod;
            const savedIds = (await DataStore.get(DS_KEY)) ?? [];
            if (!running)
                return;
            for (const id of savedIds) {
                if (fakeAccounts.find(f => f.id === id))
                    continue;
