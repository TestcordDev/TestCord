...
export function MemberCount({ isTooltip, tooltipGuildId }) {
    ...
    const totalCount = useStateFromStores([GuildMemberCountStore], () => guildId ? GuildMemberCountStore.getMemberCount(guildId) : null);
    let onlineCount = useStateFromStores([OnlineMemberCountStore], () => guildId ? OnlineMemberCountStore.getCount(guildId) : null);
    const memberListOnlineCount = useStateFromStores([ChannelMemberStore], () => {
        if (isTooltip || !guildId)
            return null;
    ...
    const threadListOnlineCount = useStateFromStores([ThreadMemberListStore], () => {
        if (isTooltip)
            return null;
    ...
    if (memberListOnlineCount != null)
        onlineCount = memberListOnlineCount;
    if (threadListOnlineCount != null)
        onlineCount = threadListOnlineCount;
    useEffect(() => {
        if (guildId) {
    ...
    return (<div className={cl("widget", { tooltip: isTooltip, "member-list": !isTooltip })}>
            <Tooltip text={`${formattedOnlineCount} online in this channel`} position="bottom">
                {props => (<div {...props} className={cl("container")}>
