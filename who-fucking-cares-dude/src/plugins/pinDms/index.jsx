...
import { canMoveCategory, canMoveCategoryInDirection, categoryLen, collapseCategory, getAllUncollapsedChannels, getCategoryByIndex, getCategoryChannels, getSections, init, isPinned, moveCategory, removeCategory, usePinnedDms } from "./data";
...
export const settings = definePluginSettings({
    pinOrder: {
        ...
        options: [
            { label: "Most recent message", value: 0 /* PinOrder.LastMessage */, default: true },
            { label: "Custom (right click channels to reorder)", value: 1 /* PinOrder.Custom */ }
        ]
    },
...
export default definePlugin({
    ...
    startAt: "WebpackReady" /* StartAt.WebpackReady */,
    start: init,
    flux: {
    ...
    isChannelHidden(categoryIndex, channelIndex) {
        if (categoryIndex === 0)
            return false;
    ...
    getScrollOffset(channelId, rowHeight, padding, preRenderedChildren, originalOffset) {
        const channels = this.getAllUncollapsedChannels();
        if (!isPinned(channelId))
            return ((rowHeight + padding) * 2 // header
    ...
    renderCategory: ErrorBoundary.wrap(({ section }) => {
        const category = getCategoryByIndex(section - 1);
        if (!category)
            return null;
        return (<Clickable onClick={() => collapseCategory(category.id, !category.collapsed)} onContextMenu={e => {
                ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="vc-pindms-header-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} color="danger" aria-label="Pin DMs Category Menu">
                            <Menu.MenuItem id="vc-pindms-edit-category" label="Edit Category" action={() => openCategoryModal(category.id, null)}/>

                            {canMoveCategory(category.id) && (<>
                                        {canMoveCategoryInDirection(category.id, -1) && <Menu.MenuItem id="vc-pindms-move-category-up" label="Move Up" action={() => moveCategory(category.id, -1)}/>}
                                        {canMoveCategoryInDirection(category.id, 1) && <Menu.MenuItem id="vc-pindms-move-category-down" label="Move Down" action={() => moveCategory(category.id, 1)}/>}
                                    </>)}

                            <Menu.MenuSeparator />
                            <Menu.MenuItem id="vc-pindms-delete-category" color="danger" label="Delete Category" action={() => removeCategory(category.id)}/>

                        </Menu.Menu>));
            }}>
                <h2 className={classes(headerClasses.privateChannelsHeaderContainer, "vc-pindms-section-container", category.collapsed ? "vc-pindms-collapsed" : "")} style={{ color: `#${category.color.toString(16).padStart(6, "0")}` }}>
    ...
    renderChannel(sectionIndex, index, ChannelComponent) {
        return ErrorBoundary.wrap(() => {
            const { channel, category } = this.getChannel(sectionIndex, index, this.instance.props.channels);
            if (!channel || !category)
                return null;
            if (this.isChannelHidden(sectionIndex, index))
                return null;
            return (<ChannelComponent channel={channel} selected={this.instance.props.selectedChannelId === channel.id}>
                    {channel.id}
                </ChannelComponent>);
        }, { noop: true });
    },
    getChannel(sectionIndex, index, channels) {
        const category = getCategoryByIndex(sectionIndex - 1);
        if (!category)
