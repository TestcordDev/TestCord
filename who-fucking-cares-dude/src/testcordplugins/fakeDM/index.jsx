...
const Native = VencordNative.pluginHelpers.fakeDM;
...
async function savePersisted(fakes) {
    _persistedFakes = fakes;
    try {
        await DataStore.set(STORAGE_KEY, fakes);
    }
    catch { }
    try {
    ...
    try {
        Native?.saveFakes(JSON.stringify(fakes));
    }
    catch { }
}
async function removePersistedItem(snowflakeId) {
    const next = _persistedFakes.filter(f => f.snowflakeId !== snowflakeId);
    await savePersisted(next);
}
async function removePersistedChannel(channelId) {
...
function registerFake(channelId, id) {
    if (!fakeIds.has(channelId))
        fakeIds.set(channelId, new Set());
    fakeIds.get(channelId).add(id);
}
function clearSingleFake(channelId, snowflakeId) {
...
function buildAuthorObject(user) {
    return {
        id: user.id,
...
function getChannelParticipants() {
    ...
    try {
        ...
        if (ch.recipients || ch.rawRecipients) {
            const recipientIds = ch.recipients ?? ch.rawRecipients?.map((r) => r.id) ?? [];
        ...
        if (ch.guild_id) {
            const memberStore = GuildMemberStore;
            ...
            if (Array.isArray(members)) {
                ...
                for (const m of Object.values(members).slice(0, 50)) {
...
// ─── Injections ─────────────────────────────────────────────────────────────
function injectMessage(channelId, author, content, date, persistedId) {
...
function restoreChannelFakes(channelId) {
    if (!channelId || !_persistedFakes.length)
        return;
    const channelFakes = _persistedFakes.filter(f => f.channelId === channelId);
    if (!channelFakes.length)
        return;
    for (const f of channelFakes) {
        ...
        if (f.type === "message") {
            ...
            if (!caller)
                caller = { id: f.callerId, username: "Caller" };
            if (!other)
                other = { id: f.otherId, username: "User" };
            injectCall(f.channelId, caller, other, f.missed, f.durationSec, new Date(f.timestamp), f.snowflakeId, f.endedTimestamp);
        }
    }
}
...
let _unsubscribeOpen = null;
function setupRestorationListeners() {
    const handleLoadMessages = (action) => {
        if (action.channelId)
            setTimeout(() => restoreChannelFakes(action.channelId), 50);
    };
...
function UserAvatar({ user, size = 22 }) {
    ...
    if (hasErr || !url) {
        return (<div className="fakedm-avatar-placeholder" style={{ width: size, height: size }}>
                {name[0]?.toUpperCase()}
            </div>);
    }
...
function UserPicker({ participants, selectedId, onSelect }) {
    ...
    const filtered = React.useMemo(() => {
        ...
        return participants.filter(p => p.id.includes(q) ||
            (p.username && p.username.toLowerCase().includes(q)) ||
            (p.globalName && p.globalName.toLowerCase().includes(q))).slice(0, 10);
    }, [participants, query]);
    return (<div className="fakedm-picker" ref={containerRef}>
            <input type="text" className="fakedm-input" placeholder={selectedUser ? `${selectedUser.globalName || selectedUser.username} (@${selectedUser.username})` : "Search user or enter User ID..."} value={query} onChange={e => {
            setQuery(e.target.value);
            setIsOpen(true);
        }} onFocus={() => setIsOpen(true)}/>

            {isOpen && (<div className="fakedm-dropdown">
                    {filtered.map(user => (<div key={user.id} className="fakedm-dropdown-item" onMouseDown={e => {
                    e.preventDefault();
                    e.stopPropagation();
                    onSelect(user);
                    setQuery("");
                    ...
                    {query.length > 3 && !filtered.length && (<div className="fakedm-dropdown-item" onMouseDown={e => {
                    e.preventDefault();
                    e.stopPropagation();
...
// ─── Main Popout Panel ──────────────────────────────────────────────────────
function FakeDMPanel({ onClose, btnRect }) {
    const me = UserStore.getCurrentUser();
    ...
    const [sender, setSender] = React.useState(() => me);
    const [caller, setCaller] = React.useState(() => me);
    const [callReceiver, setCallReceiver] = React.useState(() => otherUser);
    ...
    const handleHeaderPointerDown = (e) => {
        if (isModal || !popoutRef.current)
            return;
        if (e.target.closest(".fakedm-close-btn"))
            return;
    ...
    function handleSendMessage() {
        if (!content.trim() || !channelId || !sender)
            return;
    ...
    function handleSendCall() {
        if (!channelId || !caller || !callReceiver)
            return;
        const targetDate = getActiveDate();
        if (!targetDate) {
            showNotice("Invalid Date/Time", false);
    ...
    return (<>
            ...
            <div ref={popoutRef} className="fakedm-popout" style={popoutStyle} onClick={e => e.stopPropagation()}>
                ...
                <div className="fakedm-header" onPointerDown={handleHeaderPointerDown}>
                    ...
                    <button className="fakedm-close-btn" onClick={onClose} onPointerDown={e => e.stopPropagation()} aria-label="Close">
                ...
                <div className="fakedm-content">
                    {notice && (<div className={`fakedm-notice ${notice.ok ? "fakedm-notice--success" : "fakedm-notice--error"}`}>
                            {notice.msg}
                        </div>)}
                    ...
                    {tab === "message" && (<>
                            ...
                            <div className="fakedm-form-item">
                                ...
                                <div className="fakedm-user-toggle">
                                    <button className={`fakedm-toggle-btn ${sender?.id === me?.id ? "fakedm-toggle-btn--active" : ""}`} onClick={() => setSender(me)}>
                                        <UserAvatar user={me}/>
                                        <span className="fakedm-user-name">{me?.globalName || me?.username || "Self"}</span>
                                    </button>

                                    {otherUser?.id !== me?.id && (<button className={`fakedm-toggle-btn ${sender?.id === otherUser?.id ? "fakedm-toggle-btn--active" : ""}`} onClick={() => setSender(otherUser)}>
                                ...
                                <div style={{ marginTop: 4 }}>
                                    <UserPicker participants={participants} selectedId={sender?.id || ""} onSelect={u => setSender(u)}/>
                                </div>
                            </div>
                            ...
                            <div className="fakedm-form-item">
                                ...
                                <div className="fakedm-date-row">
                                    <input type="datetime-local" className="fakedm-date-input" value={dateStr} onChange={e => {
                ...
                setTextTimeInput("");
                                ...
                                <div className="fakedm-time-text-wrap" style={{ marginTop: 4 }}>
                                    <input type="text" className="fakedm-input" placeholder="Or type time (e.g. 14:30, -10m, -1h, now)..." value={textTimeInput} onChange={e => setTextTimeInput(e.target.value)}/>
                                    {parsedCustomDate ? (<span className="fakedm-time-preview fakedm-time-preview--valid">
                                            Parsed: {parsedCustomDate.toLocaleString()}
                                        </span>) : textTimeInput.trim() ? (<span className="fakedm-time-preview fakedm-time-preview--invalid">
                            ...
                            <div className="fakedm-form-item">
                                ...
                                <textarea ref={textareaRef} className="fakedm-textarea" rows={3} placeholder="Enter message content..." value={content} onChange={e => setContent(e.target.value)} onKeyDown={e => {
                if (e.key === "Enter" && !e.shiftKey) {
                    ...
                    handleSendMessage();
                            ...
                            <div className="fakedm-actions">
                                ...
                                <button className="fakedm-btn-danger" onClick={() => {
                if (!channelId)
                    return;
                ...
                showNotice(`Cleared ${count} fake(s)`, true);
                    ...
                    {tab === "call" && (<>
                            <div className="fakedm-form-item">
                                ...
                                <UserPicker participants={participants} selectedId={caller?.id || ""} onSelect={u => setCaller(u)}/>
                            </div>

                            <div className="fakedm-form-item">
                                <span className="fakedm-form-label">Receiver</span>
                                <UserPicker participants={participants} selectedId={callReceiver?.id || ""} onSelect={u => setCallReceiver(u)}/>
                            </div>

                            <div className="fakedm-form-item">
                                <span className="fakedm-form-label">Status</span>
                                <div className="fakedm-call-group">
                                    <button className={`fakedm-call-btn ${!callMissed ? "fakedm-call-btn--answered" : ""}`} onClick={() => setCallMissed(false)}>
                                        Answered
                                    </button>
                                    <button className={`fakedm-call-btn ${callMissed ? "fakedm-call-btn--missed" : ""}`} onClick={() => setCallMissed(true)}>
                                        Missed
                                    </button>
                                </div>
                            </div>

                            {!callMissed && (<div className="fakedm-form-item">
                                    <span className="fakedm-form-label">Duration (minutes)</span>
                                    <input type="number" min="1" step="1" className="fakedm-input" value={callDuration} onChange={e => setCallDuration(e.target.value)}/>
                                </div>)}
                ...
                showNotice(`Cleared ${count} fake(s)`, true);
                    ...
                    return (<div key={fake.snowflakeId} className="fakedm-list-item">
                            ...
                            : `Call (${fake.missed ? "Missed" : `${fake.durationSec}s`})`}
                                                ...
                                                <button className="fakedm-list-del" title="Delete fake message" onClick={() => {
                            if (!channelId)
                                return;
                            ...
                            {activeChannelFakes.length > 0 && (<div className="fakedm-actions" style={{ marginTop: 8 }}>
                                    <button className="fakedm-btn-danger" style={{ width: "100%" }} onClick={() => {
                    if (!channelId)
                        return;
                    const count = clearChannelFakes(channelId);
...
// ─── SVG Icon ───────────────────────────────────────────────────────────────
function FakeDMIcon({ height = 20, width = 20, className }) {
    return (<svg className={className} aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24">
...
function FakeDMHeaderButton() {
    return (<HeaderBarButton icon={FakeDMIcon} tooltip="FakeDM" onClick={() => {
            openModal(modalProps => (<ModalRoot {...modalProps}>
                        <ModalHeader>
                            FakeDM
...
function FakeDMChannelButton() {
    return (<ChannelToolbarButton icon={FakeDMIcon} tooltip="FakeDM" onClick={() => {
            openModal(modalProps => (<ModalRoot {...modalProps}>
                        <ModalHeader>
                            FakeDM
                            <ModalCloseButton onClick={modalProps.onClose}/>
...
// ─── Chat Bar Button ────────────────────────────────────────────────────────
const FakeDMButton = ({ isMainChat }) => {
    const [btnRect, setBtnRect] = React.useState(null);
    if (!isMainChat || fakeDMSettings.store.location !== "chatbar")
        return null;
    function handleClick(e) {
        if (btnRect) {
            ...
            const el = e.currentTarget.closest("button") ?? e.currentTarget;
            setBtnRect(el.getBoundingClientRect());
        }
    }
    return (<div onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} onMouseUp={e => e.stopPropagation()} style={{ display: "contents" }}>
            <ChatBarButton tooltip="FakeDM" onClick={handleClick}>
                <FakeDMIcon />
            </ChatBarButton>
            {btnRect && ReactDOM.createPortal(<FakeDMPanel onClose={() => setBtnRect(null)} btnRect={btnRect}/>, document.body)}
        </div>);
};
// ─── Plugin Export ──────────────────────────────────────────────────────────
