...
function PopOverIcon() {
    return (<svg fill="var(--text-default)" width={24} height={24} viewBox={"0 0 64 64"}>
...
function Indicator() {
    return (<Tooltip text="This message has a hidden message! (InvisibleChat)">
            {({ onMouseEnter, onMouseLeave }) => (<img aria-label="Hidden Message Indicator (InvisibleChat)" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} src="https://github.com/SammCheese/invisible-chat/raw/NewReplugged/src/assets/lock.png" width={20} height={20} style={{ transform: "translateY(4p)", paddingInline: 4 }}/>)}
        </Tooltip>);
}
const ChatBarRender = ({ isMainChat }) => {
    if (!isMainChat)
        return null;
    return (<ChatBarButton tooltip="Encrypt Message" onClick={() => buildEncModal()} buttonProps={{
...
function ChatBarIcon() {
    return (<svg aria-hidden role="img" width="20" height="20" viewBox={"0 0 64 64"} style={{ scale: "1.39", translate: "0 -1px" }}>
...
function colorCodeFromNumber(color) {
...
export async function buildEmbed(message, revealed) {
...
export default definePlugin({
    ...
    reporterTestable: 8 /* ReporterTestable.Patches */,
    settings,
    chatBarButton: {
        icon: ChatBarIcon,
