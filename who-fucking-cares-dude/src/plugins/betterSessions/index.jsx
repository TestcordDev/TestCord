...
export default definePlugin({
    ...
    renderName: ErrorBoundary.wrap((props) => {
    ...
    renderDescription: ErrorBoundary.wrap((props, origDescription) => {
        const session = props?.session;
        ...
        return (<div className={cl("description")}>
                <Paragraph size="sm" weight="normal" color="text-muted">{label}</Paragraph>
                {timeLabel && (<>
                        {" \xb7 "}
                        {session?.approx_last_used_time ? (<Tooltip text={new Date(session.approx_last_used_time).toLocaleString()}>
                                {props => (<span {...props} className={TimestampClasses.timestamp}>
                                        {timeLabel}
                                    </span>)}
                            </Tooltip>) : (<span className={TimestampClasses.timestamp}>
                                {timeLabel}
                            </span>)}
                    </>)}
            </div>);
    }, { noop: true }),
    renderIcon: ErrorBoundary.wrap((props) => {
        const session = props?.session;
        ...
        return (<BlobMask isFolder style={{ cursor: "unset" }} selected={false} lowerBadge={<div className={cl("lowerBadge")}>
    ...
    async checkNewSessions() {
        ...
        if (!Array.isArray(data?.body?.user_sessions))
            return;
        for (const session of data.body.user_sessions) {
            if (!session?.id_hash)
                continue;
            if (savedSessionsCache.has(session.id_hash))
                continue;
    ...
    flux: {
        USER_SETTINGS_ACCOUNT_RESET_AND_CLOSE_FORM() {
            ...
            if (!Array.isArray(rawSessions))
                return;
            const lastFetchedHashes = rawSessions.map(session => session?.id_hash).filter(Boolean);
