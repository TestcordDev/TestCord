...
function openDecodedBase64Modal(decodedContent) {
    openModal(props => (<ErrorBoundary>
            <Modal {...props} size="lg" title="Decoded Base64 Content" actions={decodedContent.map((content, index) => ({
            ...
            onClick: () => copyWithToast(content, "Decoded content copied to clipboard!")
                ...
                <div style={{ padding: "16px 0" }}>
                    ...
                    {decodedContent.map((content, index) => (<CodeBlock key={index} content={content} lang=""/>))}
                </div>
            </Modal>
        </ErrorBoundary>));
}
