...
let favoriteGifPickerInstance = null;
let isServerSearchButtonRegistered = false;
function renderServerSearchButton() {
    return <SearchServerButton />;
}
function syncServerSearchButton(enabled) {
    if (enabled) {
        if (isServerSearchButtonRegistered)
            return;
        addServerListElement(0 /* ServerListRenderPosition.Above */, renderServerSearchButton);
        isServerSearchButtonRegistered = true;
        return;
    }
    if (!isServerSearchButtonRegistered)
        return;
    removeServerListElement(0 /* ServerListRenderPosition.Above */, renderServerSearchButton);
...
function getSearchTemplate(engineKey) {
    if (engineKey === "custom")
        return settings.store.customSearchEngine || defaultCustomSearchEngine;
...
function getSelectedTextSearchEngines() {
...
function getEngineIconUrl(template) {
...
function makeEngineLabel(label, template) {
    const iconUrl = getEngineIconUrl(template);
    if (!iconUrl)
        return label;
    return (<Flex gap="0.5em" alignItems="center">
            <img aria-hidden="true" height={16} width={16} src={iconUrl} style={{ borderRadius: "50%" }}/>
            {label}
        </Flex>);
}
...
function openSearchModal() {
    openModal(modalProps => React.createElement(SearchModal, { modalProps: modalProps }));
}
function isAdvancedSearchButton(button) {
...
function onDocumentClick(event) {
    if (settings.store.disableCustomSearchPopup)
        return;
    const { target } = event;
    ...
    if (!(button instanceof HTMLButtonElement))
        return;
    if (!isAdvancedSearchButton(button))
        return;
    event.preventDefault();
    event.stopPropagation();
    openSearchModal();
}
function getCorrectUsername(userId) {
...
function buildSearchQueryRequest(query, offset) {
    return {
...
function messageMatchesQuery(message, query) {
    if (query.author_id?.length && message.author?.id !== query.author_id[0])
        return false;
    if (query.channel_id?.length && message.channel_id !== query.channel_id[0])
        return false;
    if (query.mentions?.length) {
        const mentions = (message.mentions ?? []);
        const mentionIds = new Set(mentions
            .map(mention => typeof mention === "string" ? mention : mention.id)
            .filter((mentionId) => Boolean(mentionId)));
        if (!query.mentions.every(mentionId => mentionIds.has(mentionId)))
            return false;
    }
    if (query.content?.trim()) {
        const messageContent = message.content?.toLowerCase() ?? "";
        if (!messageContent.includes(query.content.trim().toLowerCase()))
            return false;
    }
    return true;
}
function getTimestampValue(timestamp) {
    ...
    if (typeof timestamp === "object" && timestamp != null) {
        const objectWithValueOf = timestamp;
...
function collectStoredMessages(channelId) {
...
async function searchGuildMessages(guildId, query) {
    ...
    for (let offset = 0; offset < maxResults; offset += 25) {
        ...
        for (const message of messageGroups.flat()) {
            if (!message.id || collected.has(message.id) || !messageMatchesQuery(message, query))
                continue;
...
async function searchChannelMessages(channelId, query) {
    const maxResults = settings.store.maxResults ?? 100;
    ...
    while (collected.size < maxResults) {
        const messages = await TestcordRequestCoordinator.request({
        ...
        if (!before || messages.length < 100)
            break;
    }
    return sortMessages(Array.from(collected.values()));
}
const QuickSearchResultsModal = ErrorBoundary.wrap(({ modalProps, query, queryString, guildId, channelId, title }) => {
    ...
    const highlightPreview = (preview) => {
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */} className={cl("root")}>
                ...
                <div className={cl("toolbar")}>
                    <div className={cl("search-row")}>
                        <div style={{ minWidth: 0 }}>
                            ...
                            <div className={cl("results-stats")} style={{ color: "var(--text-muted)" }}>
            ...
            <ModalContent className={cl("content")}>
                {loading ? (<div className={cl("loading")}>
                        ...
                        <span>Searching...</span>
                    </div>) : error ? (<div className={cl("no-results")}>
                        <span>{error}</span>
                    </div>) : results.length === 0 ? (<div className={cl("no-results")}>
                        <span>No results found.</span>
                    </div>) : (<div className={classes(cl("results", "message-results"), cl("quick-search-results"))}>
                        {results.map(message => {
                const channel = ChannelStore.getChannel(message.channel_id);
                const user = UserStore.getUser(message.author.id) ?? message.author;
                return (<div key={`${message.channel_id}-${message.id}`} className={cl("result-item")} onClick={() => navigateToMessage(message)} style={{
                        display: "block",
                        background: "var(--background-secondary)",
                        borderColor: "var(--background-modifier-accent)",
                        color: "var(--text-normal)"
                    }}>
                                    <div className={classes(cl("result-content-wrapper"), cl("quick-search-row"))} style={{
                        display: "flex",
                        alignItems: "flex-start",
                        gap: 14,
                        width: "100%"
                    }}>
                                        <div className={classes(cl("result-avatar"), cl("quick-search-avatar"))} style={{
                        width: 40,
                        minWidth: 40,
                        display: "flex",
                        alignItems: "flex-start",
                        justifyContent: "center"
                                            ...
                                            <Avatar src={user.getAvatarURL?.(channel?.guild_id, 128) || undefined} size="SIZE_40" className={cl("avatar")}/>
                                        </div>
                                        <div className={classes(cl("result-main"), cl("quick-search-body"))} style={{
                        minWidth: 0,
                        flex: 1,
                        display: "flex",
                        ...
                        overflow: "hidden"
                                            ...
                                            <div className={cl("result-header")} style={{
                        display: "flex",
                        ...
                        flexWrap: "wrap"
                                                ...
                                                <div className={classes(cl("result-author"), cl("quick-search-meta"))} style={{
                        display: "flex",
                        alignItems: "center",
                        minWidth: 0,
                        gap: 8,
                        ...
                        flexWrap: "wrap"
                                                    ...
                                                    <span className={classes(cl("result-author-name"), cl("quick-search-author"))} style={{
                        color: "var(--white-500)",
                        fontSize: 16,
                        fontWeight: 600,
                        lineHeight: 1.2
                                                    ...
                                                    <span className={cl("result-channel")} style={{
                        display: "inline-flex",
                        alignItems: "center",
                        maxWidth: "100%",
                        padding: "2px 8px",
                        ...
                        whiteSpace: "nowrap"
                                                ...
                                                <span className={cl("result-time")} style={{
                        ...
                        flexShrink: 0
                                            ...
                                            <div className={classes(cl("result-content"), cl("quick-search-preview"))} style={{
                        color: "var(--white-500)",
                        fontSize: 15,
                        lineHeight: 1.45,
                        marginTop: 0,
...
function openQuickSearchResults(options) {
    openModal(modalProps => React.createElement(QuickSearchResultsModal, {
        ...options,
        modalProps: modalProps
    }));
}
const quickSearchState = { keys: {} };
const quickSearchContextMenuPatch = (children, props) => {
    if (!props)
        return;
    if (props.channel && !PermissionStore.can(PermissionsBits.VIEW_CHANNEL, props.channel))
        return;
    ...
    const searchAction = () => {
        const query = { include_nsfw: true };
        quickSearchItems.forEach(item => {
            if (!quickSearchState.keys[item.name])
                return;
    ...
    children.push(<Menu.MenuSeparator />, <Menu.MenuItem id="quick-search" label="Quick Search">
            {quickSearchItems.map(item => {
            if (!item.present)
                return null;
            return (<Menu.MenuCheckboxItem key={item.name} id={item.name} label={item.label} checked={false} action={() => {
                    quickSearchState.keys[item.name] = !(quickSearchState.keys[item.name] ?? false);
                }}/>);
        })}
            <Menu.MenuItem id="quick-search-start" label="Search" action={searchAction}/>
        </Menu.MenuItem>);
};
function runUserMessageSearch(user, guildId, modifierPressed, isDm) {
    const searchCurrentChannel = (settings.store.searchCurrentChannel ? !modifierPressed : modifierPressed) && !isDm;
    const query = { author_id: [user.id] };
...
const userMessageSearchContextMenuPatch = (children, { guildId, user }) => {
    if (!guildId || !user)
        return;
    const group = findGroupChildrenByChildId("user-profile", children);
    if (!group)
        return;
    group.push(<Menu.MenuItem id="vc-user-context-search-messages" label="Search Messages" action={event => runUserMessageSearch(user, guildId, event.ctrlKey || event.metaKey, false)}/>);
};
function makeSelectedTextSearchItem(selection) {
    const trimmedSelection = selection.trim();
    if (!trimmedSelection)
        return null;
    const engines = getSelectedTextSearchEngines();
    const replacementEngine = settings.store.selectedTextReplacementEngine;
    const directEngine = replacementEngine === "off"
        ? null
        : engines.find(engine => engine.key === replacementEngine);
    if (directEngine) {
        return (<Menu.MenuItem id="vc-search-selected-text" key="vc-search-selected-text" label={`Search with ${directEngine.label}`} action={() => openExternalSearch(buildSearchUrl(directEngine.template, trimmedSelection))}/>);
    }
    return (<Menu.MenuItem id="vc-search-selected-text" key="vc-search-selected-text" label="Search Text">
            {engines.map(engine => (<Menu.MenuItem key={`vc-search-selected-text-${engine.key}`} id={`vc-search-selected-text-${engine.key}`} label={makeEngineLabel(engine.label, engine.template)} action={() => openExternalSearch(buildSearchUrl(engine.template, trimmedSelection))}/>))}
        </Menu.MenuItem>);
}
const selectedTextSearchContextMenuPatch = children => {
...
function makeReverseImageSearchItem(src) {
    return (<Menu.MenuItem id="vc-search-image" key="vc-search-image" label="Search Image">
            {Object.entries(reverseImageSearchEngines).map(([engine, template]) => (<Menu.MenuItem key={`vc-search-image-${engine}`} id={`vc-search-image-${engine}`} label={makeEngineLabel(engine, template)} action={() => openExternalSearch(template + encodeURIComponent(src))}/>))}
            <Menu.MenuItem id="vc-search-image-all" key="vc-search-image-all" label={<Flex alignItems="center" gap="0.5em">
                        <OpenExternalIcon height={16} width={16}/>
                        All
                    </Flex>} action={() => Object.values(reverseImageSearchEngines).forEach(template => openExternalSearch(template + encodeURIComponent(src)))}/>
        </Menu.MenuItem>);
}
const reverseImageMessageContextMenuPatch = (children, props) => {
    if (props.reverseImageSearchType !== "img")
        return;
...
const fullSearchResultContextMenuPatch = (children, props) => {
...
function boostCurrentGuildResults(results) {
    if (!settings.store.quickSwitcherCurrentGuildFirst)
        return results;
    const currentGuild = getCurrentGuild();
    if (!currentGuild || !results.length)
        return results;
...
function modifyQuickSwitcherResults(query, originalResults) {
    const baseResults = boostCurrentGuildResults([...originalResults]);
    if (!settings.store.quickSwitcherFrequentEnabled)
        return baseResults;
    ...
    const frequentResults = Object.entries(frequentChannels)
        .map(([id]) => id)
    ...
    const frequentIds = new Set(frequentResults.map(result => result.record?.id).filter((id) => Boolean(id)));
...
function adjustSearchOffset(query) {
    if ((query.offset ?? 0) <= 5000)
        return;
    query.sort_order = query.sort_order === "asc" ? "desc" : "asc";
    query.offset = query.offset > 5000 ? 0 : query.offset;
}
function getFavoriteGifTargetString(urlString) {
    let url;
    try {
...
function FavoriteGifSearchBar({ SearchBarComponent, instance }) {
    const [query, setQuery] = useState("");
    const ref = useRef(null);
    const onChange = useCallback((searchQuery) => {
        ...
        const filteredFavorites = instance.props.favCopy
            .map(gif => ({
            score: fuzzySearch(searchQuery.toLowerCase(), getFavoriteGifTargetString(gif.url ?? gif.src).replace(/(%20|[_-])/g, " ").toLowerCase()),
            gif
        }))
    ...
    return (<SearchBarComponent ref={ref} autoFocus={true} size="md" className="" onChange={onChange} onClear={() => {
            setQuery("");
            instance.props.favorites = instance.props.favCopy;
            instance.forceUpdate();
        }} query={query} placeholder="Search Favorite Gifs"/>);
}
    ...
    return (<ErrorBoundary noop>
            <div className={cl("server-list-item-container")}>
                <div className={cl("server-list-pill-container")}>
                    <GuildedServerListItemPillComponent unread={false} selected={false} hovered={hovered} className={hovered ? `${cl("server-list-pill")} hovered` : cl("server-list-pill")}/>
                </div>
                <div className={cl("server-list-button-container")}>
                    <GuildlessServerListItemComponent showPill={false} tooltip="Search" className={cl("server-list-button")} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => FluxDispatcher.dispatch({
            type: "QUICKSWITCHER_SHOW",
            query: "",
            queryMode: null
        })} icon={() => (<div className={cl("server-list-icon-container")}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" viewBox="0 0 24 24" className={cl("server-search-button-icon")}>
                                    <path fill="currentColor" fillRule="evenodd" d="M15.62 17.03a9 9 0 1 1 1.41-1.41l4.68 4.67a1 1 0 0 1-1.421.42l-4.67-4.68ZM17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" clipRule="evenodd"/>
                                </svg>
                            </div>)}/>
                </div>
            </div>
    ...
    commands: [
        {
            ...
            execute(args) {
                const query = findOption(args, "query", "").trim();
                const engineKey = findOption(args, "engine", settings.store.defaultSearchEngine || defaultSearchEngine);
                if (!query) {
        ...
        },
        {
            ...
            async execute(args, ctx) {
                const word = findOption(args, "word", "").trim();
                if (!word) {
                    return { content: "Please provide a word to search for." };
                }
                try {
                    const query = encodeURIComponent(word);
                    const { list } = await TestcordRequestCoordinator.request({
