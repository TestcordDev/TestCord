...
/* ── Modal ── */
function LeaveAllServersModal({ rootProps }) {
    ...
    const allGuilds = useMemo(() => {
        ...
        return Object.values(raw).sort((a, b) => a.name.localeCompare(b.name));
    }, []);
    // Exclude owned servers if safeMode
    const availableGuilds = useMemo(() => settings.store.safeMode ? allGuilds.filter(g => g.ownerId !== myId) : allGuilds, [allGuilds, myId]);
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="las-content">
                ...
                <div className="las-search-bar">
                    <SearchIcon />
                    <input className="las-search-input" type="text" placeholder="Search a server..." value={search} onChange={e => setSearch(e.currentTarget.value)} autoFocus/>
                    {search && (<button className="las-search-clear" onClick={() => setSearch("")}>✕</button>)}
                </div>
                ...
                <div className="las-guild-list">
                    {filtered.length === 0 && (<div className="las-empty">
                            {search ? `No results for "${search}"` : "No servers found"}
                        </div>)}
                    {filtered.map(g => {
            const av = getGuildIcon(g);
            const isSel = selected.has(g.id);
            return (<div key={g.id} className={`las-guild-row ${isSel ? "las-guild-row--selected" : ""}`} onClick={() => toggleGuild(g.id)}>
                                {av
                ...
                {/* Status */}
                {status !== "idle" && (<div className={`las-status las-status--${status}`}>{progress}</div>)}

                {/* Compteur */}
                <div className="las-footer-info">
                    <span>{selected.size} server{selected.size > 1 ? "s" : ""} selected</span>
                    {settings.store.safeMode && (<span className="las-safe-note">· Safe mode active (owners excluded)</span>)}
                </div>

                {/* Bouton principal */}
                <button className="las-leave-btn" onClick={handleLeave} disabled={selected.size === 0 || status === "running"}>
                    {status === "running"
            ? `In progress... (${pct}%)`
...
/* ── Context menu patch ── */
const patchGuildContext = (children, { guild }) => {
    if (!children || !Array.isArray(children))
        return;
    try {
        if (!guild)
            return;
        children.push(<Menu.MenuSeparator key="las-sep"/>, <Menu.MenuItem id="leave-all-servers" key="leave-all-servers" label="Leave All Servers" color="danger" action={() => openModal(props => <LeaveAllServersModal rootProps={props}/>)}/>);
    }
    catch (e) {
