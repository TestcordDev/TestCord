...
export function DetailPage({ spec }) {
    return (<div className={cl("detail")}>
            <div className={cl("detail-heading")}>{spec.heading}</div>
            {spec.body != null && <div className={cl("detail-body")}>{spec.body}</div>}
            {spec.rows && spec.rows.length > 0 && (<div className={cl("detail-rows")}>
                    {spec.rows.map(row => (<div key={row.label} className={cl("detail-row")}>
