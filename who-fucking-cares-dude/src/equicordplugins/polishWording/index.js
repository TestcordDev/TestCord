...
function textProcessing(input) {
    ...
    if (settings.store.quickDisable)
        return input;
    let text = input;
    ...
    if (settings.store.fixApostrophes || settings.store.expandContractions)
        text = ensureApostrophe(text); // Note: if expanding contractions, fix them first.
    ...
    if (settings.store.expandContractions)
...
function ensureApostrophe(textInput) {
    ...
    const findMissingRegex = new RegExp(`\\b(${potentialContractions.join("|")})\\b`, // Match any of the keys as whole words
...
function expandContractions(textInput) {
...
function capitalize(textInput) {
    ...
    const blockedWordsArray = (Settings.plugins.PolishWording.blockedWords || "")
        .split(/,\s?/)
        .filter(bw => bw)
...
function addPeriods(textInput) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!strippedLine) {
            ...
            if (/[A-Za-z0-9]/.test(lastChar) && // If it doesn't already end with punctuation
