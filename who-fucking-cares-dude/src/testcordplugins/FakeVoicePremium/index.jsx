...
const fakedListeners = new Set();
function subscribeFaked(listener) {
...
function notifyFakedChanged() {
    for (const listener of fakedListeners)
        listener();
}
...
function setFakeCameraEnabled(enabled) {
    settings.store.fakeCam = enabled;
    if (faked) {
        syncFakeVoiceState();
    }
}
function canUseFakeCamera(channel) {
...
function getEmbeddedActivityLocation(channelId) {
    return {
        channelId,
        guildId: ChannelStore.getChannel(channelId)?.guild_id ?? null
    };
}
async function startActivity(channelId) {
...
function hasFakeActivity(channelId) {
...
function leaveActivity(channelId) {
    const activityApi = findByProps("su", "_H");
...
function syncMicCutoff(enabled) {
...
function scheduleMicCutoffSync(enabled, delay = 0) {
    if (micCutoffTimeout != null)
        clearTimeout(micCutoffTimeout);
    if (delay <= 0) {
...
function Icon({ className, enabled }) {
    ...
    return (<svg className={className} xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 512 512">
            <mask id="FakeVoicePremiumLine">
                <rect width="100%" height="100%" fill="#ffffff"/>
                <line className="blackLine" x1="469" y1="42" x2="42" y2="469" stroke="#000000" strokeWidth="128" strokeLinecap="round" style={lineStyle}/>
            </mask>

            <path mask="url(#FakeVoicePremiumLine)" fill={!enabled ? "var(--status-danger)" : "currentColor"} d="M256 48C141.1 48 48 141.1 48 256v40c0 13.3-10.7 24-24 24s-24-10.7-24-24V256C0 114.6 114.6 0 256 0S512 114.6 512 256V400.1c0 48.6-39.4 88-88.1 88L313.6 488c-8.3 14.3-23.8 24-41.6 24H240c-26.5 0-48-21.5-48-48s21.5-48 48-48h32c17.8 0 33.3 9.7 41.6 24l110.4.1c22.1 0 40-17.9 40-40V256c0-114.9-93.1-208-208-208zM144 208h16c17.7 0 32 14.3 32 32V352c0 17.7-14.3 32-32 32H144c-35.3 0-64-28.7-64-64V272c0-35.3 28.7-64 64-64zm224 0c35.3 0 64 28.7 64 64v48c0 35.3-28.7 64-64 64H352c-17.7 0-32-14.3-32-32V240c0-17.7 14.3-32 32-32h16z"/>

            <line x1="469" y1="42" x2="42" y2="469" stroke="var(--status-danger, currentColor)" strokeWidth="42" strokeLinecap="round" style={lineStyle}/>
        </svg>);
}
function setFakeVoiceEnabled(enabled) {
...
function renderFakeVoiceMenuItems(includeEnabledToggle = false, enabled = faked, setEnabled) {
    return [
        includeEnabledToggle && (<Menu.MenuCheckboxItem id="toggle-fake-voice" key="toggle-fake-voice" label="Enabled" checked={enabled} action={() => {
                const newEnabled = !enabled;
                setEnabled?.(newEnabled);
                setFakeVoiceEnabled(newEnabled);
            }}/>),
        <Menu.MenuCheckboxItem id="update-mute" key="update-mute" label="Fake Mute" checked={settings.store.fakeMute} action={() => {
                settings.store.fakeMute = !settings.store.fakeMute;
                if (faked)
                    syncFakeVoiceState();
            }}/>,
        <Menu.MenuCheckboxItem id="update-deafen" key="update-deafen" label="Fake Deafen" checked={settings.store.fakeDeafen} action={() => {
                settings.store.fakeDeafen = !settings.store.fakeDeafen;
                applyAutoMute();
                if (faked)
                    syncFakeVoiceState();
            }}/>,
        <Menu.MenuCheckboxItem id="update-camera" key="update-camera" label="Fake Camera" checked={settings.store.fakeCam} action={() => {
                setFakeCameraEnabled(!settings.store.fakeCam);
            }}/>,
        <Menu.MenuCheckboxItem id="update-stream" key="update-stream" label="Fake Stream" checked={settings.store.fakeStream} action={() => {
                settings.store.fakeStream = !settings.store.fakeStream;
            }}/>,
        <Menu.MenuCheckboxItem id="update-game" key="update-game" label="Fake Game" checked={settings.store.fakeGame} action={() => {
                settings.store.fakeGame = !settings.store.fakeGame;
            }}/>,
        <Menu.MenuCheckboxItem id="update-cut-mic" key="update-cut-mic" label="Cut Mic Transmission" checked={settings.store.cutMicTransmission} action={() => {
                settings.store.cutMicTransmission = !settings.store.cutMicTransmission;
                scheduleMicCutoffSync(faked);
            }}/>
    ];
}
function FakeVoiceOptionToggleButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : faked ? "Disable Fake States" : "Enable Fake States"} icon={<Icon className={iconForeground} enabled={faked}/>} role="switch" aria-checked={faked} redGlow={!faked} plated={nameplate != null} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => <ContextMenu />)} onClick={() => {
...
function ContextMenu() {
    ...
    return (<Menu.Menu navId="Voice-state-modifier" onClose={() => { }} aria-label="Voice state modifier">
            <Menu.MenuGroup label="FAKE VOICE STATES">
                {renderFakeVoiceMenuItems(true, enabled, setEnabled)}
            </Menu.MenuGroup>
        </Menu.Menu>);
}
const VoiceChannelContext = (children, { channel }) => {
    if (!settings.store.contextMenu)
        return;
    if (!channel || (channel.type !== 2 && channel.type !== 13))
        return;
    children.splice(-1, 0, <Menu.MenuItem id="fake-voice-context" key="fake-voice-context" label="Fake Voice">
            {renderFakeVoiceMenuItems(true)}
        </Menu.MenuItem>);
};
// --- Folded-in feature: device context menus (from fakeMuteDeafen) ---
const AudioDeviceContext = (children, props) => {
    if (!settings.store.deviceContextMenu)
        return;
    if (props?.renderInputDevices) {
        children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-mute" label="Fake Mute" checked={settings.store.fakeMute} action={() => {
                settings.store.fakeMute = !settings.store.fakeMute;
                applyAndSync();
            }}/>);
    }
    if (props?.renderOutputDevices) {
        children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-deafen" label="Fake Deafen" checked={settings.store.fakeDeafen} action={() => {
                settings.store.fakeDeafen = !settings.store.fakeDeafen;
                applyAutoMute();
                applyAndSync();
            }}/>);
...
const VideoDeviceContext = children => {
    if (!settings.store.deviceContextMenu)
        return;
    children.push(<Menu.MenuSeparator />, <Menu.MenuCheckboxItem id="fake-camera" label="Fake Camera" checked={settings.store.fakeCam} action={() => {
            setFakeCameraEnabled(!settings.store.fakeCam);
        }}/>);
};
export default definePlugin({
    ...
    userAreaButton: {
        icon: (props) => <Icon {...props} enabled={faked}/>,
    ...
    start() {
        ...
        if (socket && !originalSocketSend) {
            ...
            socket.send = function (op, data, ...args) {
                if (op === 4 && data && faked) {
                    if (settings.store.fakeMute)
                        data.self_mute = true;
                    if (settings.store.fakeDeafen)
                        data.self_deaf = true;
                    if (settings.store.fakeCam)
    ...
    toggle: (value, key) => (faked ? settings.store[key] : value)
});
