...
export function SendCustomScreenSharePreviewImageButton() {
    ...
    useEffect(() => {
        return CustomStreamPreviewState.subscribeToField("isStreaming", setIsStreaming);
    }, []);
    const openScreenSharePreviewImageModal = useCallback(() => {
        const key = openModal((modalProps) => (<ScreenSharePreviewImageModal modalProps={modalProps} close={() => closeModal(key)}/>));
    }, []);
    return (<ErrorBoundary noop>
            <PanelButton disabled={!isStreaming} tooltipText="Stream Preview Image" icon={StreamPreviewChangeIcon} onClick={openScreenSharePreviewImageModal}/>
        </ErrorBoundary>);
}
