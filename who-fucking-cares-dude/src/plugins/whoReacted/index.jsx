...
let reactions = {};
function fetchReactions(msg, emoji, type) {
...
function getReactionsWithQueue(msg, e, type) {
...
function ReactionUsers({ message, users }) {
    ...
    return (<div style={{ marginLeft: "0.5em", transform: "scale(0.9)" }}>
            <div onClick={handleClickAvatar} onKeyDown={handleClickAvatar} style={settings.store.avatarClick ? {} : { pointerEvents: "none" }}>
                <UserSummaryItem users={users} guildId={ChannelStore.getChannel(message.channel_id)?.guild_id} renderIcon={false} max={5} showDefaultAvatarsForNullUsers showUserPopout/>
            </div>
        </div>);
}
...
export default definePlugin({
    ...
    renderUsers: ErrorBoundary.wrap(({ message, emoji, type }) => {
