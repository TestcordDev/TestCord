...
export async function analyzeUserWithCordCat(userId, username) {
    ...
    const breachCount = data.breach?.resultsCount ?? 0;
    const parts = [];
    if (statements.length > 0)
        parts.push(`${statements.length} sanction${statements.length !== 1 ? "s" : ""}`);
    if (breachCount > 0)
        parts.push(`${breachCount} breach${breachCount !== 1 ? "es" : ""}`);
    const suffix = parts.length > 0 ? ` — ${parts.join(", ")}` : "";
    const title = `CordCat: ${data.userInfo?.global_name || username}${suffix}`;
    openModal(modalProps => (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            <ModalHeader>
