...
export function ImageModal(folderProps) {
    const [data, setData] = useState((settings.store.folderIcons ?? {})[folderProps.folderId]?.url ?? "");
    const [size, setSize] = useState(100);
    return (<>
            <TextInput 
    // this looks like a horrorshow
    defaultValue={data} onChange={(val, _n) => {
            setData(val);
            ...
            {data && <>
                ...
                <Slider initialValue={100} onValueChange={(v) => {
                setSize(v);
            }} maxValue={200} minValue={25} 
        // [25, 200]
        markers={Array.apply(0, Array(176)).map((_, i) => i + 25)} stickToMarkers={true} keyboardStep={1} renderMarker={() => null}/>
            </>}
            <Button onClick={() => {
...
export function RenderPreview({ folderProps, url, size }) {
    if (!url)
        return null;
    return (<div className="test1234" style={{
            width: "20vh",
            height: "20vh",
            overflow: "hidden",
            // 16/48
...
export function makeContextItem(a) {
    return (<Menu.MenuItem id="custom-folder-icons" key="custom-folder-icons" label="Change Icon" action={() => {
            openModalLazy(async () => {
                return props => (<Modal {...props} size="sm" title="Set a New Icon.">
