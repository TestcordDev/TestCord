...
export default definePlugin({
            ...
            replacement: [
                {
                    ...
                    replace: (m, rest, ...args) => {
                        ...
                        if (before && Math.max(before.lastIndexOf("PureComponent{"), before.lastIndexOf("Component{")) > before.lastIndexOf("function"))
                            return m;
                        // Skip if inside a destructuring pattern
                        if (rest.match(/}=.+/))
                            return m;
                        if (/^\s*[\w$]+(?:\s*=\s*[^,}]+)?\s*[,}].*?\}\s*=/.test(after))
                            return m;
                        if (before && /(?:let|const|var|\()\s*\{[^}]*$/.test(before) && /^\s*[\w$]+(?:\s*=\s*[^,}]+)?\s*[,}].*?\}\s*\)\s*(?:=>|\{)/.test(after))
                            return m;
    ...
    registerMenuItem(name, component) {
