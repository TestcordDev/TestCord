...
let accountPanelRef = { current: null };
const AccountPanelContextMenu = ErrorBoundary.wrap(() => {
    const { prioritizeServerProfile } = settings.use(["prioritizeServerProfile"]);
    return (<Menu.Menu navId="vc-ap-server-profile" onClose={ContextMenuApi.closeContextMenu}>
            <Menu.MenuItem id="vc-ap-view-alternate-popout" label={prioritizeServerProfile ? "View Account Profile" : "View Server Profile"} disabled={getCurrentChannel()?.getGuildId() == null} action={async (e) => {
            ...
            <Menu.MenuCheckboxItem id="vc-ap-prioritize-server-profile" label="Prioritize Server Profile" checked={prioritizeServerProfile} action={() => settings.store.prioritizeServerProfile = !prioritizeServerProfile}/>
        </Menu.Menu>);
}, { noop: true });
...
export default definePlugin({
    ...
    UserProfile: ErrorBoundary.wrap(({ popoutProps, currentUser, originalRenderPopout }) => {
        ...
        return (<UserProfile {...popoutProps} user={currentUser} currentUser={currentUser} guildId={currentChannel.getGuildId()} channelId={currentChannel.id}/>);
    }, { noop: true })
});
function ServerProfileLauncher({ popoutProps, userId, guildId }) {
    useEffect(() => {
