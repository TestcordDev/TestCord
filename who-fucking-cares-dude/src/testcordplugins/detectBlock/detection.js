...
export function getDetectionTtlMs(state) {
    switch (state) {
        case "unknown":
...
function getRequestVersion(userId) {
    return requestVersions.get(userId) ?? 0;
}
function bumpRequestVersion(userId) {
    requestVersions.set(userId, getRequestVersion(userId) + 1);
}
function setRecord(userId, state) {
    cache.set(userId, {
        state,
        checkedAt: Date.now()
    });
...
export function getDetectionRecord(userId) {
    const record = cache.get(userId);
    if (!isFresh(record))
        return void 0;
    return record;
}
export function getDetectionState(userId) {
    return getDetectionRecord(userId)?.state ?? "unknown";
}
export function primeClear(userId) {
    bumpRequestVersion(userId);
    setRecord(userId, "clear");
}
export function subscribeToDetection(userId, listener) {
...
export function ensureDetection(userId) {
    const record = cache.get(userId);
...
export async function detectBlockedUsers(userIds) {
