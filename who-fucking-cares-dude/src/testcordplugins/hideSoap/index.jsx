...
let _latinWordRegex = null;
let _hebrewWordRegex = null;
let _wordRegexCaseSensitive = null;
...
function detectHebrewText(message) {
    if (!message?.content)
        return false;
    ...
    if (message.embeds?.length) {
        for (const embed of message.embeds) {
            // @ts-ignore
            if (embed.rawTitle && detectText(embed.rawTitle))
                return true;
            // @ts-ignore
            if (embed.rawDescription && detectText(embed.rawDescription))
                return true;
        }
    }
    return false;
...
function shouldHideUser(userId) {
...
function hiddenReplyComponent() {
    return (<div style={{ marginTop: "0px", marginBottom: "0px" }}>
            <i>↓ Replying to hidden message</i>
        </div>);
}
const hebrewCheckCache = new Map();
    ...
    flux: {
        MESSAGE_CREATE({ message }) {
            ...
            if (hebrewCheckCache.size >= HEBREW_CACHE_MAX) {
                ...
                if (firstKey)
                    hebrewCheckCache.delete(firstKey);
            }
