...
let originalMakeTextNotification = null;
    ...
    try {
        ...
        if (userStream) {
            if (settings.store.debugMode)
                console.log("[NoDMWhileStreaming] [DEBUG] Stream detected via getActiveStreamForUser", userStream);
            return true;
        }
...
export default definePlugin({
    ...
    filterChannels(ids) {
        ...
        const filtered = ids.filter((id) => {
            ...
            if (type === 1)
                return false;
            if (type === 3 && settings.store.hideGroups)
                return false;
            return true;
        });
        if (settings.store.debugMode) {
    ...
    start() {
        if (settings.store.debugMode)
            console.log("[NoDMWhileStreaming] Plugin started");
        ...
        try {
            ...
            if (soundModule) {
                ...
                soundModule.playSound = function (soundName, ...args) {
        ...
        try {
            ...
            if (notificationActions) {
                if (notificationActions.showNotification) {
                    ...
                    notificationActions.showNotification = function (channelId, ...args) {
                        if (isStreaming()) {
                ...
                if (notificationActions.makeTextNotification) {
                    ...
                    notificationActions.makeTextNotification = function (channel, ...args) {
                        if (isStreaming()) {
                            const isDM = channel?.type === 1;
                            const isGroup = channel?.type === 3 && settings.store.hideGroups;
                            if (isDM || isGroup) {
    ...
    stop() {
        if (settings.store.debugMode)
            console.log("[NoDMWhileStreaming] Plugin stopped");
