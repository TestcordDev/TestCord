...
function SettingsComponent() {
    ...
    return (<Flex flexDirection="column">
            ...
            gap: "16px",
                ...
                {tags.map(t => (<Card key={t.name} style={{
                padding: "1em 1em 0",
                ...
                boxSizing: "border-box",
                        ...
                        <TextInput type="text" value={tagSettings[t.name]?.text ?? t.displayName} placeholder={`Text on tag (default: ${t.displayName})`} onChange={v => tagSettings[t.name].text = v} className={Margins.bottom16}/>

                        <FormSwitch title="Show in messages" value={tagSettings[t.name]?.showInChat ?? true} onChange={v => tagSettings[t.name].showInChat = v} hideBorder/>

                        <FormSwitch title="Show in member list and profiles" value={tagSettings[t.name]?.showInNotChat ?? true} onChange={v => tagSettings[t.name].showInNotChat = v} hideBorder/>
                    </Card>))}
            </div>
        </Flex>);
}
