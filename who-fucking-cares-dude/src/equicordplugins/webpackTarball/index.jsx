...
async function saveTar(patched) {
...
function TarModal({ modalProps }) {
    const webpackRequire = wreq;
    const { buildNumber, builtAt } = getBuildNumber();
    ...
    return (<Modal {...modalProps} size="md" title="Webpack Tarball" actions={[
            {
                text: "Create",
                variant: "primary",
                onClick: () => {
            ...
            <div style={{ marginTop: "8px", marginBottom: "24px" }}>
                ...
                <Flex alignItems="center">
                    ...
                    <Button disabled={loading === all || isLoading} onClick={async () => {
            setLoading(true);
            // @ts-ignore
            ...
            <FormSwitch value={patched} onChange={v => settings.store.patched = v} title={settings.def.patched.description} hideBorder/>
        </Modal>);
}
