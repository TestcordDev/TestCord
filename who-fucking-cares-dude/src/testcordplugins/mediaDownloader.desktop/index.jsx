...
const Native = VencordNative.pluginHelpers.MediaDownloader;
...
export default definePlugin({
    ...
    commands: [{
            ...
            execute: async (args, ctx) => {
                if (!await Native.isYtdlpAvailable())
                ...
                const gifQuality = parseInt(findOption(args, "gif_quality", settings.store.defaultGifQuality.toString()));
    ...
    handleButtonClick: (buttonComponent) => {
        if (!(buttonComponent.emoji?.animated === "media-downloader-stop-download"))
            return;
        Native.interrupt();
    },
    start: async () => {
...
async function download(channel, { url, format, ytdlpArgs, ffmpegArgs, gifQuality }) {
