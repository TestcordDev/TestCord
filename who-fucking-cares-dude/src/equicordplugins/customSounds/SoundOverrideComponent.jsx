...
const capitalizeWords = (str) => str.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
export function SoundOverrideComponent({ type, override, onChange }) {
    ...
    return (<Card className={cl("card")}>
            <FormSwitch title={type.name} value={override.enabled || false} onChange={async (val) => {
        ...
        }} className={Margins.bottom16} hideBorder/>
            ...
            {override.enabled && (<>
                    ...
                    <div className={Margins.bottom16}>
                        ...
                        <Slider markers={makeRange(0, 100, 10)} initialValue={override.volume} onValueChange={val => {
            ...
            ]} isSelected={v => v === override.selectedSound} select={async (v) => {
                ...
                await saveAndNotify();
                    ...
                    ...customFileOptions
                ]} isSelected={v => v === (override.selectedFileId || "")} select={async (id) => {
                    if (!id) {
                        override.selectedFileId = undefined;
                    }
                    else {
                    ...
                    await saveAndNotify();
                            ...
                            <input className={cl("file-input")} ref={fileInputRef} type="file" accept=".mp3,.wav,.ogg,.m4a,.flac,.aac,.webm,.wma,.mp4" onChange={uploadFile}/>
                            <div className={cl("override-controls")}>
                                <Button variant="primary" onClick={() => fileInputRef.current?.click()}>
                                    Upload New
                                </Button>

                                {override.selectedFileId && files[override.selectedFileId] && (<Button variant="dangerPrimary" onClick={() => deleteFile(override.selectedFileId)}>
