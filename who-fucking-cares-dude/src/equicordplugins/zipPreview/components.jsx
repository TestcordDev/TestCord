...
import { createImageObjectUrl, getAttachmentFileName, getAttachmentUrl, getCachedZip, getCodeLanguage, isZipFile, makeDownload, MAX_ENTRIES, readTextEntry } from "./utils";
export const cl = classNameFactory("vc-zip-preview-");
const breadcrumbColorStyle = { color: "var(--text-muted)" };
function getVisibleEntries(entries, currentPath) {
    const dirs = new Set();
    const files = [];
...
export function ZipPreviewInline(props) {
        ...
        if (state.status === "pending") {
            state.promise
                ...
                if (loadRequestIdRef.current === loadRequestId)
                    setCacheState(getCachedZip(url));
            })
                .catch(error => {
                if (loadRequestIdRef.current !== loadRequestId)
                    return;
    ...
    const setExpanded = (nextIsExpanded) => {
        ...
        if (animationTimeoutRef.current != null)
            clearTimeout(animationTimeoutRef.current);
        if (nextIsExpanded) {
            if (!cacheState || cacheState.status === "rejected")
                loadPreview();
    ...
    return (<div className={cl("inline", isContentMounted ? "expanded" : "collapsed", isExpanded ? "open" : "closed")}>
            {isContentMounted && (<div className={cl("content")} aria-hidden={!isExpanded}>
                    <div className={cl("content-inner")}>
                        <ZipPreviewContent cacheState={cacheState} currentPath={currentPath} onNavigate={setCurrentPath}/>
                    </div>
                </div>)}
            <button className={cl("toggle")} type="button" aria-expanded={isExpanded} aria-label={isExpanded ? "Collapse ZIP preview" : "Expand ZIP preview"} onClick={event => {
...
function ZipPreviewContent({ cacheState, currentPath, onNavigate }) {
...
function ZipPreviewBreadcrumb({ path, onNavigate }) {
    if (!path) {
        return (<div className={cl("breadcrumb")}>
    ...
    return (<div className={cl("breadcrumb")}>
            <FolderIcon className={cl("breadcrumb-icon")} style={breadcrumbColorStyle}/>
            <button className={cl("breadcrumb-segment")} type="button" onClick={event => {
            event.preventDefault();
            event.stopPropagation();
            onNavigate("");
        }}>
...
function ZipPreviewFileList({ entries, currentPath, onNavigate }) {
    ...
    return (<>
            {directories.map(dir => (<button key={dir} className={cl("row", "directory", "button")} type="button" onClick={event => {
                event.preventDefault();
                event.stopPropagation();
            ...
            {files.map(entry => (<ZipPreviewFileRow key={entry.path} entry={entry}/>))}
        </>);
}
function ZipPreviewFileRow({ entry }) {
    const previewable = entry.kind !== "unsupported";
    if (!previewable) {
        return (<div className={cl("row", "file", "unsupported")}>
                {entry.name} <span className={cl("size")}>({formatBytes(entry.size)})</span>
            </div>);
    }
    return (<button className={cl("row", "file", "button")} type="button" onClick={event => {
            event.preventDefault();
            event.stopPropagation();
...
function openZipEntryModal(entry) {
    if (entry.kind === "image") {
...
function openTextEntryModal(entry) {
    const content = readTextEntry(entry);
    openModal(modalProps => (<ErrorBoundary>
            <Modal {...modalProps} size="lg" title={entry.name} actions={[
            {
                text: "Copy",
                variant: "secondary",
...
function openImageEntryModal(entry) {
    openModal(modalProps => (<ErrorBoundary>
            <Modal {...modalProps} size="lg" title={entry.name} actions={[
            {
                text: "Download",
...
function ZipImagePreview({ entry }) {
    const [url] = useState(() => createImageObjectUrl(entry));
    useEffect(() => {
        return () => URL.revokeObjectURL(url);
    }, [url]);
    return (<div className={cl("image-wrap")}>
