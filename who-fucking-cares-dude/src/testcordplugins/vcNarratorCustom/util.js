...
export function parseUserVoiceMap(input) {
    const map = new Map();
...
export function serializeUserVoiceMap(map) {
    return Array.from(map.entries())
        .map(([userId, voiceId]) => `${userId}:${voiceId}`)
        .join(",");
}
export function getVoiceForUser(userId, options) {
...
export function upsertUserVoiceMap(userVoiceMap, userId, voiceId) {
    const map = parseUserVoiceMap(userVoiceMap ?? "");
    map.set(userId, voiceId);
    return serializeUserVoiceMap(map);
}
export function removeUserVoiceFromMap(userVoiceMap, userId) {
    const map = parseUserVoiceMap(userVoiceMap ?? "");
    map.delete(userId);
    return serializeUserVoiceMap(map);
}
export function parseUserIdList(input) {
    const set = new Set();
    const trimmed = input?.trim();
    if (!trimmed)
        return set;
...
export function serializeUserIdList(list) {
    return Array.from(list).join(",");
}
export function addUserToList(input, userId) {
    const set = parseUserIdList(input);
    set.add(userId);
    return serializeUserIdList(set);
}
export function removeUserFromList(input, userId) {
    const set = parseUserIdList(input);
    set.delete(userId);
    return serializeUserIdList(set);
}
export function parseStateChangeFilterList(input) {
    const set = new Set();
    const trimmed = input?.trim();
    if (!trimmed)
        return set;
...
export function serializeStateChangeFilterList(list) {
    return Array.from(list).join(",");
}
export function addUserToStateChangeFilterList(input, userId) {
    const set = parseStateChangeFilterList(input);
    set.add(userId);
    return serializeStateChangeFilterList(set);
}
export function removeUserFromStateChangeFilterList(input, userId) {
    const set = parseStateChangeFilterList(input);
    set.delete(userId);
    return serializeStateChangeFilterList(set);
}
export function clean(str, latinOnly) {
...
export function formatText(str, user, channel, displayName, nickname, latinOnly) {
    return str
        .replaceAll("{{USER}}", clean(user, latinOnly) || (user ? "Someone" : ""))
        .replaceAll("{{CHANNEL}}", clean(channel, latinOnly) || "channel")
        .replaceAll("{{DISPLAY_NAME}}", clean(displayName, latinOnly) || (displayName ? "Someone" : ""))
        .replaceAll("{{NICKNAME}}", clean(nickname, latinOnly) || (nickname ? "Someone" : ""));
}
...
export async function getPersistentTtsCacheStats() {
...
async function trimPersistentCacheToMaxBytes(maxBytes = PERSISTENT_TTS_CACHE_MAX_BYTES) {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const tx = db.transaction([VOICES_STORE, META_STORE], "readwrite");
...
export async function getCachedVoiceFromDB(cacheKey) {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const tx = db.transaction([VOICES_STORE, META_STORE], "readwrite");
        const voicesStore = tx.objectStore(VOICES_STORE);
        ...
        request.onsuccess = () => {
            ...
            if (blob) {
                ...
                metaGetReq.onsuccess = () => {
                    const existing = metaGetReq.result;
                    const updated = {
...
export async function setCachedVoiceInDB(cacheKey, blob) {
    const db = await openDB();
    await new Promise((resolve, reject) => {
        ...
        metaStore.put({ size: blob.size, createdAt: now, lastAccess: now }, cacheKey);
        tx.oncomplete = () => resolve();
        tx.onerror = () => reject(tx.error);
    });
