...
const messageContextMenuPatch = (children, { channel, message: { author, messageSnapshots, embeds, flags, id: messageId } }) => {
    ...
    if (author.id === UserStore.getCurrentUser().id && !hasEmbedPerms)
        return;
    const menuGroup = findGroupChildrenByChildId("delete", children);
    const deleteIndex = menuGroup?.findIndex(i => i?.props?.id === "delete");
    if (deleteIndex == null || !menuGroup)
        return;
    menuGroup.splice(deleteIndex - 1, 0, (<Menu.MenuItem id="unsuppress-embeds" key="unsuppress-embeds" label={isEmbedSuppressed ? "Unsuppress Embeds" : "Suppress Embeds"} color={isEmbedSuppressed ? undefined : "danger"} icon={isEmbedSuppressed ? ImageVisible : ImageInvisible} action={() => RestAPI.patch({
