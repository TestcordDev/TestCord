...
async function findLastMessageFromUser(guildId, channelId, userId) {
    try {
...
async function jumpToLastActive(channel, targetUserId) {
    if (!channel) {
        Toasts.show({
            type: Toasts.Type.FAILURE,
...
const ChannelContextMenuPatch = (children, { channel }) => {
    children.push(<Menu.MenuItem id="LastActive" label={<span style={{ color: "#aa6746" }}>Your Last Message</span>} icon={LastActiveIcon} action={() => {
            jumpToLastActive(channel);
        }}/>);
};
const UserContextMenuPatch = (children, { user, channel }) => {
    if (!channel || !user?.id)
        return;
    children.push(<Menu.MenuItem id="LastActive" label={<span style={{ color: "#aa6746" }}>User's Last Message</span>} icon={UserLastActiveIcon} action={() => {
            jumpToLastActive(channel, user.id);
        }}/>);
};
export function UserLastActiveIcon() {
    return (<svg viewBox="0 0 52 52" width="20" height="20" fill="#aa6746">
            <g>
...
export function LastActiveIcon() {
    return (<svg viewBox="0 0 24 24" width="20" height="20" fill="#aa6746" xmlns="http://www.w3.org/2000/svg">
