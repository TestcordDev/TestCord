...
const stateToPayload = (guildId, state) => ({
    userId: state.userId,
    username: GuildMemberStore.getNick(guildId, state.userId) ||
    ...
    for (const state of dispatch.voiceStates) {
        ...
        if (ourState) {
            if (state.channelId && state.channelId !== currentChannel) {
                ...
                ws?.send(JSON.stringify({
                    cmd: "CHANNEL_JOINED",
                    states: Object.values(voiceStates).map(s => stateToPayload(guildId, s)),
                }));
