...
const error = (...args) => console.error("[Testcord:HostUpdate]", ...args);
const hookedUpdaters = new WeakSet();
let hooked = false;
 ...
 */
const resourcesPathFor = (versionDir) => {
    if (process.platform !== "darwin")
        return join(versionDir, "resources");
 ...
 */
const resolveCommittedVersion = (updater) => {
    if (Array.isArray(updater.committedHostVersion))
        return updater.committedHostVersion;
    try {
        const versions = updater.queryCurrentVersionsSync?.();
        if (Array.isArray(versions?.current_host))
            return versions.current_host;
    }
    catch (err) {
...
const retainEquicord = (updater, reason) => {
    try {
        ...
        if (!committed || !rootPath) {
            ...
            if (stale)
                patchResourcesDir(stale, getPatcherJsPath());
            return;
        }
        const versionDir = join(rootPath, `app-${committed.join(".")}`);
        if (resolvePath(versionDir) === resolvePath(currentVersionDir()))
            return;
...
const attachToUpdater = (updater) => {
    if (!updater || hookedUpdaters.has(updater))
        return;
    try {
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options) => {
            bound(options);
            try {
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions, options) => {
...
const wrapStartup = (coreExports) => {
    if (!coreExports?.startup || coreExports.__equicordStartupWrapped)
        return;
...
export const installHostUpdateHook = () => {
    ...
    Module.prototype.require = function (id) {
        ...
        if (!id.includes("discord_desktop_core"))
            return result;
