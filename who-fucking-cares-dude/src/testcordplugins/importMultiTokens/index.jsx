...
const ImportMultiTokensComponent = () => {
    ...
    return (<div style={{ padding: "16px" }}>
            ...
            <TextArea placeholder="tokens from your local storage > tokens;" value={input} onChange={setInput} rows={10} style={{ marginBottom: "12px", fontFamily: "monospace" }}/>
            <Flex style={{ gap: "8px" }}>
                <Button color={Button.Colors.BRAND} onClick={handleImport} disabled={importing || !input.trim()}>
                    {importing ? "Importing..." : "Import Tokens"}
                </Button>
                <Button color={Button.Colors.TRANSPARENT} onClick={() => setInput("")} disabled={!input.trim()}>
                    Clear
                </Button>
            </Flex>
        </div>);
