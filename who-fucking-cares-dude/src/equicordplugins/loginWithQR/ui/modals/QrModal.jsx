...
var LoginStateType;
(function (LoginStateType) {
    LoginStateType[LoginStateType["Idle"] = 0] = "Idle";
    LoginStateType[LoginStateType["Loading"] = 1] = "Loading";
})(LoginStateType || (LoginStateType = {}));
const limitSize = (width, height) => {
    if (width > height) {
...
const verifyUrl = async (token, { current: modalProps }) => {
    // yay
    let handshake = null;
    try {
        ...
        if (res.ok && res.status === 200)
            handshake = res.body?.handshake_token;
    }
    catch { }
...
const handleProcessImage = (file, modalPropsRef) => {
...
function QrModal() {
    ...
    const modalProps = useRef({
        ...
        setPreview: (media, location) => new Promise(res => {
            if (!media)
                return res(setPreview(null));
            const size = {};
            ...
            if (media instanceof HTMLImageElement)
                ...
                source = (<video controls={false} style={{ width: "100%", height: "100%" }} ref={e => {
                        if (e) {
            ...
            if (!location)
                return res(setPreview({ source, size }));
            else {
                const combinations = [
                    [location.topLeftCorner, location.bottomRightCorner],
                    [location.topRightCorner, location.bottomLeftCorner],
                ];
                const crossSize = (Math.sqrt(Math.abs(location.topLeftCorner.x - location.topRightCorner.x) **
                    2 +
                    Math.abs(location.topLeftCorner.y - location.topRightCorner.y) **
                        2) +
                    Math.sqrt(Math.abs(location.topRightCorner.x - location.bottomRightCorner.x) **
                        2 +
                        Math.abs(location.topRightCorner.y - location.bottomRightCorner.y) **
                            2)) /
                    3 /
                    media.height;
                const crosses = [];
                for (const combination of combinations) {
                    for (let i = 0; i < 2; i++) {
                        ...
                        const rot = (Math.atan2(opposite.y - current.y, opposite.x - current.x) -
                            Math.PI / 4) *
                            (180 / Math.PI);
                        crosses.push({
    ...
    useEffect(() => {
        const callback = (e) => {
            e.preventDefault();
            if (state !== LoginStateType.Idle || !e.clipboardData)
                return;
            for (const item of e.clipboardData.items) {
                if (item.kind === "file" && item.type.startsWith("image/")) {
                    ...
                    item.getAsString(text => {
                        ...
                        if (token)
                            verifyUrl(token, modalProps).catch(e => modalProps.current.exit(e?.message));
                        else
    ...
    return (<div className={cl("modal-container")}>
            <div className={cl("modal-filepaste", preview?.source && "modal-filepaste-preview", preview?.crosses && "modal-filepaste-crosses")} onClick={() => state === LoginStateType.Idle && inputRef.current?.click()} onDragEnter={e => e.currentTarget.classList.add(cl("modal-filepaste-drop"))} onDragLeave={e => e.currentTarget.classList.remove(cl("modal-filepaste-drop"))} onDrop={e => {
            e.preventDefault();
            ...
            : {}}>
                ...
                ["--offset-y"]: preview.crosses
                    ...
                    : undefined,
                        ...
                        {preview.source}
                        {preview.crosses?.map(({ x, y, rot, size }) => (<span key={cl("preview-cross")} className={cl("preview-cross")} style={{
                    left: `${x}%`,
                    top: `${y}%`,
                    ["--size"]: `${size}rem`,
                    ["--rot"]: `${rot}deg`,
                    ...
                    </div>) : state === LoginStateType.Loading ? (<Spinner type={SpinnerTypes.WANDERING_CUBES}/>) : error.current ? (<BaseText size="md" weight="semibold" color="text-danger">
                        {error.current}
                    </BaseText>) : (<>
