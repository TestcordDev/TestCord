...
let ExpandableList = () => null;
function getGroupDMName(channel) {
    return channel.name ||
        channel.recipients
...
const getMutualGroupDms = (userId) => ChannelStore.getSortedPrivateChannels()
    .filter(c => c.isGroupDM() && c.recipients.includes(userId));
const isBotOrSelf = (user) => user.bot || user.id === UserStore.getCurrentUser().id;
function getMutualGDMCountText(user) {
    const count = getMutualGroupDms(user.id).length;
    return `${count === 0 ? "No" : count} Mutual Group${count !== 1 ? "s" : ""}`;
}
function renderClickableGDMs(mutualDms, onClose) {
    return mutualDms.map(c => (<Clickable key={c.id} className={MutualsListClasses.row} onClick={() => {
            onClose();
            SelectedChannelActionCreators.selectPrivateChannel(c.id);
        }}>
            <Avatar src={IconUtils.getChannelIconURL({ id: c.id, icon: c.icon, size: 32 })} size="SIZE_40" className={MutualsListClasses.icon}>
            </Avatar>
...
export default definePlugin({
    ...
    set ExpandableList(value) {
        ExpandableList = value;
    },
    getMutualGroupDms(userId) {
        try {
    ...
    pushSection(sections, user) {
        try {
            if (isBotOrSelf(user) || sections[IS_PATCHED])
                return;
    ...
    renderMutualGDMs: ErrorBoundary.wrap(({ user, onClose }) => {
        const mutualGDms = useMemo(() => getMutualGroupDms(user.id), [user.id]);
        const entries = renderClickableGDMs(mutualGDms, onClose);
        return (<ScrollerThin className={TabBarClasses.tabPanelScroller} fade={true} onClose={onClose}>
                {entries.length > 0
                ? entries
    ...
    renderDMPageList: ErrorBoundary.wrap(({ user, hasDivider, Divider, listStyle }) => {
        ...
        if (mutualGDms.length === 0)
            return null;
        return (<>
                {hasDivider && Divider}
                <ExpandableList listClassName={listStyle} header={"Mutual Groups"} isLoading={false} items={renderClickableGDMs(mutualGDms, () => { })}/>
            </>);
    }, { noop: true })
});
