...
function rememberLocation(guildId, channelId) {
    if (!channelId)
        return;
    const next = {
...
function toggleStarred(location) {
...
function getValidLocations() {
    ...
    return locations.filter(location => {
        if (seen.has(location.channelId))
            return false;
...
function getChannelName(location) {
    const channel = ChannelStore.getChannel(location.channelId);
    if (!channel)
        return "Unknown Channel";
    if (channel.name)
        return channel.isDM?.() || channel.isGroupDM?.() || channel.isMultiUserDM?.() ? channel.name : `#${channel.name}`;
    const recipientIds = channel.recipients ?? channel.rawRecipients?.map((recipient) => recipient.id) ?? [];
    ...
    const rawRecipientNames = channel.rawRecipients?.map((recipient) => recipient.globalName ?? recipient.username).filter(Boolean);
    if (rawRecipientNames?.length)
        return rawRecipientNames.join(", ");
    return "Direct Message";
}
function getGuildName(location) {
    if (location.guildId === "@me")
        return "Direct Messages";
    return GuildStore.getGuild(location.guildId)?.name ?? "Unknown Server";
}
function getInitial(location) {
...
function getMentionCount(location) {
    if (!settings.store.showUnreadBadges)
        return 0;
...
function messageMentionsCurrentUser(message) {
    ...
    if (message.mentioned)
        return true;
    if (message.mentions?.some(user => user.id === currentUserId))
        return true;
...
function rememberMentionedChannel({ message }) {
    if (!settings.store.addMentionedChannels || !message?.channel_id)
        return;
...
function styleCard(card, selected) {
...
function createIcon(location, size) {
...
function animateRemoveLocation(card, location) {
...
function renderOverlay() {
    ...
    list.onwheel = event => {
        ...
        if ((wantsDown && canScrollDown) || (!wantsDown && canScrollUp))
            return;
        event.preventDefault();
    ...
    for (let i = 0; i < switcherCandidates.length; i++) {
        ...
        card.onmouseenter = () => {
            ...
            for (let j = 0; j < cards.length; j++)
                styleCard(cards[j], j === selectedIndex);
            if (settings.store.animations)
...
export default definePlugin({
    ...
    settings,
    async start() {
        starredLocations = await DataStore.get(STARRED_DATA_KEY) ?? [];
        if (settings.store.persistRecents) {
            const stored = await DataStore.get(RECENTS_DATA_KEY) ?? [];
