...
const safeParse = (json, fallback) => { try {
    return JSON.parse(json) || fallback;
}
catch {
    return fallback;
} };
const getPresets = (items) => [...new Set(items.map(x => x.preset).filter(Boolean))];
const getEmojiUrl = (id, animated) => `https://cdn.discordapp.com/emojis/${id}.${animated ? "gif" : "png"}`;
const getAvatarUrl = (userId, avatar) => avatar ? `https://cdn.discordapp.com/avatars/${userId}/${avatar}.png?size=64` : `https://cdn.discordapp.com/embed/avatars/${Number(BigInt(userId) >> 22n) % 6}.png`;
const formatInterval = (seconds) => seconds < 60 ? `${seconds}s` : seconds % 60 === 0 ? `${Math.floor(seconds / 60)}m` : `${Math.floor(seconds / 60)}m ${seconds % 60}s`;
...
const IconButton = ({ onClick, children, className = "" }) => <div onClick={onClick} className={cl("icon-btn", className)}>{children}</div>;
const StatusIndicator = ({ type }) => <span className={cl("status-indicator", type)}/>;
const CustomStatus = getUserSettingLazy("status", "customStatus");
const StatusSetting = getUserSettingLazy("status", "status");
const settings = definePluginSettings({
    interval: { type: 1 /* OptionType.NUMBER */, description: "Interval between status changes (seconds)", default: DEFAULT_INTERVAL, minimum: MIN_INTERVAL, maximum: MAX_INTERVAL },
    statuses: { type: 0 /* OptionType.STRING */, description: "JSON array of statuses", default: JSON.stringify([{ text: "Hey there!" }]) },
    presets: { type: 0 /* OptionType.STRING */, description: "JSON array of presets", default: "[]" },
    randomize: { type: 3 /* OptionType.BOOLEAN */, description: "Shuffle status order", default: false },
    autoStart: { type: 3 /* OptionType.BOOLEAN */, description: "Start automatically when Discord loads", default: false }
});
let _statusesCache = null;
let _statusesRaw = null;
function getParsedStatuses() {
    const raw = settings.store.statuses;
    if (raw === _statusesRaw && _statusesCache)
...
const state = { currentIndex: 0, interval: null, isRunning: false, _started: false };
function AnimatedStatusIcon({ color = "currentColor" }) {
...
function AnimatedStatusButton() {
    ...
    return (<HeaderBarButton icon={AnimatedStatusIcon} tooltip={isRunning ? "Animated Status: Running" : "Animated Status: Stopped"} aria-label="Toggle Animated Status" selected={isRunning} onClick={() => openModal(modalProps => <SettingsModal {...modalProps}/>)}/>);
}
export default definePlugin({
    ...
    async setStatus(step) {
        ...
        try {
            ...
            if (step.status && StatusSetting)
                await StatusSetting.updateSetting(step.status);
            return true;
        }
        catch {
            return false;
    ...
    async begin(preset) {
        ...
        if (!all.length)
            return Toasts.show({ message: "Add some statuses first!", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        const items = preset ? all.filter(s => s.preset === preset) : all;
        if (!items.length)
            return Toasts.show({ message: `No "${preset}" statuses found`, type: Toasts.Type.FAILURE, id: Toasts.genId() });
        if (state.interval)
            clearInterval(state.interval);
        state.currentIndex = 0;
        await this.setStatus(items[0]);
        state.interval = setInterval(() => this.next(preset).catch(() => { if (state.interval) {
            clearInterval(state.interval);
            state.interval = null;
        } state.isRunning = false; this.forceUpdate(); }), Math.max(settings.store.interval * 1000, MIN_INTERVAL * 1000));
...
function StatusPreview({ emojiId, emojiName, animated, text, statusType }) {
    const currentUser = UserStore.getCurrentUser();
    return (<div className={cl("preview")}>
...
function SettingsModal({ onClose, transitionState }) {
    const scrollerRef = useRef(null);
    const [currentTab, setCurrentTab] = useState(Tab.STATUSES);
    const [statuses, setStatuses] = useState(() => safeParse(settings.store.statuses, []));
    ...
    const plugin = window.Vencord?.Plugins?.plugins?.AnimatedStatus;
    const presets = useMemo(() => safeParse(settings.store.presets, []), [settings.store.presets, presetListTrigger]);
    ...
    const addStatus = () => {
        ...
        if (!parsed.text.trim().length && !(parsed.emojiId || parsed.emojiName))
            return Toasts.show({ message: "Please enter some text or an emoji!", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        const newStatus = { ...parsed, preset: preset.trim() || undefined, status: selectedStatus };
    ...
    const saveEdit = (index) => {
        ...
        if (!card)
            return;
        const textInput = card.querySelector("[data-edit-input=\"text\"]");
        const presetSelect = card.querySelector("[data-edit-input=\"preset\"]");
        const statusBtn = card.querySelector("[data-edit-input=\"status\"][data-selected=\"true\"]");
        if (!textInput)
            return;
        ...
        updated[index] = { ...parsed, preset: presetSelect?.value?.trim() || undefined, status: statusBtn?.dataset?.value || "online" };
        setStatuses(updated);
        settings.store.statuses = JSON.stringify(updated);
        setEditingIndex(null);
    ...
    return (<ErrorBoundary>
            <ModalRoot {...{ transitionState, onClose }} size={"large" /* ModalSize.LARGE */} className={cl("modal-root")}>
                ...
                <ModalContent scrollerRef={scrollerRef} className={cl("modal-content")}>
                    {currentTab === Tab.STATUSES && (<StatusesTab statuses={statuses} filteredStatuses={filteredStatuses} presetNames={presetNames} presets={presets} filterPreset={filterPreset} inputText={inputText} setInputText={setInputText} preset={preset} setPreset={setPreset} selectedStatus={selectedStatus} setSelectedStatus={setSelectedStatus} running={running} editingIndex={editingIndex} previewData={previewData} onAddStatus={addStatus} onDeleteStatus={deleteStatus} onEditStart={setEditingIndex} onEditSave={saveEdit} onEditCancel={cancelEdit} onStart={startAnimation} onStop={stopAnimation} onFilterChange={setFilterPreset}/>)}
                    {currentTab === Tab.PRESETS && (<PresetsTab statuses={statuses} setStatuses={setStatuses} presetNames={presetNames} running={running} runningPreset={runningPreset} onStart={startAnimation} onStop={stopAnimation} onPresetChange={() => setPresetListTrigger(t => t + 1)}/>)}
                    {currentTab === Tab.SETTINGS && (<SettingsTab statuses={statuses} interval={settings.store.interval} randomize={settings.store.randomize} autoStart={settings.store.autoStart} onIntervalChange={val => { settings.store.interval = val; plugin?.updateInterval(val); }} onRandomizeChange={v => settings.store.randomize = v} onAutoStartChange={v => settings.store.autoStart = v} onClearAll={() => { settings.store.statuses = "[]"; setStatuses([]); Toasts.show({ message: "All statuses cleared", type: Toasts.Type.SUCCESS, id: Toasts.genId() }); }}/>)}
                    {currentTab === Tab.INFO && <InfoTab />}
                </ModalContent>
            </ModalRoot>
        </ErrorBoundary>);
}
function StatusesTab({ statuses, filteredStatuses, presetNames, presets, filterPreset, inputText, setInputText, preset, setPreset, selectedStatus, setSelectedStatus, running, editingIndex, previewData, onAddStatus, onDeleteStatus, onEditStart, onEditSave, onEditCancel, onStart, onStop, onFilterChange }) {
    return (<div className={cl("tab-content")}>
            ...
            <Forms.FormSection className={cl("section")}>
                ...
                <div>
                    ...
                    <div className={cl("status-selector")}>{STATUS_OPTIONS.map(status => (<button key={status.value} className={cl("status-btn", { active: selectedStatus === status.value })} onClick={() => setSelectedStatus(status.value)}>
            ...
            <Forms.FormDivider />
            {filterPreset && (<div className={cl("filter-indicator")}>
                    <Text variant="text-sm/normal">Filtered by preset: <strong>{filterPreset}</strong></Text>
                    <Button onClick={() => { onFilterChange(null); if (running)
            onStop(); }} size={Button.Sizes.SMALL} color={Button.Colors.SECONDARY} look={Button.Looks.OUTLINED}>Clear Filter</Button>
                </div>)}
            <Forms.FormSection className={cl("section")}>
                ...
                {filteredStatuses.length === 0 ? (<div className={cl("empty-state")}>
                    ...
                    </div>) : (<div className={cl("status-list")}>
                        {filteredStatuses.map(status => {
                const actualIndex = statuses.indexOf(status);
                const isEditing = editingIndex === actualIndex;
                return (<div key={actualIndex} className={cl("status-card", { editing: isEditing })} data-status-index={actualIndex}>
                                    {isEditing ? (<div className={cl("edit-mode")}>
                                            <div className={cl("edit-inputs")}>
                                                ...
                                                <div className={cl("status-selector")}>{STATUS_OPTIONS.map(s => (<button key={s.value} data-edit-input="status" data-value={s.value} data-selected={s.value === status.status} className={cl("status-btn", { active: s.value === status.status })} onClick={e => { const btn = e.currentTarget; btn.parentElement?.querySelectorAll('[data-edit-input="status"]').forEach(b => b.dataset.selected = "false"); btn.dataset.selected = "true"; }}>
                                            ...
                                            <div className={cl("status-info")}>
                                                ...
                                                {status.preset && (<div className={cl("status-meta")}><span className={cl("meta-tag")}>{status.preset}</span></div>)}
                                            </div>
...
function PresetsTab({ statuses, setStatuses, presetNames, running, runningPreset, onStart, onStop, onPresetChange }) {
    const [newPresetName, setNewPresetName] = useState("");
    const [editingPresetId, setEditingPresetId] = useState(null);
    const [editingPresetName, setEditingPresetName] = useState("");
    const [expandedPresets, setExpandedPresets] = useState(new Set());
    const presets = useMemo(() => safeParse(settings.store.presets, []), [settings.store.presets]);
    const toggleExpanded = (presetId) => {
    ...
    const getStatusesForPreset = (presetId) => {
    ...
    const createPreset = () => {
        ...
        if (!name)
            return Toasts.show({ message: "Please enter a preset name", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        if (presets.some(p => p.name.toLowerCase() === name.toLowerCase()))
            return Toasts.show({ message: "A preset with this name already exists", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        const newPreset = { id: Date.now().toString(), name };
    ...
    const startEditingPreset = (preset) => { setEditingPresetId(preset.id); setEditingPresetName(preset.name); };
    const savePresetEdit = (presetId) => {
        const newName = editingPresetName.trim();
        if (!newName)
            return setEditingPresetId(null);
        if (presets.find(p => p.id !== presetId && p.name.toLowerCase() === newName.toLowerCase()))
            return Toasts.show({ message: "A preset with this name already exists", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        const preset = presets.find(p => p.id === presetId);
        if (!preset)
            return;
    ...
    const runPreset = (preset) => {
        ...
        if (presetStatuses.length === 0)
            return Toasts.show({ message: `No statuses in "${preset.name}". Add some in the Statuses tab!`, type: Toasts.Type.FAILURE, id: Toasts.genId() });
        onStart(preset.name);
    };
    return (<div className={cl("tab-content")}>
            <Forms.FormSection className={cl("section")}>
                ...
                {presets.length === 0 ? (<div className={cl("empty-state")}><div className={cl("empty-icon")}><I.List /></div><Text className={cl("empty-title")} variant="heading-md/semibold">No presets yet</Text><Text className={cl("empty-desc")} variant="text-sm/normal">Create a preset above, then add statuses to it in the Statuses tab</Text></div>) : (<div className={cl("preset-list")}>
                        {presets.map(preset => {
                ...
                return (<div key={preset.id} className={cl("preset-item-wrapper")}>
                                    <div className={cl("preset-item", { running: isRunningThisPreset })}>
                                        <div className={cl("preset-expandable")} onClick={() => !isEditing && toggleExpanded(preset.id)}>
                                            ...
                                            <div className={cl("preset-info")}>
                                                {isEditing ? (<TextInput value={editingPresetName} onChange={setEditingPresetName} placeholder="Preset name..." autoFocus className={cl("edit-input")} style={{ marginBottom: "4px" }}/>) : (<>
                                    ...
                                    {isExpanded && presetStatuses.length > 0 && (<div className={cl("preset-statuses")}>
                                            {presetStatuses.map(status => {
                            const actualIndex = statuses.indexOf(status);
                            return (<div key={actualIndex} className={cl("preset-status-item")}>
...
function SettingsTab({ statuses, interval, randomize, autoStart, onIntervalChange, onRandomizeChange, onAutoStartChange, onClearAll }) {
            ...
            <Forms.FormSection className={cl("section")}>
                ...
                <div className={cl("setting-row")}>
                    ...
                    <div className={cl("interval-input-wrapper")}>
                        <TextInput value={intervalText} onChange={v => { setIntervalText(v); if (!v || /[sm]/i.test(v))
        onIntervalChange(parseInterval(v)); }} onBlur={() => { const current = parseInterval(intervalText); if (current < MIN_INTERVAL) {
        setIntervalText(formatInterval(MIN_INTERVAL));
        onIntervalChange(MIN_INTERVAL);
    } }} placeholder="10s or 1m 30s" className={cl("interval-input")}/>
                    </div>
                </div>
                {interval < 10 && (<div style={{ marginTop: "12px", padding: "12px", background: "rgba(237, 66, 69, 0.1)", border: "1px solid var(--text-danger)", borderRadius: "8px" }}>
