...
function confirmKickAll(memberCount) {
    if (!settings.store.confirmBeforeKick)
        return true;
    return confirm(`⚠️ Are you sure you want to kick all ${memberCount} members from this group?\n\n` +
        "This action cannot be undone.\n" +
        "All members will be removed from the group instantly.");
}
// Function to kick a specific user from a group
async function kickUserFromGroup(channelId, userId) {
    try {
...
async function kickAllMembers(channelId) {
...
- Current user: ${currentUserId}`);
        ...
        log(`🚀 Starting to kick ${recipients.length} member(s) from group "${channelName}"`);
        let successCount = 0;
        let failureCount = 0;
        // Start notification
        if (settings.store.showNotifications) {
...
const GroupContextMenuPatch = (children, { channel }) => {
    ...
    if (group) {
        group.push(<Menu.MenuSeparator />, <Menu.MenuItem id="vc-kick-all-members" label={`🦶 Kick all members (${memberCount})`} color="danger" action={() => kickAllMembers(channel.id)} icon={() => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
export default definePlugin({
    ...
    start() {
        ...
        debugLog(`Notifications: ${settings.store.showNotifications ? "ENABLED" : "DISABLED"}`);
        debugLog(`Confirmation: ${settings.store.confirmBeforeKick ? "ENABLED" : "DISABLED"}`);
        if (settings.store.showNotifications) {
            showNotification({
