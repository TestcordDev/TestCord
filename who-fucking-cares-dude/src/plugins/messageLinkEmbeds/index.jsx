...
import { Button, ChannelStore, GuildStore, IconUtils, MessageStore, Parser, PermissionsBits, PermissionStore, UserStore } from "@webpack/common";
const messageCache = new Map();
const Embed = findComponentLazy(m => m.prototype?.renderSuppressButton);
const ChannelMessage = findComponentByCodeLazy("childrenExecutedCommand:", ".hideAccessories");
let AutoModEmbed = () => null;
...
const settings = definePluginSettings({
    ...
    clearMessageCache: {
        ...
        component: () => (<Button onClick={() => messageCache.clear()}>
...
function noContent(attachments, embeds) {
...
function requiresRichEmbed(message) {
    if (message.components.length)
        return true;
    if (message.attachments.some(a => !a.content_type?.startsWith("image/")))
        return true;
    if (message.embeds.some(e => e.type !== "image" && (e.type !== "gifv" || tenorRegex.test(e.url))))
        return true;
    return false;
}
function computeWidthAndHeight(width, height) {
    const maxWidth = 400;
    const maxHeight = 300;
    if (width > height) {
...
function withEmbeddedBy(message, embeddedBy) {
    return new Proxy(message, {
        get(_, prop) {
            if (prop === "vencordEmbeddedBy")
                return embeddedBy;
...
function MessageEmbedAccessory({ message }) {
    // @ts-expect-error
    const embeddedBy = message.vencordEmbeddedBy ?? [];
    const accessories = [];
    for (const [_, channelID, messageID] of message.content.matchAll(messageLinkRegex)) {
        ...
        if (listMode === "blacklist" && isListed)
            continue;
        if (listMode === "whitelist" && !isListed)
            continue;
        ...
        accessories.push(type === "always" || (type === "prefer" && !requiresRichEmbed(linkedMessage))
            ? <AutomodEmbedAccessory {...messageProps}/>
            : <ChannelMessageEmbedAccessory {...messageProps}/>);
    }
    return accessories.length ? <>{accessories}</> : null;
}
function getChannelLabelAndIconUrl(channel) {
    if (channel.isDM())
        return ["Direct Message", IconUtils.getUserAvatarURL(UserStore.getUser(channel.recipients[0]))];
...
function ChannelMessageEmbedAccessory({ message, channel }) {
    ...
    return (<Embed embed={{
        ...
        }} renderDescription={() => (<div key={message.id} style={!settings.store.messageBackgroundColor ? undefined : {
                backgroundColor: "var(--background-base-lower)",
                border: "1px solid var(--border-subtle)",
                borderRadius: "8px",
                paddingBottom: "8px",
            }}>
                    <ChannelMessage id={`message-link-embeds-${message.id}`} message={message} channel={channel} subscribeToComponentDispatch={false} compact={compact}/>
                </div>)}/>);
}
function AutomodEmbedAccessory(props) {
    const { message, channel } = props;
    const compact = MessageDisplayCompact.useSetting();
    const images = getImages(message);
    const { parse } = Parser;
    const [channelLabel, iconUrl] = getChannelLabelAndIconUrl(channel);
    return <AutoModEmbed channel={channel} childrenAccessories={<BaseText size="xs" weight="medium" color="text-muted" tag="span" className={`${EmbedClasses.embedAuthor} ${EmbedClasses.embedMargin}`}>
            ...
            </BaseText>} compact={compact} content={<>
            ...
            </>} hideTimestamp={false} message={message} _messageEmbed="automod"/>;
}
