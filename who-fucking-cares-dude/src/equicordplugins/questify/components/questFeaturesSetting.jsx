...
import { ManaButton, SettingsCard, SettingsDescription, SettingsHeader, SettingsNotice, SettingsParagraph, SettingsSelect, SettingsSubheader, SettingsSubtleSwitch } from "./shared";
...
const disableManaOptions = disableFeatureOptions.map(({ key, label }) => ({
    id: key,
    label,
    value: key,
}));
const autoCompleteQuestTypeLabels = {
    ["WATCH_VIDEO" /* QuestTaskType.WATCH_VIDEO */]: "Watch Video",
    ["WATCH_VIDEO_ON_MOBILE" /* QuestTaskType.WATCH_VIDEO_ON_MOBILE */]: "Watch Video on Mobile",
    ["ACHIEVEMENT_IN_ACTIVITY" /* QuestTaskType.ACHIEVEMENT_IN_ACTIVITY */]: "Achievement in Activity",
    ["PLAY_ACTIVITY" /* QuestTaskType.PLAY_ACTIVITY */]: "Play Activity",
    ["PLAY_ON_DESKTOP" /* QuestTaskType.PLAY_ON_DESKTOP */]: "Play on Desktop",
    ["PLAY_ON_PLAYSTATION" /* QuestTaskType.PLAY_ON_PLAYSTATION */]: "Play on PlayStation",
    ["PLAY_ON_XBOX" /* QuestTaskType.PLAY_ON_XBOX */]: "Play on Xbox",
};
const autoCompleteQuestTypeOptions = autoCompleteQuestTaskTypes.map(questType => ({
    label: autoCompleteQuestTypeLabels[questType],
    value: questType,
}));
const autoCompleteQuestTypeManaOptions = autoCompleteQuestTypeOptions.map(({ label, value }) => ({
    id: String(value),
    label,
    value: String(value),
    disabled: !isDesktopCompatible(value),
}));
function SettingsAllowDangerousButton({ allowed, disabled, onClick, }) {
    return (<div className={q("settings-button", "allow-dangerous-button")}>
            <ManaButton text={allowed
            ? "Reset and disallow changing dangerous settings..."
            : "Allow changing dangerous settings..."} variant={allowed ? "critical-secondary" : "critical-primary"} fullWidth={true} disabled={disabled} onClick={onClick} size="sm"/>
        </div>);
}
export function QuestFeaturesSetting() {
    ...
    function updateDisableValue(value) {
    ...
    function confirmDangerousSettingChange(body, onConfirm) {
        Alerts.show({
            title: "Are you sure?",
            body,
    ...
    function updateAutoCompleteQuestTypes(value) {
    ...
    function updateDisableEverything(checked) {
        function setDisableEverything() {
            getQuestifySettings().disableQuestsEverything = checked;
        }
        if (checked) {
            confirmDangerousSettingChange("This will completely disable Quest functionality.", () => {
    ...
    function updateDangerousAccess(checked) {
        function setDangerousAccess() {
            getQuestifySettings().allowChangingDangerousSettings = checked;
        }
        if (checked) {
            confirmDangerousSettingChange("This will allow changing dangerous settings.", setDangerousAccess);
        }
        else {
            resetDangerousSettings();
            setDangerousAccess();
    ...
    function updateModifyValue(key, checked) {
    ...
    return (<SettingsCard>
            ...
            <SettingsSubtleSwitch checked={questFeatures.disableQuestsEverything} label="Completely disable Quest functionality:" onChange={updateDisableEverything} bottomSpacing="10" tooltip={{
            position: "top",
            ...
            <SettingsSelect label="Disable specific features:" wrapTags={true} options={disableManaOptions} value={selectedDisableValues} closeOnSelect={false} maxOptionsVisible={7} selectionMode="multiple" disabled={questFeatures.disableQuestsEverything} onSelectionChange={updateDisableValue} tooltip={{
            position: "top",
            ...
            <SettingsNotice className={["notice-card-red", questFeatures.disableQuestsEverything ? "dimmed-settings-item" : undefined, questFeatures.allowChangingDangerousSettings ? undefined : "notice-card-solo", "no-bottom-margin"].filter(c => c !== undefined)}>
                ...
                <SettingsAllowDangerousButton allowed={questFeatures.allowChangingDangerousSettings} disabled={questFeatures.disableQuestsEverything} onClick={() => updateDangerousAccess(!questFeatures.allowChangingDangerousSettings)}/>
                {questFeatures.allowChangingDangerousSettings && <>
                    <SettingsSubtleSwitch disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} checked={questFeatures.completeVideoQuestsQuicker} label="Accelerate Video Quest auto-completion:" onChange={checked => updateModifyValue("completeVideoQuestsQuicker", checked)} topSpacing="10" bottomSpacing="5" tooltip={{
                position: "top",
                text: "Discord allows Video Quests to be completed once 24 seconds less than the duration of the video has passed since you enrolled into the Quest."
                    ...
                    <SettingsSubtleSwitch disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} checked={questFeatures.preventVideoQuestsPausing} label="Prevent Video Quests from pausing when losing focus:" onChange={checked => updateModifyValue("preventVideoQuestsPausing", checked)} bottomSpacing="5" tooltip={{
                position: "top",
                text: "Discord forces you to be tabbed into the client for Video Quests to play their video."
                    ...
                    <SettingsSubtleSwitch disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} checked={questFeatures.makeMobileVideoQuestsDesktopCompatible} label="Make some mobile-only Video Quests completable on desktop:" onChange={checked => updateModifyValue("makeMobileVideoQuestsDesktopCompatible", checked)} bottomSpacing="5" tooltip={{
                position: "top",
                text: "Some mobile-only Video Quests can be enrolled in on desktop, but still must be completed on mobile. This setting will allow those to be completed on desktop."
                    ...
                    <SettingsSubtleSwitch disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} checked={questFeatures.autoCompleteQuestsSimultaneously} label="Auto-complete Quests simultaneously rather than sequentially:" onChange={checked => updateModifyValue("autoCompleteQuestsSimultaneously", checked)} bottomSpacing="5" tooltip={{
                position: "top",
                text: "By default, attempting to auto-complete multiple Quests will queue them to be completed in order."
                    ...
                    <SettingsSubtleSwitch disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} checked={questFeatures.resumeInterruptedQuests} label="Resume interrupted auto-completions after a reload or restart:" onChange={checked => updateModifyValue("resumeInterruptedQuests", checked)} bottomSpacing="5" tooltip={{
                position: "top",
                text: "This setting will automatically resume any interrupted auto-completions caused by reloads or restarts, including requeuing Quests which had yet to start auto-completing but had been queued."
                    ...
                    <SettingsSelect label="Auto-complete specific Quest types:" labelClassName="margin-top-9" wrapTags={true} options={autoCompleteQuestTypeManaOptions} value={selectedAutoCompleteQuestTypeValues} closeOnSelect={false} maxOptionsVisible={7} selectionMode="multiple" disabled={questFeatures.disableQuestsEverything || !questFeatures.allowChangingDangerousSettings} onSelectionChange={updateAutoCompleteQuestTypes} tooltip={{
                position: "top",
                wider: true,
