...
const patchUserContext = (children, { user }) => {
    ...
    if (regularButtonIndex === -1)
        return;
    const newUserNotesButton = <Menu.MenuItem id="vc-open-user-notes" label="Open User Notes" action={() => {
...
function ProfileContainer({ user }) {
    ...
    return (<div className={"vc-user-notes-profile-container"}>
            <TextArea className={"vc-user-notes-profile-text-area"} placeholder="Click to add a note" value={userNotes} onChange={setUserNotes} onBlur={() => saveUserNotes(user.id, userNotes)}/>
            <div style={{ "--vc-plugin-icon-color": iconColor }}>
                <HeaderBarIcon className="vc-plugin-icon-button vc-user-notes-profile-button" iconClassName="vc-plugin-icon-button" icon={PopupIcon} onClick={() => openUserNotesModal(user)} tooltip={"Open User Notes"}/>
            </div>
        </div>);
}
export default definePlugin({
    ...
    notesSectionRender: (userId) => {
        const user = UserStore.getUser(userId);
        return <ProfileContainer user={user}/>;
    },
    addToolBarButton: (children) => {
        if (Array.isArray(children.toolbar))
            return children.toolbar.push(<ErrorBoundary key="user-notes-data" noop={true}>
                    <OpenNotesDataButton />
                </ErrorBoundary>);
        children.toolbar = [
