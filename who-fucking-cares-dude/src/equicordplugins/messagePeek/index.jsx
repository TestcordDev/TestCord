...
const hasRelevantActivity = findByCodeLazy(".OFFLINE||", ".INVISIBLE)return!1");
const ActivityText = findComponentByCodeLazy('"ActivityStatus"');
...
function getActivityIcons(activities, user) {
    if (!activities?.length)
        return null;
    if (!isPluginEnabled(betterActivities.name))
        return null;
    try {
...
function getAttachmentType(contentType = "") {
...
function getMessageContent(message) {
    ...
    if (message.flags & 8192 /* MessageFlags.IS_VOICE_MESSAGE */) {
...
function MessagePreviewContent({ channel, user }) {
    const lastMessage = useStateFromStores([MessageStore], () => MessageStore.getLastMessage(channel.id));
    ...
    return (<div className={classes(ActivityClasses.container, ActivityClasses.textXs, cl("preview"))}>
            <span className={ActivityClasses.truncated}>{authorName}: {content.text}</span>
            {Icon && (<span className={cl("icon")}>
                    <Icon size="xxs" className={ActivityClasses.icon}/>
                </span>)}
        </div>);
}
function SubText({ channel, user, activities, applicationStream, voiceChannel, showActivity }) {
    if (showActivity) {
        return (<ActivityText user={user} activities={activities} voiceChannel={voiceChannel} applicationStream={applicationStream}/>);
    }
    const activityIcons = getActivityIcons(activities, user);
    if (activityIcons) {
        return (<div className={PrivateChannelClasses.subtext}>
...
function shouldShowActivity(lastMessage, hasActivity) {
    if (!hasActivity)
        return false;
    if (!lastMessage)
        return true;
...
export default definePlugin({
    ...
    async start() {
        ...
        for (let i = 0; i < channels.length; i += 5) {
            ...
            await Promise.allSettled(batch.map(channel => MessageActions.fetchMessages({
    ...
    renderMemberListDecorator({ channel }) {
        if (!channel)
            return null;
    ...
    getSubText(props) {
        ...
        const lastMessage = MessageStore.getLastMessage(channel.id);
        const hasActivity = hasRelevantActivity({ activities, status, applicationStream, voiceChannel });
        const showActivity = shouldShowActivity(lastMessage, hasActivity);
        return (<SubText channel={channel} user={user} activities={activities} status={status} applicationStream={applicationStream} voiceChannel={voiceChannel} showActivity={showActivity}/>);
    }
});
