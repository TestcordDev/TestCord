...
waitFor((m) => m?.prototype?.getAvatarURL && m?.prototype?.hasAvatarForGuild, (m) => { UserClass = m; });
...
function parseFakeAccounts() {
    try {
        return JSON.parse(settings.store.fakeAccounts);
    }
    catch {
        return [];
    }
...
function parseAvatar(input) {
    if (!input)
        return null;
    const match = input.match(/avatars\/\d+\/([a-f0-9_]+)(?:\.webp|\.png|\.gif)?/i);
    if (match)
        return match[1];
    if (/^[a-f0-9_]{32,}$/i.test(input))
        return input;
    return null;
}
const _originals = {};
let _fakeSessionActive = false;
let _fakeSessionUser = null;
function buildUserObject(acc) {
    if (!UserClass) {
...
export function activateFakeSession(acc) {
    if (_fakeSessionActive)
...
export function deactivateFakeSession() {
    if (!_fakeSessionActive)
        return;
    const restoreOn = (store, key) => {
        if (_originals[key])
            store[key] = _originals[key];
    };
...
function FakeAccountModal({ modalProps }) {
    const [accounts, setAccounts] = React.useState(parseFakeAccounts());
    ...
    const [activeId, setActiveId] = React.useState(_fakeSessionUser?.id ?? null);
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            ...
            <ModalHeader separator>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style={{ marginRight: "8px", flexShrink: 0, color: "var(--interactive-normal)" }}>
                    <path d="M12 12a5 5 0 1 0 0-10 5 5 0 0 0 0 10zm0 2c-5.33 0-8 2.67-8 4v1h16v-1c0-1.33-2.67-4-8-4z"/>
                </svg>
                <Forms.FormTitle tag="h4" style={{ margin: 0, flex: 1 }}>
                    Fake Accounts
                    {_fakeSessionActive && (<span style={{ color: "var(--status-danger)", fontSize: "12px", marginLeft: "8px" }}>
            ...
            <ModalContent>
                <div style={{ padding: "16px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", marginBottom: "16px" }}>
                        <TabBar type="top" look="brand" selectedItem={tab} onItemSelect={(id) => setTab(id)} style={{ flex: 1 }}>
                            <TabBar.Item id="id">Fetch by User ID</TabBar.Item>
                            <TabBar.Item id="manual">Manual</TabBar.Item>
                        </TabBar>

                        {_fakeSessionActive && (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} style={{ marginLeft: "12px", flexShrink: 0 }} onClick={() => { deactivateFakeSession(); setActiveId(null); }}>
                                ✕ Exit Fake Session
                            </Button>)}
                    </div>

                    {/* Fetch-by-ID input row */}
                    {tab === "id" && (<div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "16px" }}>
                            <div style={{ flex: 1 }}>
                                <TextInput value={userId} placeholder="User ID (e.g. 1460553978973655155)" onChange={(v) => setUserId(v)}/>
                            </div>
                            <Button size={Button.Sizes.MEDIUM} color={Button.Colors.GREEN} disabled={loading || !userId.trim()} onClick={fetchAndAdd}>
                                {loading ? "Fetching…" : "Add"}
                            </Button>
                        </div>)}

                    {/* Manual input row */}
                    {tab === "manual" && (<div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "16px" }}>
                            <div style={{ flex: 1 }}>
                                <TextInput value={manualUsername} placeholder="Username" onChange={(v) => setManualUsername(v)}/>
                            </div>
                            <div style={{ flex: 2 }}>
                                <TextInput value={manualAvatar} placeholder="Avatar URL or hash (optional)" onChange={(v) => setManualAvatar(v)}/>
                            </div>
                            <Button size={Button.Sizes.MEDIUM} color={Button.Colors.GREEN} disabled={!manualUsername.trim()} onClick={addManual}>
                                Add
                            </Button>
                        </div>)}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", maxHeight: "300px", overflowY: "auto" }}>
                        {accounts.length === 0 && (<Forms.FormText style={{ padding: "8px 0" }}>
                                No fake accounts added yet.
                            </Forms.FormText>)}
                        {accounts.map(acc => {
            const isActive = activeId === acc.id;
            return (<div key={acc.id} style={{
                    display: "flex",
                    alignItems: "center",
                    padding: "10px 14px",
                    ...
                    : `https://cdn.discordapp.com/embed/avatars/${parseInt(acc.id) % 6}.png`} style={{ width: 36, height: 36, borderRadius: "50%", flexShrink: 0 }} onError={(e) => { e.target.src = "https://cdn.discordapp.com/embed/avatars/0.png"; }}/>
                                    ...
                                    <Button size={Button.Sizes.SMALL} color={isActive ? Button.Colors.RED : Button.Colors.BRAND} onClick={() => {
                    if (isActive) {
                        deactivateFakeSession();
                        setActiveId(null);
                    }
...
export default definePlugin({
    ...
    injectFakes(realUsers) {
        const fakes = parseFakeAccounts();
        if (!fakes.length || !UserClass)
            return realUsers ?? [];
