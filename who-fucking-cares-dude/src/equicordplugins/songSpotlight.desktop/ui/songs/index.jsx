...
import { CardClasses, ExplicitTag, ImageBrokenIcon, OverlayClasses, PlayButton, PuzzlePieceIcon, TrashIcon, } from "@equicordplugins/songSpotlight.desktop/ui/common";
...
import { Clickable, ContextMenuApi, FluxDispatcher, Menu, React, ScrollerThin, showToast, Toasts, Tooltip, useCallback, useMemo, useRef, useState } from "@webpack/common";
function SongEntry({ entry, number, isLoaded, isPlaying, big, onClick }) {
    const baseSize = big ? "md" : "sm";
    const subSize = big ? "sm" : "xs";
    return (<Clickable onContextMenu={e => ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="vc-songspotlight-song-entry" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label={entry.label}>
                        <Menu.MenuItem id="copy-link" label="Copy link" icon={LinkIcon} action={() => copyWithToast(entry.link)}/>
                        <Menu.MenuItem id="steal-song" label="Steal song" icon={PuzzlePieceIcon} action={async () => {
            ...
            <Flex alignItems="center" gap="4px" className={cl("song-entry")} aria-disabled={!isLoaded} data-playing={isPlaying}>
...
function SongInfo({ owned, song, render, big }) {
    const [playing, setPlaying] = useState(undefined);
    const [loaded, setLoaded] = useState(new Set());
    const setLoadedAudio = useCallback((index, state) => {
        if (state)
            loaded.add(index);
        else
            loaded.delete(index);
    ...
    const duration = useMemo(() => render.form === "single" ?
        render.single.audio?.duration :
        playing !== undefined ?
            render.list[playing].audio?.duration :
            undefined, [playing, render]);
    const baseSize = big ? "md" : "sm";
    const subSize = big ? "sm" : "xs";
    return (<Flex flexDirection="column" gap={0} className={cl("song")}>
            <AudioPlayer audioRef={audioRef} list={audios} playing={playing} setPlaying={setPlaying} setLoadedAudio={setLoadedAudio}/>
            <div className={cl("song-grid")}>
                <Flex gap="8px" alignItems="center" className={cl("song-core")}>
                    <Tooltip text={formatCoverTooltip(song, render)}>
                        {props => (<Flex {...props} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="vc-songspotlight-song" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label={render.label}>
                                            <Menu.MenuItem id="copy-link" label="Copy link" icon={LinkIcon} action={() => copyWithToast(render.link)}/>
                                            {!owned
                    ? (<Menu.MenuItem id="steal-song" label="Steal song" icon={PuzzlePieceIcon} action={() => {
                            const self = useSongStore.getState().self?.data ?? [];
                            if (self.length >= apiConstants.songLimit) {
                                return showToast("You don't have enough space!");
                            }
                    ...
                    : (<Menu.MenuItem id="remove-song" color="danger" label="Remove song" icon={TrashIcon} action={() => {
                            const self = useSongStore.getState().self?.data ?? [];
                            ...
                            openSettingsModal(self.toSpliced(i, 1));
                                        ...
                                        </Menu.Menu>))} justifyContent="center" alignItems="center" gap={0} className={cl("song-cover")}>
                ...
                : (<ImageBrokenIcon className={cl("icon")}/>)}
                    ...
                    <Flex flexDirection="column" justifyContent="center" gap={0} className={cl("clamped")}>
                        ...
                        <BaseText size={baseSize} weight="normal" className={cl("clamped", "sub")} title={render.sublabel}>
                            {render.sublabel}
                        </BaseText>
                    </Flex>
                </Flex>
                ...
                <Flex justifyContent="flex-end" alignItems="flex-end" gap="6px" className={cl("song-player")} data-idle={playing === undefined && !big}>
                    {duration && (<BaseText size={subSize} weight="medium" className={cl("mono", "sub")}>
                            {formatDurationMs(duration)}
                        </BaseText>)}
                    <div className={cl("song-progress-container")}>
                        <ProgressCircle border={2.5} playingRef={playingRef} audioRef={audioRef} className={cl("song-progress")}/>
                        <PlayButton state={playing !== undefined} disabled={loaded.size < 1} onClick={() => {
            if (playing !== undefined)
                return setPlaying(undefined);
            const loadedIndex = loaded.values().toArray().sort()[0];
            if (loadedIndex !== undefined)
                setPlaying(loadedIndex);
        }}/>
                    </div>
                </Flex>
            </div>
            {render.form === "list" && (<>
                    ...
                    <ScrollerThin fade className={cl("song-list")}>
                        <Flex flexDirection="column" gap="3px">
                            {render.list.map((entry, i) => (<SongEntry entry={entry} number={(i + 1).toString()} isLoaded={loaded.has(i)} isPlaying={playing === i} onClick={() => {
                    if (playing === i)
                        setPlaying(undefined);
                    else if (loaded.has(i))
                        setPlaying(i);
                }} big={big} key={entry.link}/>))}
                        </Flex>
                    </ScrollerThin>
                </>)}
        </Flex>);
}
export default function Song({ owned, song, index, big }) {
    const { failed, render } = useRender(song);
    if (failed)
        return null;
    return (<div className={classes(OverlayClasses.overlay, CardClasses.card, cl("song-container", {
