...
export function collectCandidateUrls(source, depth = 0, out = new Set()) {
    if (!source || depth > 2)
        return out;
...
export function orderCandidateUrls(preferred, candidates) {
...
export function ensureGifUrl(url) {
    if (/\.gif(\?|$)/i.test(url))
        return url;
...
export function stripDiscordFormatParam(url) {
    const parsed = new URL(url);
