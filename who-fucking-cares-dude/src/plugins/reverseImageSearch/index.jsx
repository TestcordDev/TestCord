...
function makeSearchItem(src) {
    return (<Menu.MenuItem label="Search Image" key="search-image" id="search-image">
            {Object.keys(Engines).map((engine, i) => {
            const key = "search-image-" + engine;
            return (<Menu.MenuItem key={key} id={key} label={<Flex alignItems="center" gap="0.5em">
                                <img style={{
                        borderRadius: "50%",
                    }} aria-hidden="true" height={16} width={16} src={`https://icons.duckduckgo.com/ip3/${new URL(Engines[engine]).host}.ico`}/>
                                {engine}
                            </Flex>} action={() => search(src, Engines[engine])}/>);
        })}
            <Menu.MenuItem key="search-image-all" id="search-image-all" label={<Flex alignItems="center" gap="0.5em">
                        <OpenExternalIcon height={16} width={16}/>
                        All
                    </Flex>} action={() => Object.values(Engines).forEach(e => search(src, e))}/>
        </Menu.MenuItem>);
}
