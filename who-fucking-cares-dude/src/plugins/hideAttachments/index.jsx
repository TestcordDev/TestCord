...
const saveHiddenMessages = (ids) => set(KEY, [...ids]);
migratePluginSettings("HideMedia", "HideAttachments");
const hasMedia = (msg) => msg.attachments.length > 0 || msg.embeds.length > 0 || msg.stickerItems.length > 0 || msg.components.length > 0;
async function toggleHide(channelId, messageId) {
...
export default definePlugin({
    ...
    messagePopoverButton: {
        icon: ImageInvisible,
        render(msg) {
            if (!hasMedia(msg) && !msg.messageSnapshots.some(s => hasMedia(s.message)))
                return null;
    ...
    renderMessageAccessory({ message }) {
        if (!this.shouldHide(message.id))
            return null;
        return (<span className={classes("vc-hideAttachments-accessory", !message.content && "vc-hideAttachments-no-content")}>
                Media Hidden
            </span>);
    },
    async start() {
