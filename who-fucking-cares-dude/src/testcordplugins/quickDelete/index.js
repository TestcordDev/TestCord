...
function matches(e) {
    ...
    if (bind === "altDelete")
        return e.altKey && e.key === "Delete" && !e.ctrlKey && !e.shiftKey;
    if (bind === "ctrlShiftDelete")
        return e.ctrlKey && e.shiftKey && e.key === "Delete" && !e.altKey;
    return false;
}
async function onKeyDown(e) {
    ...
    const lastMine = [...all].reverse().find((m) => m.author?.id === me && !m.deleted);
    if (!lastMine)
        return;
    e.preventDefault();
    try {
