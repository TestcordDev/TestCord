...
import { getUserOverride, hasFlag, settings } from "./data";
...
export default definePlugin({
    ...
    contextMenus: {
        "user-context"(children, { user }) {
            if (!user)
                return;
            children.push(<Menu.MenuItem id="vc-edit-user" label="Edit User" action={() => openUserEditModal(user)}/>);
        }
    },
    patches: [
        {
        ...
        },
        {
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace: "set pronouns(v){this._vcPronouns=v}" +
    ...
    getUsername: (user) => getUserOverride(user.id).username,
    getAvatarUrl: (user) => getUserOverride(user.id).avatarUrl,
    getBannerUrl: (userId) => getUserOverride(userId).bannerUrl,
    getPronouns: (userId) => getUserOverride(userId).pronouns,
    shouldIgnoreGuildAvatar(user) {
        const { avatarUrl, flags } = getUserOverride(user.id);
        if (avatarUrl && !hasFlag(flags, 4 /* OverrideFlags.KeepServerAvatar */))
            return true;
        return hasFlag(flags, 8 /* OverrideFlags.DisableServerAvatars */);
    },
    shouldIgnoreGuildBanner(userId) {
        const { bannerUrl, flags } = getUserOverride(userId);
        if (bannerUrl && !hasFlag(flags, 16 /* OverrideFlags.KeepServerBanner */))
            return true;
        return hasFlag(flags, 32 /* OverrideFlags.DisableServerBanners */);
    },
    shouldIgnoreNick(userId) {
        if (!userId)
            return false;
        const { username, flags } = getUserOverride(userId);
        if (username && !hasFlag(flags, 1 /* OverrideFlags.PreferServerNicks */))
            return true;
        return hasFlag(flags, 2 /* OverrideFlags.DisableNicks */);
    }
});
