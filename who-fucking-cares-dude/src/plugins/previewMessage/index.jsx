...
const getDraft = (channelId) => DraftStore.getDraft(channelId, DraftType.ChannelMessage);
const getImageBox = (url) => new Promise(res => {
    const img = new Image();
    img.onload = () => res({ width: img.width, height: img.height });
    img.onerror = () => res(null);
    img.src = url;
});
const getAttachments = async (channelId) => await Promise.all(UploadAttachmentStore.getUploads(channelId, DraftType.ChannelMessage)
    .map(async (upload) => {
...
const PreviewIcon = ({ height = 20, width = 20, className }) => {
    return (<svg fill="currentColor" fillRule="evenodd" width={width} height={height} className={className} viewBox="0 0 24 24" style={{ scale: "1.096", translate: "0 -1px" }}>
...
const PreviewButton = ({ isAnyChat, isEmpty, type: { attachments }, channel: { id: channelId } }) => {
    ...
    if (!hasContent && !hasAttachments)
        return null;
    return (<ChatBarButton tooltip="Preview Message" onClick={async () => {
