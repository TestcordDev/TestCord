...
const ErrorBoundary = LazyComponent(() => {
    ...
    return class ErrorBoundary extends Vencord.Webpack.Common.React.PureComponent {
        state = {
            error: NO_ERROR,
            stack: "",
            message: ""
        ...
        render() {
            ...
            if (this.props.fallback)
                return (<this.props.fallback wrappedProps={this.props.wrappedProps} {...this.state}>
                        {this.props.children}
                    </this.props.fallback>);
            const msg = this.props.message || "An error occurred while rendering this Component. More info can be found below and in your console.";
            return (<ErrorCard style={{ overflow: "hidden" }}>
                    <h1>Oh no!</h1>
                    <p>{msg}</p>
                    <code>
                        {this.state.message}
                        {!!this.state.stack && (<pre className={Margins.top8}>
                                {this.state.stack}
                            </pre>)}
                    </code>
                </ErrorCard>);
...
ErrorBoundary.wrap = (Component, errorBoundaryProps) => {
    const wrapper = props => (<ErrorBoundary {...errorBoundaryProps} wrappedProps={props}>
            <Component {...props}/>
        </ErrorBoundary>);
