...
function getStoredReasons() {
    return settings.store.reasons.map(normalizeReason);
}
function ReasonsComponent() {
    ...
    return (<section>
            ...
            {reasons.map((r, i) => (<div key={i} className={cl("reason-wrapper")}>
                    <TextInput value={r.text} onChange={v => update(reasons.map((x, j) => j === i ? { ...x, text: v } : x))} placeholder="Reason"/>
                    <Button className={cl("remove-button")} color={Button.Colors.TRANSPARENT} onClick={() => update(reasons.filter((_, j) => j !== i))} look={Button.Looks.FILLED} size={Button.Sizes.MIN}>
                        <DeleteIcon />
                    </Button>
                    <div className={cl("duration-row")}>
                        ...
                        <Select options={DELETE_DURATION_OPTIONS} select={v => update(reasons.map((x, j) => j === i ? { ...x, deleteSeconds: v === null ? undefined : v } : x))} isSelected={v => (v ?? null) === (r.deleteSeconds ?? null)} serialize={v => v == null ? "default" : String(v)}/>
                    </div>
                </div>))}
            <div className={cl("reason-wrapper")}>
                <Button onClick={() => update([...reasons, { text: "" }])} className={cl("add-button")} size={Button.Sizes.LARGE} color={Button.Colors.TRANSPARENT}>
...
export default definePlugin({
    ...
    captureDeleteState(hook, initial) {
