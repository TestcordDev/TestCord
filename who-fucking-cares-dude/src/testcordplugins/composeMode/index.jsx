...
const ComposeModeIcon = ({ width = 24, height = 24 }) => (<svg width={width} height={height} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
...
const ContextMenuPatch = (children, props) => {
    ...
    if (container && contextMenu) {
        ...
        container.splice(idx + 1, 0, <Menu.MenuCheckboxItem id="vc-compose-mode" label="Compose Mode" checked={isEnabled} action={toggle}/>);
    }
};
const ComposeModeToggleButton = ({ isMainChat }) => {
    const { isEnabled, showIcon } = settings.use(COMPOSE_BUTTON_KEYS);
    if (!isMainChat || !showIcon || settings.store.location !== "chatbar")
        return null;
    return (<ChatBarButton tooltip={isEnabled ? "Disable Compose Mode" : "Enable Compose Mode"} onClick={toggle}>
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("ComposeMode", () => (<HeaderBarButton icon={ComposeModeIcon} tooltip={settings.store.isEnabled ? "Disable Compose Mode" : "Enable Compose Mode"} onClick={toggle}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("ComposeMode", () => (<ChannelToolbarButton icon={ComposeModeIcon} tooltip={settings.store.isEnabled ? "Disable Compose Mode" : "Enable Compose Mode"} onClick={toggle}/>), 5);
        }
    },
    stop() {
