...
function isLegitimateOptimisticConfirmation(action) {
...
function maybeStripAntilogNonce(action) {
    try {
        if (!settings.store.blockNonceAntilog)
            return;
        const message = action?.message;
        if (!message || !message.nonce)
            return;
        // If message.id is equal to nonce, this is an optimistic local message, not an antilog payload
        if (message.id === message.nonce)
            return;
        const channelId = action.channelId ?? message.channel_id;
        if (!channelId)
            return;
        if (isLegitimateOptimisticConfirmation(action))
            return;
...
function preserveRemovedMedia(action) {
    try {
        const newMsg = action?.message;
        if (!newMsg?.id || !newMsg?.channel_id)
            return;
        const old = MessageStore.getMessage(newMsg.channel_id, newMsg.id);
        if (!old)
            return;
        let updated = null;
        const ensureClone = () => {
        ...
        if (settings.store.preserveRemovedEmbeds) {
            ...
            if (oldEmbeds.length > 0) {
                ...
                let removed = [];
                let baseEmbeds = incomingEmbeds ?? oldEmbeds;
                if (!wasSuppressed && nowSuppressed) {
                    ...
                    const fingerprint = (e) => `${e?.type ?? ""}|${e?.url ?? ""}|${e?.timestamp ?? ""}`;
