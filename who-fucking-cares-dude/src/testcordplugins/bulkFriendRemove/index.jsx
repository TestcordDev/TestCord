...
function removeFriend(id) {
    ...
    const token = (() => { try {
        return window.Vencord?.Webpack?.findByProps?.("getToken")?.getToken?.() ?? "";
    }
    catch {
        return "";
    } })();
...
function BulkRemoveIcon({ width = 20, height = 20 }) {
...
function SpinnerIcon() {
    return (<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ animation: "bfr-spin 1s linear infinite" }}>
            <path d="M12 4V2A10 10 0 0 0 2 12h2a8 8 0 0 1 8-8Z"/>
        </svg>);
}
function getAvatarUrl(id, avatar) {
    if (avatar)
        return `https://cdn.discordapp.com/avatars/${id}/${avatar}.webp?size=40`;
...
function loadFriends() {
    try {
        let ids = [];
        try {
            const r = RelationshipStore.getFriendIDs?.();
        ...
        if (!ids.length) {
            try {
                ...
                if (rels)
                    ids = Object.entries(rels).filter(([, t]) => t === 1).map(([id]) => id);
            }
            catch { }
        }
        return ids.map(id => {
...
function BulkFriendRemoveModal({ rootProps }) {
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="bfr-content">
                ...
                <div className="bfr-search-bar">
                    ...
                    {search && (<button className="bfr-search-clear" onClick={() => setSearch("")}>
                ...
                <div className="bfr-list">
                    ...
                    {filtered.map(f => (<div key={f.id} className={`bfr-row ${f.selected ? "bfr-row--selected" : ""}`} onClick={() => !f.removing && toggle(f.id)}>
                ...
                {removing && (<div className="bfr-progress-wrap">
...
function BulkFriendRemoveButton() {
    return (<HeaderBarButton icon={BulkRemoveIcon} tooltip={t("Friends deletion")} onClick={() => openModal(props => <BulkFriendRemoveModal rootProps={props}/>)}/>);
}
