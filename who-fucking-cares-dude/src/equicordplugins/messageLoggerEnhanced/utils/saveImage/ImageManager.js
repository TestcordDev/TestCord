...
*/
import { createStore, del, get, keys, set, } from "@api/DataStore";
...
(async () => {
    try {
        ...
        paths.forEach(path => {
            ...
            if (!str.startsWith(DEFAULT_IMAGE_CACHE_DIR))
                return;
...
export async function getImage(attachmentId, fileExt) {
...
export async function downloadAttachment(attachemnt) {
    if (IS_WEB) {
...
export async function deleteImage(attachmentId) {
    const idbPath = idbSavedImages.get(attachmentId)?.path;
    if (idbPath)
...
async function downloadAttachmentWeb(attachemnt, attempts = 0) {
