...
const TrashIcon = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill={getAccentColor()}>
...
const pendingReplacementDeletes = new Map();
function queueReplacementDelete(nonce) {
...
function consumeReplacementDelete(nonce) {
...
function handleMessageCreate({ message, optimistic, type }) {
    if (!settings.store.deleteReplacementMarker || optimistic || type !== "MESSAGE_CREATE")
        return;
    if (!message.nonce || !consumeReplacementDelete(message.nonce))
        return;
    setTimeout(() => {
        void MessageActions.deleteMessage(message.channel_id, message.id);
    }, settings.store.deleteDelay);
}
async function antiLogDelete(channelId, messageId) {
    try {
...
export default definePlugin({
                ...
                (async () => {
                    try {
                        ...
                        while (userMessages.length < actualCount) {
                            ...
                            for (const msg of messages) {
                                if (msg.author?.id === currentUserId) {
                                    ...
                                    if (userMessages.length >= actualCount)
                                        break;
                                }
                            }
                        ...
                        for (let i = 0; i < userMessages.length; i++) {
                            if (await antiLogDelete(channelId, userMessages[i].id))
                                successCount++;
    ...
    start() {
        ...
        addButton("AntilogPremium", msg => {
            if (msg.author.id !== UserStore.getCurrentUser().id || msg.deleted)
                return null;
            return {
