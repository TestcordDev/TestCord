...
async function loadCover(coverUrl) {
    ...
    if (resolved)
        lruSet(cache, coverUrl, resolved, MAX_CACHE_ENTRIES);
    return resolved;
}
export function TrackCover({ coverUrl, title }) {
    ...
    useEffect(() => {
        ...
        if (!coverUrl || source || delay === undefined)
            return;
        let cancelled = false;
    ...
    if (!source)
        return null;
    return (<img className={cl("cover")} src={source} alt={`${TEXT.cover}: ${title}`} draggable={false} referrerPolicy="no-referrer" onError={() => setSource("")}/>);
}
