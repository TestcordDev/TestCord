...
function TypingIndicator({ channelId, guildId }) {
    const typingUsers = useStateFromStores([TypingStore], () => ({ ...TypingStore.getTypingUsers(channelId) }), null, (old, current) => {
    ...
    const typingUsersArray = Object.keys(typingUsers).filter(id => id !== myId && !(RelationshipStore.isBlocked(id) && !settings.store.includeBlockedUsers) && !(RelationshipStore.isIgnored(id) && !settings.store.includeIgnoredUsers));
    ...
    if (typingUsersArray.length > 0) {
        return (<Tooltip text={tooltipText}>
                {props => (<div className="vc-typing-indicator" {...props}>
                        {((settings.store.indicatorMode & 2 /* IndicatorMode.Avatars */) === 2 /* IndicatorMode.Avatars */) && (<div onClick={e => {
                        e.stopPropagation();
                        e.preventDefault();
                    }} onKeyPress={e => e.stopPropagation()}>
                                <UserSummaryItem users={typingUsersArray.map(id => UserStore.getUser(id))} guildId={guildId} renderIcon={false} max={3} showDefaultAvatarsForNullUsers showUserPopout size={16} className="vc-typing-indicator-avatars"/>
                            </div>)}
                        {((settings.store.indicatorMode & 1 /* IndicatorMode.Dots */) === 1 /* IndicatorMode.Dots */) && (<div className="vc-typing-indicator-dots">
...
const settings = definePluginSettings({
    ...
    indicatorMode: {
        ...
        options: [
            { label: "Avatars and animated dots", value: 1 /* IndicatorMode.Dots */ | 2 /* IndicatorMode.Avatars */, default: true },
            { label: "Animated dots", value: 1 /* IndicatorMode.Dots */ },
            { label: "Avatars", value: 2 /* IndicatorMode.Avatars */ },
        ],
    }
});
export default definePlugin({
    ...
    TypingIndicator: (channelId, guildId) => (<ErrorBoundary noop>
