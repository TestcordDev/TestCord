...
let ghostRoot = null;
let ghostMountNode = null;
let ghostRaf = null;
let ghostPendingPos = null;
...
export function scheduleGhostPosition(x, y) {
    ghostPendingPos = { x, y };
    if (ghostRaf !== null)
        return;
    setGhostState({ x, y });
    ghostRaf = requestAnimationFrame(() => {
        if (ghostPendingPos)
            setGhostState({ x: ghostPendingPos.x, y: ghostPendingPos.y });
...
export function showGhost(next, position) {
    if (ghostHideTimer !== null) {
        clearTimeout(ghostHideTimer);
        ghostHideTimer = null;
    }
    if (position)
        scheduleGhostPosition(position.x, position.y);
...
export function mountGhost() {
    ...
    if (!body)
        return;
    if (ghostRoot) {
        if (ghostMountNode && !ghostMountNode.isConnected)
            body.appendChild(ghostMountNode);
        return;
    }
    if (!ghostMountNode)
        ghostMountNode = document.createElement("div");
    if (!ghostMountNode.isConnected)
        body.appendChild(ghostMountNode);
    ghostRoot = createRoot(ghostMountNode);
    ghostRoot.render(<div className={cl("ghost-container")}>
            <ErrorBoundary noop>
                <DragGhost />
            </ErrorBoundary>
        </div>);
...
const DragGhost = () => {
    ...
    const voiceState = useStateFromStores([VoiceStateStore], () => (state.kind === "user" && state.entityId
    ...
    if (!state.visible)
        return null;
    return (<div className={classes(cl("ghost"), state.exiting ? cl("ghost-exit") : "")} style={{ transform: `translate3d(${state.x}px, ${state.y}px, 0)` }}>
...
function VoiceStateIcon({ className, size = 14 }) {
    return (<svg className={className} xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill="none" viewBox="0 0 24 24">
            <path fill="currentColor" d="M7 2a1 1 0 0 0-1 1v18a1 1 0 1 0 2 0V3a1 1 0 0 0-1-1ZM11 6a1 1 0 1 1 2 0v12a1 1 0 1 1-2 0V6ZM1 8a1 1 0 0 1 2 0v8a1 1 0 1 1-2 0V8ZM16 5a1 1 0 1 1 2 0v14a1 1 0 1 1-2 0V5ZM22 8a1 1 0 0 0-1 1v6a1 1 0 1 0 2 0V9a1 1 0 0 0-1-1Z"/>
        </svg>);
}
function StreamIcon({ className, size = 14 }) {
    return (<svg className={className} xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill="none" viewBox="0 0 24 24">
            <path fill="currentColor" d="M4 5a3 3 0 0 0-3 3v7a3 3 0 0 0 3 3h4.59l-1.3 2.3a1 1 0 1 0 1.74.98L10.84 18h2.32l1.81 3.28a1 1 0 1 0 1.74-.98L15.41 18H20a3 3 0 0 0 3-3V8a3 3 0 0 0-3-3H4Zm0 2h16a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V8a1 1 0 0 1 1-1Zm6 2.57c0-.8.89-1.27 1.54-.82l3.22 2.23a1 1 0 0 1 0 1.64l-3.22 2.23A1 1 0 0 1 10 14.03V9.57Z"/>
        </svg>);
}
function VoiceMutedIcon({ className, size = 14 }) {
    return (<svg className={className} xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill="none" viewBox="0 0 24 24">
