...
export function openCollectionInfoModal(collection) {
    openModal(props => (<InfoModal props={props} title="Collection Information" rows={[
...
export function openGifInfoModal(gif) {
    openModal(props => (<InfoModal props={props} title="Information" rows={[
...
function MoveToCollectionModal({ props, gifId }) {
    return (<Modal {...props} size="sm" title="Move To Collection" actions={[
            { text: "Close", variant: "secondary", onClick: props.onClose }
        ]}>
            ...
            .map(col => (<Button key={col.name} className={cl("button")} onClick={async () => {
...
function CreateCollectionModal({ props, gif }) {
    ...
    return (<Modal {...props} size="sm" title="Create Collection" actions={[
...
function RenameCollectionModal({ props, name }) {
    ...
    const onSubmit = useCallback(async (e) => {
        e?.preventDefault();
        if (!newName.length || tooLong)
            return;
        await renameCollection(name, newName);
        props.onClose();
    }, [newName, name, tooLong, props]);
    return (<Modal {...props} size="sm" title="Rename Collection" actions={[
