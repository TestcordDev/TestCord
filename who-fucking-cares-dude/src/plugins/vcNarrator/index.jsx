...
function formatText(str, user, channel, displayName, nickname) {
    return str
...
function playSample(type) {
    ...
    speak(formatText(settings.store[type + "Message"], currentUser.username, "general", currentUser.globalName ?? currentUser.username, GuildMemberStore.getNick(myGuildId, currentUser.id) ?? currentUser.username));
}
...
export default definePlugin({
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            if (ChannelStore.getChannel(myChanId)?.type === 13 /* Stage Channel */)
                return;
            for (const state of voiceStates) {
                const { userId, channelId, oldChannelId } = state;
                const isMe = userId === myId;
                if (isMe && state.sessionId !== AuthenticationStore.getSessionId())
                    continue;
                if (!isMe) {
                    if (!myChanId)
                        continue;
    ...
    start() {
        if (typeof speechSynthesis === "undefined" || speechSynthesis.getVoices().length === 0) {
            new Logger("VcNarrator").warn("SpeechSynthesis not supported or no Narrator voices found. Thus, this plugin will not work. Check my Settings for more info");
            return;
        }
    },
    settingsAboutComponent() {
        ...
        const types = useMemo(() => Object.keys(settings.def).filter(k => k.endsWith("Message")).map(k => k.slice(0, -7)), []);
        ...
        return (<section>
                ...
                {hasEnglishVoices && (<>
                    ...
                    gap: "1rem",
                            ...
                            {types.map(t => (<Button key={t} onClick={() => playSample(t)}>
                                    {wordsToTitle([t])}
                                </Button>))}
                        </div>
                    </>)}
