...
function TagConfigCard(props) {
    ...
    return (<>
            ...
            if (!userData)
                return null;
            return (<div style={{ display: "flex" }} key={user.id}>
...
const userPatch = (children, { user }) => {
    const buttonElement = <Menu.MenuItem id="vc-tag-group" label="Tag">
            {SavedData.map(tag => {
            const isTagged = SavedData.filter(e => e.tagName === tag.tagName)[0].userIds.includes(user.id);
            return (<Menu.MenuItem label={`${isTagged ? "Remove from" : "Add to"} ${tag.tagName}`} key={`vc-tag-${tag.tagName}`} id={`vc-tag-${tag.tagName}`} action={() => { UserToTagID(user.id, tag.tagName, isTagged); }}/>);
        })}
        </Menu.MenuItem>;
