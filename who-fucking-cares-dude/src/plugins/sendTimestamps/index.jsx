...
function PickerModal(props) {
    ...
    return (<Modal {...props} title="Timestamp Picker" actions={[{
            ...
            <input className={cl("date-picker")} type="datetime-local" value={value} onChange={e => setValue(e.currentTarget.value)} style={{
            colorScheme: getTheme() === 2 /* Theme.Light */ ? "light" : "dark",
        }}/>

            <Heading tag="h5">Timestamp Format</Heading>
            <div className={cl("format-select")}>
                <Select options={Formats.map(m => ({
            label: m,
            value: m
        }))} isSelected={v => v === format} select={v => setFormat(v)} serialize={v => v} renderOptionLabel={o => (<div className={cl("format-label")}>
                            {Parser.parse(formatTimestamp(time, o.value))}
                        </div>)} renderOptionValue={() => rendered}/>
            </div>
...
const SendTimestampIcon = ({ height = 20, width = 20, className }) => {
    return (<svg aria-hidden="true" role="img" width={width} height={height} className={className} viewBox="0 0 24 24" style={{ scale: "1.2" }}>
...
const SendTimestampButton = ({ isAnyChat }) => {
    if (!isAnyChat)
        return null;
    return (<ChatBarButton tooltip="Insert Timestamp" onClick={() => openModal(props => <PickerModal {...props}/>)} buttonProps={{ "aria-haspopup": "dialog" }}>
            <SendTimestampIcon />
        </ChatBarButton>);
};
