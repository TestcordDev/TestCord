...
function encrypt(plaintext, technique) {
    ...
    for (let i = 0; i < raw.length; i++)
        xored[i] = raw[i] ^ Math.floor(xorRng() * 256);
    const substituted = new Uint8Array(raw.length);
    for (let i = 0; i < raw.length; i++)
        substituted[i] = sbox[xored[i]];
    let hex = "";
    for (let i = 0; i < substituted.length; i++) {
        hex += substituted[i].toString(16).padStart(2, "0");
        if (noiseRng() < 0.35)
            hex += charset[Math.floor(noiseRng() * charset.length)];
    }
    return MARKER + hex;
}
function decrypt(ciphertext, technique) {
    ...
    for (let i = 0; i < bytes.length; i++)
        unsubstituted[i] = inv[bytes[i]];
    const plain = new Uint8Array(bytes.length);
    for (let i = 0; i < bytes.length; i++)
        plain[i] = unsubstituted[i] ^ Math.floor(xorRng() * 256);
    let text;
    try {
...
function autoDecrypt(ciphertext) {
    ...
    for (let t = 0; t < 400; t++) {
        ...
        if (result !== null)
            return { text: result, technique: t };
    }
    return null;
}
let encryptionEnabled = false;
function LockIcon({ enabled, width = 20, height = 20 }) {
...
const EncryptButton = ({ type }) => {
    const [enabled, setEnabled] = React.useState(encryptionEnabled);
    if (!["normal", "sidebar"].some(n => type.analyticsName === n) || encryptSettings.store.location !== "chatbar")
        return null;
    const tooltip = enabled
        ? "Encryption active — random technique"
        : "Encryption disabled";
    return (<ChatBarButton tooltip={tooltip} onClick={() => {
...
const DecryptionSetters = new Map();
function DecryptionAccessory({ message }) {
    ...
    if (!decrypted)
        return null;
    return (<span style={{ color: "var(--text-muted)", fontStyle: "italic", fontSize: "0.9em", lineHeight: "1.2rem", display: "block", marginTop: 4 }}>
            <LockIcon enabled={true} width={16} height={16}/>
            {" "}
            {Parser.parse(decrypted)}
            <br />
            (decrypted —{" "}
            <button onClick={() => setDecrypted(undefined)} style={{
            ...
            {" | "}
            <button onClick={() => { navigator.clipboard.writeText(decrypted); Toasts.show({ message: "Copied!", type: Toasts.Type.SUCCESS, id: Toasts.genId() }); }} style={{
            background: "none", border: "none", color: "var(--text-link)",
            cursor: "pointer", padding: 0, font: "inherit", fontStyle: "italic",
        }}>
                Copy
...
const messageContextPatch = (children, { message }) => {
    ...
    try {
        if (!message?.content || !isEncrypted(message.content))
            return;
        children.splice(-1, 0, (<Menu.MenuGroup key="nc-encryption-group">
                <Menu.MenuItem id="nc-decrypt-message" label="🔓 Decrypt message" action={() => {
...
export default definePlugin({
    ...
    dependencies: ["ChatInputButtonAPI", "MessageEventsAPI", "MessageAccessoriesAPI", "HeaderBarAPI", "ContextMenuAPI"],
    settings: encryptSettings,
    chatBarButton: {
        icon: () => <LockIcon enabled={encryptionEnabled}/>,
        render: EncryptButton,
    },
    renderMessageAccessory: (props) => props?.message ? <DecryptionAccessory message={props.message}/> : null,
    start() {
        addContextMenuPatch("message", messageContextPatch);
        const { location } = encryptSettings.store;
        if (location === "headerbar") {
            addHeaderBarButton("EncryptedMessage", () => (<HeaderBarButton icon={() => <LockIcon enabled={encryptionEnabled}/>} tooltip="Encryption" onClick={() => { encryptionEnabled = !encryptionEnabled; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("EncryptedMessage", () => (<ChannelToolbarButton icon={() => <LockIcon enabled={encryptionEnabled}/>} tooltip="Encryption" onClick={() => { encryptionEnabled = !encryptionEnabled; }}/>), 5);
        }
    },
    stop() {
    ...
    async onBeforeMessageSend(_channelId, messageObj) {
        if (!encryptionEnabled || !messageObj.content || messageObj.content.trim().length === 0)
            return;
