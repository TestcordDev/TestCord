...
const imageContextMenuPatch = (children, props) => {
    ...
    children.push(<Menu.MenuGroup id="image-zoom">
            <Menu.MenuCheckboxItem id="vc-square" label="Square Lens" checked={square} action={() => {
            settings.store.square = !square;
        }}/>
            <Menu.MenuCheckboxItem id="vc-nearest-neighbour" label="Nearest Neighbour" checked={nearestNeighbour} action={() => {
            settings.store.nearestNeighbour = !nearestNeighbour;
        }}/>
            <Menu.MenuControlItem id="vc-zoom" label="Zoom" control={(props, ref) => (<Menu.MenuSliderControl ref={ref} {...props} minValue={1} maxValue={50} value={settings.store.zoom} onChange={debounce((value) => settings.store.zoom = value, 100)}/>)}/>
            <Menu.MenuControlItem id="vc-size" label="Lens Size" control={(props, ref) => (<Menu.MenuSliderControl ref={ref} {...props} minValue={50} maxValue={1000} value={settings.store.size} onChange={debounce((value) => settings.store.size = value, 100)}/>)}/>
            <Menu.MenuControlItem id="vc-zoom-speed" label="Zoom Speed" control={(props, ref) => (<Menu.MenuSliderControl ref={ref} {...props} minValue={0.1} maxValue={5} value={settings.store.zoomSpeed} onChange={debounce((value) => settings.store.zoomSpeed = value, 100)} renderValue={(value) => `${value.toFixed(3)}x`}/>)}/>
        </Menu.MenuGroup>);
};
export default definePlugin({
    ...
    makeProps(instance) {
        return {
            ...
            onMouseDown: (e) => this.onMouseDown(e, instance),
    ...
    onMouseOver(instance) {
        instance.setState((state) => ({ ...state, mouseOver: true }));
    },
    onMouseOut(instance) {
        instance.setState((state) => ({ ...state, mouseOver: false }));
    },
    onMouseDown(e, instance) {
        if (e.button === 0 /* left */)
            instance.setState((state) => ({ ...state, mouseDown: true }));
    },
    onMouseUp(instance) {
        instance.setState((state) => ({ ...state, mouseDown: false }));
    },
    start() {
