...
export async function updateAuth(newAuth) {
    return DataStore.update(DATA_STORE_KEY, auth => {
        ...
        if (newAuth.user)
            Auth.user = newAuth.user;
        return auth;
    });
}
export function authorize(callback) {
    openModal(props => <OAuth2AuthorizeModal {...props} scopes={["identify"]} responseType="code" redirectUri="https://manti.vendicated.dev/api/reviewdb/auth" permissions={0n} clientId="915703782174752809" cancelCompletesFlow={false} callback={async (response) => {
            try {
