...
const UploadContextMenuPatch = (children, props) => {
    if (children.find(c => c?.props?.id === "vc-remix"))
        return;
    children.push(<Menu.MenuItem id="vc-remix" label="Remix" action={() => {
...
const MessageContextMenuPatch = (children, props) => {
    ...
    if (!url)
        return;
    if (props.attachment && !validMediaTypes.includes(props.attachment.content_type))
        return;
    const group = findGroupChildrenByChildId("copy-text", children);
    if (!group)
        return;
    if (group.find(c => c?.props?.id === "vc-remix"))
        return;
    const index = group.findIndex(c => c?.props?.id === "copy-text");
    group.splice(index + 1, 0, <Menu.MenuItem id="vc-remix" label="Remix" icon={PaintbrushIcon} action={() => {
            const key = openModal(modalProps => <RemixModal modalProps={modalProps} close={() => closeModal(key)} url={url}/>);
        }}/>);
};
export function sendRemix(blob) {
    ...
    if (reply)
        FluxDispatcher.dispatch({ type: "DELETE_PENDING_REPLY", currentChannelId });
