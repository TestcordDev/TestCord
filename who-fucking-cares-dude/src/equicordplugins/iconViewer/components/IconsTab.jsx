...
function getIcons() {
    ...
    if (!iconsModule)
        return {};
    cachedIcons = Object.fromEntries(Object.entries(iconsModule).filter(([name, fn]) => typeof fn === "function" && name.endsWith("Icon")));
    return cachedIcons;
}
function searchMatch(search, name, Icon, searchByFunction) {
    if (!search)
        return true;
    if (searchByFunction)
...
function IconsTab() {
    ...
    const debouncedSetSearch = useMemo(() => debounce((query) => setSearch(query), 150), []);
    const onSearch = useCallback((query) => {
        setSearchInput(query);
        debouncedSetSearch(query);
    }, [debouncedSetSearch]);
    const filteredIcons = useMemo(() => Object.entries(icons).filter(([name, Icon]) => searchMatch(search, name, Icon, searchByFunction)), [icons, search, searchByFunction]);
    ...
    return (<SettingsTab>
            <div className={classes(Margins.top16, "vc-icon-tab-search-bar-grid")}>
                ...
                <TooltipContainer text="Search by function context">
                    <Button size="small" aria-label="Search by function context" className="vc-icon-search-func-btn" variant={searchByFunction ? "positive" : "primary"} onClick={() => setSearchByFunction(!searchByFunction)}>
                        Func
                    </Button>
                </TooltipContainer>
            </div>
            <div className="vc-icons-tab-grid-container">
                {visibleIcons.map(([iconName, Icon]) => (<IconItem key={iconName} iconName={iconName} Icon={Icon}/>))}
            </div>
