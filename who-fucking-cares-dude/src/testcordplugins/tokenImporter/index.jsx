...
const Native = VencordNative.pluginHelpers.TokenImporter;
...
function routeToLoginManager(accounts) {
    const plugin = Vencord.Plugins.plugins.TokenLoginManager;
    if (!plugin)
        return { ok: false, reason: "TokenLoginManager plugin not found" };
    if (!Vencord.Plugins.isPluginEnabled("TokenLoginManager"))
...
async function encryptIfEnabled(tok) {
    if (!settings.store.encryptStoredTokens)
        return tok;
    if (tok.startsWith("dQw4w9WgXcQ:"))
        return tok;
    try {
...
function getAccounts() {
    ...
    if (!loadPromise) {
        loadPromise = DataStore.get(STORE_KEY).then(async (v) => {
            const raw = v ?? [];
            const decrypted = [];
            for (const a of raw) {
...
function switchToAccount(token, userId) {
    try {
        const isMultiInstance = window.location.href.includes("multi-instance=true") || window.IS_MULTI_INSTANCE;
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent>
                ...
                <div className="ti-list" style={{ maxHeight: 120, marginBottom: 12 }}>
                    {invalidAccounts.map(a => (<div key={a.id} className="ti-row ti-row--invalid">
                            {a.avatar
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                    ...
                    <button className="ti-del-btn" style={{ padding: "4px 12px", fontSize: 13 }} onClick={() => { onConfirm(); rootProps.onClose(); }}>
                        Remove invalid
                    </button>
                </div>
            </ModalContent>
...
function TokenModal({ rootProps }) {
    ...
    async function processTokens(raw, destOverride) {
        ...
        if (!tokens.length) {
            setResults([{ token: "No tokens found", status: "invalid" }]);
            return;
        }
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            try {
                ...
                if (result.valid && result.user) {
                    ...
                    updated[i] = { ...updated[i], status: result.error === "rate_limited" ? "rate_limited" : result.error ? "error" : "invalid" };
                }
            }
            catch {
        ...
        if (dest !== "importer" && validForRouting.length) {
            ...
            if (!routed.ok)
                ...
                showToast(`Sent ${validForRouting.length} account(s) to Login Manager`, Toasts.Type.SUCCESS);
        }
        setChecking(false);
        setDone(true);
    }
    ...
    return (<ModalRoot {...rootProps} size="medium">
                    ...
                    try {
                        ...
                        if (addedCount > 0) {
                            ...
                            if (!mountedRef.current)
                                return;
                            setAccounts([...existing]);
                            window.Vencord?.Webpack?.findByProps?.("showToast")?.showToast?.(`${addedCount} new accounts imported!`);
                        }
                        else {
                            window.Vencord?.Webpack?.findByProps?.("showToast")?.showToast?.("No new accounts found.");
                        }
                    }
                    catch (err) {
                        ...
                        if (mountedRef.current)
                            ...
                            <button className="ti-verify-btn" style={{ marginRight: 6, opacity: copied ? 0.7 : 1 }} onClick={() => { copyMyToken(); setCopied(true); if (copiedTimer.current)
            clearTimeout(copiedTimer.current); copiedTimer.current = setTimeout(() => { copiedTimer.current = null; if (mountedRef.current)
            setCopied(false); }, 1500); }}>
                ...
                : accounts.length === 0 ? <div className="ti-empty">No accounts — add tokens via the tab above.</div>
                    : filteredAccounts.length === 0 ? <div className="ti-empty">No accounts match your search.</div>
                        : <div className="ti-list">
                                ...
                                return (<div key={a.id} className={`ti-row ti-row--${st === "invalid" ? "invalid" : st === "error" ? "warn" : st === "valid" ? "valid" : "idle"}`}>
                                                    {a.avatar ? <img src={a.avatar} className="ti-avatar" alt="" onError={e => { e.target.style.display = "none"; }}/>
                                        : <div className="ti-avatar ti-avatar--ph">{a.username?.[0]?.toUpperCase() ?? "?"}</div>}
                                        ...
                                        const native = window.DiscordNative?.clipboard?.copy;
                                        if (typeof native === "function")
                                            native(a.token);
                                        else
                                            navigator.clipboard.writeText(a.token);
                                        const { showToast, Toasts } = window.Vencord.Webpack.common;
                ...
                {tab === "add" && (<div className="ti-add-body">
                        ...
                        {results.length > 0 && (<div className="ti-results">
                                {done && (<div className="ti-results-summary">
                                ...
                                <div className="ti-list">
                                    {results.map((r, i) => (<div key={i} className={`ti-row ti-row--${r.status === "valid" ? "valid" : r.status === "checking" ? "idle" : "invalid"}`}>
...
function useDangerousToggle(key) {
    const current = settings.use([key])[key];
    const [acked, setAcked] = useState(new Set());
    useEffect(() => {
        let cancelled = false;
        getAckedDangerous().then(v => { if (!cancelled)
            setAcked(v); });
        return () => { cancelled = true; };
    }, []);
    const set = useCallback((next) => {
        if (!next) {
            settings.store[key] = false;
            return;
        }
        if (acked.has(key)) {
...
function TokenImporterAbout() {
    ...
    const encrypt = settings.use(["encryptStoredTokens"]).encryptStoredTokens;
    const row = (label, desc, checked, onChange, dangerous = true) => (<div style={{ padding: "10px 0", borderBottom: "1px solid var(--background-modifier-accent)" }}>
    ...
    return (<Forms.FormSection>
            ...
            {row("Encrypt stored tokens at rest", "Uses Electron safeStorage (OS keychain / DPAPI). Strongly recommended.", encrypt, v => { settings.store.encryptStoredTokens = v; }, false)}
...
export default definePlugin({
    ...
    async _injectAccounts() {
        ...
        try {
            ...
            console.error("[TokenImporter] inject:", e);
        }
    },
    stop() {
        this._started = false;
