...
let rulesCache = null;
let rulesCacheKey = null;
function parseRules() {
    const raw = settings.store.rules;
    if (rulesCache && rulesCacheKey === raw)
        return rulesCache;
    try {
...
async function addReactions(channelId, messageId, reactions) {
    for (const emoji of reactions) {
        ...
        try {
            ...
            if (e?.status !== 404)
                console.error("[AutoChannelReact]", e);
        }
    }
}
function handleMessageCreate(data) {
    const { message } = data;
    if (!message || message.author?.id === UserStore.getCurrentUser().id)
        return;
    const rules = parseRules();
    const rule = rules.find(r => r.channelId === message.channel_id);
    if (!rule || rule.reactions.length === 0)
        return;
            ...
            if (trimmed.length > 0 && trimmed.length <= 10) {
                ...
                setInputValue("");
                    ...
                    <div style={{ marginTop: "8px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        {["👍", "❤️", "😂", "🔥", "👀", "💯", "🎉", "😎", "👌", "💪", "🙌", "✨"].map(emoji => (<button key={emoji} type="button" onClick={() => addEmoji({ name: emoji, id: null, animated: false })} style={{ fontSize: "20px", padding: "4px 8px", cursor: "pointer", background: "var(--background-secondary)", border: "none", borderRadius: "4px" }}>
                                {emoji}
                            </button>))}
                    </div>
                </div>
...
function ChannelContextMenuPatch() {
    return (children, props) => {
        ...
        children.splice(-1, 0, (<Menu.MenuGroup>
                <Menu.MenuItem id="autochannelreact" label={hasRule ? "Edit AutoReact" : "AutoReact Every Msg"} action={() => openModal(modalProps => (<EmojiPickerModal {...modalProps} channelId={channel.id} channelName={channel.name || channel.id}/>))}/>
            </Menu.MenuGroup>));
    };
}
