...
*/
export function makeLazy(factory, attempts = 5) {
    let tries = 0;
    let cache;
    let resolved = false;
    return () => {
...
for (const method of [
    ...
    handler[method] =
        (target, ...args) => Reflect[method](target[SYM_LAZY_GET](), ...args);
}
 ...
 */
export function proxyLazy(factory, attempts = 5, isChild = false) {
    let isSameTick = true;
    if (!isChild)
