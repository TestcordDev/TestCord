...
import { makeEmptyOverride, seasonalSounds, soundTypes } from "./types";
...
export const getCustomSoundURL = (data) => {
    ...
    if (override.selectedSound !== "default" && override.selectedSound !== "custom") {
        ...
        if (soundType?.seasonal) {
            const seasonalId = soundType.seasonal.find(seasonalId => seasonalId.startsWith(`${override.selectedSound}_`));
...
export async function refreshDataURI(id) {
...
const soundSettings = Object.fromEntries(allSoundTypes.map(type => [
    type.id,
    {
...
const settings = definePluginSettings({
    ...
    overrides: {
        ...
        component: () => {
                    ...
                    reader.onload = async (e) => {
                        try {
                            ...
                            if (imported.overrides && Array.isArray(imported.overrides)) {
                                imported.overrides.forEach((setting) => {
                                    if (setting.id) {
                                        const override = {
            ...
            const filteredSoundTypes = allSoundTypes.filter(type => type.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            ...
            return (<div>
                    <div className="vc-custom-sounds-buttons">
                        ...
                        <input className={cl("file-input")} ref={fileInputRef} type="file" accept=".json" onChange={handleSettingsUpload}/>
                    </div>

                    <div className={cl("search")}>
                        <Heading>Search Sounds</Heading>
                        <TextInput value={searchQuery} onChange={e => setSearchQuery(e)} placeholder="Search by name or ID"/>
                    </div>

                    <div className={cl("sounds-list")}>
                        {filteredSoundTypes.map(type => {
                    const currentOverride = getOverride(type.id);
                    return (<SoundOverrideComponent key={`${type.id}-${resetTrigger}`} type={type} override={currentOverride} onChange={async () => {
...
export function isOverriden(id) {
    return !!getOverride(id)?.enabled;
}
export function findOverride(id) {
    const override = getOverride(id);
