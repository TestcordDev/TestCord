...
function ApiKeyNotice() {
    ...
    return (<div className={cl("settings-notice")}>
...
const Native = VencordNative.pluginHelpers.DsaWarnings;
...
function hasLightProfileTheme(displayProfile) {
...
function buildDsaBrowseUrl(parsedId) {
...
function getBreachSummary(breach) {
    const parts = [
...
function formatIllegality(value) {
...
function StatusCard({ title, message, onClick }) {
    const content = (<div className={cl("card-top")}>
    ...
    if (!onClick)
        return <div className={cl("card")}>{content}</div>;
    return (<Clickable className={cl("card")} onClick={onClick} aria-label={title}>
            {content}
        </Clickable>);
}
function UserInfoSection({ userInfo }) {
    ...
    return (<div className={cl("user-info")}>
            {userInfo.avatar && (<img className={cl("user-avatar")} src={`https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}.${userInfo.avatar.startsWith("a_") ? "gif" : "png"}?size=64`} alt=""/>)}
            <div className={cl("user-details")}>
                {userInfo.global_name && (<BaseText className={cl("user-display")} size="lg" weight="bold" defaultColor={false}>
                        {userInfo.global_name}
                    </BaseText>)}
                {handle && (<BaseText className={cl("user-handle")} size="sm" weight="medium" defaultColor={false}>
                        {handle}
                    </BaseText>)}
                <BaseText className={cl("user-id")} size="xs" weight="medium" defaultColor={false}>
                    {userInfo.id}
                </BaseText>
                {userInfo.public_flags != null && userInfo.public_flags !== 0 && (<BaseText className={cl("user-flags")} size="xs" weight="medium" defaultColor={false}>
                        Flags: {userInfo.public_flags}
                    </BaseText>)}
                {guild && (<BaseText className={cl("user-clan")} size="xs" weight="medium" defaultColor={false}>
                        Clan: [{guild.tag}]
                    </BaseText>)}
            </div>
        </div>);
}
function ActionDetailRow({ label, value }) {
    return (<div className={cl("detail-row")}>
...
const DsaWarningsCollection = ErrorBoundary.wrap(function DsaWarningsCollection({ user, displayProfile, isSideBar = false }) {
    ...
    const content = (<section className={classes(cl("section"), isLightTheme && cl("light"))}>
            ...
            {isReady && userInfo && (<UserInfoSection userInfo={userInfo}/>)}

            <div className={cl("list")}>
                {result == null && (<StatusCard title="Loading Warnings" message="Fetching active DSA warnings for this profile."/>)}
            ...
            return (<Clickable className={cl("card")} key={action.uuid} onClick={() => VencordNative.native.openExternal(buildDsaBrowseUrl(action.parsedId))} aria-label={`Open DSA warning ${formatLabel(action.category)}`}>
                                ...
                                <div className={cl("card-left")}>
                                    ...
                                    <div className={cl("card-content")}>
                                        <div className={cl("chip-row")}>
                                            ...
                                            {restrictionLabels.map(label => (<span className={cl("chip", "chip-restriction")} key={label}>
                                                    {formatLabel(label)}
                                                </span>))}
                                            {illegality === "Yes" && (<span className={cl("chip", "chip-illegal")}>Illegal</span>)}
                                        </div>
                                        ...
                                        <div className={cl("details")}>
                                            {action.incompatibleContentExplanation && (<ActionDetailRow label="Explanation" value={action.incompatibleContentExplanation}/>)}
                                            {action.incompatibleContentGround && (<ActionDetailRow label="Ground" value={formatLabel(action.incompatibleContentGround)}/>)}
                                            {action.decisionGround && action.decisionGround !== action.incompatibleContentGround && (<ActionDetailRow label="Decision ground" value={formatLabel(action.decisionGround)}/>)}
                                            {action.categorySpecificationOther && (<ActionDetailRow label="Sub-category" value={action.categorySpecificationOther}/>)}
                                            {action.sourceType && (<ActionDetailRow label="Source" value={formatLabel(action.sourceType)}/>)}
                                            {action.automatedDetection && action.automatedDetection !== "" && (<ActionDetailRow label="Automated" value={String(action.automatedDetection)}/>)}
                                            {illegality != null && (<ActionDetailRow label="Illegal content" value={illegality}/>)}
                                        </div>
                                        {!!tags.length && (<div className={cl("chip-row")}>
                                                {tags.map(tag => (<span className={cl("chip", "chip-tag")} key={tag}>
                                                        {formatLabel(tag)}
                                                    </span>))}
                                            </div>)}
                                    </div>
                            ...
                            <div className={cl("card-meta")}>
                                ...
                                {action.createdAt && action.createdAt !== action.applicationDate && (<BaseText className={cl("date")} size="xs" weight="medium" defaultColor={false}>
            ...
            return (<Clickable className={classes(cl("card"), cl("card-breach"))} key={`${breach.source}-${breach.id || breach.no || index}`} onClick={() => VencordNative.native.openExternal(buildCordCatUrl(user.id))} aria-label={`Open breach result ${getBreachName(breach)}`}>
                            <div className={cl("card-top")}>
                                <div className={cl("card-left")}>
                                    ...
                                    <div className={cl("card-content")}>
                                        ...
                                        <div className={cl("details")}>
                                            {breach.ip && breach.ip !== "None" && (<ActionDetailRow label="IP" value={breach.ip}/>)}
                                            {breach.discordid && (<ActionDetailRow label="Discord ID" value={breach.discordid}/>)}
                                            {(breach.discriminator || breach.tag) && (<ActionDetailRow label="Tag" value={`#${breach.discriminator || breach.tag}`}/>)}
                                        </div>
                                        {!!tags.length && (<div className={cl("chip-row")}>
                                                {tags.map(tag => (<span className={cl("chip", "chip-breach-tag")} key={tag}>
                                                        {formatLabel(tag)}
                                                    </span>))}
                                            </div>)}
                                    </div>
                ...
                {isReady && actions.length === 0 && breaches.length === 0 && (<StatusCard title="No Intelligence Results" message="No active warnings or breach results were returned for this profile." onClick={() => VencordNative.native.openExternal(buildDsaBrowseUrl(user.id))}/>)}
                {isReady && breachStatus === "error" && (<StatusCard title="Breach Lookup Error" message={breachError ?? "The upstream breach provider returned an error for this lookup."} onClick={() => VencordNative.native.openExternal(buildCordCatUrl(user.id))}/>)}
                {isReady && breachStatus === "unavailable" && breaches.length === 0 && (<StatusCard title="Breach Lookup Unavailable" message="CordCat returned the user intelligence report, but the upstream breach provider was blocked or unavailable for this lookup." onClick={() => VencordNative.native.openExternal(buildCordCatUrl(user.id))}/>)}
                {isUnavailable && (<StatusCard title="Lookup Unavailable" message={result.error ?? "The DSA service is temporarily unavailable. Click to retry in the public lookup page."} onClick={() => VencordNative.native.openExternal(buildDsaBrowseUrl(user.id))}/>)}
                {isError && (<StatusCard title="Lookup Failed" message={result.error ?? "The DSA lookup request failed. Click to retry with a local lookup window."} onClick={openCaptchaWindow}/>)}
            </div>
            {isReady && (actions.length > MAX_VISIBLE_CARDS || breaches.length > MAX_VISIBLE_CARDS || actions.length + breaches.length > MAX_VISIBLE_CARDS) && (<Clickable className={cl("toggle")} onClick={() => setExpandedUserId(current => current === user.id ? null : user.id)} aria-label={isExpanded ? "Show fewer DSA results" : "Show all DSA results"}>
...
export default definePlugin({
    ...
    renderProfileCollection: {
        priority: 0,
        render: (props) => <DsaWarningsCollection {...props}/>,
    },
});
