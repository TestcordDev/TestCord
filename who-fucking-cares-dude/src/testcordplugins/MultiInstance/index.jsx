...
const Native = VencordNative?.pluginHelpers?.MultiInstance;
const ICON_SETTING_KEYS = ["showIcon"];
const SESSION_SETTING_KEYS = ["blockExternalTokenAccess", "performanceMode"];
const DOMAINS = ["discord.com", "ptb.discord.com", "canary.discord.com"];
const DOMAIN_LABELS = {
    "discord.com": "Discord",
    "ptb.discord.com": "PTB",
    "canary.discord.com": "Canary"
};
const DEFAULT_DOMAIN = "discord.com";
const DEFAULT_PROFILES = [{ id: "secondary", name: "Secondary Discord", domain: DEFAULT_DOMAIN }];
...
function shouldSaveSession(profile) {
    if (settings.store.blockExternalTokenAccess)
        return false;
...
export function MultiInstanceIcon({ width = 20, height = 20, className }) {
    return (<svg className={className} width={width} height={height} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
function MultiInstanceSettingsButton() {
    return (<Button size="small" variant="secondary" onClick={openMultiInstanceModal}>
            Open Multi Instance
        </Button>);
}
...
function TokenModal({ profile, onSave, rootProps }) {
    const [tokenVal, setTokenVal] = React.useState(profile.token ?? "");
    return (<Modal {...rootProps} title={`Account Token — ${profile.name}`} size="small">
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <TextInput value={tokenVal} placeholder="Discord User Token (e.g. OT...)" onChange={setTokenVal}/>
                <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "8px" }}>
                    {profile.token && (<Button size="small" variant="secondary" color="danger" onClick={() => {
                onSave(undefined);
                rootProps.onClose();
                showToast(`Token removed from ${profile.name}.`, Toasts.Type.SUCCESS);
                    ...
                    <Button size="small" variant="secondary" onClick={rootProps.onClose}>
                        Cancel
                    </Button>
                    <Button size="small" variant="positive" onClick={() => {
...
function MultiInstanceModal({ rootProps }) {
    ...
    const refreshInstances = React.useCallback(async () => {
        ...
        const openInstances = await Native.getOpenInstances().catch(() => []);
    ...
    function changeProfiles(change) {
    ...
    async function openInstance(profile, mode = profile.mode ?? "detached") {
        if (!Native) {
    ...
    async function closeInstance(profile) {
        if (!Native) {
            showToast("Multi Instance native helper is not available in this build.", Toasts.Type.FAILURE);
            return;
        }
    ...
    async function clearSavedSession(profile) {
        if (!Native) {
            showToast("Multi Instance native helper is not available in this build.", Toasts.Type.FAILURE);
            return;
        }
    ...
    function toggleSessionSaving(profile) {
        updateProfile(profile.id, { saveSession: !shouldSaveSession(profile) });
    }
    function cycleDomain(profile) {
    ...
    async function removeInstance(profile) {
        if (instances.some(instance => instance.id === profile.id))
            await closeInstance(profile);
        changeProfiles(profiles => {
    ...
    function openInstanceMenu(event, profile, status) {
        event.preventDefault();
        const isBusy = busyId === profile.id || busyId === ALL_INSTANCES_BUSY_ID;
        ContextMenuApi.openContextMenu(event, () => (<Menu.Menu navId="multi-instance-profile-menu" onClose={ContextMenuApi.closeContextMenu} aria-label={`${profile.name} options`}>
                <Menu.MenuItem id="multi-instance-open-grouped" label="Open grouped with Discord" disabled={!!status || isBusy} action={() => void openInstance(profile, "grouped")}/>
                <Menu.MenuItem id="multi-instance-open-detached" label="Open separate Testcord window" disabled={!!status || isBusy} action={() => void openInstance(profile, "detached")}/>
                {status && (<>
                        <Menu.MenuItem id="multi-instance-focus" label="Focus instance" disabled={isBusy} action={() => void openInstance(profile, status.mode)}/>
                        <Menu.MenuItem id="multi-instance-close" label="Close instance" disabled={isBusy} action={() => void closeInstance(profile)}/>
                    </>)}
                <Menu.MenuSeparator />
                <Menu.MenuItem id="multi-instance-rename" label="Rename profile" disabled={isBusy} action={() => startRename(profile)}/>
                <Menu.MenuItem id="multi-instance-token" label={profile.token ? "Edit saved token" : "Set account token"} disabled={isBusy} action={() => openModal(props => <TokenModal profile={profile} onSave={t => updateProfile(profile.id, { token: t })} rootProps={props}/>)}/>
                <Menu.MenuItem id="multi-instance-session" label={shouldSaveSession(profile) ? "Use a temporary session" : "Save this session"} disabled={isBusy || !!status || blockExternalTokenAccess} action={() => toggleSessionSaving(profile)}/>
                <Menu.MenuItem id="multi-instance-domain" label={`Switch to ${DOMAIN_LABELS[DOMAINS[(DOMAINS.indexOf(getDomain(profile)) + 1) % DOMAINS.length]]}`} disabled={isBusy || !!status} action={() => cycleDomain(profile)}/>
                <Menu.MenuItem id="multi-instance-clear" label="Clear saved session" disabled={isBusy || !!status} action={() => void clearSavedSession(profile)}/>
                <Menu.MenuItem id="multi-instance-remove" label="Remove profile" color="danger" disabled={isBusy} action={() => void removeInstance(profile)}/>
            </Menu.Menu>));
    }
    return (<Modal {...rootProps} size="xl" title="Multi Instance Studio" subtitle="Left click Open to launch or focus. Right click a profile for grouped, separate and advanced options.">
            <div className="vc-multi-instance-body">
                {!Native && (<div className="vc-multi-instance-warning">
            ...
            : <InstanceAvatar />}
                ...
                {blockExternalTokenAccess && (<div className="vc-multi-instance-warning">
                ...
                <div className="vc-multi-instance-toolbar">
                    ...
                    <div className="vc-multi-instance-toolbar-actions">
                        ...
                        <Button size="small" variant="secondary" disabled={!instances.length || busyId === ALL_INSTANCES_BUSY_ID} onClick={() => void closeAllInstances()}>
                            Close all
                        </Button>
                    </div>
                </div>
            ...
            return (<div className={classes("vc-multi-instance-row", isOpen && "vc-multi-instance-row-open")} key={profile.id} onContextMenu={event => openInstanceMenu(event, profile, status)}>
                                <div className="vc-multi-instance-row-info">
                                    ...
                                    <div className="vc-multi-instance-profile">
                                        {isEditing ? (<div className="vc-multi-instance-rename">
                                                <div className="vc-multi-instance-rename-input">
                                                    <TextInput value={editingName} placeholder="Instance name" onChange={setEditingName}/>
                                                </div>
                ...
                <div className="vc-multi-instance-add">
                    ...
                    <div className="vc-multi-instance-add-controls">
                        <div className="vc-multi-instance-add-input">
                            <TextInput value={newName} placeholder="Profile name" onChange={setNewName}/>
                        </div>
...
function MultiInstanceButton() {
    ...
    if (!showIcon)
        return null;
    return (<HeaderBarButton icon={MultiInstanceIcon} tooltip="Multi Instance" onClick={openMultiInstanceModal}/>);
}
