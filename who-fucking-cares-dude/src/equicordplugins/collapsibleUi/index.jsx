...
import { addSurfacePropsProvider, notifySurfaceClassesChanged } from "@api/SurfaceClasses";
...
import { collapseSettingKeys, panelRegistry, setCollapseSettingChangeHandler, settings, setUserAreaDetachSettingChangeHandler, toolbarPanelOrder } from "./settings";
import managedStyle from "./style.css?managed";
const cl = classNameFactory("vc-collapsible-ui-");
const panelDependentSurfaces = {
...
let userAreaDragState;
let detachedUserAreaDragPosition;
let detachedUserAreaPositionChanged = false;
let detachedUserAreaAnimationFrame;
let channelListElement = null;
let userAreaElement = null;
const surfaceCssPxCache = new Map();
...
function readSurfacePx(property, fallback) {
...
function handleHeaderBarPointerMove(event) {
...
function setHeaderBarPointerTrackerEnabled(enabled) {
    if (headerBarPointerTrackerEnabled === enabled)
        return;
...
function setPanelCollapsed(panelId, collapsed) {
    const key = panelRegistry[panelId].collapsedKey;
    if (settings.plain[key] === collapsed)
        return;
...
function nodeWithin(element, node) {
    return node instanceof Node && !!element?.contains(node);
}
function eventWithin(element, event) {
    if (!element)
        return false;
    if (nodeWithin(element, event.target))
        return true;
    if (event.nativeEvent.composedPath().some(node => nodeWithin(element, node)))
        return true;
...
function eventWithinBetterFolders(event) {
...
function handleSidebarLeave(event) {
    ...
    if (containsRelatedTarget(event))
        return;
    if (isPanelCollapsed("channelList"))
        setChannelListExpandedByInteraction(false);
}
function isInteractiveDragTarget(node) {
...
function setChannelListElement(element) {
    channelListElement = element;
}
function setUserAreaElement(element) {
...
function clampUserAreaPosition(x, y, width = DETACHED_USER_AREA_WIDTH, height = DETACHED_USER_AREA_HEIGHT) {
...
function getDetachedUserAreaPosition() {
    if (detachedUserAreaDragPosition)
        return detachedUserAreaDragPosition;
    const storedX = settings.plain.detachedUserAreaX;
    const storedY = settings.plain.detachedUserAreaY;
    const defaultPosition = clampUserAreaPosition(window.innerWidth - DETACHED_USER_AREA_WIDTH - DETACHED_USER_AREA_DEFAULT_OFFSET_X, window.innerHeight - DETACHED_USER_AREA_HEIGHT - DETACHED_USER_AREA_DEFAULT_OFFSET_Y);
    if (!Number.isFinite(storedX) || !Number.isFinite(storedY) || storedX < 0 || storedY < 0)
        return defaultPosition;
    return clampUserAreaPosition(storedX, storedY);
}
function persistDetachedUserAreaPosition(x, y, width, height) {
    ...
    if (settings.plain.detachedUserAreaX === roundedX && settings.plain.detachedUserAreaY === roundedY)
        return;
...
function scheduleDetachedUserAreaUpdate() {
    if (detachedUserAreaAnimationFrame != null)
        return;
...
function handleDetachedUserAreaMouseMove(event) {
    if (!userAreaDragState)
        return;
    const position = clampUserAreaPosition(event.clientX - userAreaDragState.offsetX, event.clientY - userAreaDragState.offsetY, userAreaDragState.width, userAreaDragState.height);
    if (detachedUserAreaDragPosition?.x === position.x && detachedUserAreaDragPosition.y === position.y)
        return;
...
function startDetachedUserAreaDrag(event) {
    if (!shouldDetachUserArea() || event.button !== 0)
        return;
    const path = event.nativeEvent.composedPath();
    if (path.some(node => node !== event.currentTarget && isInteractiveDragTarget(node)))
        return;
...
const ToolbarMenu = ErrorBoundary.wrap(({ onClose }) => {
    const store = settings.use(collapseSettingKeys);
    return (<Menu.Menu navId="vc-collapsible-ui-toolbar-menu" onClose={onClose} aria-label="Collapsible UI">
            {toolbarPanelOrder.map(panelId => {
            const panel = panelRegistry[panelId];
            const collapsed = store[panel.collapsedKey];
            return (<Menu.MenuCheckboxItem key={panelId} id={`vc-collapsible-ui-${panel.classId}`} label={panel.label} checked={!collapsed} action={() => togglePanel(panelId)}/>);
        })}
        </Menu.Menu>);
}, { noop: true });
const ToolbarButtons = ErrorBoundary.wrap(() => {
    ...
    return (<ChannelToolbarButton icon={PanelsIcon} tooltip="Collapsible UI" aria-label="Collapsible UI" selected={anyCollapsed} onClick={openToolbarMenu} onContextMenu={openToolbarMenu}/>);
}, { noop: true });
const CollapsedMenuButton = ErrorBoundary.wrap(() => (<Clickable className={cl("restore-button")} role="button" tabIndex={0} aria-label="Collapsible UI" onClick={openToolbarMenu} onContextMenu={openToolbarMenu}>
        <PanelsIcon width={18} height={18}/>
    </Clickable>), { noop: true });
const ChatButtonsRow = ErrorBoundary.wrap(({ buttons }) => {
    const chatButtonsCollapsed = usePanelCollapsed("chatButtons");
    if (buttons.length === 0)
        return <>{buttons}</>;
    return (<div className={classes(cl("chat-buttons"), chatButtonsCollapsed && cl("chat-buttons-collapsed"))}>
...
export default definePlugin({
    ...
    start() {
        const panelAttr = (classId, collapsed) => ({
        ...
        providerUnsubs = [
            addSurfacePropsProvider("guildBar", () => {
                ...
                const attrs = panelAttr(panelRegistry.guildBar.classId, collapsed);
            ...
            addSurfacePropsProvider("channelList", () => {
                const attrs = panelAttr(panelRegistry.channelList.classId, isPanelCollapsed("channelList"));
            ...
            addSurfacePropsProvider("headerBar", () => {
                ...
                const attrs = panelAttr(panelRegistry.headerBar.classId, collapsed);
                ...
                attrs.onFocusCapture = () => {
                    if (isPanelCollapsed("headerBar"))
                        setHeaderBarExpandedByInteraction(true);
                };
            ...
            addSurfacePropsProvider("userArea", () => {
                ...
                const attrs = panelAttr(panelRegistry.userArea.classId, uaCollapsed);
