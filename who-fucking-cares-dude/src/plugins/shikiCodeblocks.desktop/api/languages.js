...
export const shikiRepoGrammar = (name) => `https://cdn.jsdelivr.net/gh/${SHIKI_REPO}@${SHIKI_REPO_COMMIT}/packages/tm-grammars/grammars/${name}.json`;
export const languages = {};
export const loadLanguages = async () => {
    const langsJson = await fetch(JSON_URL).then(res => res.ok ? res.json() : []);
    const loadedLanguages = Object.fromEntries(langsJson.map(lang => {
