...
function DefaultRoleIcon() {
    return (<svg className="vc-mentionAvatars-icon vc-mentionAvatars-role-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
...
export default definePlugin({
    ...
    settings,
    renderUsername: ErrorBoundary.wrap(({ user, username, showMeYourNameMention }) => {
        ...
        return (<span onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)} className="vc-mentionAvatars-container">
                {user && (<img src={user.getAvatarURL(SelectedGuildStore.getGuildId(), 16, isHovering)} className="vc-mentionAvatars-icon" style={{ borderRadius: "50%" }}/>)}
                {nameContent}
            </span>);
    }, { noop: true }),
    renderInputMention(username, user) {
        if (!user)
            return getUsernameString(username);
        return (<>
                <img src={user.getAvatarURL(SelectedGuildStore.getGuildId(), 16)} className="vc-mentionAvatars-icon" style={{ borderRadius: "50%" }}/>
    ...
    renderRoleIcon: ErrorBoundary.wrap(({ roleId, guildId }) => {
        ...
        return (<img className="vc-mentionAvatars-icon vc-mentionAvatars-role-icon" src={`${location.protocol}//${window.GLOBAL_ENV.CDN_HOST}/role-icons/${roleId}/${role.icon}.webp?size=24&quality=lossless`}/>);
    }, { noop: true }),
});
