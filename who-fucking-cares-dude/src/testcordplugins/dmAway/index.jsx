...
async function onMessage({ message, channelId }) {
    if (!afkEnabled || !message?.author?.id)
        return;
    ...
    if (lastReplied && now - lastReplied < settings.store.cooldownMinutes * 60 * 1000)
        return;
...
function MoonIcon({ active }) {
    return (<svg width="20" height="20" viewBox="0 0 24 24" fill={active ? "var(--brand-500)" : "currentColor"}>
...
function AfkButton() {
    ...
    return (<HeaderBarButton icon={() => <MoonIcon active={enabled}/>} tooltip={enabled ? "Away: ON (click to disable)" : "Away: OFF (click to enable)"} onClick={toggle}/>);
}
