...
function onVoiceStateUpdate({ voiceStates }) {
...
function AntiMoveDecoIcon({ className, enabled }) {
    const lineLength = 30;
    const lineStyle = {
...
function AntiMoveDecoButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton onClick={toggle} role="switch" redGlow={!enabled} aria-checked={enabled} plated={nameplate != null} className="button__201d5 wrapper__201d5" tooltipText={hideTooltips ? void 0 : enabled ? "Disable AntiMove&Deco" : "Enable AntiMove&Deco"} icon={<AntiMoveDecoIcon enabled={enabled} className={iconForeground}/>}/>);
}
