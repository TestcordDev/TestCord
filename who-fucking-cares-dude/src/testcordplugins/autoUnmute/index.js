...
async function unmuteUserViaAPI(userId, guildId) {
    try {
        console.log(`[AutoUnmute] Attempting to unmute via API for user ${userId} in server ${guildId}`);
        await RestAPI.patch({
...
async function undeafenUserViaAPI(userId, guildId) {
    try {
        console.log(`[AutoUnmute] Attempting to undeafen via API for user ${userId} in server ${guildId}`);
        await RestAPI.patch({
            url: Constants.Endpoints.GUILD_MEMBER(guildId, userId),
            body: {
                deaf: false,
...
export default definePlugin({
    name: "AutoUnmute",
    description: "Automatically unmutes and undeafens when server mute/deafen occurs if you have permissions (no notifications)",
    tags: ["Voice", "Utility"],
    authors: [
        {
            name: "Bash",
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            for (const state of voiceStates) {
                ...
                if (mute && !selfMute) {
                    console.log(`[AutoUnmute] Server mute detected for user ${currentUserId} in channel ${channelId}`);
                    ...
                    if (hasMutePermission) {
                        console.log("[AutoUnmute] MUTE_MEMBERS permission detected, automatic unmute via API in progress...");
                        // Automatically unmute via Discord API
                        setTimeout(async () => {
                            try {
                                // Use Discord API to unmute via server
                                await unmuteUserViaAPI(currentUserId, guildId);
                                console.log("[AutoUnmute] Automatic unmute via API completed successfully");
                            }
                            catch (error) {
                                console.error("[AutoUnmute] Error during automatic unmute via API:", error);
                                // Fallback: try with toggleSelfMute if API fails
                                try {
                                    console.log("[AutoUnmute] Attempting fallback with toggleSelfMute...");
                                    VoiceActions.toggleSelfMute();
                                    console.log("[AutoUnmute] Automatic unmute via fallback completed successfully");
                                }
                                catch (fallbackError) {
                                    console.error("[AutoUnmute] Error during fallback:", fallbackError);
                                }
                            }
                        }, 100); // Small delay to avoid conflicts
                    }
                    else {
                        console.log("[AutoUnmute] No MUTE_MEMBERS permission, no automatic unmute");
                    }
                }
                // Check if we were deafened by the server (not by ourselves)
                if (deaf && !selfDeaf) {
                    console.log(`[AutoUnmute] Server deafen detected for user ${currentUserId} in channel ${channelId}`);
                    ...
                    if (hasDeafenPermission) {
                        console.log("[AutoUnmute] DEAFEN_MEMBERS permission detected, automatic undeafen via API in progress...");
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                console.log("[AutoUnmute] Automatic undeafen via API completed successfully");
                            }
                            catch (error) {
                                console.error("[AutoUnmute] Error during automatic undeafen via API:", error);
                                // Fallback: try with toggleSelfDeaf if API fails
                                try {
                                    console.log("[AutoUnmute] Attempting fallback with toggleSelfDeaf...");
                                    VoiceActions.toggleSelfDeaf();
                                    console.log("[AutoUnmute] Automatic undeafen via fallback completed successfully");
                                }
                                catch (fallbackError) {
                                    console.error("[AutoUnmute] Error during fallback:", fallbackError);
                                }
                        ...
                        console.log("[AutoUnmute] No DEAFEN_MEMBERS permission, no automatic undeafen");
                    }
                }
            }
        },
