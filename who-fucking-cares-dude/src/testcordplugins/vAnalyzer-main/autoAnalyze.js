...
const AUTO_ANALYZED_MESSAGE_IDS = new Map();
...
let lastPrune = 0;
function pruneAnalyzedMessages(now) {
...
function runScan(messageId, url, task, label) {
...
function addUrlsFromText(urls, text) {
...
function collectEmbedUrls(embed, urls) {
...
function collectComponentUrls(component, urls) {
...
function walkMessageValue(value, urls, seen) {
    ...
    for (const [key, nestedValue] of Object.entries(value)) {
        // skip CDN URLs
        if (/^(image|thumbnail|cdn|media)/.test(key))
            continue;
...
export function extractUrlsFromMessage(message) {
    ...
    if (settings.store.checkEmbeds) {
        for (const embed of message.embeds ?? []) {
            ...
            walkMessageValue(embed, urls, new WeakSet());
        }
    }
    for (const component of message.components ?? []) {
        collectComponentUrls(component, urls);
        walkMessageValue(component, urls, new WeakSet());
    }
...
export function extractUrlsFromEmbeds(message) {
    if (!settings.store.checkEmbeds)
        return [];
    const urls = new Set();
    for (const embed of message.embeds ?? []) {
...
function analyzeUrlsAndInvites(message, urls, options) {
                ...
                void (async () => {
                    try {
                        ...
                        if (s.autoScanUrlsSucuri) {
                            ...
                            if (sucuriResult.domainResolved === false)
                                return;
                        ...
                        if (s.autoScanUrlsCertPL)
                            runScan(message.id, url, () => analyzeWithCertPL(url, true), "URL scan error");
                        if (s.autoScanUrlsFishFish)
                            runScan(message.id, url, () => analyzeWithFishFish(url, true), "URL scan error");
                        if (s.autoScanUrlsWhereGoes)
                            runScan(message.id, url, () => analyzeWithWhereGoes(url, true), "URL scan error");
                        if (s.autoScanUrlsHybridAnalysis)
                            runScan(message.id, url, () => analyzeUrlWithHybridAnalysis(url, true), "URL scan error");
                    }
                    catch (error) {
...
export function manualAnalyzeMessageUrls(message) {
    if (!message?.id)
        return;
...
export function manualAnalyzeUrls(message, urls) {
    if (!message?.id || urls.length === 0)
        return;
...
function isEphemeralMessage(message) {
    try {
        if (typeof message.hasFlag === "function" && message.hasFlag(EPHEMERAL_MESSAGE_FLAG)) {
            return true;
        }
    }
    catch {
...
function isScannableAttachment(attachment) {
    if (!settings.store.ignoreMediaFiles)
        return true;
...
function matchesFilter(module, target) {
    const { filter } = module;
...
function runAutoModularScans(messageId, urls, attachments) {
    ...
    for (const module of modules) {
        ...
        if (module.type === "file" && attachments?.length) {
            for (const attachment of attachments) {
                if (!matchesFilter(module, attachment.filename))
                    continue;
...
export function autoAnalyzeMessage(message) {
    if (!message?.id)
        return;
    const now = Date.now();
    ...
    if (s.skipFriends && RelationshipStore.isFriend(message.author.id))
        return;
    ...
    if (urls.length === 0 && (!canScanFiles || scannableAttachments.length === 0))
        return;
    const normalUrls = analyzeUrlsAndInvites(message, urls, {
