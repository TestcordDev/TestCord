...
let DecorationGridItem = null;
let DecorationGridDecoration = null;
let AvatarDecorationModalPreview = null;
...
function buildOverrides(target) {
    ...
    if (target.clan !== undefined)
        overrides.clan = target.clan;
    ...
    if (target.createdAt !== undefined)
        overrides.createdAt = target.createdAt;
...
function wrapUser(base) {
    ...
    if (!base || !target || !isActive())
        return base;
    if (wrapping)
        return base;
    if (cachedWrap && cachedWrap.base === base && cachedWrap.target === target)
        return cachedWrap.wrap;
    wrapping = true;
    try {
...
let originalUseName = null;
let originalUseUserTag = null;
let originalExtractTimestamp = null;
...
function patchUtils() {
    ...
    UsernameUtils.useName = function (user) {
        if (isActive() && user && isCurrentUser(user.id)) {
            const t = getTargetUser();
            if (t)
                return originalUseName.call(this, t);
        }
        return originalUseName.call(this, user);
    };
    UsernameUtils.useUserTag = function (user, options) {
        if (isActive() && user && isCurrentUser(user.id)) {
            const t = getTargetUser();
            if (t)
                return originalUseUserTag.call(this, t, options);
        }
        return originalUseUserTag.call(this, user, options);
    };
    originalExtractTimestamp = SnowflakeUtils.extractTimestamp;
    SnowflakeUtils.extractTimestamp = function (id) {
...
function unpatchUtils() {
    ...
    if (originalUseName)
        UsernameUtils.useName = originalUseName;
    if (originalUseUserTag)
        UsernameUtils.useUserTag = originalUseUserTag;
    if (originalExtractTimestamp)
...
function FakeUserProfileIcon({ className, style }) {
    const lineLength = 30;
    const lineStyle = {
    ...
    return (<svg className={className} style={style} width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
...
function FakeUserProfileButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton tooltipText={tooltip} icon={<FakeUserProfileIcon className={iconForeground}/>} role="button" plated={nameplate != null} redGlow={!active} onClick={() => openModal(modalProps => <FakeUserProfileModal modalProps={modalProps}/>)} onContextMenu={() => {
...
let cachedBadgesTarget = null;
let cachedBadgesManual = null;
...
export default definePlugin({
    ...
    userAreaButton: {
        ...
        render: (props) => <FakeUserProfileButton {...props}/>,
    },
    async start() {
        await loadData();
    ...
    set AvatarDecorationModalPreview(e) {
    ...
    profileHook(userId, original) {
        ...
        const manual = getCachedTarget()?.manualProfile;
        const overrides = {};
        if (targetProfile.bio != null)
            overrides.bio = targetProfile.bio;
        if (targetProfile.pronouns != null)
            overrides.pronouns = targetProfile.pronouns;
        if (targetProfile.themeColors)
        ...
        overrides.banner = targetProfile.banner ?? target.banner ?? null;
        overrides.accentColor = targetProfile.accentColor ?? target.accentColor ?? null;
        if (settings.store.spoofProfileEffect && targetProfile.profileEffect)
            overrides.profileEffect = targetProfile.profileEffect;
        if (settings.store.spoofProfileEffect && targetProfile.popoutAnimationParticleType != null)
            overrides.popoutAnimationParticleType = targetProfile.popoutAnimationParticleType;
        if (settings.store.spoofProfileEffect && targetProfile.profileEffectExpiresAt != null)
            overrides.profileEffectExpiresAt = targetProfile.profileEffectExpiresAt;
        if (targetProfile.premiumType != null)
            overrides.premiumType = targetProfile.premiumType;
        if (targetProfile.premiumSince != null)
            overrides.premiumSince = targetProfile.premiumSince;
        if (targetProfile.premiumGuildSince != null)
        ...
        const targetUserProfile = targetProfile.userProfile ?? {};
        ...
        if (settings.store.spoofActivities) {
            if (targetProfile.connectedAccounts)
                overrides.connectedAccounts = targetProfile.connectedAccounts;
            if (targetProfile.legacyApplications)
                overrides.legacyApplications = targetProfile.legacyApplications;
            if (targetProfile.applicationRoleConnections)
                overrides.applicationRoleConnections = targetProfile.applicationRoleConnections;
