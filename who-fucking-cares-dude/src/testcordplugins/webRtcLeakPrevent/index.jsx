...
function createPatchedConnection(OriginalConnection, icePolicy) {
    return class extends OriginalConnection {
        constructor(configuration) {
            ...
            if (settings.store.enableLogs) {
                ...
                logger.info(`RTCPeerConnection created - ICE policy: ${actualConfig.iceTransportPolicy}`, `ICE servers: ${actualConfig.iceServers?.length ?? 0}`, `Bundle policy: ${actualConfig.bundlePolicy}`);
            }
        }
        setConfiguration(configuration) {
            ...
            if (settings.store.enableLogs && configuration.iceTransportPolicy !== icePolicy) {
                logger.warn("Discord attempted to change ICE policy to:", configuration.iceTransportPolicy, "- forcing back to:", icePolicy);
            }
...
export default definePlugin({
    ...
    start() {
        if (settings.store.icePolicy === "all") {
            if (settings.store.enableLogs)
                logger.info("Plugin disabled - allowing all ICE candidates");
            return;
        }
        ...
        for (const { name, ctor } of connections) {
            ...
            if (name === "webkitRTCPeerConnection") {
                window.webkitRTCPeerConnection = PatchedConnection;
            }
            else {
    ...
    stop() {
        ...
        for (const { name, ctor } of originalConnections) {
            if (name === "webkitRTCPeerConnection") {
                window.webkitRTCPeerConnection = ctor;
            }
            else {
