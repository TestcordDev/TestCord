...
export async function groqFetch(url, method, headers, body) {
...
let _reidverseKeyPromise = null;
export async function getReidverseKey() {
    const key = await DataStore.get(DS_REIDVERSE_KEY);
    if (key?.trim())
        return key.trim();
    return registerReidverse();
}
async function registerReidverse() {
    if (_reidverseKeyPromise)
        return _reidverseKeyPromise;
...
async function _doRegister() {
    ...
    if (!res.ok)
        throw new Error(`Reidverse register failed: ${res.status}`);
    const data = await res.json();
    const key = data?.key;
    if (typeof key !== "string" || !key.trim())
        throw new Error("Reidverse register returned no key");
    await DataStore.set(DS_REIDVERSE_KEY, key.trim());
    return key.trim();
}
// ── Legacy Groq key (kept for voiceDictation Whisper) ─────────────────────────
let _settingsFallback = null;
export function registerSettingsFallback(fn) {
    _settingsFallback = fn;
}
export async function getGroqKey() {
    const key = await DataStore.get(DS_GROQ_KEY);
    if (key?.trim())
        return key.trim();
...
function enqueue(fn) {
    ...
    queue = result.then(() => sleep(MIN_DELAY_MS), () => sleep(MIN_DELAY_MS));
    return result;
}
export async function reidverseChat(opts) {
    return enqueue(() => _reidverseChat(opts));
}
async function _reidverseChat(opts, attempt = 0) {
...
export async function groqChat(opts) {
    ...
    if (!key)
        throw new Error("Missing Groq API key. Add one in TestcordAI settings.");
