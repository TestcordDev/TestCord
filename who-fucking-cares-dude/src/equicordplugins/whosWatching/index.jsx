    ...
    return (<div className={cl("content")}>
            ...
            (<div className={cl("spectating")}>
                        ...
                        <Flex flexDirection="column" gap="6">
                            {users.map(user => (<Flex key={user.id} flexDirection="row" gap="6" alignContent="center">
...
export default definePlugin({
    ...
    WrapperComponent: ErrorBoundary.wrap(props => {
        ...
        function renderMoreUsers(_label, count) {
            ...
            return (<Tooltip text={<Watching userIds={userIds} guildId={guildId}/>}>
        ...
        return (<div className={cl("screenshare-panel")}>
                ...
                <div className={cl("spectating-users")}>
                            <UserSummaryItem users={users} count={userIds.length} renderIcon={false} max={12} showDefaultAvatarsForNullUsers renderMoreUsers={renderMoreUsers} renderUser={(user, index) => (<Clickable key={index} className={AvatarStyles.clickableAvatar} onClick={() => openUserProfile(user.id)}>
                                        <img className={AvatarStyles.avatar} src={getUserAvatarUrl(user, guildId, true, 80)} alt={user.username} title={user.username}/>
                                    </Clickable>)}/>
                        </div>
                : <Paragraph>
                            No spectators
