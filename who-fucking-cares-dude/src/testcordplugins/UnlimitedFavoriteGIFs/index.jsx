...
function log(...args) { console.log("[UnlimitedFavoriteGIFs]", ...args); }
function warn(...args) { console.warn("[UnlimitedFavoriteGIFs]", ...args); }
async function getLocalFavs() {
    return (await DataStore.get(LOCAL_FAVS_KEY)) ?? [];
}
async function saveLocalFavs(list) {
...
const settings = definePluginSettings({
    ...
    clearLocalFavs: {
        ...
        component: () => {
            const { Button } = require("@webpack/common");
            return (<Button color={Button.Colors.RED} onClick={async () => {
                    await saveLocalFavs([]);
