...
const profileSectionListeners = new Set();
/**
 ...
 * ));
 */
export function addProfileSection(id, render, priority = 0) {
 ...
 */
export function removeProfileSection(id) {
    profileSections.delete(id);
    profileSectionListeners.forEach(listener => listener());
}
function ProfileSections({ userId, isSideBar }) {
    ...
    return Array.from(profileSections)
        ...
        .map(([id, { render: Section }]) => (<ErrorBoundary noop key={id} onError={e => logger.error(`Failed to render profile section: ${id}`, e.error)}>
                <Section userId={userId} isSideBar={isSideBar}/>
            </ErrorBoundary>));
}
/** @internal Injected by ProfileSectionsAPI patch (do NOT call directly) */
export function renderProfileSections(props) {
