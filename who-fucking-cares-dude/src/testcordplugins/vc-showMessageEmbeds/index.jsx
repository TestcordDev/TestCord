...
const addShowEmbedButton = (children, props) => {
    if (props.itemSrc || !props.itemHref || !props.message)
        return; // itemSrc means the right clicked item is an image/attachment
...
const addButton = (children, message, url) => {
    ...
    if (!isEmbedInMessage(message, url)) {
        children.splice(0, 0, <Menu.MenuItem id="vc-sme-show" label="Show Embed" action={_ => unfurlEmbed(url, message)} icon={ImageVisible} key="vc-sme-show"/>);
    }
        ...
        children.splice(0, 0, <Menu.MenuItem id="vc-sme-remove" label="Remove Embed" action={_ => removeEmbed(url, message)} icon={ImageInvisible} key="vc-sme-remove"/>);
    }
};
function isEmbedInMessage(message, url) {
...
function isUrlInMessage(message, url) {
    ...
    for (const embed of message.embeds) {
        ...
        if (embed.url !== url && (embed.rawDescription?.includes(url) || embed.fields.some((field) => field.value.includes(url)))) {
            return true;
        }
    }
    return false;
    ...
    if (youtubeRegex.test(url)) {
        ...
        if (params.has("t") || params.has("start")) {
            ...
            if (startParam && startParam.match(/^(?:(\d+h)?(\d+m)?(\d+s)?(\d+)?)?$/)) {
                ...
                if (hours)
                    start += parseInt(hours[1]) * 3600;
                if (minutes)
                    start += parseInt(minutes[1]) * 60;
                if (seconds)
                    start += parseInt(seconds[1]);
                if (unqualifiedSeconds)
                    start += parseInt(unqualifiedSeconds[1]);
            }
        }
...
async function unfurlEmbed(url, message) {
...
function removeEmbed(url, message) {
    const newEmbeds = message.embeds.filter((embed) => embed.url !== url);
    updateMessage(message.channel_id, message.id, { embeds: newEmbeds });
}
