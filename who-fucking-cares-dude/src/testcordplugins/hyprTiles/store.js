...
const { PersistedStore } = FluxWP;
const WORKSPACE_IDS = [1, 2, 3, 4, 5, 6, 7, 8, 9];
const MANUAL_LAYOUTS = ["dwindle", "master", "grid", "columns"];
const STATE_VERSION = 2;
const DEFAULT_MASTER_RATIO = 0.6;
const DEFAULT_FLOAT_BOUNDS = { x: 0.16, y: 0.1, w: 0.68, h: 0.78 };
const DEFAULT_SCRATCHPAD_BOUNDS = { x: 0.14, y: 0.08, w: 0.72, h: 0.82 };
...
const isSplit = (node) => node?.kind === "split";
const cloneBounds = (bounds) => ({ ...bounds });
const clampMasterRatio = (value) => Math.max(0.3, Math.min(0.75, value));
const clampSplitRatio = (value) => Math.max(MIN_SPLIT_RATIO, Math.min(1 - MIN_SPLIT_RATIO, value));
const defaultLayout = () => {
    const layout = settings.store.defaultLayout;
    return MANUAL_LAYOUTS.includes(layout) ? layout : "dwindle";
};
const createId = (prefix) => `hypr-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
const getLayoutFromSettingsKey = (value) => {
    const [layout] = value.split(":");
    return MANUAL_LAYOUTS.includes(layout) ? layout : null;
};
function normalizeWorkspaceId(value) {
    const workspaceId = Number(value);
    return WORKSPACE_IDS.includes(workspaceId) ? workspaceId : null;
}
function normalizeOpenedBy(value) {
    switch (value) {
        case "user":
        case "rule":
        case "dragDrop":
...
function normalizeRestoreState(value) {
    ...
    return {
        ...
        tabIndex: Number.isInteger(value.tabIndex) ? value.tabIndex : 0,
        tabGroupName: typeof value.tabGroupName === "string" ? value.tabGroupName : null,
    };
}
function normalizeBounds(value, fallback) {
    if (!value || typeof value !== "object")
        return cloneBounds(fallback);
...
function getWorkspace(id = state.activeWorkspace) {
...
function createTile(target, workspaceId, openedBy) {
    return {
        id: createId("tile"),
...
function createLeaf(tileId, tabGroupName = null) {
    return {
        id: createId("leaf"),
        kind: "leaf",
        tileIds: [tileId],
...
function setNode(workspace, node) {
    workspace.nodesById[node.id] = node;
    return node;
}
function findParentInfo(workspace, targetId, nodeId = workspace.rootNodeId) {
    if (!nodeId)
        return { parent: null, side: null };
    const node = workspace.nodesById[nodeId];
    if (!isSplit(node))
        return { parent: null, side: null };
    if (node.firstId === targetId)
        return { parent: node, side: "firstId" };
    if (node.secondId === targetId)
        return { parent: node, side: "secondId" };
...
function replaceNodeReference(workspace, oldId, nextId) {
...
function buildRowsTree(workspace, nodeIds) {
    if (!nodeIds.length)
        return null;
    if (nodeIds.length === 1)
        return nodeIds[0];
...
function buildDwindleTree(workspace, nodeIds, axis = "x") {
    if (!nodeIds.length)
        return null;
    if (nodeIds.length === 1)
        return nodeIds[0];
...
function buildGridTree(workspace, nodeIds) {
    if (!nodeIds.length)
        return null;
    if (nodeIds.length === 1)
        return nodeIds[0];
...
function buildMasterTree(workspace, nodeIds) {
    if (!nodeIds.length)
        return null;
    if (nodeIds.length === 1)
        return nodeIds[0];
...
function rebuildWorkspaceTree(workspace, layout = workspace.layout) {
...
function getWorkspaceScratchpadTileIds(workspaceId, currentState = state) {
...
function getWorkspaceFocusableTileIds(workspace, currentState = state) {
...
function normalizeLeaf(leaf, currentState = state) {
...
function normalizeWorkspaceRefs(workspace, currentState = state) {
    for (const leafId of getLeafIds(workspace)) {
        const leaf = workspace.nodesById[leafId];
        if (!isLeaf(leaf))
            continue;
        normalizeLeaf(leaf, currentState);
        if (!leaf.tileIds.length)
            removeNode(workspace, leaf.id);
    }
    workspace.layout = workspace.layout === "single" || MANUAL_LAYOUTS.includes(workspace.layout)
        ? workspace.layout
        : defaultLayout();
...
function syncWorkspaceLayoutsToSettings(currentState = state) {
    let changed = false;
    ...
    for (const workspaceId of WORKSPACE_IDS) {
        ...
        if (previousFallbackLayout && workspace.layout === previousFallbackLayout && workspace.layout !== fallbackLayout) {
            ...
            if (tiledLeafCount)
                rebuildWorkspaceTree(workspace, fallbackLayout);
            changed = true;
            continue;
        }
...
function insertNodeRelative(workspace, targetNodeId, newNodeId, direction) {
...
function appendLeaf(workspace, leaf, direction = "right") {
...
function detachLeafFromTree(workspace, leafId) {
    ...
    const direction = parent.axis === "x"
        ? side === "firstId" ? "left" : "right"
        : side === "firstId" ? "up" : "down";
...
function detachTileFromWorkspace(workspace, tileId) {
...
function removeTileFromOverlayLists(tileId) {
    for (const workspaceId of WORKSPACE_IDS) {
...
function placeTileInLeaf(workspace, tileId, leafId, tabIndex) {
    const leaf = workspace.nodesById[leafId];
    if (!isLeaf(leaf) || leaf.tileIds.includes(tileId))
        return false;
...
function restoreTileToWorkspace(tileId) {
    const tile = state.tilesById[tileId];
    if (!tile?.restoreState)
        return false;
...
function focusLeafTile(workspace, tileId) {
    const leaf = findLeafByTileId(workspace, tileId);
    if (leaf)
        leaf.activeTileId = tileId;
}
function getNamedGroupLeaf(workspace, groupName) {
    for (const leafId of getLeafIds(workspace)) {
        const leaf = workspace.nodesById[leafId];
        if (isLeaf(leaf) && leaf.tabGroupName === groupName)
            return leaf;
    }
    return null;
}
function applyAutoLayout(workspace, layoutHint) {
    if (!workspace.autoLayout)
        return;
...
function addTileToNamedGroup(workspace, tileId, groupName, fallbackDirection = null) {
...
function findExistingTileByTarget(target, workspaceId) {
    for (const tile of Object.values(state.tilesById)) {
        if (!sameTarget(tile, target))
            continue;
        if (tile.workspaceId === workspaceId || tile.pinned || tile.scratchpadId)
            return tile.id;
    }
    return null;
}
function sanitizeLegacyWorkspace(workspaceId, source, next) {
    ...
    const tileIds = Array.isArray(source?.tileIds)
        ? source.tileIds.filter((tileId) => typeof tileId === "string" && !!next.tilesById[tileId])
        : [];
...
function sanitizeWorkspace(workspaceId, source, next) {
    ...
    const sanitizeNode = (nodeId) => {
        ...
        if (!node || typeof node !== "object")
            return null;
        if (node.kind === "leaf") {
            const tileIds = Array.isArray(node.tileIds)
                ? [...new Set(node.tileIds.filter((tileId) => typeof tileId === "string" && !!next.tilesById[tileId]))]
                : [];
            if (!tileIds.length)
                return null;
            for (const tileId of tileIds)
    ...
    workspace.floatingTileIds = Array.isArray(source.floatingTileIds)
        ? [...new Set(source.floatingTileIds.filter((tileId) => typeof tileId === "string" && !!next.tilesById[tileId]))]
        : [];
...
const sanitizeState = (previous) => {
    const next = createDefaultState();
    const rawTiles = previous?.tilesById ?? {};
    for (const [id, rawTile] of Object.entries(rawTiles)) {
        if (!rawTile || typeof rawTile !== "object")
            continue;
        if (typeof rawTile.channelId !== "string" || !rawTile.channelId)
            continue;
        next.tilesById[id] = {
            id,
            channelId: rawTile.channelId,
            guildId: typeof rawTile.guildId === "string" ? rawTile.guildId : null,
            createdAt: typeof rawTile.createdAt === "number" ? rawTile.createdAt : Date.now(),
            workspaceId: normalizeWorkspaceId(rawTile.workspaceId) ?? 1,
            openedBy: normalizeOpenedBy(rawTile.openedBy) ?? "restore",
            floating: !!rawTile.floating,
            pinned: !!rawTile.pinned,
            scratchpadId: typeof rawTile.scratchpadId === "string" ? rawTile.scratchpadId : null,
            floatBounds: normalizeBounds(rawTile.floatBounds, DEFAULT_FLOAT_BOUNDS),
            restoreState: normalizeRestoreState(rawTile.restoreState),
        };
    }
    for (const workspaceId of WORKSPACE_IDS) {
    ...
    next.pinnedTileIds = Array.isArray(previous?.pinnedTileIds)
        ? [...new Set(previous.pinnedTileIds.filter((tileId) => typeof tileId === "string" && !!next.tilesById[tileId]))].slice(0, PINNED_TILE_MAX)
        : [];
    ...
    if (previous?.scratchpadsById && typeof previous.scratchpadsById === "object") {
        for (const [scratchpadId, rawScratchpad] of Object.entries(previous.scratchpadsById)) {
            if (!rawScratchpad || typeof rawScratchpad !== "object")
                continue;
            const scratchpadTileId = rawScratchpad.tileId;
            ...
            next.scratchpadsById[scratchpadId] = {
                id: scratchpadId,
                tileId,
                workspaceId: normalizeWorkspaceId(rawScratchpad.workspaceId),
                visible: !!rawScratchpad.visible,
                bounds: normalizeBounds(rawScratchpad.bounds, DEFAULT_SCRATCHPAD_BOUNDS)
            };
            if (tileId) {
    ...
    for (const scratchpad of Object.values(next.scratchpadsById)) {
        if (scratchpad.tileId)
            referencedTiles.add(scratchpad.tileId);
    }
    for (const tileId of Object.keys(next.tilesById)) {
        if (!referencedTiles.has(tileId))
            delete next.tilesById[tileId];
    }
...
export function resyncWorkspaceLayoutsToSettings(force = false) {
    ...
    if (!force && settingsKey === appliedLayoutSettingsKey)
        return false;
...
export function getPinnedTiles() {
    return state.pinnedTileIds.map(tileId => state.tilesById[tileId]).filter(Boolean);
}
export function getVisibleScratchpads(workspaceId = state.activeWorkspace) {
...
function focusTileInternal(workspace, tileId, promotePrimary = true) {
    const focusable = getWorkspaceFocusableTileIds(workspace);
    if (!focusable.includes(tileId))
        return false;
    ...
    if (promotePrimary)
        workspace.primaryTileId = tileId;
    return true;
}
export function focusTile(tileId, promotePrimary = true) {
    const workspace = getWorkspace();
    if (!focusTileInternal(workspace, tileId, promotePrimary))
        return false;
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return true;
...
export function focusPreviousTile() {
    const workspace = getWorkspace();
    if (!workspace.lastFocusedTileId)
        return null;
    if (!focusTileInternal(workspace, workspace.lastFocusedTileId, false))
        return null;
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return workspace.focusedTileId;
}
export function promoteTileToPrimary(tileId) {
    return focusTile(tileId, true);
}
export function openTile(target, options) {
    ...
    if (workspace.autoLayout)
        applyAutoLayout(workspace, plan.layoutHint);
    if (plan.focus) {
        if (state.activeWorkspace !== workspaceId)
            state.activeWorkspace = workspaceId;
...
export function ensurePrimaryForRoute(target) {
    const workspace = getWorkspace();
...
export function updatePrimaryTileTarget(target) {
    return ensurePrimaryForRoute(target);
}
export function updateTileTarget(tileId, target) {
    const tile = state.tilesById[tileId];
    if (!tile || sameTarget(tile, target))
        return false;
...
export function cycleLayout() {
    ...
    const index = MANUAL_LAYOUTS.indexOf(current);
...
export function updateSplitRatio(splitId, ratio) {
    const workspace = getWorkspace();
...
export function adjustActiveWorkspaceMasterRatio(delta) {
    const workspace = getWorkspace();
    const next = clampMasterRatio(workspace.masterRatio + delta);
    if (next === workspace.masterRatio)
        return false;
    workspace.masterRatio = next;
    if (workspace.layout === "master")
        rebuildWorkspaceTree(workspace, "master");
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return true;
}
...
export function swapTilesByIds(fromId, toId) {
    const workspace = getWorkspace();
    ...
    if (fromLeaf.activeTileId === fromId)
        fromLeaf.activeTileId = toId;
    if (toLeaf.activeTileId === toId)
        toLeaf.activeTileId = fromId;
    if (workspace.focusedTileId === fromId)
        workspace.focusedTileId = toId;
    else if (workspace.focusedTileId === toId)
        workspace.focusedTileId = fromId;
    if (workspace.primaryTileId === fromId)
        workspace.primaryTileId = toId;
    else if (workspace.primaryTileId === toId)
        workspace.primaryTileId = fromId;
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return true;
}
...
export function moveFocusDirection(direction, wrap = false, promotePrimary = true) {
    const workspace = getWorkspace();
    if (!workspace.focusedTileId)
        return null;
...
export function swapDirection(direction, wrap = false) {
    const workspace = getWorkspace();
    if (!workspace.focusedTileId)
        return null;
...
function getVisibleTileCount(workspace) {
    return getWorkspaceFocusableTileIds(workspace).length;
}
function removeTileEverywhere(tileId) {
    const tile = state.tilesById[tileId];
    if (!tile)
        return false;
    for (const workspaceId of WORKSPACE_IDS) {
...
export function closeTileById(tileId) {
    const workspace = getWorkspace();
    if (workspace.focusedTileId === tileId && getVisibleTileCount(workspace) <= 1)
        return false;
    if (!removeTileEverywhere(tileId))
        return false;
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return true;
...
export function moveFocusedTileToWorkspace(destinationId) {
    const source = getWorkspace();
    const focusedId = source.focusedTileId;
    if (!focusedId || destinationId === source.id)
        return null;
    const tile = state.tilesById[focusedId];
    if (!tile || tile.pinned || tile.scratchpadId)
        return null;
    ...
    if (tile.floating)
        destination.floatingTileIds.push(focusedId);
    else
        appendLeaf(destination, createLeaf(focusedId));
    normalizeWorkspaceRefs(source);
    normalizeWorkspaceRefs(destination);
    emitChange();
    return focusedId;
}
export function getPrimaryTileForWorkspace(id = state.activeWorkspace) {
    const workspace = getWorkspace(id);
    return workspace.primaryTileId ? state.tilesById[workspace.primaryTileId] ?? null : null;
}
export function getWorkspaceSnapshot(id = state.activeWorkspace) {
    return getWorkspace(id);
}
export function getActiveWorkspaceIndex() {
    return state.activeWorkspace;
}
export function seedWorkspaceFromTarget(workspaceId, target) {
    const workspace = getWorkspace(workspaceId);
    if (workspace.rootNodeId || workspace.floatingTileIds.length)
        return workspace.primaryTileId;
...
export function cycleFocusedLeafTab(step) {
    const workspace = getWorkspace();
    if (!workspace.focusedTileId)
        return null;
    const leaf = findLeafByTileId(workspace, workspace.focusedTileId);
    if (!leaf || leaf.tileIds.length <= 1)
        return null;
...
export function moveFocusedTileIntoNewGroup() {
    ...
    if (restore?.anchorNodeId && restore.direction)
        insertNodeRelative(workspace, restore.anchorNodeId, newLeaf.id, restore.direction);
    else
        appendLeaf(workspace, newLeaf);
    focusTileInternal(workspace, tileId, true);
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return groupName;
}
export function sendFocusedTileToNamedGroup(groupName) {
    const workspace = getWorkspace();
    const tileId = workspace.focusedTileId;
    if (!tileId || !groupName)
        return null;
...
export function updateTileFloatBounds(tileId, bounds) {
    const tile = state.tilesById[tileId];
    if (!tile)
        return false;
    ...
    if (tile.scratchpadId) {
        ...
        if (scratchpad)
            scratchpad.bounds = tile.floatBounds;
    }
    emitChange();
    return true;
}
function toggleFloating(tileId) {
    const workspace = getWorkspace();
    const tile = state.tilesById[tileId];
    if (!tile || tile.pinned || tile.scratchpadId)
        return false;
...
export function toggleScratchpad(id) {
    if (!id)
        return null;
    ...
    if (!scratchpad.tileId) {
        ...
        if (!tile || tile.pinned)
            return null;
        if (tile.floating)
            workspace.floatingTileIds = workspace.floatingTileIds.filter(entry => entry !== tileId);
        else
            tile.restoreState = detachTileFromWorkspace(workspace, tileId);
        tile.floating = true;
    ...
    if (scratchpad.visible)
        focusTileInternal(workspace, tile.id, true);
    else if (workspace.focusedTileId === tile.id)
        focusPreviousTile();
    normalizeWorkspaceRefs(workspace);
    emitChange();
    return tile.id;
}
export function moveTileByDrop(tileId, targetTileId, zone, groupOnCenter = false) {
...
export function getWorkspaceLayoutRects(id = state.activeWorkspace) {
