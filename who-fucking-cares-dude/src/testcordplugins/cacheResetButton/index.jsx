...
import { ActiveJoinedThreadsStore, ApplicationCommandIndexStore, ApplicationStore, ApplicationStreamingStore, ApplicationStreamPreviewStore, DraftStore, EditMessageStore, EmojiStore, ExperimentStore, InviteStore, MessageCache, MessageStore, NotificationSettingsStore, PendingReplyStore, PresenceStore, QuestStore, RunningGameStore, showToast, SoundboardStore, SpellCheckStore, SpotifyStore, StickersStore, Toasts, TypingStore, UploadAttachmentStore, UserAffinitiesStore, UserGuildSettingsStore, UserProfileStore, } from "@webpack/common";
let boostStyleEl = null;
let springs = [];
...
function clearStoreCache(name, store, cleared) {
    try {
        if (store.clearCache) {
            store.clearCache();
            cleared.push(name);
...
function clearMapCache(name, store, mapKey, cleared) {
    try {
...
function applyBoost() {
    ...
    if (springs.length > 0)
        applied.push("Spring animations");
    ...
    if (typeof window.performance?.memory !== "undefined") {
        ...
        if (ratio > 0.75 && typeof window.gc === "function") {
            window.gc();
...
function CacheResetButton() {
    const handleClick = () => {
        ...
        const parts = [];
        if (cleared.length > 0)
            parts.push(`Cleared: ${cleared.join(", ")}`);
        if (boosted.length > 0)
            parts.push(`Boosted: ${boosted.join(", ")}`);
        showToast(parts.length > 0 ? parts.join(" | ") : "Cache cleared and boost applied", Toasts.Type.SUCCESS);
    };
    return (<ChannelToolbarButton icon={CacheIcon} tooltip="Performance Boost" onClick={handleClick}/>);
}
