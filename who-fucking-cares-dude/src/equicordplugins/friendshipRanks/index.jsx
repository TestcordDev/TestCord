...
function openRankModal(rank) {
    openModal((props) => (<ErrorBoundary>
            <Modal {...props} size="sm" title={<Flex className={cl("flex")}>
...
function getBadgesToApply() {
    return ranks.map((rank, index) => {
        return ({
            ...
            shouldShow: (info) => shouldShowBadge(info.userId, rank.requirement, index),
            props: {
                style: {
                    borderRadius: "50%",
                    transform: "scale(0.9)"
...
const FriendDecoration = ErrorBoundary.wrap(({ userId }) => {
    const isFriend = useStateFromStores([RelationshipStore], () => RelationshipStore.isFriend(userId), [userId]);
    if (!isFriend)
        return null;
    return (<Tooltip text="Friend">
            {tooltipProps => (<span {...tooltipProps} className={cl("decoration")}>
...
export default definePlugin({
    ...
    renderMessageDecoration({ message }) {
        if (!settings.store.showFriendsInChat || !message?.author)
            return null;
