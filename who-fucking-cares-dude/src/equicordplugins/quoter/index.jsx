...
export default definePlugin({
    ...
    contextMenus: {
        "message": (children, { message }) => {
            if (!message.content)
                return;
            const buttonElement = (<Menu.MenuItem id="vc-quote" label="Quote" icon={QuoteIcon} action={() => openModal(props => <QuoteModal message={message} {...props}/>)}/>);
            children.push(buttonElement);
        }
    }
});
function QuoteModal({ message, ...props }) {
    ...
    return (<Modal {...props} size="md" title="Catch Them In 4K." actions={[
            {
                text: "Export",
                variant: "primary",
            ...
            {showWatermark && (<div style={{ marginTop: "8px", marginBottom: "20px" }}>
