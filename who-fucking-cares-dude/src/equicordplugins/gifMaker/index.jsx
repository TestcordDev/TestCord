...
function resolveInitialSize(sourceWidth, sourceHeight, storedWidth, storedHeight) {
...
const messageContextMenuPatch = (children, props) => {
    ...
    if (!info)
        return;
    children.push(<Menu.MenuItem id={GIFMAKER_ID} label="Make GIF" action={() => openModal(modalProps => (<GifMakerModal url={info.url} isVideo={info.isVideo} sourceWidth={info.sourceWidth} sourceHeight={info.sourceHeight} {...modalProps}/>))}/>);
};
const imageContextMenuPatch = (children, props) => {
    if (!props?.src)
        return;
    if ("href" in props && !props.src)
        return;
...
function FontOptionLabel({ option }) {
    ...
    return (<span style={{ fontFamily: getFontFamilyCss(option.value) }}>
            {option.label}
        </span>);
}
function SelectedFontLabel({ options }) {
    const option = options[0];
    return option ? <FontOptionLabel option={option}/> : null;
}
function FontSelector({ initialFont, onSelect }) {
    const [fonts, setFonts] = React.useState([]);
    const [selectedFont, setSelectedFont] = React.useState(initialFont !== "Arial" ? initialFont : null);
    React.useEffect(() => {
    ...
    return (<Select placeholder="Select a font..." options={options} maxVisibleItems={10} closeOnSelect={true} select={handleSelect} isSelected={value => value === selectedFont} serialize={value => String(value)} renderOptionLabel={(option) => <FontOptionLabel option={option}/>} renderOptionValue={(options) => <SelectedFontLabel options={options}/>}/>);
}
function GifMakerModal({ url, isVideo, sourceWidth, sourceHeight, ...props }) {
    const [options, setOptions] = useState(() => {
        const [width, height] = resolveInitialSize(sourceWidth, sourceHeight, settings.store.lastWidth, settings.store.lastHeight);
        return {
            width, height,
            captionMode: settings.store.lastCaptionMode,
            captionText: settings.store.lastCaptionText,
            captionSize: settings.store.lastCaptionSize,
            fontFamily: settings.store.lastFontFamily || DEFAULT_OPTIONS.fontFamily,
    ...
    return (<Modal {...props} size="lg" title="Make GIF" actions={[
            {
                text: "Send",
                variant: "primary",
                onClick: handleSend
            ...
            <div className={cl("modal-body")}>
                <div className={cl("preview-section")}>
                    <div className={cl("preview-wrapper")}>
                        <img ref={previewRef} alt="GIF preview" src={previewUrl ?? ""} onClick={handlePreviewClick} className={cl("preview", { "preview-generating": generating, "preview-crosshair": options.captionMode === "speechbubble" })}/>

                        {generating && (<div className={cl("generating-overlay")}>
                                Generating GIF...
                            </div>)}

                        {error && !generating && (<div className={cl("error-overlay")}>
                                {error}
                            </div>)}
                    </div>
                </div>
                ...
                <div className={cl("controls-section")}>
                    ...
                    <div style={{ display: "flex", gap: 8 }} className={Margins.bottom16}>
                        {CAPTIONS.map(c => (<Button key={c.type} size="min" onClick={() => patch({ captionMode: c.type })} variant={options.captionMode === c.type ? "primary" : "secondary"}>
                                {c.name}
                            </Button>))}
                    </div>

                    {options.captionMode === "caption" && (<div className={cl("section")}>
                            <div className={Margins.bottom16}>
                                <label className={cl("label")}>Text</label>
                                <TextInput value={options.captionText} onChange={v => patch({ captionText: v })} placeholder="Enter caption..."/>
                            </div>
                            <div className="vc-gifmaker-font-range">
                                <div>Font</div>
                                <div className={cl("font-selector")}>
                                    <FontSelector initialFont={options.fontFamily} onSelect={font => {
                patch({ fontFamily: font.family });
                            ...
                            <div className="vc-gifmaker-font-range">
                                <div>Font size</div>
                                <Slider initialValue={options.captionSize} onValueChange={v => patch({ captionSize: v })} markers={[10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100]} minValue={10} maxValue={100}/>
                                <span>{(options.captionSize).toFixed(2)}px</span>
                            </div>
                        </div>)}

                    {options.captionMode === "speechbubble" && (<div className={cl("section")}>
                            <div className={cl("section-hint")}>
                                Click on the preview to position the bubble tip
                            </div>
                            <div className="vc-gifmaker-font-range">
                                <div>Tip Base</div>
                                <Slider initialValue={Math.round(options.bubbleTipBase * 100)} onValueChange={v => patch({ bubbleTipBase: v / 100 })} minValue={0} maxValue={80}/>
                    ...
                    <div className={cl("dims-row")}>
                        <div className={cl("field")}>
                            ...
                            <input type="number" min={32} max={1024} value={options.width} onChange={e => patch({ width: Number(e.target.value) })} onBlur={e => patch({ width: clamp(Number(e.target.value), 32, 1024, 32) })} className={cl("input")} aria-label="Width"/>
                        </div>
                        <div className={cl("field")}>
                            <label className={cl("label")}>Height</label>
                            <input type="number" min={32} max={1024} value={options.height} onChange={e => patch({ height: Number(e.target.value) })} onBlur={e => patch({ height: clamp(Number(e.target.value), 32, 1024, 32) })} className={cl("input")} aria-label="Height"/>
                        </div>
                    </div>
                </div>
            </div>
...
function openGifMakerFromItem(item) {
    ...
    if (directUrl)
        adjustedCandidates.add(normalizeUrl(directUrl));
...
export default definePlugin({
    ...
    gifPickerContextMenu(instance, _e) {
        if (!instance?.props?.item?.src)
            return null;
        return (<Menu.MenuItem id="gif-maker-edit" key="gif-maker-edit" label="Edit GIF" action={() => openGifMakerFromItem(instance?.props?.item)}/>);
    },
});
