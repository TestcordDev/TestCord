...
async function embedDidMount() {
    try {
        const { embed } = this.props;
        const { replaceElements, dearrowByDefault } = settings.store;
        if (!embed || embed.dearrow || embed.provider?.name !== "YouTube" || !embed.video?.url)
            return;
        ...
        if (hasTitle && replaceElements !== 2 /* ReplaceElements.ReplaceThumbnailsOnly */) {
            ...
            if (dearrowByDefault)
                embed.rawTitle = replacementTitle;
        }
        if (hasThumb && replaceElements !== 1 /* ReplaceElements.ReplaceTitlesOnly */) {
...
function DearrowButton({ component }) {
    const { embed } = component.props;
    if (!embed?.dearrow)
        return null;
    return (<Tooltip text={embed.dearrow.enabled ? "This embed has been dearrowed, click to restore" : "Click to dearrow"}>
            {({ onMouseEnter, onMouseLeave }) => (<button onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={"vc-dearrow-toggle-" + (embed.dearrow.enabled ? "on" : "off")} onClick={() => {
                ...
                component.forceUpdate();
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 36 36" aria-label="Toggle Dearrow" className="vc-dearrow-icon">
                        <path fill="#1213BD" d="M36 18.302c0 4.981-2.46 9.198-5.655 12.462s-7.323 5.152-12.199 5.152s-9.764-1.112-12.959-4.376S0 23.283 0 18.302s2.574-9.38 5.769-12.644S13.271 0 18.146 0s9.394 2.178 12.589 5.442C33.931 8.706 36 13.322 36 18.302z"/>
...
const settings = definePluginSettings({
    ...
    replaceElements: {
        ...
        restartNeeded: true,
        options: [
            { label: "Everything (Titles & Thumbnails)", value: 0 /* ReplaceElements.ReplaceAllElements */, default: true },
            { label: "Titles", value: 1 /* ReplaceElements.ReplaceTitlesOnly */ },
            { label: "Thumbnails", value: 2 /* ReplaceElements.ReplaceThumbnailsOnly */ },
        ],
    },
    dearrowByDefault: {
