...
async function nativeFetch(url) {
    const helper = window.VencordNative?.pluginHelpers?.ThemeStore;
    if (!helper?.fetchUrl)
        throw new Error("ThemeStore native helper not found.");
...
function addOnlineTheme(url) {
    const S = window.Vencord?.Settings ?? Settings;
    if (!S)
        throw new Error("Could not access Vencord Settings.");
    const currentLinks = Array.isArray(S.themeLinks) ? [...S.themeLinks] : [];
    const alreadyIn = currentLinks.includes(url);
    if (!alreadyIn)
        S.themeLinks = [...currentLinks, url];
    const currentEnabled = Array.isArray(S.enabledThemeLinks) ? [...S.enabledThemeLinks] : [];
    if (!currentEnabled.includes(url))
        S.enabledThemeLinks = [...currentEnabled, url];
    return !alreadyIn;
}
...
function injectStyles() {
    if (document.getElementById("ts-plugin-styles"))
        return;
...
function ThemePreview({ theme, installed, onInstall }) {
    const [state, setState] = React.useState(installed ? "done" : "idle");
    ...
    const releaseDate = theme.release_date
        ...
        : "Unknown";
    return (<div className="ts-right">
            {/* Preview image */}
            <div className="ts-preview-img-wrap">
                {theme.thumbnail_url && !imgErr ? (<img key={theme.id} className="ts-preview-img" src={theme.thumbnail_url} alt={theme.name} onError={() => setImgErr(true)}/>) : (<div className="ts-preview-placeholder">🎨</div>)}
            </div>
            ...
            <div className="ts-preview-info">
                ...
                {theme.description && (<p className="ts-preview-desc">{theme.description}</p>)}
                ...
                {/* Tags */}
                {theme.tags?.length > 0 && (<div className="ts-preview-tags">
                        {theme.tags.map(tag => (<span key={tag} className="ts-ptag">{tag}</span>))}
                    </div>)}

                {/* Install */}
                <button className={`ts-install-btn${state === "done" ? " done" : state === "busy" ? " busy" : ""}`} onClick={handleInstall} disabled={state !== "idle"}>
                    {state === "done" ? "✓ Installed & Active" : state === "busy" ? "Installing…" : "Install Theme"}
                </button>

                <button className="ts-source-link" onClick={() => window.VencordNative?.native?.openExternal?.(theme.source)}>
...
// ─── Main Modal ───────────────────────────────────────────────────────────────
function ThemeStoreModal({ onClose }) {
    ...
    React.useEffect(() => {
        const hiddenEls = [];
        const hideBar = () => {
            document.querySelectorAll(".vc-plugin-modal-links").forEach(el => {
                let parent = el;
    ...
    return (<div className="ts-overlay" onClick={e => { if (e.target === e.currentTarget)
            ...
            <div className="ts-modal">
                ...
                <div className="ts-left">
                    ...
                    <div className="ts-search-wrap">
                        <input className="ts-search" placeholder="Search themes, authors, tags…" value={search} onChange={e => setSearch(e.currentTarget.value)}/>
                    </div>

                    <div className="ts-sort-tabs">
                        {SORTS.map(s => (<button key={s.key} className={`ts-sort-tab${sort === s.key ? " active" : ""}`} onClick={() => setSort(s.key)}>{s.label}</button>))}
                        <button className={`ts-refresh-btn${refreshing ? " spinning" : ""}`} onClick={() => load(true)} title="Refresh">↻</button>
                    </div>

                    {/* Tag filter bar */}
                    {!loading && !error && allTags.length > 0 && (<div className="ts-tag-bar">
                            {allTags.map(tag => (<button key={tag} className={`ts-tag-filter${activeTag === tag ? " active" : ""}`} onClick={() => setActiveTag(activeTag === tag ? null : tag)}>
                                    {tag}
                                </button>))}
                        </div>)}

                    {!loading && !error && (<div className="ts-stats">
                            {visible.length} theme{visible.length !== 1 ? "s" : ""}
                            {activeTag ? ` tagged "${activeTag}"` : search ? ` for "${search}"` : ""}
                            {" · "}{installed.size} installed
                            {activeTag && (<span onClick={() => setActiveTag(null)} style={{ marginLeft: 6, color: "#5865f2", cursor: "pointer" }}>✕ clear</span>)}
                        </div>)}

                    <div className="ts-list">
                        {loading ? (<div className="ts-center" style={{ minHeight: 200 }}>
                                <span className="ts-loader">⟳</span>
                                <p>Loading themes…</p>
                            </div>) : error ? (<div className="ts-center" style={{ minHeight: 200 }}>
                                <span style={{ fontSize: 36 }}>⚠️</span>
                                <p>Failed to load</p>
                                <p className="sub">{error}</p>
                                <button className="ts-retry-btn" onClick={() => load()}>Retry</button>
                            </div>) : visible.length === 0 ? (<div className="ts-empty">
                                <span className="ts-empty-ico">🔍</span>
                                <p>No themes found</p>
                            </div>) : visible.map(theme => (<ThemeListItem key={theme.id} theme={theme} selected={selected?.id === theme.id} installed={installed.has(theme.id)} onClick={() => setSelected(theme)}/>))}
                    </div>
                </div>

                {/* ── Right panel ── */}
                {selected ? (<ThemePreview theme={selected} installed={installed.has(selected.id)} onInstall={() => handleInstall(selected)}/>) : (<div className="ts-right">
...
function ThemeListItem({ theme, selected, installed, onClick }) {
    const [imgErr, setImgErr] = React.useState(false);
    return (<div className={`ts-item${selected ? " selected" : ""}`} onClick={onClick}>
            {theme.thumbnail_url && !imgErr ? (<img className="ts-item-thumb" src={theme.thumbnail_url} alt={theme.name} loading="lazy" onError={() => setImgErr(true)}/>) : (<div className="ts-item-thumb-placeholder">🎨</div>)}
...
export default definePlugin({
    ...
    start() { injectStyles(); },
    stop() { removeStyles(); },
    settingsAboutComponent: () => (<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <Forms.FormText>
