...
function searchProtoClassField(localName, protoClass) {
    const field = protoClass?.fields?.find((field) => field.localName === localName);
    if (!field)
        return;
...
const IS_BYPASSEABLE_INTENTION = `[${3 /* EmojiIntentions.CHAT */},${4 /* EmojiIntentions.GUILD_STICKER_RELATED_EMOJI */}].includes(fakeNitroIntention)`;
...
const hasExternalEmojiPerms = (channelId) => hasPermission(channelId, PermissionsBits.USE_EXTERNAL_EMOJIS);
const hasExternalStickerPerms = (channelId) => hasPermission(channelId, PermissionsBits.USE_EXTERNAL_STICKERS);
const hasEmbedPerms = (channelId) => hasPermission(channelId, PermissionsBits.EMBED_LINKS);
const hasAttachmentPerms = (channelId) => hasPermission(channelId, PermissionsBits.ATTACH_FILES);
function getWordBoundary(origStr, offset) {
    return (!origStr[offset] || /\s/.test(origStr[offset])) ? "" : " ";
}
function CannotEmbedNoticeModal({ modalProps, resolve }) {
    const s = settings.use(["disableEmbedPermissionCheck"]);
    return (<ConfirmModal {...modalProps} title="Hold on!" subtitle="You are trying to send/edit a message that contains a FakeNitro emoji or sticker, however you do not have permissions to embed links in the current channel. Are you sure you want to send this message? Your FakeNitro items will appear as a link only." confirmText="Send Anyway" cancelText="Cancel" onConfirm={() => resolve(true)} onCloseCallback={() => setImmediate(() => resolve(false))} checkboxProps={{
...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace: (_, rest, props, rest2, reactNode) => `${rest}let{fakeNitroRenderableSticker}=${props};${rest2}$self.addFakeNotice(${0 /* FakeNoticeType.Sticker */},${reactNode},!!fakeNitroRenderableSticker?.fake)`
                }
            ]
        },
        {
        ...
        },
        {
            ...
            replacement: {
                ...
                replace: (_, reactNode) => `$self.addFakeNotice(${1 /* FakeNoticeType.Emoji */},${reactNode},!!arguments[0]?.fakeNitroNode?.fake)`
            }
        },
    ...
    patchFakeNitroEmojisOrRemoveStickersLinks(content, inline) {
        ...
        if ((content.length > 1 || typeof content[0]?.type === "string") && !settings.store.transformCompoundSentence)
            return content;
            ...
            switch (embed.type) {
                case "image": {
                    ...
                    if (!settings.store.transformCompoundSentence
                        ...
                        return false;
                    if (settings.store.transformEmojis) {
                        if (fakeNitroEmojiRegex.test(url))
                            return true;
                    }
                    if (settings.store.transformStickers) {
                        if (fakeNitroStickerRegex.test(url))
                            return true;
    ...
    filterAttachments(attachments) {
        return attachments.filter(attachment => {
            ...
            if (match) {
                // There is no way to differentiate a regular gif attachment from a fake nitro animated sticker, so we check if the StickersStore contains the id of the fake sticker
                if (StickersStore.getStickerById(match[1]))
                    return false;
            }
            return true;
        });
    ...
    async sendAnimatedSticker(stickerLink, stickerId, channelId) {
        ...
        for (const frame of frames) {
            ...
            if (blendOp === 0 /* ApngBlendOp.SOURCE */) {
            ...
            if (disposeOp === 1 /* ApngDisposeOp.BACKGROUND */) {
                ctx.clearRect(left, top, width, height);
            }
            else if (disposeOp === 2 /* ApngDisposeOp.PREVIOUS */) {
    ...
    start() {
        ...
        this.preSend = addMessagePreSendListener(async (channelId, messageObj, options) => {
            ...
            stickerBypass: {
                ...
                if (sticker.format_type === 2 /* StickerFormatType.APNG */) {
                    if (!hasAttachmentPerms(channelId)) {
                        openModal(props => (<ConfirmModal {...props} title="Hold on!" confirmText="OK" variant="primary">
                                <div>
                                    <Paragraph>
            ...
            if (s.enableEmojiBypass) {
                for (const emoji of messageObj.validNonShortcutEmojis) {
                    if (this.canUseEmote(emoji, channelId))
                        continue;
                    hasBypass = true;
