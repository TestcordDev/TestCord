...
export function SettingsPresetList(isResolution) {
    ...
    return (<section>
            <Flex gap={"0.5em"} flexWrap="wrap">
                {(isResolution ? resolutions : fpss).map(({ label, value }, index) => (<Card key={index} className={cl("settings-card")}>
                ...
                <Card className={cl("settings-card")}>
                    <Flex gap={"0.5em"} alignItems="center" className={cl("settings-flex-input")}>
                        <TextInput type="number" onBeforeInput={(e) => {
            if (e.data !== null && !/^\d*$/.test(e.data))
                e.preventDefault();
        }} value={value} onChange={e => setValue((e.length > 0 ? parseInt(e) : undefined))} error={errorString}/>
