...
export default definePlugin({
    ...
    stickerButton({ innerClassName, isActive, onClick }) {
        return (<button className={innerClassName} onClick={onClick} style={{ backgroundColor: "transparent" }}>
                {/* Icon taken from: https://github.com/Pitu/Magane/blob/0ebb09acf9901933ebebe19fbd473ec08cf917b3/src/Button.svelte#L29 */}
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" className={cl("icon", { "icon-active": isActive })}>
    ...
    moreStickersComponent({ channel, closePopout }) {
        ...
        const [selectedStickerPackId, setSelectedStickerPackId] = React.useState(null);
        const ffmpegLoaded = React.useState(false);
        const ffmpeg = React.useState(new FFmpeg());
        const getMetasSignature = (m) => m.map(x => x.id).sort().join(",");
        React.useEffect(() => {
            (async () => {
                console.log("Updating sticker packs...", counter);
                setCounter(counter + 1);
                const sps = (await Promise.all(stickerPackMetas.map(meta => getStickerPack(meta.id))))
                    .filter((x) => x !== null);
                setStickerPacks(sps);
            })();
        }, [stickerPackMetas]);
        ...
        return (<Wrapper>
                ...
                isLoaded: ffmpegLoaded[0]
                    ...
                    <PickerContent stickerPacks={stickerPacks} selectedStickerPackId={selectedStickerPackId} setSelectedStickerPackId={setSelectedStickerPackId} channelId={channel.id} closePopout={closePopout} query={query}/>
                </FFmpegStateContext.Provider>
                <PickerSidebar packMetas={stickerPackMetas.map(meta => ({
                id: meta.id,
                name: meta.title,
