...
function describe(userId, channelId) {
    ...
    if (isCall && !settings.store.includeCalls)
        return null;
    ...
    return {
        ...
        channelName: isCall ? t("مكالمة", "Call") : channel.name,
        placeName: isCall
            ? t("الرسائل الخاصة", "Direct messages")
...
function collectEntries() {
    ...
    const interesting = new Set([...watched, ...(settings.store.showAllFriends ? friends : [])]);
    if (settings.store.showAllFriends) {
        ...
        for (const guildStates of Object.values(allStates ?? {})) {
            for (const userId in guildStates) {
                ...
                if (interesting.has(userId) && channelId)
                    channelIds.set(userId, channelId);
            }
        }
    }
    for (const userId of interesting) {
        if (channelIds.has(userId))
            continue;
...
function useEntries() {
    ...
    return useStateFromStores([VoiceStateStore, RelationshipStore, ChannelStore, GuildStore], collectEntries, undefined, (a, b) => keyOf(a) === keyOf(b));
}
function FriendRow({ entry }) {
    return (<div className={cl("row")}>
            ...
            : t("ليس في روم صوتي", "Not in a voice channel")}
                    {entry.isHidden && <span className={cl("state")}>🔒 {t("مخفي", "Hidden")}</span>}
                    {entry.isLocked && <span className={cl("state")}>🔒 {t("مقفل", "Locked")}</span>}
                </Text>
            </div>
            {entry.channelId && (<button className={cl("join")} disabled={!entry.canJoin} onClick={() => ChannelActions.selectVoiceChannel(entry.channelId)}>
                    {entry.canJoin ? t("انضمام", "Join") : t("لا صلاحية", "No access")}
                </button>)}
        </div>);
}
function FriendsInVoicePanel({ transitionState }) {
    const entries = useEntries();
    return (<ModalRoot transitionState={transitionState} size={"small" /* ModalSize.SMALL */}>
            <ModalHeader>
                <Text variant="heading-lg/semibold">{t("الأصدقاء في الصوت", "Friends in voice")}</Text>
            </ModalHeader>
            <ModalContent>
                <div className={cl("panel")} dir={isArabicMode() ? "rtl" : "ltr"}>
                    {entries.length === 0 ? (<div className={cl("empty")}>
                            {t("لا أحد الآن. كليك يمين على شخص ← «مراقبة في الصوت» ليظهر هنا دائماً.", "Nobody right now. Right-click someone → \"Watch in voice\" to always track them here.")}
                        </div>) : (<ScrollerThin className={cl("list")} fade>
...
function PeopleIcon({ className }) {
    return (<svg className={className} width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path fill="currentColor" d="M9 4a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Zm7.5 1a3 3 0 1 0 0 6 3 3 0 0 0 0-6ZM9 12.5c-3.31 0-6 1.79-6 4 0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2 0-2.21-2.69-4-6-4Zm7.5.5c-.62 0-1.2.07-1.74.2A5.7 5.7 0 0 1 16.5 17c0 .55-.15 1.06-.4 1.5H20c1.1 0 2-.9 2-2 0-1.93-2.24-3.5-5.5-3.5Z"/>
        </svg>);
}
function FriendsInVoiceButton() {
    const entries = useEntries();
    const inVoice = entries.filter(e => e.channelId).length;
    return (<HeaderBarButton icon={PeopleIcon} tooltip={inVoice > 0
            ? t(`الأصدقاء في الصوت (${inVoice})`, `Friends in voice (${inVoice})`)
            : t("الأصدقاء في الصوت", "Friends in voice")} selected={inVoice > 0} aria-label={t("الأصدقاء في الصوت", "Friends in voice")} onClick={() => {
...
const userContextPatch = (children, props) => {
    ...
    const group = findGroupChildrenByChildId("user-profile", children)
        ...
        ?? children;
    group.push(<Menu.MenuCheckboxItem id="vc-fiv-watch" key="vc-fiv-watch" label={t("مراقبة في الصوت", "Watch in voice")} checked={watched.has(userId)} action={() => toggleWatch(userId)}/>);
};
