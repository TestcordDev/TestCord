...
async function applyRules(channelID, content) {
    let workingContent = content;
    for (const rule of settings.store.rules) {
        if (!rule.find || !rule.replace)
            continue;
        const fn = new Function("_", `return (async () => { ${rule.replace} })();`);
        workingContent = await asyncReplace(workingContent, stringToRegex(rule.find), async (match, ...args) => {
