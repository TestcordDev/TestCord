...
import { ChannelStore, FluxDispatcher, ScrollerThin, TabBar, Tooltip, useRef, UserStore, useState } from "@webpack/common";
import { KeywordEntries } from "./components/KeywordEntries";
export let keywordEntries = [];
let keywordLog = [];
let interceptor;
...
const getMessageScrollerOptions = findByCodeLazy("onKeyDown", "tabIndex", "useContext", "aria-orientation");
...
export async function addKeywordEntry(forceUpdate) {
    keywordEntries.push({
        regex: "",
        ignoreCase: false,
        ignoreBots: true,
...
export async function removeKeywordEntry(idx, forceUpdate) {
    keywordEntries.splice(idx, 1);
    await DataStore.set(KEYWORD_ENTRIES_KEY, keywordEntries);
    forceUpdate();
}
const regexCache = new Map();
function getCachedRegex(regex, flags) {
...
function safeMatchesRegex(str, regex, flags) {
...
function highlightKeywords(str, entries) {
    let regexes;
    try {
...
export default definePlugin({
    ...
    async start() {
        ...
        keywordEntries.forEach(entry => {
            ...
            if (entry.listType == null || entry.listIds == null)
                return;
    ...
    checkEmbeds(embeds, regex, flags) {
        if (!embeds?.length)
            return false;
    ...
    storeMessage(m) {
        ...
        DataStore.get(KEYWORD_LOG_KEY).then(log => {
            log = log ? log.map((e) => JSON.parse(e)) : [];
            log.push(m);
    ...
    discardMessage(id) {
        DataStore.get(KEYWORD_LOG_KEY).then((log) => {
            let parsed_logs = log ? log.map(e => JSON.parse(e)) : [];
    ...
    keywordTabBar() {
        return (<TabBar.Item className={classes(tabClass.tab)} id={8}>
                Keywords
            </TabBar.Item>);
    },
    keywordClearButton() {
        return (<Tooltip text="Clear All">
                {({ onMouseLeave, onMouseEnter }) => (<Button variant="secondary" size="iconOnly" onMouseLeave={onMouseLeave} onMouseEnter={onMouseEnter} onClick={() => {
                    keywordLog = [];
    ...
    tryKeywordMenu(onJump) {
        ...
        const MessageScrollerHelper = ({ children }) => {
        ...
        return (<ErrorBoundary>
                <PopoutContainer navigator={navigator}>
                    <MessageScrollerHelper>
                        {({ ref, ...restOpts }) => {
                return <ScrollerThin ref={thinScrollerRef => {
                        navigatorScrollerRef.current = thinScrollerRef;
                        // @ts-ignore
                        ref.current = thinScrollerRef?.getScrollerNode() ?? null;
                    }} className={classes(scrollerClass.scroller)} onScroll={void 0} {...restOpts}>
