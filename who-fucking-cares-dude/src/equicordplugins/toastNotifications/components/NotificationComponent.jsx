...
function isMessageNotification(props) {
    return "mockedMessage" in props;
}
function renderContextHeader(channel) {
    if (channel.isGroupDM()) {
        ...
        return (<div className={cl("context-header")}>
...
export default ErrorBoundary.wrap(function NotificationComponent(props) {
    const [isHover, setIsHover] = useState(false);
    const timeout = (PluginSettings.store.timeout ?? 5) * 1000;
    const opacity = PluginSettings.store.opacity / 100;
    useEffect(() => {
        if (isHover || props.permanent || timeout === 0)
            return;
    ...
    const closeButton = useMemo(() => (<button className={cl("notification-close-btn")} onClick={e => {
            e.preventDefault();
            e.stopPropagation();
            props.onClose();
        }}>
    ...
    let content;
    if (isMessageNotification(props)) {
        content = (<div className={cl("notification-content")}>
                {renderContextHeader(props.channel)}
                <MessageComponent id={`toastnotification-mock-${props.message.id}`} message={props.mockedMessage} channel={props.channel} subscribeToComponentDispatch={false}/>
            </div>);
    }
    else {
        const { title, body, icon } = props;
        content = (<div className={cl("notification-system")}>
                ...
                <div className={cl("notification-content")}>
                    ...
                    {body && (<p className={cl("notification-p")}>
                            {body.length > 500 ? body.slice(0, 500) + "..." : body}
                        </p>)}
                </div>
            </div>);
    }
    return (<button style={{ opacity }} className={cl("notification-root")} onClick={handleClick} onContextMenu={handleContextMenu} onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
            {closeButton}
            {content}
            {timeout !== 0 && !props.permanent && (<div className={cl("notification-progressbar", { paused: isHover })} style={{ animationDuration: `${timeout}ms` }}/>)}
        </button>);
}, {
