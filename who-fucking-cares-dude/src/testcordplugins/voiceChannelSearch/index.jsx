...
async function scan() {
    ...
    if (scanCache && Date.now() - scanCacheAt < SCAN_TTL)
        return scanCache;
...
function SearchIcon({ width = 20, height = 20 }) {
    return (<svg width={width} height={height} viewBox="0 0 24 24" fill="currentColor">
...
function SpinnerIcon() {
    return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: "vcs-spin 0.8s linear infinite" }}>
            <circle cx="12" cy="12" r="10" strokeOpacity="0.25"/>
            <path d="M12 2a10 10 0 0 1 10 10" strokeLinecap="round"/>
        </svg>);
}
function VoiceSearchModal({ rootProps, channels }) {
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="vcs-content">
                {channels === null ? (<div className="vcs-loading">
                        <SpinnerIcon />
                        ...
                        <div className="vcs-search-bar">
                            ...
                            <input ref={inputRef} autoFocus className="vcs-search-input" placeholder={t("Channel or server...")} value={query} onChange={handleQueryChange}/>
                            {query && (<button className="vcs-search-clear" onClick={() => { setQuery(""); setFiltered(channels); }}>
                        ...
                        <div className="vcs-channel-list">
                            {displayList.length === 0 && (<div className="vcs-empty">{query ? t("No channel found") : t("No voice channels")}</div>)}
                            {displayList.slice(0, 150).map(ch => (<div key={ch.channelId} className={`vcs-row${ch.canAccess ? "" : " vcs-row--locked"}`} onClick={() => ch.canAccess && join(ch)} title={ch.canAccess ? undefined : t("No permission to join this channel")}>
                    ...
                    : <span style={{ opacity: 0.5, fontSize: 13 }}>🔒</span>}
                                    ...
                                    <div className="vcs-info">
                                        ...
                                        <div className="vcs-guild">
                                            ...
                                            {ch.memberCount > 0 && (<div className="vcs-members-info">
                        ...
                        const channelStates = Object.values(guildStates).filter((s) => s.channelId === ch.channelId);
                        return channelStates.slice(0, 10).map((s) => {
                    ...
                    : ch.canAccess
                        ...
                        : <span className="vcs-locked-label">{t("Private")}</span>}
                            ...
                            {displayList.length > 80 && !query && (<div className="vcs-empty" style={{ fontSize: 11, opacity: 0.5 }}>
...
function VoiceSearchModalWrapper({ rootProps }) {
...
function VCSHeaderButton() {
    return (<HeaderBarButton icon={SearchIcon} tooltip={t("Search voice channel")} onClick={() => openModal(props => <VoiceSearchModalWrapper rootProps={props}/>)}/>);
}
