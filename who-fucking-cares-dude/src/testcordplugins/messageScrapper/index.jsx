    ...
    logActions: {
        ...
        component: function LogActions() {
            ...
            return (<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <Button disabled={!lastLogFilePath} onClick={() => lastLogFilePath && showItemInFolder(lastLogFilePath)}>Open last log location</Button>
...
async function fetchAllMessages(channelId, throttleMs = 250, onProgress) {
    const result = [];
    let before = undefined;
    while (true) {
        const batch = await TestcordRequestCoordinator.request({
...
function FriendTag({ id, onRemove }) {
    ...
    return (<div style={{
            ...
            <span style={{ color: "var(--text-normal)", fontWeight: 500 }}>{user.globalName || user.username}</span>
            <button aria-label="remove" onClick={() => onRemove(id)} style={{
...
function ProgressModal({ modalProps, onClose, progressRef, onStop }) {
    ...
    const [lastTimeoutTime, setLastTimeoutTime] = React.useState(null);
    const startTimeRef = React.useRef(Date.now());
    const deletedTimesRef = React.useRef([]); // Track deletion times for better averaging
    ...
    React.useImperativeHandle(progressRef, () => ({
        setTotalMessages,
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            fontWeight: "bold"
                ...
                <div style={{ display: "flex", gap: 24, fontSize: "14px" }}>
                    ...
                    {timeoutCount > 0 && (<div>
                            ...
                            {lastTimeoutTime && (<span style={{ color: "var(--text-muted)", marginLeft: 8, fontSize: "12px" }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: "200px" }}>
                    ...
                    <div id="message-scrapper-logs" style={{
            flex: 1,
            ...
            maxHeight: "300px"
                        ...
                        {logs.length === 0 && (<div style={{ color: "var(--text-muted)", fontStyle: "italic" }}>Waiting for logs...</div>)}
                        {logs.map((log, idx) => (<div key={idx} style={{
                marginBottom: "4px",
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: 12, width: "100%", justifyContent: "space-between" }}>
                    <Button onClick={onStop} disabled={!isRunning} color={Button.Colors.RED} style={{
...
function WhitelistModal({ modalProps }) {
    const [query, setQuery] = React.useState("");
    const [wl, setWl] = React.useState([]);
    const [mode, setMode] = React.useState(null);
    const [selectedGuilds, setSelectedGuilds] = React.useState([]);
    ...
    const candidateIds = React.useMemo(() => {
        ...
        const base = (friendIds.length ? friendIds : dms.map(c => c.recipients?.[0]).filter(Boolean));
        return base
    ...
    async function start() {
        ...
        // Open progress modal
        const progressModalKey = openModal((props) => (<ProgressModal modalProps={props} progressRef={progressRef} onStop={stopProcess} onClose={() => {
                if (!isProcessRunning) {
                    if (progressModalKey)
                        closeModal(progressModalKey);
                    setIsRunning(false);
                    props.onClose();
                }
            }}/>));
        ...
        if (mode === "dm") {
            ...
            for (const guildId of selectedGuilds) {
                const info = GuildChannelStore.getChannels?.(guildId);
        ...
        for (const ch of targets) {
            try {
                ...
                const toDelete = messages.filter((m) => m?.author?.id === myId);
                ...
                for (const m of toDelete) {
                    ...
                    try {
                        ...
                        deleted.push({
                            ...
                            attachments: (m.attachments || []).map((a) => ({ filename: a.filename, url: a.url, content_type: a.content_type }))
                        });
        ...
        try {
            if (window.IS_DISCORD_DESKTOP) {
                const data = new TextEncoder().encode(JSON.stringify(body, null, 2));
                const savedPath = await window.DiscordNative.fileManager.saveWithDialog(data, filename, "application/json");
                if (savedPath) {
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            ...
            color: mode === "dm" ? "var(--white-500)" : "var(--text-normal)"
                        ...
                        <Button onClick={() => setMode("server")} style={{
            flex: 1,
            ...
            color: mode === "server" ? "var(--white-500)" : "var(--text-normal)"
                    ...
                    {mode === "server" && (<div style={{ marginTop: 12 }}>
                ...
                padding: "8px"
                                ...
                                {guilds.map((guild) => (<div key={guild.id} onClick={() => {
                    setSelectedGuilds(prev => prev.includes(guild.id)
                    ...
                    marginBottom: "4px"
                                        ...
                                        <input type="checkbox" checked={selectedGuilds.includes(guild.id)} onChange={() => { }} style={{ cursor: "pointer" }}/>
                                        <img src={guild.getIconURL?.({ size: 24 })} width={24} height={24} style={{ borderRadius: "50%" }}/>
            ...
            border: "1px solid var(--background-modifier-accent)"
                        ...
                        {wl.map(id => (<FriendTag key={id} id={id} onRemove={idToRemove => {
                setWl(wl.filter(x => x !== idToRemove));
            }}/>))}
                        {wl.length === 0 && (<div style={{ color: "var(--text-muted)", fontStyle: "italic", alignSelf: "center" }}>
                ...
                <div style={{ marginBottom: 20 }}>
                    ...
                    <input placeholder="Search users by name" value={query} onChange={e => setQuery(e.target.value)} style={{
            width: "100%",
            padding: "10px 12px",
            borderRadius: 8,
            border: "1px solid var(--background-modifier-accent)",
            ...
            const label = (u?.globalName || u?.username || id || "Unknown");
            ...
            return (<div key={id} style={{
                    ...
                    e.currentTarget.style.background = "transparent";
                        ...
                        {candidateIds.length === 0 && query && (<div style={{ padding: "20px", textAlign: "center", color: "var(--text-muted)", fontStyle: "italic" }}>
                                No matches found
                            </div>)}
                    </div>
                </div>
            ...
            border: "1px solid var(--background-modifier-accent)"
                    ...
                    <Button color={Button.Colors.RED} onClick={start} disabled={isRunning || !mode || (mode === "server" && selectedGuilds.length === 0)} style={{
...
async function openMessageScrapperModal() {
    ...
    if (IS_DISCORD_DESKTOP) {
        try {
            ...
            await ipcRenderer.invoke("OPEN_MESSAGE_SCRAPPER_WINDOW");
    ...
    currentModalKey = openModal((props) => (<WhitelistModal modalProps={{
            ...props,
            onClose: () => {
                // Reset on close
                if (!isProcessRunning) {
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("MessagesScrapper", () => (<HeaderBarButton icon={() => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M9 3h6a1 1 0 0 1 1 1v1h4v2H4V5h4V4a1 1 0 0 1 1-1Zm-3 6h12l-1 11a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2L6 9Z"/>
                        </svg>)} tooltip="Messages Scrapper" onClick={openMessageScrapperModal}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("MessagesScrapper", () => (<ChannelToolbarButton icon={() => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M9 3h6a1 1 0 0 1 1 1v1h4v2H4V5h4V4a1 1 0 0 1 1-1Zm-3 6h12l-1 11a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2L6 9Z"/>
                        </svg>)} tooltip="Messages Scrapper" onClick={openMessageScrapperModal}/>), 5);
        }
    },
    ...
    renderChatBarButton: (({ isMainChat }) => {
        if (!isMainChat || settings.store.location !== "chatbar")
            return null;
        return (<ChatBarButton tooltip="Messages Scrapper" onClick={openMessageScrapperModal} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="pc-messages-scrapper-menu" onClose={ContextMenuApi.closeContextMenu} aria-label="Messages Scrapper">
                            <Menu.MenuItem id="pc-messages-scrapper-open" label="Open Messages Scrapper" action={openMessageScrapperModal}/>
                            <Menu.MenuItem id="pc-messages-scrapper-stop" label="Stop" action={stopMessageScrapper} disabled={!isProcessRunning}/>
                        </Menu.Menu>))}>
