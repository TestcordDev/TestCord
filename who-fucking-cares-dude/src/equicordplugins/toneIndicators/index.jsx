...
function splitTextWithIndicators(text) {
    const nodes = [];
    let lastIndex = 0;
...
function patchChildrenTree(children) {
    const transform = (node) => {
        ...
        if (typeof node === "string") {
            ...
            if (!new RegExp(`${escapedPrefix}[\\p{L}_]+`, "iu").test(node))
                return node;
    ...
    if (Array.isArray(children))
        return children.map(transform).flat();
