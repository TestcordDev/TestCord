...
const settings = definePluginSettings({
    apiKey: {
        ...
        description: "Your OpenAI API key (obtained from https://platform.openai.com/api-keys)",
        default: "",
        placeholder: "sk-proj-...",
    },
    model: {
...
export default definePlugin({
    name: "ChatGPT",
    description: "Allows using ChatGPT directly in Discord with configurable settings",
    tags: ["Chat", "Utility"],
    ...
    commands: [
        {
            ...
            execute: async (opts, ctx) => {
                try {
                    ...
                    if (isProcessing) {
                        sendBotMessage(ctx.channel.id, {
                            content: "⏳ A ChatGPT request is already in progress. Please wait...",
                        });
                        return;
                    }
                    ...
                    if (!validateApiKey(apiKey)) {
                        sendBotMessage(ctx.channel.id, {
                            content: "❌ API key not configured or invalid. Configure your key in the ChatGPT plugin settings.",
                        });
                        return;
                    }
                    isProcessing = true;
    ...
    start() {
        ...
        try {
            ...
            const hasValidKey = validateApiKey(settings.store.apiKey);
            if (!hasValidKey) {
                notify("ChatGPT Plugin", "API key not configured. Configure your key in the plugin settings.", true);
            }
            else {
