...
const CustomPresetList = ({ onChange, initialValue, group, id, list }) => {
    return list.map(({ label, value }, index) => <Menu.MenuRadioItem key={index} group={`${group}`} id={`${id}-${value}`} action={() => onChange(value)} checked={initialValue === value} label={label}>
        </Menu.MenuRadioItem>);
};
export default definePlugin({
    ...
    OptionsRange(changeStream, initialValue, isResolution) {
        ...
        return [
            CustomRange(isResolution ? {
                onChange: (value) => changeStream(value * rounder),
                ...
                onChange: (value) => changeStream(value),
                initialValue,
    ...
    SettingsRange(changeStream, params, isResolution) {
        const { maxFPS, maxResolution, roundResolution, resolutions, fpss } = settings.store;
        const rounder = roundResolution ? 10 : 1;
        const [p1, p2] = params;
        ...
        return [
            CustomRange(isResolution ? {
                onChange: (value) => changeStream(p1, value * rounder, getFPS(), p2),
                ...
                onChange: (value) => changeStream(p1, getResolution(), value, p2),
                initialValue: getFPS(),
                minMax: [MIN_FPS, maxFPS],
            ...
            ...CustomPresetList(isResolution ? {
                onChange: (value) => changeStream(p1, value, getFPS(), p2),
