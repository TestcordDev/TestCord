...
export const isDonor = (userId) => !!(BadgeAPI.getDonorBadges(userId)?.length > 0
...
export function DonateButtonComponent({ donated = false }) {
    return (<Flex>
            <DonateButton equicord={true} className={!donated ? "vc-donate-support-button" : ""} style={{ marginTop: "1em" }}/>
            <InviteButton className={!donated ? "vc-invite-support-button" : ""} style={{ marginTop: "1em" }}/>
        </Flex>);
}
