...
export const HighlighterContainer = (props) => {
    const [_, _err, isPending] = useAwaiter(requireHljs);
    useEffect(() => {
        if (!isPending)
            didLoadHljs = true;
    }, [isPending]);
    if (!didLoadHljs && isPending)
        return null;
    return (<pre className={cl("container")}>
            <ErrorBoundary>
                <Highlighter {...props}/>
            </ErrorBoundary>
        </pre>);
};
export const Highlighter = ({ lang, content, isPreview, }) => {
    ...
    const [tokens] = useAwaiter(async () => {
        if (!shikiLang || useHljs || !isIntersecting)
            return null;
    ...
    return (<div ref={rootRef} className={cl("root", { plain: !langName, preview: isPreview })} style={{
