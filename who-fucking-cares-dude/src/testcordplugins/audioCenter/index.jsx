    ...
    try {
        ...
        _patchRestores.push(RuntimeInterposition.register({
            ...
            wrap: next => async (action) => {
                ...
                if (action.type === "AUDIO_SET_INPUT_DEVICE" &&
...
function patchDiscordComponents() {
    try {
        ...
        if (AudioDeviceModule) {
            ...
            if (AudioDeviceModule.getInputDevices &&
...
function createGlobalFunction() {
    try {
        ...
        _patchRestores.push(() => { delete window.getInputDevicesWithVirtual; });
        window.getInputDevicesWithVirtual = () => {
...
async function createVirtualInputDevice() {
    try {
        ...
        if (window.navigator && window.navigator.mediaDevices && !navigator.mediaDevices.getUserMedia._audioCenterPatched) {
            ...
            navigator.mediaDevices.getUserMedia._audioCenterPatched = true;
        }
...
async function createAudioMixer(primaryDeviceId, secondaryDeviceId) {
    try {
...
async function runFullDiagnostic() {
    ...
    try {
        ...
        console.log("- setSinkId support (HTMLAudioElement):", "setSinkId" in HTMLAudioElement.prototype);
        console.log("- setSinkId support (AudioContext):", "setSinkId" in AudioContext.prototype);
        console.log("- Virtual device injected:", navigator.mediaDevices?.enumerateDevices
            ?.toString()
...
function PrimaryDeviceSelector() {
    ...
    return (<Select options={devices.map((device) => ({
            value: device.id,
            label: `🎤 ${device.name}`,
        }))} serialize={identity} isSelected={value => value === selectedPrimaryDevice} select={id => {
...
function SecondaryDeviceSelector() {
    ...
    return (<Select options={devices.map((device) => ({
            value: device.id,
            label: `🎵 ${device.name}`,
        }))} serialize={identity} isSelected={value => value === selectedSecondaryDevice} select={id => {
...
export default definePlugin({
    name: "AudioCenter",
    description: "Complete audio center: mixing, virtual device, limiting and diagnostics",
    tags: ["Voice", "Utility"],
    authors: [TestcordDevs.x2b],
    settings,
    settingsAboutComponent: () => (<div>
            <h3>AudioCenter</h3>
            <p>
    ...
    settingsPanel: () => (<div style={{ padding: "20px" }}>
            ...
            <div style={{ marginTop: "20px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    <Button onClick={startAudioMixing} disabled={mixerState.isActive} style={{
            padding: "8px 16px",
            ...
            cursor: mixerState.isActive ? "not-allowed" : "pointer",
                    ...
                    <Button onClick={stopAudioMixing} disabled={!mixerState.isActive} style={{
            padding: "8px 16px",
            ...
            <div style={{ marginTop: "20px" }}>
                ...
                <Button onClick={runFullDiagnostic} style={{
            padding: "10px 20px",
