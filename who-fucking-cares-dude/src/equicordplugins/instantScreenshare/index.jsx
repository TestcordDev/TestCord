...
async function autoStartStream(instant = true) {
    if (!instant && !WindowStore.isFocused() && settings.store.focusDiscord)
        return;
    ...
    if (channel.type === 13 || isGuildChannel && !PermissionStore.can(PermissionsBits.STREAM, channel))
        return;
...
export default definePlugin({
    ...
    flux: {
        async VOICE_STATE_UPDATES({ voiceStates }) {
            if (!settings.store.toolboxManagement || !settings.store.instantScreenshare)
                return;
