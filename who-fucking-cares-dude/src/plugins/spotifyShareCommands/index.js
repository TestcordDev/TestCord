...
*/
import { findOption, OptionalMessageOption, sendBotMessage } from "@api/Commands";
...
function makeCommand(name, formatUrl) {
    return {
        name,
        ...
        execute(options, { channel }) {
            ...
            sendMessage(channel.id, { content: message ? `${message} ${data}` : data }, false, MessageActions.getSendMessageOptionsForReply(PendingReplyStore.getPendingReply(channel.id))).then(() => {
