...
function RolesAndUsersPermissionsComponent({ permissions, guild, modalProps, header }) {
    ...
    useEffect(() => {
        const usersToRequest = permissions
            .filter(p => p.type === 1 /* PermissionOverwriteType.MEMBER */ && !GuildMemberStore.isMember(guild.id, p.id))
            .map(({ id }) => id);
    ...
    return (<Modal {...modalProps} size="xl" title={`${header} Permissions`}>
            ...
            {selectedItem && (<div className={cl("modal-container")}>
                ...
                return (<div key={index} className={cl("modal-list-item-btn")} onClick={() => selectItem(index)} role="button" tabIndex={0}>
                        ...
                        if (permission.type === 0 /* PermissionOverwriteType.ROLE */)
                            ContextMenuApi.openContextMenu(e, () => (<RoleContextMenu guild={guild} roleId={permission.id} onClose={modalProps.onClose}/>));
                        else if (permission.type === 1 /* PermissionOverwriteType.MEMBER */) {
                            ContextMenuApi.openContextMenu(e, () => (<UserContextMenu userId={permission.id}/>));
                        }
                    }}>
                                        {(permission.type === 0 /* PermissionOverwriteType.ROLE */ || permission.type === 2 /* PermissionOverwriteType.OWNER */) && (<span className={cl("modal-role-circle")} style={{ backgroundColor: role?.colorString ?? "var(--primary-300)" }}/>)}
                                        {permission.type === 0 /* PermissionOverwriteType.ROLE */ && roleIconSrc != null && (<img className={cl("modal-role-image")} src={roleIconSrc}/>)}
                                        {permission.type === 1 /* PermissionOverwriteType.MEMBER */ && user != null && (<img className={cl("modal-user-img")} src={user.getAvatarURL(void 0, void 0, false)}/>)}
                                        <Text variant="text-md/normal" className={cl("modal-list-item-text")}>
                                            {permission.type === 0 /* PermissionOverwriteType.ROLE */
                        ? role?.name ?? "Unknown Role"
                        : permission.type === 1 /* PermissionOverwriteType.MEMBER */
                ...
                if (overrideType === "default")
                    return null;
                return (<div key={bit} className={cl("modal-perms-item")}>
...
function RoleContextMenu({ guild, roleId, onClose }) {
    ...
    return (<Menu.Menu navId={cl("role-context-menu")} onClose={ContextMenuApi.closeContextMenu} aria-label="Role Options">
            ...
            <Menu.MenuItem id={cl("copy-role-id")} label={getIntlMessage("COPY_ID_ROLE")} icon={CopyIdIcon} action={() => copyToClipboard(roleId)}/>

            {after}

            {settings.store.unsafeViewAsRole && (<Menu.MenuItem id={cl("view-as-role")} label={getIntlMessage("VIEW_AS_ROLE")} icon={ViewAsRoleIcon} action={() => {
                onClose();
                FluxDispatcher.dispatch({
...
export default function openRolesAndUsersPermissionsModal(permissions, guild, header) {
    return openModalLazy(async () => {
        ...
        return modalProps => (<RolesAndUsersPermissions modalProps={modalProps} permissions={permissions} guild={guild} header={header}/>);
    });
}
