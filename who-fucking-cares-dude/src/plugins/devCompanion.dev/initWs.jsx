...
export function initWs(isManual = false) {
    ...
    ws.addEventListener("message", e => {
        ...
        function reply(error) {
            const toSend = { nonce: d.nonce, ok: !error };
            if (error)
                toSend.error = error;
        ...
        switch (d.type) {
            case "disable": {
                ...
                if (!settings)
                    throw new Error("Plugin not found: " + m.pluginName);
...
const AllModulesNoti = ErrorBoundary.wrap(function ({ done, close }) {
