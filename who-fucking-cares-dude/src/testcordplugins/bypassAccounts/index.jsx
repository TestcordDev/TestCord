...
const Flex = ({ children, style, ...props }) => (<div style={{ display: "flex", ...style }} {...props}>{children}</div>);
...
class PasswordGenerator {
    ...
    static generatePassword(config) {
        ...
        if (!charset)
            return ""; // Return empty string when no charset selected
        let password = "";
...
function PasswordConfigModal(modalProps) {
    const [config, setConfig] = React.useState({ ...DEFAULT_PASSWORD_CONFIG });
            ...
            <ModalContent>
                <Flex style={{ gap: "15px", flexDirection: "column" }}>
                    ...
                    <Flex style={{ gap: "15px" }}>
                        ...
                        {config.specialChars && (<div style={{ flex: 1 }}>
...
class FileTokenEncryption {
    ...
    static async deriveKeyFromPassphrase(passphrase, salt) {
    ...
    static async encryptAccountData(accountData, passphrase) {
        const encoder = new TextEncoder();
        ...
        const encrypted = await crypto.subtle.encrypt({ name: this.ALGORITHM, iv }, key, encoder.encode(dataString));
    ...
    static async decryptAccountData(encryptedData, passphrase) {
...
class FileAccountManager {
    ...
    static currentPassphrase = null;
    static async loadAccountsFromFile(file, passphrase) {
        try {
    ...
    static async saveAccountsToFile() {
    ...
    static addAccount(id, username, token) {
...
function LoadAccountsModal(modalProps) {
...
function AddAccountModal(modalProps) {
...
function CreateAccountsFileModal(modalProps) {
    const [passphrase, setPassphrase] = React.useState("");
    ...
    const openPasswordGenerator = () => {
        openModal(props => (<PasswordConfigModal {...props} onGenerate={password => {
...
function AccountSwitcherModal(modalProps) {
    ...
    const openLoadModal = () => {
        modalProps.onClose();
        openModal(props => (<LoadAccountsModal {...props} onAccountsLoaded={() => {
                setTimeout(() => openModal(newProps => <AccountSwitcherModal {...newProps}/>), 100);
            }}/>));
    };
    const openAddModal = () => {
        modalProps.onClose();
        openModal(props => (<AddAccountModal {...props} onAccountAdded={() => {
                setTimeout(() => openModal(newProps => <AccountSwitcherModal {...newProps}/>), 100);
            }}/>));
    };
    const openCreateModal = () => {
        modalProps.onClose();
        openModal(props => (<CreateAccountsFileModal {...props} onFileCreated={() => {
                setTimeout(() => openModal(newProps => <AccountSwitcherModal {...newProps}/>), 100);
            }}/>));
    };
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            <ModalContent>
                <Flex style={{ gap: "15px", flexDirection: "column" }}>
                    {!FileAccountManager.hasAccountsLoaded() ? (<Flex style={{ justifyContent: "center", padding: "20px", flexDirection: "column", alignItems: "center" }}>
                            <Forms.FormText>No accounts loaded</Forms.FormText>
                            <Forms.FormText {...{ type: Forms.FormText.Types?.DESCRIPTION }}>
                                Load an existing file or create a new one to get started
                            </Forms.FormText>
                        </Flex>) : (<Flex style={{ flexDirection: "column", gap: "8px" }}>
                            {accounts.map(account => (<Flex key={account.id} style={{
                    gap: "12px",
                    padding: "10px",
                    ...
                    justifyContent: "center"
                                    ...
                                    <Flex style={{ gap: "6px" }}>
                                        {!account.isActive && (<>
                                                ...
                                                {account.token !== "current" && (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={() => removeAccount(account.id)} disabled={isLoading}>
                                                        Remove
                                                    </Button>)}
                                            </>)}
                                    </Flex>
...
export default definePlugin({
    ...
    handleQuickSwitch(event) {
        if (event.altKey && event.key === "g") {
            ...
            openModal(modalProps => (<AccountSwitcherModal {...modalProps}/>));
        }
    }
});
