...
const { ModalStack, DraftManager } = proxyLazyWebpack(() => {
    const [ModalStack, DraftManager] = findBulk(filters.byProps("pushLazy", "popAll"), filters.byProps("clearDraft", "saveDraft"));
    return {
        ModalStack,
        DraftManager
    };
...
let _crashLogsCache = null;
let _crashLogsRaw = null;
function getCrashLogs() {
    const raw = settings.store.crashLogs || "[]";
    if (raw === _crashLogsRaw && _crashLogsCache)
...
let _crashStatsCache = null;
let _crashStatsRaw = null;
function getCrashStatistics() {
    const raw = settings.store.crashStats || "{}";
    if (raw === _crashStatsRaw && _crashStatsCache)
...
function saveCrashStatistics(stats) {
    try {
...
function updateCrashStatistics(crashReport) {
    if (!settings.store.enableStatistics)
        return;
    try {
...
function logCrash(errorState, recoveryAttempted, recoverySuccessful) {
    if (!settings.store.enableCrashLogging)
        return;
    try {
        const crashReport = {
            id: generateCrashId(),
...
function checkMemoryUsage() {
    ...
    if (memory.usedMB > threshold) {
        ...
        if (settings.store.forceGarbageCollection && typeof window.gc === "function") {
            try {
                window.gc();
...
function startPerformanceMonitoring() {
    if (!settings.store.monitorPerformance || performanceMonitorInterval)
        return;
...
function isDiscordHookError(errorState) {
    let msg;
...
export default definePlugin({
    ...
    handlePreventCrash(_this, errorState) {
