...
const realContextBuffers = new Map();
function isMentioningUser(message, userId) {
...
function getServerTrigger(message, currentUserId) {
    const now = Date.now();
    const thread = serverThreads.get(message.channel_id);
    const referenced = getReferencedMessage(message);
    if (isMentioningUser(message, currentUserId))
        return "mention";
    if (referenced?.author?.id === currentUserId)
        return "reply";
    if (thread && thread.userId === message.author.id && now - thread.lastActivity < SERVER_THREAD_TTL)
        return "followup";
    return undefined;
}
...
async function respondToMessage(message, currentUser, isDm, serverTrigger, realContextMessages) {
    try {
...
async function handleMessage(message) {
    ...
    const blacklistedUsers = settings.store.blacklistedUsers?.split(",").map((id) => id.trim()) || [];
...
const messageCreateListener = (data) => {
...
const KeyboardIcon = (props) => (<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
...
const AutoResponderButton = () => {
    ...
    if (settings.store.location !== "chatbar")
        return null;
    const toggle = async () => {
    ...
    return (<ChatBarButton tooltip={`AutoResponder: ${isEnabled ? "ON" : "OFF"}`} onClick={toggle}>
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("AutoResponder", () => (<HeaderBarButton icon={() => <KeyboardIcon enabled={settings.store.isActive}/>} tooltip={`AutoResponder: ${settings.store.isActive ? "ON" : "OFF"}`} onClick={() => { settings.store.isActive = !settings.store.isActive; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("AutoResponder", () => (<ChannelToolbarButton icon={() => <KeyboardIcon enabled={settings.store.isActive}/>} tooltip={`AutoResponder: ${settings.store.isActive ? "ON" : "OFF"}`} onClick={() => { settings.store.isActive = !settings.store.isActive; }}/>), 5);
        }
    },
    stop() {
        ...
        for (const { timeout } of realContextBuffers.values())
            clearTimeout(timeout);
