 ...
 */
export function completeAndDispatch(uploadId) {
    const before = { active: activeUploads.size, completed: completedFiles, total: totalFiles };
    // Check if already removed (prevent double completion)
    if (!activeUploads.has(uploadId)) {
...
export function getIsDispatched() {
    return isDispatched;
}
...
export function getForceHide() {
    return forceHide;
}
// Set force hide immediately (for error handling)
export function setForceHide(value) {
...
export function clearAndForceHide() {
...
export function startUploadBatch(fileCount) {
...
export function switchToUpload(uploadId) {
    if (activeUploads.has(uploadId)) {
        currentUploadId = uploadId;
    }
}
// Cancel an upload
export async function cancelUploadTracking(uploadId) {
