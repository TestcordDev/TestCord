...
export function removePatchesByPlugin(pluginName) {
...
export const getBuildNumber = makeLazy(() => {
    try {
        function matchBuildNumber(factoryStr) {
...
export function getFactoryPatchedSource(moduleId, webpackRequire = wreq) {
    return webpackRequire.m[moduleId]?.[SYM_PATCHED_SOURCE];
}
export function getFactoryPatchedBy(moduleId, webpackRequire = wreq) {
...
const define = (target, p, attributes) => {
...
const moduleFactoryHandler = {
    apply(target, thisArg, argArray) {
        ...
        if (target[SYM_ORIGINAL_FACTORY] != null) {
            return runFactoryWithWrap(target, thisArg, argArray);
        }
...
function proxyFactoryAndUpdateExisting(moduleFactories, moduleId, newFactory, receiver, ignoreExistingInTarget = false) {
 ...
 */
function updateExistingFactory(moduleFactories, moduleId, newFactory, newProxiedFactory, ignoreExistingInTarget) {
    let existingFactory;
 ...
 */
function defineInWebpackInstances(moduleId, factory) {
    for (const wreq of allWebpackInstances) {
 ...
 */
function notifyFactoryListeners(moduleId, factory) {
    if (factoryListeners.size === 0)
        return;
 ...
 */
function runFactoryWithWrap(patchedFactory, thisArg, argArray) {
    ...
    if (wreq == null) {
        if (!wreqFallbackApplied) {
            ...
            if (typeof require === "function" && require.m != null && require.c != null) {
                ...
                logger.warn("WebpackRequire was not initialized, falling back to WebpackRequire passed to the first called wrapped module factory (" +
    ...
    for (const [filter, callback] of waitForSubscriptions) {
        try {
            ...
            logger.error("Error while filtering or firing callback for Webpack waitFor subscription:\n", err, "\n\nModule exports:", exports, "\n\nFilter:", filter, "\n\nCallback:", callback);
        }
...
function patchFactory(moduleId, originalFactory) {
    ...
    for (let i = 0; i < patches.length; i++) {
        ...
        const executePatch = traceFunctionWithResults(`patch by ${patch.plugin}`, (match, replace) => {
        ...
        for (const replacement of patch.replacement) {
            ...
            let newPatchedCode = "";
            try {
                const [patchResult, totalTime] = executePatch(replacement.match, replacement.replace);
        ...
        if (patchedBy.has(patch.plugin)) {
            ...
            PatchVersioning.checkAndStore(patch.plugin, String(patch.find), originalFactoryCode, shouldCheckBuildNumber ? buildNumber : -1, String(moduleId));
        }
    }
    if (patchedFactory !== originalFactory) {
        patchedModuleIds.add(moduleId);
...
function diffErroredPatch(code, lastCode, match) {
    ...
    const diff = require("diff").diffWordsWithSpace(context, patchedContext);
    let fmt = "%c %s ";
    const elements = [];
    for (const d of diff) {
...
function getPatchContext(code, find) {
