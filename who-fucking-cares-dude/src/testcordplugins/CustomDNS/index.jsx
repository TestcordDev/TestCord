...
const Native = VencordNative.pluginHelpers.CustomDNS;
var DnsProvider;
(function (DnsProvider) {
    DnsProvider["DNS_SB"] = "dns_sb";
    DnsProvider["QUAD9"] = "quad9";
    DnsProvider["MULLVAD"] = "mullvad";
    DnsProvider["CUSTOM"] = "custom";
})(DnsProvider || (DnsProvider = {}));
var MullvadProfile;
(function (MullvadProfile) {
    MullvadProfile["DNS"] = "dns";
    MullvadProfile["ADBLOCK"] = "adblock";
    MullvadProfile["BASE"] = "base";
    MullvadProfile["EXTENDED"] = "extended";
    MullvadProfile["FAMILY"] = "family";
    MullvadProfile["ALL"] = "all";
})(MullvadProfile || (MullvadProfile = {}));
var LogLevel;
(function (LogLevel) {
...
function isValidIpAddress(value, family) {
    const trimmed = value.trim();
    if (!trimmed)
        return true;
    if (family === 4)
        return IPV4_ADDRESS.test(trimmed);
    return trimmed.includes(":") && IPV6_ADDRESS.test(trimmed);
}
function validateIpAddress(value, family) {
...
const settings = definePluginSettings({
    ...
    customDNSv4Primary: {
        ...
        isValid: (value) => validateIpAddress(value, 4)
    },
    ...
    customDNSv6Primary: {
        ...
        isValid: (value) => validateIpAddress(value, 6)
    },
...
function showPluginToast(message, type = Toasts.Type.MESSAGE) {
...
function getConfiguredLines(value, fallback) {
    const lines = (value ?? "")
        .split("\n")
...
function hostnameMatches(hostname) {
    ...
    return getTrackedDomains().some(domain => normalizedHostname === domain || normalizedHostname.endsWith(`.${domain}`));
}
...
function getCustomServers(family) {
...
function createFetchInput(input, url) {
...
function rewriteUrl(url, address) {
...
export default definePlugin({
    ...
    start() {
        ...
        async function resolveHostname(hostname, shouldWarn = true) {
        ...
        const CustomDNS = {
            ...
            async start() {
                ...
                try {
                    void VencordNative?.privacy?.setDnsEnabled?.(true)?.catch?.(() => { });
                }
                catch { }
            ...
            stop() {
                ...
                isActive = false;
                try {
                    void VencordNative?.privacy?.setDnsEnabled?.(false)?.catch?.(() => { });
                }
                catch { }
