...
const userJoinCounts = new Map();
const userTimeouts = new Map();
const bannedUsers = new Set();
let banQueue = Promise.resolve();
let lastUserChannelId = null;
...
async function executeBan(channelId, userId) {
    ...
    banQueue = banOperation.then(() => sleep(2000));
    await banOperation;
}
...
function handleUserVoiceActivity(userId, oldChannelId) {
    if (bannedUsers.has(userId))
        return;
    const currentCount = userJoinCounts.get(userId) || 0;
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            voiceStates.forEach(state => {
                ...
                if (!targetChannelId || oldChannelId !== targetChannelId)
                    return;
