...
function SwitchSetting({ name, description, settingsKey }) {
    const [value, setValue] = useState(settings.store[settingsKey] ?? false);
    return (<SettingsSection tag="label" inlineSetting id={name} name={name} description={description}>
            <Switch checked={Boolean(value)} onChange={v => { setValue(v); settings.store[settingsKey] = v; }}/>
        </SettingsSection>);
}
function TextSetting({ name, description, settingsKey, placeholder }) {
    const [value, setValue] = useState(settings.store[settingsKey] ?? "");
    return (<SettingsSection id={name} name={name} description={description}>
            <TextInput type="text" value={String(value)} onChange={v => { setValue(v); settings.store[settingsKey] = v; }} placeholder={placeholder ?? "Enter a value"}/>
        </SettingsSection>);
}
function SelectSetting({ name, description, settingsKey, options }) {
    const [value, setValue] = useState(settings.store[settingsKey] ?? options[0]?.value);
    return (<SettingsSection id={name} name={name} description={description}>
            <Select options={options} isSelected={v => v === value} select={v => { setValue(v); settings.store[settingsKey] = v; }} serialize={String} closeOnSelect maxVisibleItems={5}/>
        </SettingsSection>);
}
...
function TosuSettings() {
    return (<SettingsSection id="tosu-settings" name="" description="Connects to tosu via WebSocket on port 24050. No configuration needed, just make sure tosu is running alongside osu!."/>);
}
const nameFormatOptions = [
    { label: "Use custom status name", value: "status-name" /* NameFormat.StatusName */ },
...
function GensokyoRadioSettings() {
    ...
    return (<>
            ...
            <SettingsSection id="refresh-interval" name="Refresh Interval" description="Refresh interval in seconds.">
                <Slider markers={[1, 2, 2.5, 3, 5, 10, 15]} initialValue={value} onValueChange={v => { setValue(v); settings.store.gr_refreshInterval = v; }} onValueRender={v => `${v}s`} stickToMarkers/>
            </SettingsSection>
        </>);
}
const ND_DYNAMIC_KEYS = ["nd_activityType", "nd_albumArtMode"];
function NavidromeSettings() {
    ...
    return (<>
            ...
            {nd_albumArtMode === "lastfm" && (<TextSetting name="Last.fm API Key" description="Optional: Provide your own Last.fm API Key (leaves default if empty)." settingsKey="nd_lastfmApiKey" placeholder="feff915bf5987580c9dc354d523dc6b9"/>)}
            ...
            {Number(nd_activityType ?? 2) !== 0 && (<TextSetting name="Activity Name Format" description="The 'Listening to' text (e.g. {artist})" settingsKey="nd_nameString" placeholder="Navidrome"/>)}
            ...
            <SettingsSection id="nd-refresh-interval" name="Refresh Interval" description="Refresh interval in seconds.">
                <Slider markers={[1, 2, 5, 10, 15]} initialValue={refreshInterval} onValueChange={v => { setRefreshInterval(v); settings.store.nd_refreshInterval = v; }} onValueRender={v => `${v}s`} stickToMarkers/>
            </SettingsSection>
        </>);
}
...
export function SettingsPanel() {
    ...
    return (<div className={classes("vc-plugins-settings", Margins.top16)}>
            <TabBar type="top" look="brand" selectedItem={currentTab} onItemSelect={setCurrentTab}>
                {TABS.map(tab => (<TabBar.Item key={tab} id={tab}>
                        {TAB_LABELS[tab]}
                    </TabBar.Item>))}
            </TabBar>
