...
export function ProfilePopoutComponent({ id, isSideBar = false }) {
    ...
    const openReposModal = () => {
        if (!userInfo)
            return;
        openModal(props => (<ReposModal groups={groups.length ? groups : [{ key: PERSONAL_GROUP_KEY, label: userInfo?.username ?? "Personal", avatarUrl: userInfo?.avatarUrl, repos }]} initialActiveKey={PERSONAL_GROUP_KEY} username={userInfo.username} rootProps={props}/>));
    };
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (!personalRepos)
                    personalRepos = await fetchReposByUsername(username);
    ...
    if (loading || error || !repos.length)
        return null;
    const topRepos = repos.slice(0, 4);
    const reposSection = (<section className={ProfileCardClasses.container}>
                ...
                <li className={ProfileCardClasses.firstCardContainer}>
                    <Clickable className={ProfileCardContainerClasses.breadcrumb} onClick={openReposModal}>
                        <div className={classes(ProfileCardOverlayClasses.overlay, ProfileCardContainerClasses.innerContainer, ProfileCardClasses.card)}>
                            ...
                            {!!repos.length && (<div className={ProfileCardContainerClasses.icons}>
                                    {topRepos.slice(0, 4).map((repo, idx) => {
                const showCount = idx === 3 && repos.length > 4;
                return (<div className={ProfileCardContainerClasses.icon} key={repo.id}>
                                                <img src={getLanguageIconUrl(repo.language)} alt={repo.language ?? "Unknown"} className={showCount ? ProfileCardContainerClasses.displayCount : undefined}/>
