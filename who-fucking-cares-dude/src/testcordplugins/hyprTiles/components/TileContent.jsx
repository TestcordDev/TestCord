...
import { ChannelStore, GuildStore, MessageActions, MessageStore, React, useEffect, useState, useStateFromStores, } from "@webpack/common";
...
// or text input.
const ChatMessages = findComponentByCodeLazy("forceCompact", "forceCozy");
const requireForumView = extractAndLoadChunksLazy(["Missing channel in Channel.renderHeaderToolbar"], new RegExp(DefaultExtractAndLoadChunksRegex.source + '.{1,150}name:"ForumChannel"'));
function TileContentComponent({ tile, active = false }) {
    ...
    const liveMessageToken = useStateFromStores([MessageStore], () => {
        if (!channel || channel.isForumLikeChannel())
            return "";
    ...
    if (!channel) {
        return (<div className={cl("tile-empty")}>
                Channel unavailable
            </div>);
    }
    if (channel.isForumLikeChannel()) {
        if (ForumView && forumReady) {
            return (<div className={cl("tile-content-inner")}>
    ...
    return (<div className={cl("tile-content-inner")} data-active={active} data-live-token={liveMessageToken}>
