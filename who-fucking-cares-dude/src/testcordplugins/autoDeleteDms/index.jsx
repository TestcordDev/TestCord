...
export default definePlugin({
    ...
    getDeleteTime(userId) {
        ...
        return this.convertToMilliseconds(settings.store.deleteAfterValue, settings.store.timeUnit);
    },
    ...
    handleNewMessage(event) {
        ...
        try {
            ...
            console.log("[AutoDeleteDMs] Added to queue. Current queue:", Array.from(this.messageTimestamps.entries()));
        }
        catch (err) {
    ...
    async processQueue() {
        if (this.isProcessingQueue || this.deletionQueue.length === 0)
            return;
    ...
    async safeGetMessage(messageId, channelId) {
        try {
    ...
    async safeDeleteMessage(channelId, messageId, retryCount = 0) {
        try {
    ...
    handleDeletionError(error, item) {
        const discordError = error;
        if (discordError?.status === 404) {
            ...
            this.retryDelay = Math.min(Math.max(retryAfter * 1000, this.retryDelay * Math.pow(2, this.rateLimitHits)), this.maxRetryDelay);
