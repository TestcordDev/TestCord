...
export const shikiRepoTheme = (name) => `https://cdn.jsdelivr.net/gh/${SHIKI_REPO}@${SHIKI_REPO_COMMIT}/packages/tm-themes/themes/${name}.json`;
export const themes = {
    // Default
...
export const getTheme = (url) => {
    if (themeCache.has(url))
        return Promise.resolve(themeCache.get(url));
