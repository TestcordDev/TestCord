...
import { ContainerClasses, DMSideBarClasses, OverlayClasses, ProfileCardClasses, Spinner, } from "@equicordplugins/songSpotlight.desktop/ui/common";
import { sid } from "@song-spotlight/api/util";
import { classes } from "@utils/index";
import { SelectedChannelStore, SelectedGuildStore, useEffect, useMemo, UserProfileActions, UserStore, useState, } from "@webpack/common";
const shownSongs = 4;
export default function CollapsedProfileSongs({ data, user, isSideBar }) {
    ...
    const songsSection = (<section className={ProfileCardClasses.container}>
            <ul className={ProfileCardClasses.cardsList} tabIndex={-1}>
                <li className={ProfileCardClasses.firstCardContainer}>
                    <div className={ContainerClasses.breadcrumb} aria-label="Song Spotlight" role="button" tabIndex={0} onClick={() => {
            ...
            ? previews.map((song, i) => {
                ...
                return (<div className={ContainerClasses.icon} key={i}>
                                                {render?.thumbnailUrl && (<img src={render.thumbnailUrl} alt={render.label} className={classes(extra && ContainerClasses.displayCount)}/>)}
                                                {extra && (<div className={ContainerClasses.displayCountText}>
