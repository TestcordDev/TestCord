...
import { Avatar, Button, React, RelationshipStore, RestAPI, TextInput, Toasts, useCallback, useEffect, useMemo, useRef, UserProfileStore, UserStore, UserUtils, useState } from "@webpack/common";
...
export function NoteSearchTab() {
    ...
    const updatePillPosition = useCallback(() => {
        if (!filtersContainerRef.current)
            return;
        const activeBtn = filtersContainerRef.current.querySelector(".vc-note-search-filter-btn.active");
        if (activeBtn) {
            setPillStyle({
        ...
        try {
            ...
            if (data) {
                ...
                for (const [id, val] of entriesArr) {
                    const text = typeof val === "string" ? val : (val && typeof val.note === "string" ? val.note : "");
        ...
        try {
            ...
            if (store) {
                ...
                if (typeof store.getNote === "function") {
                    ...
                    const usersObj = UserStore._users ?? {};
            ...
            for (const userId of notes.keys()) {
                ...
                if (storeUser) {
                    newCache.set(userId, {
                        ...
                        globalName: storeUser.globalName,
                    });
                    updated = true;
                }
                else {
    ...
    const handleDeleteNote = async (userId) => {
    ...
    const filteredNotesList = useMemo(() => {
        ...
        for (const [userId, note] of notes.entries()) {
            ...
            if (filterStatus === "FRIENDS" && !isFriend)
                continue;
            ...
            const globalName = user?.globalName ?? cached?.globalName ?? "";
    ...
    return (<div className="vc-note-search-tab">
            <div className="vc-note-search-header">
                <div className="vc-note-search-title-row">
                    ...
                    <Button className="vc-note-btn" size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} onClick={loadNotes}>
                        Refresh Notes
                    </Button>
                </div>
                ...
                <div className="vc-note-search-controls">
                    <div className="vc-note-search-input-wrapper">
                        <TextInput value={query} onChange={setQuery} placeholder="Search by note content, username, global name, or user ID..."/>
                    </div>
                    <div className="vc-note-search-filters" ref={filtersContainerRef}>
                        <div className="vc-note-search-slider-pill" style={pillStyle}/>
                        <button type="button" className={`vc-note-search-filter-btn ${filterStatus === "ALL" ? "active" : ""}`} onClick={() => setFilterStatus("ALL")}>
                            All ({notes.size})
                        </button>
                        <button type="button" className={`vc-note-search-filter-btn ${filterStatus === "FRIENDS" ? "active" : ""}`} onClick={() => setFilterStatus("FRIENDS")}>
                            Friends
                        </button>
                        <button type="button" className={`vc-note-search-filter-btn ${filterStatus === "BLOCKED" ? "active" : ""}`} onClick={() => setFilterStatus("BLOCKED")}>
                            Blocked
                        </button>
                    </div>
                </div>
            ...
            {loading ? (<div className="vc-note-empty-state">
                    <h3>Loading user notes...</h3>
                </div>) : filteredNotesList.length === 0 ? (<div className="vc-note-empty-state">
                    <h3>No notes found</h3>
                    <span>{query ? "Try matching a different keyword or user ID." : "You haven't saved any user notes yet."}</span>
                </div>) : (<div className="vc-note-grid">
                ...
                const displayName = user ? (user.globalName || user.username) : (cachedProfile?.globalName || cachedProfile?.username || "Unknown User");
                const handleName = user ? `@${user.username}` : (cachedProfile?.username ? `@${cachedProfile.username}` : userId);
                return (<div key={userId} className="vc-note-card">
                                <div className="vc-note-card-header">
                                    <div className="vc-note-card-user-info">
                                        <Avatar src={user && typeof user.getAvatarURL === "function" ? user.getAvatarURL(null, 40) : undefined} size="SIZE_40"/>
                                    ...
                                    <div className="vc-note-card-actions">
                                        {isDirty && (<Button className="vc-note-btn" size={Button.Sizes.SMALL} color={Button.Colors.GREEN} onClick={() => handleSaveNote(userId)}>
                                                Save
                                            </Button>)}
                                        <Button className="vc-note-btn" size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} onClick={() => openUserProfile(userId)}>
                                            Profile
                                        </Button>
                                        <Button className="vc-note-btn" size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} onClick={() => copyWithToast(currentText, "Note text copied to clipboard")}>
                                            Copy
                                        </Button>
                                        <Button className="vc-note-btn" size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={() => handleDeleteNote(userId)}>
                                            Delete
                                        </Button>
                                    </div>
                                </div>
                                <div className="vc-note-card-body">
                                    <textarea className="vc-note-textarea" value={currentText} onChange={e => setEditedNotes({ ...editedNotes, [userId]: e.target.value })} placeholder="Write a note..."/>
                                </div>
                            </div>);
            })}
                </div>)}
