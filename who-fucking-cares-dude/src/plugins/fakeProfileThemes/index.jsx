...
function encode(primary, accent) {
...
function decode(bio) {
    if (bio == null)
        return null;
    const colorString = bio.match(/\u{e005b}\u{e0023}([\u{e0061}-\u{e0066}\u{e0041}-\u{e0046}\u{e0030}-\u{e0039}]{1,6})\u{e002c}\u{e0023}([\u{e0061}-\u{e0066}\u{e0041}-\u{e0046}\u{e0030}-\u{e0039}]{1,6})\u{e005d}/u);
    if (colorString != null) {
...
const ProfileModal = findComponentByCodeLazy("isTryItOut:", "pendingThemeColors:", "pendingAvatarDecoration:", "EDIT_PROFILE_BANNER");
...
function SettingsAboutComponent() {
    ...
    return (<section>
            ...
            <Paragraph className={Margins.top8}>
                ...
                <Flex gap="1em">
                    <ColorPicker color={color1} label={<BaseText size="xs" style={{ marginTop: "4px" }}>
                                Primary
                            </BaseText>} onChange={(color) => {
            setColor1(color);
        }}/>
                    <ColorPicker color={color2} label={<BaseText size="xs" style={{ marginTop: "4px" }}>
                                Accent
                            </BaseText>} onChange={(color) => {
            setColor2(color);
        }}/>
            ...
            copyWithToast(colorString);
        }} color={Button.Colors.PRIMARY} size={Button.Sizes.XLARGE} style={{ marginBottom: "auto" }}>
                        Copy 3y3
                    </Button>
                </Flex>
                <Divider className={classes(Margins.top8, Margins.bottom8)}/>
                ...
                <div className="vc-fpt-preview">
                    <ProfileModal user={UserStore.getCurrentUser()} pendingThemeColors={[color1, color2]} onAvatarChange={() => { }} onBannerChange={() => { }} canUsePremiumCustomization={true} hideExampleButton={true} hideFakeActivity={true} isTryItOut={true}/>
                </div>
            </Paragraph>
        </section>);
}
export default definePlugin({
    ...
    colorDecodeHook(user) {
        if (user?.bio) {
            ...
            if (settings.store.nitroFirst && user.themeColors)
                return user;
    ...
    addCopy3y3Button: ErrorBoundary.wrap(function ({ primary, accent }) {
        return <Button onClick={() => {
                ...
                copyWithToast(colorString);
            }} color={Button.Colors.PRIMARY} size={Button.Sizes.XLARGE}>
            Copy 3y3
        </Button>;
    }, { noop: true }),
});
