...
export const EmbedContext = proxyLazyWebpack(() => React.createContext(null));
export const EmbedMosaicContext = proxyLazyWebpack(() => React.createContext(null));
const AttachmentContext = proxyLazyWebpack(() => React.createContext(null));
const ManaSearchBar = findComponentByCodeLazy("#{intl::SEARCH}),ref");
const FavoriteButton = findComponentByCodeLazy("#{intl::GIF_TOOLTIP_ADD_TO_FAVORITES}");
...
function createPreviewMessage(attachment, channelId) {
    ...
    return createMessageRecord(previewMessage);
}
export const AttachmentPreview = proxyLazyWebpack(() => {
    ...
    const MessageComponent = findComponentByCode("this.renderAttachments");
    class MessageAttachmentsComponent extends MessageComponent {
        render() {
            return this.renderAttachments(this.props.message);
        }
    }
    const channel = Object.freeze(createChannelRecordFromServer({ id: "0", type: 0 /* ChannelType.GUILD_TEXT */ }));
    return function AttachmentPreview({ attachment }) {
        const message = useMemo(() => createPreviewMessage(attachment, channel.id), [attachment, channel.id]);
        return (<MessageAttachmentsComponent channel={channel} message={message} canDeleteAttachments={false} shouldHideMediaOptions={false} inlineAttachmentMedia/>);
    };
});
const noopRender = () => null;
export function FilePicker({ onSelectItem }) {
    ...
    const handleSubmit = useCallback((url) => onSelectItem({ url }), []);
    const handleChange = useCallback((query) => ExpressionPickerStore.setSearchQuery(query), []);
    const handleClear = useCallback(() => ExpressionPickerStore.setSearchQuery(""), []);
    const renderRow = useCallback(({ row }) => {
        const item = favs?.[row];
        if (!item)
            return null;
        return (<FilePickerItem key={item.url} url={item.url} file={item.data} channel={channel} reducePadding={row !== count - 1} onResize={handleResize} onSubmit={handleSubmit}/>);
    }, [favs, channel, count, handleResize, handleSubmit]);
    const rowHeight = useCallback((_, row) => (favs?.[row] && rowHeights.get(favs[row].url)) ?? 100, [favs, rowHeights]);
    useEffect(() => void listRef.current?.scrollToTop(), [query]);
    return (<div id="files-picker-tab-panel" role="tabpanel" aria-labelledby="files-picker-tab" className={cl("container")}>
            ...
            {count > 0 ? (<div className={cl("container-body")}>
                    <ListScrollerThin ref={listRef} sections={[count]} sectionHeight={0} rowHeight={rowHeight} renderSection={noopRender} renderRow={renderRow}/>
                </div>) : (<div className={cl("container-body", "container-info")} inert>
...
function Demo() {
    return (<>
            <div className={cl("attachment-container", "demo", "first")}>
                ...
                <FavoriteButton className={cl("demo-favourite-button")} url="https://example.org" src="https://example.org" width={100} height={100} format={FavouriteItemFormat.NONE}/>
            </div>
...
export function FilePickerItem({ url, file, channel, onResize, onSubmit, reducePadding }) {
    const [isFetching, setIsFetching] = useState(false);
    const ref = useRef(null);
    useResizeObserver(ref, ({ height }) => onResize(url, height), [onResize, url]);
    const attachment = useStateFromStores([SignedUrlsStore], () => ({ ...file, url: SignedUrlsStore.get(file.url), proxy_url: SignedUrlsStore.get(file.proxy_url) }), [file], lodash.isEqual);
    const { canAttachFiles, canSendMessages } = useStateFromStores([PermissionStore], () => ({
    ...
    return (<div ref={ref} className={cl("attachment-container", reducePadding && "reduced-padding")}>
            <AttachmentPreview attachment={attachment}/>
            {handleClick && (<Button onClick={handleClick} variant="secondary" disabled={isFetching}>
...
export function EmbedAccessory() {
    ...
    return (props && (<div className={cl("image-accessory")}>
                <FavoriteButton {...props} className={Classes.gifFavoriteButton}/>
            </div>));
}
export function AttachmentContextProvider({ attachment, component, children }) {
    const attachmentItem = useMemo(() => {
        ...
        if (attachment) {
            ...
            const raw = "media" in originalItem
                ? {
                    ...originalItem.media,
                    id: rest.uniqueId,
                    size: 0,
...
const visualMediaFormats = Object.freeze({
