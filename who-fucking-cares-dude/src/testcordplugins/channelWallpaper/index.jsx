...
function saveWallpaper(channelId, url, skipSync = false) {
...
function saveLocalWallpaperOnly(channelId, url) {
    const wp = getWallpapers();
    if (url)
        wp[channelId] = url;
    else
...
function getWallpaper(channelId) {
    const wp = getWallpapers();
    return wp[channelId] || settings.store.defaultWallpaper || "";
}
function hasWallpaper(channelId) {
    const wp = getWallpapers();
    return !!wp[channelId];
}
...
function applyWallpaper(channelId) {
...
async function setWallpaperFromFile(channelId) {
...
async function setWallpaperFromUrl(channelId) {
...
function removeWallpaper(channelId) {
...
function buildWallpaperMenu(channelId) {
    const has = hasWallpaper(channelId);
    const channel = ChannelStore.getChannel(channelId);
    const isDM = channel?.type === 1;
    return (<Menu.MenuItem id="channel-wallpaper" label="Wallpaper" icon={WallpaperIcon}>
            <Menu.MenuItem id="wallpaper-from-file" label="📁 From a file..." action={() => setWallpaperFromFile(channelId)}/>
            <Menu.MenuItem id="wallpaper-from-url" label="🔗 From a URL..." action={() => setWallpaperFromUrl(channelId)}/>
            {has && (<>
                    <Menu.MenuSeparator />
                    <Menu.MenuItem id="wallpaper-remove" label={isDM ? "🗑️ Delete for both" : "🗑️ Delete wallpaper"} color="danger" action={() => removeWallpaper(channelId)}/>
                </>)}
        </Menu.MenuItem>);
}
// ── VPS Sync logic ─────────────────────────────────────────────────────────────
let vpsSocket = null;
let vpsReconnectTimer = null;
let stopping = false;
...
function initVPSSync() {
    if (!settings.store.vpsUrl || stopping)
        return;
    try {
...
export default definePlugin({
    ...
    flux: {
        ...
        MESSAGE_CREATE(data) {
            if (!settings.store.syncWithFriends)
                return;
