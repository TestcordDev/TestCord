...
async function addToHeartGifs(gif) {
    try {
        const items = (await DataStore.get(HG_DATA_KEY)) ?? [];
        if (items.some((g) => g.url === gif.url))
            return false;
        const newItem = {
...
async function addToHeartGifsBatch(gifs) {
    if (gifs.length === 0)
        return 0;
    try {
        const items = (await DataStore.get(HG_DATA_KEY)) ?? [];
        const existingUrls = new Set(items.map((g) => g.url));
        let added = 0;
...
async function isInHeartGifs(url) {
    try {
        const items = (await DataStore.get(HG_DATA_KEY)) ?? [];
        return items.some((g) => g.url === url);
    }
    catch {
        return false;
    }
...
// ─── GIF Extraction ──────────────────────────────────────────────────────────
function extractGifsFromMessage(msg) {
    const gifs = [];
    ...
    for (const embed of msg.embeds ?? []) {
        ...
        const thumbUrl = embed.thumbnail?.url ?? embed.image?.url ?? "";
        if (embed.type === "gifv" || embedUrl.includes("tenor.com") || embedUrl.includes("giphy.com") || videoUrl.includes("tenor.com") || embedUrl.match(/\.gif/i)) {
            ...
            if (finalUrl)
                gifs.push({ url: finalUrl, src: videoUrl || thumbUrl || finalUrl, width: embed.video?.width ?? embed.thumbnail?.width ?? 498, height: embed.video?.height ?? embed.thumbnail?.height ?? 280, format: 2, order: 0 });
        }
    }
...
// ─── Search ───────────────────────────────────────────────────────────────────
async function searchMessages(guildId, userId, filter, maxResults, channelId, seen, label, onGifFound) {
    const gifs = [];
    let offset = 0;
    const pageSize = 25;
    let total = 0;
    do {
        ...
        if (filter.type === "content")
            url += "&content=" + encodeURIComponent(filter.value);
        else
            url += "&has=" + filter.value;
        if (channelId)
...
// ─── Main Scan ────────────────────────────────────────────────────────────────
async function scanUserGifs(userId, username, guildId) {
    if (isScanning) {
    ...
    const favUrls = !useHeartGifs ? getCurrentFavoriteUrls() : new Set();
    const savedRealtime = new Set();
    const heartGifsPending = [];
            ...
            : (addGif
                ? (g) => {
                    if (!favUrls.has(g.url) && !savedRealtime.has(g.url)) {
                        savedRealtime.add(g.url);
                        try {
                            addGif({ url: g.url, src: g.src ?? g.url, width: Number(g.width) || 498, height: Number(g.height) || 280, format: Number(g.format) || 2 });
                        }
                        catch { }
                    }
                }
    ...
    const allGifs = [];
    const searches = [
        { type: "content", value: "tenor.com", label: "tenor" },
        { type: "content", value: "giphy.com", label: "giphy" },
        { type: "content", value: "klipy.com", label: "klipy" },
        { type: "has", value: "file", label: "files" },
    ];
...
const userContextMenuPatch = (children, { user, guildId }) => {
    if (!user)
        return;
    children.push(<Menu.MenuSeparator key="save-user-gifs-separator"/>, <Menu.MenuItem key="save-user-gifs" id="save-user-gifs" label="Save GIFs from user" disabled={isScanning} action={() => scanUserGifs(user.id, user.username ?? user.globalName ?? user.id, guildId ?? null)}/>, ...(isScanning ? [
        <Menu.MenuItem key="save-user-gifs-stop" id="save-user-gifs-stop" label="⏹ Stop saving GIFs" action={() => {
                stopRequested = true;
                showToast("Stopping...", Toasts.Type.MESSAGE);
            }}/>,
        <Menu.MenuItem key="save-user-gifs-status" id="save-user-gifs-status" label={`📊 ${currentGifsFound} GIFs found so far`} action={() => showToast(`${currentGifsFound} GIFs found so far`, Toasts.Type.MESSAGE)}/>
    ] : []));
};
// ─── Plugin ───────────────────────────────────────────────────────────────────
export default definePlugin({
    ...
    settingsAboutComponent() {
        return (<div style={{ fontSize: "14px", lineHeight: "1.8" }}>
