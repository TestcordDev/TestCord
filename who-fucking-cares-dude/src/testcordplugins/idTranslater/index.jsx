...
function createIdRegex(minLength, maxLength) {
...
function isRoleId(id, channelId) {
    if (!channelId)
        return false;
    try {
...
function isIdInContext(content, id, index) {
...
function translateIds(content, channelId) {
    if (!content)
        return content;
    ...
    const processedIds = new Map(); // ID -> replacement
...
export default definePlugin({
    name: "ID Translater",
    description: "Automatically translates Discord IDs to clickable @ mentions or # references",
    tags: ["Chat", "Utility"],
    ...
    start() {
        console.log("[ID Translater] Plugin started - Automatic ID conversion enabled");
    },
    stop() {
