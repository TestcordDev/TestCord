...
export function canDeleteReview(profileId, review) {
    const myId = UserStore.getCurrentUser().id;
    return (myId === profileId
        || review.sender.discordID === myId
        || Auth.user?.type === 1 /* UserType.Admin */);
}
export function canBlockReviewAuthor(profileId, review) {
    const myId = UserStore.getCurrentUser().id;
    return profileId === myId && review.sender.discordID !== myId;
}
export function canReportReview(review) {
    return review.sender.discordID !== UserStore.getCurrentUser().id;
}
export function showToast(message, type = Toasts.Type.MESSAGE) {
    Toasts.show({
        id: Toasts.genId(),
        message,
        type,
