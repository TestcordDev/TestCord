...
const Native = VencordNative.pluginHelpers.PdfViewer;
...
function loadPdfjs() {
    ...
    return pdfjsPromise ??= new Promise((resolve, reject) => {
        ...
        s.onload = () => {
            if (!window.pdfjsLib)
                return reject(new Error("Failed to load PDF.js"));
...
async function loadBytes(att) {
    ...
    if (hit) {
        ...
        bytes = await Native.fetchPdf(att.url, maxBytes());
...
function IconBtn({ label, active, disabled, onClick, children }) {
    return (<Tooltip text={label}>
            {tt => (<button {...tt} aria-label={label} aria-pressed={active} className={`vc-pdfViewer-button${active ? " vc-pdfViewer-button-active" : ""}`} disabled={disabled} type="button" onClick={e => {
                e.stopPropagation();
                if (!disabled)
                    onClick();
            }}>
...
const EyeOpen = () => (<svg {...svgProps}>
        <path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/>
        <circle cx="12" cy="12" r="3"/>
    </svg>);
const EyeClosed = () => (<svg {...svgProps}>
        <path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/>
        <path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/>
        <path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/>
        <path d="m2 2 20 20"/>
    </svg>);
const ZoomIn = () => (<svg {...svgProps}>
        <circle cx="11" cy="11" r="8"/>
        <path d="m21 21-4.3-4.3M11 8v6M8 11h6"/>
    </svg>);
const ZoomOut = () => (<svg {...svgProps}>
        <circle cx="11" cy="11" r="8"/>
        <path d="m21 21-4.3-4.3M8 11h6"/>
    </svg>);
const FitWidth = () => (<svg {...svgProps}>
...
function PageView({ page, scale, n, w, h, render }) {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        task.promise.then(() => { if (!cancelled)
            lastKey.current = key; }, err => {
    ...
    return (<div className="vc-pdfViewer-page" data-page-number={n} style={{ width, height }}>
...
function PdfView({ bytes }) {
    ...
    useEffect(() => {
        if (!doc || !sizes.length || !container.current)
            return;
        const io = new IntersectionObserver(entries => {
            setVisible(prev => {
                ...
                for (const e of entries) {
                    if (!e.isIntersecting)
                        continue;
                    const n = Number(e.target.dataset.pageNumber);
        ...
        for (const el of container.current.querySelectorAll(".vc-pdfViewer-page"))
            io.observe(el);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (cancel || !got.length)
                return;
            setPages(prev => {
                const next = new Map(prev);
                for (const [n, p] of got)
                    next.set(n, p);
                return next;
            });
    ...
    const effectiveScale = useMemo(() => {
        if (scale != null)
            return scale;
        if (!sizes.length || containerW <= 0)
            return 1;
        return Math.max(0.25, Math.min(3, (containerW - 32) / sizes[0].w));
    }, [scale, sizes, containerW]);
    if (err)
        return <div className="vc-pdfViewer-centeredState vc-pdfViewer-error"><span>Couldn't open PDF: {err}</span></div>;
    if (!doc)
        return <div className="vc-pdfViewer-centeredState"><Spinner /><span>Opening PDF…</span></div>;
    return (<div className="vc-pdfViewer-document">
            ...
            <div className="vc-pdfViewer-pages" ref={container}>
                {sizes.map((s, i) => (<PageView key={i + 1} n={i + 1} w={s.w} h={s.h} scale={effectiveScale} page={pages.get(i + 1) ?? null} render={visible.has(i + 1) && pages.has(i + 1)}/>))}
            </div>
        </div>);
}
function PdfBody({ attachment }) {
    ...
    useEffect(() => {
        ...
        setBytes(null);
        setErr(null);
        loadBytes(attachment).then(b => { if (!cancel)
            setBytes(b); }, e => {
    ...
    if (err)
        return (<div className="vc-pdfViewer-centeredState vc-pdfViewer-error">
            <span>Couldn't load PDF: {err}</span>
            <button className="vc-pdfViewer-textButton" type="button" onClick={e => { e.stopPropagation(); setRetry(n => n + 1); }}>
                Retry
            </button>
        </div>);
    if (!bytes)
        return <div className="vc-pdfViewer-centeredState"><Spinner /><span>Loading PDF…</span></div>;
...
function Preview({ attachment }) {
    ...
    return (<div className="vc-pdfViewer-shell" onClick={e => e.stopPropagation()}>
            <div className="vc-pdfViewer-fileBar">
                ...
                <IconBtn active={open} disabled={tooLarge} label={tooLarge ? `Preview disabled — over ${maxFileSizeMb} MB` : open ? "Hide preview" : "Preview PDF"} onClick={() => setOpen(o => !o)}>
...
export default definePlugin({
    ...
    renderMessageAccessory({ message }) {
        ...
        if (!pdfs?.length)
            return null;
        return (<div className="vc-pdfViewer-stack">
