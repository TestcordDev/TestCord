...
const IGPIcon = () => (<svg version="1.1" width="20" height="20" viewBox="0 0 47 47">
        <g>
...
const ChatBarIcon = ({ isMainChat, channel }) => {
    ...
    return (<ChatBarButton tooltip="PGP/GPG Encrypt" onClick={() => buildModal()} buttonProps={{
            "aria-haspopup": "dialog",
        }}>
            <svg version="1.1" id="Capa_1" width="20" height="20" viewBox="0 0 47 47">
                <g>
                    <path fill="currentColor" d="M23.5,0C10.522,0,0,10.522,0,23.5C0,36.479,10.522,47,23.5,47C36.479,47,47,36.479,47,23.5C47,10.522,36.479,0,23.5,0z
                    M30.07,34.686L30.07,34.686c0,2.53-2.941,4.58-6.573,4.58c-3.631,0-6.577-2.05-6.577-4.58c0-0.494,3.648-14.979,3.648-14.979
                    c-2.024-1.06-3.418-3.161-3.418-5.609c0-3.515,2.838-6.362,6.361-6.362c3.514,0,6.35,2.848,6.35,6.362
...
function DecryptMessageIcon() {
    return (<svg fill="currentColor" width={20} height={20} viewBox={"0 0 16 16"}>
...
function formatKey(key) {
    const start = key.match(/-----.*?-----/g)?.at(0)?.toString();
    const end = key.match(/-----.*?-----/g)?.pop()?.toString();
    return start + key.replace(start, "").replace(end, "").replaceAll(" ", "\n") + end;
}
function normalizeSingleLineKey(key) {
...
async function loadOpenPGP() {
    ...
    openpgpLoadPromise = new Promise((resolve, reject) => {
        ...
        script.onload = () => {
            const loadedOpenpgp = window.openpgp;
            if (!loadedOpenpgp) {
...
export function preprocessKey(key) {
    return normalizeSingleLineKey(key);
}
// Key Generation Functionality
async function generateKeyPair(name, email, passphrase, type = "ecc") {
    await ensureOpenPGP();
...
export async function encrypt(message, public_key_recipient) {
    await ensureOpenPGP();
    const pgp = requireOpenPGP();
...
class KeyManager {
    ...
    getVerifiedKeyForUser(userId) {
        this.ensureInitialized();
        return this.keyCache.get(userId) || null;
    }
    async importPublicKeyForUser(userId, armoredKey) {
        this.ensureInitialized();
        await ensureOpenPGP();
        const pgp = requireOpenPGP();
...
async function searchKeyserver(query, keyserver = "OPENPGP") {
...
async function decryptMessage(message, authorId) {
    await ensureOpenPGP();
    const pgp = requireOpenPGP();
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("IGP", () => (<HeaderBarButton icon={IGPIcon} tooltip="PGP/GPG Encrypt" onClick={() => buildModal()}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("IGP", () => (<ChannelToolbarButton icon={IGPIcon} tooltip="PGP/GPG Encrypt" onClick={() => buildModal()}/>), 5);
        }
    },
    stop() {
    ...
    commands: [
        {
                    ...
                    name: "generate", description: "Generate a new PGP key pair", type: 1, options: [
                        ...
                        { name: "passphrase", description: "Passphrase for private key", type: 3, required: true },
                        {
                            name: "type", description: "Key type", type: 3, choices: [
                                { name: "ECC (recommended)", label: "ECC (recommended)", value: "ecc" },
                                { name: "RSA 4096", label: "RSA 4096", value: "rsa" }
                            ]
                        }
                    ]
                },
            ...
            async execute(args, ctx) {
                const sub = args[0];
                const getOpt = (n) => sub.options?.find((o) => o.name === n)?.value;
                try {
                    await ensureOpenPGP();
                    switch (sub.name) {
                        case "encrypt": {
                            ...
                            if (!publicKey)
                                return { content: "❌ No public key for this user. Use `/pgp import` to add their key." };
                        ...
                        case "sign": {
                            if (!settings.store.pgpPrivateKey)
                                return { content: "❌ No private key configured. Use `/pgp generate` to create keys or set them in plugin settings." };
                            const messageOpt = getOpt("message");
                            if (!messageOpt) {
                        ...
                        case "fingerprint": {
                            if (!settings.store.pgpPublicKey)
                                return { content: "❌ No public key configured" };
                            await ensureOpenPGP();
                            const pgp = requireOpenPGP();
