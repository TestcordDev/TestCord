...
import { closeModal, ModalCloseButton as ModalCloseButtonRaw, ModalContent as ModalContentRaw, ModalFooter as ModalFooterRaw, ModalHeader as ModalHeaderRaw, ModalRoot as ModalRootRaw, openModal } from "@utils/modal";
import definePlugin from "@utils/types";
import { ApplicationStreamingStore, Button, Forms, React, showToast, Text, Toasts, useEffect, UserStore, useState, useStateFromStores } from "@webpack/common";
...
const ModalRoot = ModalRootRaw;
const ModalHeader = ModalHeaderRaw;
const ModalContent = ModalContentRaw;
const ModalFooter = ModalFooterRaw;
const ModalCloseButton = ModalCloseButtonRaw;
const FILE_INPUT_ID = "custom-stream-preview-upload";
function PreviewIcon({ className }) {
    return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
...
function PreviewModal({ modalProps, close }) {
    ...
    return (<ModalRoot {...modalProps} size={"small" /* ModalSize.SMALL */}>
            ...
            <ModalContent>
                ...
                <br />
                <Forms.FormText>
                    {t("ملاحظة: تُحدَّث المعاينة مرة واحدة كل 60 ثانية كحد أقصى (قيد Discord)، وتُعاد كل 5 دقائق أثناء البث.", "Note: the preview updates at most once every 60s (Discord limit) and is resent every 5 minutes while streaming.")}
                </Forms.FormText>

                <Button style={{ marginTop: "1rem" }} onClick={openFilePicker}>
                    {t("اختيار صورة", "Choose Image")}
                </Button>
                <input id={FILE_INPUT_ID} style={{ display: "none" }} type="file" accept="image/*" onChange={onPickFile}/>

                {preview && (<>
                        <Divider style={{ marginTop: "1rem", marginBottom: "1rem" }}/>
                        <img src={preview} alt={t("معاينة", "Preview")} style={{ maxWidth: "100%", borderRadius: "8px", display: "block" }}/>
                    </>)}
            </ModalContent>

            <ModalFooter>
                <Flex style={{ width: "100%", justifyContent: "space-between" }}>
                    {sending && (<Button color={Button.Colors.RED} onClick={onStop}>
...
function PreviewButton(props) {
    ...
    const openPreviewModal = () => {
        const key = openModal(modalProps => (<PreviewModal modalProps={modalProps} close={() => closeModal(key)}/>));
    };
    return (<ErrorBoundary noop>
            <UserAreaButton tooltipText={props.hideTooltips ? void 0 : t("معاينة البث المخصصة", "Custom Stream Preview")} icon={<PreviewIcon className={props.iconForeground}/>} onClick={openPreviewModal}/>
        </ErrorBoundary>);
}
export default definePlugin({
    name: "CustomStreamPreview",
    description: "Set a custom image as your Discord stream preview.",
    tags: ["Voice", "Utility"],
    ...
    flux: {
        async STREAM_CREATE({ streamKey }) {
            const parsed = parseStreamKey(streamKey);
            if (!parsed || parsed.userId !== UserStore.getCurrentUser().id)
                return;
            const image = await getSavedPreview();
            if (image)
                startSendingPreview(image);
        },
        STREAM_DELETE({ streamKey }) {
            const parsed = parseStreamKey(streamKey);
            if (!parsed || parsed.userId !== UserStore.getCurrentUser().id)
                return;
            stopSendingPreview();
