...
export function enabledTogglablePlugins() {
 ...
 */
export function enterPerformanceMode(keep) {
    const keepSet = new Set(keep);
    const wasEnabled = [];
    for (const name of Object.keys(plugins)) {
        if (isEssentialPlugin(name))
            continue;
        const s = Settings.plugins[name];
        if (!s)
            continue;
        if (s.enabled)
            wasEnabled.push(name);
        if (keepSet.has(name))
            continue;
        s.enabled = false;
    }
...
export function exitPerformanceMode(saved) {
