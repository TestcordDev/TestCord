...
function LyricsDisplay({ scroll = true }) {
    ...
    if (!currentLyrics) {
        return showMusicNoteOnNoLyrics ? (<div className="eq-tidal-lyrics" onContextMenu={e => ContextMenuApi.openContextMenu(e, () => <LyricsContextMenu />)}>
                <TooltipContainer text="No lyrics found">
                    {NoteElement}
                </TooltipContainer>
            </div>) : null;
    }
    return (<div className="eq-tidal-lyrics" onClick={() => openModal(props => <LyricsModal rootProps={props}/>)} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => <LyricsContextMenu />)}>
            {currentLyrics.map((line, i) => (<div ref={lyricRefs[i]} key={i}>
...
export function TidalLyrics({ scroll = true } = {}) {
    TidalLrcStore.init();
    const track = useStateFromStores([TidalStore], () => TidalStore.track, null, (prev, next) => (prev?.id ? prev.id === next?.id : prev?.name === next?.name));
