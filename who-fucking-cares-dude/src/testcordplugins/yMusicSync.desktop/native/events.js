...
export function emitSnapshot(snapshot) {
    if (state.lastSnapshot && sameSnapshot(state.lastSnapshot, snapshot))
        return;
...
export function emitStatus(connectionState, error = null) {
    state.connectionState = connectionState;
    state.lastError = error;
    enqueue({ type: "status", status: statusSnapshot(), at: Date.now() });
}
export function awaitEvents(timeoutMs) {
    const previous = eventWaiter;
    eventWaiter = null;
    previous?.([]);
    if (eventQueue.length > 0)
        return Promise.resolve(eventQueue.splice(0, MAX_EVENTS));
