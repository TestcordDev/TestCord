...
function interceptAddMember(originalMethod) {
...
- Current user: ${currentUserId}`);
                ...
                if (channel && channel.type === 3) {
                    const channelName = channel.name || "Unnamed group";
                    log(`🚫 Unauthorized addition detected in "${channelName}" - Auto-kick scheduled`);
...
function toggleGroupLock(channelId) {
    const channel = ChannelStore.getChannel(channelId);
    const currentUserId = UserStore.getCurrentUser()?.id;
    if (!channel) {
...
const GroupContextMenuPatch = (children, { channel }) => {
    if (!channel || channel.type !== 3)
        return; // 3 = GROUP_DM
    const currentUserId = UserStore.getCurrentUser()?.id;
    ...
    if (group) {
        ...
        if (!isLocked) {
            menuItems.push(<Menu.MenuItem key="lock-group" id="vc-lock-group" label="🔒 Lock the group" color="danger" action={() => toggleGroupLock(channel.id)} icon={() => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        ...
        if (isLocked) {
            menuItems.push(<Menu.MenuItem key="unlock-group" id="vc-unlock-group" label="🔓 Unlock the group" color="brand" action={() => toggleGroupLock(channel.id)} icon={() => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
let originalPutMethod = null;
let pluginActive = false;
let pluginGeneration = 0;
const pendingKickTimeouts = new Set();
function scheduleKick(callback, ms) {
...
export default definePlugin({
    name: "LockGroup",
    description: "Allows locking/unlocking groups via context menu (prevents adding members)",
    tags: ["Utility", "Chat"],
    authors: [
        {
            name: "Bash",
...
- Debug: ${settings.store.debugMode ? "ON" : "OFF"}`);
    ...
    stop() {
        ...
        // Clean up state
        lockedGroups.clear();
        for (const timeout of pendingKickTimeouts)
