...
export function analyzeSurveillanceEvents(events, use24h = false) {
    if (events.length === 0) {
    ...
    for (const event of [...voiceJoins, ...voiceUpdates]) {
        if (event.metadata?.selfStream === true)
            streamCount++;
        if (event.metadata?.selfVideo === true)
            cameraCount++;
    }
    sections.push({
    ...
    for (const event of statusEvents) {
        ...
        if (typeof to === "string")
            statusCounts.set(to, (statusCounts.get(to) ?? 0) + 1);
    }
    ...
    for (const event of [...activityStarts, ...activityUpdates]) {
        const name = event.metadata?.activity ?? event.metadata?.to ?? event.details.replace(/^(Started|Changed activity from .+ to) /, "").replace(/\.$/, "");
        if (name)
            activityNames.set(name, (activityNames.get(name) ?? 0) + 1);
    }
    ...
    for (const event of sorted) {
        if (event.type === "activity_start") {
            const name = event.metadata?.activity ?? event.details.replace(/^Started /, "").replace(/\.$/, "");
            activeSessions.set(name, event.timestamp);
        }
        else if (event.type === "activity_stop") {
            const name = event.metadata?.activity ?? event.details.replace(/^Stopped /, "").replace(/\.$/, "");
    ...
    for (const event of sorted) {
        if (event.channelName)
            channelCounts.set(event.channelName, (channelCounts.get(event.channelName) ?? 0) + 1);
        if (event.guildName)
            guildCounts.set(event.guildName, (guildCounts.get(event.guildName) ?? 0) + 1);
    }
    ...
    const behaviors = [];
    if (nightEvents > dayEvents)
        behaviors.push("Nocturnal activity pattern");
    if (messageEvents.length > totalEvents * 0.4)
        behaviors.push("Primarily a communicator");
    if (voiceJoins.length > 10)
        behaviors.push("Frequent voice chat user");
    if (totalVoiceMs > 3600000)
        behaviors.push("Long voice sessions");
    if (editEvents.length > messageEvents.length * 0.2)
        behaviors.push("Frequently edits messages");
    if (deleteEvents.length > messageEvents.length * 0.1)
        behaviors.push("Frequently deletes messages");
    if (totalLinks > messageEvents.length * 0.3)
        behaviors.push("Frequent link sharer");
    if (totalAttachments > messageEvents.length * 0.2)
        behaviors.push("Frequent file sender");
    if (statusEvents.length > 20)
        behaviors.push("Frequently changes status");
    if (activityStarts.length > 10)
        behaviors.push("Active gamer/media consumer");
    if (streamCount > 5)
        behaviors.push("Regular streamer");
    if (profileUpdates.length > 3)
        behaviors.push("Frequently updates profile");
    if (reactionAddEvents.length > messageEvents.length)
        behaviors.push("Heavy reaction user");
    if (uniqueDays > 30)
        behaviors.push("Long-term consistent user");
    else if (uniqueDays < 3 && totalEvents > 50)
        behaviors.push("Burst activity pattern");
    if (longestStreak > 14)
        behaviors.push(`Highly consistent (${longestStreak} day streak)`);
    sections.push({
