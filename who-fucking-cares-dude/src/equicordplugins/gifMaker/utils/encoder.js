...
const MediaNative = VencordNative?.pluginHelpers?.gifMaker;
...
async function fetchFullGifBytes(url) {
...
async function getMediaBlobUrl(url) {
    if (MediaNative) {
...
export function cleanupBlobUrl(el) {
...
export function loadVideo(url) {
    const resolved = resolveMediaUrl(url);
    if (isDiscordCdnUrl(resolved)) {
        return getMediaBlobUrl(resolved).then(blobUrl => createVideoElement(blobUrl).then(video => {
...
export function getCaptionHeight(ctx, width, options) {
...
async function encodeFrames(width, height, options, frameCount, drawFrame, delays) {
    ...
    return new Blob([new Uint8Array(bytes.buffer, bytes.byteOffset, bytes.byteLength)], { type: "image/gif" });
}
async function createGifFromImage(url, options) {
...
async function createGifFromVideo(url, options) {
...
export async function getSourceFrameInfo(url, isVideo) {
    if (isVideo)
        return getVideoSourceInfo(url);
    if (hasExt(url, ".gif"))
        return getGifInfo(url);
    if (hasExt(url, ".webp"))
        return getWebpInfo(url);
    return null;
}
export async function createGif(url, isVideo, options) {
    if (isVideo)
...
export function parseGifBytes(bytes) {
...
export async function getGifInfo(url) {
    try {
        const resolved = resolveMediaUrl(url);
        let bytes;
        if (MediaNative) {
...
async function fetchGifBytes(url) {
    ...
    if (!full.ok)
        throw new Error(`fetch failed: ${full.status}`);
    const reader = full.body?.getReader();
    if (!reader)
        return new Uint8Array(await full.arrayBuffer()).slice(0, MAX_GIF_SCAN_BYTES);
    const chunks = [];
    let total = 0;
...
async function getWebpInfo(url) {
    if (!MediaNative)
        return null;
    try {
        const resolved = resolveMediaUrl(url);
...
async function getVideoSourceInfo(url) {
    try {
        const resolved = resolveMediaUrl(url);
        let src;
        ...
        return new Promise(resolve => {
            ...
            v.crossOrigin = "anonymous";
            v.addEventListener("loadedmetadata", () => {
                const info = { frameWidth: v.videoWidth, frameHeight: v.videoHeight };
                if (needsCleanup)
...
async function createGifFromAnimatedImage(url, options) {
    const bytes = await fetchFullGifBytes(url);
    const parsedGif = parseGIF(bytes.buffer);
    const frames = decompressFrames(parsedGif, true);
    if (frames.length <= 1)
        throw new Error("No animated frames found");
    ...
    if (!ctx)
        throw new Error("Failed to get canvas context for GIF compositing.");
    ...
    for (let i = 0; i < totalFrames; i++) {
        ...
        if (!patchCtx)
            throw new Error("Failed to get canvas context for patch rendering.");
        ...
        if (!snapCtx)
            throw new Error("Failed to get canvas context for frame snapshot.");
    ...
    return await encodeFrames(options.width, options.height, options, totalFrames, (encodeCtx, i) => {
