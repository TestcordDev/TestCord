...
const AR_ALL_EXTS = ["png", "jpg", "jpeg", "jfif", "gif", "webp", "avif"];
const AR = {
...
let bcrRandomBatch = [];
let bcrSeqBatch = [];
let bcrRotatorTimer = null;
let bcrCachedHue = null;
let bcrGradientState = null;
let bcrMonoBaseHue = null;
let bcrSeqBaseHue = 0;
let bcrShadeStep = 0;
let bcrShadeDir = 1;
let bcrCurrentColor = null;
let bcrOnColorApplied = null;
function bcrIsValidHex(v) { return /^#[0-9a-fA-F]{6}$/.test(v); }
function bcrHsvToHex(h, s, v) {
    h = ((h % 360) + 360) % 360;
    ...
    const toB = (n) => Math.round((n + m) * 255).toString(16).padStart(2, "0");
    return "#" + toB(r) + toB(g) + toB(b);
}
function bcrHexToHsv(hex) {
...
function bcrHslToHex(h, s, l) {
    s /= 100;
    l /= 100;
    const v = l + s * Math.min(l, 1 - l);
    return bcrHsvToHex(h, v === 0 ? 0 : 2 * (1 - l / v), v);
}
function bcrRandomHex() { return "#" + Math.floor(Math.random() * 0xFFFFFF).toString(16).padStart(6, "0"); }
function bcrSfShuffle(arr) {
    const a = [...arr];
...
function bcrBuildBatch(size, gen) {
    const set = new Set();
    let tries = 0;
    while (set.size < size && tries++ < size * 8)
        set.add(gen());
    return [...set];
}
function bcrPickFromFavs() {
    if (!bcrFavorites.length)
        return bcrRandomHex();
    const available = bcrFavorites.filter(c => !bcrUsedFavs.includes(c));
    if (!available.length) {
        bcrUsedFavs = [];
        return bcrPickFromFavs();
    }
    const color = available[Math.floor(Math.random() * available.length)];
    bcrUsedFavs = [...bcrUsedFavs, color];
    return color;
}
function bcrGetBaseHsv() {
    const base = settings.store.bannerCustomBaseColor ?? "#c084fc";
    return bcrIsValidHex(base) ? bcrHexToHsv(base) : [270, 0.48, 0.79];
}
async function bcrGetAvatarHue() {
    if (bcrCachedHue !== null)
        return bcrCachedHue;
    try {
        ...
        return await new Promise(res => {
            ...
            img.onload = () => {
                ...
                bcrCachedHue = bcrHexToHsv(hex)[0];
                res(bcrCachedHue);
            };
...
function bcrBuildShadeSequence(lightFirst) {
    const [h, s] = bcrGetBaseHsv();
    const steps = 20;
    const seq = [];
    for (let i = 0; i < steps; i++) {
        const t = i / (steps - 1);
        const l = lightFirst ? 88 - t * 83 : 5 + t * 83;
        seq.push(bcrHslToHex(h * 360, s * 100, l));
    }
    return seq;
}
async function bcrPickNextColor() {
    const mode = (settings.store.bannerMode ?? "full_random");
    const R = Math.max(1, Math.min(180, settings.store.bannerHueRadius ?? 35));
    if (mode === "full_random") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, bcrRandomHex));
        return bcrRandomBatch.shift();
    }
    if (mode === "avatar_hue") {
        if (!bcrRandomBatch.length) {
            const hue = await bcrGetAvatarHue();
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(hue + (Math.random() * 2 - 1) * R, 40 + Math.random() * 55, 20 + Math.random() * 50)));
        }
        return bcrRandomBatch.shift();
    }
    if (mode === "mono_cycle") {
        if (bcrMonoBaseHue === null || !bcrRandomBatch.length) {
            const [h] = bcrGetBaseHsv();
            bcrMonoBaseHue = h * 360;
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(bcrMonoBaseHue, 25 + Math.random() * 70, 10 + Math.random() * 75)));
        }
        return bcrRandomBatch.shift();
    }
    if (mode === "warm") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(Math.random() * 60, 55 + Math.random() * 45, 20 + Math.random() * 50)));
        return bcrRandomBatch.shift();
    }
    if (mode === "cool") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(180 + Math.random() * 120, 50 + Math.random() * 50, 20 + Math.random() * 45)));
        return bcrRandomBatch.shift();
    }
    if (mode === "pastel") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(Math.random() * 360, 25 + Math.random() * 35, 70 + Math.random() * 18)));
        return bcrRandomBatch.shift();
    }
    if (mode === "dark") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(Math.random() * 360, 40 + Math.random() * 55, 4 + Math.random() * 16)));
        return bcrRandomBatch.shift();
    }
    if (mode === "vivid") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(Math.random() * 360, 88 + Math.random() * 12, 38 + Math.random() * 22)));
        return bcrRandomBatch.shift();
    }
    if (mode === "gradient_walk") {
        if (!bcrGradientState)
            bcrGradientState = [Math.random() * 360, 50 + Math.random() * 40, 25 + Math.random() * 40];
    ...
    if (mode === "chromatic") {
        if (!bcrSeqBatch.length) {
            const STEPS = 24;
            bcrSeqBatch = Array.from({ length: STEPS }, (_, i) => bcrHslToHex((bcrSeqBaseHue + (360 / STEPS) * i) % 360, 75, 42));
            bcrSeqBaseHue = (bcrSeqBaseHue + 15) % 360;
        }
        return bcrSeqBatch.shift();
    }
    if (mode === "rgb_loop") {
        if (!bcrSeqBatch.length) {
            const r2 = Array.from({ length: 8 }, (_, i) => bcrHslToHex(0, 55 + i * 5, 20 + i * 7));
            const g2 = Array.from({ length: 8 }, (_, i) => bcrHslToHex(120, 55 + i * 5, 20 + i * 7));
            const b2 = Array.from({ length: 8 }, (_, i) => bcrHslToHex(240, 55 + i * 5, 20 + i * 7));
            bcrSeqBatch = [...r2, ...g2, ...b2];
        }
        return bcrSeqBatch.shift();
    }
    if (mode === "complementary") {
        if (!bcrSeqBatch.length) {
            ...
            bcrSeqBatch = bcrSfShuffle([...Array.from({ length: 8 }, () => bcrHslToHex(h0 + (Math.random() - 0.5) * 10, 50 + Math.random() * 40, 25 + Math.random() * 40)), ...Array.from({ length: 8 }, () => bcrHslToHex((h0 + 180) % 360 + (Math.random() - 0.5) * 10, 50 + Math.random() * 40, 25 + Math.random() * 40))]);
        }
        return bcrSeqBatch.shift();
    }
    if (mode === "triadic") {
        if (!bcrSeqBatch.length) {
            const h0 = Math.random() * 360;
            bcrSeqBatch = bcrSfShuffle([...Array.from({ length: 6 }, () => bcrHslToHex(h0 + (Math.random() - 0.5) * 8, 55 + Math.random() * 35, 28 + Math.random() * 35)), ...Array.from({ length: 6 }, () => bcrHslToHex((h0 + 120) % 360 + (Math.random() - 0.5) * 8, 55 + Math.random() * 35, 28 + Math.random() * 35)), ...Array.from({ length: 6 }, () => bcrHslToHex((h0 + 240) % 360 + (Math.random() - 0.5) * 8, 55 + Math.random() * 35, 28 + Math.random() * 35))]);
        }
        return bcrSeqBatch.shift();
    }
    if (mode === "analogous") {
        if (!bcrRandomBatch.length) {
            const hue = await bcrGetAvatarHue();
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(24, () => bcrHslToHex(hue + (Math.random() * 2 - 1) * Math.min(R, 60), 45 + Math.random() * 50, 22 + Math.random() * 48)));
        }
        return bcrRandomBatch.shift();
    }
    if (mode === "earth") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => { const h = [25, 32, 40, 50, 20, 15][Math.floor(Math.random() * 6)]; return bcrHslToHex(h + (Math.random() - 0.5) * 14, 30 + Math.random() * 40, 18 + Math.random() * 40); }));
        return bcrRandomBatch.shift();
    }
    if (mode === "neon") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(Math.random() * 360, 100, 50 + Math.random() * 12)));
        return bcrRandomBatch.shift();
    }
    if (mode === "sunset") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => { const h = 0 + Math.random() * 55; return bcrHslToHex(h, 70 + Math.random() * 30, 28 + Math.random() * 38); }));
        return bcrRandomBatch.shift();
    }
    if (mode === "ocean") {
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, () => bcrHslToHex(175 + Math.random() * 50, 50 + Math.random() * 45, 18 + Math.random() * 42)));
        return bcrRandomBatch.shift();
    }
    if (mode === "shade_light_dark") {
        if (!bcrSeqBatch.length) {
            bcrSeqBatch = bcrBuildShadeSequence(true);
            bcrShadeStep = 0;
        }
        const out1 = bcrSeqBatch[bcrShadeStep % bcrSeqBatch.length];
        bcrShadeStep++;
        if (bcrShadeStep >= bcrSeqBatch.length)
            bcrSeqBatch = [];
        return out1;
    }
    if (mode === "shade_dark_light") {
        if (!bcrSeqBatch.length) {
            bcrSeqBatch = bcrBuildShadeSequence(false);
            bcrShadeStep = 0;
        }
        const out2 = bcrSeqBatch[bcrShadeStep % bcrSeqBatch.length];
        bcrShadeStep++;
        if (bcrShadeStep >= bcrSeqBatch.length)
            bcrSeqBatch = [];
        return out2;
    ...
    if (mode === "shade_oscillate") {
        ...
        const out3 = bcrSeqBatch[Math.max(0, Math.min(bcrSeqBatch.length - 1, bcrShadeStep))];
        bcrShadeStep += bcrShadeDir;
        if (bcrShadeStep >= bcrSeqBatch.length - 1)
            bcrShadeDir = -1;
        if (bcrShadeStep <= 0)
            bcrShadeDir = 1;
        return out3;
    ...
    if (mode === "favs_sequential") {
        if (!bcrFavorites.length)
            return bcrRandomHex();
        if (!bcrSeqBatch.length)
            bcrSeqBatch = [...bcrFavorites];
        return bcrSeqBatch.shift();
    }
    if (mode === "favorites_only")
        return bcrPickFromFavs();
    if (mode === "favorites_mix") {
        if (bcrFavorites.length && Math.random() < 0.5)
            return bcrPickFromFavs();
        if (!bcrRandomBatch.length)
            bcrRandomBatch = bcrSfShuffle(bcrBuildBatch(32, bcrRandomHex));
        return bcrRandomBatch.shift();
    ...
    if (mode === "favs_shade") {
        const base = bcrFavorites.length ? bcrPickFromFavs() : bcrRandomHex();
        const [hF, , vF] = bcrHexToHsv(base);
        const lF = vF * 100;
        return bcrHslToHex(hF * (180 / Math.PI) || hF, 60 + Math.random() * 35, Math.max(8, Math.min(88, lF + (Math.random() * 2 - 1) * 40)));
    }
    return bcrRandomHex();
}
async function bcrApplyColor(hex) {
    const p = (async () => {
        try {
            ...
            if (!pluginActive)
                return;
            bcrCurrentColor = hex;
            bcrOnColorApplied?.(hex);
            if (settings.store.bannerShowToast)
                Toasts.show({ message: `Banner → ${hex}`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
            await bcrSaveData();
        }
        catch (e) {
            if (settings.store.bannerShowToast)
                Toasts.show({ message: `Banner failed: ${e?.body?.message ?? "Unknown"}`, type: Toasts.Type.FAILURE, id: Toasts.genId() });
        }
    })();
    bcrApplyPromise = p;
    await p;
}
async function bcrRotateNext() { if (!pluginActive || isManualStop || wasInvisible)
    return; await bcrApplyColor(await bcrPickNextColor()); if (!pluginActive || isManualStop || wasInvisible)
    return; bcrSchedule(); }
function bcrSchedule() { if (bcrRotatorTimer)
    clearTimeout(bcrRotatorTimer); if (!pluginActive || isManualStop || wasInvisible)
    return; bcrRotatorTimer = setTimeout(bcrRotateNext, Math.max(1, settings.store.bannerIntervalSeconds ?? BCR_DEFAULT_S) * 1000); }
function bcrStartRotator(immediate = false) {
    if (!pluginActive)
        return;
    if (bcrRotatorTimer)
        clearTimeout(bcrRotatorTimer);
    bcrRandomBatch = [];
    bcrSeqBatch = [];
    bcrUsedFavs = [];
    bcrGradientState = null;
    bcrMonoBaseHue = null;
    bcrSeqBaseHue = 0;
    bcrShadeStep = 0;
    bcrShadeDir = 1;
    if (immediate)
        ...
        bcrRotatorTimer = setTimeout(bcrRotateNext, Math.max(1, settings.store.bannerIntervalSeconds ?? BCR_DEFAULT_S) * 1000);
}
function bcrStopRotator() { if (bcrRotatorTimer) {
    clearTimeout(bcrRotatorTimer);
    bcrRotatorTimer = null;
} void bcrSaveData(); }
const bcrSaveData = () => DataStore.set(BCR_SK, { favorites: bcrFavorites, usedFavs: bcrUsedFavs, wasRunning: bcrRotatorTimer !== null, currentColor: bcrCurrentColor });
const BCR_MODES = [
...
const NM_NEXT = { custom: "global", global: "both", both: "custom" };
const NM_LABEL = { custom: "Custom", global: "Global", both: "Both" };
const NM_COLOR = { custom: "#9575cd", global: "#4dd0e1", both: "#f48fb1" };
const C = {
...
let bioSeqIdx = 0;
let prSeqIdx = 0;
let statusLastVal = null;
let clanLastVal = null;
let bioLastVal = null;
let prLastVal = null;
let globalNickEntries = [];
let globalNickSeqIdx = 0;
let globalNickLastVal = null;
let globalGuildPronouns = [];
let cachedToken = null;
let cachedGuildStore = null;
let cachedClanGuilds = [];
let lastClanFetch = 0;
const domTagCache = new Map();
const nickTimers = new Map();
const guildPronounsTimers = new Map();
let statusTimer = null;
let clanTimer = null;
let clanAbortCtrl = null;
let bcrApplyPromise = Promise.resolve();
let bioTimer = null;
let pronounsTimer = null;
let globalNickTimer = null;
...
let cachedVoiceStateStore = null;
let cachedChannelStore = null;
let globalStopTimer = null;
let globalStopEndTime = null;
let isManualStop = false;
let invisibleWatchInterval = null;
let wasInvisible = false;
let cachedPresenceStore = null;
...
const saveCloseConfig = () => {
    ...
    try {
        localStorage.setItem(RS_CLOSE_LS, JSON.stringify(cfg));
    }
    catch { }
};
async function applyOnCloseAll() {
        ...
        try {
            if (settings.store.bannerNitroCheck && arHasNitro()) { }
                ...
                if (bcrIsValidHex(hex)) {
                    await bcrApplyPromise.catch(() => { });
                    void bcrApplyColor(hex);
                }
            }
        }
        catch { }
    ...
    if (closeBannerEnabled) {
        try {
            if (settings.store.bannerNitroCheck && arHasNitro()) { }
                ...
                const hex = closeBannerColor.trim();
                if (bcrIsValidHex(hex))
                    fetch("https://discord.com/api/v9/users/@me", { method: "PATCH", keepalive: true, headers: hdrs, body: JSON.stringify({ banner_color: hex }) });
            }
        }
        catch { }
    }
...
async function applyCloseStatus() {
    if (!closeStatusEnabled)
        return;
    const raw = closeStatusText.trim();
    const emojiRaw = closeStatusEmoji.trim();
...
async function applyCloseBanner() {
    if (!closeBannerEnabled)
        return;
    const hex = closeBannerColor.trim();
    if (!bcrIsValidHex(hex))
...
const saveData = () => DataStore.set(SK, {
    ...
    guilds: guilds.map(g => { const { lastNickVal: _, ...rest } = g; return rest; }),
...
function getMs(sec) { return Math.max(1000, (parseFloat(sec) || 30) * 1000); }
function pickItem(list, idx, rnd, lastVal) {
    if (!list.length)
        return { val: null, next: idx, lastPicked: lastVal ?? null };
    if (!rnd) {
        const i = idx % list.length;
        return { val: list[i], next: idx + 1, lastPicked: list[i] };
    }
    if (list.length === 1)
        return { val: list[0], next: idx, lastPicked: list[0] };
...
function getToken() { if (!cachedToken)
    cachedToken = findByProps("getToken"); return cachedToken?.getToken?.() ?? null; }
function getGuildStore() { if (!cachedGuildStore)
    cachedGuildStore = findByProps("getGuilds"); return cachedGuildStore; }
function getDiscordGuilds() {
    try {
        return Object.values(getGuildStore()?.getGuilds?.() ?? {}).map((x) => ({ id: x.id, name: x.name }));
    }
    catch {
        return [];
    }
...
function nickModeOf(g) { return g.nickMode ?? (g.useGlobal ? "global" : "custom"); }
function nicksForGuild(g) {
    const m = nickModeOf(g);
    if (m === "global")
        return globalNicks;
    if (m === "both")
        return [...new Set([...globalNicks, ...g.nicks])];
    return g.nicks.length ? g.nicks : globalNicks;
}
async function applyNick(guildId, nick) {
    try {
        await RestAPI.patch({ url: `/guilds/${guildId}/members/@me`, body: { nick } });
        if (settings.store.enableLogs)
            console.log(`[RS/Nick] [${guildId}] -> "${nick}"`);
        return null;
    }
    catch (err) {
        ...
        if (st === 429) {
            ...
            if (settings.store.enableLogs)
                console.warn(`[RS/Nick] 429 retry ${ra}s`);
            return ra;
        }
        if (settings.store.enableLogs)
            console.error("[RS/Nick] err:", err);
        return null;
    }
}
function scheduleNickTick(g, ms) {
    nickTimers.set(g.id, setTimeout(async () => {
        if (!pluginActive || !g.enabled || !settings.store.nickEnabled || !nickTimers.has(g.id))
            return;
        if (g.voiceActivated && settings.store.voiceActivateEnabled && !settings.store.voiceActivateGlobal) {
            if (getMyVoiceGuildId() !== g.id) {
...
function startNickGuild(g) {
    if (!nickTimers.has(g.id) && !settings.store.globalSync && settings.store.nickEnabled)
        scheduleNickTick(g, getMs(settings.store.nickIntervalSeconds));
}
function stopNickGuild(id) { const t = nickTimers.get(id); if (t) {
    clearTimeout(t);
    nickTimers.delete(id);
} }
...
function pronounsForGuild(g) {
    const m = g.guildPronounsMode ?? "custom";
    if (m === "global")
        return globalGuildPronouns;
    if (m === "both")
        return [...new Set([...globalGuildPronouns, ...(g.guildPronouns ?? [])])];
    const local = g.guildPronouns ?? [];
    return local.length > 0 ? local : globalGuildPronouns;
}
function scheduleGuildPronounsTick(g, ms) {
    const tid = setTimeout(async () => {
        if (guildPronounsTimers.get(g.id) !== tid)
            return;
        ...
        g.guildPronounsSeqIdx = next;
        g.guildPronounsLastVal = lastPicked;
        if (pr)
            await applyGuildPronoun(g.id, pr);
        saveData();
...
function stopGuildPronouns(id) { const t = guildPronounsTimers.get(id); if (t) {
    clearTimeout(t);
    guildPronounsTimers.delete(id);
} }
...
function tickAllGuildPronouns() {
    ...
    for (const g of guilds.filter(x => x.guildPronounsEnabled && pronounsForGuild(x).length > 0)) {
        ...
        g.guildPronounsLastVal = lastPicked;
        if (pr)
            applyGuildPronoun(g.id, pr);
    }
}
function parseLegacyStatuses(raw) {
    return parseList(raw).map(line => {
        const cm = line.match(CUSTOM_EMOJI_RE);
        if (cm)
            return { emojiName: cm[1], emojiId: cm[2], text: line.slice(cm[0].length).trim() };
        const um = line.match(UNICODE_EMOJI_RE);
        if (um)
            return { emojiName: um[1], emojiId: null, text: line.slice(um[0].length).trim() };
        return { emojiName: null, emojiId: null, text: line };
    });
}
function parseStatuses(raw) { return parseLegacyStatuses(raw); }
function parseDiscordEmoji(input) {
    const dm = input.replace(/^\\/, "").match(DISCORD_EMOJI_RE);
    if (dm)
        return { text: input.replace(DISCORD_EMOJI_RE, "").trim(), emojiName: dm[1], emojiId: dm[2], animated: input.includes("<a:") };
    const cm = input.match(CUSTOM_EMOJI_RE);
    if (cm)
        return { text: input.slice(cm[0].length).trim(), emojiName: cm[1], emojiId: cm[2], animated: false };
    const um = input.match(UNICODE_EMOJI_RE);
    if (um)
        return { text: input.slice(um[0].length).trim(), emojiName: um[1], emojiId: null, animated: false };
    return { text: input, emojiName: null, emojiId: null, animated: false };
}
function statusKey(e) { return `${e.emojiId ?? ""}|${e.emojiName ?? ""}|${e.text}`; }
const RS_EVAL = "eval ";
async function resolveField(value) {
    if (!value.startsWith(RS_EVAL))
        return value;
    try {
        ...
        if (settings.store.enableLogs)
            console.error("[RS/eval]", e?.message ?? e);
        return "";
    }
}
async function resolveStatusEntry(entry) {
    ...
    return { ...entry, text, emojiName: emojiName };
}
const CustomStatusSetting = getUserSettingLazy("status", "customStatus");
const PresenceSetting = getUserSettingLazy("status", "status");
async function applyStatus(entry, retries = 3) {
    ...
    if (CustomStatusSetting) {
        try {
            ...
            if (entry.status && entry.status !== "auto" && PresenceSetting)
                await PresenceSetting.updateSetting(entry.status);
            if (settings.store.enableLogs)
                console.log(`[RS/Status] -> "${entry.text}" [${entry.status ?? "auto"}]`);
            return;
        }
        catch (e) {
            ...
            if (st === 429)
                return;
            if (settings.store.enableLogs)
                console.warn("[RS/Status] UserSetting fallback RestAPI:", e);
        }
    }
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            if (settings.store.enableLogs)
                console.log(`[RS/Status] -> "${entry.text}" attempt=${attempt + 1}`);
            return;
        }
        catch (e) {
            const st = e?.status ?? e?.response?.status ?? 0;
            ...
            if (settings.store.enableLogs)
                console.error(`[RS/Status] attempt=${attempt + 1} err:`, e);
...
function getActiveStatusPool() {
    const mode = settings.store.statusRunMode;
    if (mode === "none")
        return [];
    if (mode !== "presets")
        return statusEntries;
    try {
        const sel = JSON.parse(settings.store.statusSelectedPresets || "[]");
        if (!sel.length)
            return statusEntries;
...
function tickStatus() {
    ...
    statusSeqIdx = next;
    statusLastVal = lastPicked ? statusKey(lastPicked) : null;
    if (entry)
        resolveStatusEntry(entry).then(resolved => applyStatus(resolved));
}
function scheduleStatusLoop() {
    ...
    statusTimer = setTimeout(() => {
        statusTimer = null;
        if (!pluginActive || settings.store.globalSync)
            return;
        tickStatus();
        saveData();
        if (settings.store.statusEnabled)
...
function stopStatusTimer() { if (statusTimer) {
    clearTimeout(statusTimer);
    statusTimer = null;
} }
function getActiveClanIds() {
    if (!settings.store.clanAutoDetect)
        return clanIds;
    ...
    if (!cachedClanGuilds.length || now - lastClanFetch >= refreshMs) {
        cachedClanGuilds = getDiscordGuilds().map(g => g.id);
        lastClanFetch = now;
        if (settings.store.enableLogs)
            console.log(`[RS/Clan] Auto-detect: ${cachedClanGuilds.length} guilds`);
    }
    return cachedClanGuilds;
}
async function applyClan(id, retries = 4, signal) {
    const token = getToken();
    if (!token)
        return;
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        try {
            ...
            if (res.ok) {
                if (settings.store.enableLogs)
                    console.log(`[RS/Clan] -> ${id} attempt=${attempt + 1}`);
                return;
            }
            if (res.status === 300) {
                ...
                if (signal?.aborted || !pluginActive)
                    return;
                continue;
            }
            if (settings.store.enableLogs)
                console.error(`[RS/Clan] HTTP ${res.status} attempt=${attempt + 1}`);
            if (attempt < retries - 1) {
                await rsSleep(1500);
                if (signal?.aborted || !pluginActive)
                    return;
            ...
            if (signal?.aborted)
                return;
            if (settings.store.enableLogs)
                console.error(`[RS/Clan] attempt=${attempt + 1} err:`, e);
            if (attempt < retries - 1) {
                await rsSleep(1500);
                if (signal?.aborted || !pluginActive)
                    return;
...
function tickClan() {
    if (!settings.store.clanEnabled || !pluginActive || isManualStop || wasInvisible)
        return;
    const list = getActiveClanIds();
    if (!list.length)
        return;
    const { val: id, next, lastPicked } = pickItem(list, clanSeqIdx, settings.store.clanRandomize, clanLastVal);
    clanSeqIdx = next;
    clanLastVal = lastPicked;
    if (id) {
        if (clanAbortCtrl)
            clanAbortCtrl.abort();
...
function scheduleClanLoop() {
    ...
    clanTimer = setTimeout(() => {
        clanTimer = null;
        if (!pluginActive || isManualStop || wasInvisible)
            return;
        tickClan();
        saveData();
        if (settings.store.clanEnabled)
            scheduleClanLoop();
...
async function patchProfile(body) {
    try {
        ...
        if (settings.store.enableLogs)
            console.log("[RS/Profile] ->", body);
    }
    catch (e) {
        if (settings.store.enableLogs)
            console.error("[RS/Profile]:", e);
    }
}
async function applyGlobalNick(displayName, retries = 3) {
    for (let attempt = 0; attempt < retries; attempt++) {
        if (!pluginActive)
            return;
        try {
            ...
            if (settings.store.enableLogs)
                console.log(`[RS/GlobalNick] -> "${displayName}"`);
            return;
        }
        catch (e) {
            const st = e?.status ?? e?.response?.status ?? 0;
            if (st === 429) {
                ...
                if (settings.store.enableLogs)
                    console.warn(`[RS/GlobalNick] 429 retry ${ra}s`);
            ...
            if (settings.store.enableLogs)
                console.error(`[RS/GlobalNick] attempt=${attempt + 1}:`, e);
            if (attempt < retries - 1) {
                await rsSleep(2000);
                if (!pluginActive)
                    return;
...
async function applyGuildPronoun(guildId, pronouns) {
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            await RestAPI.patch({ url: `/users/@me/guilds/${guildId}/profile`, body: { pronouns } });
            if (settings.store.enableLogs)
                console.log(`[RS/GuildPronouns] [${guildId}] -> "${pronouns}"`);
            return;
        }
        catch (e) {
            const st = e?.status ?? e?.response?.status ?? 0;
            if (st === 300) {
                ...
                if (settings.store.enableLogs)
                    console.warn(`[RS/GuildPronouns] 429 [${guildId}] retry ${ra}s`);
                await rsSleep(ra * 1000 + 300);
                if (!pluginActive)
                    return;
                continue;
            ...
            if (st === 403 || st === 404) {
                if (settings.store.enableLogs)
                    console.warn(`[RS/GuildPronouns] [${guildId}] HTTP ${st} - server pronouns not supported for this server`);
                return;
            }
            if (settings.store.enableLogs)
                console.error("[RS/GuildPronouns]:", e);
            return;
        }
    }
}
function tickBio() {
    if (!settings.store.profileBioEnabled || !bioEntries.length)
        return;
    const { val, next, lastPicked } = pickItem(bioEntries, bioSeqIdx, settings.store.bioRandomize, bioLastVal);
    bioSeqIdx = next;
    bioLastVal = lastPicked;
    if (val)
        patchProfile({ bio: val });
}
function scheduleBioLoop() {
    ...
    bioTimer = setTimeout(() => {
        ...
        tickBio();
        saveData();
        if (settings.store.profileBioEnabled)
            scheduleBioLoop();
    }, getMs(settings.store.bioIntervalSeconds));
}
function stopBioTimer() { if (bioTimer) {
    clearTimeout(bioTimer);
    bioTimer = null;
} }
function tickPronouns() {
    if (!settings.store.profilePronounsEnabled)
        return;
    ...
    prSeqIdx = next;
    prLastVal = lastPicked;
    if (val)
        patchProfile({ pronouns: val });
}
function schedulePronounsLoop() {
    if (pronounsTimer !== null)
        return;
    pronounsTimer = setTimeout(() => {
        ...
        saveData();
        if (settings.store.profilePronounsEnabled)
            schedulePronounsLoop();
    }, getMs(settings.store.pronounsIntervalSeconds));
}
function stopPronounsTimer() { if (pronounsTimer) {
    clearTimeout(pronounsTimer);
    pronounsTimer = null;
} }
function tickGlobalNick() {
    if (!settings.store.globalNickEnabled || !globalNickEntries.length)
        return;
    const now = Date.now();
    if (now - lastGlobalNickApply < GLOBAL_NICK_MIN_MS)
        return;
    ...
    globalNickSeqIdx = next;
    globalNickLastVal = lastPicked;
    if (val)
        applyGlobalNick(val);
}
function scheduleGlobalNickLoop() {
    if (globalNickTimer !== null)
        return;
    ...
    globalNickTimer = setTimeout(() => {
        ...
        tickGlobalNick();
        saveData();
        if (settings.store.globalNickEnabled)
            scheduleGlobalNickLoop();
    }, ms);
}
function stopGlobalNickTimer() { if (globalNickTimer) {
    clearTimeout(globalNickTimer);
    globalNickTimer = null;
} }
...
function stopGlobalTimer() { if (globalSyncTimer) {
    clearTimeout(globalSyncTimer);
    globalSyncTimer = null;
} }
function getMyVoiceGuildId() {
    try {
        const user = UserStore.getCurrentUser();
        if (!user)
            return null;
        if (!cachedVoiceStateStore)
            cachedVoiceStateStore = findByProps("getVoiceStateForUser", "getVoiceStatesForChannel");
        const state = cachedVoiceStateStore?.getVoiceStateForUser?.(user.id);
        if (!state?.channelId)
            return null;
        if (!cachedChannelStore)
            cachedChannelStore = findByProps("getChannel", "getDMFromUserId");
...
function onVoiceLeave(prevGuildId) {
    if (!pluginActive)
        return;
    if (settings.store.voiceActivateGlobal) {
        stopAllNicks();
        stopAllGuildPronouns();
        return;
    }
    if (!prevGuildId || prevGuildId === "DM")
        return;
    const g = guilds.find(x => x.id === prevGuildId && x.voiceActivated);
    if (g) {
        stopNickGuild(prevGuildId);
        stopGuildPronouns(prevGuildId);
    }
}
function startVoiceWatcher() {
    ...
    if (lastVoiceGuildId)
        onVoiceJoin(lastVoiceGuildId);
    voiceCheckInterval = setInterval(() => {
        if (!pluginActive)
            return;
        const curr = getMyVoiceGuildId();
        if (curr === lastVoiceGuildId)
            return;
        const prev = lastVoiceGuildId;
        lastVoiceGuildId = curr;
        if (curr && !prev)
            onVoiceJoin(curr);
        else if (!curr && prev)
            onVoiceLeave(prev);
...
function startAllRotators() {
    ...
    if (!pluginActive)
        return;
    if (isManualStop || wasInvisible)
        return;
    if (settings.store.avatarEnabled && arGetActive().length)
        arStartRotator(false);
    if (settings.store.clanEnabled)
        scheduleClanLoop();
    if (settings.store.globalSync) {
        globalTick();
        ...
        if (!settings.store.voiceActivateGlobal) {
            if (settings.store.nickEnabled)
                for (const g of guilds.filter(x => x.enabled && !x.voiceActivated))
                    startNickGuild(g);
            for (const g of guilds.filter(x => x.guildPronounsEnabled && !x.voiceActivated && pronounsForGuild(x).length > 0))
                startGuildPronouns(g);
        }
    }
    if (settings.store.voiceActivateEnabled)
        startVoiceWatcher();
    if (settings.store.bannerEnabled)
...
function doGlobalStop(durationMs) {
    isManualStop = true;
    stopAllRotators();
...
function getMyCurrentStatus() {
    try {
        if (!cachedPresenceStore)
            cachedPresenceStore = findByProps("getStatus", "getActivities");
        const user = UserStore.getCurrentUser();
        if (!user)
            return "unknown";
...
function startInvisibleWatcher() {
    if (invisibleWatchInterval !== null)
        return;
    wasInvisible = getMyCurrentStatus() === "invisible";
    invisibleWatchInterval = setInterval(() => {
        if (!pluginActive || !settings.store.stopOnInvisible)
            return;
...
function stopInvisibleWatcher() {
    if (invisibleWatchInterval) {
        clearInterval(invisibleWatchInterval);
        invisibleWatchInterval = null;
    }
    wasInvisible = false;
}
function startNitroWatcher() {
    stopNitroWatcher();
    if (!settings.store.bannerNitroCheck)
        return;
    let bcrWasStopped = false;
    const check = () => {
        if (!pluginActive || !settings.store.bannerNitroCheck)
            return;
        if (arHasNitro()) {
            if (!bcrWasStopped && bcrRotatorTimer !== null) {
                bcrWasStopped = true;
                bcrStopRotator();
                settings.store.bannerEnabled = false;
            ...
            if (bcrWasStopped) {
                bcrWasStopped = false;
                bcrStartRotator(false);
                settings.store.bannerEnabled = true;
    ...
    startNitroWatcher._cleanup = () => UserStore.removeChangeListener(check);
}
function startAvatarNitroWatcher() {
    stopAvatarNitroWatcher();
    if (!settings.store.avatarNitroCheck)
        return;
    const check = () => {
        if (!pluginActive || !settings.store.avatarNitroCheck)
            return;
    ...
    check();
    UserStore.addChangeListener(check);
    startAvatarNitroWatcher._cleanup = () => UserStore.removeChangeListener(check);
}
function stopAvatarNitroWatcher() {
    if (startAvatarNitroWatcher._cleanup) {
        startAvatarNitroWatcher._cleanup();
        delete startAvatarNitroWatcher._cleanup;
    }
}
function stopNitroWatcher() {
    if (startNitroWatcher._cleanup) {
        startNitroWatcher._cleanup();
        delete startNitroWatcher._cleanup;
    }
}
function SettingsSep({ title, color = "#9c67ff" }) {
    return (<div style={{ margin: "14px 0 2px", display: "flex", alignItems: "center", gap: 8 }}>
...
const settings = definePluginSettings({
    _sOpen: {
        type: 6 /* OptionType.COMPONENT */, description: "",
        component: () => (<div style={{ marginTop: 4 }}>
    ...
    _sSyncGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Master Sync" color={C.data}/> },
    globalSync: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Master Sync (configure in Data tab).", onChange: () => { if (pluginActive)
            startAllRotators(); } },
    globalSyncSeconds: { type: 0 /* OptionType.STRING */, default: "500", description: "Master Sync interval seconds (configure in Data tab)." },
    noDuplicateRandom: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "No-Duplicate Random (configure in Data tab)." },
    _sStatusGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Status" color={C.status}/> },
    statusEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Status rotator enabled (configure in Status tab)." },
    statusIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "10", description: "Status interval seconds (configure in Status tab)." },
    statusRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Status randomize (configure in Status tab)." },
    statusRunMode: { type: 0 /* OptionType.STRING */, default: "all", description: "Status run mode: all | presets | none (configure in Status tab)." },
    statusSelectedPresets: { type: 0 /* OptionType.STRING */, default: "[]", description: "JSON array of preset names to include when run mode is presets." },
    _sClanGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Clan" color={C.clan}/> },
    clanEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Clan switcher enabled (configure in Clan tab)." },
    clanIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "5", description: "Clan interval seconds (configure in Clan tab)." },
    clanAutoDetect: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Clan auto-detect (configure in Clan tab)." },
    clanAutoDetectRefreshSeconds: { type: 0 /* OptionType.STRING */, default: "429", description: "Clan auto-detect refresh seconds (configure in Clan tab)." },
    clanRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Clan randomize (configure in Clan tab)." },
    _sProfileGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Profile" color={C.bio}/> },
    globalNickEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Global display name rotation enabled (Profile tab)." },
    globalNickRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Randomize global display name rotation order (Profile tab)." },
    globalNickIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "429", description: "Seconds between display name changes. Discord rate-limits /users/@me - minimum enforced at 429." },
    profilePronounsEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Global pronouns rotation enabled (Profile tab)." },
    pronounsRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Randomize global pronouns rotation order (Profile tab)." },
    pronounsIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "30", description: "Seconds between global pronoun changes (Profile tab)." },
    profileBioEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Bio rotation enabled (Profile tab)." },
    bioRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Randomize bio rotation order (Profile tab)." },
    bioIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "60", description: "Seconds between bio changes (Profile tab)." },
    _sAvatarGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Avatar" color="#9c67ff"/> },
    avatarEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Avatar rotator enabled (configure in Avatar tab)." },
    avatarIntervalSeconds: { type: 1 /* OptionType.NUMBER */, default: AR_DEFAULT_S, description: "Seconds between avatar changes (min 60 recommended - Discord rate-limits)." },
    avatarRandom: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Random avatar order - no repeats until all shown once." },
    avatarShowToast: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Show toast notifications for avatar changes." },
    avatarNitroCheck: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Skip GIF avatars automatically if no Nitro detected. Toggle in the Avatar tab." },
    avatarExcludedExtensions: { type: 0 /* OptionType.STRING */, default: "", description: "Comma-separated extensions to skip during avatar rotation (e.g. gif,avif)." },
    _sBannerGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Banner Color" color="#c084fc"/> },
    bannerEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Banner color rotator enabled (configure in Banner tab)." },
    bannerIntervalSeconds: { type: 1 /* OptionType.NUMBER */, default: BCR_DEFAULT_S, description: "Seconds between banner color changes." },
    bannerMode: { type: 0 /* OptionType.STRING */, default: "full_random", description: "Banner color cycle mode (configure in Banner tab)." },
    bannerHueRadius: { type: 1 /* OptionType.NUMBER */, default: 35, description: "Hue spread for avatar-hue based modes (1-180)." },
    bannerCustomBaseColor: { type: 0 /* OptionType.STRING */, default: "#c084fc", description: "Base color for shade/mono modes (configure in Banner tab)." },
    bannerShowToast: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Show toast on each banner color change." },
    bannerShowCurrentColor: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Show active color hex+swatch in the footer next to ColorBanner (updates live)." },
    bannerNitroCheck: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Auto-pause ColorBanner cycle when Nitro is detected (you can use animated banners instead). Toggle in the Banner tab." },
    _sServerProfilesGroup: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Server Profiles" color={C.nick}/> },
    nickEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Server nicknames master switch - when OFF, no nick timers run even if servers are toggled on." },
    nickIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "30", description: "Seconds between server nickname changes (Server Profiles tab)." },
    nickRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Randomize server nickname order (Server Profiles tab)." },
    serverPronounsEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Server pronouns master switch - when OFF, no server pronoun timers run." },
    serverPronounsRandomize: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Randomize server pronoun order (Server Profiles tab)." },
    serverPronounsIntervalSeconds: { type: 0 /* OptionType.STRING */, default: "30", description: "Seconds between server pronoun changes (Server Profiles tab)." },
    _sVoice: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Voice Activation" color="#7986cb"/> },
    voiceActivateEnabled: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Enable voice-based activation (configure in Server Profiles tab).", onChange: v => { if (pluginActive) {
            stopVoiceWatcher();
            if (v)
                startVoiceWatcher();
            else
    ...
    voiceActivateGlobal: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Global: activate ALL server nick+pronoun rotators when in any voice/call. Overrides per-server." },
    _sMisc: { type: 6 /* OptionType.COMPONENT */, description: "", component: () => <SettingsSep title="Misc" color={C.hint}/> },
    showButton: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Show the Rotator Suite button in the user area (bottom-left)." },
    autoStart: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Auto-start all enabled rotators when Discord loads." },
    enableLogs: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Print rotator activity and errors to the console (F12)." },
    stopOnInvisible: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Pause all rotators automatically when status is set to invisible." },
});
function injectCSS() {
...
function useDrag(onReorder) {
    const dragRef = React.useRef(null);
    const [overIdx, setOverIdx] = React.useState(null);
    const props = (i) => ({
        draggable: true,
        onDragStart: (e) => { dragRef.current = i; e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", String(i)); },
        onDragOver: (e) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; setOverIdx(prev => prev !== i ? i : prev); },
        onDrop: (e) => { e.preventDefault(); const from = dragRef.current; if (from !== null && from !== i)
            onReorder(from, i); dragRef.current = null; setOverIdx(null); },
        onDragEnd: (e) => { e.preventDefault(); dragRef.current = null; setOverIdx(null); },
        onDragLeave: () => { setOverIdx(prev => prev === i ? null : prev); },
    });
    const cls = (i, base) => `${base}${overIdx === i && dragRef.current !== i ? " rs-over" : ""}${dragRef.current === i ? " rs-dragging" : ""}`;
    return { props, cls };
}
function Hdr({ label, color, count }) {
    return (<div className="rs-sec-row" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
            <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color }}>{label}</span>
            {count !== undefined && <span className="rs-count">{count}</span>}
        </div>);
}
function ConfirmBox({ msg, onConfirm, onCancel }) {
    return (<div className="rs-confirm-box">
            <span>{msg}</span>
...
function PanelToggle({ label, description, value, color, onChange, compact }) {
...
function PanelInterval({ label, description, storeKey, onApply, disabled }) {
    const [val, setVal] = React.useState(String(settings.store[storeKey]));
    const commit = () => {
        const n = Math.max(1, parseFloat(val) || 1);
        settings.store[storeKey] = String(n);
        setVal(String(n));
        onApply?.();
    };
    return (<div style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 10px", borderRadius: 7, border: "1px solid rgba(124,77,255,.18)", background: "rgba(20,5,50,.45)", marginBottom: 4, opacity: disabled ? .45 : 1 }}>
            <div style={{ flex: 1 }}>
            ...
            <input value={val} onChange={e => setVal(e.target.value)} onBlur={commit} onKeyDown={(e) => { if (e.key === "Enter")
        commit(); }} disabled={disabled} style={{ width: 52, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 12, padding: "2px 6px", outline: "none", textAlign: "center", fontFamily: "monospace" }}/>
...
function EvalSnippetPanel({ setDraft }) {
    ...
    return (<div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 2, marginBottom: 5 }}>
                {EVAL_SNIPPETS.map((s, ci) => (<button key={s.cat} style={btnStyle(cat === ci)} onClick={() => setCat(ci)}>{s.cat}</button>))}
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
                {EVAL_SNIPPETS[cat].items.map(([label, snippet]) => (<button key={label} title={snippet} onClick={() => setDraft(snippet)} style={{ display: "inline-flex", alignItems: "center", margin: "2px 2px 2px 0", padding: "2px 8px", borderRadius: 5, border: "1px solid rgba(255,167,38,.3)", background: "rgba(255,167,38,.1)", color: "#faa61a", cursor: "pointer", fontSize: 10, fontWeight: 600, outline: "none" }}>
                        {label}
                    </button>))}
            </div>
            <div style={{ marginTop: 6, borderTop: "1px solid rgba(255,167,38,.15)", paddingTop: 5 }}>
                <button onClick={() => setBuilderOpen(o => !o)} style={{ fontSize: 10, fontWeight: 800, color: builderOpen ? "#ffd580" : "#c8a050", background: "none", border: "none", cursor: "pointer", outline: "none", padding: 0 }}>
                    {builderOpen ? "▾" : "▸"} Text builder - left text | dynamic | right text
                </button>
                {builderOpen && (<div style={{ marginTop: 5, display: "flex", flexDirection: "column", gap: 5 }}>
                        <div style={{ fontSize: 10, color: "#9e9e9e" }}>
                            Write text around a dynamic part. Example: <span style={{ fontFamily: "monospace", color: "#faa61a", userSelect: "all" }}>🖤 ... | HH:MM:SS</span>
                        </div>
                        <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
                            <input value={bLeft} onChange={e => setBLeft(e.target.value)} placeholder="text before..." style={{ flex: 1, minWidth: 80, background: "rgba(10,0,30,.7)", border: "1px solid rgba(255,167,38,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
                            <select value={bToken} onChange={e => setBToken(Number(e.target.value))} style={{ background: "rgba(10,0,30,.85)", border: "1px solid rgba(255,167,38,.4)", borderRadius: 5, color: "#ffa726", fontSize: 11, padding: "3px 6px", outline: "none" }}>
                                {EVAL_TOKENS.map((t, i) => <option key={t.label} value={i}>{t.label}</option>)}
                            </select>
                            <input value={bRight} onChange={e => setBRight(e.target.value)} placeholder="text after..." style={{ flex: 1, minWidth: 80, background: "rgba(10,0,30,.7)", border: "1px solid rgba(255,167,38,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
                        </div>
                        <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                            <span style={{ flex: 1, fontSize: 10, color: "#9e9e9e", fontFamily: "monospace", wordBreak: "break-all", userSelect: "all" }}>{buildExpr()}</span>
                            <button onClick={() => setDraft(buildExpr())} style={{ padding: "3px 11px", borderRadius: 5, border: "1px solid rgba(255,167,38,.5)", background: "rgba(255,167,38,.18)", color: "#ffd580", cursor: "pointer", fontSize: 10, fontWeight: 800, outline: "none", flexShrink: 0 }}>
                                Insert ↑
                            </button>
                        </div>
                    </div>)}
...
function StatusLivePreview({ entry, statusType }) {
    const user = UserStore.getCurrentUser();
    const dot = STATUS_OPTIONS.find(s => s.value === statusType)?.color ?? "#23a55a";
    return (<div className="rs-status-preview">
            <div style={{ position: "relative", flexShrink: 0 }}>
                <img className="rs-preview-avatar" src={user ? getAvatarUrl(user.id, user.avatar) : undefined} alt=""/>
...
function StatusTypeSelector({ value, onChange }) {
    const hint = STATUS_OPTIONS.find(s => s.value === value)?.hint ?? "";
    return (<div>
            <div className="rs-status-type-row">
                {STATUS_OPTIONS.map(s => (<button key={s.value} title={s.hint} className={`rs-status-type-btn${value === s.value ? " active" : ""}`} style={value === s.value ? { color: s.color, borderColor: s.color } : {}} onClick={() => onChange(s.value)}>
...
function ClearAfterSelector({ value, onChange }) {
    ...
    return (<div style={{ display: "flex", alignItems: "center", gap: 6, margin: "4px 0" }}>
            ...
            <input value={raw} onChange={e => setRaw(e.target.value)} onBlur={commit} onKeyDown={(e) => { if (e.key === "Enter")
        commit(); }} placeholder="0 = never" style={{ width: 72, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "2px 6px", outline: "none", fontFamily: "monospace" }}/>
            <span style={{ fontSize: 11, color: "#757575" }}>min</span>
        </div>);
}
function StatusRunModeSelector({ presets, forceUpdate }) {
    const mode = settings.store.statusRunMode;
    const getSel = () => { try {
        return JSON.parse(settings.store.statusSelectedPresets || "[]");
    }
    catch {
        return [];
    } };
    const setSel = (v) => { settings.store.statusSelectedPresets = JSON.stringify(v); forceUpdate(); };
    const selected = getSel();
    const setMode = (m) => {
        settings.store.statusRunMode = m;
        if (pluginActive) {
            stopStatusTimer();
            if (settings.store.statusEnabled && m !== "none" && !settings.store.globalSync)
                scheduleStatusLoop();
        }
        forceUpdate();
    };
    const togglePreset = (name) => {
        ...
        setSel(next);
        if (pluginActive && settings.store.statusEnabled && !settings.store.globalSync) {
            stopStatusTimer();
            scheduleStatusLoop();
        }
    };
    ...
    return (<div style={{ marginBottom: 4 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: C.status, marginBottom: 5 }}>
                Run Mode - pool: <span style={{ color: pool > 0 ? C.enabled : C.del }}>{pool} entries</span>
            </div>
            <div className="rs-run-mode-row">
                {MODES.map(m => (<button key={m.key} className={`rs-run-mode-btn${mode === m.key ? ` ${m.cls}` : ""}`} onClick={() => setMode(m.key)}>
                        {m.label}
                    </button>))}
            </div>
            <div className="rs-run-mode-hint">{activeHint}</div>
            {mode === "presets" && (<div className="rs-preset-check-row">
                ...
                return (<div key={p.id} className="rs-preset-check-item" onClick={() => togglePreset(p.name)}>
                    ...
                    {presets.length > 0 && (<div style={{ display: "flex", gap: 6, marginTop: 4, paddingTop: 4, borderTop: "1px solid rgba(124,77,255,.15)" }}>
                            <button className="rs-btn" style={{ background: "rgba(124,77,255,.25)", fontSize: 10, padding: "2px 9px" }} onClick={() => setSel(presets.map(p => p.name))}>Select All</button>
                            <button className="rs-btn" style={{ background: "rgba(80,60,110,.25)", fontSize: 10, padding: "2px 9px" }} onClick={() => setSel([])}>Clear</button>
                        </div>)}
                </div>)}
        </div>);
}
function OnCloseStatusPanel({ forceUpdate }) {
    ...
    const [type, setType] = React.useState(closeStatusType);
    const save = () => {
        closeStatusText = text.trim();
        closeStatusEmoji = emoji.trim();
        closeStatusType = type;
        void saveCloseConfig();
        forceUpdate();
    };
    return (<div>
            <PanelToggle label="On-Close Status" description="Apply a fixed status when Discord closes (beforeunload - not fired on crash/kill)" value={enabled} color="#64b5f6" onChange={v => { closeStatusEnabled = v; setEnabled(v); void saveCloseConfig(); forceUpdate(); }}/>
            {enabled && (<div style={{ display: "flex", flexDirection: "column", gap: 5, padding: "7px 10px", border: "1px solid rgba(100,181,246,.2)", borderRadius: 7, background: "rgba(10,20,50,.5)", marginTop: 3 }}>
                    <StatusTypeSelector value={type} onChange={v => { setType(v); closeStatusType = v; void saveCloseConfig(); }}/>
                    <div style={{ display: "flex", gap: 6 }}>
                        <input value={emoji} onChange={e => setEmoji(e.target.value)} onBlur={save} placeholder="Emoji (opt.)" style={{ width: 110, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
                        <input value={text} onChange={e => setText(e.target.value)} onBlur={save} onKeyDown={(e) => { if (e.key === "Enter")
            save(); }} placeholder="Status text..." style={{ flex: 1, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none" }}/>
                    </div>
...
function StatusTab({ forceUpdate }) {
    const [draft, setDraft] = React.useState("");
    const [draftStatusType, setDraftStatusType] = React.useState("online");
    ...
    const [editStatusType, setEditStatusType] = React.useState("online");
    const [editPreset, setEditPreset] = React.useState("");
    const [editClearAfter, setEditClearAfter] = React.useState(0);
    const [confirm, setConfirm] = React.useState(false);
    const [filterPreset, setFilterPreset] = React.useState(null);
    ...
    function add() {
        ...
        if (!parsed.text && !parsed.emojiId && !parsed.emojiName)
            return;
    ...
    function startEdit(i) {
        ...
        setEditIdx(i);
        setEditText(raw.trim());
        setEditStatusType(e.status ?? "online");
        setEditPreset(e.preset ?? "");
        setEditClearAfter(e.clearAfter ?? 0);
    }
    function saveEdit(i) {
    ...
    function addPreset() {
        ...
        if (!name)
            return;
        if (statusPresets.some(p => p.name.toLowerCase() === name.toLowerCase()))
            return;
    ...
    function removePreset(id) {
        ...
        if (pName)
            statusEntries = list.map(e => e.preset === pName ? { ...e, preset: undefined } : e);
        if (filterPreset === pName)
            setFilterPreset(null);
        saveData();
        forceUpdate();
    }
    function applyPresetClearAfter(presetName, ms) {
    ...
    return (<div>
            <div className="rs-settings-panel">
                <PanelToggle label="Enabled" description="Automatically cycle your Discord custom status" value={settings.store.statusEnabled} color={C.status} onChange={v => { settings.store.statusEnabled = v; if (pluginActive) {
        stopStatusTimer();
        if (v && !settings.store.globalSync)
            scheduleStatusLoop();
    } }}/>
                <PanelToggle label="Randomize" description="Pick randomly instead of cycling in order" value={settings.store.statusRandomize} onChange={v => { settings.store.statusRandomize = v; }}/>
                <PanelInterval label="Interval" description="Seconds between status changes (ignored when Master Sync is ON)" storeKey="statusIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.statusEnabled && !settings.store.globalSync) {
        stopStatusTimer();
        scheduleStatusLoop();
    } }}/>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: C.status }}>Add Status</span>
            </div>
            <TextInput value={draft} onChange={setDraft} placeholder="Status text... or Discord emoji <:name:id> + text" onKeyDown={(e) => { if (e.key === "Enter" && e.ctrlKey) {
        e.preventDefault();
        add();
    } }}/>
            <div style={{ margin: "4px 0 6px", padding: "7px 10px", borderRadius: 7, border: "1px solid rgba(255,167,38,.22)", background: "rgba(255,167,38,.04)", userSelect: "text" }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: "#faa61a", marginBottom: 4, letterSpacing: ".5px", textTransform: "uppercase" }}>
                    ⚡ eval prefix - dynamic JS
                </div>
                <div style={{ fontSize: 10, color: "#9e9e9e", marginBottom: 5 }}>
                    Prefix with <span style={{ fontFamily: "monospace", background: "rgba(255,167,38,.2)", padding: "0px 5px", borderRadius: 3, color: "#ffd580", userSelect: "all", letterSpacing: 1 }}>eval&nbsp;</span> to evaluate JS live. Click snippet to insert:
                </div>
            ...
            {presets.length > 0 && (<div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                    <span style={{ fontSize: 11, color: C.hint }}>Preset:</span>
                    <select value={draftPreset} onChange={e => setDraftPreset(e.target.value)} style={{ flex: 1, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.3)", borderRadius: 6, color: "#f0eaff", fontSize: 11, padding: "2px 6px", outline: "none" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "7px 0 5px" }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: C.status }}>
                    Preset Groups
                </span>
                <span className="rs-count">{presets.length}</span>
            </div>
            <div className="rs-preset-pill-row">
                <button className="rs-preset-tag" style={{ cursor: "pointer", opacity: filterPreset === null ? 1 : .5 }} onClick={() => setFilterPreset(null)}>All ({list.length})</button>
                {presets.map(p => {
            const cnt = list.filter(e => e.preset === p.name).length;
            return (<span key={p.id} style={{ display: "flex", alignItems: "center", gap: 3 }}>
                            <button className="rs-preset-tag" style={{ cursor: "pointer", opacity: filterPreset === p.name ? 1 : .6 }} onClick={() => setFilterPreset(filterPreset === p.name ? null : p.name)}>
                                {p.name} ({cnt})
                            </button>
            ...
            {presets.length > 0 && (<div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 6, padding: "6px 8px", border: "1px solid rgba(124,77,255,.18)", borderRadius: 7, background: "rgba(10,0,25,.4)" }}>
                    <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".6px", color: C.hint, marginBottom: 2 }}>Bulk clear after (per preset)</span>
                    {presets.map(p => {
                ...
                return (<div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                ...
                                <input value={rawMin} onChange={e => setRawMin(e.target.value)} onBlur={commit} onKeyDown={(e) => { if (e.key === "Enter")
                    commit(); }} placeholder="0 = never" style={{ width: 72, background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "2px 6px", outline: "none", fontFamily: "monospace" }}/>
                                <span style={{ fontSize: 11, color: "#757575" }}>min</span>
            ...
            <div className="rs-add-preset-row">
                <TextInput value={newPresetName} onChange={setNewPresetName} placeholder="New preset name..." onKeyDown={(e) => { if (e.key === "Enter")
        addPreset(); }}/>
            ...
            return (<div key={`st_${i}`} {...dProps(fi)} className={cls(fi, "rs-item")}>
                    ...
                    : entry.emojiName
                        ...
                        : null}
                        {isEdit ? (<div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
                                <input autoFocus className="rs-item-input" value={editText} onChange={e => setEditText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter")
                    saveEdit(i); if (e.key === "Escape")
                    setEditIdx(null); }}/>
                                ...
                                {presets.length > 0 && (<select value={editPreset} onChange={e => setEditPreset(e.target.value)} style={{ background: "rgba(10,0,30,.7)", border: "1px solid rgba(124,77,255,.3)", borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "2px 5px", outline: "none" }}>
                                ...
                                {(entry.text?.startsWith("eval ") || entry.emojiName?.startsWith("eval ")) && (<span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 6, background: "rgba(255,167,38,.15)", color: "#ffa726", fontWeight: 800, border: "1px solid rgba(255,167,38,.3)", flexShrink: 0 }}>EVAL</span>)}
                                <span className="rs-entry-status-dot" style={{ background: dot }} title={entry.status ?? "online"}/>
                                {entry.clearAfter && entry.clearAfter > 0 && (<span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 6, background: "rgba(66,165,245,.12)", color: "#64b5f6", fontWeight: 800, border: "1px solid rgba(66,165,245,.25)", flexShrink: 0 }}>
...
function OnCloseClanPanel({ forceUpdate }) {
    ...
    const resolved = React.useMemo(() => {
        ...
        try {
            ...
            if (g) {
                ...
                const tag = (storeTag ?? domTagCache.get(v) ?? clanServerNames[v]?.tag ?? null);
                return { name: g.name, tag };
            }
    ...
    return (<div>
            <PanelToggle label="On-Close Clan" description="Switch to a specific clan server when Discord closes (beforeunload - not fired on crash/kill)" value={enabled} color={C.clan} onChange={v => { closeClanEnabled = v; setEnabled(v); void saveCloseConfig(); forceUpdate(); }}/>
            {enabled && (<div style={{ display: "flex", flexDirection: "column", gap: 5, padding: "7px 10px", border: `1px solid ${C.clan}33`, borderRadius: 7, background: "rgba(10,20,50,.5)", marginTop: 3 }}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        <span style={{ fontSize: 11, color: C.hint, flexShrink: 0 }}>Clan Server ID:</span>
                        <input value={id} onChange={e => setId(e.target.value)} onBlur={save} onKeyDown={(e) => { if (e.key === "Enter")
            save(); }} placeholder="Server ID (17-20 digits)..." style={{ flex: 1, background: "rgba(10,0,30,.7)", border: `1px solid ${C.clan}44`, borderRadius: 5, color: "#f0eaff", fontSize: 11, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
                    </div>
                    {id.trim() && !/^\d{17,20}$/.test(id.trim()) && (<div style={{ fontSize: 10, color: "#ef9a9a" }}>⚠ Invalid ID - must be 17-20 digits.</div>)}
                    {resolved && (<div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 8px", borderRadius: 5, background: `${C.clan}10`, border: `1px solid ${C.clan}30` }}>
                    ...
                    {id.trim() && /^\d{17,20}$/.test(id.trim()) && !resolved && (<div style={{ fontSize: 10, color: "#faa61a", opacity: .8 }}>⚠ Server not found in your guild list or cache.</div>)}
...
function updateDomTagCache() {
    try {
        document.querySelectorAll('img[src*="/clan-badges/"]').forEach(img => {
            const m = img.src.match(/clan-badges\/(\d{17,20})\//);
            if (!m)
                return;
            const id = m[1];
            if (domTagCache.has(id))
                return;
...
function ClanTab({ forceUpdate }) {
    ...
    function add() {
        const v = input.trim();
        if (!v || !/^\d{17,20}$/.test(v) || clanIds.includes(v))
            return;
        const g = allDiscordGuilds.find(x => x.id === v);
        if (g)
            clanServerNames[v] = { name: g.name, tag: g.tag };
        clanIds = [...clanIds, v];
        saveData();
        setInput("");
        forceUpdate();
    }
    function remove(id) { clanIds = clanIds.filter(c => c !== id); clanSeqIdx = 0; clanLastVal = null; saveData(); forceUpdate(); }
    function saveEdit(i) {
    ...
    const allDiscordGuilds = React.useMemo(() => {
        updateDomTagCache();
        try {
            const raw = Object.values(getGuildStore()?.getGuilds?.() ?? {});
            const result = raw.map((g) => {
                const storeTag = g.clan?.tag ?? g.clanTag ?? g.clan?.identity_tag ?? g.clan?.identityTag ?? null;
                const tag = (storeTag ?? domTagCache.get(g.id) ?? null);
                return { id: g.id, name: g.name, tag };
            });
            for (const g of result) {
                if (clanIds.includes(g.id))
                    clanServerNames[g.id] = { name: g.name, tag: g.tag };
            }
            return result;
        }
        catch {
    ...
    return (<div>
            <div className="rs-settings-panel">
                <PanelToggle label="Enabled" description="Rotate your visible clan badge through server IDs" value={settings.store.clanEnabled} color={C.clan} onChange={v => { settings.store.clanEnabled = v; if (pluginActive) {
        stopClanTimer();
        if (v)
            scheduleClanLoop();
    } }}/>
                <PanelToggle label="Randomize" description="Pick clan randomly instead of in order" value={settings.store.clanRandomize} onChange={v => { settings.store.clanRandomize = v; }}/>
                <PanelToggle label="Auto-Detect" description="Automatically cycle through all your joined servers" value={settings.store.clanAutoDetect} onChange={v => { settings.store.clanAutoDetect = v; cachedClanGuilds = []; lastClanFetch = 0; forceUpdate(); }}/>
                <PanelInterval label="Interval" description="Seconds between clan changes (always independent timer)" storeKey="clanIntervalSeconds" onApply={() => { if (pluginActive && settings.store.clanEnabled) {
        stopClanTimer();
        scheduleClanLoop();
    } }}/>
                {settings.store.clanAutoDetect && (<PanelInterval label="Auto-Detect Refresh" description="How often to re-fetch your server list (seconds)" storeKey="clanAutoDetectRefreshSeconds"/>)}
                <div className="rs-divider" style={{ margin: "6px 0" }}/>
                <OnCloseClanPanel forceUpdate={forceUpdate}/>
            </div>
            <div className="rs-divider" style={{ margin: "8px 0" }}/>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: C.clan }}>Clan IDs</span>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {autoDetect ? (<div style={{ padding: "8px 10px", border: "1px solid rgba(66,165,245,.2)", borderRadius: 8, marginBottom: 8, background: "rgba(10,20,50,.6)" }}>
                    ...
                    <div style={{ maxHeight: 200, overflowY: "auto" }}>
                        ...
                        {browserGuilds.map(g => (<div className="rs-item rs-item-compact" key={g.id} style={{ marginBottom: 2 }}>
                                ...
                                <span style={{ fontSize: 9, color: "#5a7a9a", fontFamily: "monospace", flexShrink: 0, userSelect: "text", cursor: "text" }}>{g.id}</span>
                            </div>))}
                    </div>
                </div>) : (<>
                ...
                return (<div key={id} {...dProps(i)} className={cls(i, "rs-item")}>
                                <span className="rs-drag">⠿</span>
                                {editIdx === i
                        ? <input autoFocus className="rs-item-input" value={editVal} onChange={e => setEditVal(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter")
                            saveEdit(i); if (e.key === "Escape")
                            setEditIdx(null); }} onBlur={() => saveEdit(i)}/>
                        : <span className="rs-item-mono" onClick={() => { setEditIdx(i); setEditVal(id); }}>{id}</span>}
                                <span style={{ display: "flex", alignItems: "center", gap: 3, flexShrink: 0, maxWidth: 220, overflow: "hidden" }}>
                                    ...
                                    {displayName
                        ? <span style={{ fontSize: 10, color: inGuild ? C.text : "#faa61a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 160 }} title={displayName}>{displayName}</span>
                    ...
                    <div className="rs-row" style={{ marginTop: 6 }}>
                        <TextInput value={input} onChange={setInput} placeholder="Server ID (17-20 digits)..." onKeyDown={(e) => { if (e.key === "Enter")
            add(); }}/>
            ...
            {!autoDetect && (<div style={{ marginTop: 10 }}>
                    <button onClick={() => { setShowBrowser(!showBrowser); setBrowserFilter(""); }} style={{ display: "flex", alignItems: "center", gap: 6, width: "100%", padding: "7px 12px", borderRadius: 8, border: `1px solid ${showBrowser ? C.clan + "55" : "rgba(66,165,245,.2)"}`, background: showBrowser ? `${C.clan}14` : "rgba(10,20,50,.5)", color: showBrowser ? C.clan : "#5a7a9a", cursor: "pointer", fontSize: 11, fontWeight: 800, textAlign: "left" }}>
                    ...
                    {showBrowser && (<div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 8, border: `1px solid ${C.clan}33`, background: "rgba(5,15,40,.7)" }}>
                    ...
                    return (<div key={g.id} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 6px", borderRadius: 6, marginBottom: 2, background: inList ? `${ACT}0e` : "rgba(255,255,255,.02)", border: `1px solid ${inList ? ACT + "33" : "transparent"}` }}>
                                            <div style={{ width: 6, height: 6, borderRadius: "50%", background: colorFor(g.id), flexShrink: 0 }} title={g.id}/>
                                            <span style={{ flex: 1, fontSize: 11, color: inList ? ACT : C.text, fontWeight: inList ? 700 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={g.name}>{g.name}</span>
                                            {g.tag
                            ...
                            : <span style={{ fontSize: 9, color: "#5a7a9a", fontWeight: 600, flexShrink: 0, opacity: 0.6 }}>?tag</span>}
                                            <span style={{ fontSize: 9, color: "#5a7a9a", fontFamily: "monospace", flexShrink: 0, userSelect: "all", cursor: "text", padding: "1px 4px", borderRadius: 3, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)" }} title="Click to select ID">{g.id}</span>
                                            {inList
                            ? <button onClick={() => { clanIds = clanIds.filter(c => c !== g.id); clanSeqIdx = 0; clanLastVal = null; saveData(); forceUpdate(); }} style={{ padding: "2px 8px", borderRadius: 5, border: `1px solid ${INACT}44`, background: `${INACT}18`, color: INACT, cursor: "pointer", fontSize: 10, fontWeight: 800, flexShrink: 0 }}>✕</button>
                            : <button onClick={() => { clanIds = [...clanIds, g.id]; saveData(); forceUpdate(); }} style={{ padding: "2px 8px", borderRadius: 5, border: `1px solid ${C.clan}44`, background: `${C.clan}18`, color: C.clan, cursor: "pointer", fontSize: 10, fontWeight: 800, flexShrink: 0 }}>+</button>}
                                        </div>);
                })}
                            </div>
                        </div>)}
...
function RndBtn({ value, color, onChange }) {
...
function SectionHeader({ label, color, count, enabled, onToggleEnabled, rndValue, onToggleRnd, enableColor }) {
    ...
    return (<div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            ...
            <button onClick={() => onToggleEnabled(!enabled)} style={{
...
function ProfileTab({ forceUpdate }) {
    ...
    function addBio() { const v = bioDraft.trim(); if (!v)
        return; bioEntries = [...bioEntries, v]; saveData(); setBioDraft(""); forceUpdate(); }
    function removeBio(i) { bioEntries = bioEntries.filter((_, j) => j !== i); if (bioEditIdx === i)
        setBioEditIdx(null); bioSeqIdx = 0; bioLastVal = null; saveData(); forceUpdate(); }
    function saveBioEdit(i) { const v = bioEditVal.trim(); if (!v) {
        setBioEditIdx(null);
        return;
    } bioEntries = [...bioEntries]; bioEntries[i] = v; saveData(); setBioEditIdx(null); forceUpdate(); }
    function addPronoun() { const v = prDraft.trim(); if (!v || prList.includes(v))
        return; pronounsList = [...prList, v].join("§"); saveData(); setPrDraft(""); forceUpdate(); }
    function removePronoun(i) { pronounsList = prList.filter((_, j) => j !== i).join("§"); prSeqIdx = 0; prLastVal = null; saveData(); forceUpdate(); }
    function savePrEdit(i) { const v = prEditVal.trim(); if (!v) {
        setPrEditIdx(null);
        return;
    } const n = [...prList]; n[i] = v; pronounsList = n.join("§"); saveData(); setPrEditIdx(null); forceUpdate(); }
    function addGn() { const v = gnDraft.trim(); if (!v || globalNickEntries.includes(v))
        return; globalNickEntries = [...globalNickEntries, v]; saveData(); setGnDraft(""); forceUpdate(); }
    function removeGn(i) { globalNickEntries = globalNickEntries.filter((_, j) => j !== i); globalNickSeqIdx = 0; globalNickLastVal = null; saveData(); forceUpdate(); }
    function saveGnEdit(i) { const v = gnEditVal.trim(); if (!v) {
        setGnEditIdx(null);
        return;
    } globalNickEntries = [...globalNickEntries]; globalNickEntries[i] = v; saveData(); setGnEditIdx(null); forceUpdate(); }
    return (<div>

            <div className="rs-card">
                <SectionHeader label="Global Display Name" color={C.nick} count={globalNickEntries.length} enabled={settings.store.globalNickEnabled} onToggleEnabled={v => { settings.store.globalNickEnabled = v; if (pluginActive) {
        stopGlobalNickTimer();
        if (v && !settings.store.globalSync)
            scheduleGlobalNickLoop();
    } forceUpdate(); }} rndValue={settings.store.globalNickRandomize} onToggleRnd={v => { settings.store.globalNickRandomize = v; forceUpdate(); }} enableColor={C.nick}/>
                <PanelInterval label="Display Name Interval" description="Seconds between display name changes. Min enforced: 429s. Uses /users/@me global_name - separate endpoint from bio/pronouns." storeKey="globalNickIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.globalNickEnabled && !settings.store.globalSync) {
        stopGlobalNickTimer();
        scheduleGlobalNickLoop();
    } }}/>
                ...
                {globalNickEntries.map((n, i) => (<div key={`gn_${i}_${n}`} {...gnDProps(i)} className={gnCls(i, "rs-item rs-item-compact")}>
                        <span className="rs-drag">⠿</span>
                        {gnEditIdx === i
                ? <input autoFocus className="rs-item-input" value={gnEditVal} onChange={e => setGnEditVal(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter")
                    saveGnEdit(i); if (e.key === "Escape")
                    setGnEditIdx(null); }} onBlur={() => saveGnEdit(i)} maxLength={32}/>
                ...
                <div className="rs-row" style={{ marginTop: 5 }}>
                    <TextInput value={gnDraft} onChange={(v) => setGnDraft(v.slice(0, 32))} placeholder="Add display name (max 32)..." onKeyDown={(e) => { if (e.key === "Enter")
        addGn(); }}/>
            ...
            <div className="rs-card" style={{ marginBottom: 8 }}>
                <SectionHeader label="Global Pronouns" color={C.pronoun} count={prList.length} enabled={settings.store.profilePronounsEnabled} onToggleEnabled={v => { settings.store.profilePronounsEnabled = v; if (pluginActive) {
        stopPronounsTimer();
        if (v && !settings.store.globalSync)
            schedulePronounsLoop();
    } forceUpdate(); }} rndValue={settings.store.pronounsRandomize} onToggleRnd={v => { settings.store.pronounsRandomize = v; forceUpdate(); }} enableColor={C.pronoun}/>
                <PanelInterval label="Pronouns Interval" description="Seconds between global pronoun changes (ignored when Master Sync is ON)" storeKey="pronounsIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.profilePronounsEnabled && !settings.store.globalSync) {
        stopPronounsTimer();
        schedulePronounsLoop();
    } }}/>
                ...
                {prList.map((p, i) => (<div key={`pr_${i}_${p}`} {...prDProps(i)} className={prCls(i, "rs-item rs-item-compact")}>
                        <span className="rs-drag">⠿</span>
                        {prEditIdx === i
                ? <input autoFocus className="rs-item-input" value={prEditVal} maxLength={40} onChange={e => setPrEditVal(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter")
                    savePrEdit(i); if (e.key === "Escape")
                    setPrEditIdx(null); }} onBlur={() => savePrEdit(i)}/>
                : <span className="rs-item-text" style={{ flex: 1, fontSize: 12, color: C.pronoun, fontWeight: 600 }} onClick={() => { setPrEditIdx(i); setPrEditVal(p); }} onDoubleClick={() => { setPrEditIdx(i); setPrEditVal(p); }}>{p}</span>}
                ...
                <div className="rs-row" style={{ marginTop: 5 }}>
                    <TextInput value={prDraft} onChange={setPrDraft} placeholder="Add pronoun (e.g. he/him)..." onKeyDown={(e) => { if (e.key === "Enter")
        addPronoun(); }}/>
            ...
            <div className="rs-card" style={{ marginBottom: 8 }}>
                <SectionHeader label="Bio" color={C.bio} count={bioEntries.length} enabled={settings.store.profileBioEnabled} onToggleEnabled={v => { settings.store.profileBioEnabled = v; if (pluginActive) {
        stopBioTimer();
        if (v && !settings.store.globalSync)
            scheduleBioLoop();
    } forceUpdate(); }} rndValue={settings.store.bioRandomize} onToggleRnd={v => { settings.store.bioRandomize = v; forceUpdate(); }} enableColor={C.bio}/>
                <PanelInterval label="Bio Interval" description="Seconds between bio changes (ignored when Master Sync is ON)" storeKey="bioIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.profileBioEnabled && !settings.store.globalSync) {
        stopBioTimer();
        scheduleBioLoop();
    } }}/>
                ...
                <div className="rs-bio-list">
                    ...
                    {bioEntries.map((e, i) => (<div key={`bio_${i}_${e.slice(0, 8)}`} {...bioDProps(i)} className={bioCls(i, `rs-bio-item${bioEditIdx === i ? " editing" : ""}`)}>
                            <span className="rs-drag" style={{ padding: "5px 3px", display: "flex", alignItems: "center", alignSelf: "stretch" }}>⠿</span>
                            {bioEditIdx === i
                ? <textarea autoFocus className="rs-bio-edit-area" value={bioEditVal} onChange={ev => setBioEditVal(ev.target.value)} onKeyDown={(ev) => { if (ev.key === "Enter" && ev.ctrlKey)
                    saveBioEdit(i); if (ev.key === "Escape")
                    setBioEditIdx(null); }}/>
                ...
                <div className="rs-add-row">
                    <textarea value={bioDraft} onChange={e => setBioDraft(e.target.value)} placeholder="New bio entry... (multi-line OK)" onKeyDown={(e) => { if (e.key === "Enter" && e.ctrlKey) {
        e.preventDefault();
        addBio();
    } }}/>
...
const arSaveData = () => DataStore.set(AR_SK, { avatars: arAvatars, seqIndex: arSeqIndex, shuffleQueue: arShuffleQueue });
function arGetExcluded() {
    return settings.store.avatarExcludedExtensions.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
}
function arSetExcluded(arr) { settings.store.avatarExcludedExtensions = arr.join(","); }
function arSfShuffle(len) {
...
function arToast(msg, type = Toasts.Type.SUCCESS) {
    if (!settings.store.avatarShowToast)
        return;
    Toasts.show({ message: msg, type, id: Toasts.genId() });
}
function arGetExt(data) {
    const m = data.match(/^data:image\/([a-z0-9]+);/i);
    if (!m)
        return "?";
    return m[1].toLowerCase() === "jpeg" ? "jpg" : m[1].toLowerCase();
}
function arIsGif(data) { return /^data:image\/gif;/i.test(data); }
function arStampGif(data) {
    try {
        ...
        for (let i = 0; i < n; i++)
            src[i] = raw.charCodeAt(i);
        if (src[0] !== 0x47 || src[1] !== 0x49 || src[2] !== 0x46)
            return data;
        let tp = n - 1;
        while (tp > 6 && src[tp] !== 0x3B)
            tp--;
        if (tp <= 6)
            return data;
        ...
        ext[1] = 0xFE;
        ext[2] = sLen;
        ext.set(stamp.subarray(0, sLen), 3);
        ext[sLen + 3] = 0x00;
        const out = new Uint8Array(tp + ext.length + 1);
        out.set(src.subarray(0, tp));
        out.set(ext, tp);
        out[out.length - 1] = 0x3B;
        const C = 0x8000;
        let bin = "";
        for (let i = 0; i < out.length; i += C)
            bin += String.fromCharCode(...out.subarray(i, Math.min(i + C, out.length)));
...
function arReorderArr(arr, from, to) { const r = [...arr]; const [x] = r.splice(from, 1); r.splice(to, 0, x); return r; }
async function arBlobToDataUrl(blob) {
    return new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result); r.onerror = rej; r.readAsDataURL(blob); });
}
async function arPrepareForDiscord(data) {
    const ext = arGetExt(data);
    if (ext === "gif")
        return data;
    const okExts = ["png", "jpg", "jpeg", "webp"];
    const bytes = (data.split(",")[1]?.length ?? 0) * 0.75;
    if (okExts.includes(ext) && bytes < 8_000_000)
        return data;
...
async function arGifFirstFramePng(data, cropParams) {
    const S = AR_EXP_S;
    return new Promise((res, rej) => {
        const img = new Image();
        img.onload = () => {
...
async function arGenPreview(data, cropParams) {
    const S = 76;
...
function arHasCrop(cp) {
    if (!cp)
        return false;
    return cp.ox !== 0 || cp.oy !== 0 || cp.rot !== 0;
}
async function arApplyAvatar(entry) {
    try {
        ...
        if (!pluginActive)
            return;
        if (!data?.split(",")[1] || data.split(",")[1].length < 10)
...
async function arRotateNext() {
    ...
    let idx;
    if (settings.store.avatarRandom) {
        if (!arShuffleQueue.length)
            arShuffleQueue = arSfShuffle(active.length);
        idx = arShuffleQueue.shift();
    }
    else {
        idx = arSeqIndex % active.length;
        arSeqIndex = (arSeqIndex + 1) % active.length;
    }
...
function arSchedule() {
    if (arRotatorTimer)
        clearTimeout(arRotatorTimer);
    if (!settings.store.avatarEnabled || !pluginActive || !arGetActive().length)
        return;
...
async function arImportJSON(file) {
...
function ArExtBadge({ ext, excluded }) {
    const color = excluded ? "#6b7280" : (AR_EXT_COLORS[ext] ?? "#6b7280");
    return (<span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.5, padding: "1px 5px", borderRadius: 4, background: color + "28", color, border: `1px solid ${color}55`, textTransform: "uppercase", flexShrink: 0, textDecoration: excluded ? "line-through" : "none" }}>
            {ext}
        </span>);
}
function ArExtFilterChips({ excluded, onChange }) {
    return (<div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            ...
            return (<button key={ext} onClick={() => onChange(isEx ? excluded.filter(e => e !== ext) : [...excluded, ext])} style={{ padding: "3px 9px", borderRadius: 5, fontSize: 11, fontWeight: 700, cursor: "pointer", outline: "none", border: `1px solid ${color}55`, background: color + "22", color, textDecoration: isEx ? "line-through" : "none", userSelect: "none" }}>
                        {ext.toUpperCase()}{isEx ? " ✕" : ""}
                    </button>);
        })}
        </div>);
}
function ArExtFilterSection({ excluded, onChange }) {
    return (<div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                ...
                {excluded.length > 0 && (<button onClick={() => onChange([])} style={{ fontSize: 11, color: AR.red, background: "none", border: "none", cursor: "pointer", outline: "none" }}>Clear all</button>)}
            </div>
            <ArExtFilterChips excluded={excluded} onChange={onChange}/>
        </div>);
}
function ArCropModal({ src, onApply, onSkip, modalProps }) {
    ...
    const doApply = async () => {
        const cp = { ox: offR.current.x, oy: offR.current.y, zoom: zoomR.current, rot: rotR.current };
        if (gif) {
            onApply(src, cp);
            modalProps.onClose();
            return;
        ...
        canvas.width = AR_EXP_S;
        canvas.height = AR_EXP_S;
    ...
    return (<ModalRoot {...modalProps} size="medium">
            ...
            <ModalContent style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ width: "100%", height: AR_CONT_H, background: "#0b0b0e", position: "relative", cursor: loaded ? "grab" : "default", userSelect: "none", overflow: "hidden" }} onPointerDown={e => { if (!loaded)
        return; drag.current = true; lastP.current = { x: e.clientX, y: e.clientY }; e.currentTarget.setPointerCapture(e.pointerId); e.preventDefault(); }} onPointerMove={e => { if (!drag.current)
        return; const dx = e.clientX - lastP.current.x, dy = e.clientY - lastP.current.y; lastP.current = { x: e.clientX, y: e.clientY }; const no = sync({ x: offR.current.x + dx, y: offR.current.y + dy }, zoomR.current, rotR.current); offR.current = no; setOffS({ ...no }); }} onPointerUp={() => { drag.current = false; }} onPointerCancel={() => { drag.current = false; }} onWheel={e => { if (!loaded)
        return; const z = Math.max(minZR.current, Math.min(minZR.current * 4, zoomR.current * (e.deltaY < 0 ? 1.08 : 0.92))); const c = sync(offR.current, z, rotR.current); zoomR.current = z; offR.current = c; setZoomS(z); setOffS({ ...c }); }}>
                    {loaded && (<div style={{ position: "absolute", left: "50%", top: "50%", width: 0, height: 0, transform: `translate(${offset.x}px, ${offset.y}px)` }}>
                ...
                <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <input type="range" min={minZoom} max={minZoom * 4} step={0.0005} value={zoom} disabled={!loaded} onChange={e => { const z = Math.max(minZR.current, parseFloat(e.target.value)); const c = sync(offR.current, z, rotR.current); zoomR.current = z; offR.current = c; setZoomS(z); setOffS({ ...c }); }} style={{ flex: 1, accentColor: AR.accent, cursor: loaded ? "pointer" : "default" }}/>
                    ...
                    <div>
                        <button disabled={!loaded} onClick={() => { const nr = (rotR.current + 90) % 360; rotR.current = nr; const c = sync(offR.current, zoomR.current, nr); offR.current = c; setRotS(nr); setOffS({ ...c }); }} style={{ padding: "6px 14px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: loaded ? "pointer" : "not-allowed", outline: "none", border: `1px solid ${AR.accent}44`, background: AR.aD, color: AR.accent, opacity: loaded ? 1 : 0.45 }}>
                            ↻ 90°
                        </button>
                    </div>
                    {gif && (<div style={{ padding: "8px 11px", borderRadius: 7, background: `${AR.warn}15`, border: `1px solid ${AR.warn}40`, fontSize: 11, color: AR.warn, lineHeight: 1.5 }}>
            ...
            <ModalFooter separator={false}>
                <div style={{ display: "flex", width: "100%", alignItems: "center" }}>
                    <button disabled={!loaded} onClick={() => setAll({ x: 0, y: 0 }, minZR.current, 0)} style={{ background: "none", border: "none", color: loaded ? AR.text : AR.sub, cursor: loaded ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 500, padding: "0 4px", outline: "none" }}>
                        Reset
                    </button>
                    <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                        <button onClick={() => { onSkip(); modalProps.onClose(); }} style={{ padding: "8px 18px", borderRadius: 7, fontSize: 13, fontWeight: 500, background: "transparent", border: `1px solid ${AR.line}`, color: AR.sub, cursor: "pointer", outline: "none" }}>
                            Skip
                        </button>
                        <button disabled={!loaded} onClick={doApply} style={{ padding: "8px 18px", borderRadius: 7, fontSize: 13, fontWeight: 600, border: "none", cursor: loaded ? "pointer" : "not-allowed", background: loaded ? AR.accent : "rgba(156,103,255,.3)", color: "#fff", outline: "none", opacity: loaded ? 1 : 0.45 }}>
                            Apply
                        </button>
                    </div>
                </div>
...
async function gifDecode(data) {
    ...
    try {
        ...
        const dec = new window.ImageDecoder({ data: bytes.buffer, type: "image/gif" });
        const frames = [];
        let fi = 0;
        while (true) {
            let r;
            try {
                r = await dec.decode({ frameIndex: fi, completeFramesOnly: true });
                fi++;
            }
            catch {
                break;
...
function gifLZWEnc(indices, minCodeSize) {
    const cc = 1 << minCodeSize, eoi = cc + 1;
    const out = [];
    let buf = 0, bits = 0, cs = minCodeSize + 1, nxt = cc + 2;
    const emit = (code) => { buf |= code << bits; bits += cs; while (bits >= 8) {
        out.push(buf & 0xFF);
        buf >>>= 8;
        bits -= 8;
    } };
...
function gifQuantize(imageData) {
    ...
    for (let i = nc - 1; i >= 0; i--)
        lut[((palette[i * 3] >> 4) << 8) | ((palette[i * 3 + 1] >> 4) << 4) | (palette[i * 3 + 2] >> 4)] = i;
    ...
    for (let i = 0; i < n; i++) {
        ...
        lut[((r >> 4) << 8) | ((g >> 4) << 4) | (b >> 4)] = best;
        indices[i] = best;
    }
    return { palette, indices };
}
function gifEncode(frames, w, h, loops) {
    const w16 = (a, i, v) => { a[i] = v & 0xFF; a[i + 1] = (v >> 8) & 0xFF; };
    const parts = [];
...
async function gifApplyCrop(src, ox, oy, zoom, rot) {
...
function arOpenCropFor(data, onDone) {
    if (arIsGif(data)) {
        openModal((p) => (<ModalRoot {...p} size="small">
                <ModalHeader separator={false}>
                ...
                <ModalFooter separator={false}>
                    <div style={{ display: "flex", justifyContent: "flex-end" }}>
                        <button onClick={() => { onDone(data); p.onClose(); }} style={{ padding: "8px 20px", borderRadius: 7, fontSize: 13, fontWeight: 600, border: "none", background: AR.accent, color: "#fff", cursor: "pointer", outline: "none" }}>
                            OK
                        </button>
                    </div>
                </ModalFooter>
...
function ArAvatarCard({ entry, isDragged, isDragOver, isExcluded, onDragStart, onDragOver, onDragLeave, onDrop, onDragEnd, onRemove, onApplyNow, onCrop, onRename, }) {
    ...
    const iStyle = { flex: 1, background: "rgba(0,0,0,.38)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 6, color: AR.text, fontSize: 12, padding: "2px 6px", outline: "none", minWidth: 0 };
    return (<div draggable={!editing} onDragStart={editing ? undefined : onDragStart} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} onDragEnd={onDragEnd} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 8, marginBottom: 4, background: isDragOver ? AR.bg2 : AR.bg1, border: `1px solid ${isDragOver ? AR.accent : AR.line}`, opacity: isDragged ? 0.3 : isExcluded ? 0.5 : 1, cursor: editing ? "default" : "grab", userSelect: "none" }}>
            ...
            <div style={{ position: "relative", flexShrink: 0, width: 38, height: 38, borderRadius: "50%", overflow: "hidden", border: `2px solid ${isExcluded ? "#6b7280" : ext === "gif" ? AR.warn : AR.accent}`, cursor: arIsGif(entry.data) ? "zoom-in" : "default" }}>
                <img src={entry.previewData ?? entry.data} alt="" draggable={false} onMouseEnter={arIsGif(entry.data) ? e => {
        ...
        } : undefined} onMouseLeave={arIsGif(entry.data) ? e => {
            const img = e.currentTarget;
            if (img.dataset.gifUrl) {
                URL.revokeObjectURL(img.dataset.gifUrl);
                delete img.dataset.gifUrl;
            }
            img.src = entry.previewData ?? entry.data;
            img.style.cssText = "width:100%;height:100%;object-fit:cover;display:block;";
        } : undefined} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}/>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                {editing
            ? <input autoFocus value={editText} onChange={e => setEditText(e.target.value)} onBlur={commit} onKeyDown={e => { if (e.key === "Enter")
                commit(); if (e.key === "Escape") {
                setEditText(entry.label);
                setEditing(false);
            } e.stopPropagation(); }} onClick={e => e.stopPropagation()} style={iStyle}/>
            : (<div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <span title="Double-click to rename" onDoubleClick={e => { e.stopPropagation(); setEditing(true); setEditText(entry.label); }} style={{ fontSize: 13, color: isExcluded ? AR.sub : AR.text, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: "text" }}>
                                {entry.label}
                            </span>
        ...
        ].map(({ color, title, onClick, icon }) => (<button key={title} onClick={e => { e.stopPropagation(); onClick?.(); }} title={title} style={{ width: 26, height: 26, borderRadius: 6, flexShrink: 0, border: `1px solid ${color}33`, background: `${color}18`, color, cursor: onClick ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", outline: "none", padding: 0, opacity: onClick ? 1 : 0.4 }}>
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">{icon}</svg>
                    </button>))}
            </div>
        </div>);
}
function AvatarTab({ forceUpdate }) {
    const [list, setList] = React.useState([...arAvatars]);
    ...
    const commit = (next) => { arAvatars = next; setList([...next]); void arSaveData(); };
    const setExcl = (arr) => {
        setExcludedS(arr);
        arSetExcluded(arr);
        if (arRotatorTimer) {
    ...
    const setPreset = (s) => { setLSecStr(String(s)); applyInterval(s); };
    const toggleEnabled = () => {
        const next = !running;
        settings.store.avatarEnabled = next;
        if (next && activeCount > 0) {
            if (settings.store.avatarNitroCheck)
                startAvatarNitroWatcher();
            arStartRotator(false);
            setRunning(true);
        }
        else {
    ...
    const toggleRandom = () => {
        const n = !lRandom;
        setLRandom(n);
        settings.store.avatarRandom = n;
        if (n)
            arShuffleQueue = arSfShuffle(activeCount);
    };
    const toggleToast = () => {
        const n = !lToast;
        setLToast(n);
        settings.store.avatarShowToast = n;
    };
    const handleAddUrl = async () => {
        const url = urlInput.trim();
        if (!url) {
            arToast("Please enter a URL", Toasts.Type.FAILURE);
            return;
        }
        let parsed;
        try {
            ...
            arToast("Invalid URL - must start with http:// or https://", Toasts.Type.FAILURE);
            return;
        }
        ...
        try {
            ...
            if (blob.size > 50_000_000)
                throw new Error("Image too large (>50 MB)");
    ...
    const handleFileUpload = async (e) => {
        ...
        if (files.length === 1) {
            try {
                const data = await new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result); r.onerror = rej; r.readAsDataURL(files[0]); });
                setLoading(false);
                ...
                arToast("Failed to read file", Toasts.Type.FAILURE);
                setLoading(false);
            }
        }
        else {
            ...
            if (entries.length) {
                commit([...arAvatars, ...entries]);
                arToast(`Added ${entries.length} avatar(s)`);
            }
            setLoading(false);
        }
        e.target.value = "";
    ...
    const handleImport = async (e) => {
        ...
        setLoading(true);
        try {
            const imp = await arImportJSON(f);
            if (!imp.length)
                arToast("No valid avatars in file", Toasts.Type.MESSAGE);
            else {
                commit([...arAvatars, ...imp]);
                arToast(`Imported ${imp.length}`);
            }
        }
        catch {
            arToast("Import failed - invalid JSON", Toasts.Type.FAILURE);
        }
        setLoading(false);
        e.target.value = "";
    };
    const removeEntry = (id) => {
        ...
        commit(next);
        if (!next.filter(a => !excluded.includes(arGetExt(a.data))).length && running) {
            arStopRotator();
            setRunning(false);
            forceUpdate();
        }
    ...
    const moveEntry = (from, to) => {
        if (to < 0 || to >= arAvatars.length)
            return;
        commit(arReorderArr(arAvatars, from, to));
    };
    const onDS = (e, i) => { e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", String(i)); setDraggedIdx(i); };
    const onDO = (e, i) => { e.preventDefault(); e.stopPropagation(); if (draggedIdx !== null && draggedIdx !== i)
        setDragOverIdx(i); };
    const onDL = () => setDragOverIdx(null);
    const onDP = (e, to) => { e.preventDefault(); e.stopPropagation(); if (draggedIdx !== null && draggedIdx !== to)
        moveEntry(draggedIdx, to); setDraggedIdx(null); setDragOverIdx(null); };
    const onDE = () => { setDraggedIdx(null); setDragOverIdx(null); };
    const inputBaseStyle = { flex: 1, background: "rgba(0,0,0,.38)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 6, color: AR.text, fontSize: 13, padding: "6px 10px", outline: "none", minWidth: 0 };
    return (<div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 10, marginBottom: 14, background: running ? `${AR.green}0f` : AR.bg1, border: `1px solid ${running ? AR.green + "2e" : AR.line}` }}>
                ...
                <button onClick={toggleEnabled} disabled={!running && !activeCount} style={{ padding: "9px 22px", borderRadius: 8, fontWeight: 700, fontSize: 14, border: "none", outline: "none", background: running ? AR.red : AR.green, color: "#fff", cursor: (!running && !activeCount) ? "not-allowed" : "pointer", opacity: (!running && !activeCount) ? 0.4 : 1 }}>
                    {running ? "Stop" : "Start"}
                </button>
            </div>
            ...
            <div className="ar-card">
                <div className="ar-card-row">
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                        <input type="number" min={1} value={lSecStr} onChange={e => setLSecStr(e.target.value)} onFocus={e => e.target.select()} onBlur={validateApply} onKeyDown={e => { if (e.key === "Enter") {
        validateApply();
        e.target.blur();
    } e.stopPropagation(); }} style={{ ...inputBaseStyle, flex: "none", width: 72, textAlign: "center", padding: "3px 6px" }}/>
                        <span style={{ fontSize: 12, color: AR.sub }}>sec</span>
                        <div style={{ display: "flex", gap: 3 }}>
                            {[60, 120, 300, 600, 900].map(s => (<button key={s} onClick={() => setPreset(s)} style={{ padding: "3px 7px", borderRadius: 5, fontSize: 11, cursor: "pointer", outline: "none", border: `1px solid ${sec === s ? AR.accent : AR.line}`, background: sec === s ? AR.aD : "transparent", color: sec === s ? AR.accent : AR.sub }}>
                                    {arFmtPreset(s)}
                                </button>))}
                        </div>
                    </div>
                    {warnSec && (<div style={{ width: "100%", marginTop: 4, padding: "5px 9px", borderRadius: 6, background: `${AR.warn}14`, border: `1px solid ${AR.warn}33`, fontSize: 11, color: AR.warn }}>
                ...
                <div className="ar-card-row">
                    ...
                    <div onClick={toggleRandom} style={{ width: 34, height: 18, borderRadius: 9, flexShrink: 0, cursor: "pointer", background: lRandom ? AR.accent : "rgba(255,255,255,.13)", position: "relative", userSelect: "none" }}>
                ...
                <div className="ar-card-row">
                    ...
                    <div onClick={toggleToast} style={{ width: 34, height: 18, borderRadius: 9, flexShrink: 0, cursor: "pointer", background: lToast ? AR.accent : "rgba(255,255,255,.13)", position: "relative", userSelect: "none" }}>
            ...
            settings.store.avatarNitroCheck = n;
            if (running) {
                if (n)
                    startAvatarNitroWatcher();
                else {
                    stopAvatarNitroWatcher();
                    const excl = arGetExcluded();
                    if (excl.includes("gif"))
                        arSetExcluded(excl.filter(e => e !== "gif"));
                }
            }
            forceUpdate();
        }} style={{ width: 34, height: 18, borderRadius: 9, flexShrink: 0, cursor: "pointer", background: settings.store.avatarNitroCheck ? AR.accent : "rgba(255,255,255,.13)", position: "relative", userSelect: "none" }}>
            ...
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.2, color: AR.sub, textTransform: "uppercase", marginBottom: 6 }}>Add Avatar</div>
            <div style={{ display: "flex", gap: 6, marginTop: 6, marginBottom: 8, flexWrap: "wrap" }}>
                <input placeholder="https://example.com/avatar.png" value={urlInput} onChange={e => setUrlInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter")
        handleAddUrl(); }} style={inputBaseStyle}/>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <label style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, padding: "12px 8px", borderRadius: 10, cursor: loading ? "not-allowed" : "pointer", userSelect: "none", background: "linear-gradient(135deg, rgba(88,101,242,.14), rgba(156,103,255,.14))", border: `1.5px dashed ${AR.accent}70` }}>
                ...
                <label style={{ width: 120, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, padding: "12px 8px", borderRadius: 10, cursor: loading ? "not-allowed" : "pointer", userSelect: "none", background: "linear-gradient(135deg, rgba(59,165,92,.12), rgba(156,103,255,.10))", border: `1.5px dashed ${AR.green}60` }}>
            ...
            {hasGifs && (<div style={{ padding: "7px 11px", borderRadius: 8, marginBottom: 8, background: `${AR.warn}12`, border: `1px solid ${AR.warn}33`, fontSize: 12, color: AR.warn }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.2, color: AR.sub, textTransform: "uppercase" }}>Avatar List ({list.length})</span>
                {list.length > 0 && (<div style={{ display: "flex", gap: 5 }}>
                        ...
                        <button onClick={() => { commit([]); arSeqIndex = 0; arShuffleQueue = []; if (running) {
            arStopRotator();
            setRunning(false);
            settings.store.avatarEnabled = false;
        } forceUpdate(); }} style={{ padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer", border: `1px solid ${AR.red}44`, background: `${AR.red}18`, color: AR.red, outline: "none" }}>Clear all</button>
                    </div>)}
            </div>
            ...
            : list.map((entry, i) => (<ArAvatarCard key={entry.id} entry={entry} isDragged={draggedIdx === i} isDragOver={dragOverIdx === i} isExcluded={excluded.includes(arGetExt(entry.data))} onDragStart={e => onDS(e, i)} onDragOver={e => onDO(e, i)} onDragLeave={onDL} onDrop={e => onDP(e, i)} onDragEnd={onDE} onRemove={() => removeEntry(entry.id)} onApplyNow={() => void arApplyAvatar(entry)} onCrop={() => arOpenCropFor(entry.data, async (d, cropParams) => { const previewData = await arGenPreview(d, cropParams); commit(arAvatars.map(a => a.id === entry.id ? { ...a, data: d, cropParams, previewData } : a)); arToast("Updated"); })} onRename={l => commit(arAvatars.map(a => a.id === entry.id ? { ...a, label: l } : a))}/>))}
            </div>

            <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 10, paddingTop: 10, borderTop: `1px solid ${AR.line}` }}>
                <button onClick={() => void arRotateNext()} disabled={!activeCount} style={{ padding: "8px 18px", borderRadius: 7, fontSize: 13, fontWeight: 600, background: activeCount ? AR.aD : "rgba(156,103,255,.1)", border: `1px solid ${AR.accent}44`, color: activeCount ? AR.accent : AR.sub, cursor: !activeCount ? "not-allowed" : "pointer", opacity: !activeCount ? 0.5 : 1, outline: "none" }}>
                    ⏭ Skip
                </button>
...
function NicksTab({ forceUpdate }) {
    const [filter, setFilter] = React.useState("");
    const [sort, setSort] = React.useState("enabled");
    const [nickInputs, setNickInputs] = React.useState({});
    const [nickEdit, setNickEdit] = React.useState({});
    ...
    const { props: gnDProps, cls: gnCls } = useDrag((f, t) => {
        globalNicks = reorder(globalNicks, f, t);
        saveData();
        forceUpdate();
    });
    const { props: gpDProps, cls: gpCls } = useDrag((f, t) => {
        globalGuildPronouns = reorder(globalGuildPronouns, f, t);
        saveData();
        forceUpdate();
    });
    function addNick(g) { const v = (nickInputs[g.id] ?? "").trim(); if (!v || g.nicks.includes(v))
        return; g.nicks = [...g.nicks, v]; saveData(); setNickInputs(p => ({ ...p, [g.id]: "" })); forceUpdate(); }
    function removeNick(g, i) { g.nicks = g.nicks.filter((_, j) => j !== i); g.seqIndex = 0; g.lastNickVal = null; saveData(); forceUpdate(); }
    function saveNickEdit(g) {
        const es = nickEdit[g.id];
        if (!es)
            return;
        const v = es.val.trim();
    ...
    function toggleGuild(g) {
        ...
        if (g.enabled) {
            ...
            if (!voiceOnly && !settings.store.globalSync) {
                if (settings.store.nickEnabled)
                    scheduleNickTick(g, getMs(settings.store.nickIntervalSeconds));
                if (g.guildPronounsEnabled && pronounsForGuild(g).length > 0)
                    startGuildPronouns(g);
            }
        }
        else {
    ...
    function toggleNickActive(g) {
        ...
        if (nickTimers.has(g.id))
            stopNickGuild(g.id);
        else if (settings.store.nickEnabled && !settings.store.globalSync)
            scheduleNickTick(g, getMs(settings.store.nickIntervalSeconds));
        forceUpdate();
    }
    function toggleGuildPronounsActive(g) {
        if (g.voiceActivated && settings.store.voiceActivateEnabled && !settings.store.voiceActivateGlobal) {
    ...
    function cycleMode(g) { g.nickMode = NM_NEXT[nickModeOf(g)]; g.seqIndex = 0; g.lastNickVal = null; saveData(); forceUpdate(); }
    function addManual() {
        const id = manualId.trim();
        const name = manualName.trim() || id;
        if (!id || !/^\d{17,20}$/.test(id) || guilds.find(g => g.id === id))
            return;
    ...
    function removeGuild(g) { stopNickGuild(g.id); guilds = guilds.filter(x => x.id !== g.id); saveData(); forceUpdate(); }
    function enableAll() { guilds.forEach(g => { if (!g.enabled) {
        g.enabled = true;
        if (!settings.store.globalSync) {
            startNickGuild(g);
            startGuildPronouns(g);
    ...
    function allInactiveNicks() {
        guilds.forEach(g => { stopNickGuild(g.id); if (g.voiceActivated)
            g.nickVoiceEnabled = false; });
        saveData();
        forceUpdate();
    }
    ...
    function allInactivePronouns() {
        guilds.forEach(g => {
            stopGuildPronouns(g.id);
            if (g.voiceActivated && settings.store.voiceActivateEnabled && !settings.store.voiceActivateGlobal)
                g.pronounsVoiceEnabled = false;
            else
                g.guildPronounsEnabled = false;
        });
        saveData();
        forceUpdate();
    }
    ...
    function allAlways() {
        guilds.forEach(g => {
            ...
            if (pluginActive && g.enabled && !settings.store.globalSync) {
                if (settings.store.nickEnabled && !nickTimers.has(g.id))
                    startNickGuild(g);
                if (g.guildPronounsEnabled && pronounsForGuild(g).length > 0 && !guildPronounsTimers.has(g.id))
                    startGuildPronouns(g);
            }
        });
        saveData();
    ...
    let sorted = [...guilds];
    if (filter)
        sorted = sorted.filter(g => g.name.toLowerCase().includes(filter.toLowerCase()) || g.id.includes(filter));
    if (sort === "enabled")
        sorted.sort((a, b) => Number(b.enabled) - Number(a.enabled));
    else if (sort === "name")
        sorted.sort((a, b) => a.name.localeCompare(b.name));
    else if (sort === "nicks")
        sorted.sort((a, b) => b.nicks.length - a.nicks.length);
    else if (sort === "running")
        sorted.sort((a, b) => Number(nickTimers.has(b.id)) - Number(nickTimers.has(a.id)));
    else if (sort === "pronouns")
        sorted.sort((a, b) => (b.guildPronouns?.length ?? 0) - (a.guildPronouns?.length ?? 0));
    ...
    return (<div>
            <div className="rs-card" style={{ marginBottom: 8 }}>
                <SectionHeader label="Server Nicknames" color={C.nick} enabled={settings.store.nickEnabled} onToggleEnabled={v => { settings.store.nickEnabled = v; if (pluginActive) {
        if (v) {
            for (const g of guilds.filter(x => x.enabled))
                startNickGuild(g);
        }
            ...
            stopAllNicks();
    } forceUpdate(); }} rndValue={settings.store.nickRandomize} onToggleRnd={v => { settings.store.nickRandomize = v; forceUpdate(); }} enableColor={C.nick}/>
                <PanelInterval label="Nickname Interval" description="Seconds between nickname changes per server (ignored when Master Sync is ON)" storeKey="nickIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.nickEnabled && !settings.store.globalSync) {
        stopAllNicks();
        for (const g of guilds.filter(x => x.enabled))
            startNickGuild(g);
    } }}/>
            ...
            <div className="rs-card" style={{ marginBottom: 8 }}>
                <SectionHeader label="Server Pronouns" color={C.pronoun} enabled={settings.store.serverPronounsEnabled} onToggleEnabled={v => { settings.store.serverPronounsEnabled = v; if (pluginActive) {
        if (v) {
            for (const g of guilds.filter(x => x.guildPronounsEnabled && pronounsForGuild(x).length > 0))
                startGuildPronouns(g);
        }
            ...
            stopAllGuildPronouns();
    } forceUpdate(); }} rndValue={settings.store.serverPronounsRandomize} onToggleRnd={v => { settings.store.serverPronounsRandomize = v; forceUpdate(); }} enableColor={C.pronoun}/>
                <PanelInterval label="Pronouns Interval" description="Seconds between pronoun changes per server (ignored when Master Sync is ON)" storeKey="serverPronounsIntervalSeconds" disabled={settings.store.globalSync} onApply={() => { if (pluginActive && settings.store.serverPronounsEnabled && !settings.store.globalSync) {
        stopAllGuildPronouns();
        for (const g of guilds.filter(x => x.guildPronounsEnabled && pronounsForGuild(x).length > 0))
            startGuildPronouns(g);
    } }}/>
            ...
            <div className="rs-card" style={{ marginBottom: 8, border: "1.5px solid rgba(121,134,203,.3)", background: "rgba(10,10,40,.5)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: "#7986cb" }}>Voice Activation</span>
                    <div style={{ flex: 1, height: 1, background: "rgba(121,134,203,.2)" }}/>
                    <button style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 10px", borderRadius: 6, border: `1px solid ${settings.store.voiceActivateEnabled ? "#7986cb55" : "rgba(80,60,110,.3)"}`, background: settings.store.voiceActivateEnabled ? "#7986cb20" : "rgba(15,5,35,.5)", color: settings.store.voiceActivateEnabled ? "#7986cb" : "#5a4a7a", cursor: "pointer", fontSize: 11, fontWeight: 800 }} onClick={() => { settings.store.voiceActivateEnabled = !settings.store.voiceActivateEnabled; if (pluginActive) {
        stopVoiceWatcher();
        if (settings.store.voiceActivateEnabled)
            startVoiceWatcher();
        else
            startAllRotators();
                ...
                {settings.store.voiceActivateEnabled && (<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        <PanelToggle label="Global Voice" description="Join ANY voice/call/DM → start ALL enabled server nick+pronoun cycles. Leave → stop all. Overrides per-server." value={settings.store.voiceActivateGlobal} color="#7986cb" onChange={v => { settings.store.voiceActivateGlobal = v; if (pluginActive)
            startAllRotators(); forceUpdate(); }}/>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: C.nick }}>Global Nick Pool</span>
            ...
            return (<div key={`gn_${ni}`} {...gnDProps(ni)} className={gnCls(ni, "rs-item rs-item-compact")}>
                            <span className="rs-drag">⠿</span>
                            {es && es.idx === ni
                    ? <input autoFocus className="rs-item-input" value={es.val} onChange={e => setNickEdit(p => ({ ...p, __g: { idx: ni, val: e.target.value } }))} onKeyDown={(e) => {
                            if (e.key === "Enter") {
                                const v = es.val.trim();
                                if (v) {
                                    globalNicks = [...globalNicks];
                                    globalNicks[ni] = v;
                                    saveData();
                                }
                                setNickEdit(p => ({ ...p, __g: null }));
                                forceUpdate();
                            }
                            if (e.key === "Escape")
                                setNickEdit(p => ({ ...p, __g: null }));
                        }} onBlur={() => { const v = es.val.trim(); if (v) {
                        globalNicks = [...globalNicks];
                        globalNicks[ni] = v;
                        saveData();
                    } setNickEdit(p => ({ ...p, __g: null })); forceUpdate(); }}/>
            ...
            <div className="rs-row" style={{ marginTop: 5, marginBottom: 10 }}>
                <TextInput placeholder="Add to global nick pool..." value={nickInputs.__g ?? ""} onChange={(v) => setNickInputs(p => ({ ...p, __g: v }))} onKeyDown={(e) => {
            if (e.key === "Enter") {
                const v = (nickInputs.__g ?? "").trim();
                if (v && !globalNicks.includes(v)) {
                    globalNicks = [...globalNicks, v];
                    saveData();
                    setNickInputs(p => ({ ...p, __g: "" }));
                    forceUpdate();
                }
            }
        }}/>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: C.pronoun }}>Global Pronoun Pool</span>
            ...
            return (<div key={`ggpr_${pi}`} {...gpDProps(pi)} className={gpCls(pi, "rs-item rs-item-compact")}>
                            <span className="rs-drag">⠿</span>
                            {es
                    ? <input autoFocus className="rs-item-input" value={es.val} maxLength={40} onChange={e => setNickEdit(p => ({ ...p, [key]: { idx: pi, val: e.target.value } }))} onKeyDown={(e) => {
                            if (e.key === "Enter") {
                                const v = es.val.trim();
                                if (v) {
                                    globalGuildPronouns = [...globalGuildPronouns];
                                    globalGuildPronouns[pi] = v;
                                    saveData();
                                }
                                setNickEdit(p => ({ ...p, [key]: null }));
                                forceUpdate();
                            }
                            if (e.key === "Escape")
                                setNickEdit(p => ({ ...p, [key]: null }));
                        ...
                        globalGuildPronouns[pi] = v;
                        saveData();
                    } setNickEdit(p => ({ ...p, [key]: null })); forceUpdate(); }}/>
                    : <span className="rs-item-text" style={{ fontWeight: 600, color: C.pronoun }} onDoubleClick={() => setNickEdit(p => ({ ...p, [key]: { idx: pi, val: pr } }))}>{pr}</span>}
            ...
            <div className="rs-row" style={{ marginTop: 5, marginBottom: 10 }}>
                <TextInput placeholder="Add to global pronoun pool (max 40)..." value={nickInputs.__gpr ?? ""} onChange={(v) => setNickInputs(p => ({ ...p, __gpr: v.slice(0, 40) }))} onKeyDown={(e) => {
            if (e.key === "Enter") {
                const v = (nickInputs.__gpr ?? "").trim();
                if (v && !globalGuildPronouns.includes(v)) {
                    globalGuildPronouns = [...globalGuildPronouns, v];
                    saveData();
                    setNickInputs(p => ({ ...p, __gpr: "" }));
                    forceUpdate();
                }
            }
        }}/>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 8, padding: "8px 10px", borderRadius: 8, border: "1px solid rgba(124,77,255,.18)", background: "rgba(10,5,30,.5)" }}>
                {confirmBulk && (<div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 8px", borderRadius: 6, background: "rgba(239,83,80,.1)", border: "1px solid rgba(239,83,80,.3)" }}>
                        <span style={{ flex: 1, fontSize: 11, color: "#ef9a9a", fontWeight: 700 }}>Apply "{confirmBulk}" to all servers?</span>
                        <button onClick={() => { const fn = bulkFns[confirmBulk]; if (fn)
            fn(); setConfirmBulk(null); }} style={{ padding: "2px 10px", borderRadius: 5, border: `1px solid ${ACT}44`, background: `${ACT}18`, color: ACT, cursor: "pointer", fontSize: 10, fontWeight: 800 }}>Yes</button>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: ".8px", color: C.nick, minWidth: 48 }}>Nicks</span>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: ".8px", color: C.pronoun, minWidth: 56 }}>Pronouns</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: ".8px", color: C.nick, minWidth: 72 }}>Nick Mode</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: ".8px", color: C.pronoun, minWidth: 72 }}>Pr Mode</span>
                ...
                {settings.store.voiceActivateEnabled && !settings.store.voiceActivateGlobal && (<div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: ".8px", color: "#7986cb", minWidth: 48 }}>Voice</span>
            ...
            <div className="rs-toolbar">
                ...
                {["enabled", "name", "nicks", "pronouns", "running"].map(m => (<button key={m} className="rs-sort-btn" style={{ background: sort === m ? "rgba(124,77,255,.35)" : "rgba(124,77,255,.1)", border: "1px solid rgba(124,77,255,.25)", color: sort === m ? "#e8d5ff" : "#757575", cursor: "pointer" }} onClick={() => setSort(m)}>{m}</button>))}
            </div>
            ...
            return (<div key={g.id} className="rs-card" style={{ borderColor: (g.enabled && settings.store.nickEnabled) ? `${color}60` : "rgba(124,77,255,.2)" }}>
                        <div className="rs-card-header">
                            <div className="rs-card-left">
                                ...
                                <div style={{ minWidth: 0, flex: 1 }}>
                                    ...
                                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3, marginTop: 2 }}>
                                        ...
                                        {g.guildPronounsEnabled && (<span className="rs-badge" style={{ background: `${NM_COLOR[g.guildPronounsMode ?? "custom"]}30`, color: NM_COLOR[g.guildPronounsMode ?? "custom"], border: `1px solid ${NM_COLOR[g.guildPronounsMode ?? "custom"]}55` }}>
                            ...
                            <div className="rs-actions">
                                {settings.store.voiceActivateEnabled && !settings.store.voiceActivateGlobal && (<button style={{ display: "flex", alignItems: "center", gap: 3, padding: "3px 8px", borderRadius: 6, border: `1px solid ${g.voiceActivated ? "#7986cb55" : "rgba(80,60,110,.3)"}`, background: g.voiceActivated ? "#7986cb22" : "rgba(15,5,35,.5)", color: g.voiceActivated ? "#7986cb" : "#5a4a7a", cursor: "pointer", fontSize: 10, fontWeight: 800, flexShrink: 0 }} title="Toggle: Voice-only activates nicks+pronouns only when in this server's voice" onClick={() => {
                        ...
                        forceUpdate();
                                ...
                                <Button size={Button.Sizes.SMALL} color={g.enabled ? Button.Colors.GREEN : Button.Colors.PRIMARY} className="rs-btn-sm" onClick={() => toggleGuild(g)}>{g.enabled ? "ON" : "OFF"}</Button>
                                {g.manual && <button className="rs-del-btn" onClick={() => removeGuild(g)}>&#10005;</button>}
                            </div>
                        </div>
                        {g.enabled && (<div className="rs-nick-expand">
                                <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                                    <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: NM_COLOR[mode] }}>Nicks</span>
                                    ...
                                    <button style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 6, border: `1px solid ${NM_COLOR[mode]}44`, background: `${NM_COLOR[mode]}18`, color: NM_COLOR[mode], cursor: "pointer", fontSize: 10, fontWeight: 800, flexShrink: 0, transition: "all .15s" }} title="Cycle: Custom → Global → Both" onClick={() => cycleMode(g)}>{NM_LABEL[mode]}
                                    </button>
                                    <button style={{
                    ...
                    }} onClick={() => { if (settings.store.nickEnabled)
                    toggleNickActive(g); }}>
                            ...
                            : g.nicks.map((n, ni) => (<div key={`${g.id}_n_${ni}`} draggable onDragStart={de => { de.dataTransfer.effectAllowed = "move"; de.dataTransfer.setData("text/plain", `NICK:${g.id}:${ni}`); }} onDragOver={de => de.preventDefault()} onDrop={de => {
                                    de.preventDefault();
                                    const [type, dGid, dI] = de.dataTransfer.getData("text/plain").split(":");
                                    if (type === "NICK" && dGid === g.id) {
                                        const from = parseInt(dI, 10);
                                        if (!isNaN(from) && from !== ni) {
                                            g.nicks = reorder(g.nicks, from, ni);
                                            g.seqIndex = 0;
                                            g.lastNickVal = null;
                                            saveData();
                                            forceUpdate();
                                                        ...
                                                        <span className="rs-drag">⠿</span>
                                                        {es && es.idx === ni
                                    ? <input autoFocus className="rs-item-input" value={es.val} onChange={e2 => setNickEdit(p => ({ ...p, [g.id]: { idx: ni, val: e2.target.value } }))} onKeyDown={(e2) => { if (e2.key === "Enter")
                                        saveNickEdit(g); if (e2.key === "Escape")
                                        setNickEdit(p => ({ ...p, [g.id]: null })); }} onBlur={() => saveNickEdit(g)}/>
                                    : <span className="rs-item-text" style={{ color: color, fontWeight: 600 }}>{n}</span>}
                                        ...
                                        <div className="rs-row">
                                            <TextInput value={nickInputs[g.id] ?? ""} onChange={(v) => setNickInputs(p => ({ ...p, [g.id]: v }))} placeholder="Add a custom nick for this server..." onKeyDown={(e) => { if (e.key === "Enter")
                        addNick(g); }}/>
                                ...
                                {mode === "global" && (<div className="rs-hint">Using global pool (<b style={{ color: C.nick }}>{globalNicks.length} nicks</b>). Switch to Custom or Both to add server-specific nicks.</div>)}

                                <div style={{ marginTop: 8, borderTop: `1px solid ${C.pronoun}22`, paddingTop: 7 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                                        <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: C.pronoun }}>Pronouns</span>
                                        ...
                                        <button style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 6, border: `1px solid ${NM_COLOR[g.guildPronounsMode ?? "custom"]}44`, background: `${NM_COLOR[g.guildPronounsMode ?? "custom"]}18`, color: NM_COLOR[g.guildPronounsMode ?? "custom"], cursor: "pointer", fontSize: 10, fontWeight: 800, flexShrink: 0, transition: "all .15s" }} title="Cycle pronoun‑source mode: Custom → Global → Both" onClick={() => { const cur = g.guildPronounsMode ?? "custom"; g.guildPronounsMode = NM_NEXT[cur]; g.guildPronounsSeqIdx = 0; g.guildPronounsLastVal = null; saveData(); forceUpdate(); }}>
                    ...
                    }} onClick={() => { if (settings.store.serverPronounsEnabled)
                    toggleGuildPronounsActive(g); }}>
                                    ...
                                    {(g.guildPronounsMode ?? "custom") === "custom" && gPrList.length === 0 && effectivePrList.length > 0 && (<div className="rs-hint" style={{ marginBottom: 4, color: C.pronoun }}>No local entries - using {effectivePrList.length} from the global pronoun pool.</div>)}
                                    {(g.guildPronounsMode ?? "custom") === "global" && (<div className="rs-hint" style={{ marginBottom: 4, color: NM_COLOR.global }}>Using global pool only (<b>{globalGuildPronouns.length} entries</b>). Switch to Custom or Both to use local.</div>)}
                                    {(g.guildPronounsMode ?? "custom") === "both" && (<div className="rs-hint" style={{ marginBottom: 4, color: NM_COLOR.both }}>Merged: {globalGuildPronouns.length} global + {gPrList.length} local = <b>{effectivePrList.length} total</b>.</div>)}
                                    {effectivePrList.length === 0 && (<div className="rs-empty" style={{ marginBottom: 4 }}>No pronouns set - add local entries or fill the global pronoun pool above.</div>)}
                                    ...
                                    {gPrList.map((pr, pi) => (<div key={`${g.id}_pr_${pi}`} draggable onDragStart={de => { de.dataTransfer.effectAllowed = "move"; de.dataTransfer.setData("text/plain", `PR:${g.id}:${pi}`); }} onDragOver={de => de.preventDefault()} onDrop={de => {
                            de.preventDefault();
                            const parts = de.dataTransfer.getData("text/plain").split(":");
                            if (parts[0] === "PR" && parts[1] === g.id) {
                                const from = parseInt(parts[2], 10);
                                if (!isNaN(from) && from !== pi) {
                                    g.guildPronouns = reorder(gPrList, from, pi);
                                    g.guildPronounsSeqIdx = 0;
                                    g.guildPronounsLastVal = null;
                                    saveData();
                                    forceUpdate();
                                }
                            }
                            ...
                            ? <input autoFocus className="rs-item-input" value={nickEdit[`__pr_${g.id}_${pi}`].val} maxLength={40} onChange={e2 => setNickEdit(p => ({ ...p, [`__pr_${g.id}_${pi}`]: { idx: pi, val: e2.target.value } }))} onKeyDown={(e2) => {
                                    if (e2.key === "Enter") {
                                        const v = nickEdit[`__pr_${g.id}_${pi}`].val.trim();
                                        if (v) {
                                            g.guildPronouns = [...gPrList];
                                            g.guildPronouns[pi] = v;
                                            saveData();
                                        }
                                        setNickEdit(p => ({ ...p, [`__pr_${g.id}_${pi}`]: null }));
                                        forceUpdate();
                                    }
                                    if (e2.key === "Escape")
                                        setNickEdit(p => ({ ...p, [`__pr_${g.id}_${pi}`]: null }));
                                }} onBlur={() => { const ek = nickEdit[`__pr_${g.id}_${pi}`]; const v = ek?.val?.trim(); if (v) {
                                g.guildPronouns = [...gPrList];
                                g.guildPronouns[pi] = v;
                                saveData();
                            } setNickEdit(p => ({ ...p, [`__pr_${g.id}_${pi}`]: null })); forceUpdate(); }}/>
                            : <span className="rs-item-text" style={{ color: C.pronoun, fontWeight: 600 }} onDoubleClick={() => setNickEdit(p => ({ ...p, [`__pr_${g.id}_${pi}`]: { idx: pi, val: pr } }))}>{pr}</span>}
                                    ...
                                    <div className="rs-row" style={{ marginTop: 4 }}>
                                        <TextInput value={nickInputs[`__pr_${g.id}`] ?? ""} onChange={(v) => setNickInputs(p => ({ ...p, [`__pr_${g.id}`]: v.slice(0, 40) }))} placeholder="Add a pronoun for this server (max 40)..." onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            const v = (nickInputs[`__pr_${g.id}`] ?? "").trim();
                            if (v && !gPrList.includes(v)) {
                                g.guildPronouns = [...gPrList, v];
                                saveData();
                                setNickInputs(p => ({ ...p, [`__pr_${g.id}`]: "" }));
                                forceUpdate();
                            }
                        }
                    }}/>
            ...
            : <Button color={Button.Colors.PRIMARY} onClick={() => setConfirmClear("__disableAll")} className="rs-btn-sm">Disable All</Button>}
...
function StopAllPanel({ forceUpdate }) {
    const [durVal, setDurVal] = React.useState("30");
    const [durUnit, setDurUnit] = React.useState("m");
    ...
    return (<div style={{ marginTop: 10 }}>
            <div className="rs-divider" style={{ margin: "8px 0" }}/>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: "#e57373", marginBottom: 6 }}>Rotator Control</div>
            {stopped && (<div style={{ padding: "7px 11px", borderRadius: 8, background: "rgba(239,83,80,.1)", border: "1px solid rgba(239,83,80,.35)", marginBottom: 8, fontSize: 11, color: "#ef9a9a", lineHeight: 1.6 }}>
                    {invisPaused
            ...
            {!stopped && (<div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                    <span style={{ fontSize: 11, color: "#9e9e9e" }}>Stop for:</span>
                    <input type="number" min={1} value={durVal} onChange={e => setDurVal(e.target.value)} onFocus={e => e.target.select()} onKeyDown={e => e.stopPropagation()} style={{ width: 56, background: "rgba(10,0,30,.7)", border: "1px solid rgba(239,83,80,.35)", borderRadius: 5, color: "#f0eaff", fontSize: 12, padding: "3px 7px", outline: "none", textAlign: "center" }}/>
                    {["s", "m", "h"].map(u => (<button key={u} onClick={() => setDurUnit(u)} style={{ padding: "3px 9px", borderRadius: 5, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${durUnit === u ? "rgba(239,83,80,.6)" : "rgba(239,83,80,.25)"}`, background: durUnit === u ? "rgba(239,83,80,.18)" : "rgba(10,0,30,.6)", color: durUnit === u ? "#ef9a9a" : "#5a4a7a" }}>
                            {u}
                        </button>))}
                    <button onClick={() => { doGlobalStop(toMs()); forceUpdate(); }} style={{ padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(239,83,80,.4)", background: "rgba(239,83,80,.15)", color: "#ef9a9a" }}>
                        ⏹ Stop
                    </button>
                    <button onClick={() => { doGlobalStop(null); forceUpdate(); }} style={{ padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(239,83,80,.25)", background: "rgba(239,83,80,.08)", color: "#ef9a9a88" }}>
                        ⏹ Stop ∞
                    </button>
                </div>)}
            <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
                {stopped && !invisPaused && (<button onClick={() => { doGlobalResume(); forceUpdate(); }} style={{ padding: "4px 14px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(67,160,71,.5)", background: "rgba(67,160,71,.15)", color: "#81c784" }}>
                        ▶ Resume Now
                    </button>)}
                <Button color={Button.Colors.BRAND} onClick={() => { if (!isManualStop && !wasInvisible) {
        startAllRotators();
        forceUpdate();
    } }} className="rs-btn-sm" style={{ opacity: (isManualStop || wasInvisible) ? 0.4 : 1 }}>
...
function DataTab({ forceUpdate }) {
    ...
    function doImport() {
        ...
        inp.onchange = async () => {
            ...
            try {
                ...
                if (Array.isArray(p.globalNicks))
                    globalNicks = p.globalNicks;
                if (Array.isArray(p.guilds))
                    guilds = p.guilds.map((g) => ({ ...g, nickMode: g.nickMode ?? (g.useGlobal ? "global" : "custom"), lastNickVal: null, nickVoiceEnabled: g.nickVoiceEnabled ?? g.enabled, pronounsVoiceEnabled: g.pronounsVoiceEnabled ?? g.guildPronounsEnabled }));
                if (Array.isArray(p.bioEntries))
                    bioEntries = p.bioEntries;
                if (typeof p.pronounsList === "string")
                    pronounsList = p.pronounsList;
                if (Array.isArray(p.statusEntries))
                    statusEntries = p.statusEntries;
                else if (typeof p.statuses === "string")
                    statusEntries = parseLegacyStatuses(p.statuses);
                if (Array.isArray(p.statusPresets))
                    statusPresets = p.statusPresets;
                if (Array.isArray(p.clanIds))
                    clanIds = p.clanIds;
                statusSeqIdx = 0;
                clanSeqIdx = 0;
                bioSeqIdx = 0;
                ...
                bioLastVal = null;
                prLastVal = null;
                if (Array.isArray(p.globalNickEntries))
                    globalNickEntries = p.globalNickEntries;
                globalNickSeqIdx = 0;
                globalNickLastVal = null;
                if (Array.isArray(p.globalGuildPronouns))
                    globalGuildPronouns = p.globalGuildPronouns;
                if (Array.isArray(p.avatars)) {
                    arAvatars = p.avatars.filter((a) => typeof a.id === "string" && typeof a.label === "string" && typeof a.data === "string");
                    arSeqIndex = typeof p.avatarSeqIndex === "number" ? p.avatarSeqIndex : 0;
                    arShuffleQueue = [];
                    await arSaveData();
                }
                if (Array.isArray(p.bannerFavorites)) {
                    bcrFavorites = p.bannerFavorites.filter((x) => typeof x === "string" && bcrIsValidHex(x));
                    bcrUsedFavs = [];
                    await bcrSaveData();
                }
                if (typeof p.bannerMode === "string")
                    settings.store.bannerMode = p.bannerMode;
                if (typeof p.bannerIntervalSeconds === "number")
                    settings.store.bannerIntervalSeconds = p.bannerIntervalSeconds;
                if (typeof p.bannerHueRadius === "number")
                    settings.store.bannerHueRadius = p.bannerHueRadius;
                if (typeof p.bannerCustomBaseColor === "string" && bcrIsValidHex(p.bannerCustomBaseColor))
                    settings.store.bannerCustomBaseColor = p.bannerCustomBaseColor;
                if (typeof p.bannerShowToast === "boolean")
                    settings.store.bannerShowToast = p.bannerShowToast;
                if (typeof p.bannerShowCurrentColor === "boolean")
                    settings.store.bannerShowCurrentColor = p.bannerShowCurrentColor;
                await saveData();
                startAllRotators();
    ...
    return (<div>
            <div className="rs-data-card rs-master-box">
                ...
                <PanelToggle label="Master Sync" description={isGlobalSync ? `All rotators fire every ${settings.store.globalSyncSeconds}s` : "Each rotator runs on its own independent timer"} value={isGlobalSync} color={C.data} onChange={v => { settings.store.globalSync = v; if (pluginActive)
        startAllRotators(); forceUpdate(); }}/>
                <PanelInterval label="Master Sync Interval" description="Unified interval in seconds (only used when Master Sync is ON)" storeKey="globalSyncSeconds" disabled={!isGlobalSync} onApply={() => { if (pluginActive && isGlobalSync)
        startAllRotators(); }}/>
                {isGlobalSync && settings.store.globalNickEnabled && parseFloat(settings.store.globalSyncSeconds) < 429 && (<div className="rs-warn-box" style={{ marginTop: 5 }}>
                ...
                <div className="rs-divider" style={{ margin: "8px 0" }}/>
                <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: "#ab47bc", marginBottom: 5 }}>Random Behavior</div>
                <PanelToggle label="No-Duplicate Random" description="Never pick the same entry twice in a row (applies to all rotators)" value={settings.store.noDuplicateRandom} onChange={v => { settings.store.noDuplicateRandom = v; }}/>
                <div className="rs-divider" style={{ margin: "8px 0" }}/>
                <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".7px", color: "#9e9e9e", marginBottom: 5 }}>Misc</div>
                <PanelToggle label="Show User Area Button" description="Show the Rotator Suite button in the bottom-left user area" value={settings.store.showButton} onChange={v => { settings.store.showButton = v; forceUpdate(); }}/>
                <PanelToggle label="Console Logs" description="Print all rotator activity and errors to the browser console (F12)" value={settings.store.enableLogs} onChange={v => { settings.store.enableLogs = v; }}/>
                <PanelToggle label="Stop When Invisible" description="Automatically pause all rotators when your status is set to Invisible (resumes on visible)" value={settings.store.stopOnInvisible} onChange={v => { settings.store.stopOnInvisible = v; if (v)
        startInvisibleWatcher();
    else
        stopInvisibleWatcher(); forceUpdate(); }}/>
                {activeLabels.length > 0 && (<div style={{ fontSize: 11, color: "#9e9e9e", marginTop: 8 }}>
            ...
            <div className="rs-data-card">
                ...
                {importMsg && (<div className="rs-import-status" style={{
                ...
                <div style={{ display: "flex", gap: 7 }}>
                    ...
                    <Button color={Button.Colors.PRIMARY} onClick={doImport} className="rs-btn-sm">Import JSON</Button>
                </div>
            </div>
...
function OnCloseBannerPanel({ forceUpdate }) {
    ...
    return (<div>
            <PanelToggle label="On-Close Banner" description="Apply a fixed banner color when Discord closes (beforeunload - not fired on crash/kill)" value={enabled} color="#c084fc" onChange={v => { closeBannerEnabled = v; setEnabled(v); void saveCloseConfig(); forceUpdate(); }}/>
            {enabled && (<div style={{ display: "flex", flexDirection: "column", gap: 5, padding: "7px 10px", border: "1px solid rgba(192,132,252,.25)", borderRadius: 7, background: "rgba(10,0,30,.5)", marginTop: 3 }}>
                    <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                        ...
                        <input value={color} onChange={e => setColor(e.target.value)} onBlur={save} onKeyDown={(e) => { if (e.key === "Enter")
            save(); }} placeholder="#rrggbb" maxLength={7} style={{ flex: 1, background: "rgba(10,0,30,.7)", border: `1px solid ${bcrIsValidHex(color) ? "rgba(192,132,252,.44)" : "rgba(239,83,80,.5)"}`, borderRadius: 5, color: "#f0eaff", fontSize: 12, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
                        <button onClick={() => { const c = bcrCurrentColor ?? "#111214"; setColor(c); closeBannerColor = c; void saveCloseConfig(); forceUpdate(); }} style={{ padding: "3px 9px", borderRadius: 5, border: "1px solid rgba(192,132,252,.3)", background: "rgba(192,132,252,.12)", color: "#c084fc", cursor: "pointer", fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
                            Use current
                        </button>
                    </div>
                    {color.trim() && !bcrIsValidHex(color.trim()) && (<div style={{ fontSize: 10, color: "#ef9a9a" }}>⚠ Invalid hex - must be #rrggbb format.</div>)}
                    {bcrIsValidHex(color.trim()) && (<div style={{ fontSize: 10, color: "#c084fc", opacity: .8 }}>Banner will be set to <b>{color.trim()}</b> upon closure.</div>)}
                </div>)}
        </div>);
}
function BcrHr() { return <div style={{ height: 1, background: "rgba(255,255,255,.07)", margin: "7px 0" }}/>; }
function BcrSwatch({ color, size = 22, active, onClick, title: t }) {
...
function BcrToggle({ value, onChange }) {
...
function BcrHsvPicker({ value, onChange }) {
    const W = 200, H = 110;
    const cvRef = React.useRef(null);
    const hsvR = React.useRef(bcrIsValidHex(value) ? bcrHexToHsv(value) : [270, 0.48, 0.79]);
    const dragCv = React.useRef(false);
    const dragHue = React.useRef(false);
    const [hsv, setHsv] = React.useState(hsvR.current);
    const [hex, setHex] = React.useState(bcrIsValidHex(value) ? value : "#c084fc");
    const draw = (hue) => { const cv = cvRef.current; if (!cv)
        return; const ctx = cv.getContext("2d"); const gH = ctx.createLinearGradient(0, 0, W, 0); gH.addColorStop(0, "#fff"); gH.addColorStop(1, bcrHsvToHex(hue, 1, 1)); ctx.fillStyle = gH; ctx.fillRect(0, 0, W, H); const gV = ctx.createLinearGradient(0, 0, 0, H); gV.addColorStop(0, "rgba(0,0,0,0)"); gV.addColorStop(1, "#000"); ctx.fillStyle = gV; ctx.fillRect(0, 0, W, H); };
    React.useEffect(() => { draw(hsvR.current[0]); }, []);
    React.useEffect(() => { if (!bcrIsValidHex(value))
        return; const vl = value.toLowerCase(); if (vl !== bcrHsvToHex(...hsvR.current)) {
        const h = bcrHexToHsv(vl);
        hsvR.current = h;
        setHsv([...h]);
        setHex(vl);
        draw(h[0]);
    } }, [value]);
    const emit = (h, s, v) => { hsvR.current = [h, s, v]; setHsv([h, s, v]); const out = bcrHsvToHex(h, s, v); setHex(out); onChange(out); };
    const onSvPtr = (e) => { const rect = cvRef.current.getBoundingClientRect(); emit(hsvR.current[0], Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)), Math.max(0, Math.min(1, 1 - (e.clientY - rect.top) / rect.height))); };
    const onHuePtr = (e) => { const rect = e.currentTarget.getBoundingClientRect(); const t = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)); const h = t * 360; draw(h); emit(h, hsvR.current[1], hsvR.current[2]); };
    ...
    return (<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ position: "relative", width: W, height: H, borderRadius: 5, overflow: "hidden", cursor: "crosshair", flexShrink: 0 }}>
                <canvas ref={cvRef} width={W} height={H} style={{ display: "block" }} onPointerDown={e => { dragCv.current = true; e.target.setPointerCapture(e.pointerId); onSvPtr(e); }} onPointerMove={e => { if (dragCv.current)
        onSvPtr(e); }} onPointerUp={() => { dragCv.current = false; }} onPointerCancel={() => { dragCv.current = false; }}/>
            ...
            <div style={{ position: "relative", height: 14, borderRadius: 7, background: HUE_GRAD, cursor: "pointer" }} onPointerDown={e => { dragHue.current = true; e.target.setPointerCapture(e.pointerId); onHuePtr(e); }} onPointerMove={e => { if (dragHue.current)
        onHuePtr(e); }} onPointerUp={() => { dragHue.current = false; }} onPointerCancel={() => { dragHue.current = false; }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <input value={hex} onChange={e => { const v = e.target.value; setHex(v); if (bcrIsValidHex(v)) {
        const h = bcrHexToHsv(v);
        hsvR.current = h;
        setHsv([...h]);
        draw(h[0]);
        onChange(v);
    } }} maxLength={7} style={{ flex: 1, background: "rgba(0,0,0,.4)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 5, color: "#f0e6ff", fontSize: 12, padding: "3px 7px", outline: "none", fontFamily: "monospace" }}/>
            </div>
        </div>);
}
function BcrHueRadiusSlider({ value, onChange }) {
    const pct = ((value - 1) / 179) * 100;
    const dragRef = React.useRef(false);
    const onPtr = (e) => { const rect = e.currentTarget.getBoundingClientRect(); const t = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)); onChange(Math.round(1 + t * 179)); };
    return (<div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <div style={{ position: "relative", height: 20, cursor: "pointer" }} onPointerDown={e => { dragRef.current = true; e.target.setPointerCapture(e.pointerId); onPtr(e); }} onPointerMove={e => { if (dragRef.current)
        onPtr(e); }} onPointerUp={() => { dragRef.current = false; }} onPointerCancel={() => { dragRef.current = false; }}>
...
function FavsReorderList({ favs, commitFavs, setPreview, setSubTab }) {
    ...
    const onDS = (e, i) => { dragRef.current = i; e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", String(i)); };
    const onDO = (e, i) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; setOverIdx(prev => prev !== i ? i : prev); };
    const onDL = (i) => setOverIdx(prev => prev === i ? null : prev);
    const onDP = (e, to) => {
        e.preventDefault();
    ...
    return (<div style={{ maxHeight: 280, overflowY: "auto", paddingRight: 2 }}>
            {favs.map((hex, i) => {
            const isDragged = dragRef.current === i;
            const isOver = overIdx === i && dragRef.current !== i;
            return (<div key={`${hex}_${i}`} draggable={editIdx !== i} onDragStart={e => editIdx !== i && onDS(e, i)} onDragOver={e => onDO(e, i)} onDragLeave={() => onDL(i)} onDrop={e => onDP(e, i)} onDragEnd={onDE} style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 8px", borderRadius: 7, marginBottom: 3, background: isOver ? "rgba(192,132,252,.09)" : "rgba(255,255,255,.03)", border: `1px solid ${isOver ? "rgba(192,132,252,.5)" : "rgba(255,255,255,.07)"}`, opacity: isDragged ? 0.3 : 1, cursor: editIdx === i ? "default" : "grab", userSelect: "none" }}>
                        <svg width="10" height="10" viewBox="0 0 12 12" fill="rgba(255,255,255,.3)" style={{ flexShrink: 0 }}>
                            <rect y="1" width="12" height="1.8" rx="0.9"/>
                            <rect y="5" width="12" height="1.8" rx="0.9"/>
                            <rect y="9" width="12" height="1.8" rx="0.9"/>
                        </svg>
                        <div style={{ width: 18, height: 18, borderRadius: 4, background: editIdx === i ? (bcrIsValidHex(editVal) ? editVal : "#333") : hex, border: "1.5px solid rgba(255,255,255,.18)", flexShrink: 0, cursor: "pointer" }} onClick={() => { if (editIdx !== i) {
                setPreview(hex);
                setSubTab("color");
            } }} title="Edit in Color tab"/>
                        {editIdx === i
                    ? <input autoFocus className="rs-item-input" value={editVal} maxLength={7} onChange={e => setEditVal(e.target.value)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter")
                        saveHexEdit(i); if (e.key === "Escape")
                        setEditIdx(null); }} onBlur={() => saveHexEdit(i)} style={{ flex: 1, fontFamily: "monospace", fontSize: 11, userSelect: "text" }}/>
                    : <span style={{ flex: 1, fontSize: 11, color: "#f0e6ff", fontFamily: "monospace", userSelect: "all", cursor: "text" }} onDoubleClick={e => { e.stopPropagation(); setEditIdx(i); setEditVal(hex); }}>{hex}</span>}
                        <button className="rs-edit-btn" title="Edit hex" onClick={e => { e.stopPropagation(); setEditIdx(i); setEditVal(hex); }}>&#9998;</button>
                        <button onClick={e => { e.stopPropagation(); if (editIdx === i)
                setEditIdx(null); void commitFavs(favs.filter((_, j) => j !== i)); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,.3)", fontSize: 12, padding: 0, outline: "none", lineHeight: 1, flexShrink: 0 }}>✕</button>
                    </div>);
        })}
        </div>);
}
function BannerTab({ forceUpdate }) {
    const [running, setRunning] = React.useState(bcrRotatorTimer !== null);
    const [subTab, setSubTab] = React.useState("color");
    const [mode, setMode] = React.useState(settings.store.bannerMode ?? "full_random");
    ...
    const [favs, setFavs] = React.useState([...bcrFavorites]);
    const [preview, setPreview] = React.useState(bcrCurrentColor ?? "#111214");
    const [applying, setApplying] = React.useState(false);
    const [liveColor, setLive] = React.useState(bcrCurrentColor);
    ...
    const commitFavs = async (nf) => { bcrFavorites = nf; setFavs([...nf]); bcrUsedFavs = bcrUsedFavs.filter(c => nf.includes(c)); await bcrSaveData(); };
    const handleModeChange = (m) => { setMode(m); settings.store.bannerMode = m; bcrRandomBatch = []; bcrSeqBatch = []; bcrUsedFavs = []; bcrGradientState = null; bcrMonoBaseHue = null; bcrSeqBaseHue = 0; bcrShadeStep = 0; bcrShadeDir = 1; if (running) {
        bcrStopRotator();
        bcrRotatorTimer = setTimeout(bcrRotateNext, Math.max(1, settings.store.bannerIntervalSeconds ?? BCR_DEFAULT_S) * 1000);
        setRunning(true);
    } };
    const handleSecBlur = (raw) => { const v = Math.max(5, parseInt(raw) || BCR_DEFAULT_S); setSec(v); setSecStr(String(v)); settings.store.bannerIntervalSeconds = v; if (running) {
        if (bcrRotatorTimer)
            clearTimeout(bcrRotatorTimer);
        bcrRotatorTimer = setTimeout(bcrRotateNext, v * 1000);
    } };
    const handleHueRChange = (v) => { setHueR(v); settings.store.bannerHueRadius = v; bcrRandomBatch = []; };
    const handleBaseChange = (hex) => { setBase(hex); settings.store.bannerCustomBaseColor = hex; bcrRandomBatch = []; bcrSeqBatch = []; bcrMonoBaseHue = null; bcrShadeStep = 0; bcrShadeDir = 1; };
    const handleToggle = () => {
        if (running) {
            bcrStopRotator();
            settings.store.bannerEnabled = false;
    ...
    const applyNow = async () => { if (applying || !bcrIsValidHex(preview))
        return; setApplying(true); await bcrApplyColor(preview); setApplying(false); };
    const skipNow = async () => { const color = await bcrPickNextColor(); setPreview(color); await bcrApplyColor(color); if (running) {
        if (bcrRotatorTimer)
            clearTimeout(bcrRotatorTimer);
        bcrRotatorTimer = setTimeout(bcrRotateNext, Math.max(1, sec) * 1000);
    } };
    ...
    const fmtS = (s) => s < 60 ? `${s}s` : s < 3600 ? `${Math.floor(s / 60)}m` : `${Math.floor(s / 3600)}h`;
    return (<div>
            ...
            <div className="bcr-sub-tab-bar">
                {[["color", "🎨 Color"], ["cycle", "⚙ Cycle"], ["favs", "⭐ Favs"]].map(([id, label]) => (<div key={id} className="bcr-sub-tab" onClick={() => setSubTab(id)} style={{ color: subTab === id ? "#c084fc" : "var(--text-muted)", borderBottomColor: subTab === id ? "#c084fc" : "transparent" }}>
                        {label}
                    </div>))}
            </div>

            {subTab === "color" && (<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 5 }}>
                        <button onClick={applyNow} disabled={applying || !bcrIsValidHex(preview)} style={{ flex: 1, padding: "6px 0", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: applying ? "wait" : "pointer", background: "rgba(192,132,252,.18)", border: "1px solid rgba(192,132,252,.44)", color: "#c084fc", outline: "none", opacity: !bcrIsValidHex(preview) ? 0.4 : 1 }}>
                            {applying ? "Applying…" : "Apply Now"}
                        </button>
                        <button onClick={() => { if (bcrIsValidHex(preview) && !favs.includes(prevLower))
            void commitFavs([...favs, prevLower]); }} disabled={!bcrIsValidHex(preview) || favs.includes(prevLower)} style={{ flex: 1, padding: "6px 0", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", background: "rgba(59,165,92,.13)", border: "1px solid rgba(59,165,92,.38)", color: "#3ba55c", outline: "none", opacity: (!bcrIsValidHex(preview) || favs.includes(prevLower)) ? 0.4 : 1 }}>
                    ...
                    {needsBase && (<div style={{ padding: "8px 10px", borderRadius: 6, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)" }}>
            ...
            {subTab === "cycle" && (<div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                    <div className="bcr-mode-grid">
                        {BCR_MODES.map(m => {
                const active = mode === m.id;
                return (<div key={m.id} className="bcr-mode-item" onClick={() => handleModeChange(m.id)} style={{ background: active ? "rgba(192,132,252,.18)" : "rgba(255,255,255,.03)", borderColor: active ? "rgba(192,132,252,.55)" : "rgba(255,255,255,.07)" }}>
                    ...
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        {PRESET_S.map(p => { const label = p < 60 ? `${p}s` : p < 3600 ? `${p / 60}m` : `${p / 3600}h`; const active = sec === p; return <button key={p} onClick={() => { setSec(p); setSecStr(String(p)); settings.store.bannerIntervalSeconds = p; if (running) {
            if (bcrRotatorTimer)
                clearTimeout(bcrRotatorTimer);
            bcrRotatorTimer = setTimeout(bcrRotateNext, p * 1000);
        } }} style={{ padding: "3px 8px", borderRadius: 5, fontSize: 10, fontWeight: 600, cursor: "pointer", outline: "none", border: `1px solid ${active ? "rgba(192,132,252,.55)" : "rgba(255,255,255,.07)"}`, background: active ? "rgba(192,132,252,.18)" : "rgba(255,255,255,.03)", color: active ? "#c084fc" : "var(--text-muted)", userSelect: "none" }}>{label}</button>; })}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                        <input type="number" min={5} value={secStr} onChange={e => setSecStr(e.target.value)} onBlur={e => handleSecBlur(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSecBlur(e.target.value)} style={{ width: 62, background: "rgba(0,0,0,.4)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 5, color: "#f0e6ff", fontSize: 12, padding: "4px 7px", outline: "none" }}/>
                        <span style={{ fontSize: 10, color: "var(--text-muted)" }}>sec = {fmtS(sec)}</span>
                    </div>
                    <BcrHr />
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {[["bannerShowToast", "Toast on each change"], ["bannerShowCurrentColor", "Show active color in footer (ColorBanner:)"]].map(([key, label]) => (<div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                                <span style={{ fontSize: 11, color: "#f0e6ff" }}>{label}</span>
                                <BcrToggle value={!!settings.store[key]} onChange={() => { settings.store[key] = !settings.store[key]; void bcrSaveData(); forceUpdate(); }}/>
                            </div>))}
                ...
                settings.store.bannerNitroCheck = n;
                if (n) {
                    startNitroWatcher();
                }
                else {
            ...
            {subTab === "favs" && (<div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                    {favs.length === 0
...
function RotatorSuiteModal({ modalProps }) {
    ...
    return (<ModalRoot {...modalProps} className="rs-modal">
            <ModalHeader>
                <div style={{ display: "flex", alignItems: "center", gap: 10, width: "100%" }}>
                    ...
                    {isGlobalSync && (<div style={{ fontSize: 10, padding: "2px 9px", borderRadius: 10, background: "rgba(255,167,38,.15)", color: C.data, fontWeight: 800, border: "1px solid rgba(255,167,38,.35)" }}>
            ...
            <ModalContent style={{ padding: "11px 15px", overflowY: "auto", maxHeight: "65vh" }}>
                {notActive && (<div style={{ background: "rgba(239,83,80,.1)", border: "1px solid rgba(239,83,80,.4)", borderRadius: 8, padding: "8px 13px", marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div className="rs-tab-bar">
                    {tabs.map(t => (<button key={t.id} className="rs-tab" style={tab === t.id ? { color: t.color, borderBottomColor: t.color } : {}} onClick={() => setTab(t.id)}>{t.label}</button>))}
                </div>
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: 8, width: "100%", alignItems: "center", flexWrap: "wrap" }}>
                    {(isManualStop || wasInvisible) && (<div style={{ width: "100%", padding: "5px 10px", borderRadius: 7, background: "rgba(239,83,80,.1)", border: "1px solid rgba(239,83,80,.3)", fontSize: 11, color: "#ef9a9a", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {settings.store.stopOnInvisible && !wasInvisible && !isManualStop && (<div style={{ width: "100%", padding: "4px 10px", borderRadius: 7, background: "rgba(124,77,255,.07)", border: "1px solid rgba(124,77,255,.18)", fontSize: 10, color: "#9575cd" }}>
            ...
            : <>
                                ...
                                <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                    ...
                                    {settings.store.bannerShowCurrentColor && bcrCurrentColor && bcrIsValidHex(bcrCurrentColor) && (<span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
...
function RSUserAreaButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : active ? "Rotator Suite - [Running]" : "Rotator Suite"} redGlow={!active} plated={nameplate != null} icon={<svg className={iconForeground} width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                ...
                </svg>} onClick={() => openModal(props => <RotatorSuiteModal modalProps={props}/>)}/>);
}
export default definePlugin({
    ...
    settingsAboutComponent: () => (<div style={{ marginTop: 10 }}>
            <Button color={Button.Colors.BRAND} onClick={() => openModal(props => <RotatorSuiteModal modalProps={props}/>)}>
                Open Rotator Suite Panel
            </Button>
    ...
    async start() {
        injectCSS();
        if (!window._rsCloseReg) {
            window.addEventListener("beforeunload", rsBeforeUnloadHandler);
            window._rsCloseReg = true;
        }
        cachedToken = null;
        cachedGuildStore = null;
        cachedClanGuilds = [];
        lastClanFetch = 0;
        statusLastVal = null;
        clanLastVal = null;
        ...
        const stored = (await DataStore.get(SK)) ?? defaults;
        storeCreatedAt = stored.createdAt ?? defaults.createdAt;
        globalNicks = stored.globalNicks ?? [];
        guilds = (stored.guilds ?? []).map((g) => ({
            ...g,
            nickMode: g.nickMode ?? (g.useGlobal ? "global" : "custom"),
            lastNickVal: null,
            ...
            guildPronounsMode: g.guildPronounsMode ?? "custom",
        ...
        if (Array.isArray(stored.statusEntries)) {
            statusEntries = stored.statusEntries;
        }
        else if (typeof stored.statuses === "string") {
            statusEntries = parseLegacyStatuses(stored.statuses);
        }
        statusPresets = Array.isArray(stored.statusPresets) ? stored.statusPresets : [];
        clanIds = stored.clanIds ?? [];
        clanServerNames = stored.clanServerNames ?? {};
        statusSeqIdx = stored.statusSeqIdx ?? 0;
        clanSeqIdx = stored.clanSeqIdx ?? 0;
        bioSeqIdx = stored.bioSeqIdx ?? 0;
        prSeqIdx = stored.prSeqIdx ?? 0;
        statusLastVal = stored.statusLastVal ?? null;
        clanLastVal = stored.clanLastVal ?? null;
        bioLastVal = stored.bioLastVal ?? null;
        prLastVal = stored.prLastVal ?? null;
        globalNickEntries = stored.globalNickEntries ?? [];
        globalNickSeqIdx = stored.globalNickSeqIdx ?? 0;
        globalNickLastVal = stored.globalNickLastVal ?? null;
        globalGuildPronouns = stored.globalGuildPronouns ?? [];
        syncGuildsFromDiscord();
        await saveData();
        const arStored = (await DataStore.get(AR_SK)) ?? { avatars: [], seqIndex: 0, shuffleQueue: [] };
        arAvatars = arStored.avatars ?? [];
        arSeqIndex = arStored.seqIndex ?? 0;
        arShuffleQueue = arStored.shuffleQueue ?? [];
        if (settings.store.avatarEnabled && arGetActive().length)
            arStartRotator(false);
        const bcrStored = (await DataStore.get(BCR_SK)) ?? { favorites: [], usedFavs: [], wasRunning: false, currentColor: null };
        bcrFavorites = bcrStored.favorites ?? [];
        bcrUsedFavs = bcrStored.usedFavs ?? [];
        bcrCurrentColor = bcrStored.currentColor ?? null;
        if (bcrStored.wasRunning || settings.store.bannerEnabled)
            bcrStartRotator(false);
        ...
        try {
            localStorage.setItem(RS_CLOSE_LS, JSON.stringify({ closeStatusEnabled, closeStatusText, closeStatusEmoji, closeStatusType, closeClanEnabled, closeClanId, closeBannerEnabled, closeBannerColor }));
        }
        catch { }
        ...
        if (settings.store.stopOnInvisible)
            startInvisibleWatcher();
        if (settings.store.bannerNitroCheck)
            startNitroWatcher();
        if (settings.store.avatarEnabled && settings.store.avatarNitroCheck)
            startAvatarNitroWatcher();
    },
    stop() {
        pluginActive = false;
        ...
        cachedToken = null;
        cachedGuildStore = null;
        cachedVoiceStateStore = null;
        cachedChannelStore = null;
        isManualStop = false;
        wasInvisible = false;
        domTagCache.clear();
        if (globalStopTimer) {
