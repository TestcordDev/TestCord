...
function playSyntheticSound(sound) {
    try {
...
async function playUrlSound(sound) {
    let audioContext;
    try {
        if (!sound.url)
            return false;
...
function playLocalAudioFileInDiscord(fileUrl, volume = 0.5) {
    return new Promise(resolve => {
        try {
            ...
            try {
                ...
                console.log("[SoundboardPro] Discord API failed, attempting with native Audio:", discordError);
                ...
                audio.oncanplaythrough = () => {
                    ...
                    audio
                        ...
                        .catch(error => {
                        console.error("[SoundboardPro] Error during file playback (native):", error);
                        resolve(false);
                    });
                };
                audio.onerror = error => {
...
function SoundboardModal({ modalProps }) {
    ...
    const addCustomSound = () => {
        if (!customSoundUrl.trim() || !customSoundName.trim())
            return;
        const newSound = {
            id: `custom_${Date.now()}`,
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            gap: "8px",
                        ...
                        {sounds.map(sound => {
            const isLocalFile = sound.url?.startsWith("blob:") || false;
            return (<Button key={sound.id} onClick={() => handlePlaySound(sound)} disabled={isPlaying === sound.id} color={Button.Colors.PRIMARY} look={Button.Looks.FILLED} style={{
                    height: "70px",
                    display: "flex",
                    ...
                    border: isLocalFile ? "2px solid var(--green-360)" : "none",
                                    ...
                                    {isLocalFile && (<span style={{ fontSize: "8px", color: "var(--green-360)" }}>
                                            📁
                                        </span>)}
                                    {sound.id.startsWith("custom_") && !isLocalFile && (<span style={{ fontSize: "8px", color: "var(--text-muted)" }}>
                                            🎵
                                        </span>)}
                                </Button>);
        })}
                ...
                {/* Add custom sound */}
                {settings.store.enableCustomSounds && (<div style={{
                ...
                paddingTop: "16px",
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                            <Button onClick={openFileSelector} color={Button.Colors.BRAND} size={Button.Sizes.SMALL} style={{ width: "100%" }}>
                                📁 Select an MP3 file
                            </Button>
                            ...
                            {/* Add via URL */}
                            <input type="text" placeholder="Sound name" value={customSoundName} onChange={e => setCustomSoundName(e.target.value)} style={{
                padding: "8px 12px",
                borderRadius: "4px",
                ...
                fontSize: "14px",
                            ...
                            <input type="url" placeholder="Audio file URL (MP3, WAV, OGG)" value={customSoundUrl} onChange={e => setCustomSoundUrl(e.target.value)} style={{
                padding: "8px 12px",
                borderRadius: "4px",
                border: "1px solid var(--background-modifier-accent)",
                backgroundColor: "var(--input-background)",
                ...
                fontSize: "14px",
                            ...
                            <Button onClick={addCustomSound} disabled={!customSoundUrl.trim() || !customSoundName.trim()} color={Button.Colors.GREEN} size={Button.Sizes.SMALL}>
                                Add via URL
                            </Button>
                        </div>
                    </div>)}
            ...
            <ModalFooter>
                {React.createElement(Flex, { flexDirection: "row-reverse" }, React.createElement(Button, {
...
export function openSoundboardPro() {
    ...
    try {
        const modalKey = openModal(modalProps => React.createElement(SoundboardModal, { modalProps }));
...
function SoundboardIcon() {
    return (<img src="./src/bashplugins/soundboardPro/icone.webp" alt="Soundboard Pro" width="28" height="28" style={{
            borderRadius: "4px",
            objectFit: "cover",
        }}/>);
}
function SoundboardButton() {
    return (<PanelButton tooltipText="Soundboard Pro" icon={SoundboardIcon} onClick={() => {
            openSoundboardPro();
        }}/>);
}
// Settings component
function SettingsComponent() {
    return (<div>
            ...
            <div style={{ marginBottom: "16px" }}>
                <Button onClick={openSoundboardPro} color={Button.Colors.BRAND} style={{ width: "100%" }}>
                    🎵 Open Soundboard Pro
                </Button>
            </div>
...
export default definePlugin({
    name: "SoundboardPro",
    description: "Advanced soundboard with real sounds and synthetic sounds. Plays sounds directly in Discord voice channel.",
    tags: ["Voice", "Utility"],
