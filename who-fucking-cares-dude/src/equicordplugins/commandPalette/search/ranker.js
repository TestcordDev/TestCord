...
export function fuzzyScore(query, text) {
    ...
    if (t === q)
        return 1000;
    if (t.startsWith(q))
        return 800 + Math.max(0, 60 - t.length);
    const wordIndex = t.indexOf(" " + q);
    if (wordIndex !== -1)
        return 650 - Math.min(wordIndex, 100);
    const subIndex = t.indexOf(q);
    if (subIndex !== -1)
        return 450 - Math.min(subIndex, 200);
    let ti = 0;
    let first = -1;
    for (const char of q) {
...
export function rankCommands(query, commands) {
    ...
    for (const command of commands) {
        ...
        if (alias)
            score = Math.max(score, fuzzyScore(query, alias) * 1.3);
        for (const keyword of command.keywords ?? []) {
            score = Math.max(score, fuzzyScore(query, keyword) * 0.7);
        }
        if (command.subtitle)
            score = Math.max(score, fuzzyScore(query, command.subtitle) * 0.4);
        score = Math.max(score, fuzzyScore(query, command.section) * 0.3);
        if (score <= 0)
            continue;
        score += frecencyScore(command.id) * 8;
        if (isPinned(command.id))
            score += 40;
        ranked.push({ command, score });
    }
    return ranked.sort((a, b) => b.score - a.score);
}
export function filterOptions(query, options, limit = 6) {
