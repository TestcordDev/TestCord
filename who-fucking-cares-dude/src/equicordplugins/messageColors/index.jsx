...
import { regex, replaceRegexp, settings } from "./constants";
...
export default definePlugin({
    ...
    getColor(order) {
        ...
        return {
            ...
            parse(matchedContent, _, parseProps) {
            ...
            react: ErrorBoundary.wrap(({ text, colorType, color }) => {
                if (settings.store.renderType === 1 /* RenderType.FOREGROUND */) {
                    return <span style={{ color: color }}>{text}</span>;
                }
                const styles = {
                    "--color": color
                };
                if (settings.store.renderType === 2 /* RenderType.BACKGROUND */) {
...
const calcRGBLightness = (r, g, b) => {
    return 0.2126 * r + 0.7152 * g + 0.0722 * b;
};
const isColorDark = (color, type) => {
    const border = 115;
    switch (type) {
        case 1 /* ColorType.RGBA */:
        case 0 /* ColorType.RGB */: {
            const match = color.match(/\d+/g);
            const lightness = calcRGBLightness(+match[0], +match[1], +match[2]);
            return lightness < border;
        }
        case 2 /* ColorType.HEX */: {
...
const getColorType = (color) => {
    ...
    if (color.startsWith("#"))
        return 2 /* ColorType.HEX */;
    if (color.startsWith("hsl"))
        return 3 /* ColorType.HSL */;
    if (color.startsWith("rgba"))
        return 1 /* ColorType.RGBA */;
    if (color.startsWith("rgb"))
        return 0 /* ColorType.RGB */;
    throw new Error(`Can't resolve color type of ${color}`);
};
function parseColor(str, type) {
