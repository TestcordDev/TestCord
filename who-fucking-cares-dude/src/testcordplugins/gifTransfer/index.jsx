...
function getAddGifFn() {
    ...
    if (!actions || typeof actions.updateAsync !== "function")
        return null;
    return (gif) => {
        const existing = getCurrentGifs();
        const orders = Object.values(existing).map((g) => g?.order ?? 0);
        const maxOrder = orders.length > 0 ? Math.max(...orders) : 0;
        const newOrder = maxOrder + 1;
        actions.updateAsync("favoriteGifs", (favoriteGifs) => {
            if (!favoriteGifs)
                return;
            if (!favoriteGifs.gifs)
                favoriteGifs.gifs = {};
            if (favoriteGifs.gifs[gif.url])
                return;
...
function applyRuntimeUnlock() {
    if (runtimeUnlockApplied)
        return true;
...
async function exportGifs() {
    ...
    const gifsArray = entries.map(([url, data]) => ({
        url,
        src: data.src,
...
// ─── Verify ──────────────────────────────────────────────────────────────────
async function verifyGifs(file) {
    try {
        ...
        for (const gif of data.gifs) {
            if (urlsSeen.has(gif.url))
                duplicates++;
            else
                urlsSeen.add(gif.url);
        }
        const report = [
...
// ─── Import ──────────────────────────────────────────────────────────────────
async function importGifs(file) {
    ...
    if (filedupes > 0)
        console.log(`[GifTransfer] Removed ${filedupes} duplicate URLs from import file.`);
    const currentGifs = getCurrentGifs();
    const currentUrls = new Set(Object.keys(currentGifs));
...
function injectButtons(navList) {
    if (navList.querySelector(`#${BUTTONS_ID}`))
        return;
...
export default definePlugin({
    ...
    start() {
        ...
        if (settings.store.runtimeUnlock)
            applyRuntimeUnlock();
        startObserver();
    },
    stop() {
