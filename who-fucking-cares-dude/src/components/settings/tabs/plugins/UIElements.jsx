...
export function UIElementsButton() {
    return (<Clickable onClick={() => openModal(modalProps => <UIElementsModal {...modalProps}/>)}>
...
function Section(props) {
    ...
    const switches = Array.from(buttonMap, ([name, { icon }]) => {
        ...
        return (<Paragraph size="md" weight="semibold" key={name} className={cl("switches-row")}>
                <Icon height={20} width={20}/>
                {name}
                <Switch checked={settings[name]?.enabled ?? true} onChange={v => {
                settings[name] ??= {};
    ...
    return (<section>
            ...
            <div className={cl("switches")}>
                {switches.length === 0 && (<Paragraph weight="medium" className={cl("switches-row")} style={{ color: "var(--text-muted)" }}>
...
function UIElementsModal(props) {
    const { uiElements } = useSettings(["uiElements.*"]);
    return (<Modal {...props} size="md" title="Manage plugin UI elements">
            <div className={cl("modal-content")}>
                <Section title="Chatbar Buttons" description="These are the buttons on the right side of the chat input bar" buttonMap={ChatBarButtonMap} settings={uiElements.chatBarButtons}/>
                <Section title="Message Popover Buttons" description="These are the floating buttons on the right when you hover over a message" buttonMap={MessagePopoverButtonMap} settings={uiElements.messagePopoverButtons}/>
            </div>
        </Modal>);
}
