...
let origAssign;
let origReplace;
let origHrefDescriptor;
let origGo;
let preventMediaError = null;
let preventMediaRejection = null;
let suppressTimer;
let suppressReload = false;
let streaming = false;
let inVoiceChannel = false;
const onVoiceChannelSelect = ({ channelId }) => {
    if (channelId) {
        inVoiceChannel = true;
        armSuppress();
    }
...
function logErrorSync(source, error) {
    try {
...
export default definePlugin({
    ...
    start() {
        ...
        try {
            ...
            if (raw) {
                ...
                if (log.length > 0) {
                    const entries = log.map((e) => JSON.parse(e));
        ...
        try {
            ...
            proto.assign = function (url) {
                if (suppressReload && (!url || url === window.location.href))
                    return;
        ...
        try {
            ...
            history.go = function (delta) {
                if (suppressReload && (delta === undefined || delta === 0))
                    return;
                return origGo(delta);
            };
        }
    ...
    stop() {
        ...
        try {
            ...
            if (origAssign) {
                proto.assign = origAssign;
                origAssign = undefined;
            }
            if (origReplace) {
                proto.replace = origReplace;
                origReplace = undefined;
            }
