...
export default definePlugin({
    ...
    wrapSort(comparator, row) {
        return row.type === 3 || row.type === 4
            ? -getSince(row.user)
            : comparator(row);
    },
    makeSubtext(user, origSubtext) {
        const since = getSince(user);
        if (isNaN(since.getTime())) {
            return null;
        }
        return (<Flex flexDirection="column" style={{ gap: "0px", flexWrap: "wrap", lineHeight: "0.9rem" }}>
                <span>{origSubtext}</span>
                <span>
                    <div className="" style={{ display: "flex", alignItems: "center" }}>
                        <svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="8" height="8" fill="none" viewBox="0 0 24 24" style={{ marginRight: "4px", display: "inline-block" }}>
                            <path fill="var(--input-placeholder-text)" fillRule="evenodd" d="M12 23a11 11 0 1 0 0-22 11 11 0 0 0 0 22Zm1-18a1 1 0 1 0-2 0v7c0 .27.1.52.3.7l3 3a1 1 0 0 0 1.4-1.4L13 11.58V5Z" clipRule="evenodd"></path>
                        </svg>
    ...
    WrapperDateComponent: ErrorBoundary.wrap(({ user, children }) => {
        const since = getSince(user);
        return <div className={cl("wrapper")}>
            {children}
            {!isNaN(since.getTime()) && (<TooltipContainer text={DateUtils.dateFormat(since, "LLLL")} tooltipClassName={cl("tooltip")}>
