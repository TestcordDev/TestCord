...
const Native = IS_DISCORD_DESKTOP
    ? VencordNative?.pluginHelpers?.StreamProof
    : undefined;
...
function isStreaming() {
    try {
        ...
        if (allStreams && allStreams.length > 0) {
            const myStream = allStreams.find((s) => s.ownerId === currentUser.id);
            if (myStream)
                return true;
        }
...
function handleStreamChange() {
    if (!settings.store.autoStreamProof)
        return;
    if (isStreaming()) {
        enableStreamProof();
    }
...
// ── Eye Icons ──────────────────────────────────────────────────────────────────
function EyeIcon({ height = 20, width = 20 }) {
...
function EyeSlashIcon({ height = 20, width = 20 }) {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24">
...
// ── Chat Bar Button ────────────────────────────────────────────────────────────
const StreamProofButton = ({ isMainChat }) => {
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("StreamProof", () => (<HeaderBarButton icon={() => streamProofActive ? <EyeSlashIcon /> : <EyeIcon />} tooltip={streamProofActive ? "StreamProof: ON — click to disable" : "StreamProof: OFF — click to enable"} onClick={() => { streamProofActive ? disableStreamProof() : enableStreamProof(); }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("StreamProof", () => (<ChannelToolbarButton icon={() => streamProofActive ? <EyeSlashIcon /> : <EyeIcon />} tooltip={streamProofActive ? "StreamProof: ON — click to disable" : "StreamProof: OFF — click to enable"} onClick={() => { streamProofActive ? disableStreamProof() : enableStreamProof(); }}/>), 5);
        }
