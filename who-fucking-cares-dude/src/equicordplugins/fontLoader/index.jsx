...
const createGoogleFontUrl = (family, options = "") => `https://fonts.googleapis.com/css2?family=${encodeURIComponent(family)}${options}&display=swap`;
const loadFontStyle = (url) => {
...
async function searchGoogleFonts(query) {
    try {
        ...
        const data = await TestcordRequestCoordinator.request({
            ...
            cacheable: value => Array.isArray(value?.[1]),
        });
        if (!data?.[1])
            return [];
        return data[1].map(([_, fontData]) => ({
            family: fontData[0],
            displayName: fontData[1],
            authors: fontData[2],
            category: fontData[3],
            variants: fontData[6].map((variant) => ({
                axes: variant[0].map(([tag, min, max]) => ({
                    tag, min, max
                }))
            }))
        }));
...
const preloadFont = (family) => loadFontStyle(createGoogleFontUrl(family, ":wght@400;700"));
...
function GoogleFontSearch({ onSelect }) {
    const [query, setQuery] = React.useState("");
    const [results, setResults] = React.useState([]);
    const [loading, setLoading] = React.useState(false);
    const previewStyles = React.useRef([]);
    React.useEffect(() => () => {
    ...
    return (<section>
            ...
            {results.length > 0 && (<div className={classes(Margins.top8, "eq-googlefonts-results")}>
                    {results.map(font => (<Card key={font.family} className={classes("eq-googlefonts-card", Margins.bottom8)} onClick={() => onSelect(font)}>
                            ...
                            {font.authors?.length && (<Paragraph className={Margins.top8} style={{ opacity: 0.7 }}>
...
const settings = definePluginSettings({
    ...
    fontSearch: {
        ...
        component: () => (<GoogleFontSearch onSelect={font => {
