...
let cachedMarketplaceThemes = null;
let lastFetchTime = 0;
...
function installMarketplaceTheme(theme) {
...
function uninstallMarketplaceTheme(theme) {
    const themeLink = getThemeLink(theme.id);
            ...
            const actions = [
                {
                    ...
                    run: () => {
                        if (installed)
                            uninstallMarketplaceTheme(theme);
                        else
                            installMarketplaceTheme(theme);
                    }
                },
                {
                    id: "details",
                    ...
                    run: ctx => {
                        ctx.push({
                            ...
                            spec: {
                                type: "detail",
                                heading: theme.name,
                                body: (<div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                                        {theme.thumbnail_url && (<img src={theme.thumbnail_url} alt={theme.name} style={{
                                            width: "100%",
                                            maxHeight: "220px",
                                            borderRadius: "8px",
                                            objectFit: "cover"
                                        }}/>)}
                                        {theme.description && (<p style={{ margin: 0, opacity: 0.9, lineHeight: 1.4 }}>
                                                {theme.description}
                                            </p>)}
                                    </div>),
                                rows: [
...
async function installedThemeItems() {
    const items = [];
    // Online Themes
    const links = Array.isArray(Settings.themeLinks) ? Settings.themeLinks : [];
