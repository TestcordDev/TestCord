...
import { cancelUploadTracking, clearGlobalState, getAllUploads, getCurrentProgress, getForceHide, getIsComplete, getIsDispatched, switchToUpload } from "../progress";
const Native = VencordNative.pluginHelpers.BigFileUpload;
export function UploadProgressBar() {
    const [progress, setProgress] = useState(getCurrentProgress());
    const [allUploads, setAllUploads] = useState(getAllUploads());
    const [isComplete, setIsComplete] = useState(false);
    const [isExpanded, setIsExpanded] = useState(false);
    const [isVisible, setIsVisible] = useState(false);
    const [barWidth, setBarWidth] = useState("50%");
    const [bgColor, setBgColor] = useState("var(--input-background)");
    const [isCancelling, setIsCancelling] = useState(false);
    // Store updateWidth function ref so it can be called from visibility effect
    const updateWidthRef = React.useRef(null);
        ...
        const interval = setInterval(() => {
            ...
            if (current && current.percent >= 100 && !complete) {
                if (!stuckAt100Timer) {
                    ...
                    stuckAt100Timer = true; // Just mark that we've logged, don't set a timer
                }
            }
    ...
    useEffect(() => {
        const updateWidthAndPosition = () => {
            ...
            const channelTextArea = document.querySelector('[class*="channelTextArea"]');
            const scrollableContainer = document.querySelector('[class*="scrollableContainer"]');
            if (channelTextArea && scrollableContainer) {
                ...
                if (targetElement) {
                    ...
                    const chatForm = document.querySelector("form[class*=\"form\"]");
                    if (chatForm) {
    ...
    return (<div id="upload-progress-wrapper" style={{
            position: "relative",
            width: "100%",
            ...
            zIndex: 1
                ...
                textAlign: "right"
                        ...
                        {allUploads.length > 1 && (<span style={{
                    marginLeft: "8px",
                    padding: "2px 6px",
                    ...
                    color: "var(--header-primary)"
                        ...
                        {/* Cancel button */}
                        {!isComplete && progress?.uploadId && (<button disabled={isCancelling} onClick={async (e) => {
                    e.stopPropagation();
                    const uploadId = progress?.uploadId;
                    const fileName = progress?.fileName || "current upload";
                    if (!uploadId || isCancelling)
                        return;
                ...
                }} disabled={allUploads.findIndex(u => u.uploadId === progress?.uploadId) === 0} style={{
                    padding: "4px 8px",
                    backgroundColor: "var(--background-secondary)",
                    border: "1px solid var(--background-modifier-accent)",
                    borderRadius: "4px",
                ...
                }} disabled={allUploads.findIndex(u => u.uploadId === progress?.uploadId) === allUploads.length - 1} style={{
                    padding: "4px 8px",
                    backgroundColor: "var(--background-secondary)",
                    border: "1px solid var(--background-modifier-accent)",
                    borderRadius: "4px",
