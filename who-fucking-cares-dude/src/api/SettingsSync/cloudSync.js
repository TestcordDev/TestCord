...
export const setCloudSyncDirection = (direction) => localStorage.setItem(SYNC_DIRECTION_KEY, direction);
...
async function loadApiVersionMap() {
    return await DataStore.get(API_VERSION_STORE_KEY) ?? {};
}
async function getApiVersion() {
    const map = await loadApiVersionMap();
    return map[getCloudUrl().origin] ?? "v2";
}
async function setApiVersion(version) {
    await DataStore.update(API_VERSION_STORE_KEY, map => {
        map ??= {};
...
async function getLocalManifest() {
    return await DataStore.get(MANIFEST_STORE_KEY) ?? [];
}
...
async function applyDownloads(downloads) {
...
async function doSyncV2(uploads, clientManifest) {
    let res;
    try {
        res = await fetch(new URL("/v2/sync", getCloudUrl()), {
            ...
            body: JSON.stringify({ client_manifest: clientManifest, uploads }),
        });
    }
    catch (e) {
...
async function getV2(shouldNotify, force) {
...
async function getV1(shouldNotify, force) {
    const res = await fetch(new URL("/v1/settings", getCloudUrl()), {
        method: "GET",
        headers: {
            Authorization: await getCloudAuth(),
