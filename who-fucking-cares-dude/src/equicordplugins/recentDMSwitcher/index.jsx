...
const settings = definePluginSettings({
    ...
    clearRdms: {
        ...
        component: () => (<Button color={Button.Colors.RED} onClick={async () => {
                rmdsDmChannelIds = [];
                cycleSnapshot = [];
                cycleIndex = -1;
...
let overlayModalKey = null;
let overlayRerender = null;
function isDirectMessageChannel(channelId) {
    if (!channelId)
        return false;
...
function pushChannelToFront(channelId) {
    rmdsDmChannelIds = rmdsDmChannelIds.filter(id => id !== channelId);
    rmdsDmChannelIds.unshift(channelId);
    if (rmdsDmChannelIds.length > settings.store.amountOfUsers)
        rmdsDmChannelIds.length = settings.store.amountOfUsers;
    void DataStore.set(STORAGE_KEY, rmdsDmChannelIds);
}
function sanitizeHistory(ids) {
    const seen = new Set();
    const result = [];
    for (const id of ids) {
        if (!id || seen.has(id))
            continue;
        if (!ChannelStore.hasChannel(id))
            continue;
        if (!isDirectMessageChannel(id))
            continue;
        seen.add(id);
        result.push(id);
        if (result.length >= settings.store.amountOfUsers)
            break;
    }
    return result;
}
function beginCycleSession() {
    if (isCyclingSessionActive)
        return;
...
function stepCycle(direction) {
    if (!isCyclingSessionActive || cycleSnapshot.length === 0)
        return;
    ...
    if (!targetId || !ChannelStore.hasChannel(targetId))
        return;
    const vis = settings.store?.visualStyle;
    if (vis === "overlay")
        renderOverlay();
    else if (vis === "toast")
        showCycleToast();
...
function endCycleSession() {
    ...
    cycleIndex = -1;
    activeToastId = null;
    const visEnd = settings.store?.visualStyle;
    if (visEnd === "overlay")
        unmountOverlay();
}
...
function onKeyUp(e) {
    if (e.key === "Control" || (IS_MAC && e.key === "Meta")) {
        stopEvent(e);
        const stillHeld = e.ctrlKey || e.metaKey;
        if (!stillHeld)
            endCycleSession();
    }
}
function getDisplayForChannel(id) {
    ...
    if (ch.isGroupDM()) {
        return { name: ch.name ?? "Group DM", avatar: IconUtils.getChannelIconURL?.(ch) ?? "" };
    }
...
function OverlayContent() {
    ...
    const cards = pageItems.map(id => {
        ...
        return (<div key={id} className={cl("background")} style={{
                boxShadow: isActive
    ...
    const dots = pageCount > 1 ? (<div className={cl("page-indicator-container")}>
                {Array.from({ length: pageCount }).map((_, i) => (<div key={i} className={cl("page-indicator")} style={{
                background: i === currentPage ? "var(--brand-500)" : "var(--interactive-muted)",
                opacity: i === currentPage ? 1 : 0.6,
            }}/>))}
            </div>) : null;
    return (<div className={cl("overlay-container")}>
...
export default definePlugin({
    ...
    async start() {
        ...
        if (isDirectMessageChannel(current))
            pushChannelToFront(current);
