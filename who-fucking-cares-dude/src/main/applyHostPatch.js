...
export const isAlreadyPatched = (resources) => existsSync(join(resources, "_app.asar"));
/**
 ...
 */
export const patchResourcesDir = (resources, patcherJsPath) => {
    const app = join(resources, "app.asar");
    const _app = join(resources, "_app.asar");
    if (isAlreadyPatched(resources) || !existsSync(app) || existsSync(_app))
        return false;
    try {
        if (lstatSync(app).isDirectory())
            return false;
    }
    catch {
        return false;
 ...
 */
export const findStaleSibling = (currentExeDir) => {
