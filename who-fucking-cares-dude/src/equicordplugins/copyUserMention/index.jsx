...
const MentionIcon = () => (<svg className="mentionicon" height="20" width="20" viewBox="0 0 24 24" fill="currentColor">
...
const UserContextMenuPatch = (children, { user }) => {
    if (!user)
        return;
    children.push(<Menu.MenuItem id="vc-copy-user-mention" label="Copy User Mention" action={() => copyToClipboard(`<@${user.id}>`)} icon={MentionIcon}/>);
};
