...
class SpamProtection {
    commandHistory = [];
    canExecuteCommand(userId, targetUserId) {
        const now = Date.now();
        const windowMs = settings.store.spamProtectionWindow * 1000;
        this.commandHistory = this.commandHistory.filter(entry => now - entry.timestamp < windowMs);
        const recentCommands = this.commandHistory.filter(entry => entry.userId === userId);
        if (recentCommands.length >= settings.store.maxCommandsPerWindow) {
            console.log(`Rate limit exceeded for user ${userId}`);
            return false;
        }
        const duplicateTarget = recentCommands.find(entry => entry.targetUserId === targetUserId);
        if (duplicateTarget) {
    ...
    getRemainingCooldown(userId) {
        const now = Date.now();
        const windowMs = settings.store.spamProtectionWindow * 1000;
        const recentCommands = this.commandHistory.filter(entry => entry.userId === userId && now - entry.timestamp < windowMs);
...
class ShareBanManager {
    ...
    addAuthorizedUser(userId, username, grantedBy) {
    ...
    toggleAuthorization(userId, username) {
    ...
    handleMessage(message) {
        if (!settings.store.enabled)
            return;
        if (!this.isUserAuthorized(message.author.id))
            return;
        if (message.author.bot || message.type !== 0)
            return;
        const content = message.content?.trim();
        if (!content || !content.startsWith(settings.store.commandPrefix))
            return;
    ...
    isVoiceChannelTextChat(channelId) {
        const channel = ChannelStore.getChannel(channelId);
        return channel && (channel.type === 2 || channel.parent_id !== null);
    }
    queueCommand(channelId, command) {
...
const UserContext = (children, props) => {
    ...
    children.splice(-1, 0, <Menu.MenuGroup>
            <Menu.MenuCheckboxItem id="share-ban-perm" label="Share Ban Permission" checked={isAuthorized} action={async () => {
...
function handleMessageCreate(data) {
    if (!settings.store.enabled)
        return;
    const { message } = data;
    if (!message?.author || !message.id || !message.channel_id)
        return;
