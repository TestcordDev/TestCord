...
const error = (...data) => console.error(`[Plugin:YTdownloader] [ERROR] ${data.join(" ")}`);
function killActiveProcesses() {
    ytdlpProcess?.kill();
    ffmpegProcess?.kill();
}
function ytdlp(ctx, args) {
...
async function metadata(ctx, options) {
    ctx.stdout = "";
...
function genFormat(ctx, { videoTitle }, { format, quality }) {
    let format_string = "";
    // Default qualities
...
async function download(ctx, { format, videoTitle }, { url, format: usrFormat, audioBitrate }) {
    cleanVideoFiles();
    ...
    if (!file)
        throw new Error("No video file was found!");
    return { file, videoTitle };
}
async function remux(ctx, { file, videoTitle }, { format, maxFileSize }) {
    const sourceExtension = file.split(".").pop();
    if (!ffmpegAvailable)
        return log(ctx, "Skipping remux, ffmpeg is unavailable."), { file, videoTitle, extension: sourceExtension };
...
export async function checkffmpeg(_) {
    try {
...
export async function checkdeno(_) {
    try {
