...
export function isS3Configured() {
    const { s3Endpoint, s3Bucket, s3Region, s3AccessKeyId, s3SecretAccessKey } = settings.store;
    return Boolean(s3Endpoint && s3Bucket && s3Region && s3AccessKeyId && s3SecretAccessKey);
}
export async function uploadToS3(fileBlob, filename, native, uploadRequest) {
    const { s3Endpoint, s3Bucket, s3Region = "auto", s3AccessKeyId, s3SecretAccessKey, s3SessionToken, s3PublicUrl, s3Prefix, s3ForcePathStyle = true } = settings.store;
