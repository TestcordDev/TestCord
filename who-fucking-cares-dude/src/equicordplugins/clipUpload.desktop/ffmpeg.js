...
function getInputName(fileName, id) {
    return `input_${id}${fileName.match(/\.[a-z0-9]+$/i)?.[0].toLowerCase() ?? ".video"}`;
}
export async function convertClipToMp4(file, fileName) {
    ...
    try {
        ...
        if (exitCode !== 0)
            throw new Error("Couldn't convert the selected file.");
        const data = await ff.readFile(outputName);
        if (typeof data === "string")
            throw new Error("Couldn't read the converted file.");
