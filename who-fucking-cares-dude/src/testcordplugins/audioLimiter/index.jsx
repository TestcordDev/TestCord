...
function startLevelMonitoring() {
    if (!settings.store.enableDbLimiting || !limiterState.analyser)
        return;
    function monitorLevels() {
        if (!limiterState.isActive || !settings.store.enableDbLimiting)
            return;
...
function SettingsPanel() {
    return (<Forms.FormSection>
      ...
      <_Slider initialValue={settings.store.maxVolume} onValueChange={(value) => (settings.store.maxVolume = value)} min={10} max={100} markers={[50, 60, 70, 80, 90, 100]} stickToMarkers={false}/>
      <Forms.FormText>
        Maximum allowed volume: {settings.store.maxVolume}%
      </Forms.FormText>

      <Forms.FormTitle>Maximum Decibels (dB)</Forms.FormTitle>
      <_Slider initialValue={settings.store.maxDecibels} onValueChange={(value) => (settings.store.maxDecibels = value)} min={-20} max={0} markers={[-20, -15, -10, -6, -3, 0]} stickToMarkers={false}/>
      <Forms.FormText>
      ...
      <FormSwitch title="Enable volume limiting" value={settings.store.enableVolumeLimiting} onChange={value => (settings.store.enableVolumeLimiting = value)}/>
      <FormSwitch title="Enable decibel limiting" value={settings.store.enableDbLimiting} onChange={value => (settings.store.enableDbLimiting = value)}/>
      <FormSwitch title="Show notifications" value={settings.store.showNotifications} onChange={value => (settings.store.showNotifications = value)}/>
      <FormSwitch title="Show visual indicator" value={settings.store.showVisualIndicator} onChange={value => (settings.store.showVisualIndicator = value)}/>
...
export default definePlugin({
    name: "Audio Limiter",
    description: "Automatically limits output volume to avoid sounds that are too loud",
    tags: ["Voice", "Utility"],
