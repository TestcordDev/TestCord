...
const makeContextMenuPatch = (shouldAddIcon) => (children, props) => {
    const { guild, folderId } = props;
    if (guild) {
        const group = findGroupChildrenByChildId("privacy", children);
        if (!group)
            return;
        group.push(<Menu.MenuItem label="Apply NewGuildSettings" id="vc-newguildsettings-apply" icon={shouldAddIcon ? CogWheel : void 0} action={() => applyDefaultSettings(guild.id)}/>);
    }
    if (folderId) {
        const folder = SortedGuildStore.getGuildFolderById(folderId);
        children.push(<Menu.MenuItem label="Apply NewGuildSettings to Folder" id="vc-newguildsettings-apply-folder" icon={shouldAddIcon ? CogWheel : void 0} action={async () => {
...
function applyVoiceNameHidingToGuild(guildId) {
...
function applyDefaultSettings(guildId) {
    if (guildId === "@me" || guildId === "null" || guildId == null)
        return;
