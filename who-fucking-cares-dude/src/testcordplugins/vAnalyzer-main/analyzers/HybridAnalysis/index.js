...
function hasResults(data) {
    ...
    if (!s)
        return false;
    return Object.values(s).some((v) => v && v.status !== "in-queue");
}
...
async function waitForResults(apiKey, scanId, silent) {
    for (let i = 0; i < 10; i++) {
        await sleep(3000);
        if (!silent && i === 0)
            safeToast("Waiting for Hybrid Analysis results...");
        const poll = await Native.hybridAnalysisGetScan(apiKey, scanId);
        if (poll.status === 200 && poll.data) {
            if (isFinished(poll.data) || hasResults(poll.data))
                return poll.data;
        }
    }
    return null;
}
function buildHashDetails(reports) {
    const details = [];
...
async function submitAndWait(submitFn, silent) {
    const apiKey = settings.store.hybridAnalysisApiKey;
    if (!apiKey) {
        if (!silent)
            safeToast("Hybrid Analysis API key required. Set it in vAnalyzer settings.", Toasts.Type.FAILURE);
        return null;
    }
    const result = await submitFn();
    if (result.status !== 200 || !result.data) {
        if (!silent)
            safeToast(`Hybrid Analysis scan failed: ${result.error ?? result.status}`, Toasts.Type.FAILURE);
        return null;
    }
    let { data } = result;
    if (!isFinished(data) && !hasResults(data) && data.id) {
        ...
        if (!data) {
            if (!silent)
                safeToast("Hybrid Analysis scan timed out", Toasts.Type.FAILURE);
            return null;
        }
    }
    return { details: buildDetails(data), timestamp: Date.now() };
}
export async function analyzeUrlWithHybridAnalysis(url, silent = false) {
    if (!silent)
        safeToast("Submitting URL to Hybrid Analysis...");
    const apiKey = settings.store.hybridAnalysisApiKey;
    return submitAndWait(() => Native.hybridAnalysisQuickScanUrl(apiKey, url), silent);
}
export async function analyzeFileWithHybridAnalysis(fileUrl, fileName, silent = false) {
    const apiKey = settings.store.hybridAnalysisApiKey;
    if (!apiKey) {
        if (!silent)
            safeToast("Hybrid Analysis API key required. Set it in vAnalyzer settings.", Toasts.Type.FAILURE);
    ...
    if (hashResult.sha256) {
        ...
        if (search.status === 200 && search.data?.reports?.length > 0) {
            if (!silent)
                safeToast(`Found ${search.data.reports.length} existing report(s)`, Toasts.Type.SUCCESS);
            return { details: buildHashDetails(search.data.reports), timestamp: Date.now() };
        }
    }
    // submit for quick scan
    if (!silent)
        safeToast("No existing report. Uploading file to Hybrid Analysis...");
