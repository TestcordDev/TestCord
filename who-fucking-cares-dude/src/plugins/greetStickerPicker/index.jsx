...
var GreetMode;
(function (GreetMode) {
    GreetMode["Greet"] = "Greet";
    GreetMode["NormalMessage"] = "Message";
})(GreetMode || (GreetMode = {}));
...
let WELCOME_STICKERS;
function greet(channel, message, stickers) {
...
function GreetMenu({ channel, message }) {
    const s = settings.use(["greetMode", "multiGreetChoices"]);
    const { greetMode, multiGreetChoices = [] } = s;
    return (<Menu.Menu navId="greet-sticker-picker" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Greet Sticker Picker">
            <Menu.MenuGroup label="Greet Mode">
                {Object.values(GreetMode).map(mode => (<Menu.MenuRadioItem key={mode} group="greet-mode" id={"greet-mode-" + mode} label={mode} checked={mode === greetMode} action={() => s.greetMode = mode}/>))}
            </Menu.MenuGroup>

            <Menu.MenuSeparator />

            <Menu.MenuGroup label="Greet Stickers">
                {WELCOME_STICKERS.map(sticker => (<Menu.MenuItem key={sticker.id} id={"greet-" + sticker.id} label={sticker.description.split(" ")[0]} action={() => greet(channel, message, [sticker.id])}/>))}
            </Menu.MenuGroup>
            ...
            {!settings.store.unholyMultiGreetEnabled ? null : (<>
                ...
                return (<Menu.MenuCheckboxItem key={sticker.id} id={"multi-greet-" + sticker.id} label={sticker.description.split(" ")[0]} checked={checked} disabled={!checked && multiGreetChoices.length >= 3} action={() => {
                        ...
                        <Menu.MenuSeparator />
                        <Menu.MenuItem id="multi-greet-submit" label="Send Greets" action={() => greet(channel, message, multiGreetChoices)} disabled={multiGreetChoices.length === 0}/>
...
export default definePlugin({
    ...
    set WELCOME_STICKERS(value) {
        WELCOME_STICKERS = value;
    },
    pickSticker(event, props) {
