...
const splitModes = new Set(["characters", "spaces", "newlines"]);
...
function getSplitMode(value) {
    return typeof value === "string" && splitModes.has(value)
        ? value
        : "newlines";
}
function canBypassSlowmode(channel) {
...
function canSplitInChannel(channel) {
    if (!channel)
        return false;
    if (!channel.rateLimitPerUser || canBypassSlowmode(channel))
        return true;
    return settings.store.splitInSlowmode
        && channel.rateLimitPerUser <= settings.store.slowmodeMax;
}
async function sendChunks(channelId, chunks, delay) {
    let sent = 0;
    try {
...
function restoreUnsentContent(channelId, chunks, sent) {
...
const listener = async (channelId, message) => {
