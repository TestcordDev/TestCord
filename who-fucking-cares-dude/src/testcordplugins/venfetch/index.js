...
const Native = VencordNative.pluginHelpers.venfetch;
const clientVersion = () => {
...
function getEnabledPlugins() {
    ...
    Object.values(Vencord.Plugins.plugins).filter(plugin => !isApiPlugin(plugin)).forEach(plugin => {
        if (PluginMeta[plugin.name]?.userPlugin) {
            if (plugin.started)
                counters.user.enabled++;
            counters.user.total++;
        }
        else {
            if (plugin.started)
                counters.official.enabled++;
    ...
    commands: [
        {
            ...
            execute: async (args, ctx) => {
                ...
                const computed = Object.entries(info).filter(([key, value]) => value === null || value.length).map(([key, value]) => [key, value]);
                let str = "";
...
const t = (e) => e.length > 0 ? e[0].toUpperCase() + e.slice(1) : "";
