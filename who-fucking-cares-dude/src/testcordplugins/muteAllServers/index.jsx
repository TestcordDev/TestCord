...
function MuteIcon({ width = 18, height = 18 }) {
...
function markAllAsRead() {
    ...
    Object.values(GuildStore.getGuilds()).forEach(guild => {
        ...
        allChannels.forEach((c) => {
            ...
            if (!channelId || !ReadStateStore.hasUnread(channelId))
                return;
            channels.push({
                channelId: channelId,
    ...
    ChannelStore.getSortedPrivateChannels().forEach((c) => {
        if (!ReadStateStore.hasUnread(c.id))
            return;
        channels.push({
            channelId: c.id,
    ...
    if (guildIds.length > 0) {
        ...
        for (const id of guildIds) {
            try {
                ...
                try {
                    await RestAPI.post({ url: `/guilds/${id}/ack`, body: {} });
                }
                catch { }
                const settings = {
                    muted: true,
...
const guildContextPatch = (children, { guild }) => {
    ...
    try {
        if (!guild)
            return;
        children.splice(-1, 0, (<Menu.MenuGroup key="nc-mute-all-group">
                <Menu.MenuItem id="nc-mute-all-servers" label="Mute all servers & mark as read" icon={() => <MuteIcon />} action={() => muteAllServers()}/>
            </Menu.MenuGroup>));
    }
    catch (e) {
