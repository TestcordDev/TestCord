...
function renderBadge(id, title) {
    const { css, label, color } = returnChannelBadge(id);
    return (<div key={id} className={`channel-badge channel-badge-${css}`} style={color ? { backgroundColor: color } : undefined} title={title}>
            {label}
        </div>);
}
export default definePlugin({
    ...
    renderChannelBadges(channel) {
        if (!channel || !isEnabled(channel.type))
            return null;
