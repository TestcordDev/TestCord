...
const shouldShowIndicator = (user) => {
    return user && !user.bot && (RelationshipStore.isFriend(user.id) || RelationshipStore.isBlocked(user.id));
};
export const RelationshipIndicator = ({ user, wantMargin = true, wantTopMargin = false }) => {
    if (!shouldShowIndicator(user))
        return null;
    return (<span className="vc-friend-indicator" style={{
            display: "inline-flex",
...
export function getBadges({ userId }) {
    ...
    if (!shouldShowIndicator(user))
        return [];
    return [{
            component: () => (<span className="vc-relationship-indicator">
