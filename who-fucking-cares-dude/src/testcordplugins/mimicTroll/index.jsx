...
class ContentFilter {
    ...
    static LOOKALIKE_REGEX = new RegExp(Object.keys(ContentFilter.UNICODE_REPLACEMENTS)
        .map(k => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
        .join("|"), "g");
    ...
    static LEET_REGEX = new RegExp(Object.keys(ContentFilter.LEET_MAP)
        .map(k => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
        .join("|"), "g");
    static baseNormalize(text) {
    ...
    static _normalizedTerms = null;
    static getNormalizedTerms() {
        if (this._normalizedTerms)
...
class MimicManager {
    ...
    isProcessing = false;
    intervalId = null;
    addTarget(userId, username, channelId) {
    ...
    toggleTarget(userId, username, channelId) {
    ...
    queueMessage(channelId, content, delay) {
...
const UserContext = (children, props) => {
    ...
    children.splice(-1, 0, <Menu.MenuGroup>
        <Menu.MenuCheckboxItem id="mimic-user" label="Mimic (Filtered)" checked={isChecked} action={async () => {
