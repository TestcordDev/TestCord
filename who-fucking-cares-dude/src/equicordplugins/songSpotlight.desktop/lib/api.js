...
export async function authFetch(url, options, retried = false) {
    ...
    try {
        ...
        if (res.status === 401) {
            ...
            showToast(!text.includes("<body>") && res.status >= 400 && res.status <= 599
                ? `Song Spotlight: ${text}`
                : `Song Spotlight fetch error at ${url.pathname}`, Toasts.Type.FAILURE);
        }
...
export async function getData() {
    return await authFetch(new URL("api/data", apiConstants.api), {
        ...
        if (!res)
            return useSongStore.getState().self?.data;
...
export async function listData(userId) {
    ...
    return await authFetch(new URL(`api/data/${userId}`, apiConstants.api), {
        ...
        if (!res)
            return useSongStore.getState().users[userId]?.data;
        const data = await res.json();
        useSongStore.getState().update({
            userId,
            data,
