...
export async function getCuteAnimeBoys(sub) {
...
export async function getCuteNeko() {
...
const mapCharacters = (text, map) => text.split("").map(char => map[char] || char).join("");
export function makeFreaky(text) {
    text = mapCharacters(text.trim() || "freaky", charMap);
    if (settings.store.addFreakyEnding)
...
export const isMorse = (text) => /^[.\-/ ]+$/.test(text);
...
export const isOneCharacterString = (str) => {
...
export function getFavoriteGif(opts, other) {
...
export function calculateAffinityScore(affinity) {
    const weights = {
        friend: 0.15,
        dm: 0.30,
        vc: 0.25,
    ...
    let score = 0;
    if (affinity.isFriend)
        score += weights.friend * 100;
...
export function loadFriendImage(source) {
...
export function generatePoissonDiskPosition(existingPositions, canvasWidth, canvasHeight, size) {
...
export function calculateCanvasSize(userCount, avatarSize) {
    const padding = 50;
    const textSpace = 60;
...
export async function resolveImage(options, ctx) {
    let image = null;
    let width = null;
    let height = null;
