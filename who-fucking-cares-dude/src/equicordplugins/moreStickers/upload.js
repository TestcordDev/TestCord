...
async function toGIF(url, ffmpeg) {
    const filename = (new URL(url)).pathname.split("/").pop() ?? "image.png";
    const res = await corsFetch(url);
    if (!res.ok)
        throw new Error("Failed to fetch image for GIF conversion");
...
export async function sendSticker({ channelId, sticker, ctrlKey, shiftKey, ffmpegState }) {
