...
function HideToggleButton(props) {
    return (<ChatBarButton onClick={props.onClick} tooltip={props.open ? "Close" : "Open"}>
        <svg fill={settings.store.color && props.open ? "#c32a32" : "currentColor"} fillRule="evenodd" width="20" height="20" viewBox="0 0 24 24" style={{ scale: "1.096", translate: "0 -1px" }}>
            {props.open ?
...
function ButtonsInnerComponent({ buttons }) {
    ...
    if (buttonItems.length === 0 || buttonItems.every(button => button?.props?.disabled === true))
        return null;
    return (<div key="chat-bar-buttons-menu" id="chat-bar-buttons-menu" style={{
            display: "flex",
            flexWrap: "nowrap",
            overflowX: "auto",
            overflowY: "hidden"
