...
export default definePlugin({
    ...
    settings,
    chatBarButton: { render: FloodPanelButton },
    start() {
        const { location } = settings.store;
        if (location === "headerbar") {
            addHeaderBarButton("FloodPanel", () => (<HeaderBarButton icon={FloodIcon} tooltip="Flood Panel" onClick={() => {
                    ...
                    if (!channel)
                        return;
                    openModal(props => (<FloodModal channel={channel} rootProps={props} onRunningChange={() => { }}/>));
                }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("FloodPanel", () => (<ChannelToolbarButton icon={FloodIcon} tooltip="Flood Panel" onClick={() => {
                    const chId = SelectedChannelStore.getChannelId();
                    if (!chId)
                        return;
                    const channel = ChannelStore.getChannel(chId);
