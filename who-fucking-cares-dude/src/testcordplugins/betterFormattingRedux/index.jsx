...
function ToolbarPopout({ closePopout }) {
    const handleFormat = (tag) => {
        applyToSlate(tag);
        closePopout();
        setTimeout(focusEditor, 50);
    };
    return (<div className="bfr-toolbar" style={{ background: "#2b2d31", border: "1px solid #1e1f22", borderRadius: 8, padding: 8, display: "flex", flexWrap: "wrap", gap: 2, maxWidth: 420 }}>
            {FORMATS.map(f => (<Clickable key={f.tag} className="bfr-toolbar-btn" onClick={() => handleFormat(f.tag)} aria-label={f.tooltip}>
                    <span className="bfr-toolbar-btn-label">{f.icon}</span>
                    <span className="bfr-toolbar-btn-tooltip">{f.tooltip}</span>
                </Clickable>))}
        </div>);
}
const FormatIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24">
...
const FormatButton = ({ isMainChat }) => {
    const { showButton } = settings.use(FORMAT_BUTTON_KEYS);
    const [open, setOpen] = React.useState(false);
    const buttonRef = React.useRef(null);
    if (!isMainChat || !showButton || settings.store.location !== "chatbar")
        return null;
    return (<div ref={buttonRef} style={{ display: "inline-flex", alignItems: "center" }}>
            <Popout position="top" align="center" spacing={8} animation={Popout.Animation.NONE} shouldShow={open} onRequestClose={() => setOpen(false)} targetElementRef={buttonRef} renderPopout={() => <ToolbarPopout closePopout={() => setOpen(false)}/>}>
                {(_, { isShown }) => (<ChatBarButton tooltip={isShown ? "" : "Text Formatting"} onClick={() => setOpen(v => !v)}>
                        <FormatIcon />
                    </ChatBarButton>)}
            </Popout>
        </div>);
...
function HeaderFormatButton() {
    ...
    const buttonRef = React.useRef(null);
    return (<div ref={buttonRef} style={{ display: "inline-flex", alignItems: "center" }}>
            <Popout position="bottom" align="center" spacing={8} animation={Popout.Animation.NONE} shouldShow={open} onRequestClose={() => setOpen(false)} targetElementRef={buttonRef} renderPopout={() => <ToolbarPopout closePopout={() => setOpen(false)}/>}>
                {(_, { isShown }) => (<HeaderBarButton icon={FormatIcon} tooltip={isShown ? null : "Text Formatting"} onClick={() => setOpen(v => !v)}/>)}
            </Popout>
        </div>);
}
function ChannelFormatButton() {
    ...
    return (<div ref={buttonRef} style={{ display: "inline-flex", alignItems: "center" }}>
            <Popout position="bottom" align="center" spacing={8} animation={Popout.Animation.NONE} shouldShow={open} onRequestClose={() => setOpen(false)} targetElementRef={buttonRef} renderPopout={() => <ToolbarPopout closePopout={() => setOpen(false)}/>}>
                {(_, { isShown }) => (<ChannelToolbarButton icon={FormatIcon} tooltip={isShown ? null : "Text Formatting"} onClick={() => setOpen(v => !v)}/>)}
            </Popout>
        </div>);
}
const wrapperMap = {
...
function transformSendTime(text) {
    const escapes = [];
    const SENTINEL = "\u0000";
    ...
    prepared = prepared.replace(WRAPPER_REGEX, (...args) => {
        for (let i = 1; i <= wrapperKeys.length; i++) {
            if (args[i] !== undefined)
                return wrapperMap[wrapperKeys[i - 1]](args[i]);
        }
        return args[0];
    });
    let escapeIdx = 0;
...
export default definePlugin({
    ...
    onBeforeMessageSend(_, message) {
        if (!settings.store.enableWrapperSyntax || !message.content)
            return;
