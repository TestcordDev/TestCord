...
async function leaveGroupDM(channelId) {
    try {
        ...
        log(`🚀 Starting leave procedure for group "${channelName}" (ID: ${channelId})`);
...
function isUserWhitelisted(userId) {
    ...
    verboseLog(`🔍 Whitelist check for user ${userId}: ${isWhitelisted ? "AUTHORIZED" : "NOT AUTHORIZED"}`);
...
function wasRecentlyAdded(channel, currentUserId) {
    ...
    if (channel.type !== 3) {
        verboseLog(`❌ Channel ${channel.id} is not a DM group (type: ${channel.type})`);
        return false;
    }
...
- Current user: ${currentUserId}`);
            ...
            log(`🎯 NEW DM GROUP DETECTED: "${channel.name || "Unnamed"}" (${channel.id})`);
            ...
            if (whitelistedMember) {
                log(`✅ Member ${whitelistedMember.id} is in whitelist, group authorized`);
                return;
            }
            log(`⚠️ NO AUTHORIZED MEMBER FOUND - Scheduling automatic leave in ${settings.store.delay}ms`);
