...
function isValidProfileBadge(badge) {
...
async function getUser(id) {
    ...
    if (userObj)
        return userObj;
    const user = await RestAPI.get({ url: Constants.Endpoints.USER(id) }).then(response => {
...
function MentionWrapper({ data, UserMention, RoleMention, parse, props }) {
    const [userId, setUserId] = useState(data.userId);
    // if userId is set it means the user is cached. Uncached users have userId set to undefined
    if (userId)
        return (<UserMention className="mention" userId={userId} channelId={data.channelId} inlinePreview={props.noStyleAndInteraction} props={props} key={props.key}/>);
    ...
    <RoleMention {...data} props={props} inlinePreview={props.formatInline}>
            <span onMouseEnter={() => {
...
export default definePlugin({
    ...
    renderMention(RoleMention, UserMention, data, parse, props) {
        return (<ErrorBoundary noop>
                <MentionWrapper key={"mention" + data.userId} RoleMention={RoleMention} UserMention={UserMention} data={data} parse={parse} props={props}/>
            </ErrorBoundary>);
    },
});
