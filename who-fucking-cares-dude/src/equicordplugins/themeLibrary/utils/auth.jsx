...
export async function authorizeUser(triggerModal = true) {
    const isAuthorized = await getAuthorization();
    if (isAuthorized === false) {
        if (!triggerModal)
            return false;
        openModal((props) => <OAuth2AuthorizeModal {...props} scopes={["identify", "connections"]} responseType="code" redirectUri="https://themes.equicord.org/api/user/auth" permissions={0n} clientId="1464006702125940736" cancelCompletesFlow={false} callback={async ({ location }) => {
                if (!location)
                    return logger.error("No redirect location returned");
                try {
...
export async function deauthorizeUser() {
    const uniqueToken = await DataStore.get("ThemeLibrary_uniqueToken");
    if (!uniqueToken)
        return Toasts.show({
...
export async function isAuthorized(triggerModal = true) {
    const isAuthorized = await getAuthorization();
