...
let origGetRelType = null;
let origIsFriend = null;
let origGetFriendIDs = null;
let origGetMutable = null;
function patchStore() {
    ...
    if (!origGetRelType && typeof store.getRelationshipType === "function") {
        ...
        store.getRelationshipType = function (userId) {
            const s = fakeState.get(userId);
            if (s === "accepted")
                return 1 /* RelationshipType.FRIEND */;
            if (s === "pending")
                return 3 /* RelationshipType.INCOMING_REQUEST */;
            return origGetRelType.call(this, userId);
        };
    }
    if (!origIsFriend && typeof store.isFriend === "function") {
        origIsFriend = store.isFriend;
        store.isFriend = function (userId) {
            if (fakeState.get(userId) === "accepted")
                return true;
    ...
    if (!origGetFriendIDs && typeof store.getFriendIDs === "function") {
        ...
        store.getFriendIDs = function () {
            const real = origGetFriendIDs.call(this);
    ...
    if (!origGetMutable && typeof store.getMutableRelationships === "function") {
        ...
        store.getMutableRelationships = function () {
            ...
            for (const [id, s] of fakeState) {
                if (s === "accepted")
                    real.set(id, 1 /* RelationshipType.FRIEND */);
                if (s === "pending")
                    real.set(id, 3 /* RelationshipType.INCOMING_REQUEST */);
            }
            return real;
        };
    }
...
function unpatchStore() {
    const store = RelationshipStore;
    if (origGetRelType) {
        store.getRelationshipType = origGetRelType;
        origGetRelType = null;
    }
    if (origIsFriend) {
        store.isFriend = origIsFriend;
        origIsFriend = null;
    }
    if (origGetFriendIDs) {
        store.getFriendIDs = origGetFriendIDs;
        origGetFriendIDs = null;
    }
    if (origGetMutable) {
        store.getMutableRelationships = origGetMutable;
        origGetMutable = null;
    }
}
// ── Patch acceptFriend ─────────────────────────────────────────────────────────
...
function patchAcceptFriend() {
    try {
        const RA = findByProps("acceptFriend", "addFriend");
        if (!RA || origAccept)
            return;
        origAccept = RA.acceptFriend;
        RA.acceptFriend = async function (userId, ...args) {
            if (fakeState.get(userId) === "pending") {
                fakeState.set(userId, "accepted");
                await persistState();
                FluxDispatcher.dispatch({
                    type: "RELATIONSHIP_UPDATE",
                    relationship: { id: userId, type: 1 /* RelationshipType.FRIEND */, nickname: null, since: new Date().toISOString() }
                });
                return;
            }
        ...
        console.warn("[FF] patchAcceptFriend:", e);
    }
}
...
function dispatchRelationship(user, type) {
    FluxDispatcher.dispatch({
        type: "RELATIONSHIP_UPDATE",
        relationship: { id: user.id, type, nickname: null, since: new Date().toISOString(), user: makeUserPayload(user) }
    });
}
async function addDirectFriend(user) {
    fakeState.set(user.id, "accepted");
    await persistState();
    dispatchRelationship(user, 1 /* RelationshipType.FRIEND */);
}
async function addPendingRequest(user) {
    ...
    FluxDispatcher.dispatch({
        ...
        relationship: {
            id: user.id,
            type: 3 /* RelationshipType.INCOMING_REQUEST */,
            nickname: null,
    ...
    for (const [userId, state] of fakeState) {
        try {
            ...
            if (state === "accepted") {
                FluxDispatcher.dispatch({
                    type: "RELATIONSHIP_UPDATE",
                    relationship: { id: userId, type: 1 /* RelationshipType.FRIEND */, nickname: null, since: new Date().toISOString(), user: makeUserPayload(user) }
                });
            }
            else if (state === "pending") {
                FluxDispatcher.dispatch({
                    type: "RELATIONSHIP_ADD",
                    relationship: { id: userId, type: 3 /* RelationshipType.INCOMING_REQUEST */, nickname: null, since: new Date().toISOString(), user: makeUserPayload(user) },
                    incoming: true,
                });
            }
            await sleep(50);
...
function getChannelClass() {
    try {
        ...
        const real = allChannels.find((c) => c.type === 1);
        if (real)
...
function buildRealDMChannel(user) {
...
const fakeDMChannelObjects = new Map();
let origGetChannel = null;
let origGetDMFromUserId = null;
let origGetSorted = null;
function patchChannelStore() {
    const store = ChannelStore;
    if (!origGetChannel && typeof store.getChannel === "function") {
        origGetChannel = store.getChannel;
        store.getChannel = function (id) {
            if (fakeDMChannelObjects.has(id))
                return fakeDMChannelObjects.get(id);
    ...
    if (!origGetDMFromUserId && typeof store.getDMFromUserId === "function") {
        ...
        store.getDMFromUserId = function (userId) {
            for (const [cid, ch] of fakeDMChannelObjects) {
                if ((ch.recipientIDs ?? []).includes(userId))
                    return cid;
            }
...
function unpatchChannelStore() {
    const store = ChannelStore;
    if (origGetChannel) {
        store.getChannel = origGetChannel;
        origGetChannel = null;
    }
    if (origGetDMFromUserId) {
        store.getDMFromUserId = origGetDMFromUserId;
        origGetDMFromUserId = null;
    }
    if (origGetSorted) {
        store.getSortedPrivateChannels = origGetSorted;
        origGetSorted = null;
    }
    fakeDMChannelObjects.clear();
}
async function sendFakeDM(user) {
    const me = UserStore.getCurrentUser();
    if (!me)
        return;
    const phrase = FAKE_DM_PHRASES[Math.floor(Math.random() * FAKE_DM_PHRASES.length)];
    let channelId = ChannelStore.getDMFromUserId?.(user.id) ?? null;
    if (!channelId) {
...
async function doFakeFriend(userId) {
    const user = await loadUser(userId);
    if (!user || isBot(user))
        return;
    await addDirectFriend(user);
}
async function doFakeFriendRequest(userId) {
    const user = await loadUser(userId);
    if (!user || isBot(user))
        return;
...
function askCount(title, max) {
    return new Promise(resolve => {
        ...
        function CountModal({ modalProps }) {
            ...
            return (<M.ModalRoot {...modalProps} size="small">
                    <M.ModalHeader>
                    ...
                    <M.ModalContent style={{ padding: "16px 20px" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            ...
                            <input autoFocus type="number" min={1} max={max} value={value} onChange={e => setValue(e.currentTarget.value)} onKeyDown={e => { if (e.key === "Enter")
                confirm(); }} style={{
...
async function fetchAllGuildMembers(guildId) {
    ...
    await sleep(1000);
    const after = GuildMemberStore.getMemberIds(guildId).length;
...
function getGuildCandidates(guildId) {
    const me = UserStore.getCurrentUser()?.id;
    const memberIds = GuildMemberStore.getMemberIds(guildId) ?? [];
    const realRelNone = (id) => {
        const fn = origGetRelType ?? ((uid) => RelationshipStore.getRelationshipType(uid));
        return fn.call(RelationshipStore, id) === 0 /* RelationshipType.NONE */;
    };
    return memberIds.filter(id => {
        if (id === me || !realRelNone(id))
            return false;
        const cached = UserStore.getUser(id);
        if (cached && isBot(cached))
            return false;
        return true;
    });
}
// ── Fake Friend Request with number input ─────────────────────────────────
async function floodGuild(guildId) {
...
async function removeFakeFriendsForGuild(guildId) {
    const memberIds = new Set(GuildMemberStore.getMemberIds(guildId));
...
async function fakeMessageRequestGuild(guildId) {
    const candidates = getGuildCandidates(guildId);
    if (!candidates.length) {
        Toasts.show({ message: "No candidates available", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        return;
...
let MessageRequestStore = null;
let reapplyTimer = null;
let running = false;
let origGetRequests = null;
let origHasRequest = null;
const fakeMessageRequests = new Map();
function patchMessageRequestStore() {
    if (MessageRequestStore)
        return;
    try {
        const store = findByProps("getRequests", "hasRequest");
        if (!store)
            return;
        ...
        if (typeof store.hasRequest === "function" && !origHasRequest) {
            ...
            store.hasRequest = function (channelId) {
                if (fakeMessageRequests.has(channelId))
                    return true;
...
async function sendIncomingMessageRequest(user) {
    const ChannelClass = getChannelClass();
    const msgId = makeSnowflake();
    const channelId = makeSnowflake();
...
// ── Context menus ──────────────────────────────────────────────────────────────
const userContextPatch = (children, props) => {
    ...
    try {
        ...
        const realRel = origGetRelType
            ...
            : RelationshipStore.getRelationshipType(userId);
        if (!state && realRel !== 0 /* RelationshipType.NONE */)
            return;
        const followIndex = children.findIndex((c) => c?.props?.id === "follow-user" || c?.key === "follow-user");
        let items = [];
        if (!state) {
            items = [
            ...
            items = [
                <Menu.MenuItem key="ff-cancel" id="ff-cancel" label="Cancel fake request" color="danger" action={async () => {
            ...
            items = [
                <Menu.MenuItem key="ff-remove" id="ff-remove" label="Remove from fake friends" color="danger" action={async () => {
                        fakeState.delete(userId);
                        await persistState();
                        FluxDispatcher.dispatch({ type: "RELATIONSHIP_REMOVE", relationship: { id: userId } });
                    }}/>
        ...
        if (followIndex !== -1)
            children.splice(followIndex + 1, 0, group);
        else
            children.push(<Menu.MenuSeparator key="ff-sep"/>, group);
    }
    catch (e) {
...
const guildContextPatch = (children, props) => {
    ...
    try {
        ...
        const items = [
            <Menu.MenuItem key="ff-g-flood" id="ff-g-flood" label="Fake Friend Request" action={() => floodGuild(guildId)}/>
        ];
        // "Remove fake friend requests" button — only visible if fakes exist for this server
        if (fakeCount > 0) {
            items.push(<Menu.MenuItem key="ff-g-remove" id="ff-g-remove" label={`Remove fake friend requests (${fakeCount})`} color="danger" action={() => removeFakeFriendsForGuild(guildId)}/>);
        }
        children.push(<Menu.MenuSeparator key="ff-g-sep"/>, <Menu.MenuGroup key="ff-g-group" label="Fake Friends">{items}</Menu.MenuGroup>);
    }
    catch (e) {
