    ...
    try {
        ...
        if (Array.isArray(data.entities)) {
            ...
            if (registrarEntity?.vcardArray?.[1]) {
                const fn = registrarEntity.vcardArray[1].find((p) => p[0] === "fn");
                if (fn?.[3])
                    registrar = fn[3];
            }
        }
        const registrationDate = data.events?.find((e) => e.eventAction === "registration")?.eventDate ??
            data.events?.find((e) => e.eventAction === "registered")?.eventDate;
        const expirationDate = data.events?.find((e) => e.eventAction === "expiration")?.eventDate ??
            data.events?.find((e) => e.eventAction === "expire")?.eventDate;
        const updatedAt = data.events?.find((e) => e.eventAction === "last changed")?.eventDate ??
            data.events?.find((e) => e.eventAction === "last update of RDAP database")?.eventDate;
        return {
...
function parseProviderResponse(data, ip) {
    const provider = settings.store.ipProvider;
    switch (provider) {
        case "ip-api":
            return {
...
function createDomainMessage(info) {
...
const messageContextMenuPatch = (children, { message }) => {
    if (!message || !message.author)
        return;
    const osintGroup = children.find((child) => child?.props?.id === "osint-tools");
    if (osintGroup)
        return;
    children.push(<Menu.MenuGroup id="osint-tools">
            <Menu.MenuItem id="osint-toolkit-main" label="OSINT Toolkit">
                <Menu.MenuItem id="csint-tools" label="CSINT Tools">
                    {OSINT_TOOLS.map(tool => (<Menu.MenuItem key={`csint-${tool.id}`} id={`csint-${tool.id}`} label={tool.name} hint={tool.description} action={() => openUrl(tool.url)}/>))}
                </Menu.MenuItem>
                <Menu.MenuItem id="osint-tools" label="OSINT Tools">
                    {OSINT_RESOURCES.map(resource => (<Menu.MenuItem key={`osint-${resource.id}`} id={`osint-${resource.id}`} label={resource.name} hint={resource.description} action={() => openUrl(resource.url)}/>))}
                </Menu.MenuItem>
            </Menu.MenuItem>
        </Menu.MenuGroup>);
};
    ...
    commands: [
        {
            ...
            execute: async (args, ctx) => {
                ...
                const domainInput = args[0]?.value;
                if (!domainInput) {
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const channelId = ctx.channel.id;
                const usernameInput = args[0]?.value;
                if (!usernameInput) {
