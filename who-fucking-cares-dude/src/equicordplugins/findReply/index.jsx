...
export const jumper = findByPropsLazy("jumpToMessage");
const FindReplyIcon = () => {
...
function findReplies(message) {
    const messages = [...MessageStore.getMessages(message.channel_id)?._array ?? []].filter(m => !m.deleted).sort((a, b) => {
        return a.timestamp.toString().localeCompare(b.timestamp.toString());
    }); // Need to deep copy Message array when sorting
    const found = [];
    for (const other of messages) {
        if (other.timestamp.toString().localeCompare(message.timestamp.toString()) <= 0)
            continue;
...
export default definePlugin({
    ...
    messagePopoverButton: {
        ...
        render(message) {
            ...
            if (settings.store.hideButtonIfNoReply && !replies.length)
                return null;
            return {
                label: "Jump to Reply",
                icon: FindReplyIcon,
