...
function uninstallSnippet(snippet) {
    const link = getSnippetLink(snippet.id);
...
function SnippetDetailsModalContent({ snippet, installed, onToggleInstall, onTagClick, modalProps }) {
    ...
    return (<>
            <Modal {...modalProps} size="md" title={`${snippet.name} - Snippet Preview`}>
                <div className={cl("modal-body")}>
                    {snippet.thumbnail_url ? (<div className={cl("modal-banner")} onClick={e => {
                e.stopPropagation();
                setLightboxOpen(true);
            }} title="Click to enlarge preview image">
                            <img src={snippet.thumbnail_url} alt={snippet.name} className={cl("modal-img")}/>
                        </div>) : (<div className="vc-snippet-modal-placeholder">
                    ...
                    <div className={cl("modal-top-actions")}>
                        ...
                        {snippet.content && (<Button variant="secondary" className={cl("modal-top-action-btn")} onClick={() => openThemeCodeModal(snippet.name, snippet.content)}>
                                View Code
                            </Button>)}
                        {snippet.source && (<Button variant="secondary" className={cl("source-btn")} onClick={() => VencordNative.native.openExternal(snippet.source)}>
                                Open GitHub / Source
                            </Button>)}
                    </div>

                    {snippet.tags?.length > 0 && (<div>
                            <Heading className={cl("modal-section-title")}>Tags (Click to filter)</Heading>
                            <div className={cl("card-tags")}>
                                {snippet.tags.map(tag => (<button key={tag} className={classes(cl("card-tag"), cl("tag-btn"))} onClick={() => {
                    modalProps.onClose();
                    onTagClick(tag);
                }}>
                                        #{tag}
                    ...
                    <div>
                        ...
                        <div className={cl("modal-details-grid")}>
                            ...
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Downloads", (snippet.downloads ?? 0).toLocaleString())} title="Click to copy downloads count">
                            ...
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Likes", (snippet.likes ?? 0).toLocaleString())} title="Click to copy likes count">
                                <span className={cl("modal-detail-label")}>Likes</span>
                                <span className={cl("modal-detail-value")}>{(snippet.likes ?? 0).toLocaleString()}</span>
                            </div>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Release Date", snippet.release_date ? new Date(snippet.release_date).toLocaleDateString() : "N/A")} title="Click to copy release date">
                            ...
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Snippet ID", snippet.id)} title="Click to copy snippet ID">
                                <span className={cl("modal-detail-label")}>Snippet ID</span>
                                <span className={cl("modal-detail-value")}>{snippet.id}</span>
                            </div>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Status", installed ? "Added" : "Available")} title="Click to copy snippet status">
                            ...
                            <div className={classes(cl("modal-detail-card"), cl("modal-detail-card-full"))} onClick={() => copyStat("Snippet Link", snippetLink)} title="Click to copy snippet link">
            ...
            {lightboxOpen && snippet.thumbnail_url && (<div className={cl("lightbox-overlay")} onClick={() => setLightboxOpen(false)}>
                    <div className={cl("lightbox-content")} onClick={e => e.stopPropagation()}>
                        <div className={cl("lightbox-header")}>
                            ...
                            <div className={cl("lightbox-actions")}>
                                <Button size="small" variant="secondary" className={cl("lightbox-btn")} onClick={() => VencordNative.native.openExternal(snippet.thumbnail_url)}>
                                    Open Original
                                </Button>
                        ...
                        <div className={cl("lightbox-img-wrapper")}>
                            <img src={snippet.thumbnail_url} alt={snippet.name} className={cl("lightbox-img")}/>
                        </div>
                    </div>
                </div>)}
        </>);
}
function openSnippetDetailsModal(snippet, installed, onToggleInstall, onTagClick) {
    openModal(modalProps => (<SnippetDetailsModalContent snippet={snippet} installed={installed} onToggleInstall={onToggleInstall} modalProps={modalProps} onTagClick={onTagClick}/>));
}
function SnippetCard({ snippet, installed, onToggleInstall, onOpenDetails, onTagClick }) {
    const [imgErr, setImgErr] = useState(false);
    const authorName = snippet.author?.discord_name ?? snippet.author?.github_name ?? "Unknown";
    return (<div className={classes(cl("card"), "vc-snippet-card")} onClick={onOpenDetails} style={{ cursor: "pointer" }}>
            <div className={cl("card-preview")}>
                {snippet.thumbnail_url && !imgErr ? (<img className={cl("card-img")} src={snippet.thumbnail_url} alt={snippet.name} loading="lazy" onError={() => setImgErr(true)}/>) : (<div className="vc-snippet-placeholder-preview">
            ...
            onTagClick(authorName);
                ...
                {snippet.description && (<p className={cl("card-desc")}>{snippet.description}</p>)}
                {snippet.tags?.length > 0 && (<div className={cl("card-tags")}>
                        {snippet.tags.slice(0, 5).map(tag => (<button key={tag} className={classes(cl("card-tag"), cl("tag-btn"))} onClick={e => {
                    e.stopPropagation();
                    onTagClick(tag);
                }}>
                                #{tag}
                ...
                <div className={cl("card-actions")}>
                    <Button size="small" variant={installed ? "dangerSecondary" : "primary"} className={classes(cl("action-btn"), !installed && "vc-snippet-add-btn")} onClick={e => {
            e.stopPropagation();
            onToggleInstall();
        }}>
                        {installed ? "Remove" : "Add Snippet"}
                    </Button>
                    {snippet.content && (<Button size="small" variant="secondary" className={cl("action-btn")} onClick={e => {
                e.stopPropagation();
...
export function SnippetMarketplaceSection() {
    ...
    function handleToggleInstall(snippet) {
        try {
            ...
            <div className={`vc-marketplace-collapsible ${isHidden ? "collapsed" : "expanded"}`}>
                ...
                <div className={classes(cl("toolbar"), Margins.bottom16)}>
                    <div className={cl("search")}>
                        <TextInput placeholder="Search snippets by name, author, or tag..." value={search} onChange={setSearch}/>
                    </div>
                ...
                {loading ? (<div className={cl("grid")}>
                    ...
                    </Paragraph>) : visible.length === 0 ? (<Paragraph color="text-muted" className={Margins.top16}>No snippets found.</Paragraph>) : (<div className={cl("grid")}>
                        {visible.map(snippet => (<SnippetCard key={snippet.id} snippet={snippet} installed={installedIds.has(snippet.id)} onToggleInstall={() => handleToggleInstall(snippet)} onOpenDetails={() => openSnippetDetailsModal(snippet, installedIds.has(snippet.id), () => handleToggleInstall(snippet), (tag) => setSearch(tag))} onTagClick={(tag) => setSearch(tag)}/>))}
                    </div>)}
            </div>
