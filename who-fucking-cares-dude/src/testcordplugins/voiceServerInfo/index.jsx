...
import { getLocation, VoiceServerIcon, VoiceServerModal } from "./VoiceServerModal";
const Native = VencordNative.pluginHelpers.VoiceServerInfo;
...
async function getCurrentVoiceServerInfo() {
    if (!RTCConnectionStore.isConnected())
        throw new Error("Join a voice channel first.");
    const hostname = RTCConnectionStore.getHostname();
    if (!hostname)
        throw new Error("Discord has not provided the voice server hostname yet.");
    ...
    if (!info)
        throw new Error("Could not retrieve information for the voice server IP.");
    return {
...
function createVoiceServerMessage(server) {
    const { info } = server;
...
async function notifyCurrentVoiceServer() {
    if (!settings.store.notifyOnConnect || !RTCConnectionStore.isConnected())
        return;
    const hostname = RTCConnectionStore.getHostname();
    if (!hostname || hostname === lastReportedHostname)
        return;
    const generation = ++lookupGeneration;
    lastReportedHostname = hostname;
    try {
        const server = await getCurrentVoiceServerInfo();
        if (!pluginActive || generation !== lookupGeneration || !RTCConnectionStore.isConnected())
            return;
        showNotification({
...
function VoiceServerButton() {
    ...
    if (!showIcon)
        return null;
    return (<HeaderBarButton icon={VoiceServerIcon} tooltip="Voice Server Info" onClick={openVoiceServerModal}/>);
}
    ...
    commands: [{
            ...
            execute: async (_args, ctx) => {
                try {
                    const server = await getCurrentVoiceServerInfo();
                    if (pluginActive)
                        sendBotMessage(ctx.channel.id, { content: createVoiceServerMessage(server) });
                }
                catch (error) {
