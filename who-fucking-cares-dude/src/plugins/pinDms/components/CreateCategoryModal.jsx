...
const ColorPickerWithSwatches = findComponentByCodeLazy('id:"color-picker"');
...
function useCategory(categoryId, initalChannelId) {
...
export function NewCategoryModal({ categoryId, modalProps, initialChannelId }) {
    ...
    return (<Modal {...modalProps} title={`${categoryId ? "Edit" : "New"} Category`} actions={[{
                text: categoryId ? "Save" : "Create",
                variant: "primary",
                onClick: onSave,
                disabled: !name
            }]}>
            <form className={cl("content")} onSubmit={e => {
            e.preventDefault();
            onSave();
        }}>
                <section>
                ...
                <section>
                    ...
                    <ColorPickerWithSwatches className={cl("color-picker")} key={category.id} defaultColor={DEFAULT_COLOR} colors={SWATCHES} onChange={c => setColor(c)} value={color} renderDefaultButton={() => null} renderCustomButton={() => (<ColorPicker color={color} onChange={c => setColor(c)} key={category.id} showEyeDropper={false}/>)}/>
                </section>
            </form>
        </Modal>);
}
export const openCategoryModal = (categoryId, channelId) => openModalLazy(async () => {
