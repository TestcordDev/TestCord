...
function ViewScheduledModalInner({ rootProps, close }) {
    ...
    return (<Modal {...rootProps} size="md" title="Scheduled Messages" actions={actions}>
            {!messages.length ? (<div className={cl("empty-state")}>
                    <CalendarIcon width={48} height={48}/>
                    <span>No scheduled messages</span>
                </div>) : (<div className={cl("message-list")}>
                    {messages.map(msg => {
                ...
                const displayContent = msg.content.length > 200
                    ...
                    : msg.content;
                return (<div key={msg.id} className={cl("message-item")}>
...
export function openViewScheduledModal() {
