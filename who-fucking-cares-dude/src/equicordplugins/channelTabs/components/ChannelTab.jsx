...
import { closeTab, ensureUnreadFallbackCountsLoaded, getNotificationDotState, getUnreadFallbackCounts, isTabSelected, moveDraggedTabs, moveToTab, openedTabs, settings, updateUnreadFallbackCounts } from "@equicordplugins/channelTabs/util";
...
function LibraryIcon(height = 20, width = 20, className) {
...
function XIcon({ size, fill }) {
    return <svg width={size} height={size} viewBox="0 0 24 24">
        <path fill={fill} d="M17.3 18.7a1 1 0 0 0 1.4-1.4L13.42 12l5.3-5.3a1 1 0 0 0-1.42-1.4L12 10.58l-5.3-5.3a1 1 0 0 0-1.4 1.42L10.58 12l-5.3 5.3a1 1 0 1 0 1.42 1.4L12 13.42l5.3 5.3Z"/>
    </svg>;
}
const GuildIcon = ({ guild }) => {
    return guild.icon
        ? <img src={`https://${window.GLOBAL_ENV.CDN_HOST}/icons/${guild?.id}/${guild?.icon}.png`} className={cl("icon")}/>
        : <div className={cl("guild-acronym-icon")}>
            <BaseText size="xs" weight="semibold" tag="span">{getGuildAcronym(guild)}</BaseText>
        </div>;
};
const ChannelIcon = ({ channel }) => <img src={channel?.icon
...
function getChannelUnreadState(channelId) {
...
export const NotificationDot = ({ channelIds }) => {
    ...
    const channelStates = useStateFromStores([ActiveJoinedThreadsStore, ReadStateStore], () => channelIds.map(getChannelUnreadState));
    ...
    const { badgeText, hasMention, shouldShow } = getNotificationDotState(channelStates, userId ? getUnreadFallbackCounts(userId) : {}, persistUnreadCountFallback);
    useEffect(() => {
        if (!userId || !persistUnreadCountFallback)
            return;
        let didCancel = false;
    ...
    return shouldShow ?
        <div data-has-mention={hasMention} className={classes(cl("notification-badge"), dotStyles.numberBadge, dotStyles.baseShapeRound)} style={{
                width: "16px"
            }} ref={node => node?.style.setProperty("background-color", hasMention ? "var(--danger-color, var(--red-400))" : "var(--main-color, var(--brand-experiment, var(--brand-500)))", "important")}>
            {badgeText}
        </div> : null;
};
export const TabNumberBadge = ({ number, position, isSelected, isCompact, isHovered }) => {
    // hide badge if:
    ...
    if (shouldHide)
        return null;
    return (<div className={cl("tab-number-badge", `position-${position}`)} data-position={position}>
            {number}
        </div>);
};
function ChannelTabContent(props) {
    ...
    const { noPomeloNames, showStatusIndicators } = settings.use(["noPomeloNames", "showStatusIndicators"]);
    const [isTyping, status, isMobile] = useStateFromStores([TypingStore, PresenceStore], () => {
        const recipientId = recipients?.[0] ?? "";
        return [
            !!(Object.keys(TypingStore.getTypingUsers(props.channelId)).filter(id => id !== userId).length),
    ...
    if (channel && recipients?.length) {
        if (recipients.length === 1) {
            ...
            return (<>
                    <Avatar size="SIZE_24" src={user.getAvatarURL(guildId, 128)} status={showStatusIndicators ? status : undefined} isTyping={isTyping} isMobile={isMobile}/>
...
export default function ChannelTab(props) {
    ...
    const [, drop] = useDrop(() => ({
        ...
        hover: (item, monitor) => {
            ...
            // get tab width
            const draggedElement = document.querySelector(".vc-channeltabs-tab-dragging");
            ...
            if (dragIndex < hoverIndex) {
                ...
                if (draggedRightEdge < hoverMiddleX + hysteresis)
                    return;
            }
            ...
            if (dragIndex > hoverIndex) {
                ...
                if (draggedLeftEdge > hoverMiddleX - hysteresis)
                    return;
            }
    ...
    return <div className={cl("tab", {
        ...
        })} key={index} ref={ref} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onAuxClick={e => {
            if (e.button === 1 /* middle click */)
                closeTab(id);
        }} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => <TabContextMenu tab={props}/>)}>
        <button className={cl("button", "channel-info")} onClick={() => moveToTab(id)}>
            <div className={cl("tab-inner")} data-compact={compact}>
                {/* left position badge */}
                {showTabNumbers && tabNumberPosition === "left" && (<TabNumberBadge number={index + 1} position="left" isSelected={isTabSelected(id)} isCompact={compact} isHovered={isHovered}/>)}

                <ChannelTabContent {...props} guild={guild} channel={channel}/>

                {/* right position badge */}
                {showTabNumbers && tabNumberPosition === "right" && (<TabNumberBadge number={index + 1} position="right" isSelected={isTabSelected(id)} isCompact={compact} isHovered={isHovered}/>)}
            </div>
        </button>

        {openedTabs.length > 1 && <button className={cl("button", "close-button", { "close-button-compact": compact, "hoverable": !compact })} onClick={() => {
                setIsClosing(true);
        ...
        {!compact && settings.store.showResizeHandle && <div className={cl("tab-resize-handle")} onMouseDown={handleResizeStart}/>}
    </div>;
}
