...
import { extractAndLoadChunksLazy, findByPropsLazy, findComponentByCodeLazy, findCssClassesLazy, } from "@webpack";
import { ChannelActionCreators, ChannelRouter, ChannelSectionStore, ChannelStore, FluxDispatcher, GuildStore, Menu, MessageActions, MessageStore, PermissionsBits, PermissionStore, PopoutActions, PopoutWindowStore, RelationshipStore, SelectedChannelStore, SelectedGuildStore, useCallback, useEffect, useLayoutEffect, UserStore, useState, useStateFromStores, } from "@webpack/common";
...
const ArrowsLeftRightIcon = ({ color, ...rest }) => {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" fill={color} viewBox="0 0 24 24" {...rest}>
...
const requireForumView = extractAndLoadChunksLazy(["Missing channel in Channel.renderHeaderToolbar"], /Promise\.all\(\[((?:\i\.e\("\d+"\),?)+)\]\)\.then\(\i\.bind\(\i,(\d+)\)\)[^}]{0,100}?name:"ForumChannel"/);
...
async function waitForChannel(channelId, timeoutMs = 2500) {
...
async function waitForDmChannel(userId, timeoutMs = 2500) {
    const startedAt = Date.now();
    while (Date.now() - startedAt <= timeoutMs) {
...
async function openPopoutFromUserMenu(userId) {
    try {
...
function closePopout(channelId, syncPersistence = true) {
...
function openPopout(channelId, syncPersistence = true) {
    const channel = ChannelStore.getChannel(channelId);
    if (!channel || !canOpenPopout(channel))
        return;
    const windowKey = getPopoutWindowKey(channelId);
...
const createSidebarChatContextMenuItem = (id, guildId) => {
    return (<Menu.MenuItem id={`vc-sidebar-chat-${id}`} label={"Open Sidebar Chat"} action={() => {
...
const createPopoutChatContextMenuItem = (id, label, action) => {
    return (<Menu.MenuItem id={`vc-sidebar-chat-popout-${id}`} label={label} action={() => {
            void action();
        }}/>);
};
const UserContextPatch = (children, args) => {
    const checks = [
        args.user,
    ...
    children.push(createPopoutChatContextMenuItem(args.user.id, isOpen ? "Close popout chat" : "Popout chat", () => {
        if (channelId && isOpen) {
...
const ChannelContextPatch = (children, args) => {
    const checks = [
        args.channel,
        args.channel.type !== 4 /* ChannelType.GUILD_CATEGORY */,
        PermissionStore.can(PermissionsBits.VIEW_CHANNEL, args.channel) || args.channel.type === 3 /* ChannelType.GROUP_DM */,
    ];
    if (checks.some(check => !check))
        return;
    children.push(createSidebarChatContextMenuItem(args.channel.id, args.channel.guild_id));
    children.push(createPopoutChatContextMenuItem(args.channel.id, getPopoutMenuLabel(args.channel.id), () => openPopout(args.channel.id)));
};
export default definePlugin({
    ...
    renderSidebar() {
        const { guild, channel /* width*/ } = useStateFromStores([SidebarStore, GuildStore, ChannelStore], () => {
        ...
        const [channelSidebar, guildSidebar] = useStateFromStores([ChannelSectionStore, SelectedChannelStore, ChannelStore], () => {
        ...
        useEffect(() => {
            if (!channel?.id || MessageStore.getLastMessage(channel.id))
                return;
        ...
        const [View, setViewComponent] = useState(null);
        useEffect(() => {
            if (!channel)
                return;
            if (channel.isForumLikeChannel()) {
                ...
                setViewComponent(<div className={ChatClasses.loader}>
        ...
        if (!channel || channelSidebar || guildSidebar)
            return null;
...
const Header = ({ guild, channel }) => {
    const recipientId = channel.isPrivate() ? channel.getRecipientId() : null;
    const name = useStateFromStores([UserStore, RelationshipStore], () => getChannelTitle(channel), [channel.id, channel.name]);
    const parentChannel = useStateFromStores([ChannelStore], () => ChannelStore.getChannel(channel?.parent_id), [channel?.parent_id]);
    ...
    return (<HeaderBar toolbar={<>
                    <HeaderBarButton icon={ArrowsLeftRightIcon} tooltip="Switch channels" onClick={switchChannels}/>
                    <HeaderBarButton key={`${channel.id}-${isPopoutOpen ? "open" : "closed"}`} icon={isPopoutOpen ? XSmallIcon : WindowLaunchIcon} tooltip={isPopoutOpen ? "Close popout chat" : "Popout chat"} selected={isPopoutOpen} onClick={openPopoutClick}/>
...
const RenderPopout = ErrorBoundary.wrap(({ channel, name, windowKey }) => {
    ...
    return (<PopoutWindow withTitleBar windowKey={windowKey} title={name || "Equicord"} channelId={channel.id}>
...
function PopoutHeaderButton() {
    const channelState = useStateFromStores([SelectedChannelStore, ChannelStore, PopoutWindowStore], () => {
    ...
    if (!channelState.channel || !canOpenPopout(channelState.channel))
        return null;
    const { channel, isOpen, label } = channelState;
    return (<HeaderBarButton key={`${channel.id}-${isOpen ? "open" : "closed"}`} icon={isOpen ? XSmallIcon : WindowLaunchIcon} tooltip={isOpen ? "Close popout chat" : `Popout chat for ${label}`} aria-label="Popout chat" selected={isOpen} onClick={() => openPopout(channel.id)}/>);
}
const WrappedPopoutHeaderButton = ErrorBoundary.wrap(PopoutHeaderButton, { noop: true });
function PopoutPersistenceSync() {
    const openWindowKeySignature = useStateFromStores([PopoutWindowStore], () => getOpenPopoutWindowKeys().join("|"), []);
    useEffect(() => {
        if (restoringPersistedPopouts)
            return;
        syncPersistedPopoutWindows();
