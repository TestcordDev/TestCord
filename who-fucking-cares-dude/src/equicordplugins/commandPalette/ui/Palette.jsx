...
import { flattenSections, ResultsList } from "./ResultsList";
const cl = classNameFactory("vc-cmdpal-");
const logger = new Logger("CommandPalette");
const SUGGESTION_LIMIT = 6;
function commandToRow(command) {
...
export function Palette({ onClose, initialPage }) {
    ...
    const sections = useMemo(() => {
        ...
        if (pageType === "root") {
            ...
            if (trimmed) {
                ...
                if (calc)
                    result.push({ label: "Calculator", items: [calc] });
                ...
                if (pinnedRows.length)
                    result.push({ label: "Pinned", items: pinnedRows });
                const suggestionRows = topFrecent(SUGGESTION_LIMIT + shown.size)
                    .map(id => byId.get(id))
                    .filter((c) => c != null && !shown.has(c.id))
                    .slice(0, SUGGESTION_LIMIT)
                    .map(c => { shown.add(c.id); return commandToRow(c); })
                    .filter((row) => row !== null);
                if (suggestionRows.length)
                    result.push({ label: "Suggestions", items: suggestionRows });
                const rest = commands
            ...
            const filtered = trimmed
                ? listItems
                    .map(item => ({
                    item,
                    score: Math.max(fuzzyScore(trimmed, item.label), ...(item.keywords ?? []).map(k => fuzzyScore(trimmed, k) * 0.7), item.sublabel ? fuzzyScore(trimmed, item.sublabel) * 0.5 : 0)
                }))
            ...
            if (filtered.length > 0)
                return [{ label: null, items: filtered.map(listItemToRow) }];
        }
        return result;
    ...
    async function runAction(action, commandId) {
        if (commandId)
            recordUse(commandId);
        setActionsOpen(false);
        try {
        ...
        if (!action.keepOpen && settings.store.closeAfterExecute)
            onClose();
    }
    ...
    function handleKey(e) {
        ...
        if (key === "enter") {
            if (pageType === "detail") {
                const detailActions = currentPage?.spec.type === "detail" ? currentPage.spec.actions ?? [] : [];
                if (detailActions[0])
                    void runAction(detailActions[0]);
                return true;
            }
            if (selectedRow) {
    ...
    useEffect(() => {
        if (pageType === "root" || pageType === "list")
    ...
    return (<div className={cl("shell")} style={{ "--cmdpal-blur": `${blur}px` }} onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
            <div className={cl("backdrop")} onMouseDown={e => {
            e.stopPropagation();
            onClose();
        }}/>
            ...
            : <SearchIcon className={cl("search-icon")}/>}
                {showSearch
            ? (<input ref={inputRef} className={cl("input")} value={query} placeholder={currentPage?.spec.type === "list"
                    ? currentPage.spec.placeholder ?? `Search ${currentPage.title.toLowerCase()}...`
                    : "Search for commands..."} onChange={e => ctx.setQuery(e.target.value)} onMouseDown={e => e.stopPropagation()} onBlur={() => {
                    if (actionsOpen || isEditableTarget(document.activeElement))
                        return;
                    inputRef.current?.focus();
                }}/>)
            ...
            {expanded && (<>
                    <div className={cl("body")}>
                        {(pageType === "root" || pageType === "list") && (<ResultsList sections={sections} selectedIndex={Math.min(selectedIndex, Math.max(0, flatRows.length - 1))} onSelect={setSelectedIndex} onRun={row => void runAction(row.actions[0], row.commandId)}/>)}
                        {currentPage?.spec.type === "form" && (<FormPage spec={currentPage.spec} ctx={ctx} formRef={formRef}/>)}
                        {currentPage?.spec.type === "detail" && (<DetailPage spec={currentPage.spec}/>)}
                        {actionsOpen && (<ActionsPanel actions={availablePanelActions} selectedIndex={actionsIndex} onSelect={setActionsIndex} onRun={action => void runAction(action, selectedRow?.commandId)}/>)}
                    </div>
                    <ActionBar hint={footerHint} primaryLabel={primaryLabel} showActionsHint={!recordingFor && availablePanelActions.length > 0}/>
                </>)}
            </div>
        </div>);
}
