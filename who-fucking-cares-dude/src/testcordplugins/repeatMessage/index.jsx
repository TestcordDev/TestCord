...
function RepeatMessageIcon({ className }) {
    return (<svg viewBox="0 -960 960 960" height={24} width={24} className={classes(classNameFactory("vc-repeat-")("icon"), className)}>
...
let shift = false;
function repeatMessage(channelId, id, content, stickers) {
...
const messageCtxPatch = (children, { message }) => {
    if (!message.content && message.stickerItems.length === 0)
        return;
    const group = findGroupChildrenByChildId("copy-text", children);
    if (!group)
        return;
    group.splice(group.findIndex(c => c?.props?.id === "reply") + 1, 0, (<Menu.MenuItem id="vc-repeat" label="Repeat" icon={RepeatMessageIcon} action={async () => repeatMessage(message.channel_id, message.id, message.content, message.stickerItems)}/>));
};
function setTitle(title) {
    const contextMenuOption = document.getElementById("message-vc-repeat");
    if (contextMenuOption)
        contextMenuOption.textContent = title;
}
