...
function shouldEdit(channel, message, timePeriod, shouldMergeWithAttachment) {
    let should = true;
...
export default definePlugin({
    ...
    onBeforeMessageSend(channelId, message) {
        ...
        const { should, content } = shouldEdit(channel, lastMessage, this.settings.store.timePeriod, this.settings.store.shouldMergeWithAttachment);
        if (should) {
