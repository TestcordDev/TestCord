...
async function fetchMediaData() {
    ...
    try {
        ...
        if (!userSession?.NowPlayingItem)
            return null;
        const item = userSession.NowPlayingItem;
        const playState = userSession.PlayState;
        if (playState?.IsPaused && !settings.store.jf_showPausedState)
            return null;
...
async function getActivity() {
    ...
    if (store.jf_overrideType !== "off") {
        richPresenceType = parseInt(store.jf_overrideType, 10);
    }
    else {
        richPresenceType = mediaData.type === "Audio" ? 2 : 3;
    }
    const templateReplace = (template) => template
