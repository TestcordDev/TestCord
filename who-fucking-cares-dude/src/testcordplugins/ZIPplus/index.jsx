...
function isZipPreviewNative(value) {
    return isRecord(value) && typeof value.fetchAttachment === "function";
}
function isExpectedFetchError(error) {
...
function findAttachment(message, mediaItem) {
    return (message.attachments ?? []).find(attachment => (attachment.proxy_url === mediaItem.proxyUrl
...
async function zipFile(file) {
    if (file.size > MAX_ZIP_SOURCE_BYTES)
        throw new Error("File is too large to zip.");
...
function handleDrop(event) {
    if (!event.dataTransfer)
        return;
    if (!settings.store.enableAutoZipper)
        return;
    ...
    for (const item of items) {
        ...
        if (entry?.isDirectory) {
            ...
            if (file) {
                if (shouldZipFile(file)) {
                    processPromises.push(zipFile(file).catch(() => file));
                }
                else {
...
async function fetchBlob(url) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (length > MAX_FETCH_BLOB_BYTES)
                return null;
            const blob = await res.blob();
            if (blob.size > 0 && blob.size <= MAX_FETCH_BLOB_BYTES)
                return blob;
        }
    }
    catch (error) {
...
function MessageContextMenu(children, props) {
    try {
        const { mediaItem, message } = props ?? {};
        if (!mediaItem || !message)
            return;
        ...
        if (!looksLikeZip)
            return;
        children.push(<Menu.MenuItem id="zippreview-open" label="Preview zip" action={async () => {
                try {
...
function ZipAttachmentPreview({ attachment }) {
    const cacheKey = getAttachmentKey(attachment);
    const [blob, setBlob] = useState(() => blobCache.get(cacheKey) || null);
    const [error, setError] = useState(null);
    const [expanded, setExpanded] = useState(() => expandedState.get(cacheKey) ?? false);
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!url) {
                    if (mounted)
                        setError("No URL for attachment");
                    return;
                }
                ...
                if (b.size === 0) {
                    if (mounted)
                        setError("Failed to fetch archive (empty file)");
                    return;
                }
                if (mounted) {
                    setBlob(b);
    ...
    if (error)
        return <div className="zp-error">{error}</div>;
    if (!blob)
        return <div className="zp-loading">Loading preview…</div>;
    return (<div className="zp-attachment-integrated">
            <ZipPreview blob={blob} name={attachment.filename || attachment.name || "archive.zip"} expanded={expanded} onExpandedChange={value => {
            setExpanded(value);
