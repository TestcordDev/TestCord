...
const SilentTypingIcon = ({ width = 24, height = 24 }) => (<svg width={width} height={height} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
...
const SilentTypingToggle = ({ isMainChat, channel }) => {
    ...
    if (!isMainChat || !showIcon || settings.store.location !== "chatbar")
        return null;
    return (<ChatBarButton tooltip={tooltip} onClick={toggle} onContextMenu={toggleGlobal}>
            ...
            <path transform="matrix(0.27724514,0,0,0.27724514,34.252062,-35.543268)" d="M 1827.701,303.065 698.835,1431.801 92.299,825.266 0,917.564 698.835,1616.4 1919.869,395.234 Z" stroke="var(--green-500)" strokeWidth="150" strokeLinecap="round" fillRule="evenodd"/>}
            </svg>
        </ChatBarButton>);
};
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("SilentTyping", () => (<HeaderBarButton icon={SilentTypingIcon} tooltip={settings.store.isEnabled ? "Silent Typing: ON" : "Silent Typing: OFF"} onClick={() => { settings.store.isEnabled = !settings.store.isEnabled; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("SilentTyping", () => (<ChannelToolbarButton icon={SilentTypingIcon} tooltip={settings.store.isEnabled ? "Silent Typing: ON" : "Silent Typing: OFF"} onClick={() => { settings.store.isEnabled = !settings.store.isEnabled; }}/>), 5);
        }
    },
    stop() {
