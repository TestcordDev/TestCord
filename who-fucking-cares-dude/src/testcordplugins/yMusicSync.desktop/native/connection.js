...
import { DEVICE_SELECT_RETRY_MS, DEVICE_TYPE_CODE, HUB_PATH, RECONNECT_BASE_MS, RECONNECT_MAX_MS, REDIRECTOR_TIMEOUT_MS, REDIRECTOR_URL } from "./constants";
...
export function wrapRequest(payload, interception = "DO_NOT_INTERCEPT_BY_DEFAULT") {
        ...
        YnisonSocket.connect(new URL(REDIRECTOR_URL), subprotocols(baseHeaders()), {
            onMessage(data) {
                ...
                try {
                    ...
                    if (!host)
                        throw new Error("Redirector returned no host");
                    resolve({
                        host,
            ...
            client = created;
            if (settled)
                created.close("Redirect resolved");
        }, error => {
            if (settled)
                return;
            settled = true;
...
function autoSelectDevice(incoming) {
    const devices = incoming.devices ?? [];
    const playable = (device) => device.info?.device_id
    ...
    if (incoming.active_device_id_optional && !isSelfDevice(incoming.active_device_id_optional))
        return;
    const target = devices.find(playable);
    if (!target?.info?.device_id)
        return;
...
function asYnisonState(value) {
    if (typeof value !== "object" || value === null)
        return null;
    const candidate = value;
    return candidate.player_state ? candidate : null;
}
function pushSnapshot() {
    if (state.lastState && !isStationSelected())
...
function scheduleReconnect() {
    if (state.reconnectTimer || !state.token)
        return;
...
export function closeConnection(reason) {
