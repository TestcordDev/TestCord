...
let channelRotationId = null;
let isRunning = false;
let remainingTime = 0;
let timerElement = null;
let currentChannelIndex = 0;
let channelRotationTimer = 0;
let channelRotationCountdownId = null;
...
let alertAudio = null;
let alertBeepTimeout = null;
let alertButton = null;
let isAlertActive = false;
let isPaused = false;
let alertDetectedUserId = "";
...
function getRandomChannelRotationInterval() {
...
function rotateChannel() {
    ...
    if (channels.length <= 1)
        return; // No rotation needed for 0 or 1 channel
...
function createAlertButton() {
    ...
    button._alertClickHandler = () => {
        if (firstClick) {
            ...
            const iconDiv = button.querySelector("div:first-child");
            if (iconDiv)
                iconDiv.textContent = "🔕";
            const actionLine = button.querySelector("#alert-action-line");
            if (actionLine)
                actionLine.textContent = "✅ Sound stopped — click again to resume";
            const hintLine = button.querySelector("#alert-hint-line");
            if (hintLine)
...
function triggerAlert(detectedUserId = "", detectedUsername = "", reason = "verification") {
    if (isAlertActive)
        return; // Don't trigger multiple alerts
    if (reason === "neon-catchlist") {
        ...
        if (detectedUsername)
            devLog(`[AutoMessageSender] From: ${detectedUsername}`);
    }
    else if (reason === "bot") {
        devLog("[AutoMessageSender] 🤖 BOT MESSAGE DETECTED IN CHANNEL!");
        if (detectedUsername)
            devLog(`[AutoMessageSender] Bot: ${detectedUsername}`);
        if (detectedUserId)
            devLog(`[AutoMessageSender] Bot ID: ${detectedUserId}`);
    }
    else {
    ...
    const iconDiv2 = button.querySelector("div:first-child");
    if (iconDiv2)
        iconDiv2.textContent = reason === "neon-catchlist" ? "🎣" : reason === "other-user" ? "👤" : reason === "bot" ? "🤖" : "⚠️";
    const titleDiv = button.querySelector("div:nth-child(2)");
    if (titleDiv)
        titleDiv.textContent = reason === "neon-catchlist" ? "NEON CATCHLIST ALERT!" : reason === "other-user" ? "OTHER USER DETECTED!" : reason === "bot" ? "BOT MESSAGE DETECTED!" : "VERIFICATION ALERT!";
    if (reason === "neon-catchlist") {
    ...
    const userLine = button.querySelector("#alert-user-line");
    if (userLine) {
        if (reason === "neon-catchlist") {
    ...
    // Build toast text
    if (reason === "neon-catchlist") {
        showToast("🎣 NEON CATCHLIST DETECTED! Rare catch incoming — Auto-messages PAUSED. Dismiss to resume.", Toasts.Type.FAILURE);
    }
    else if (reason === "other-user") {
        const who = detectedUsername && detectedUsername !== "Unknown" ? detectedUsername : "Someone";
        showToast(`👤 OTHER USER DETECTED! ${who} sent a message — Auto-messages PAUSED. Dismiss to resume.`, Toasts.Type.FAILURE);
    }
    else if (reason === "bot") {
        const who = detectedUsername && detectedUsername !== "Unknown" ? detectedUsername : "A bot";
        showToast(`🤖 BOT DETECTED! ${who} sent a message — Auto-messages PAUSED. Dismiss to resume.`, Toasts.Type.FAILURE);
    }
    else {
        ...
        showToast(`⚠️ VERIFICATION DETECTED! ${who} — Auto-messages PAUSED. Dismiss alert to resume.`, Toasts.Type.FAILURE);
    }
}
function stopAlert() {
    ...
    if (alertButton) {
        ...
        const iconDiv = alertButton.querySelector("div:first-child");
        if (iconDiv)
            iconDiv.textContent = "⚠️";
        const actionLine = alertButton.querySelector("#alert-action-line");
        if (actionLine)
            actionLine.textContent = "🔊 Click once to stop sound";
        const hintLine = alertButton.querySelector("#alert-hint-line");
        if (hintLine)
            hintLine.style.opacity = "0.7";
    }
...
function checkForVerificationMessage(message) {
    if (!message.content)
        return { detected: false, userId: "" };
    ...
    const isHumanCheck = (raw.includes("⚠️") || raw.includes("⚠")) &&
    ...
    const isCaptchaCheck = clean.includes("please complete your captcha") &&
        clean.includes("verify that you are human") &&
        /\(\d\/\d\)/.test(clean); // matches (1/5) through (5/5)
    // ─── Fallback broad patterns ─────────────────────────────────────────────
    const isBroadMatch = ((raw.includes("⚠️") || raw.includes("⚠")) &&
        (clean.includes("are you a real human") ||
    ...
    if (detected && !userId) {
        ...
        if (numericMatch)
            userId = numericMatch[1];
    }
    return { detected, userId };
}
function checkForNeonCatchlist(message) {
    const TRIGGER = 'Use "neon catchlist" to see a list of your rare catches';
    // Check plain text content
    if (message.content && message.content.includes(TRIGGER))
        return true;
    // Check embeds (description, fields, footer, title)
    if (Array.isArray(message.embeds)) {
        for (const embed of message.embeds) {
            if (embed.description && embed.description.includes(TRIGGER))
                return true;
            if (embed.title && embed.title.includes(TRIGGER))
                return true;
            if (embed.footer?.text && embed.footer.text.includes(TRIGGER))
                return true;
            if (Array.isArray(embed.fields)) {
                for (const field of embed.fields) {
                    if ((field.value && field.value.includes(TRIGGER)) ||
                        (field.name && field.name.includes(TRIGGER)))
                        return true;
                }
            }
        }
...
function checkForOtherUserMessage(message, myUserId) {
    // Ignore messages from ourselves
    if (!message.author || message.author.id === myUserId)
        return false;
    // Ignore bots
    ...
    const ignoreIds = ignoreIdsRaw
        ...
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
    if (ignoreIds.includes(message.author.id))
        return false;
    ...
    if (ignoreList.includes(rawLower))
        return false;
    ...
    for (const ignored of ignoreList) {
        ...
        if (rawLower.startsWith(ignored) && rawLower.slice(ignored.length).trim() === "")
            return false;
    }
    return true; // Someone else sent a non-ignored message → alert!
}
function checkForBotMessage(message) {
...
function addCurrentChannel() {
    ...
    for (let i = 0; i < slots.length; i++) {
        const existingId = settings.store[slots[i].idKey];
    ...
    const emptySlot = slots.findIndex(s => {
        const id = settings.store[s.idKey];
        return !id || id.trim() === "";
    });
    if (emptySlot === -1) {
    ...
    settings.store[slots[emptySlot].idKey] = currentChannel.id;
    settings.store[slots[emptySlot].enableKey] = true;
...
function toggleChannel(channelNumber) {
    const enableKey = `channel${channelNumber}Enabled`;
    const channelIdKey = `channelId${channelNumber}`;
    const channelId = settings.store[channelIdKey];
    if (!channelId || channelId.trim() === "") {
        showToast(`Channel ${channelNumber} is not configured!`, Toasts.Type.FAILURE);
        return;
    }
    const currentValue = settings.store[enableKey];
...
export default definePlugin({
    ...
    flux: {
        MESSAGE_CREATE({ message }) {
            if (!settings.store.enableVerificationDetection)
                return;
            if (!isRunning && !isPaused)
                return; // only monitor while active
            ...
            if (result.detected) {
                ...
                if (result.userId)
                    devLog(`[AutoMessageSender] Targeted user ID: ${result.userId}`);
    ...
    contextMenus: {
        "channel-context"(children, props) {
            ...
            if (!channel)
                return;
            children.push(<Menu.MenuItem label="Auto Message Sender" id="auto-message-sender">
                    <Menu.MenuItem id="auto-message-start" label={isRunning ? "✅ Stop Auto Messages" : "▶️ Start Auto Messages"} action={() => {
                    if (isRunning) {
                        stopMessageLoop();
                    }
                    else {
                    ...
                    <Menu.MenuItem id="auto-message-pause" label={isPaused ? "▶️ Resume Auto Messages" : "⏸️ Pause Auto Messages"} disabled={!isRunning} action={() => {
                    if (isPaused) {
                        manualResume();
                    }
                    else {
                    ...
                    <Menu.MenuSeparator />
                    <Menu.MenuItem id="auto-message-channels" label="📋 Channels">
                        {[
                    { idKey: "channelId1", enableKey: "channel1Enabled", num: 1 },
                    { idKey: "channelId2", enableKey: "channel2Enabled", num: 2 },
                    { idKey: "channelId3", enableKey: "channel3Enabled", num: 3 },
                    { idKey: "channelId4", enableKey: "channel4Enabled", num: 4 },
                    { idKey: "channelId5", enableKey: "channel5Enabled", num: 5 },
                    { idKey: "channelId6", enableKey: "channel6Enabled", num: 6 },
                    { idKey: "channelId7", enableKey: "channel7Enabled", num: 7 },
                    { idKey: "channelId8", enableKey: "channel8Enabled", num: 8 },
                    { idKey: "channelId9", enableKey: "channel9Enabled", num: 9 },
                    { idKey: "channelId10", enableKey: "channel10Enabled", num: 10 }
                    ...
                    return (<Menu.MenuItem key={`ch-${num}`} id={`auto-message-channel-${num}`} label={`${enabled ? "✅" : "❌"} Slot ${num}: ${name}`} action={() => toggleChannel(num)}/>);
                })}
                        <Menu.MenuSeparator />
                        <Menu.MenuItem id="auto-message-add-channel" label={(() => {
                    const slots = ["channelId1", "channelId2", "channelId3", "channelId4", "channelId5", "channelId6", "channelId7", "channelId8", "channelId9", "channelId10"];
                    ...
                    <Menu.MenuSeparator />
                    <Menu.MenuItem id="auto-message-goto-channels" label="🔗 Go To Channel">
                        {[
                    { idKey: "channelId1", num: 1 },
                    { idKey: "channelId2", num: 2 },
                    { idKey: "channelId3", num: 3 },
                    { idKey: "channelId4", num: 4 },
                    { idKey: "channelId5", num: 5 },
                    { idKey: "channelId6", num: 6 },
                    { idKey: "channelId7", num: 7 },
                    { idKey: "channelId8", num: 8 },
                    { idKey: "channelId9", num: 9 },
                    { idKey: "channelId10", num: 10 }
                ].map(({ idKey, num }) => {
                    const channelId = settings.store[idKey];
                    if (!channelId || channelId.trim() === "")
                        return null;
                    return (<Menu.MenuItem key={`goto-${num}`} id={`auto-message-goto-channel-${num}`} label={`Slot ${num}: ${getChannelDisplayName(channelId)}`} action={() => {
                    ...
                    <Menu.MenuSeparator />
                    <Menu.MenuItem id="auto-message-status" label={`Status: ${isRunning ? (isPaused ? "⏸️ Paused" : "🟢 Running") : "🔴 Stopped"}`} action={() => {
                    const allChannels = [
                    ...
                    showToast(`Status: ${runState}\n\n${statusList || "No channels configured"}\n\nMessage Delay: ${settings.store.minIntervalSeconds}s - ${settings.store.maxIntervalSeconds}s\nRotation: ${settings.store.minChannelRotationMinutes}-${settings.store.maxChannelRotationMinutes} min\nVerification Detection: ${settings.store.enableVerificationDetection ? "ON" : "OFF"}\nOther User Detection: ${settings.store.enableOtherUserDetection ? "ON" : "OFF"}\nBot Detection: ${settings.store.enableBotDetection ? "ON" : "OFF"}`, Toasts.Type.MESSAGE);
                }}/>
                    <Menu.MenuItem id="auto-message-rotate-now" label="🔄 Rotate Channel Now" disabled={!isRunning || getActiveChannels().length <= 1} action={() => {
                    rotateChannel();
                }}/>
                    <Menu.MenuSeparator />
                    <Menu.MenuItem id="auto-message-test-alert" label="🔔 Test Verification Alert" action={() => {
                    triggerAlert();
                }}/>
                    <Menu.MenuItem id="auto-message-settings" label="⚙️ Open Settings" action={() => {
