...
let originalSubscribe = null;
let originalUnsubscribe = null;
const fluxSubscribers = new Map();
const fluxFans = new Map();
let fluxBusActive = false;
const COALESCE_MS = 50;
const pendingCoalesce = new Map();
function dispatchCoalesced(actionType, event) {
...
function maybeCoalesce(actionType, event) {
    if (actionType !== "MESSAGE_CREATE" || !settings.store.messageCoalesce)
        return false;
    ...
    const entry = {
        ...
        timer: setTimeout(() => {
            ...
            if (latest?.event)
                dispatchCoalesced(actionType, latest.event);
        }, COALESCE_MS)
    };
...
function clearAllCoalesce() {
    for (const { timer } of pendingCoalesce.values())
        clearTimeout(timer);
    pendingCoalesce.clear();
}
function fluxFan(actionType) {
    return event => {
        if (maybeCoalesce(actionType, event))
            return;
        const set = fluxSubscribers.get(actionType);
        if (!set)
            return;
...
function wrappedSubscribe(actionType, handler) {
...
function wrappedUnsubscribe(actionType, handler) {
...
function startFluxBus() {
    if (fluxBusActive)
        return;
    originalSubscribe = FluxDispatcher.subscribe.bind(FluxDispatcher);
    originalUnsubscribe = FluxDispatcher.unsubscribe.bind(FluxDispatcher);
    fluxBusActive = true;
    FluxDispatcher.subscribe = wrappedSubscribe;
    FluxDispatcher.unsubscribe = wrappedUnsubscribe;
    for (const name in Plugins) {
        const p = Plugins[name];
        if (!p?.flux || !isPluginEnabled(name))
            continue;
        for (const event of Object.keys(p.flux)) {
            const handler = p.flux[event];
            if (!handler)
                continue;
            try {
...
export function wrapFluxHandlers(wrapper) {
...
function stopFluxBus() {
    if (!fluxBusActive || !originalSubscribe || !originalUnsubscribe)
        return;
    fluxBusActive = false;
    FluxDispatcher.subscribe = originalSubscribe;
    FluxDispatcher.unsubscribe = originalUnsubscribe;
    for (const [actionType, set] of fluxSubscribers) {
        const fan = fluxFans.get(actionType);
        if (fan) {
            try {
...
const wrappedToOriginal = new Map();
const failCounts = new Map();
const disabledPatches = new Set();
function makeHardenedNav(fn) {
    const wrapped = function (children, ...args) {
        if (!hardeningActive) {
        ...
        try {
            ...
            const patchName = fn.name || fn.__name || "anonymous";
            if (count >= 5) {
...
function makeHardenedGlobal(fn) {
    const wrapped = function (navId, children, ...args) {
        if (!hardeningActive) {
...
function startContextMenuHardening() {
    ...
    for (const set of navPatches.values()) {
        ...
        for (const fn of originals) {
            if (typeof fn !== "function" || wrappedToOriginal.has(fn))
                continue;
            set.delete(fn);
            set.add(makeHardenedNav(fn));
        }
    }
    ...
    for (const fn of globals) {
        ...
        globalPatches.add(makeHardenedGlobal(fn));
    }
}
...
export default definePlugin({
    ...
    start() {
        ...
        try {
            if (settings.store.contextMenuHardening)
                startContextMenuHardening();
        }
        catch (e) {
