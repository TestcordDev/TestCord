...
export function classifyHost(host) {
    ...
    if (FIRST_PARTY_HOSTS.some(fp => h === fp || h.endsWith("." + fp)))
        return "first-party";
    ...
    if (matchKnown(UNTRUSTED_KNOWN_HOSTS, h))
        return "third-party";
    if (matchKnown(TRUSTED_KNOWN_HOSTS, h))
        return "trusted-third-party";
    return "third-party";
}
export function hostReputationLabel(host) {
    const h = (host || "").toLowerCase();
    const rep = classifyHost(h);
    if (rep === "first-party")
        return "Discord";
    if (matchKnown(UNTRUSTED_KNOWN_HOSTS, h))
        return "Untrusted third party";
    if (rep === "trusted-third-party")
        return "Trusted CDN";
    return "Third party";
}
export function hostReputationNote(host) {
    const h = (host || "").toLowerCase();
    if (classifyHost(h) === "first-party")
        return "Discord's own servers.";
...
export function repTagClass(host) {
...
function getEventTitle(log) {
    if (log.category === "tracking")
        return "Discord analytics event blocked";
    if (log.category === "sentry")
        return "Sentry telemetry drop intercepted";
    if (log.category === "metrics")
        return "Metrics reporting disabled";
    if (log.category === "tokens")
        return "Authorization header stripped";
    if (log.category === "linkTracker")
        return `Tracking params stripped from ${log.domain}`;
    return `${log.action} on ${log.domain}`;
}
function buildBlockReport(log) {
...
function getShieldForCategory(category) {
    switch (category) {
...
export function PrivacySecurityPanel() {
    ...
    const limitSelectRef = useRef(null);
    const [isAllowedRouteOpen, setIsAllowedRouteOpen] = useState(false);
    const allowedRouteSelectRef = useRef(null);
    const LIMIT_OPTIONS = [
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            ...
            if (limitSelectRef.current && !limitSelectRef.current.contains(e.target)) {
                setIsLimitOpen(false);
            }
            if (allowedRouteSelectRef.current && !allowedRouteSelectRef.current.contains(e.target)) {
                setIsAllowedRouteOpen(false);
            }
        };
    ...
    const [activeTestBtn, setActiveTestBtn] = useState(null);
    ...
    const [selectedAllowedRoute, setSelectedAllowedRoute] = useState("all");
    const handleMaxLogsChange = async (newLimit) => {
        setMaxLogsState(newLimit);
        if (VencordNative?.privacy?.setMaxLogs) {
            const updated = await VencordNative.privacy.setMaxLogs(newLimit);
            if (updated)
                setMaxLogsState(updated);
        }
    };
    const fetchData = async () => {
        try {
            if (VencordNative?.privacy?.getData) {
                ...
                if (data) {
                    ...
                    if (typeof data.maxLogs === "number")
                        setMaxLogsState(data.maxLogs);
                    if (data.outboundRoutes) {
                        setOutboundRoutes(data.outboundRoutes.filter((r) => r.id !== "updates" && r.id !== "services" && r.id !== "client_updates" && r.id !== "client_services"));
                    }
                    if (data.dnsProviders)
                        setDnsProviders(data.dnsProviders);
                    if (data.selectedDnsProvider)
                        setSelectedDns(data.selectedDnsProvider);
                    if (typeof data.dnsEnabled === "boolean")
                        setDnsEngineEnabled(data.dnsEnabled);
                    if (data.dnsCacheStats)
                        setDnsCacheStats(data.dnsCacheStats);
                    const fetchedLogs = data.logs || [];
                    setLogs(fetchedLogs);
                    if (Array.isArray(data.allowedLogs))
                        setAllowedLogs(data.allowedLogs);
                    if (data.hostRules)
                        setHostRules(data.hostRules);
        ...
        try {
            const custom = globalThis.CustomDNS;
            ...
            if (active) {
                ...
                if (stats && typeof stats.nativeCalls === "number")
                    setNativeCalls(stats.nativeCalls);
            }
            else {
                setNativeCalls(0);
            }
    ...
    const setHostRule = async (host, rule) => {
        ...
        if (VencordNative?.privacy?.setHostRule) {
            ...
            if (updated)
                setHostRules(updated);
        }
    };
    const clearHostRule = async (host) => {
        setHostRules(prev => {
            const next = { ...prev };
            delete next[host];
            return next;
    ...
    const toggleShield = async (key) => {
        const nextVal = !shields[key];
        setShields(prev => ({ ...prev, [key]: nextVal }));
        if (VencordNative?.privacy?.toggleShield) {
            await VencordNative.privacy.toggleShield(key, nextVal);
        }
    };
    const handleSelectDns = async (name) => {
        setSelectedDns(name);
    ...
    const mergeDiagnosticLogs = (returned) => {
    ...
    const handleRunDiagnostic = async (mode) => {
    ...
    const handleClearDnsCache = async () => {
        ...
        try {
            globalThis.CustomDNS?.clearCache?.();
        }
        catch { }
    ...
    const filteredLogs = logs.filter(log => {
        ...
        return (log.domain.toLowerCase().includes(q) ||
    ...
    return (<SettingsTab>
            <div className="ps-command-center">
                <TabBar className="vc-settings-tab-bar" type="top" selectedItem={activeView} onItemSelect={setActiveView}>
                ...
                {activeView === "experimental" ? <ExperimentalPanel /> : <>
                    {ipcError && (<div className="ps-alert-banner" role="alert">
                ...
                <div className="ps-card">
                    ...
                    <div className="ps-privacy-toggles">
                        <div className="ps-toggle-row">
                            ...
                            <button type="button" className={`ps-toggle-switch${noTrackOn ? " ps-toggle-switch-on" : ""}`} onClick={toggleNoTrack}>
                        ...
                        <div className="ps-toggle-row">
                            ...
                            <button type="button" className={`ps-toggle-switch${dnsActive ? " ps-toggle-switch-on" : ""}`} onClick={toggleDns}>
                                <span className="ps-toggle-knob"/>
                            </button>
                        </div>
                        <div className="ps-toggle-row">
                            ...
                            <button type="button" className={`ps-toggle-switch${dnsRewrite ? " ps-toggle-switch-on" : ""}`} onClick={toggleDnsRewrite}>
                                <span className="ps-toggle-knob"/>
                            </button>
                        </div>
                        <div className="ps-toggle-row">
                            ...
                            <div className="ps-custom-select-wrapper" ref={limitSelectRef} style={{ width: "170px" }}>
                                <button type="button" className={`ps-custom-select-trigger ${isLimitOpen ? "ps-custom-select-open" : ""}`} onClick={() => setIsLimitOpen(!isLimitOpen)}>
                                    <span className="ps-custom-select-value">
                                        {LIMIT_OPTIONS.find(o => o.value === maxLogs)?.label || `${maxLogs} logs`}
                                    </span>
                                    <svg className={`ps-custom-select-chevron ${isLimitOpen ? "ps-chevron-rotated" : ""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        <polyline points="6 9 12 15 18 9"/>
                                    </svg>
                                </button>

                                {isLimitOpen && (<div className="ps-custom-select-menu">
                                        {LIMIT_OPTIONS.map(option => {
                    const isSelected = option.value === maxLogs;
                    return (<div key={option.value} className={`ps-custom-select-item ${isSelected ? "ps-custom-select-item-selected" : ""}`} onClick={() => {
                            handleMaxLogsChange(option.value);
                            setIsLimitOpen(false);
                        }}>
                                                    <span>{option.label}</span>
                                                    {isSelected && (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="ps-main-layout">
                                ...
                                <div className="ps-sc-left">
                                    ...
                                    <div className="ps-form-group" ref={selectRef}>
                                        ...
                                        <div className="ps-custom-select-wrapper">
                                            <button type="button" className={`ps-custom-select-trigger ${isDnsOpen ? "ps-custom-select-open" : ""}`} onClick={() => setIsDnsOpen(!isDnsOpen)}>
                                                <span className="ps-custom-select-value">{selectedDns}</span>
                                                <svg className={`ps-custom-select-chevron ${isDnsOpen ? "ps-chevron-rotated" : ""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                    <polyline points="6 9 12 15 18 9"/>
                                                </svg>
                                            </button>

                                            {isDnsOpen && (<div className="ps-custom-select-menu">
                                                    {Object.keys(dnsProviders).map(name => {
                    const isSelected = name === selectedDns;
                    return (<div key={name} className={`ps-custom-select-item ${isSelected ? "ps-custom-select-item-selected" : ""}`} onClick={() => {
                            ...
                            setIsDnsOpen(false);
                                ...
                                <div className="ps-sc-right">
                                    ...
                                    <div className="ps-terminal-box">
                                        {diagnosticLogs.slice(-4).map((line, idx) => (<div key={idx} className="ps-terminal-line">{line}</div>))}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="ps-card">
                            <div className="ps-card-header">
                                ...
                                <div className="ps-routes-summary">
                                    ...
                                    <div className="ps-summary-badge ps-summary-badge-interactive" role="button" tabIndex={0} onClick={() => { setSelectedAllowedRoute("all"); setIsAllowedModalOpen(true); }} title="Click to view all allowed outbound requests in a popup window">
                            ...
                            <div className="ps-outbound-grid">
                                {outboundRoutes.map(route => (<div key={route.id} className="ps-route-card ps-route-card-interactive" role="button" tabIndex={0} onClick={() => { setSelectedAllowedRoute(route.title); setIsAllowedModalOpen(true); }} title={`Click to inspect allowed requests for ${route.title}`}>
                    ...
                    <div className="ps-main-right-col">
                        <div className="ps-card">
                            ...
                            {alerts.some(a => !a.acknowledged) && (<div className="ps-alert-banner">
                                    <div className="ps-alert-banner-head">
                                        <div className="ps-alert-banner-title">
                                            <span className="ps-alert-banner-icon">⚠</span>
                                            Security alert
                                    ...
                                    <div className="ps-alert-list">
                                        {alerts.filter(a => !a.acknowledged).slice(0, 4).map(a => (<div key={a.id} className="ps-alert-item">
                            ...
                            <div className="ps-protection-bottom-grid">
                                ...
                                <div className="ps-sub-section">
                                    ...
                                    <div className="ps-tag-pills-wrap">
                                        {surfaceTags.map(({ key, title }, idx) => (<button key={idx} className={`ps-tag-pill ${shields[key] ? "ps-tag-pill-active" : ""}`} onClick={() => toggleShield(key)}>
                                                {title}
                                            </button>))}
                                    </div>
                                </div>
                                    ...
                                    <div className="ps-sub-header">
                                        ...
                                        <div className="ps-recent-blocks-actions">
                                            <div className="ps-search-wrapper">
                                                <input type="text" className="ps-search-input" placeholder="Search blocks..." value={searchQuery} onChange={e => handleSearchChange(e.target.value)}/>
                                                {searchQuery && (<button type="button" className="ps-search-clear" onClick={() => handleSearchChange("")}>
                                                        ✕
                                                    </button>)}
                                            </div>
                                        </div>
                ...
                return (<div key={log.id} className="ps-block-card-amber" role="button" tabIndex={0} onClick={() => setSelectedBlock(log)} onKeyDown={e => {
                            ...
                            setSelectedBlock(log);
                                                        ...
                                                        <div className="ps-block-left">
                                                            <div className="ps-block-event-title">
                                                                {getEventTitle(log)}
                                                                {group.count > 1 && (<span className="ps-block-count-badge">×{group.count}</span>)}
                                                            </div>
                                                            <div className="ps-block-event-sub">
                                                                {log.domain}
                                                                <span className={`ps-rep-tag ps-rep-${repTagClass(log.domain)}`}>{repLabel}</span>
                                                                {rule && (<span className={`ps-rule-tag ps-rule-${rule}`}>{rule === "allow" ? "Allowed" : "Always blocked"}</span>)}
                                                            </div>
                                    ...
                                    <div className="ps-pagination-footer">
                                        <div className="ps-export-actions">
                                            <button type="button" className="ps-export-btn" disabled={logs.length === 0} onClick={() => exportLogs("json")} title="Export the full log as JSON">
                                                Export JSON
                                            </button>
                                            <button type="button" className="ps-export-btn" disabled={logs.length === 0} onClick={() => exportLogs("csv")} title="Export the full log as CSV">
                                                Export CSV
                                            </button>
                                        </div>
                                        <div className="ps-pagination-controls">
                                            <button className="ps-page-btn" disabled={safePage <= 1} onClick={() => setCurrentPage(p => Math.max(1, p - 1))}>
                                                Prev
                                            </button>
                                            <span className="ps-page-label">Page</span>
                                            <input type="number" className="ps-page-input" min={1} max={totalPages} value={safePage} onChange={e => {
                ...
                if (!isNaN(val)) {
                    setCurrentPage(Math.max(1, Math.min(totalPages, val)));
                                            ...
                                            <button className="ps-page-btn" disabled={safePage >= totalPages} onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))}>
                                                Next
                                            </button>
                                        </div>
                                    </div>
            ...
            {selectedBlock && (<div className="ps-block-modal-overlay" onClick={() => setSelectedBlock(null)}>
                    <div className="ps-block-modal" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()}>
                        <div className="ps-block-modal-header">
                            ...
                            <button type="button" className="ps-block-modal-close" onClick={() => setSelectedBlock(null)}>
                                ✕
                            </button>
                        </div>
                ...
                return (<>
                                        ...
                                        <div className="ps-block-detail-grid">
                                            ...
                                            {parts.query && (<div className="ps-block-detail-row">
                        ...
                        <div className="ps-block-modal-footer">
                            <div className="ps-block-modal-actions">
                                {classifyHost(selectedBlock.domain) !== "first-party" && (hostRules[selectedBlock.domain]
                ? (<button type="button" className="ps-host-action-btn ps-host-action-clear" onClick={() => clearHostRule(selectedBlock.domain)}>
                                                Clear host rule
                                            </button>)
                : (<>
                                                <button type="button" className="ps-host-action-btn ps-host-action-allow" onClick={() => setHostRule(selectedBlock.domain, "allow")}>
                                                    Allow this host
                                                </button>
                                                <button type="button" className="ps-host-action-btn ps-host-action-block" onClick={() => setHostRule(selectedBlock.domain, "block")}>
                                                    Always block this host
                                                </button>
                                            </>))}
                            </div>
                            <button type="button" className="ps-block-copy-btn" onClick={() => copyToClipboard(buildBlockReport(selectedBlock), "all")}>
                                {copiedField === "all" ? "Copied" : "Copy details"}
                            </button>
                        </div>
                    </div>
                </div>)}
            {isAllowedModalOpen && (<div className="ps-block-modal-overlay" onClick={() => setIsAllowedModalOpen(false)}>
                    <div className="ps-block-modal" role="dialog" aria-modal="true" style={{ maxWidth: "820px", width: "90%" }} onClick={e => e.stopPropagation()}>
                        <div className="ps-block-modal-header">
                            <div className="ps-block-modal-title-group">
                            ...
                            <button type="button" className="ps-block-modal-close" onClick={() => setIsAllowedModalOpen(false)}>
                                ✕
                            </button>
                        </div>
                        ...
                        <div className="ps-block-modal-body ps-allowed-modal-body" style={{ maxHeight: "68vh", overflowY: "auto", padding: "16px" }}>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
                                <div style={{ display: "flex", gap: "8px", alignItems: "center", flex: 1, minWidth: "240px" }}>
                                    <input type="text" className="ps-search-input" placeholder="Search allowed requests by URL, domain, or method..." value={allowedSearchQuery} onChange={e => setAllowedSearchQuery(e.target.value)} style={{ width: "100%", padding: "6px 12px", borderRadius: "6px", fontSize: "13px" }}/>
                                    {allowedSearchQuery && (<button type="button" className="ps-search-clear" onClick={() => setAllowedSearchQuery("")}>
                                            ✕
                                        </button>)}
                                </div>
                                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                                    <div className="ps-custom-select-wrapper" ref={allowedRouteSelectRef} style={{ width: "190px" }}>
                                        <button type="button" className={`ps-custom-select-trigger ${isAllowedRouteOpen ? "ps-custom-select-open" : ""}`} onClick={() => setIsAllowedRouteOpen(!isAllowedRouteOpen)}>
                                            <span className="ps-custom-select-value">
                                                {selectedAllowedRoute === "all" ? "All Outbound Routes" : selectedAllowedRoute}
                                            </span>
                                            <svg className={`ps-custom-select-chevron ${isAllowedRouteOpen ? "ps-chevron-rotated" : ""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                <polyline points="6 9 12 15 18 9"/>
                                            </svg>
                                        </button>

                                        {isAllowedRouteOpen && (<div className="ps-custom-select-menu">
                                                <div className={`ps-custom-select-item ${selectedAllowedRoute === "all" ? "ps-custom-select-item-selected" : ""}`} onClick={() => {
                    setSelectedAllowedRoute("all");
                    setIsAllowedRouteOpen(false);
                }}>
                                                    <span>All Outbound Routes</span>
                                                    {selectedAllowedRoute === "all" && (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                            <polyline points="20 6 9 17 4 12"/>
                                                        </svg>)}
                                                </div>
                                                {outboundRoutes.map(r => {
                    const isSelected = r.title === selectedAllowedRoute;
                    return (<div key={r.id} className={`ps-custom-select-item ${isSelected ? "ps-custom-select-item-selected" : ""}`} onClick={() => {
                            ...
                            setIsAllowedRouteOpen(false);
                                    ...
                                    <button type="button" className="ps-btn ps-btn-stop" style={{ padding: "6px 12px", fontSize: "12px" }} onClick={async () => {
                if (VencordNative?.privacy?.clearAllowedLogs) {
                    ...
                    setAllowedLogs([]);
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                                {filteredAllowedLogs.length > 0 ? (filteredAllowedLogs.map(log => (<div key={log.id} style={{
                    ...
                    fontFamily: "monospace"
                                            ...
                                            <div className="ps-mono" onClick={() => copyToClipboard(log.url, log.id)} style={{
                    fontSize: "11px",
                    ...
                    cursor: "pointer"
                                        ...
                                        </div>))) : (<div className="ps-no-results" style={{ padding: "32px 0", textAlign: "center", color: "var(--text-muted)" }}>
                        ...
                        <div className="ps-block-modal-footer">
                            ...
                            <button type="button" className="ps-btn" onClick={() => setIsAllowedModalOpen(false)}>
                                Close
                            </button>
                        </div>
                    </div>
