 ...
 */
export function addGifPickerContextMenuPatch(id, render, priority = 0) {
 ...
 */
export function removeGifPickerContextMenuPatch(id) {
    handlers.delete(id);
}
/** @internal Called by ExtraContextMenusAPI to render the merged context menu. */
export function _openGifPickerContextMenu(e, instance) {
    if (!handlers.size)
        return;
    const items = [];
...
function GifPickerContextMenuRoot({ items }) {
    return (<Menu.Menu navId="gif-picker-context" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="GIF Options">
            {items}
        </Menu.Menu>);
}
