...
function EditIcon() {
    return (<svg role="img" width="18" height="18" viewBox="0 0 24 24">
...
function SoundTriggerEntryMenu(props) {
    const { trigger, index, onDelete } = props;
    return (<Menu.Menu navId="sound-trigger-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })}>
            <Menu.MenuItem id="sound-trigger-edit" label="Edit" icon={() => <EditIcon />} action={() => openTriggerModal({
            mode: "edit",
            onSubmit(trigger) {
            ...
            <Menu.MenuItem id="sound-trigger-delete" label="Delete" icon={() => <DeleteIcon width={18} height={18}/>} color="danger" action={() => Alerts.show({
...
export function SoundTriggerEntry(props) {
    const { trigger, index, onDelete } = props;
    return (<Flex flexDirection="row" className={classFactory("trigger-entry", "section", "hoverable-section")}>
            ...
            <Flex flexDirection="row">
                <Button look={"BLANK"} className={classFactory("rounded-button", "more-button")} onClick={e => ContextMenuApi.openContextMenu(e, () => <SoundTriggerEntryMenu index={index} trigger={trigger} onDelete={onDelete}/>)}>
                    <MoreIcon />
                </Button>
            </Flex>
        </Flex>);
