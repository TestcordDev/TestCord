...
const translationStateListeners = new Set();
const AITranslateIcon = ({ height = 20, width = 20, className }) => (<svg viewBox="0 0 24 24" height={height} width={width} className={className}>
...
const AITranslateSpinnerIcon = ({ height = 20, width = 20, className }) => (<svg viewBox="0 0 24 24" height={height} width={width} className={className} aria-hidden="true">
        <circle className={cl("spinner-track")} cx="12" cy="12" r="8"/>
        <circle className={cl("spinner-ring")} cx="12" cy="12" r="8"/>
    </svg>);
function showRateLimitToast(rateLimitMs) {
    const now = Date.now();
    if (now - lastRateLimitToast < 5_000)
        return;
...
const AITranslateChatButtonComponent = ({ isMainChat }) => {
    ...
    function toggle() {
        ...
        showToast(getAutoTranslateToggleLabel(settings.store.autoTranslate), settings.store.autoTranslate ? Toasts.Type.SUCCESS : Toasts.Type.MESSAGE);
    }
    return (<ChatBarButton tooltip={isTranslating ? "Translating message" : autoTranslate ? "Turn off AI outgoing translate" : "Turn on AI outgoing translate"} onClick={toggle} onContextMenu={toggle}>
...
const AITranslateChatButton = props => <WrappedAITranslateChatButton {...props}/>;
export default definePlugin({
    ...
    async onBeforeMessageSend(_, message) {
        ...
        if (!shouldTranslateOutgoingContent(message.content, targetLanguage, autoTranslate))
            return;
        ...
        try {
            ...
            if (!isTranslationGenerationCurrent(translationGeneration))
                return { cancel: true };
            if (translation) {
            ...
            if (rateLimitMs > 0)
                ...
                showToast(getTranslationStatusMessage("failed"), Toasts.Type.FAILURE);
            return { cancel: true };
        }
        finally {
