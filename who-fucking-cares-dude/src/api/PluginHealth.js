...
function push(list, value) {
    list.push(value);
    if (list.length > MAX_ENTRIES_PER_PLUGIN)
        list.shift();
}
function bumpSessionCounter(plugin, kind) {
...
async function loadHistory() {
    ...
    try {
        const stored = await DataStore.get(DB_KEY_HISTORY);
...
function computeStability(plugin) {
    let sessionsSeen = 0;
...
export const PluginHealth = {
     ...
     */
    recordPatchFailure(plugin, failure) {
        if (!plugin)
            return;
        ...
        const duplicate = entry.patchFailures.find(f => f.kind === failure.kind
            && f.find === failure.find
     ...
     */
    recordRuntimeError(plugin, source, error) {
        if (!plugin)
            return;
        const entry = ensureEntry(plugin);
     ...
     */
    registerEnabledPlugins(plugins) {
    ...
    clearRuntimeErrors(plugin) {
        const entry = registry.get(plugin);
        if (!entry)
            return;
        if (entry.runtimeErrors.length === 0)
            return;
        entry.runtimeErrors = [];
    ...
    setRuntimeHookProvider(provider) {
    ...
    async loadSafeMode() {
        if (!safeModeLoaded) {
            try {
                safeModeEnabled = (await DataStore.get(DB_KEY_SAFE_MODE)) ?? false;
            }
            catch {
    ...
    async loadQuarantine() {
        if (!quarantineLoaded) {
            try {
                const stored = await DataStore.get(DB_KEY_QUARANTINE);
                if (Array.isArray(stored))
                    quarantinedPlugins = stored;
            }
            catch {
                quarantinedPlugins = [];
            }
    ...
    getQuarantinedPlugins() {
        return [...quarantinedPlugins];
    },
    isQuarantined(pluginName) {
        return quarantinedPlugins.includes(pluginName);
    },
    async quarantinePlugin(pluginName) {
    ...
    async unquarantinePlugin(pluginName) {
        if (quarantinedPlugins.includes(pluginName)) {
    ...
    async loadCrashHistory() {
        if (!crashLogsLoaded) {
            try {
                const stored = await DataStore.get(DB_KEY_CRASH_LOGS);
                if (Array.isArray(stored))
                    crashHistory = stored;
            }
            catch {
    ...
    async recordCrash(entry) {
        ...
        if (entry.pluginName && !quarantinedPlugins.includes(entry.pluginName)) {
            ...
            const recentCrashes = crashHistory.filter(c => c.pluginName === entry.pluginName && c.timestamp >= dayAgo).length;
            if (recentCrashes >= 3) {
    ...
    async loadPluginChanges() {
        if (!pluginChangesLoaded) {
            try {
                const stored = await DataStore.get(DB_KEY_PLUGIN_CHANGES);
                if (Array.isArray(stored))
                    pluginStateChanges = stored;
            }
            catch {
                pluginStateChanges = [];
            }
    ...
    getRecentPluginChanges() {
        return [...pluginStateChanges];
    },
    async recordPluginChange(pluginName, enabled) {
        const entry = {
            timestamp: Date.now(),
            pluginName,
            enabled
        ...
        if (pluginStateChanges.length > 50)
            pluginStateChanges.pop();
        pluginChangesLoaded = true;
...
if (typeof globalThis !== "undefined") {
    globalThis.__pluginHealth = PluginHealth;
}
...
function attributeGlobalError(source, error) {
    try {
        ...
        if (plugin === "Unknown source") {
            ...
            const now = Date.now();
            if (now - lastUnknownErrorAt < 5_000)
                return;
