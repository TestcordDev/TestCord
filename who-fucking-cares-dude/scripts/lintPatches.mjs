...
function warn(at, rule, msg) {
    if (VERBOSE)
        console.warn(`${at}: WARN  ${rule} ${msg}`);
    warnings++;
}
for (const rel of tracked) {
    const text = readFileSync(join(ROOT, rel), "utf8");
    if (!text.includes("match:") && !text.includes("find:"))
        continue;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (matchM) {
            ...
            if (unbounded)
                warn(at, "P002", `unbounded ${unbounded[0]} in match`);
            const minified = src.match(/\b[a-z]\.[a-z]\b/);
            if (minified)
                fail(at, "P001", `hardcoded minified var "${minified[0]}", use \\i.\\i`);
        }
