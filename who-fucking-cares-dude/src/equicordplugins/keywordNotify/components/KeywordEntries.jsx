...
import { addKeywordEntry, cl, KEYWORD_ENTRIES_KEY, keywordEntries, removeKeywordEntry } from "..";
...
export function KeywordEntries() {
    ...
    async function setRegex(index, value) {
        keywordEntries[index].regex = value;
        updateStoreAndRender();
    }
    async function setListPriority(index, value) {
        keywordEntries[index].listPriority = value;
        updateStoreAndRender();
    }
    async function setWhitelist(index, value) {
        keywordEntries[index].whitelist = value;
        updateStoreAndRender();
    }
    async function setBlacklist(index, value) {
        keywordEntries[index].blacklist = value;
        updateStoreAndRender();
    }
    async function setIgnoreCase(index, value) {
        keywordEntries[index].ignoreCase = value;
        updateStoreAndRender();
    }
    async function setIgnoreBots(index, value) {
    ...
    const elements = keywordEntries.map((entry, i) => {
        return (<>
                <Collapsible title={`Keyword Entry ${i + 1}`}>
                    <Flex flexDirection="row">
                        <div style={{ flexGrow: 1 }}>
                            <TextInput placeholder="example|regex" spellCheck={false} value={values[i].regex} onChange={e => setRegex(i, e)}/>
                        </div>
                        <Button onClick={() => removeKeywordEntry(i, update)} variant="none" size="iconOnly" className={cl("delete")}>
                            <DeleteIcon />
                        </Button>
                    </Flex>
                    <FormSwitch value={values[i].ignoreCase} onChange={() => {
                setIgnoreCase(i, !values[i].ignoreCase);
            }} title="Ignore Case" className={cl("ignoreCaseSwitch")}/>
                    <FormSwitch value={values[i].ignoreBots ?? false} onChange={() => {
                setIgnoreBots(i, !values[i].ignoreBots);
            }} title="Ignore Bots" description="Ignore messages from bots" className={cl("ignoreCaseSwitch")}/>
                    <Flex flexDirection="row">
                        <div style={{ flexGrow: 1 }}>
                ...
                update();
                    ...
                    <div className={classes(Margins.top8, Margins.bottom8)}/>
                    <FormGenericLabel title="List priority (Advanced)" description={<>
                            <Paragraph>
