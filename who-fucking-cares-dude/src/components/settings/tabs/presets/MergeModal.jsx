...
import { getMergePlan, hasPreset, mergePresets } from "./presets";
function uniqueMergeName(sourceName, targetName) {
    const base = `${targetName} + ${sourceName}`;
    if (!hasPreset(base))
        return base;
    let index = 2;
    while (hasPreset(`${base} (${index})`))
        index++;
    return `${base} (${index})`;
}
function MergeActionDropdown({ conflict, value, onChange }) {
    const [open, setOpen] = React.useState(false);
    const selected = conflict.options.find(option => option.value === value) ?? conflict.options[0];
    return (<div className={`vc-presets-merge-dropdown${open ? " vc-presets-merge-dropdown-open" : ""}`} onBlur={e => {
            if (!e.currentTarget.contains(e.relatedTarget))
                setOpen(false);
        }}>
            <button type="button" className="vc-presets-merge-dropdown-button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(o => !o)}>
                <span>{selected.label}</span>
                <span className="vc-presets-merge-dropdown-chevron">▾</span>
            </button>
            {open && (<div className="vc-presets-merge-dropdown-menu" role="listbox">
                    {conflict.options.map(option => (<button key={option.value} type="button" role="option" aria-selected={option.value === value} className={`vc-presets-merge-dropdown-option${option.value === value ? " vc-presets-merge-dropdown-option-selected" : ""}`} onMouseDown={e => e.preventDefault()} onClick={() => {
...
function MergeConflictRow({ conflict, value, onChange }) {
    return (<div className="vc-presets-merge-conflict">
...
function MergeModal({ modalProps, sourceName, targetName, onMerged }) {
    ...
    const [themeCodeMatches, setThemeCodeMatches] = React.useState([]);
    const [decisions, setDecisions] = React.useState(() => Object.fromEntries(plan?.conflicts.map(conflict => [conflict.id, conflict.defaultAction]) ?? []));
    React.useEffect(() => {
        if (!plan?.source.themes?.length || !plan.target.themes?.length)
            return;
        ...
        for (const [sourceTheme, sourceContent] of sourceThemeFiles) {
            for (const [targetTheme, targetContent] of targetThemeFiles) {
                if (sourceTheme !== targetTheme && sourceContent === targetContent)
                    storedMatches.push(`${sourceTheme} matches ${targetTheme}`);
            }
        }
        ...
        void VencordNative.themes.getThemesList().then(themes => {
            ...
            for (const sourceTheme of plan.source.themes ?? []) {
                ...
                for (const targetTheme of plan.target.themes ?? []) {
                    ...
                    if (targetContent && sourceContent === targetContent)
                        matches.push(`${sourceTheme} matches ${targetTheme}`);
                }
            }
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            ...
            <ModalContent className="vc-presets-modal-content">
                ...
                {plan.identical && (<div className="vc-presets-merge-section vc-presets-merge-warning">
                ...
                {themeCodeMatches.length > 0 && (<div className="vc-presets-merge-section vc-presets-merge-warning">
                ...
                <div className="vc-presets-merge-section vc-presets-merge-options">
                    ...
                    <FormSwitch value={deleteSource} onChange={setDeleteSource} title={`Delete source preset (${sourceName})`} description="Removes the preset you dragged after the merged preset is created."/>
                    <FormSwitch value={deleteTarget} onChange={setDeleteTarget} title={`Delete target preset (${targetName})`} description="Removes the preset you dropped onto after the merged preset is created."/>
                </div>

                {plan.additions.length > 0 && (<div className="vc-presets-merge-section vc-presets-merge-additions">
                ...
                {plan.overlaps.length > 0 && (<div className="vc-presets-merge-section">
                ...
                {plan.conflicts.length > 0 ? (<div className="vc-presets-merge-section vc-presets-merge-conflicts">
                        ...
                        {plan.conflicts.map(conflict => (<MergeConflictRow key={conflict.id} conflict={conflict} value={decisions[conflict.id] ?? conflict.defaultAction} onChange={value => setDecisions(current => ({ ...current, [conflict.id]: value }))}/>))}
                    </div>) : (<Paragraph className="vc-presets-merge-clean">No conflicts found. This merge is only additions and identical overlaps.</Paragraph>)}
            </ModalContent>
...
export function openMergeModal(sourceName, targetName, onMerged) {
