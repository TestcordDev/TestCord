...
function RepeatMessageIcon({ className }) {
    return (<svg viewBox="0 -960 960 960" height={24} width={24} className={cl("icon", className)}>
...
function repeatMessage(msg) {
    ...
    if (msg.attachments?.length) {
        allAttachments.push(...msg.attachments.map(a => ({
            filename: a.filename,
            url: a.url,
        })));
    }
    if (msg.messageSnapshots) {
        msg.messageSnapshots.forEach(snapshot => {
            if (snapshot.message.attachments?.length) {
                allAttachments.push(...snapshot.message.attachments.map(a => ({
                    filename: a.filename,
                    url: a.url,
                })));
            }
...
const messageCtxPatch = (children, { msg }) => {
    if (!msg)
        return null;
    const group = findGroupChildrenByChildId("copy-text", children);
    if (!group)
        return;
    group.splice(group.findIndex(c => c?.props?.id === "reply") + 1, 0, (<Menu.MenuItem id="vc-repeat" label={shift ? "Repeat and Reply" : "Repeat"} icon={RepeatMessageIcon} action={async () => repeatMessage(msg)}/>));
};
