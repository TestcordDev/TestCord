...
export default function UserpluginInstallButton({ props }) {
    ...
    if (![...WHITELISTED_SHARE_CHANNELS, ...(settings.store.allowlistedChannels || "").split(",")].includes(ChannelStore.getChannel(message.channel_id).parent_id) && !WHITELISTED_SHARE_CHANNELS.includes(message.channel_id))
        return;
    const gitLink = props.message.content.match(CLONE_LINK_REGEX);
    if (!gitLink)
        return;
    ...
    return <>
        <div style={{ display: "flex" }}>
            <Button style={{
            marginTop: "5px"
        }} variant={installed ? "secondary" : "primary"} onClick={async () => {
            if (installed)
                return void OpenSettingsModule.openUserSettings("vencord_userplugins_panel");
            try {
