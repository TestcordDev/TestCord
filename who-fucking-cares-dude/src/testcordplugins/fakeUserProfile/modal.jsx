...
function Collapsible({ label, count, open, onToggle, children }) {
    return (<div className="fup-field">
...
function ColorPicker({ label, value, onChange }) {
    ...
    return (<div className="fup-field">
            ...
            <div className="fup-color-row">
                <input type="color" className="fup-color-swatch" value={hex || "#5865f2"} onChange={e => {
            const n = parseInt(e.target.value.replace("#", ""), 16);
            if (!isNaN(n))
                onChange(String(n));
        }}/>
                <input className="fup-input fup-color-input" placeholder="#5865f2 (decimal)" value={value} onChange={e => onChange(e.target.value)}/>
                {value && (<button className="fup-clear-btn" onClick={() => onChange("")} title="Clear">
...
function BadgeBtn({ label, icon, active, onClick }) {
    return (<button onClick={onClick} className={`fup-badge ${active ? "fup-badge--on" : ""}`} style={{ display: "flex", alignItems: "center", gap: 5 }}>
...
export function FakeUserProfileModal({ modalProps }) {
    ...
    useEffect(() => {
        fetch(DECORATIONS_API)
            ...
            if (!items.length)
                logger.warn("Decorations API returned no usable data", data);
        ...
        fetch(EFFECTS_API)
            ...
            if (!items.length)
                logger.warn("Effects API returned no usable data at fakeprofile.sampath.me/profile-effects", data);
            setEffects(items);
        })
    ...
    const [manualCustomBadgeIds, setManualCustomBadgeIds] = useState(initialManual.customBadgeIds ?? []);
    ...
    async function uploadImage(kind) {
        ...
        input.onchange = async () => {
            ...
            try {
                ...
                if (kind === "avatar")
                    setManualAvatarDataUrl(dataUrl);
                else
                    setManualBannerDataUrl(dataUrl);
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            <ModalContent className="fup-content">
                ...
                <Flex style={{ gap: 8 }} className={Margins.bottom16}>
                    <Button color={mode === "lookup" ? Button.Colors.PRIMARY : Button.Colors.TRANSPARENT} onClick={() => { settings.store.targetMode = "lookup"; setMode("lookup"); }}>
                        Lookup user
                    </Button>
                    <Button color={mode === "manual" ? Button.Colors.PRIMARY : Button.Colors.TRANSPARENT} onClick={() => { settings.store.targetMode = "manual"; setMode("manual"); }}>
                        Manual profile
                    </Button>
                </Flex>
                ...
                { flag: 1 << 22, label: "Active Developer", icon: "https://cdn.discordapp.com/badge-icons/6bdc42827a38498929a4920da12695d9.png" },
            ].map(b => (<BadgeBtn key={b.flag} label={b.label} icon={b.icon} active={!!(manualFlags & b.flag)} onClick={() => setManualFlags(v => v ^ b.flag)}/>))}
                        </div>

                        <SectionLabel>Nitro badge tier</SectionLabel>
                        <div className="fup-badges" style={{ marginBottom: 14 }}>
                            {NITRO_LEVELS.map((n, i) => (<BadgeBtn key={i} label={n.label} icon={n.icon || undefined} active={manualNitroLevel === (i - 1)} onClick={() => {
                    const level = i - 1;
                    ...
                    if (level >= 0) {
                        ...
                        <div className="fup-badges" style={{ marginBottom: 14 }}>
                            {BOOST_LEVELS.map((b, i) => (<BadgeBtn key={i} label={b.label} icon={b.icon || undefined} active={manualBoostMonths === (i - 1)} onClick={() => setManualBoostMonths(i - 1)}/>))}
                        </div>
                        ...
                        <Collapsible label="Avatar decoration" count={decorations.length} open={decoOpen} onToggle={() => setDecoOpen(!decoOpen)}>
                            <div className="fup-decoration-grid">
                                <div className={`fup-decoration-item ${!manualAvatarDecoration ? "fup-decoration-item--selected" : ""}`} onClick={() => { setManualAvatarDecoration(""); setManualDecorationAsset(""); }}>
                                ...
                                {decorations.map(d => (<div key={d.asset} className={`fup-decoration-item ${manualAvatarDecoration === d.asset ? "fup-decoration-item--selected" : ""}`} onClick={() => {
                    const newval = manualAvatarDecoration === d.asset ? "" : d.asset;
                    setManualAvatarDecoration(newval);
                    setManualDecorationAsset(newval);
                }}>
                                        <div className="fup-decoration-preview">
                                            <img src={`https://cdn.discordapp.com/avatar-decoration-presets/${d.asset}.png`} alt={d.name || d.asset} className="fup-decoration-img" onError={e => {
                    e.target.style.display = "none";
                        ...
                        <Collapsible label="Nameplate" count={nameplatesLoaded ? nameplates.length : undefined} open={nameplateOpen} onToggle={() => setNameplateOpen(!nameplateOpen)}>
                            {!nameplatesLoaded ? (<button className="fup-btn fup-btn-primary" style={{ width: "100%", justifyContent: "center", marginBottom: 10 }} onClick={loadNameplates}>Load nameplates</button>) : (<div className="fup-decoration-grid">
                                    <div className={`fup-decoration-item ${!manualNameplateAsset ? "fup-decoration-item--selected" : ""}`} onClick={() => { setManualNameplateAsset(""); setManualNameplateSkuId(""); }}>
                                        <div className="fup-decoration-preview">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                                <circle cx="12" cy="12" r="10"/>
                                                <line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/>
                                    ...
                                    {nameplates.filter(n => n.asset).map(n => (<div key={n.src} className={`fup-decoration-item ${manualNameplateAsset === n.asset ? "fup-decoration-item--selected" : ""}`} onClick={() => {
                        ...
                        setManualNameplateSkuId(newval);
                                            ...
                                            <div className="fup-decoration-preview" style={{ height: 40, borderRadius: 6 }}>
                                                <img src={n.src} alt={n.name} className="fup-decoration-img" style={{ objectFit: "cover" }} onError={e => {
                        e.target.style.display = "none";
                    }}/>
                                            </div>
                            ...
                            <div className="fup-field">
                                ...
                                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                    <button className={`fup-badge ${!manualNameplatePalette ? "fup-badge--on" : ""}`} onClick={() => setManualNameplatePalette("")}>None</button>
                                    {NAMEPLATE_PALETTES.map(p => (<button key={p} className={`fup-badge ${manualNameplatePalette === p ? "fup-badge--on" : ""}`} onClick={() => setManualNameplatePalette(p)}>{p}</button>))}
                                </div>
                            </div>
                        ...
                        <Collapsible label="Profile effect" count={effects.length} open={effectOpen} onToggle={() => setEffectOpen(!effectOpen)}>
                            <div className="fup-effect-grid">
                                <div className={`fup-effect-item ${!manualProfileEffectId ? "fup-effect-item--selected" : ""}`} onClick={() => { setManualProfileEffectId(""); setManualProfileEffectAsset(""); }}>
                                ...
                                {effects.map(e => (<div key={e.skuId} className={`fup-effect-item ${manualProfileEffectId === e.skuId ? "fup-effect-item--selected" : ""}`} onClick={() => {
                    const newval = manualProfileEffectId === e.skuId ? "" : e.skuId;
                    setManualProfileEffectId(newval);
                    setManualProfileEffectAsset(newval);
                }}>
                                        <div className="fup-effect-preview">
                                            <img src={e.config.thumbnailPreviewSrc} alt={e.config.title || e.skuId} className="fup-effect-img" onError={ev => {
            ...
            ].map(b => (<BadgeBtn key={b.id} label={b.label} icon={b.icon} active={manualCustomBadgeIds.includes(b.id)} onClick={() => setManualCustomBadgeIds(manualCustomBadgeIds.includes(b.id) ? manualCustomBadgeIds.filter(x => x !== b.id) : [...manualCustomBadgeIds, b.id])}/>))}
                        </div>
                        {manualCustomBadgeIds.includes("oldname") && (<Field label="Old username" value={manualOldName} placeholder="OldUser#0000" onChange={setManualOldName}/>)}
                ...
                {target && (<Flex style={{ alignItems: "center", gap: 12 }} className={Margins.bottom16}>
                        <img src={mode === "manual" && manualAvatarDataUrl ? manualAvatarDataUrl : IconUtils.getUserAvatarURL(target.user, true, 64)} alt="" width={48} height={48} style={{ borderRadius: "50%" }}/>
                        <div>
