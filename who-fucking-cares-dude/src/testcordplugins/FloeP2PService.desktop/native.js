...
const allowedReceiveDirs = new Set();
let latestSessionId = null;
let nextSessionId = 0;
export async function startSendFiles(_) {
    try {
...
export async function startSendFolder(_) {
    try {
...
export async function chooseReceiveDirectory(_) {
    try {
        ...
        if (selected.canceled || !outputDir)
...
export async function startReceive(_, locator, outputDir) {
    try {
        const cleanLocator = validateLocator(locator);
        if (!cleanLocator)
            return { success: false, error: "Enter a valid Floe code or link." };
...
export async function getLatestSession(_) {
    if (!latestSessionId)
        return { success: false, error: "No Floe session has been started." };
    const session = sessions.get(latestSessionId);
    if (!session)
        return { success: false, error: "The last Floe session is no longer available." };
    return { success: true, data: toPublicSession(session) };
}
export async function cancelLatestSession(_) {
    if (!latestSessionId)
        return { success: false, error: "No Floe session is running." };
    const session = sessions.get(latestSessionId);
    if (!session)
        return { success: false, error: "No Floe session is running." };
...
export async function cancelAllSessions(_) {
...
export async function installFloe(_) {
    try {
...
export async function updateFloe(_) {
    try {
...
export async function checkFloeUpdate(_) {
    try {
...
export async function getFloeVersion(_) {
    try {
...
async function chooseAndStartSend(kind) {
    const selected = await dialog.showOpenDialog({
    ...
    if (selected.canceled || paths.length === 0)
        return { success: false, cancelled: true };
...
function startFloeSession(kind, args, paths, outputDir) {
...
function waitForSessionReady(session) {
...
function appendOutput(session, stream, chunk) {
    ...
    if (link)
        session.link = link[1];
    if (/Waiting for peer/i.test(session.output))
        setSessionStatus(session, "waiting", "Floe is waiting for a peer.");
    if (/Connected/i.test(session.output))
        setSessionStatus(session, "running", "Floe connected to a peer.");
    if (/(?:^|\n)\s*Sent\s+/i.test(session.output) || /(?:^|\n)\s*Received\s+/i.test(session.output))
        setSessionStatus(session, "completed", "Floe reported transfer completion.");
}
...
function setSessionStatus(session, status, detail) {
...
async function resolveReceiveDir(outputDir) {
    if (typeof outputDir !== "string" || outputDir.trim().length === 0) {
        return { success: true, data: app.getPath("downloads") };
    }
    const normalizedDir = normalize(outputDir);
    if (!allowedReceiveDirs.has(normalizedDir))
        return { success: false, error: "Choose the download folder again." };
    try {
...
async function installFromGitHubRelease() {
    ...
    if (!target)
        return { code: null, output: "Unsupported platform or architecture." };
...
async function installFloeWithFallbacks(operation) {
    const failures = [];
    ...
    if (process.platform !== "win32")
        return { code: githubResult.code, output: failures.join("\n\n") };
    ...
    if (wingetResult.code === 0)
        return mergeInstallerOutput(failures, "Winget", wingetResult);
...
async function runInstallerStep(label, action) {
    try {
        return await action();
    }
    catch (error) {
...
function mergeInstallerOutput(failures, label, result) {
    return {
        code: 0,
        output: [
            ...failures,
...
function formatInstallerFailure(label, result) {
    return [
        `${label} failed.`,
        result.output
    ].filter(Boolean).join("\n");
...
function downloadText(url, redirects) {
    return new Promise((resolve, reject) => {
        const req = get(url, { headers: REQUEST_HEADERS }, res => {
            ...
            res.on("data", chunk => {
                body += chunk;
                if (body.length > MAX_TEXT_DOWNLOAD_LENGTH)
                    req.destroy(new Error("Download response is too large."));
            });
...
function appendProcessOutput(output, chunk) {
    const text = Buffer.isBuffer(chunk) ? chunk.toString("utf8") : chunk;
    return (output + text).slice(-MAX_OUTPUT_LENGTH);
}
function resultToNativeResult(result, fallbackError) {
    if (result.code === 0)
        return { success: true, data: { output: result.output || "Done." } };
    return {
        success: false,
...
function getFloeError(output, code) {
...
function formatSpawnError(message) {
    if (/spawn floe ENOENT/i.test(message))
        return "Floe was not found. Install Floe first.";
    if (/spawn .*floe\.exe ENOENT/i.test(message))
        return "Floe was not found. Install Floe first.";
    if (/spawn winget ENOENT/i.test(message))
        return "Winget was not found on this Windows install.";
    if (/spawn powershell\.exe ENOENT/i.test(message))
        return "PowerShell was not found on this Windows install.";
    if (/spawn sh ENOENT/i.test(message))
        return "The sh shell was not found.";
    return message || "Could not start the command.";
}
async function getLatestVersion() {
    const release = JSON.parse(await downloadText(GITHUB_API_LATEST_RELEASE_URL, 0));
...
async function verifyChecksum(archivePath, archiveName, checksumsUrl) {
...
async function extractBinary(archivePath, archiveExt, binaryName, outputPath) {
    const archiveData = await readFile(archivePath);
...
function extractFromZip(archiveData, binaryName) {
...
function extractFromTarGz(archiveData, binaryName) {
...
async function ensureInstallDirInPath(target) {
    if (target.os === "windows")
        return ensureWindowsPath(target.installDir);
    return `${target.installDir} must be in PATH.`;
}
async function ensureWindowsPath(installDir) {
