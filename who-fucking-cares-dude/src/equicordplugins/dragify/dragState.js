...
export function beginDrag(entity) {
    cancelGuildCleanup();
...
export function shouldIgnoreDrop(key) {
    const now = Date.now();
    if (state.lastHandledDrop.key === key && now - state.lastHandledDrop.at < dropDedupeWindowMs)
        return true;
...
export function startDragWatchdog(onExpire) {
    if (state.watchdogTimer !== null)
        return;
    state.watchdogTimer = window.setInterval(() => {
        if (!state.active)
            return;
        if (Date.now() - state.lastDragEventAt < 1200)
            return;
        clearDragState();
        onExpire();
    }, 500);
...
export function scheduleGuildCleanup(onExpire) {
    cancelGuildCleanup();
...
export function cancelGuildCleanup() {
    if (state.guildCleanupTimer === null)
        return;
