...
// ── SVG Icon ──────────────────────────────────────────────────────────────────
const DictationIcon = ({ recording = false, processing = false, height = 20, width = 20, className }) => (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24" className={className} style={{ color: processing ? "var(--text-warning)" : recording ? "var(--status-danger)" : "currentColor" }}>
...
// ── Chat Bar Button ───────────────────────────────────────────────────────────
const VoiceDictationButton = ({ isMainChat }) => {
    ...
    async function startDictation() {
        ...
        async function getRealInputDeviceId(discordId) {
            if (!discordId || discordId === "default")
                return "default";
            try {
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("VoiceDictation", () => (<HeaderBarButton icon={() => <DictationIcon />} tooltip="Voice Dictation" onClick={() => {
                    showToast("Use the chat bar button for voice dictation", Toasts.Type.FAILURE);
                }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("VoiceDictation", () => (<ChannelToolbarButton icon={() => <DictationIcon />} tooltip="Voice Dictation" onClick={() => {
                    showToast("Use the chat bar button for voice dictation", Toasts.Type.FAILURE);
                }}/>), 5);
        }
    },
