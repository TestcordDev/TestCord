...
function Row({ item, selected, index, onSelect, onRun }) {
    ...
    return (<div ref={ref} className={cl("row", { "row-selected": selected })} onMouseMove={() => { if (!selected)
        onSelect(index); }} onClick={() => onRun(item)}>
...
export function ResultsList({ sections, selectedIndex, onSelect, onRun }) {
    let index = -1;
    return (<div className={cl("results")}>
            {sections.map(section => (<div key={section.label ?? "default"} className={cl("section")}>
                    {section.label && <div className={cl("section-label")}>{section.label}</div>}
                    {section.items.map(item => {
                index += 1;
                const itemIndex = index;
                return (<Row key={item.id} item={item} index={itemIndex} selected={itemIndex === selectedIndex} onSelect={onSelect} onRun={onRun}/>);
            })}
                </div>))}
