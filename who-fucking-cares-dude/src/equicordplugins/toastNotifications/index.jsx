...
(function (NotificationLevel) {
    NotificationLevel[NotificationLevel["ALL_MESSAGES"] = 0] = "ALL_MESSAGES";
    NotificationLevel[NotificationLevel["ONLY_MENTIONS"] = 1] = "ONLY_MENTIONS";
    NotificationLevel[NotificationLevel["NO_MESSAGES"] = 2] = "NO_MESSAGES";
...
export default definePlugin({
    ...
    flux: {
        MESSAGE_CREATE({ message }) {
            ...
            if ((message.author.id === currentUser.id) // If message is from the user.
            ...
            // Channel type checks.
            if (channel.guild_id) {
                if (!shouldNotifyForGuildMessage(message, channel))
                    return;
            }
            else {
                if ((!settings.store.directMessages && channel.isDM()) // If DM notifications are disabled.
            ...
            const mockedMessage = MessageStore.getMessages(message.channel_id)?.receiveMessage(message)?.get(message.id);
            if (!mockedMessage)
                return console.error(`[ToastNotifications] Failed to retrieve mocked message from MessageStore for message ID ${message.id}!`);
            showNotification({
                message,
                mockedMessage,
                channel,
...
function navigateToChannel(channel) {
    if (!ChannelStore.hasChannel(channel.id))
        return;
 ...
 */
function shouldNotifyForGuildMessage(message, channel) {
    // Allowlist always wins.
    if (notifyFor.includes(channel.id))
        return true;
    // Friend-in-server toggle.
    if (settings.store.friendServerNotifications && RelationshipStore.isFriend(message.author.id))
        return true;
    // Respect the user's mute state for the guild/category/channel.
    if (MuteStore.isGuildOrCategoryOrChannelMuted(channel.guild_id, channel.id))
        return false;
    ...
    if (level === NotificationLevel.NO_MESSAGES)
        return false;
    if (level === NotificationLevel.ALL_MESSAGES)
        return true;
