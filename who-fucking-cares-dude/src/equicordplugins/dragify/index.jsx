...
import { hideGhost as hideDragGhost, isGhostVisible, mountGhost as mountDragGhost, scheduleGhostPosition as scheduleDragGhostPosition, showGhost as showDragGhost, unmountGhost as unmountDragGhost } from "./ghost";
import { clearInviteCache, createInvite, isGroupMessageChannel } from "./invite";
import { inspectDragEvent } from "./targets";
import { collectPayloadStrings, extractStrings, parseDragifyPayload, parseFromStrings, serializeDragEntity } from "./utils";
...
function suppressDefaultDragPreview(event) {
    const image = getTransparentDragImage();
    if (!image || !event.dataTransfer?.setDragImage)
        return;
    event.dataTransfer.setDragImage(image, 0, 0);
}
function setDragifyDataTransfer(dataTransfer, entity) {
    if (!dataTransfer)
        return;
...
function hasDragifyTransfer(dataTransfer) {
    return dataTransfer?.types?.includes("application/dragify");
}
function buildDropKey(entity, channelId) {
    return `${entity.kind}:${entity.id}:${channelId}`;
}
function inspectDragTarget(event, runtime) {
...
export default definePlugin({
    ...
    patches: [
        ...
        // Thread rows in channel list (sidebar thread items)
        {
            ...
            replacement: {
                match: /className:\i\.\i,onClick:\i,(?:onAuxClick:\i,)?"aria-label":(?=.{0,100}__invalid_threadMainContent)/,
    ...
    async onDrop(event, channel) {
        if (isInputDragSource())
            return;
        ...
        if (hasDragifyTransfer(dataTransfer)) {
            ...
            if (shouldIgnoreDrop(buildDropKey(entity, resolvedChannel.id)))
                return;
    ...
    async handleDropEntity(entity, channel) {
        try {
            ...
            if (!inserted)
                throw new Error("Unable to insert drag content");
            clearDragState();
        }
        catch (error) {
            clearDragState();
    ...
    async buildText(entity, currentChannel) {
        switch (entity.kind) {
            case "user":
    ...
    formatChannel(channelId, guildId) {
    ...
    insertText(channelId, text, options) {
        try {
    ...
    beginDrag(event, entity, options) {
        ...
        if (entity.kind === "user")
            event.dataTransfer.setData("data-user-id", entity.id);
    },
    onChannelDragStart(event, channel) {
        if (isUserDragActive())
            return;
    ...
    onUserDragStart(event, user) {
        const inspection = inspectDragTarget(event, this);
        if (inspection.hasAttachment)
            return;
    ...
    onGuildDragStart(event, guildId) {
    ...
    globalDragOver: (event) => {
        ...
        if (!inst || !inst.isMessageInputEvent(event))
            return;
        if (isInputDragSource())
            return;
        const { dataTransfer } = event;
        ...
        if (!hasActiveDrag() && !hasDragifyTransfer(dataTransfer)) {
            ...
            if (!parseFromStrings(payloads, { ChannelStore, GuildStore, UserStore }))
                return;
        }
    ...
    globalDragStart: (event) => {
        ...
        if (hasDragifyTransfer(event.dataTransfer))
            return;
    ...
    globalDragEnd: (_event) => {
        setTimeout(() => {
            if (Date.now() - getLastDropAt() < 100)
                return;
            clearDragState();
            hideDragGhost();
        }, 0);
    ...
    globalDragMove: (event) => {
        ...
        if (event instanceof DragEvent) {
            touchDrag();
            if (isGuildDragActive())
                scheduleGuildCleanup(hideDragGhost);
        }
    ...
    buildGhost(entity) {
        ...
        if (entity.kind === "channel") {
            ...
            const isGroupDm = channel.type === 3 /* ChannelType.GROUP_DM */;
            ...
            else if (channel.type === 3 /* ChannelType.GROUP_DM */) {
            ...
            if (!iconUrl && (channel.isDM() || channel.type === 3 /* ChannelType.GROUP_DM */)) {
            ...
            if (isThread) {
                ...
                switch (channel.type) {
                    case 2 /* ChannelType.GUILD_VOICE */:
                    case 13 /* ChannelType.GUILD_STAGE_VOICE */:
                        badge = "voice";
                        break;
                    case 15 /* ChannelType.GUILD_FORUM */:
                        badge = "forum";
                        break;
                    case 16 /* ChannelType.GUILD_MEDIA */:
                        badge = "media";
                        break;
                    case 5 /* ChannelType.GUILD_ANNOUNCEMENT */:
                        badge = "announcement";
                        break;
                    case 1 /* ChannelType.DM */:
    ...
    getGroupDmDisplayName(channel) {
        if (!channel || channel.type !== 3 /* ChannelType.GROUP_DM */)
            return null;
