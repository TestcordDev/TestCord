...
function ScheduleTimeModalInner({ channelId, content, attachments, rootProps, close }) {
    const [scheduleType, setScheduleType] = useState("delay");
    ...
    return (<Modal {...rootProps} size="sm" title="Schedule Message" actions={[
            {
                text: "Schedule",
                variant: "primary",
            ...
            <div className={cl("schedule-type-buttons")}>
                <Button size="small" variant={scheduleType === "delay" ? "primary" : "secondary"} onClick={() => setScheduleType("delay")}>
                    Delay
                </Button>
                <Button size="small" variant={scheduleType === "time" ? "primary" : "secondary"} onClick={() => setScheduleType("time")}>
                    Specific Time
                </Button>
            </div>
...
export function openScheduleTimeModal(channelId, content, attachments) {
    const key = openModal(props => (<ScheduleTimeModal channelId={channelId} content={content} attachments={attachments} rootProps={props} close={() => closeModal(key)}/>));
}
