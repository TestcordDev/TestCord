...
import { Clickable, ContextMenuApi, createRoot, FluxDispatcher, Menu, Modal, openModal, ReactDOM, useCallback, useEffect, useMemo, useState } from "@webpack/common";
...
function useColorNavigation(initialColor) {
...
function ColorContextMenu({ colorKeys }) {
    const [query, setQuery] = useState("");
    const filtered = colorKeys.filter(k => !query || k.toLowerCase().includes(query.toLowerCase()));
    return (<Menu.Menu navId="vc-ic-colors-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Icon Viewer Colors">
            <Menu.MenuControlItem id="vc-ic-colors-search" control={(props, ref) => (<Menu.MenuSearchControl {...props} query={query} onChange={setQuery} ref={ref} placeholder={getIntlMessage("SEARCH")} autoFocus/>)}/>
            <Menu.MenuSeparator />
            {filtered.map(colorKey => (<Menu.MenuItem key={colorKey} id={colorKey} label={colorKey} action={() => FluxDispatcher.dispatch({ type: "ICONVIEWER_COLOR_CHANGE", color: colorKey })}/>))}
        </Menu.Menu>);
}
...
function saveIcon(iconName, icon, color, size, type) {
...
function OtherContextMenu({ iconName, Icon, color }) {
    ...
    const handleSave = (type) => {
        ...
        const iconEl = type === "image/svg+xml"
            ...
            : document.querySelector(".vc-ic-icon-preview .vc-ic-icon-large");
        if (iconEl)
            saveIcon(iconName, iconEl, color, size, type);
    };
    return (<Menu.Menu navId="vc-ic-other-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Icon Options">
...
function IconModal({ iconName, Icon, onClose, transitionState }) {
    ...
    const openOtherMenu = (e) => {
        if (e)
            ContextMenuApi.openContextMenu(e, () => (<OtherContextMenu iconName={iconName} Icon={Icon} color={color}/>));
    };
    return (<Modal transitionState={transitionState} onClose={onClose} size="md" title={iconName} actions={[
            {
                text: "Actions",
                variant: "primary",
            ...
            <Flex className="vc-ic-modal-main">
                <div className="vc-ic-icon-preview" aria-label={colorData?.name} onContextMenu={openColorMenu} onWheel={onWheel}>
                ...
                <Flex flexDirection="column" className="vc-ic-icon-info">
                    <Flex className="vc-ic-icon-sizes">
                        {iconSizes.map(size => (<TooltipContainer text={size} key={size}>
...
export function openIconModal(iconName, Icon) {
