...
function BooIndicator() {
    ...
    const handleClick = () => {
        ...
        openModal(modalProps => (<ErrorBoundary>
                <GhostedUsersModal modalProps={modalProps} ghostedChannels={ghostedChannels} onClearGhost={clearChannelFromGhost}/>
            </ErrorBoundary>));
    };
    ...
    return (<>
            {settings.store.showIndicator && getGhostedChannels().length > 0 && (<div id={cl("container")}>
                    <Tooltip text={getTooltipText()} position="right">
                        {({ onMouseEnter, onMouseLeave }) => (<div id={cl("container")} className={cl("clickable")} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={handleClick}>
...
function makeContextItem(props) {
    return <Menu.MenuItem id="ec-ghosted-clear" key="ec-ghosted-clear" label="unghost" action={() => {
