...
import { CardClasses, MoreHorizontalIcon, OverlayClasses, Spinner, } from "@equicordplugins/songSpotlight.desktop/ui/common";
import { openSettingsModal } from "@equicordplugins/songSpotlight.desktop/ui/settings";
import { sid } from "@song-spotlight/api/util";
import { copyWithToast } from "@utils/discord";
import { classes } from "@utils/index";
import { ContextMenuApi, FluxDispatcher, Menu, React, Tooltip, useEffect, useMemo, UserStore, useState, } from "@webpack/common";
import Song from ".";
import CollapsedProfileSongs from "./CollapsedProfileSongs";
export default function ProfileSongs({ user, isSideBar }) {
    const [failed, setFailed] = useState(false);
    const { isAuthorized } = useAuthorizationStore();
    const { users } = useSongStore();
    const { profileSongsLimit, collapseSongList } = settings.use(["profileSongsLimit", "collapseSongList"]);
    const userId = user.id;
    const data = users[userId]?.data;
    useEffect(() => {
        if (isAuthorized() && !data)
            listData(userId).catch(() => setFailed(true));
    }, [isAuthorized()]);
    ...
    return (<div className={classes(isSideBar && OverlayClasses.overlay, isSideBar && CardClasses.card, cl("songs-container", isSideBar && "songs-container-sidebar"))} key="song-spotlight-profile-songs">
            <Flex justifyContent="space-between">
                <BaseText size="xs" weight={isSideBar ? "semibold" : "medium"} className={cl("header")}>
                    Song Spotlight
                </BaseText>
                <Tooltip text="More">
                    {props => (<MoreHorizontalIcon {...props} width={16} height={16} onClick={e => ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="vc-songspotlight-profile-songs" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label={"Profile songs"}>
                                        {owned && (<Menu.MenuItem id="edit-songs" label="Edit songs" icon={PencilIcon} action={() => openSettingsModal()}/>)}
                                        <Menu.MenuItem id="copy-link" label="Copy JSON" icon={LinkIcon} action={() => copyWithToast(JSON.stringify(data))}/>
                                    </Menu.Menu>))} className={cl("icon", "songs-more")}/>)}
                </Tooltip>
            </Flex>
            <Flex flexDirection="column" gap="6px">
                {clampedData.map((song, i) => <Song owned={owned} song={song} index={i} key={sid(song)}/>)}
            </Flex>
            {clamped && data.length > profileSongsLimit && (<Link onClick={() => setClamped(false)}>
