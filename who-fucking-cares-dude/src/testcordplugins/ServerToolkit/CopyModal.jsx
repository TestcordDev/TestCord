...
import { Button, Forms, GuildStore, React, Select, Text, TextInput, useEffect, useMemo, useRef, UserStore, useState, } from "@webpack/common";
import { runCopy } from "./copy/engine";
import { settings } from "./index";
export function openCopyModal(guild) {
    openModal(props => <CopyModal modalProps={{ ...props, title: `Duplicate Server — ${guild.name}` }} guild={guild}/>);
}
function CopyModal({ modalProps, guild }) {
    ...
    const [newName, setNewName] = useState(guild.name);
    const [existingId, setExistingId] = useState(ownedGuilds[0]?.id ?? "");
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */} className="guild-toolkit-modal">
            ...
            <ModalContent className="guild-toolkit-content">
                <Forms.FormSection title="Target">
                    <div className="gt-target-toggle">
                        <Button size={Button.Sizes.SMALL} color={targetMode === "new" ? Button.Colors.BRAND : Button.Colors.PRIMARY} className={targetMode === "new" ? "gt-mode-btn gt-selected" : "gt-mode-btn"} onClick={() => setTargetMode("new")}>
                            Create new server
                        </Button>
                        <Button size={Button.Sizes.SMALL} color={targetMode === "existing" ? Button.Colors.BRAND : Button.Colors.PRIMARY} className={targetMode === "existing" ? "gt-mode-btn gt-selected" : "gt-mode-btn"} onClick={() => setTargetMode("existing")} disabled={ownedGuilds.length === 0}>
                            Use existing server I own
                        </Button>
                    </div>

                    {targetMode === "new" ? (<TextInput placeholder="New server name" value={newName} onChange={setNewName} maxLength={100} className="gt-input"/>) : (<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                            <Select options={ownedGuilds.map(g => ({ label: g.name, value: g.id }))} isSelected={(v) => v === existingId} select={(v) => setExistingId(v)} serialize={(v) => v}/>
                            <TextInput placeholder="Overwrite server name to..." value={newName} onChange={setNewName} maxLength={100} className="gt-input"/>
                        </div>)}
                </Forms.FormSection>
                ...
                {targetMode === "existing" && (<Forms.FormSection title="Wipe target before copying">
                        ...
                        {anyWipe && (<>
                                ...
                                <TextInput placeholder="OVERWRITE" value={confirmText} onChange={setConfirmText} className="gt-input"/>
                            </>)}
                    </Forms.FormSection>)}

                {log.length > 0 && (<Forms.FormSection title="Progress">
                        <div className="gt-log">
                            {log.map((line, i) => (<div key={i} className="gt-log-line">{line}</div>))}
                        </div>
                    </Forms.FormSection>)}
            </ModalContent>
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: "12px", marginLeft: "auto" }}>
                    <Button color={Button.Colors.PRIMARY} look={Button.Looks.LINK} onClick={closeModal} disabled={running}>
                        {running ? "Running…" : "Close"}
                    </Button>
                    <Button color={Button.Colors.BRAND} onClick={handleRun} disabled={!canRun}>
