...
function TimeAlogIcon({ height = 20, width = 20, className, color = "currentColor" }) {
    return (<svg viewBox="0 0 24 24" width={width} height={height} className={className} aria-hidden="true">
            <path fill={color} d="M12 1.75A10.25 10.25 0 1 0 22.25 12 10.26 10.26 0 0 0 12 1.75Zm0 18.5A8.25 8.25 0 1 1 20.25 12 8.26 8.26 0 0 1 12 20.25Z"/>
            <path fill={color} d="M12 6.25a1 1 0 0 0-1 1v4.34l-2.62 1.57a1 1 0 1 0 1.02 1.72l3.1-1.86a1 1 0 0 0 .5-.86V7.25a1 1 0 0 0-1-1Z"/>
        </svg>);
}
const TIME_ALOG_KEYS = ["isActive"];
function TimeAlogButton() {
    const { isActive } = settings.use(TIME_ALOG_KEYS);
    const ButtonIcon = (props) => (<TimeAlogIcon {...props} color={isActive ? ACTIVE_ICON_COLOR : props.color}/>);
    return (<HeaderBarButton icon={ButtonIcon} tooltip={isActive ? "timeAlog: ON" : "timeAlog: OFF"} aria-label="Toggle timeAlog" selected={isActive} onClick={() => {
...
export default definePlugin({
    ...
    tags: ["Utility", "Chat", "Privacy"],
    authors: [TestcordDevs.x2b],
    dependencies: ["HeaderBarAPI"],
    settings,
    pendingTimeouts: new Map(),
    pendingReplacementDeletes: new Map(),
    ignoredNonces: new Set(),
    boundOnMessageCreate: null,
    queueReplacementDelete(nonce) {
    ...
    consumeReplacementDelete(nonce) {
    ...
    async antiLogDelete(channelId, messageId) {
    ...
    scheduleDeletion(message) {
        if (this.pendingTimeouts.has(message.id))
            return;
        const timeout = setTimeout(() => {
    ...
    onMessageCreate({ message, optimistic, type }) {
        if (!settings.store.isActive || optimistic || type !== "MESSAGE_CREATE")
            return;
        ...
        if (message.nonce && this.ignoredNonces.has(message.nonce))
            return;
