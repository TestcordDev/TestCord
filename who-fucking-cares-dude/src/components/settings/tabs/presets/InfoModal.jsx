...
function InfoStat({ label, value, copyValue, mono }) {
    const copy = () => {
        copyToClipboard(String(copyValue ?? value));
        showToast(`${label} copied.`, Toasts.Type.SUCCESS);
    };
    return (<div className="vc-presets-stat" role="button" tabIndex={0} onClick={copy} onKeyDown={e => {
...
function DevInfo({ preset, globalDefault, globalForce }) {
    ...
    return (<div className="vc-presets-stats">
            ...
            <InfoStat label="Restore on apply" value={preset.restoreSettings === undefined
            ? `Default → ${effectiveRestore ? "On" : "Off"}`
            : preset.restoreSettings ? "On" : "Off"}/>
            <InfoStat label="Force apply" value={preset.forceApply === undefined
...
function Summary({ preset }) {
    const entries = Object.entries(preset.plugins);
    return (<div className="vc-presets-summary">
            {entries.length === 0
            ...
            : entries.map(([pluginName, cfg]) => {
                ...
                return (<div key={pluginName} className="vc-presets-summary-plugin">
                            <span className="vc-presets-summary-name">{pluginName}{cfg.enabled ? "" : " (disabled)"}</span>
                            {settingKeys.length > 0 && (<div className="vc-presets-summary-settings">
                                    {settingKeys.map(k => (<div key={k}><span className="vc-presets-summary-key">{k}</span>: {stringifyPreset(cfg[k])}</div>))}
                                </div>)}
                        </div>);
            })}
        </div>);
}
function InfoModal({ modalProps, name, onChange }) {
    ...
    const showTab = (next) => {
        if (next === "json" && !jsonDirty && preset)
            setJson(stringifyPreset(preset, 4));
        setTab(next);
    };
    if (!preset) {
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            ...
            <ModalContent className="vc-presets-modal-content">
                ...
                {tab === "settings" && (<div className="vc-presets-settings">
                        <FormSwitch value={!!preset.liveBackup} onChange={toggleLive} title="Live backup" description="Auto-resnapshot this preset's plugins and plugin settings whenever a plugin is toggled or a setting changes. Themes, QuickCSS, and DataStore only update when you save over the preset."/>
                ...
                {tab === "json" && (<div className="vc-presets-json">
                        ...
                        <TextArea className="vc-presets-modal-textarea" value={json} onChange={v => { setJson(v); setJsonDirty(true); }} placeholder="{ … }"/>
                    </div>)}
            </ModalContent>
            <ModalFooter className="vc-presets-modal-footer">
...
export function openInfoModal(name, onChange) {
