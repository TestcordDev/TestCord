...
export const TranslateIcon = ({ height = 20, width = 20, className }) => {
    return (<svg viewBox="0 96 960 960" height={height} width={width} className={classes(cl("icon"), className)}>
...
function AutoTranslateConfirmModal(props) {
    const s = settings.use(["dismissedAutoTranslateAlert"]);
    return (<ConfirmModal {...props} title="Vencord Auto-Translate Enabled" subtitle="You just enabled Auto Translate! Any message will automatically be translated before being sent." confirmText="Disable Auto-Translate" onConfirm={() => settings.store.autoTranslate = false} cancelText="Got it" variant="primary" checkboxProps={{
            checked: s.dismissedAutoTranslateAlert === true,
            onChange: checked => s.dismissedAutoTranslateAlert = checked,
        }}/>);
}
export const TranslateChatBarIcon = ({ isMainChat }) => {
    ...
    const button = (<ChatBarButton tooltip="Open Translate Modal" onClick={e => {
            if (e.shiftKey)
                return toggle();
            else
                openTranslateModal();
        }} onContextMenu={toggle} buttonProps={{
    ...
    if (shouldShowTranslateEnabledTooltip && settings.store.showAutoTranslateTooltip)
        return (<TooltipContainer text="Auto Translate Enabled" forceOpen>
                {button}
            </TooltipContainer>);
    return button;
};
