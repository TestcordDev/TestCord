...
export const gitHash = process.env.TESTCORD_HASH ||
...
export async function buildOrWatchAll(buildConfigs) {
    if (watch) {
        await Promise.all(buildConfigs.map((cfg) => context(cfg).then((ctx) => ctx.watch())));
    }
    else {
        await Promise.all(buildConfigs.map((cfg) => build(cfg))).catch((error) => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            ...
            for (const dir of pluginDirs) {
                ...
                for (const file of files) {
                    ...
                    if (!file.isDirectory() && !/\.tsx?$/.test(fileName))
                        continue;
                    if (file.isDirectory() &&
                    ...
                    if (target && !IS_REPORTER) {
                        const excluded = (target === "dev" && !IS_DEV) ||
