...
import { ChannelStore, EmojiStore, GuildMemberStore, GuildRoleStore, GuildStore, RestAPI, StickersStore, UserStore, } from "@webpack/common";
...
async function fetchBlob(url, maxSize, signal) {
...
async function cloneEmoji(guildId, emoji, signal) {
...
async function cloneSticker(guildId, sticker, signal) {
...
async function getGuildIconBase64(guild, signal) {
    if (!guild.icon)
        return null;
...
export async function runCopy(opts) {
    ...
    if (target.kind === "new") {
        ...
        try {
            ...
            const nonCategories = defaultChannels.filter((c) => c.type !== 4);
            const categories = defaultChannels.filter((c) => c.type === 4);
        ...
        if (!target.guildId)
            throw new Error("No target guild selected.");
    ...
    if (include.roles) {
        ...
        try {
            ...
            const backupRoles = roles.filter((role) => role.name !== "@everyone");
    ...
    if (include.channels) {
        ...
        try {
            ...
            for (const [, channelData] of Object.entries(allChannels)) {
                if (channelData && channelData.guild_id === source.id) {
            ...
            for (const category of categories) {
                throwIfAborted(signal);
                try {
                    const permissionOverwrites = category.permission_overwrites.map((o) => ({
                        ...o,
            ...
            for (const parentKey of Object.keys(channelsByParent)) {
                ...
                for (const ch of [...nonForums, ...forums]) {
                    throwIfAborted(signal);
                    try {
                        const permissionOverwrites = ch.permission_overwrites.map((o) => ({
                            ...o,
                            id: roleMapping[o.id] || o.id,
                        }));
                        const channelBody = {
                        ...
                        if (ch.topic)
                            channelBody.topic = ch.topic;
                        if (ch.nsfw !== undefined)
                            channelBody.nsfw = ch.nsfw;
                        if (ch.rate_limit_per_user)
                            channelBody.rate_limit_per_user = ch.rate_limit_per_user;
                        if (ch.bitrate)
                            channelBody.bitrate = ch.bitrate;
                        else if (ch.type === 2)
                            channelBody.bitrate = 96000;
                        if (ch.user_limit)
                            channelBody.user_limit = ch.user_limit;
                        ...
                        if (ch.rtc_region && ch.rtc_region !== null)
                            channelBody.rtc_region = ch.rtc_region;
                        if (ch.video_quality_mode)
                            channelBody.video_quality_mode = ch.video_quality_mode;
    ...
    if (include.bots) {
        ...
        try {
            const members = Object.values(GuildMemberStore.getMembers(source.id));
            const bots = members
    ...
    if (include.emojis) {
        ...
        try {
            ...
            const limitedEmotes = [
                ...allEmotes.filter((emote) => !emote.animated).slice(0, maxPerType),
                ...allEmotes.filter((emote) => emote.animated).slice(0, maxPerType),
            ];
    ...
    if (include.stickers) {
        ...
        try {
            ...
            for (const sticker of allStickers) {
                throwIfAborted(signal);
                try {
                    const ext = StickerExtMap[sticker.format_type] || "png";
