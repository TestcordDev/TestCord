...
import { clearStaleNavigationContext, closeTab, createTab, handleChannelSwitch, isNavigationFromSource, isTabSelected, moveToTab, openedTabs, openStartupTabs, saveTabs, settings, setUpdaterFunction, useGhostTabs } from "@equicordplugins/channelTabs/util";
...
export default function ChannelsTabsContainer(props) {
    ...
    useEffect(() => {
        const matchesKeybind = (event, keybindString) => {
    ...
    const newTabButton = (<button onClick={() => createTab(props, true)} className={cl("button", "new-button", "hoverable")}>
            <PlusSmallIcon />
        </button>);
    return (<div className={classes(cl("container"), tabBarPosition === "top" && cl("container-top"), !animationHover && cl("no-hover-animation"), !animationSelection && cl("no-selection-animation"), !animationDragDrop && cl("no-drag-animation"), !animationEnterExit && cl("no-enter-exit-animation"), !animationIconPop && cl("no-icon-pop-animation"), !animationCloseRotation && cl("no-close-rotation"), !animationPlusPulse && cl("no-plus-animation"), !animationMentionGlow && cl("no-mention-glow"), !animationCompactExpand && cl("no-compact-animation"), !animationSelectedBorder && cl("no-selected-border"), !animationSelectedBackground && cl("no-selected-background"), !animationTabShadows && cl("no-tab-shadows"), !animationTabPositioning && cl("no-tab-positioning"), !animationResizeHandle && cl("no-resize-handle-animation"), !animationQuestsActive && cl("no-quests-active-animation"), !compactAutoExpandSelected && cl("no-compact-auto-expand"), !compactAutoExpandOnHover && cl("no-compact-hover-expand"))} ref={ref} style={{ "--tab-width-scale": tabWidthScale / 100, "--tab-height-scale": tabHeightScale / 100 }} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => <BasicContextMenu />)}>
            {showBookmarkBar && <>
                <BookmarkContainer {...props} userId={userId}/>
                <div className={cl("separator")}/>
            </>}
            <div className={cl("tab-container")}>
                <HorizontalScroller customRef={node => { scrollerRef.current = node; }} className={cl("tab-scroller", shouldFollowNewTabButton && "tab-scroller-following")}>
                    {openedTabs.filter(tab => tab != null).map((tab, i) => <ChannelTab {...tab} index={i} key={tab.id}/>)}
                    {GhostTabs}
...
export function ChannelTabsPreview(p) {
    ...
    if (!id)
        return <Paragraph>there's no logged in account?????</Paragraph>;
    const { setValue } = p;
    const { tabSet } = settings.use(["tabSet"]);
