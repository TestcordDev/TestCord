...
export function getTotalCpu(_e) {
    try {
        let total = 0;
        for (const m of app.getAppMetrics())
            total += m.cpu?.percentCPUUsage ?? 0;
        return total;
    }
    catch {
        return 0;
...
export async function setProcessPriority(_e, level) {
...
export function relaunchApp(_e) {
    app.relaunch();
    app.exit(0);
}
// Determines Discord app data directory based on executable path.
function getDiscordAppDataPath(appData) {
    try {
        const exe = process.execPath.toLowerCase();
        if (exe.includes("discorddevelopment"))
            return join(appData, "discorddevelopment");
        if (exe.includes("discordcanary"))
            return join(appData, "discordcanary");
        if (exe.includes("discordptb"))
            return join(appData, "discordptb");
    }
    catch {
        // Fallback to default
    }
...
export async function cleanCache(_e) {
