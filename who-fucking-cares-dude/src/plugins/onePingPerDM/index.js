...
export default definePlugin({
    ...
    isPrivateChannelRead(message) {
        const ignoreList = settings.store.ignoreUsers.split(", ").filter(Boolean);
        if (ignoreList.includes(message.author.id))
            return true;
        const channelType = ChannelStore.getChannel(message.channel_id)?.type;
        if ((channelType !== 1 /* ChannelType.DM */ && channelType !== 3 /* ChannelType.GROUP_DM */) ||
            (channelType === 1 /* ChannelType.DM */ && settings.store.channelToAffect === "group_dm") ||
            (channelType === 3 /* ChannelType.GROUP_DM */ && settings.store.channelToAffect === "user_dm") ||
