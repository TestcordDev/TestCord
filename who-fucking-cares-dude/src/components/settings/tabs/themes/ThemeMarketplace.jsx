...
import { fetchMarketplaceCatalog, getItemLink } from "./MarketplaceData";
...
export function decodeThemeContent(content) {
    if (!content)
        return "";
    try {
...
export async function saveThemeAsLocal(name, css) {
...
function getThemeLink(themeId) {
    return getItemLink(themeId);
}
function installTheme(theme) {
    const themeLink = getThemeLink(theme.id);
    const currentLinks = Array.isArray(Settings.themeLinks) ? Settings.themeLinks : [];
...
function openEnlargedImageModal(url, title, width = 1280, height = 720) {
    if (!url)
        return;
    try {
        openImageModal({
        ...
        openModal(props => (<Modal {...props} size="lg" title={title ? `${title} - Image Preview` : "Image Preview"}>
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: "16px" }}>
                    <img src={url} alt={title ?? "Image Preview"} style={{
                maxWidth: "100%",
                maxHeight: "75vh",
                borderRadius: "8px",
                objectFit: "contain",
...
export function openThemeCodeModal(name, content) {
    const code = decodeThemeContent(content);
    openModal(modalProps => (<CodeViewerModal modalProps={modalProps} title={`${name} - Code`} code={code} editable={false} onSaveAsLocal={css => saveThemeAsLocal(name, css)}/>));
}
function ThemeDetailsModalContent({ theme, installed, onToggleInstall, onTagClick, modalProps }) {
    const themeLink = getThemeLink(theme.id);
    ...
    return (<>
            <Modal {...modalProps} size="md" title={theme.name}>
                <div className={cl("modal-body")}>
                    {theme.thumbnail_url && (<div className={cl("modal-banner")} onClick={e => {
                e.stopPropagation();
                setLightboxOpen(true);
            }} title="Click to enlarge preview image">
                            <img src={theme.thumbnail_url} alt={theme.name} className={cl("modal-img")}/>
                        </div>)}

                    {/* Top Action Bar: Immediately visible on open without scrolling */}
                    <div className={cl("modal-top-actions")}>
                        <Button variant={installed ? "dangerPrimary" : "primary"} className={cl("modal-top-action-btn")} onClick={handleToggleInstall}>
                            {installed ? "Uninstall Theme" : "Install Theme"}
                        </Button>
                        {theme.content && (<Button variant="secondary" className={cl("modal-top-action-btn")} onClick={() => openThemeCodeModal(theme.name, theme.content)}>
                                View Code
                            </Button>)}
                        {theme.source && (<Button variant="secondary" className={cl("source-btn")} onClick={() => VencordNative.native.openExternal(theme.source)}>
                                Open GitHub / Source
                            </Button>)}
                    </div>

                    {/* Tags: Immediately visible at top without scrolling */}
                    {theme.tags?.length > 0 && (<div>
                            <Heading className={cl("modal-section-title")}>Tags (Click to filter)</Heading>
                            <div className={cl("card-tags")}>
                                {theme.tags.map(tag => (<button key={tag} className={classes(cl("card-tag"), cl("tag-btn"))} onClick={() => {
                    modalProps.onClose();
                    onTagClick(tag);
                }}>
                                        #{tag}
                    ...
                    <div>
                        ...
                        <div className={cl("modal-details-grid")}>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Author", authorName)} title="Click to copy author">
                                <span className={cl("modal-detail-label")}>Author</span>
                                <span className={cl("modal-detail-value")}>{authorName}</span>
                            </div>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Downloads", (theme.downloads ?? 0).toLocaleString())} title="Click to copy downloads count">
                            ...
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Likes", (theme.likes ?? 0).toLocaleString())} title="Click to copy likes count">
                                <span className={cl("modal-detail-label")}>Likes</span>
                                <span className={cl("modal-detail-value")}>{(theme.likes ?? 0).toLocaleString()}</span>
                            </div>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Release Date", theme.release_date ? new Date(theme.release_date).toLocaleDateString() : "N/A")} title="Click to copy release date">
                            ...
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Theme ID", theme.id)} title="Click to copy theme ID">
                                <span className={cl("modal-detail-label")}>Theme ID</span>
                                <span className={cl("modal-detail-value")}>{theme.id}</span>
                            </div>
                            <div className={cl("modal-detail-card")} onClick={() => copyStat("Status", installed ? "Installed" : "Available")} title="Click to copy theme status">
                                <span className={cl("modal-detail-label")}>Status</span>
                                <span className={cl("modal-detail-value")} style={{ color: installed ? "var(--status-positive, #23a55a)" : "var(--brand-500, #5865f2)" }}>
                                    {installed ? "✓ Installed" : "Available"}
                                </span>
                            </div>
                            <div className={classes(cl("modal-detail-card"), cl("modal-detail-card-full"))} onClick={() => copyStat("Theme Link", themeLink)} title="Click to copy theme link">
            ...
            {lightboxOpen && theme.thumbnail_url && (<div className={cl("lightbox-overlay")} onClick={() => setLightboxOpen(false)}>
                    <div className={cl("lightbox-content")} onClick={e => e.stopPropagation()}>
                        <div className={cl("lightbox-header")}>
                            ...
                            <div className={cl("lightbox-actions")}>
                                <Button size="small" variant="secondary" className={cl("lightbox-btn")} onClick={() => VencordNative.native.openExternal(theme.thumbnail_url)}>
                                    Open Original
                                </Button>
                                <button className={cl("lightbox-close")} onClick={() => setLightboxOpen(false)} title="Close preview">
                                    ✕
                                </button>
                            </div>
                        </div>
                        <div className={cl("lightbox-img-wrapper")}>
                            <img src={theme.thumbnail_url} alt={theme.name} className={cl("lightbox-img")}/>
                        </div>
                    </div>
                </div>)}
        </>);
}
function openThemeDetailsModal(theme, installed, onToggleInstall, onTagClick) {
    openModal(modalProps => (<ThemeDetailsModalContent theme={theme} installed={installed} onToggleInstall={onToggleInstall} modalProps={modalProps} onTagClick={onTagClick}/>));
}
function MarketplaceCard({ theme, installed, onToggleInstall, onOpenDetails, onTagClick }) {
    const [imgErr, setImgErr] = useState(false);
    const authorName = theme.author?.discord_name ?? theme.author?.github_name ?? "Unknown";
    return (<div className={cl("card")} onClick={onOpenDetails} style={{ cursor: "pointer" }}>
            <div className={cl("card-preview")}>
                {theme.thumbnail_url && !imgErr ? (<img className={cl("card-img")} src={theme.thumbnail_url} alt={theme.name} loading="lazy" onError={() => setImgErr(true)}/>) : (<div className={cl("card-img-placeholder")}>No Preview</div>)}
            ...
            <div className={cl("card-body")}>
                ...
                <p className={classes(cl("card-author"), cl("author-clickable"))} onClick={e => {
            e.stopPropagation();
            onTagClick(authorName);
        }} title="Click to search themes by this author">
                    by {authorName}
                </p>
                {theme.description && (<p className={cl("card-desc")}>{theme.description}</p>)}
                {theme.tags?.length > 0 && (<div className={cl("card-tags")}>
                        {theme.tags.slice(0, 5).map(tag => (<button key={tag} className={classes(cl("card-tag"), cl("tag-btn"))} onClick={e => {
                    e.stopPropagation();
                    onTagClick(tag);
                }}>
                                #{tag}
                ...
                <div className={cl("card-actions")}>
                    <Button size="small" variant={installed ? "dangerSecondary" : "primary"} className={cl("action-btn")} onClick={e => {
            e.stopPropagation();
            onToggleInstall();
        }}>
                        {installed ? "Uninstall" : "Install"}
                    </Button>
                    {theme.content && (<Button size="small" variant="secondary" className={cl("action-btn")} onClick={e => {
                e.stopPropagation();
...
export function ThemeMarketplaceSection() {
    ...
    const visible = useMemo(() => {
        ...
        if (sort === "newest")
            sorted.sort((a, b) => new Date(b.release_date ?? 0).getTime() - new Date(a.release_date ?? 0).getTime());
    ...
    function handleToggleInstall(theme) {
        try {
    ...
    return (<>
            ...
            cursor: "pointer",
            userSelect: "none"
        }} className={Margins.top20} onClick={() => {
            ...
            <div className={`vc-marketplace-collapsible ${isHidden ? "collapsed" : "expanded"}`}>
                ...
                <div className={classes(cl("toolbar"), Margins.bottom16)}>
                    <div className={cl("search")}>
                        <TextInput placeholder="Search themes by name, author, or tag..." value={search} onChange={setSearch}/>
                    </div>
                    <div className={cl("sort-tabs")}>
                        {SORTS.map(s => (<button key={s.key} className={classes(cl("sort-tab"), sort === s.key && cl("sort-tab", "active"))} onClick={() => setSort(s.key)}>
                                {s.label}
                            </button>))}
                    </div>
                </div>

                {loading ? (<div className={cl("grid")}>
                        {Array.from({ length: 6 }).map((_, i) => (<div key={i} className={cl("skeleton-card")}/>))}
                    </div>) : error ? (<Paragraph color="text-muted" className={Margins.top16}>
                        Failed to load themes: {error}
                    </Paragraph>) : visible.length === 0 ? (<Paragraph color="text-muted" className={Margins.top16}>No themes found.</Paragraph>) : (<div className={cl("grid")}>
                        {visible.map(theme => (<MarketplaceCard key={theme.id} theme={theme} installed={installedIds.has(theme.id)} onToggleInstall={() => handleToggleInstall(theme)} onOpenDetails={() => openThemeDetailsModal(theme, installedIds.has(theme.id), () => handleToggleInstall(theme), (tag) => setSearch(tag))} onTagClick={(tag) => setSearch(tag)}/>))}
                    </div>)}
            </div>
