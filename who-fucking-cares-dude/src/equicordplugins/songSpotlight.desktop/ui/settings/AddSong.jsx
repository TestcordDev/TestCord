...
function AddSongModal({ modalProps, close, onAdd }) {
    const [url, setURL] = useState("");
    const [error, setError] = useState();
    const [pending, setPending] = useState(false);
    return (<ErrorBoundary>
            <Modal {...modalProps} size="md" title="Add a new song" actions={[
            {
                text: "Add song",
                variant: "primary",
                ...
                <Flex flexDirection="column" gap="10px">
                    ...
                    <TextInput placeholder="https://open.spotify.com/..." error={error} onChange={value => {
            setURL(value);
            try {
                if (value)
                    new URL(value);
...
export default function AddSong({ onAdd }) {
    return (<Clickable onClick={() => {
            const key = openModal(modalProps => (<AddSongModal modalProps={modalProps} close={() => closeModal(key)} onAdd={onAdd}/>));
        }}>
