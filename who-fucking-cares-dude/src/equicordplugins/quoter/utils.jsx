...
import { CANVAS_CONFIG, FONT_SIZES, SPACING } from "./types";
...
export function generateFileNamePreview(message) {
    const words = message.split(" ");
    const preview = words.length > 6 ? words.slice(0, 6).join(" ") : words.join(" ");
    return preview.slice(0, 10);
}
export function getFileExtension(saveAsGif) {
    return saveAsGif ? "gif" : "png";
}
export function getMimeType(saveAsGif) {
...
function applyGrayscaleFilter(ctx, config) {
    ctx.globalCompositeOperation = "saturation";
    ctx.fillStyle = "#fff";
    ctx.fillRect(0, 0, config.width, config.height);
    ctx.globalCompositeOperation = "source-over";
}
function drawGradientOverlay(ctx, config) {
...
function extractCustomEmojis(text) {
    const emojis = [];
    const cleanText = text.replace(CUSTOM_EMOJI_REGEX, (match, name, id) => {
        emojis.push({
            id,
            name,
...
async function loadCustomEmojiImage(emoji) {
...
async function loadCustomEmojiImages(emojis) {
    if (!emojis.length)
        return new Map();
    ...
    emojis.forEach(emoji => {
        ...
        if (!unique.has(key))
            unique.set(key, emoji);
    });
    const entries = await Promise.all(Array.from(unique.entries()).map(async ([key, emoji]) => [key, await loadCustomEmojiImage(emoji)]));
...
function measureTextWithCustomEmojis(ctx, text, fontSize) {
    const emojiSize = fontSize * CUSTOM_EMOJI_SIZE_MULTIPLIER;
    let width = 0;
    let start = 0;
    for (let i = 0; i < text.length; i++) {
        if (text[i] !== CUSTOM_EMOJI_PLACEHOLDER)
            continue;
...
function calculateOptimalFontSize(ctx, quote, font, config) {
...
function drawQuoteText(ctx, calculation, font, config, emojis, emojiImages) {
    ctx.fillStyle = "#fff";
    ...
    calculation.lines.forEach(line => {
        ...
        for (let i = 0; i < line.length; i++) {
            if (line[i] !== CUSTOM_EMOJI_PLACEHOLDER)
                continue;
...
function drawAuthorInfo(ctx, author, calculation, config, startY) {
...
function drawWatermark(ctx, watermark, config) {
    ctx.fillStyle = "#888";
...
export async function createQuoteImage(options) {
