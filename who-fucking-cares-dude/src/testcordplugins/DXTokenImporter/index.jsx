...
import { isEncryptedPayload, TOKEN_REGEX_SOURCE } from "./common";
const cl = classNameFactory("vc-dxtokenimporter-");
const logger = new Logger("DXTokenImporter");
const Native = VencordNative.pluginHelpers.DXTokenImporter;
...
async function migrateLegacyData() {
    ...
    try {
        if ((await DataStore.get(STORE_KEY)) === undefined) {
            const legacy = await DataStore.get(LEGACY_STORE_KEY);
            if (legacy)
                await DataStore.set(STORE_KEY, legacy);
        }
        if ((await DataStore.get(ACK_KEY)) === undefined) {
            const legacyAcks = await DataStore.get(LEGACY_ACK_KEY);
            if (legacyAcks)
                await DataStore.set(ACK_KEY, legacyAcks);
        }
    }
    catch (e) {
...
function routeToLoginManager(accounts) {
    if (!isPluginEnabled("TokenLoginManager"))
        return { ok: false, reason: "TokenLoginManager is disabled" };
    const manager = TokenLoginPlugin.tokenLoginManager;
    if (!manager)
        return { ok: false, reason: "TokenLoginManager not initialized" };
    for (const { username, token } of accounts)
...
async function getAckedDangerous() {
    await migrateLegacyData();
    const list = (await DataStore.get(ACK_KEY)) ?? [];
    return new Set(list);
}
async function markAckedDangerous(key) {
    const list = (await DataStore.get(ACK_KEY)) ?? [];
...
function accountFromUser(u, token) {
    return {
        id: u.id,
        token,
...
async function verifyToken(tok) {
    try {
        ...
        if (r.error === "rate_limited")
            return { status: "rate_limited", rateLimited: true, account: null };
        if (r.error && r.error !== "unauthorized")
...
function normalizeAvatarHash(avatar) {
    if (!avatar)
        return null;
...
async function decryptIfNeeded(tok) {
    if (!isEncryptedPayload(tok))
        return tok;
    try {
...
async function encryptIfEnabled(tok) {
    if (!settings.store.encryptStoredTokens || isEncryptedPayload(tok))
        return tok;
    try {
...
function getAccounts() {
    ...
    if (!loadPromise) {
        loadPromise = migrateLegacyData().then(() => DataStore.get(STORE_KEY)).then(async (raw) => {
            const accounts = [];
            for (const a of raw ?? []) {
...
async function persistAccounts(accounts) {
    ...
    for (const a of deduplicated) {
        ...
        if (settings.store.encryptStoredTokens && !isEncryptedPayload(a.token) && encrypted === a.token)
...
let originalEncryptAndStoreTokens = null;
async function patchTokenStore() {
    ...
    try {
        ...
        if (!tokenMod?.encryptAndStoreTokens)
            return;
        ...
        Object.defineProperty(tokenMod, "encryptAndStoreTokens", {
            value: async function (tokens) {
                try {
                    const saved = await getAccounts();
                    for (const acc of saved) {
                        if (!acc.undecryptable && !tokens[acc.id])
                            tokens[acc.id] = acc.token;
                    }
                }
                catch (e) {
...
function switchToAccount(token, userId) {
    try {
        const isMultiInstance = window.location.href.includes("multi-instance=true")
            || window.IS_MULTI_INSTANCE;
        if (isMultiInstance && userId) {
            const multiInstance = VencordNative?.pluginHelpers?.MultiInstance;
        ...
        if (typeof TokenStore?.setToken === "function")
...
function extractTokens(raw) {
    ...
    let m;
    while ((m = TOKEN_REGEX.exec(raw)) !== null)
        found.add(m[0]);
...
function parseBulkTokens(input) {
    ...
    if (lines.length >= 3) {
        ...
        for (let i = 0; i + 2 < lines.length; i += 3) {
            ...
            if (SNOWFLAKE_REGEX.test(userId) && separator === "" && token && TOKEN_SHAPE.test(token))
                seen.add(token);
        }
        if (seen.size > 0)
...
// `shouldStop` aborted.
async function importLocalTokens(accounts, shouldStop) {
    ...
    for (const tok of tokens) {
        ...
        if (accounts.find(a => a.token === tok))
            continue;
...
async function verifyAllHeadless() {
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        if (account) {
            valid++;
            if (account.username !== acc.username || account.avatar !== acc.avatar || account.discriminator !== acc.discriminator)
                refreshed++;
            accounts[i] = account;
            dirty = true;
        }
...
function FolderIcon({ width = 20, height = 20, style }) {
...
function AccountAvatar({ id, avatar }) {
    return (<img src={avatarUrl(id, avatar)} className={cl("avatar")} alt="" onError={e => {
...
function RemoveInvalidModal({ rootProps, invalidAccounts, onConfirm }) {
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent>
                ...
                <div className={cl("list")} style={{ maxHeight: 120, marginBottom: 12 }}>
                    {invalidAccounts.map(a => (<div key={a.id} className={cl("row", "row-invalid")}>
...
function TokenModal({ rootProps }) {
    ...
    const filteredAccounts = useMemo(() => {
        ...
        if (sortMode === "manual")
            ...
            sorted.sort((a, b) => (b.lastVerifiedAt ?? 0) - (a.lastVerifiedAt ?? 0));
        else
            sorted.sort((a, b) => (a.lastVerifiedAt ?? Infinity) - (b.lastVerifiedAt ?? Infinity));
        return sorted;
    ...
    function bulkExport() {
        ...
        openModal(props => (<PassphraseModal rootProps={props} title="Export selected accounts" confirmField warnText={`Exporting ${selected.length} account${selected.length !== 1 ? "s" : ""} to an encrypted backup file.`} onConfirm={async (passphrase) => {
                ...
                if (!res.ok)
                    return "Encryption failed, nothing was written";
    ...
    function handleDragOver(e, id) {
        ...
        setDropIndicator(prev => {
            if (prev?.id === id && prev?.position === position)
                return prev;
            return { id, position };
        });
    }
    ...
    async function handleDrop(e, targetId) {
        ...
        dragItem.current = null;
        setDropIndicator(null);
        if (!sourceId || sourceId === targetId || !indicator)
            return;
    ...
    async function verifyAll() {
        ...
        try {
            ...
            if (refreshed > 0)
                showToast(`${refreshed} profile${refreshed !== 1 ? "s" : ""} refreshed`, Toasts.Type.MESSAGE);
            const invalidAccs = working.filter(a => ns[a.id] === "invalid");
            if (invalidAccs.length > 0) {
                openModal(props => (<RemoveInvalidModal rootProps={props} invalidAccounts={invalidAccs} onConfirm={async () => {
    ...
    async function scanLocalDiscords() {
        ...
        try {
            ...
            if (result.added > 0 || result.updated > 0) {
                ...
                const parts = [];
                if (result.added > 0)
                    parts.push(`${result.added} new account${result.added !== 1 ? "s" : ""} imported`);
                if (result.updated > 0)
                    parts.push(`${result.updated} token${result.updated !== 1 ? "s" : ""} refreshed`);
    ...
    async function processTokens(raw, destOverride) {
        ...
        const initial = tokens.map(t => ({ token: t, status: "pending" }));
        setResults(initial);
        setChecking(true);
        setDone(false);
        try {
            ...
            if (dest !== "importer" && validForRouting.length) {
                ...
                if (!routed.ok)
                    showToast(`Login Manager import skipped: ${routed.reason}`, Toasts.Type.FAILURE);
                else
                    showToast(`Sent ${validForRouting.length} account${validForRouting.length !== 1 ? "s" : ""} to Login Manager`, Toasts.Type.SUCCESS);
            }
            setDone(true);
        }
        finally {
    ...
    function handleFile(e) {
        ...
        reader.onload = ev => processTokens(ev.target?.result ?? "");
    ...
    function copyMyToken() {
        try {
            ...
            if (token)
                ...
                showToast("No token found", Toasts.Type.FAILURE);
        }
        catch (e) {
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className={cl("content")}>
                ...
                {tab === "saved" && (<>
                        <div className={cl("bar")}>
                            <div className={cl("search")}>
                                <input className={cl("search-input")} placeholder="Search accounts..." value={accountSearch} onChange={e => setAccountSearch(e.target.value)}/>
                            </div>
            ...
            ]} serialize={identity} isSelected={(v) => v === statusFilter} select={(v) => setStatusFilter(v)}/>
                            </div>
                            <div className={cl("filter-select")}>
                                <Select options={[
            ...
            ]} serialize={identity} isSelected={(v) => v === sortMode} select={(v) => setSortMode(v)}/>
                            </div>
                        </div>
                        <div className={cl("actions")}>
                            {settings.store.enableLocalScan && (<Button size={Button.Sizes.MIN} color={Button.Colors.PRIMARY} onClick={scanLocalDiscords} disabled={scanning}>
                ...
                copiedTimer.current = setTimeout(() => {
                    ...
                    if (mountedRef.current)
                        ...
                        {bulkMode && (<div className={cl("bulk-bar")}>
                ...
                : accounts.length === 0 ? <div className={cl("empty")}>No accounts — add tokens via the tab above.</div>
                    : filteredAccounts.length === 0 ? <div className={cl("empty")}>No accounts match your search.</div>
                        : <div className={cl("list")}>
                                ...
                                return (<div key={a.id} className={cl("row", rowClass, isSelected && "row-selected", showDropAbove && "row-drop-above", showDropBelow && "row-drop-below")} draggable={isDraggable} onDragStart={isDraggable ? e => handleDragStart(e, a.id) : undefined} onDragOver={isDraggable ? e => handleDragOver(e, a.id) : undefined} onDragLeave={isDraggable ? handleDragLeave : undefined} onDragEnd={isDraggable ? handleDragEnd : undefined} onDrop={isDraggable ? e => handleDrop(e, a.id) : undefined} onDoubleClick={() => {
                                        if (!bulkMode) {
                                            setBulkMode(true);
                                            ...
                                            toggleSelect(a.id);
                                                    ...
                                                    {bulkMode && (<div className={cl("select-indicator", isSelected && "select-indicator-active")} onClick={e => { e.stopPropagation(); toggleSelect(a.id); }}>
                                        ...
                                        : <div className={cl("avatar-ph")}>{a.username?.[0]?.toUpperCase() ?? "?"}</div>}
                                                    <div className={cl("row-info")}>
                                                        <span className={cl("username")}>
                                                            ...
                                                            {(st === "rate_limited" || st === "error") && (<span className={cl("st", "st-warn")} title={st === "rate_limited" ? "Discord rate limited the check, this is not an invalid token. Verify again later." : "The check request failed."}>!</span>)}
                                        ...
                                        : revealedId === a.id
                                            ...
                                            : <span className={cl("token-hidden", "token-copyable")} onClick={() => copyWithToast(a.token, "Token copied!")} title="Copy token">•••••••••••••••••••••••••</span>}
                                                    </div>
                ...
                {tab === "add" && (<div className={cl("add-body")}>
                        <TextArea className={cl("textarea")} placeholder="Paste your Discord tokens here... (one per line, comma-separated, or 3-line userId/blank/token format)" value={pasteValue} onChange={handlePasteChange}/>
                        ...
                        {results.length > 0 && (<div className={cl("results")}>
                                {done && (<div className={cl("results-summary")}>
                                ...
                                <div className={cl("list")}>
                                    {results.map(r => (<div key={r.token} className={cl("row", r.status === "valid" ? "row-valid" : r.status === "checking" ? null : "row-invalid")}>
                                            {r.status === "valid"
                        ...
                        : <span className={cl("token-hidden", r.status === "checking" ? null : "token-copyable")} onClick={r.status === "checking" ? undefined : () => copyWithToast(r.token, "Token copied!")} title="Copy token">{r.status === "checking" ? "Checking..." : "••••••••••••••••••••••••"}</span>}
                                            </div>
                ...
                {tab === "settings" && (<div className={cl("settings-body")}>
                        <DXTokenImporterSettingsPanel />
                    </div>)}
            </ModalContent>
        </ModalRoot>);
}
const DANGEROUS_SETTING_BLURBS = {
...
function DangerousAckModal({ rootProps, settingKey, onConfirm }) {
    return (<ModalRoot {...rootProps} size="small">
            <ModalHeader separator={false}>
...
async function getLock() {
    const stored = await DataStore.get(LOCK_KEY);
    if (!stored)
        return null;
    ...
    if (typeof stored.salt === "string" && typeof stored.verifier === "string")
        return stored;
    return null;
}
async function setLock(rec) {
    if (!rec) {
...
function UnlockModal({ rootProps }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent>
                <input autoFocus type="password" className={cl("search-input")} style={{ width: "100%" }} placeholder="Passphrase" value={passphrase} onChange={e => setPassphrase(e.target.value)} onKeyDown={e => {
            if (e.key === "Enter")
                submit();
        }}/>
...
async function requestSurface(open) {
    const lock = await getLock();
    ...
    openModal(props => (<UnlockModal rootProps={{
            ...props,
            onClose: () => {
...
function PassphraseModal({ rootProps, title, confirmField = false, warnText, onConfirm }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent>
                ...
                <input autoFocus type="password" className={cl("search-input")} style={{ width: "100%" }} placeholder="Passphrase (8+ characters)" value={passphrase} onChange={e => setPassphrase(e.target.value)}/>
                {confirmField && (<input type="password" className={cl("search-input")} style={{ width: "100%", marginTop: 8 }} placeholder="Confirm passphrase" value={confirm} onChange={e => setConfirm(e.target.value)} onKeyDown={e => {
                if (e.key === "Enter")
                    submit();
            }}/>)}
                {error && <Forms.FormText style={{ color: "var(--text-feedback-critical, var(--text-danger, #f23f43))", marginTop: 8 }}>{error}</Forms.FormText>}
...
function VaultBackupSection() {
    const [busy, setBusy] = useState(false);
    const [hasLock, setHasLockState] = useState(null);
    useEffect(() => {
        let cancelled = false;
        getLock().then(l => {
            if (!cancelled)
                setHasLockState(l !== null);
        });
    ...
    function exportVault() {
        if (busy)
            return;
        openModal(props => (<PassphraseModal rootProps={props} title="Export encrypted backup" confirmField warnText="The backup file contains your tokens (encrypted with this passphrase). Anyone with the file AND the passphrase gets the accounts — store it accordingly." onConfirm={async (passphrase) => {
                const accounts = (await getAccounts()).filter(a => !a.undecryptable);
                if (!accounts.length)
                    return "No exportable accounts";
    ...
    async function importVault() {
        ...
        try {
            ...
            openModal(props => (<PassphraseModal rootProps={props} title="Import encrypted backup" warnText="Accounts are merged by user id: new accounts are added, existing ones get refreshed tokens." onConfirm={async (passphrase) => {
                    const res = await Native.importVault(payload, passphrase);
                    if (!res.ok)
                        return res.error === "bad_passphrase_or_corrupt" ? "Wrong passphrase or corrupted file" : "Import failed";
                    let parsed;
                    try {
                    ...
                    for (const imp of parsed.accounts) {
                        if (!imp || typeof imp.id !== "string" || typeof imp.token !== "string" || !imp.token)
                            continue;
...
function LockSection() {
    ...
    function setPassphrase() {
        openModal(props => (<PassphraseModal rootProps={props} title="Set UI passphrase" confirmField warnText="This locks the DXTokenImporter UI (modal, quick switcher and commands) behind a passphrase. It does not re-encrypt stored tokens." onConfirm={async (passphrase) => {
                const salt = await Native.makeLockSalt();
                const verifier = await Native.deriveLockVerifier(passphrase, salt);
                if (!verifier)
                    return "Could not derive verifier";
    ...
    function changePassphrase() {
        openModal(props => (<PassphraseModal rootProps={props} title="Change UI passphrase" confirmField onConfirm={async (passphrase) => {
                const lock = await getLock();
                if (!lock)
                    return "No lock is configured";
    ...
    function verifyCurrent(passphrase) {
    ...
    function removePassphrase() {
        openModal(props => (<PassphraseModal rootProps={props} title="Remove UI passphrase" warnText="Enter the current passphrase to remove the lock." onConfirm={async (passphrase) => {
                if (!(await verifyCurrent(passphrase)))
...
function EncryptionCheckSection() {
    ...
    async function runCheck() {
        try {
            ...
            const raw = (await DataStore.get(STORE_KEY)) ?? [];
            const next = {
                ...native,
                total: raw.length,
    ...
    async function encryptStoredNow() {
        ...
        try {
            ...
            if (next)
                showToast("Stored tokens re-encrypted", Toasts.Type.SUCCESS);
        }
        finally {
            setBusy(false);
        }
    ...
    return (<>
            ...
            try {
                ...
                {report !== null && settings.store.encryptStoredTokens && report.encrypted < report.total && (<Button size={Button.Sizes.SMALL} color={Button.Colors.GREEN} disabled={busy} onClick={encryptStoredNow}>
                        Encrypt stored tokens now
                    </Button>)}
            </div>
            {report !== null && (<div className={cl("enc-report")}>
...
function DXTokenImporterSettingsPanel() {
    ...
    function setDangerous(key, next) {
        if (!next || acked.has(key)) {
            settings.store[key] = next;
            return;
        }
        openModal(props => (<DangerousAckModal rootProps={props} settingKey={key} onConfirm={async () => {
                await markAckedDangerous(key);
                setAcked(prev => new Set(prev).add(key));
                settings.store[key] = true;
            }}/>));
    }
    async function setEncrypt(next) {
    ...
    return (<>
            ...
            { label: "DXTokenImporter only", value: "importer" },
            { label: "Login Manager only", value: "loginManager" },
        ]} serialize={identity} isSelected={(v) => v === store.importDestination} select={(v) => {
            settings.store.importDestination = v;
        }}/>
            <div style={{ height: 12 }}/>
            <FormSwitch title="Encrypt stored tokens at rest" description="Uses Electron safeStorage (OS keychain / DPAPI) with an AES-256-GCM built-in fallback. Strongly recommended." value={store.encryptStoredTokens} onChange={setEncrypt}/>
            <EncryptionCheckSection />
            <VaultBackupSection />
            <LockSection />
            <FormSwitch title="Alt+G quick switch hotkey" description="Opens the compact account switcher from anywhere. Takes effect after a reload." value={store.enableQuickSwitch} onChange={v => settings.store.enableQuickSwitch = v}/>
            ...
            { label: "Every 12 hours", value: 12 },
            { label: "Every 24 hours", value: 24 },
        ]} serialize={String} isSelected={(v) => v === store.autoVerifyInterval} select={(v) => {
            ...
            <FormSwitch title="Enable local Discord scan (advanced)" description={DANGEROUS_SETTING_BLURBS.enableLocalScan} value={store.enableLocalScan} onChange={v => setDangerous("enableLocalScan", v)}/>
            <FormSwitch title="Auto-scan on startup (advanced)" description={DANGEROUS_SETTING_BLURBS.autoScanOnStartup} value={store.autoScanOnStartup} onChange={v => setDangerous("autoScanOnStartup", v)}/>
            <FormSwitch title="Patch Discord's token store (advanced)" description={DANGEROUS_SETTING_BLURBS.patchTokenStore} value={store.patchTokenStore} onChange={v => setDangerous("patchTokenStore", v)}/>
            <FormSwitch title="Inject into multi-account store (advanced)" description={DANGEROUS_SETTING_BLURBS.injectIntoMultiAccountStore} value={store.injectIntoMultiAccountStore} onChange={v => setDangerous("injectIntoMultiAccountStore", v)}/>
            <FormSwitch title="localStorage bypass during switch (advanced)" description={DANGEROUS_SETTING_BLURBS.useLocalStorageBypass} value={store.useLocalStorageBypass} onChange={v => setDangerous("useLocalStorageBypass", v)}/>
        </>);
}
...
function QuickSwitcherModal({ rootProps }) {
    ...
    function doSwitch(acc) {
        if (acc.undecryptable)
            return;
        rootProps.onClose();
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent className={cl("qs-content")}>
                <input autoFocus className={cl("search-input")} placeholder="Search accounts...  ↑↓ to move, Enter to switch" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={onKeyDown}/>
                <div className={cl("list", "qs-list")}>
                    {results.map((a, i) => (<div key={a.id} className={cl("row", "qs-row", i === selected && "qs-row-selected", a.undecryptable && "row-invalid")} onClick={() => doSwitch(a)} onMouseEnter={() => setSelected(i)}>
                            {a.avatar
                ? <AccountAvatar id={a.id} avatar={a.avatar}/>
                : <div className={cl("avatar-ph")}>{a.username?.[0]?.toUpperCase() ?? "?"}</div>}
                            <div className={cl("row-info")}>
                    ...
                    {results.length === 0 && (<div className={cl("empty")}>{loaded ? "No accounts match." : "Loading accounts..."}</div>)}
                </div>
            </ModalContent>
        </ModalRoot>);
}
...
async function listAccountsCommand(ctx) {
    if (!(await ensureUnlocked(ctx)))
        return;
    const accounts = await getAccounts();
    if (!accounts.length) {
...
async function verifyAllCommand(ctx) {
    if (!(await ensureUnlocked(ctx)))
        return;
...
async function switchCommand(ctx, query) {
    if (!(await ensureUnlocked(ctx)))
        return;
    const accounts = await getAccounts();
...
async function importTokenCommand(ctx, token) {
    if (!(await ensureUnlocked(ctx)))
        return;
...
async function exportVaultCommand(ctx) {
    if (!(await ensureUnlocked(ctx)))
        return;
    const accounts = (await getAccounts()).filter(a => !a.undecryptable);
    if (!accounts.length) {
...
async function removeAccountCommand(ctx, query) {
    if (!(await ensureUnlocked(ctx)))
        return;
    const accounts = await getAccounts();
    const matches = accounts.filter(a => a.username.toLowerCase().includes(query) || a.id === query);
...
export default definePlugin({
    ...
    async start() {
        ...
        try {
            ...
            if (settings.store.enableLocalScan
        ...
        const intervalHours = settings.store.autoVerifyInterval;
        if (intervalHours > 0) {
            ...
            this._autoVerifyTimer = setInterval(async () => {
                ...
                try {
                    ...
                    if (invalid > 0)
                        showToast(`Auto-verify: ${invalid} token${invalid !== 1 ? "s" : ""} invalid`, Toasts.Type.FAILURE);
                }
                catch (e) {
    ...
    async _injectAccounts() {
        ...
        try {
            ...
            if (!FluxDispatcher?.dispatch)
                return;
            const existing = new Set((findByProps("getAccounts")?.getAccounts?.() ?? []).map((u) => u.id));
