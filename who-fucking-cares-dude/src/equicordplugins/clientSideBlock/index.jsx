...
let usersToBlockSet = null;
let lastUsersToBlock = "";
let guildBlackListSet = null;
let lastBlackList = "";
let guildWhiteListSet = null;
let lastWhiteList = "";
function getCachedSet(value, cache) {
    if (cache.set && cache.last === value)
        return cache.set;
    cache.last = value;
...
function isChannelInGuildBlocked(channelID, guild) {
    ...
    if (blacklist.has(guildID))
        return true;
    if (whitelist.size && !whitelist.has(guildID))
        return true;
    return false;
}
function shouldHideUser(userId, channelId) {
    if (channelId && isChannelInGuildBlocked(channelId, false))
        return true;
...
function isRoleAllBlockedMembers(roleId, guildId) {
    ...
    if (!role)
        return false;
    const membersWithRole = GuildMemberStore.getMembers(guildId).filter(member => member.roles.includes(roleId));
    if (!membersWithRole.length)
        return false;
    if (isChannelInGuildBlocked(guildId, true))
        return true;
...
export default definePlugin({
    ...
    tags: ["Utility", "Appearance", "Privacy"],
