...
import { callAI, fetchCordCatData } from "./aiManager";
import { analyzeMessages } from "./algorithms";
...
async function loadHistory() {
    return (await DataStore.get(DS_HISTORY_KEY)) ?? [];
}
...
async function removeFromHistory(userId) {
    const history = await loadHistory();
    await DataStore.set(DS_HISTORY_KEY, history.filter(h => h.userId !== userId));
}
function getMutualGuildsForUser(userId) {
    const guilds = [];
...
function getDMChannelsWithUser(userId) {
    const dmChannelIds = [];
...
function toMessageData(msg) {
    ...
    return {
        ...
        author: {
            ...
            accentColor: msg.author.accentColor,
            publicFlags: msg.author.publicFlags,
            bot: msg.author.bot,
        },
...
async function searchMessages(userId, guildId, channelId, offset) {
    const query = {
        author_id: userId,
        sort_by: "timestamp",
        sort_order: "desc",
    ...
    const body = res?.body;
    const flat = (body?.messages ?? []).flat();
    return { messages: flat, total: body?.total_results ?? 0 };
}
async function buildAIPrompt(messages, algorithmResult) {
...
function ResultSections({ algorithmResult, aiResult }) {
    return (<>
            {algorithmResult?.sections.map((section, i) => (<div key={i} className="vc-osint-section">
                    <div className="vc-osint-section-title">{section.title}</div>
                    <div className="vc-osint-section-content">{section.content}</div>
                </div>))}
            {aiResult && (<div className="vc-osint-section">
...
// ── Links Modal ──
function LinksModal({ modalProps, messages }) {
    ...
    for (const msg of messages) {
        ...
        for (const url of matches) {
            ...
            if (existing)
                ...
                links.set(cleaned, { count: 1, firstTimestamp: msg.timestamp });
        }
    }
    ...
    return (<ModalRoot {...modalProps} size="large" className="vc-osint-root">
            <ModalHeader>
            ...
            <ModalContent>
                <div className="vc-osint-body">
                    {sorted.length === 0 && (<div className="vc-osint-empty">
                    ...
                    {sorted.map(({ url, count, firstTimestamp }) => (<div key={url} className="vc-osint-link-entry">
...
// ── Attachments Modal ──
function AttachmentsModal({ modalProps, messages }) {
    const attachments = messages.flatMap(m => m.attachments.map(a => ({
        ...a,
    ...
    function getAttachmentType(a) {
        ...
        if (a.content_type?.startsWith("image/") || imageExts.has(ext))
            return "image";
        if (a.content_type?.startsWith("video/") || videoExts.has(ext))
            return "video";
        if (a.content_type?.startsWith("audio/") || audioExts.has(ext))
            return "audio";
        return "other";
    }
    ...
    return (<ModalRoot {...modalProps} size="large" className="vc-osint-root">
            ...
            <ModalContent>
                <div className="vc-osint-body">
                    {attachments.length === 0 && (<div className="vc-osint-empty">
                    ...
                    {images.length > 0 && (<div className="vc-osint-section">
                            <div className="vc-osint-section-title">Images ({images.length})</div>
                            <div className="vc-osint-attachment-grid">
                                {images.map((a, i) => (<a key={i} href={a.url} target="_blank" rel="noopener noreferrer" className="vc-osint-attachment-thumb">
                    ...
                    {videos.length > 0 && (<div className="vc-osint-section">
                            <div className="vc-osint-section-title">Videos ({videos.length})</div>
                            {videos.map((a, i) => (<div key={i} className="vc-osint-link-entry">
                    ...
                    {audio.length > 0 && (<div className="vc-osint-section">
                            <div className="vc-osint-section-title">Audio ({audio.length})</div>
                            {audio.map((a, i) => (<div key={i} className="vc-osint-link-entry">
                                    <div className="vc-osint-link-url" title={a.filename}>{a.filename}</div>
                                    <div className="vc-osint-link-meta"><span>{formatSize(a.size)}</span></div>
                                </div>))}
                        </div>)}

                    {other.length > 0 && (<div className="vc-osint-section">
                            <div className="vc-osint-section-title">Other Files ({other.length})</div>
                            {other.map((a, i) => (<div key={i} className="vc-osint-link-entry">
                                    <div className="vc-osint-link-url" title={a.filename}>{a.filename}</div>
                                    <div className="vc-osint-link-meta"><span>{formatSize(a.size)}</span></div>
                                </div>))}
                        </div>)}
...
// ── Live Scan Panel ──
function OSINTScanPanel({ userId, channelId, modalProps }) {
    ...
    const [cordcatResult, setCordcatResult] = useState(null);
    const [cordcatLoading, setCordcatLoading] = useState(false);
    const fetchStateRef = useRef({ running: false, aborted: false, total: 0 });
    const messagesRef = useRef([]);
    const lastMessagesUpdateRef = useRef(0);
    const user = UserStore.getUser(userId);
    const runAnalysis = useCallback((msgs) => {
        if (msgs.length === 0)
            return;
    ...
    useEffect(() => {
        ...
        async function searchGuild(guildId, guildName, chanId) {
            let offset = 0;
            ...
            if (scanMutual) {
                ...
                for (const guild of guilds) {
                    if (cancelled || !state.running)
                        break;
                    if (!canAccessChannel(channelId) && guild.id !== ChannelStore.getChannel(channelId)?.guild_id)
                        continue;
                ...
                if (!canAccessChannel(channelId)) {
                    setError("No access to this channel");
                    setPhase("error");
                    return;
                }
            ...
            if (!cancelled) {
                ...
                if (settings.store.useAI && acc.length > 0) {
                    try {
                        ...
                        setAiResult(`AI analysis failed: ${e.message}`);
                    }
                }
    ...
    return (<ModalRoot {...modalProps} size="large" className="vc-osint-root">
            ...
            <ModalContent>
                {phase === "error" && (<div className="vc-osint-empty">
                ...
                {(phase === "fetching" || phase === "analyzing") && allMessages.length === 0 && (<div className="vc-osint-progress">
                        <div className="vc-osint-spinner"/>
                        <div>{progress}</div>
                        {unlimited && <button className="vc-osint-stop-btn" onClick={handleStop}>Stop & Analyze</button>}
                    </div>)}
                {allMessages.length > 0 && (<div className="vc-osint-body">
                        <div className="vc-osint-user-info">
                            ...
                            <div className="vc-osint-user-details">
                                ...
                                {scanMutual && mutualGuilds.length > 0 && (<div className="vc-osint-guild-list">
                                        {mutualGuilds.map(g => (<span key={g.id} className="vc-osint-guild-chip">
                    ...
                    {phase === "fetching" && (<div className="vc-osint-live-bar">
                        ...
                        {/* CordCat Section */}
                        {(cordcatLoading || cordcatResult) && (<div className={`vc-osint-section ${cordcatLoading ? "vc-osint-section--analyzing" : ""}`}>
                                <div className="vc-osint-section-title">
                                    {cordcatLoading && <div className="vc-osint-spinner-small"/>}
                                    CordCat Intelligence
                                </div>
                                {!cordcatLoading && cordcatResult && (<div className="vc-osint-section-content">
                                        ...
                                        {cordcatResult.actions.length > 0 && (<div style={{ marginBottom: 12 }}>
                                                <div style={{ fontWeight: 600, marginBottom: 6 }}>DSA Actions ({cordcatResult.actions.length})</div>
                                                {cordcatResult.actions.map((a, i) => (<div key={i} className="vc-osint-cordcat-action">
                                        ...
                                        {cordcatResult.breaches.length > 0 && (<div>
                                                <div style={{ fontWeight: 600, marginBottom: 6 }}>Breach Records ({cordcatResult.breaches.length})</div>
                                                {cordcatResult.breaches.map((b, i) => (<div key={i} className="vc-osint-cordcat-action">
                                                        <div className="vc-osint-cordcat-action-header">
                        ...
                        {phase === "analyzing" && (<div className="vc-osint-section vc-osint-section--analyzing">
                                <div className="vc-osint-section-title">
                                    <div className="vc-osint-spinner-small"/>
            ...
            {(phase === "done" || phase === "error") && (<ModalFooter>
                    {phase === "done" && (<>
...
// ── History Panel ──
function OSINTHistoryPanel({ modalProps, onSelect }) {
    ...
    return (<ModalRoot {...modalProps} size="medium" className="vc-osint-root">
            <ModalHeader>
                <div className="vc-osint-header-left">
                    <div className="vc-osint-header-icon">
            ...
            <ModalContent>
                <div className="vc-osint-body">
                    ...
                    {!loading && history.length === 0 && (<div className="vc-osint-empty">
                    ...
                    {history.map(entry => (<div key={entry.userId} className="vc-osint-history-entry" onClick={() => { modalProps.onClose(); onSelect(entry.userId); }}>
...
function OSINTMultiScan({ modalProps }) {
            ...
            <ModalContent>
                <div className="vc-osint-body">
                    ...
                    <div className="vc-osint-multi-input">
                        <input className="vc-osint-input" value={userInput} onChange={e => setUserInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter")
        addTarget(); }} placeholder="Enter user ID or @mention..." disabled={scanning}/>
                        <Button size={Button.Sizes.SMALL} onClick={addTarget} disabled={scanning || !userInput.trim()}>Add</Button>
                    </div>

                    {/* Target list */}
                    {targets.map((t, i) => (<div key={t.userId} className={`vc-osint-target ${t.status === "scanning" ? "vc-osint-target--active" : ""} ${t.status === "done" ? "vc-osint-target--done" : ""}`}>
                    ...
                    {/* Results */}
                    {targets.filter(t => t.status === "done").map(t => (<div key={t.userId} className="vc-osint-section" style={{ marginTop: 16 }}>
                            <div className="vc-osint-section-title">@{t.username} — {t.msgCount} messages</div>
                            {t.result?.sections.slice(0, 3).map((s, i) => (<div key={i} className="vc-osint-section-content" style={{ marginBottom: 8 }}>{s.title}: {s.content.split("\n")[0]}</div>))}
...
// ── User Picker Modal ──
function UserPickerModal({ modalProps, onSelect }) {
    ...
    useEffect(() => {
        ...
        for (const id of friendIds.slice(0, 100)) {
            ...
            if (user && !seen.has(id)) {
                seen.add(id);
                users.push({ id, username: user.username, globalName: user.globalName, avatar: user.avatar });
            }
        }
        for (const guild of Object.values(GuildStore.getGuilds())) {
            for (const member of GuildMemberStore.getMembers(guild.id).slice(0, 50)) {
                ...
                if (user) {
                    ...
                    users.push({ id: member.userId, username: user.username, globalName: user.globalName, avatar: user.avatar });
                }
            }
            if (users.length > 300)
                break;
    ...
    function select(userId) { modalProps.onClose(); onSelect(userId); }
    return (<ModalRoot {...modalProps} size="small" className="vc-osint-root">
            <ModalHeader>
                <div className="vc-osint-header-left">
                    <div className="vc-osint-header-icon">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
            ...
            <ModalContent>
                <div className="vc-osint-body">
                    <TextInput autoFocus placeholder="Username, display name, or user ID..." value={query} onChange={setQuery} onKeyDown={(e) => {
            if (e.key === "Enter") {
                ...
                if (filtered.length > 0)
                    ...
                    <div className="vc-osint-picker-list">
                        {filtered.map(user => (<div key={user.id} className="vc-osint-picker-item" onClick={() => select(user.id)}>
...
function OSINTButton() {
    ...
    const SearchIcon = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>);
    function renderPopout(onClose) {
        return (<Menu.Menu navId="vc-osint-menu" onClose={onClose}>
                ...
                <Menu.MenuSeparator />
                <Menu.MenuItem id="vc-osint-settings" label="Settings" action={() => { onClose(); }}/>
            </Menu.Menu>);
    }
    return (<span ref={buttonRef} style={{ display: "inline-flex", alignItems: "center" }}>
            <Popout position="bottom" align="center" spacing={0} animation={Popout.Animation.NONE} shouldShow={show} onRequestClose={() => setShow(false)} targetElementRef={buttonRef} renderPopout={() => renderPopout(() => setShow(false))}>
                {(_, { isShown }) => (<HeaderBarButton icon={SearchIcon} tooltip={isShown ? null : "OSINT Tools"} selected={isShown} onClick={() => setShow(v => !v)}/>)}
            </Popout>
        </span>);
}
// ── Plugin ──
    ...
    commands: [
        {
            ...
            execute: async (args, ctx) => {
                ...
                const limitOverride = findOption(args, "limit", 0);
                const mutualOverride = findOption(args, "mutual-servers", false);
                if (!userId) {
                    sendBotMessage(ctx.channel.id, { content: "Please specify a user to scan." });
                    return;
                }
                if (limitOverride < 0)
                    settings.store.unlimitedMessages = true;
                else if (limitOverride > 0) {
                    settings.store.unlimitedMessages = false;
                    settings.store.messageLimit = limitOverride;
                }
                if (mutualOverride)
                    settings.store.scanMutualServers = true;
                openScan(userId, ctx.channel.id);
            },
        },
    ],
    contextMenus: {
        "user-context"(children, { id }) {
            if (!id)
                return;
            const OSINTIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="var(--interactive-normal)"><path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>);
            children.push(<Menu.MenuSeparator />, <Menu.MenuItem id="vc-osint-scan" label="OSINT Scan" icon={OSINTIcon} action={() => openScan(id, SelectedChannelStore.getChannelId())}/>);
        },
    },
    toolboxActions: {
        "OSINT Scan"() {
