...
export function presentOAuth2Modal() {
    openModal(props => (<OAuth2AuthorizeModal {...props} clientId={apiConstants.oauth2.clientId} scopes={["applications.commands", "identify"]} integrationType={1 /* ApplicationIntegrationType.USER_INSTALL */} permissions={0n} responseType="code" redirectUri={apiConstants.oauth2.redirectURL} cancelCompletesFlow={false} callback={async ({ location }) => {
            if (!location)
                return;
            try {
                ...
                if (typeof error === "string")
                    showToast(error, Toasts.Type.FAILURE);
            }
        }}/>));
}
