...
const getMembers = (guildId) => {
    try {
        return Object.values(GuildMemberStore.getMembers(guildId) || {});
    }
    catch { }
    return [];
};
async function startBomb(guildId, roleId, message) {
    if (state.running)
        return;
...
function BombIcon(props) {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24" fill="currentColor" {...props}>
...
function DMBombModal({ rootProps, guildId }) {
    ...
    return (<ModalRoot {...rootProps} className="dmb-modal">
            ...
            <ModalContent className="dmb-content">
                ...
                ...roles.map((r) => ({ label: `@${r.name} [${countByRole[r.id] || 0}]`, value: r.id }))
            ]} select={setRoleId} serialize={(v) => v} isSelected={(v) => v === roleId}/>
                        </div>

                        <p className="dmb-label">Message:</p>
                        <textarea className="dmb-textarea" placeholder="Type your message here..." value={msg} onChange={e => setMsg(e.currentTarget.value)} rows={5}/>
                        <p className="dmb-warn">
                            ⚠️ Intensive botting can get your account banned. Delay:{" "}
                            {editingDelay ? (<input className="dmb-delay-input" type="number" step="0.1" min="0.5" max="60" value={delayInput} onChange={e => setDelayInput(e.currentTarget.value)} onBlur={() => {
                    ...
                    if (e.key === "Escape") {
                        setDelayInput(String(state.delayMs / 1000));
                        setEditingDelay(false);
                    }
                }} autoFocus/>) : (<span className="dmb-delay-value" onClick={() => { setDelayInput(String(s.delayMs / 1000)); setEditingDelay(true); }} title="Click to modify delay">
                                    {s.delayMs / 1000}s
                                </span>)}
                        </p>
                    </>)}
                {(s.running || s.finished) && (<>
                        ...
                        {s.finished && (<p className="dmb-done">✅ Finished with {s.done} DMs sent.</p>)}
...
export default definePlugin({
    ...
    patchGuildContext(children, { guild }) {
        if (!children || !Array.isArray(children))
            return;
        try {
            if (!guild)
                return;
            const bombsItem = (<Menu.MenuItem id="dmbomb-btn" key="dmbomb-btn" label="DM Bomb" action={() => openModal(props => <DMBombModal rootProps={props} guildId={guild.id}/>)}/>);
