...
function HighlightedCode({ code }) {
    ...
    const highlightLine = React.useCallback((line) => {
        ...
        while ((match = tokenRegex.exec(line)) !== null) {
            ...
            if (comment)
                className = "vc-code-token-comment";
            else if (stringLit)
                className = "vc-code-token-string";
            else if (atRule)
                className = "vc-code-token-atrule";
            else if (selector)
                className = "vc-code-token-selector";
            else if (prop)
                className = "vc-code-token-property";
            else if (color)
                className = "vc-code-token-color";
            else if (numberUnit)
                className = "vc-code-token-number";
            else if (punct)
                className = "vc-code-token-punct";
            elements.push(<span key={match.index} className={className}>
                    {full}
                </span>);
    ...
    return (<pre className="vc-code-highlight-pre">
            {lines.map((line, idx) => (<div key={idx} className="vc-code-line">
                    {highlightLine(line)}
                </div>))}
        </pre>);
}
export function CodeViewerModal({ modalProps, title, code, editable, onSave, onSaveAsLocal }) {
    ...
    return (<Modal {...modalProps} size="lg" title={title}>
            <div className={cl("body")}>
                {!editable && (<Paragraph className={cl("hint")}>
                ...
                <div className="vc-code-viewer-editor-wrapper theme-dark wrap">
                    ...
                    <div className="vc-code-viewer-gutter" ref={gutterRef}>
                        {Array.from({ length: lineCount }).map((_, i) => (<div key={i + 1} className="vc-code-gutter-line">
                                {i + 1}
                            </div>))}
                    </div>
                    ...
                    <div className="vc-code-viewer-code-area">
                        ...
                        <textarea ref={textareaRef} className={cl("textarea")} value={value} onChange={editable ? e => setValue(e.currentTarget.value) : undefined} onScroll={handleScroll} onKeyDown={handleKeyDown} readOnly={!editable} spellCheck={false} wrap="soft"/>
                    </div>
                </div>
                ...
                <div className={cl("actions")}>
                    ...
                    {onSaveAsLocal && (<Button variant="secondary" onClick={handleSaveAsLocal} disabled={saving}>
                            Save as Local Theme
                        </Button>)}
                    {editable && onSave && (<Button variant="primary" onClick={handleSave} disabled={saving || !dirty}>
