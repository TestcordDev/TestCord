...
export function OnlineThemeCard({ disabled, isNew, name, infoButton, footer, author, enabled, setEnabled, description, onMouseEnter, onMouseLeave, customName, onEditName, }) {
    const titleRef = useRef(null);
    const titleContainerRef = useRef(null);
    const { truncated, containerRef } = useTruncatedText(description ? description.toString() : "");
    return (<div className={cl("card", { "card-disabled": disabled })} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
                ...
                <div className={cl("name-author")}>
                    <BaseText size="md" weight="bold" className={cl("name")}>
                        <div ref={titleContainerRef} className={cl("title-container")}>
                            <div ref={titleRef} className={cl("title")} onMouseOver={() => {
            ...
            title.style.setProperty("--duration", `${Math.max(0.5, (title.scrollWidth - titleContainer.clientWidth) / 7)}s`);
                                ...
                                {onEditName ? (<EditableText value={customName || (name ? name.toString() : "")} onChange={onEditName} className={cl("editable")}/>) : (customName || name)}
                            </div>
                        </div>
                        {isNew && <Badge text="NEW" variant="danger"/>}
                    </BaseText>

                    {!!author && (<BaseText size="md" color="text-subtle" className={cl("author")}>
                            {author}
                        </BaseText>)}
                </div>
            ...
            <div ref={containerRef} className={cl("note")} style={{ lineHeight: "1.25em", fontSize: "small" }} title={description ? description.toString() : ""}>
                {truncated}
            </div>
