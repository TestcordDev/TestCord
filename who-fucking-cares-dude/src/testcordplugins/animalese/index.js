...
const BASE_URL_HIGH = "https://raw.githubusercontent.com/ryawaa/vencord-animalese/main/sounds/high";
async function initSoundBuffers() {
    if (!audioContext)
        audioContext = new AudioContext();
    for (const file of highSounds) {
        const nameWithoutExt = file.replace(".wav", "");
        soundBuffers[nameWithoutExt] = await loadSound(`${BASE_URL_HIGH}/${file}`);
    }
}
...
async function generateAnimalese(text) {
    if (!audioContext)
        audioContext = new AudioContext();
    const soundIndices = [];
    ...
    const outputBuffer = audioContext.createBuffer(1, audioContext.sampleRate * totalDuration, audioContext.sampleRate);
...
async function playSound(buffer, volume) {
    if (!audioContext)
        audioContext = new AudioContext();
...
export default definePlugin({
    ...
    stop() {
        ...
        FluxDispatcher.unsubscribe("CHANNEL_SELECT", this.channelSelectListener);
        FluxDispatcher.unsubscribe("MESSAGE_CREATE", this.messageCreateListener);
        if (audioContext) {
