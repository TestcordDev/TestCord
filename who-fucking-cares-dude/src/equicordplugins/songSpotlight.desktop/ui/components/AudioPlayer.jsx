...
function AudioItem({ audio, index, handleRef, handleLoaded, handleStopped }) {
    const refCallback = useCallback((node) => handleRef(index, node), [index, handleRef]);
    ...
    return <audio src={audio.previewUrl} preload="metadata" ref={refCallback} onLoadedData={loadedCallback} onError={erroredCallback} onPause={pausedCallback} onEnded={endedCallback}/>;
}
const BASE_VOLUME = 0.35;
// only allows one song to play at a time
let globalPlaying = undefined;
export default function AudioPlayer({ audioRef, list, playing, setPlaying, setLoadedAudio }) {
    const audios = useMemo(() => list.map(x => x.audio), [list]);
    const nodes = useRef(new Map());
    const loaded = useRef(new Set());
    const nodeEvents = useRef(new Map());
    const handleRef = useCallback((index, node) => {
        if (node) {
            nodes.current.set(index, node);
            const audio = audios[index];
            if (audio?.previewStart === undefined || !audio.previewSlice)
                return;
    ...
    const handleLoaded = useCallback((index, state) => {
        if (state)
    ...
    const handleStopped = useCallback((index, ended) => {
        if (ended) {
    ...
    return (<div style={{ display: "none" }} aria-hidden="true">
            {audios.map((audio, index) => audio && (<AudioItem key={audio.previewUrl} audio={audio} index={index} handleRef={handleRef} handleLoaded={handleLoaded} handleStopped={handleStopped}/>))}
        </div>);
}
