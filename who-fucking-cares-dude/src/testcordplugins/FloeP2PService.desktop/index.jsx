...
const Native = VencordNative?.pluginHelpers?.FloeP2PService;
...
async function runNativeAction(action, fallbackError) {
    const native = Native;
    if (!native) {
...
function formatNativeActionLog(message, fallbackError, result) {
...
function getShareText(session) {
    if (session.link && session.code)
        return `Floe P2P transfer: ${session.link}\nCode: ${session.code}`;
    if (session.link)
        return `Floe P2P transfer: ${session.link}`;
    if (session.code)
        return `Floe P2P code: ${session.code}`;
    return null;
}
function publishSendSession(session) {
...
async function cancelLatestSession() {
...
function formatCommandResult(title, result) {
    ...
    if (result.success)
        return [`**${title}**`, output || "Done."].join("\n");
    return [`**${title}**`, result.error ?? "Command failed.", output ? `\`\`\`\n${output.slice(-1500)}\n\`\`\`` : ""].filter(Boolean).join("\n");
}
async function sendReceiveStartedMessage(ctx, locator) {
...
function getSubOption(subCommand, name) {
    return findOption(subCommand.options, name, "");
}
function ReceiveModal({ modalProps }) {
    ...
    return (<Modal {...modalProps} title="Floe Receive" actions={[
            {
                text: "Cancel",
                variant: "secondary",
            ...
            <section className={Margins.bottom16}>
                ...
                <TextInput value={locator} onChange={setLocator} placeholder="olive-tiger-castle or https://floe.one?room=..."/>
            </section>
...
const channelAttachMenuPatch = children => {
    if (children.some(child => child?.props?.id === "floe-p2p-service"))
        return;
    children.splice(1, 0, <Menu.MenuItem id="floe-p2p-service" key="floe-p2p-service" label={(<Flex alignItems="center" gap="8px">
                    <CloudIcon height={18} width={18}/>
                    <span>Floe P2P Service</span>
                </Flex>)}>
            <Menu.MenuItem id="floe-p2p-send-files" key="floe-p2p-send-files" label="Send files" icon={CloudUploadIcon} action={() => void sendFiles()}/>
            <Menu.MenuItem id="floe-p2p-send-folder" key="floe-p2p-send-folder" label="Send folder" icon={FolderIcon} action={() => void sendFolder()}/>
            <Menu.MenuItem id="floe-p2p-receive" key="floe-p2p-receive" label="Receive files" icon={CloudDownloadIcon} action={openReceiveModal}/>
            <Menu.MenuSeparator />
            <Menu.MenuItem id="floe-p2p-cancel" key="floe-p2p-cancel" label="Cancel current session" color="danger" action={() => void cancelLatestSession()}/>
            <Menu.MenuSeparator />
            <Menu.MenuItem id="floe-p2p-install" key="floe-p2p-install" label="Install Floe CLI" icon={CloudDownloadIcon} action={() => void installFloe()}/>
            <Menu.MenuItem id="floe-p2p-update" key="floe-p2p-update" label="Update Floe CLI" icon={UpdaterIcon} action={() => void updateFloe()}/>
        </Menu.MenuItem>);
};
