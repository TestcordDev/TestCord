...
// 15 minutes
const coverArtCache = new TTLMap(15 * 60 * 1000);
const metadataCache = new TTLMap(15 * 60 * 1000);
const isCustomInstance = () => settings.store.scrobblerBackend === "listenbrainz-compatible";
const url = (path) => `${isCustomInstance() ? settings.store.instanceBaseURL : "https://listenbrainz.org"}${path}`;
const apiUrl = (path) => `${isCustomInstance() ? settings.store.instanceAPIBaseUrl : "https://api.listenbrainz.org"}${path}`;
...
function fallbackToYoutubeThumbnail(originUrl) {
    if (!originUrl)
        return undefined;
    const match = originUrl.match(YoutubeVideoURLRegex);
    return match ? `https://i.ytimg.com/vi/${match[5]}/maxresdefault.jpg` : undefined;
}
async function fetchCoverArt(releaseGroupMBID, originUrl) {
    if (!releaseGroupMBID)
        return fallbackToYoutubeThumbnail(originUrl);
...
async function getUrls(additionalInfo, trackName, artistName, releaseName) {
...
export const ListenBrainzScrobbler = {
    name: "ListenBrainz",
    id: "listenbrainz",
