...
function getGifByTarget(url, target) {
    const liElement = target?.closest("li");
    if (!target || !liElement?.id)
        return null;
    const [channelId, messageId] = liElement.id.split("-").slice(2);
    if (!channelId || !messageId || !isValidSnowflake(channelId) || !isValidSnowflake(messageId))
        return null;
    const message = MessageStore.getMessage(channelId, messageId);
    if (!message || (!message.embeds.length && !message.attachments.length))
        return null;
    return getGifByMessageAndUrl(url, message);
}
function getGifByMessageAndTarget(target, message) {
...
function getGifByMessageAndUrl(url, message) {
...
export function getGif(message, url, target) {
    if (message && url)
        return getGifByMessageAndUrl(url, message);
    if (message && target && !url)
        return getGifByMessageAndTarget(target, message);
