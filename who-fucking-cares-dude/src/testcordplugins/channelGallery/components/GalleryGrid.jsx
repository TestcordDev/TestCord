...
const MIN_THUMB = 120;
const MAX_THUMB = 150;
function withSizeParams(url, size) {
    try {
        const u = new URL(url);
...
function getThumbUrl(item, size) {
...
export function GalleryGrid(props) {
    ...
    return (<div ref={scrollRef} className="vc-channel-gallery-scroll" onScroll={e => {
            const el = e.currentTarget;
            setViewport(v => {
                if (v.scrollTop === el.scrollTop)
                    return v;
            ...
            return (<button key={item.key} onClick={() => onSelect(idx)} style={{
                    position: "absolute",
                    left: col * (cell + GAP),
                    top: row * rowHeight,
                    width: cell,
                    ...
                    background: "var(--background-secondary)"
                                ...
                                <img src={getThumbUrl(item, thumbSize)} alt={item.filename ?? "Image"} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}/>
                            </div>
                            {showCaptions && item.filename && (<div title={item.filename} style={{
                        marginTop: 6,
                        fontSize: 12,
            ...
            <div style={{ padding: "10px 0 16px", textAlign: "center" }}>
                {error ? (<div style={{ color: "var(--text-danger)" }}>
                        {error}{" "}
                        <Button size={Button.Sizes.SMALL} onClick={onRetry}>
                            Retry
                        </Button>
                    </div>) : isLoading ? (<div style={{ color: "var(--text-muted)" }}>Loading…</div>) : !items.length ? (<div style={{ color: "var(--text-muted)" }}>No images found yet</div>) : !hasMore ? (<div style={{ color: "var(--text-muted)" }}>End of history</div>) : null}
            </div>
        </div>);
}
