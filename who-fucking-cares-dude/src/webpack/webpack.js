...
export let wreq;
export let cache;
export const fluxStores = {};
let cachedLibdiscoreExports = null;
let libdiscoreInitAttempted = false;
function initLibdiscoreCache() {
    if (libdiscoreInitAttempted)
        return;
...
export const stringMatches = (s, filter) => {
...
export function makeClassNameRegex(className) {
...
export function addModuleListener(cb) {
    moduleListeners.add(cb);
    return () => moduleListeners.delete(cb);
}
export function addFactoryListener(cb) {
...
export function _blacklistBadModules(requireCache, exports, moduleId) {
    try {
        ...
        logger.error("Error while blacklisting module:\n", err, "\n\nModule id:", moduleId, "\n\nModule exports:", exports);
    }
    ...
    for (const exportKey in exports) {
        try {
            ...
            logger.error("Error while blacklistng module:\n", err, "\n\nModule id:", moduleId, "\n\nExport value:", exportValue);
        }
    }
 ...
 */
export const find = traceFunction("find", function find(filter, { isIndirect = false, isWaitFor = false, topLevelOnly = false } = {}) {
    if (IS_ANTI_CRASH_TEST)
        return null;
...
export function findAll(filter, { topLevelOnly = false } = {}) {
    if (typeof filter !== "function")
        throw new Error("Invalid filter. Expected a function got " + typeof filter);
    const ret = [];
    for (const key in cache) {
 ...
 */
export function findLazy(filter, warning = true) {
    if (IS_REPORTER && warning)
...
export function mapMangledCssClasses(mappedModule, classes) {
    const values = Object.values(mappedModule);
    const mapped = {};
    for (const cls of classes) {
        const re = makeClassNameRegex(cls);
        mapped[cls] = values.find(v => typeof v === "string" && re.test(v));
...
export function findCssClasses(...classes) {
...
export function findCssClassesLazy(...classes) {
    if (IS_REPORTER)
        lazyWebpackSearchHistory.push(["findCssClasses", classes]);
    return proxyLazy(() => findCssClasses(...classes));
}
function getAllPropertyNames(object, includeNonEnumerable) {
    const names = new Set();
 ...
 *          })
 */
export const mapMangledModule = traceFunction("mapMangledModule", function mapMangledModule(code, mappers, includeBlacklistedExports = false) {
    const exports = {};
 ...
 */
export function mapMangledModuleLazy(code, mappers, includeBlacklistedExports = false) {
    if (IS_REPORTER)
...
export function waitFor(filter, callback, { isIndirect = false } = {}) {
    ...
    if (IS_ANTI_CRASH_TEST && filter !== "useState")
        return;
 ...
 */
export function extract(moduleId) {
