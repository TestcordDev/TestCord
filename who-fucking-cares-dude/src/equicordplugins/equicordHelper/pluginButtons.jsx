...
export const PluginButtons = ErrorBoundary.wrap(function PluginCards({ message }) {
    const pluginButtons = [];
    ...
    if (shouldAddPluginButtons) {
        if (pluginData.required || pluginData.name.endsWith("API"))
            return;
        ...
        pluginButtons.push(<Button key="vc-plugin-toggle" color={disabled ? Button.Colors.PRIMARY : (isEnabled ? Button.Colors.RED : Button.Colors.GREEN)} disabled={disabled} size={Button.Sizes.SMALL} onClick={async () => {
                try {
    ...
    if (pluginButtons.length === 0)
        return null;
    return (<div className="vc-plugins-action-buttons" style={{ marginTop: "0px" }}>
            {pluginButtons}
        </div>);
}, { noop: true });
