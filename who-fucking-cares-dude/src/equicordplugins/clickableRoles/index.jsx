...
function getCachedMemberIds(guildId, roleId) {
...
function setCachedMemberIds(guildId, roleId, ids) {
    memberCache.set(`${guildId}-${roleId}`, { ids, timestamp: Date.now() });
}
function RoleMembersList({ roleId, guildId, closePopout, setPopoutRef }) {
    ...
    useEffect(() => {
        ...
        if (cached) {
            ...
            if (visible.length)
                GuildActions.requestMembersById(guildId, visible, false);
            setLoading(false);
        }
        else {
            RestAPI.get({
    ...
    const users = useStateFromStores([UserStore], () => memberIds.map(id => UserStore.getUser(id)).filter(isTruthy), [memberIds]);
    return (<div className={cl("popout")} ref={setPopoutRef}>
            ...
            <ScrollerThin className={cl("list")} fade>
                {loading ? (<div className={cl("empty")}>Loading members...</div>) : users.length === 0 ? (<div className={cl("empty")}>No members found.</div>) : users.map(user => (<div key={user.id} className={cl("user")} onClick={() => {
                closePopout();
                ...
                {totalCount > MAX_VISIBLE_MEMBERS && (<div className={cl("overflow")}>and {totalCount - MAX_VISIBLE_MEMBERS} more</div>)}
            </ScrollerThin>
        </div>);
}
function ClickableRole({ roleId, guildId, children }) {
    const ref = useRef(null);
    return (<Popout targetElementRef={ref} position="right" align="center" autoInvert nudgeAlignIntoViewport renderPopout={({ closePopout, setPopoutRef }) => (<RoleMembersList roleId={roleId} guildId={guildId} closePopout={closePopout} setPopoutRef={setPopoutRef}/>)}>
            {popoutProps => (<div ref={ref} {...popoutProps} className={cl("trigger")}>
                    {children}
                </div>)}
        </Popout>);
}
...
export default definePlugin({
    ...
    wrapRoleGroup(originalFn) {
        return (props) => {
            const result = originalFn(props);
            if (!GuildRoleStore.getRole(props.guildId, props.id))
                return result;
            return (<WrappedClickableRole roleId={props.id} guildId={props.guildId}>
                    {result}
                </WrappedClickableRole>);
        };
    },
    wrapRolePill(props, renderOriginal) {
        return (<WrappedClickableRole roleId={props.role.id} guildId={props.guildId}>
                {renderOriginal()}
            </WrappedClickableRole>);
    },
});
