...
function resolveChannelTarget(element) {
...
function resolveGuildId(element) {
    const listId = element.getAttribute("data-list-id");
    const rawId = element.getAttribute("data-list-item-id") ?? "";
...
function resolveUserId(element, context) {
    ...
    if (listCandidate) {
        if (context.UserStore.getUser(listCandidate))
    ...
    if (ariaMatch && context.UserStore.getUser(ariaMatch))
        return ariaMatch;
    return null;
}
function resolveAvatarUserId(element) {
    const src = element.getAttribute("src") ?? "";
    const styleAttr = element.getAttribute("style") ?? "";
    const bgImage = element.style?.backgroundImage ?? "";
    return extractUserFromAvatar(`${src} ${styleAttr} ${bgImage}`);
}
export function inspectDragEvent(event, context) {
    return inspectElements(collectEventElements(event), context);
}
export function inspectElement(target, context) {
    return inspectElements(target ? [target] : [], context);
}
function inspectElements(elements, context) {
    const resolved = {
        hasAttachment: false,
    ...
    for (const element of collectAncestors(elements)) {
        ...
        const dataAttachment = element.getAttribute("data-attachment-id")
