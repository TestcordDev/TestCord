...
let abortController;
let currentTrackId;
let cachedStartTimestamp;
let cachedPauseTimestamp;
let cachedTrackState;
let lastMinutesAgo;
let cachedActivity;
let cachedSettingsJSON;
const lastFmCache = new Map();
function customFormat(formatStr, track) {
    if (!formatStr)
        return "";
    return formatStr
...
async function getAsset(applicationId, key) {
...
async function fetchNowPlaying(signal) {
    ...
    try {
        ...
        if (!entries || !Array.isArray(entries) || entries.length === 0)
            return null;
        const myEntry = entries.find((e) => e.username?.toLowerCase() === nd_username.toLowerCase());
        return myEntry ?? null;
    }
    catch (e) {
...
async function getActivity(signal) {
    ...
    const activity = {
        ...
        }[nd_statusDisplayType] : undefined,
...
async function updatePresence() {
    ...
    if (abortController && !abortController.signal.aborted) {
        const interval = settings.store.nd_refreshInterval ?? 10;
