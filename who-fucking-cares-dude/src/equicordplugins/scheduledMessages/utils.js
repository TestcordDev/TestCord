...
export const phantomMessageMap = new Map();
const pendingReactions = new Map();
const recentReactionChanges = new Map();
const REACTION_COOLDOWN_MS = 2000;
const pendingRecreations = new Map();
const RECREATE_DEBOUNCE_MS = 300;
export async function loadScheduledMessages() {
    const saved = await DataStore.get(STORAGE_KEY);
...
export function getScheduledMessagesForChannel(channelId) {
    const messages = getScheduledMessages();
    if (!channelId)
        return messages;
    return messages.filter(message => message.channelId === channelId);
}
export function getChannelDisplayInfo(channelId) {
...
function getVideoPreview(dataUrl) {
    return new Promise(resolve => {
...
async function buildPhantomAttachments(attachments) {
    const result = [];
...
export async function createPhantomMessage(msg) {
    if (!settings.store.showPhantomMessages)
        return;
...
function applyPhantomClassToMessage(channelId, messageId) {
...
function removePhantomMessage(msg) {
    const messageId = `scheduled-${msg.id}`;
...
function updatePhantomReactions(messageId, channelId, reactions) {
...
function doRecreatePhantomMessage(messageId, channelId, reactions) {
...
async function uploadAttachment(channelId, att) {
    return new Promise(resolve => {
        const bytes = Uint8Array.from(atob(att.data), c => c.charCodeAt(0));
        const file = new File([bytes], att.filename, { type: att.type });
        const upload = new CloudUploader({ file, platform: 1 /* CloudUploadPlatform.WEB */ }, channelId);
...
async function postMessage(channelId, content, attachments) {
    await RestAPI.post({
...
async function addReactionsToMessage(channelId, messageId, reactions) {
...
async function sendScheduledMessage(msg) {
    try {
        if (!ChannelStore.getChannel(msg.channelId))
            return false;
        const reactions = pendingReactions.get(msg.id) ?? msg.reactions ?? [];
        removePhantomMessage(msg);
        pendingReactions.delete(msg.id);
        if (msg.attachments?.length) {
            const uploaded = (await Promise.all(msg.attachments.map((att, i) => uploadAttachment(msg.channelId, att).then(r => r ? { ...r, id: String(i) } : null)))).filter(Boolean);
        ...
        if (reactions.length) {
            ...
            const msgArray = MessageStore.getMessages(msg.channelId)?._array ?? [];
...
export async function addScheduledMessage(channelId, content, scheduledTime, attachments) {
    const minuteStart = Math.floor(scheduledTime / 60000) * 60000;
    const count = scheduledMessages.filter(m => m.channelId === channelId && m.scheduledTime >= minuteStart && m.scheduledTime < minuteStart + 60000).length;
...
export async function updateScheduledMessageTime(id, scheduledTime) {
    ...
    const minuteStart = Math.floor(scheduledTime / 60000) * 60000;
    const count = scheduledMessages.filter(entry => entry.id !== id
        && entry.channelId === message.channelId
        && entry.scheduledTime >= minuteStart
        && entry.scheduledTime < minuteStart + 60000).length;
    if (count >= settings.store.maxMessagesPerMinute) {
        return { success: false, error: `Maximum of ${settings.store.maxMessagesPerMinute} messages per channel per minute reached` };
    }
...
export async function removeScheduledMessage(id) {
...
export async function clearAllScheduledMessages() {
...
export async function recreatePhantomMessages() {
    for (const msg of scheduledMessages)
        await createPhantomMessage(msg);
}
export function cleanupAllPhantomMessages() {
    for (const msg of scheduledMessages)
        removePhantomMessage(msg);
}
function modifyReaction(messageId, channelId, emoji, delta) {
    ...
    if (delta > 0) {
        if (idx >= 0)
            ...
            reactions.push({ emoji: { id: emoji.id ?? null, name: emoji.name, animated: emoji.animated }, count: 1 });
    }
    else if (idx >= 0) {
        reactions[idx].count += delta;
...
function getReactionKey(messageId, emoji) {
    return `${messageId}:${emoji.name}:${emoji.id ?? ""}`;
}
export function handleReactionAdd(messageId, channelId, emoji) {
...
export function handleReactionRemove(messageId, channelId, emoji) {
    const key = getReactionKey(messageId, emoji);
    const recent = recentReactionChanges.get(key);
    const now = Date.now();
    if (recent && recent.action === "remove" && now - recent.timestamp < REACTION_COOLDOWN_MS) {
...
export function isPhantomMessage(messageId) {
    return phantomMessageMap.has(messageId);
}
export function resyncPhantomReactions(messageId, channelId) {
