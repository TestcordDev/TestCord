...
const Native = VencordNative.pluginHelpers.ThemeLibrary;
...
export const ThemeInfoModal = ({ author, theme, ...props }) => {
    ...
    return (<Modal {...props} size="md" title={`${name} details`} actions={[
            {
                text: "Close",
                variant: "secondary",
            ...
            },
            {
                ...
                onClick: async () => {
                    ...
                    if (exists) {
                        openModal(modalProps => (<Modal {...modalProps} size="sm" title="Conflict!" actions={[
                                {
                                    text: "Overwrite",
            ...
            <div style={{ display: "flex", alignItems: "center", marginBottom: "10px" }}>
                <div>
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <UserSummaryItem users={authors} count={authors.length} guildId={undefined} renderIcon={false} max={4} size={32} showDefaultAvatarsForNullUsers showUserPopout className={Margins.right8}/>
                    ...
                    {(guild || invite) && (<>
                            ...
                            <Paragraph>
                                <Button color={Button.Colors.BRAND} look={Button.Looks.FILLED} className={Margins.top8} onClick={async (e) => {
                e.preventDefault();
                ...
                useInvite != null && openInviteModal(useInvite).catch(() => showToast("Invalid or expired invite!", Toasts.Type.FAILURE));
                    ...
                    <Paragraph>
                        <Button disabled={!theme.content || theme.id === "preview"} onClick={() => openModal(modalProps => (<Modal {...modalProps} size="lg" title="Theme Source" actions={[
                {
                    text: "Close",
                    variant: "dangerPrimary",
                    onClick: () => modalProps.onClose()
                },
                {
                    ...
                    {tags && (<>
                            ...
                            <Paragraph>
                                {tags.map(tag => (<span className="vce-theme-info-tag" key={"vce-theme-info-tag"}>
                                        {tag}
                                    </span>))}
                            </Paragraph>
                        </>)}
                    {requiresThemeAttributes && (<Paragraph style={{ marginTop: "10px" }}>
                            <WarningIcon /> This theme requires the <b>ThemeAttributes</b> plugin!
                        </Paragraph>)}
                    {last_updated && (<Paragraph style={{ marginTop: "10px" }}>
