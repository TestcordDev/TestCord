...
import { IS_MAC, TestcordDevs } from "@utils/constants";
...
const cl = classNameFactory("vc-class-highlight-");
...
function KeybindRecorder() {
    ...
    return (<div className={cl("keybind-row")}>
            <button type="button" className={cl("keybind-button", isListening ? "listening" : "")} onClick={() => setIsListening(true)}>
                {isListening ? "Recording..." : <KeybindShortcut shortcut={currentKeybind.join("+")}/>}
            </button>
            <Button size="small" variant="secondary" onClick={() => { settings.store.keybind = DEFAULT_KEYBIND; }}>
                Reset
            </Button>
        </div>);
}
const settings = definePluginSettings({
    ...
    showText: {
        description: "Display the element's text content in the tooltip",
        type: 3 /* OptionType.BOOLEAN */,
        default: false
    }
});
...
function getColorVarFromElement(el) {
...
function buildTooltipContent(el, computed, rect) {
    ...
    if (store.showId) {
        const { id } = el;
        if (id)
            html += `<div class="${cl("info")}"><span class="${cl("label")}">id:</span> #${id}</div>`;
    }
    ...
    if (store.showText) {
        ...
        if (text) {
            ...
            html += `<div class="${cl("info")}"><span class="${cl("label")}">text:</span> ${truncated}</div>`;
        }
    }
    if (store.showFont) {
    ...
    if (store.showPadding) {
        ...
        if (padding)
            html += `<div class="${cl("info")}"><span class="${cl("label")}">padding:</span> ${padding}</div>`;
    }
    if (store.showMargin) {
        const margin = formatBoxValue(computed.margin);
        if (margin)
            html += `<div class="${cl("info")}"><span class="${cl("label")}">margin:</span> ${margin}</div>`;
    }
    if (store.showBorderRadius) {
        const radius = formatBoxValue(computed.borderRadius);
        if (radius)
            html += `<div class="${cl("info")}"><span class="${cl("label")}">radius:</span> ${radius}</div>`;
    }
...
function onMouseMove(e) {
    ...
    if (!el || el === overlay || el === tooltip || tooltip?.contains(el))
        return;
...
function onClick(e) {
    ...
    if (className && typeof className === "string") {
        copyWithToast(className, "Copied class to clipboard!");
    }
}
