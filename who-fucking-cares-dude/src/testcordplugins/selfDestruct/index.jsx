...
function addTimerListener(listener) {
    timerListeners.add(listener);
    if (!listenerInterval)
        listenerInterval = setInterval(notifyListeners, 1000);
}
function removeTimerListener(listener) {
...
function scheduleDelete(channelId, messageId, delaySec) {
...
function SelfDestructIcon({ active, width = 20, height = 20 }) {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width={width} height={height} fill="none" viewBox="0 0 24 24" style={{ color: active ? "#ed4245" : "currentColor" }}>
            {/* Bombe / timer icon */}
            ...
            {!active && (<path fill="var(--status-danger)" d="M21.178 1.707 22.592 3.12 4.12 21.593l-1.414-1.415L21.178 1.707Z"/>)}
        </svg>);
}
const SelfDestructButton = ({ isMainChat }) => {
...
export default definePlugin({
    ...
    renderMessageAccessory(props) {
        ...
        if (!message?.id || !activeTimers.has(message.id))
            return null;
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("SelfDestruct", () => (<HeaderBarButton icon={() => <SelfDestructIcon active={settings.store.active}/>} tooltip="SelfDestruct" onClick={() => { settings.store.active = !settings.store.active; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("SelfDestruct", () => (<ChannelToolbarButton icon={() => <SelfDestructIcon active={settings.store.active}/>} tooltip="SelfDestruct" onClick={() => { settings.store.active = !settings.store.active; }}/>), 5);
        }
    },
    stop() {
        cleanup();
