...
let uploadAddFilesInterceptor = null;
let pasteEventListener = null;
function shouldInterceptUploadFiles(files, payload) {
    if (!settings.store.bypassDiscordUploadOnlyOverLimit)
        return true;
    const directLimit = [payload.maxFileSize, payload.fileSizeLimit, payload.limits?.fileSize].find(limit => Number.isFinite(limit));
...
function extractFilesFromValue(value) {
...
function interceptUploadAddFiles(event) {
    ...
    if (payload.draftType !== DraftType.ChannelMessage)
        return;
    if (!settings.store.bypassDiscordUpload || !isConfigured())
        return;
    const files = [
    ...
    if (!uniqueFiles.length)
        return;
    if (!shouldInterceptUploadFiles(uniqueFiles, payload))
        return;
    payload.files = [];
    payload.uploads = [];
    payload.items = [];
...
function handlePaste(event) {
    ...
    if (files.length === 0)
        return;
    if (!settings.store.autoUploadPastedFiles || !isConfigured())
        return;
...
const ProgressBarInner = () => {
    ...
    return (<div className={cl("progress-wrap")} data-phase={state.phase}>
            <div className={cl("progress-head")}>
                ...
                <div className={cl("progress-meta")}>
                    ...
                    {state.canCancel && (<button className={cl("progress-cancel")} type="button" onClick={cancelCurrentUpload}>
                            Cancel
                        </button>)}
                </div>
            </div>
...
const messageContextMenuPatch = (children, props) => {
    ...
    if (group && !group.some(child => child?.props?.id === "file-upload")) {
        const serviceType = settings.store.serviceType;
        const serviceName = serviceLabels[serviceType];
        group.push(<Menu.MenuItem label={`Upload to ${serviceName}`} key="file-upload" id="file-upload" action={() => uploadFile(url)}/>);
    }
};
const imageContextMenuPatch = (children, props) => {
    ...
    if (!url)
        return;
    if (children.some(child => child?.props?.id === "file-upload-group"))
        return;
    const serviceType = settings.store.serviceType;
    const serviceName = serviceLabels[serviceType];
    children.push(<Menu.MenuGroup id="file-upload-group">
            <Menu.MenuItem label={`Upload to ${serviceName}`} key="file-upload" id="file-upload" action={() => uploadFile(url)}/>
        </Menu.MenuGroup>);
};
async function handleUploadFileFromDraft(upload) {
...
const channelAttachMenuPatch = (children, props) => {
    ...
    const draftUploads = Array.isArray(uploads) ? uploads.filter((u) => u.item?.file && isFileTypeAllowed(u.item.file)) : [];
    if (draftUploads.length > 0) {
        children.splice(1, 0, <Menu.MenuItem id="file-upload-uploads" key="file-upload-uploads" label="Upload to Host" iconLeft={ExternalIcon} leadingAccessory={{
                type: "icon",
                icon: ExternalIcon
            }}>
                {draftUploads.map((upload) => (<Menu.MenuItem id={`file-upload-draft-${upload.id}`} key={upload.id} label={upload.filename} action={() => handleUploadFileFromDraft(upload)}/>))}
                <Menu.MenuSeparator />
                <Menu.MenuItem id="file-upload-manual" key="file-upload-manual" label="Choose File..." action={() => uploadPickedFile()}/>
            </Menu.MenuItem>);
    }
    else {
        children.splice(1, 0, <Menu.MenuItem id="file-upload-manual" key="file-upload-manual" label="Upload to Host" iconLeft={ExternalIcon} leadingAccessory={{
                type: "icon",
                icon: ExternalIcon
