...
export const settings = definePluginSettings({
    ...
    includeProfileIcons: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include avatar CDN links and profile icons." },
    includeProfileDetails: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include bio, pronouns, badges, status, and creation dates." },
    includeRoles: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include user roles, role colors, and role permissions." },
    includeDeletedMessages: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include deleted messages from local cache / MessageLogger." },
    includeMedia: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include image/video/audio attachments." },
    includeEmbeds: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include link embeds." },
    includeReactions: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include message reactions and stickers." },
    includeBots: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Include bot accounts in member list exports." },
    loadedOnly: {
...
function getAccountAgeDays(id) {
    const created = getSnowflakeDate(id).getTime();
    if (!created)
        return 0;
    return Math.floor((Date.now() - created) / (1000 * 60 * 60 * 24));
}
function getUserAvatarUrl(user, member, guildId) {
...
function getUserBannerUrl(user, member, guildId) {
    const memberBanner = member?.banner;
...
function serializeUserFull(user, member, guildId) {
    ...
    const flags = (user.publicFlags ?? user.flags ?? 0);
...
function getToken() {
    try {
        const mod = window.Vencord?.Webpack?.findByProps?.("getToken");
        return mod?.getToken?.() ?? "";
    }
    catch {
        return "";
...
async function getDeletedMessagesFromIDB(channelId) {
    try {
        const dbReq = indexedDB.open("MessageLoggerIDB", 1);
...
function fromCachedMessage(m, deleted) {
    ...
    return {
        ...
        stickers: (m.sticker_items ?? m.stickerItems ?? m.stickers ?? []).map((s) => ({ name: s.name, id: s.id })),
        reactions: (m.reactions ?? []).map((r) => ({ emoji: r.emoji?.name ?? r.emoji?.id, count: r.count })),
...
function getLoadedRichMessages(channelId) {
...
async function fetchAllMessages(channelId, token, onProgress, signal) {
    ...
    while (true) {
        ...
        let batch;
        try {
            batch = await TestcordRequestCoordinator.request({
        ...
        for (const m of batch) {
            messageMap.set(m.id, {
                ...
                stickers: (m.sticker_items ?? []).map((s) => ({ name: s.name, id: s.id })),
                reactions: (m.reactions ?? []).map((r) => ({ emoji: r.emoji.name ?? r.emoji.id, count: r.count })),
    ...
    if (settings.store.includeDeletedMessages) {
        try {
            const cached = findStoreLazy("MessageStore")?.getMessages?.(channelId);
            if (cached) {
        ...
        try {
            ...
            for (const record of idbRecords) {
                ...
                if (!m || messageMap.has(record.message_id))
                    continue;
...
// ── Helpers ──
function getMediaType(url, ct) {
    if (ct.startsWith("image/") || /\.(jpg|jpeg|png|gif|webp|avif|svg)(\?|$)/i.test(url))
        return "image";
    if (ct.startsWith("video/") || /\.(mp4|webm|mov|avi|mkv)(\?|$)/i.test(url))
        return "video";
    if (ct.startsWith("audio/") || /\.(mp3|ogg|wav|flac|m4a)(\?|$)/i.test(url))
        return "audio";
    return "file";
}
...
// 1. MESSAGES BUILDERS
function buildMessagesHtml(messages, channelName) {
    const rows = messages.map(m => {
        ...
        const mediaHtml = settings.store.includeMedia ? m.attachments.map(a => {
            ...
            if (t_type === "image")
                return `<div class="media"><img src="${a.url}" alt="${escapeXml(a.filename)}" loading="lazy"><div class="media-name">${escapeXml(a.filename)} (${formatSize(a.size)})</div></div>`;
            if (t_type === "video")
                return `<div class="media"><video src="${a.url}" controls preload="none"></video><div class="media-name">${escapeXml(a.filename)}</div></div>`;
            if (t_type === "audio")
                return `<div class="media"><audio src="${a.url}" controls></audio><div class="media-name">${escapeXml(a.filename)}</div></div>`;
        ...
        const embedHtml = settings.store.includeEmbeds ? m.embeds.map(e => {
            ...
            if (e.title)
                html += `<div class="embed-title">${escapeXml(e.title)}</div>`;
            if (e.description)
                html += `<div class="embed-desc">${escapeXml(e.description.slice(0, 300))}</div>`;
            if (e.image)
                html += `<img src="${e.image}" class="embed-img" loading="lazy">`;
            if (e.url)
                html += `<a href="${e.url}" target="_blank" class="embed-url">${escapeXml(e.url)}</a>`;
            return html + "</div>";
        }).join("") : "";
...
function buildMessagesTxt(messages, channelName) {
    const lines = [`=== Export Messages — ${channelName} ===`, `Exported on ${new Date().toLocaleString()}`, `Total: ${messages.length} messages`, ""];
    for (const m of messages) {
        const d = new Date(m.timestamp).toLocaleString();
        if (m.referencedMessage)
            lines.push(`  > [${m.referencedMessage.authorName}]: ${m.referencedMessage.content}`);
        lines.push(`[${d}]${m.editedAt ? " (edited)" : ""}${m.deleted ? " [DELETED]" : ""} ${m.authorName}: ${m.content}`);
        if (settings.store.includeMedia) {
            for (const a of m.attachments)
                lines.push(`  [${getMediaType(a.url, a.contentType).toUpperCase()}] ${a.filename} (${formatSize(a.size)}) — ${a.url}`);
        }
        if (settings.store.includeEmbeds) {
            for (const e of m.embeds) {
                if (e.url)
                    lines.push(`  [LINK] ${e.title ?? "Embed"}: ${e.url}`);
            }
        }
        if (settings.store.includeReactions) {
            ...
            if (m.reactions.length)
                lines.push(`  ${m.reactions.map(r => `${r.emoji} x${r.count}`).join(" ")}`);
        }
    }
    return lines.join("\n");
}
function buildMessagesMd(messages, channelName) {
    ...
    for (const m of messages) {
        ...
        if (settings.store.includeMedia) {
            for (const a of m.attachments)
                lines.push(`[${a.filename}](${a.url}) *(${formatSize(a.size)})*`);
        }
        if (settings.store.includeEmbeds) {
            for (const e of m.embeds) {
                if (e.url)
                    lines.push(`[${e.title ?? "Link"}](${e.url})`);
            }
        }
        if (settings.store.includeReactions) {
            for (const s of m.stickers)
                lines.push(`*Sticker: ${s.name}*`);
            if (m.reactions.length)
                lines.push(m.reactions.map(r => `${r.emoji} \`${r.count}\``).join(" "));
        }
        lines.push("");
    }
    return lines.join("\n");
}
function buildMessagesJson(messages, channelName) {
    return JSON.stringify({ channel: channelName, exportedAt: new Date().toISOString(), count: messages.length, messages }, null, 2);
}
function buildMessagesJsonl(messages) {
...
function buildMessagesXml(messages, channelName) {
    const lines = [
    ...
    for (const m of messages) {
        ...
        if (m.attachments.length) {
            ...
            for (const a of m.attachments)
                lines.push(`        <attachment filename="${escapeXml(a.filename)}" size="${a.size}" url="${escapeXml(a.url)}"/>`);
            lines.push("      </attachments>");
        }
        lines.push("    </message>");
    }
...
function buildMessagesYaml(messages, channelName) {
    return toYamlString({
        exporter: {
            type: "messages",
            channelName,
...
function buildMessagesBbcode(messages, channelName) {
    ...
    for (const m of messages) {
        ...
        if (settings.store.includeMedia) {
            for (const a of m.attachments) {
                if (getMediaType(a.url, a.contentType) === "image")
                    lines.push(`[img]${a.url}[/img]`);
                else
                    lines.push(`[url=${a.url}]${a.filename}[/url] (${formatSize(a.size)})`);
            }
        }
        lines.push("");
    }
...
function buildMembersHtml(members, title) {
...
function buildMembersTxt(members, title) {
    ...
    for (const m of members) {
        ...
        if (m.joinedAt)
            lines.push(`  Joined: ${m.joinedAt} (${m.serverTenureDays} days ago)`);
        if (m.roles.length)
            lines.push(`  Roles: ${m.roles.map(r => r.name).join(", ")}`);
        if (m.badges.length)
            lines.push(`  Badges: ${m.badges.join(", ")}`);
        lines.push("");
    }
    return lines.join("\n");
}
function buildMembersMd(members, title) {
    ...
    for (const m of members) {
        ...
        if (m.joinedAt)
            lines.push(`- **Joined Server:** ${m.joinedAt.slice(0, 10)} (*${m.serverTenureDays} days ago*)`);
        if (m.roles.length)
            lines.push(`- **Roles:** ${m.roles.map(r => `\`${r.name}\``).join(", ")}`);
        if (m.badges.length)
            lines.push(`- **Badges:** ${m.badges.map(b => `\`${b}\``).join(", ")}`);
        if (m.bio)
...
function buildMembersXml(members, title) {
    const lines = [
        '<?xml version="1.0" encoding="UTF-8"?>',
    ...
    for (const m of members) {
        ...
        if (m.joinedAt)
            lines.push(`      <joinedAt>${m.joinedAt}</joinedAt>`);
        lines.push(`      <status>${escapeXml(m.status)}</status>`);
        if (m.roles.length) {
            lines.push("      <roles>");
            for (const r of m.roles)
                lines.push(`        <role id="${r.id}" name="${escapeXml(r.name)}" colorHex="${r.colorHex}"/>`);
            lines.push("      </roles>");
        }
        lines.push("    </member>");
    }
...
function buildMembersYaml(members, title) {
    return toYamlString({
        exporter: {
            type: "memberlist",
            title,
...
function buildMembersBbcode(members, title) {
...
async function saveOrDownloadFile(content, filename, mime = "text/plain") {
...
// ── Icons ──
function ExportIcon({ width = 18, height = 18 }) {
    return (<svg aria-hidden="true" role="img" width={width} height={height} viewBox="0 0 24 24" fill="none">
...
export function ExporterModal({ rootProps, initialTab }) {
    const [tab, setTab] = useState(initialTab ?? settings.store.exportType ?? "messages");
    ...
    const [format, setFormat] = useState((tab === "memberlist" ? settings.store.memberFormat : settings.store.chatFormat));
    ...
    useEffect(() => {
        ...
        try {
            ...
            const guildRoles = GuildRoleStore.getRoles?.(currentGuildId) ?? GuildRoleStore.getGuildRoles?.(currentGuildId);
            if (guildRoles) {
    ...
    async function doExport() {
        ...
        try {
            ...
            if (tab === "memberlist") {
                ...
                switch (format) {
                    case "html":
                        content = buildMembersHtml(serializedMembers, title);
                        ext = "html";
                        mime = "text/html";
                        break;
                    case "txt":
                    ...
                    case "md":
                        ...
                        break;
                    case "json":
                        content = buildMembersJson(serializedMembers, title);
                        ext = "json";
                        mime = "application/json";
                        break;
                    case "jsonl":
                        content = buildMembersJsonl(serializedMembers);
                        ext = "jsonl";
                        mime = "application/x-ndjson";
                        break;
                    case "csv":
                        content = buildMembersCsv(serializedMembers);
                        ext = "csv";
                        mime = "text/csv";
                        break;
                    case "xml":
                        content = buildMembersXml(serializedMembers, title);
                        ext = "xml";
                        mime = "application/xml";
                        break;
                    case "yaml":
                        content = buildMembersYaml(serializedMembers, title);
                        ext = "yaml";
                        mime = "text/yaml";
                        break;
                    case "bbcode":
                        content = buildMembersBbcode(serializedMembers, title);
                        ext = "txt";
                        mime = "text/plain";
                        break;
                    default:
            ...
            // 2. Export Messages Only
                ...
                for (let i = 0; i < targetChannels.length; i++) {
                    ...
                    if (!includeMedia)
                        msgs = msgs.map(m => ({ ...m, attachments: [] }));
                    if (!includeEmbeds)
                        msgs = msgs.map(m => ({ ...m, embeds: [] }));
                    if (!includeReactions)
                        msgs = msgs.map(m => ({ ...m, reactions: [], stickers: [] }));
                    ...
                    switch (format) {
                        case "html":
                            content = buildMessagesHtml(msgs, ch.name);
                            ext = "html";
                            mime = "text/html";
                            break;
                        case "txt":
                            content = buildMessagesTxt(msgs, ch.name);
                            ext = "txt";
                            mime = "text/plain";
                            break;
                        case "md":
                            content = buildMessagesMd(msgs, ch.name);
                            ext = "md";
                            mime = "text/markdown";
                            break;
                        case "json":
                            content = buildMessagesJson(msgs, ch.name);
                            ext = "json";
                            mime = "application/json";
                            break;
                        case "jsonl":
                            content = buildMessagesJsonl(msgs);
                            ext = "jsonl";
                            mime = "application/x-ndjson";
                            break;
                        case "csv":
                            content = buildMessagesCsv(msgs);
                            ext = "csv";
                            mime = "text/csv";
                            break;
                        case "xml":
                            content = buildMessagesXml(msgs, ch.name);
                            ext = "xml";
                            mime = "application/xml";
                            break;
                        case "yaml":
                            content = buildMessagesYaml(msgs, ch.name);
                            ext = "yaml";
                            mime = "text/yaml";
                            break;
                        case "bbcode":
                            content = buildMessagesBbcode(msgs, ch.name);
                            ext = "txt";
                            mime = "text/plain";
                            break;
                        default:
    ...
    const filteredMembers = members.filter(({ member, user }) => {
        if (!includeBots && user.bot)
            return false;
        if (selectedRole !== "all" && !member.roles.includes(selectedRole))
            return false;
        if (search.trim()) {
    ...
    return (<ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>
                {/* 1. TabBar */}
                <TabBar type="top" selectedItem={tab} onItemSelect={(id) => setTab(id)}>
                ...
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                    <div style={{ flex: 1 }}>
                        <TextInput value={search} onChange={setSearch} placeholder={tab === "memberlist" ? "Search members by name or ID..." : tab === "messages" ? "Search channels..." : "Search channels or members..."}/>
                    </div>
                    {tab !== "messages" && rolesList.length > 0 && (<div style={{ width: 160 }}>
                            <Select options={[
                { label: "All Roles", value: "all" },
                ...rolesList.map(r => ({ label: r.name, value: r.id }))
            ]} isSelected={v => v === selectedRole} select={v => setSelectedRole(v)} serialize={v => v}/>
                        </div>)}
                </div>

                {/* 3. Items Selection List */}
                {tab === "both" ? (<div style={{ display: "flex", gap: "12px" }}>
                        <div style={{ flex: 1 }}>
                ...
                return (<div key={c.id} style={{
                    ...
                    }} onClick={() => toggleChannel(c.id)} onMouseEnter={() => setHoveredId(c.id)} onMouseLeave={() => setHoveredId(null)}>
                ...
                return (<div key={user.id} style={{
                        ...
                        background: bg,
                        transition: "background 0.15s ease"
                    }} onClick={() => toggleMember(user.id)} onMouseEnter={() => setHoveredId(user.id)} onMouseLeave={() => setHoveredId(null)}>
                                            <div>
                    ...
                    </div>) : (<Forms.FormSection title={`${tab === "messages" ? "SELECT CHANNELS / DMS" : "SELECT MEMBERS"} (${tab === "messages" ? selectedChannels.size : selectedMembers.size} selected)`}>
                        <ScrollerThin style={{
                maxHeight: 180,
                background: "var(--background-secondary, var(--background-secondary-alt))",
                borderRadius: "var(--radius-sm, 4px)",
                ...
                <Forms.FormSection title="EXPORT FORMAT">
                    <Select options={FORMATS.map(f => ({ label: `${f.label} — ${f.desc}`, value: f.key }))} isSelected={v => v === format} select={v => setFormat(v)} serialize={v => v}/>
                </Forms.FormSection>

                {/* 5. Options (Discord Native FormSwitches) */}
                <Forms.FormSection title="EXPORT OPTIONS">
                    <FormSwitch value={includeProfileIcons} onChange={setIncludeProfileIcons} title="Profile Icons" description="Include avatar CDN links and fallback avatars"/>
                    <FormSwitch value={includeProfileDetails} onChange={setIncludeProfileDetails} title="Bio & Badges" description="Include bio, pronouns, badges, and account creation dates"/>
                    <FormSwitch value={includeRoles} onChange={setIncludeRoles} title="Roles & Colors" description="Include role lists, role positions, and role hex colors"/>
                    <FormSwitch value={includeMedia} onChange={setIncludeMedia} title="Media Attachments" description="Include image, video, and audio attachments"/>
                    <FormSwitch value={includeEmbeds} onChange={setIncludeEmbeds} title="Link Embeds" description="Include embedded links and open-graph data"/>
                    <FormSwitch value={includeReactions} onChange={setIncludeReactions} title="Reactions & Stickers" description="Include message reaction counts and sticker items"/>
                    <FormSwitch value={includeDeletedMessages} onChange={setIncludeDeletedMessages} title="Deleted Messages" description="Include deleted messages cached locally by MessageLogger"/>
                    <FormSwitch value={includeBots} onChange={setIncludeBots} title="Include Bots" description="Include bot accounts in member list exports"/>
                    <FormSwitch value={loadedOnly} onChange={setLoadedOnly} title="Loaded Messages Only" description="Export only messages already loaded in Discord without fetching full history"/>
                </Forms.FormSection>
            </ModalContent>
            ...
            <ModalFooter>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", flexDirection: "row" }}>
                    <div style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 12 }}>
                        {status !== "idle" && progress && (<Text variant="text-xs/normal" style={{ color: status === "error" ? "var(--text-danger, #ed4245)" : status === "done" ? "var(--text-positive, #23a55a)" : "var(--text-warning, #f0b232)" }}>
                                {progress}
                            </Text>)}
                    </div>
                    <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                        ...
                        <Button color={Button.Colors.BRAND} onClick={() => doExport()} disabled={status === "fetching" || (tab === "messages" && selectedChannels.size === 0) || ((tab === "memberlist" || tab === "both") && selectedMembers.size === 0)}>
...
export function openExporter(tab) {
    if (isModalOpen)
        return;
    isModalOpen = true;
    openModal(props => (<ExporterModal rootProps={{
            ...props,
            onClose: () => {
                isModalOpen = false;
                props.onClose();
...
function ExportButton() {
    return (<HeaderBarButton icon={ExportIcon} tooltip="Exporter" onClick={() => openExporter()}/>);
}
function ExporterChannelToolbarButton() {
    return (<ChannelToolbarButton icon={ExportIcon} tooltip="Exporter" onClick={() => openExporter("memberlist")}/>);
}
function ExporterMenu({ channelId }) {
    return (<Menu.Menu navId="pc-exporter-menu" onClose={ContextMenuApi.closeContextMenu} aria-label="Exporter">
            <Menu.MenuItem id="pc-exporter-open-messages" label="Export Messages" action={() => openExporter("messages")}/>
            <Menu.MenuItem id="pc-exporter-open-members" label="Export Member List" action={() => openExporter("memberlist")}/>
            <Menu.MenuItem id="pc-exporter-open-both" label="Export Both (All-in-One)" action={() => openExporter("both")}/>
        </Menu.Menu>);
}
function openCurrentChannelMenu(e) {
    const channelId = SelectedChannelStore.getChannelId();
    ContextMenuApi.openContextMenu(e, () => <ExporterMenu channelId={channelId}/>);
}
const ExporterChatBarButton = ({ isMainChat }) => {
    if (!isMainChat || settings.store.location !== "chatbar")
        return null;
    return (<ChatBarButton tooltip="Exporter" onClick={() => openExporter()} onContextMenu={openCurrentChannelMenu}>
