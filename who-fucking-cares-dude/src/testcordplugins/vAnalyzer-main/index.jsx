...
async function genericAnalyze(messageId, url, analyzer, silent = false) {
    const result = await analyzer(url, silent);
    if (result) {
        handleAnalysis(messageId, result, url);
    }
}
async function genericAnalyzeFile(messageId, fileUrl, fileName, analyzer, silent = false) {
...
function extractUserIdFromContext(context) {
    ...
    if (typeof id === "string" && /^\d{17,20}$/.test(id))
        return id;
    if ((typeof id === "number" || typeof id === "bigint") && /^\d{17,20}$/.test(String(id)))
        return String(id);
    return undefined;
}
function FindUserByIdModal({ modalProps }) {
    ...
    return (<ModalRoot {...modalProps} size={"small" /* ModalSize.SMALL */}>
            ...
            <ModalContent style={{ padding: "16px" }}>
                ...
                <TextInput autoFocus placeholder="447812212241989632" value={userId} onChange={setUserId} onKeyDown={(e) => { if (e.key === "Enter")
        submit(); }}/>
            </ModalContent>
            <ModalFooter>
...
function getUserSearchLinks(userId) {
...
function getServerSearchLinks(guildId) {
...
const urlAnalyzers = [
    { id: "auto-url-checks", label: "Run all automatic checks", fn: null },
...
const fileAnalyzers = [
    { id: "vt", label: "Scan file with VirusTotal", fn: (msgId, url, _name) => genericAnalyze(msgId, url, (u, s) => analyzeWithVirusTotal(msgId, u, s)) },
    { id: "ha-file", label: "Scan file with Hybrid Analysis", fn: (msgId, url, name) => genericAnalyzeFile(msgId, url, name, analyzeFileWithHybridAnalysis) },
];
const messageCtxPatch = (children, { message }) => {
    ...
    if (settings.store.enableCordCat) {
        ...
        group.push(<Menu.MenuItem id="vc-analyze-author-cordcat" label="Scan author with CordCat" icon={SafetyIcon} action={() => analyzeUserWithCordCat(message.author.id, authorName)}/>);
    }
    if (settings.store.enableFindUserById) {
        group.push(<Menu.MenuItem id="vc-analyze-find-user-by-id" label="Find User by ID" icon={SafetyIcon} action={openFindUserByIdModal}/>);
    }
    if (!hasAttachments && !hasUrls && !hasInvites && !hasCdnFiles)
        return;
    if (hasAttachments) {
        for (const analyzer of fileAnalyzers) {
            if (message.attachments.length === 1) {
                group.push(<Menu.MenuItem id={`vc-analyze-${analyzer.id}`} label={analyzer.label} icon={SafetyIcon} action={() => analyzer.fn(message.id, message.attachments[0].url, message.attachments[0].filename)}/>);
            }
            else {
                group.push(<Menu.MenuItem id={`vc-analyze-${analyzer.id}`} label={analyzer.label} icon={SafetyIcon}>
                        {message.attachments.map((attachment, i) => (<Menu.MenuItem id={`vc-analyze-${analyzer.id}-${i}`} key={attachment.id} label={attachment.filename} action={() => analyzer.fn(message.id, attachment.url, attachment.filename)}/>))}
                    </Menu.MenuItem>);
            }
        }
    }
    if (hasCdnFiles) {
        for (const analyzer of fileAnalyzers) {
            if (cdnFiles.length === 1) {
                group.push(<Menu.MenuItem id={`vc-analyze-cdn-${analyzer.id}`} label={`${analyzer.label} (${cdnFiles[0].fileName})`} icon={SafetyIcon} action={() => analyzer.fn(message.id, cdnFiles[0].url, cdnFiles[0].fileName)}/>);
            }
            else {
                group.push(<Menu.MenuItem id={`vc-analyze-cdn-${analyzer.id}`} label={analyzer.label} icon={SafetyIcon}>
                        {cdnFiles.map((file, i) => (<Menu.MenuItem id={`vc-analyze-cdn-${analyzer.id}-${i}`} key={file.url} label={file.fileName} action={() => analyzer.fn(message.id, file.url, file.fileName)}/>))}
                    </Menu.MenuItem>);
            }
        }
    }
    if (hasUrls) {
        ...
        group.push(<Menu.MenuItem id="vc-analyze-url-group" label="Analyze URL" icon={LinkIcon}>
                {urlAnalyzers.map(analyzer => {
                let action;
                if (analyzer.fn) {
                    action = (url) => genericAnalyze(message.id, url, analyzer.fn);
                }
                else {
                    action = (url) => manualAnalyzeUrls(message, [url]);
                }
                return (<Menu.MenuItem id={`vc-analyze-${analyzer.id}`} key={analyzer.id} label={analyzer.label} action={() => action(primaryUrl)}>
                            {normalUrls.length > 1 && normalUrls.map((url, i) => (<Menu.MenuItem id={`vc-analyze-${analyzer.id}-${i}`} key={url} label={truncateUrl(url)} action={() => action(url)}/>))}
                        </Menu.MenuItem>);
            })}
            </Menu.MenuItem>);
    }
    if (hasInvites) {
        const analyzeInvite = async (url) => {
            const result = await analyzeDiscordInvite(url);
            if (result)
                handleAnalysis(message.id, result);
        };
        if (inviteUrls.length === 1) {
            group.push(<Menu.MenuItem id="vc-analyze-invite" label="Analyze Discord invite" icon={OpenExternalIcon} action={() => analyzeInvite(inviteUrls[0])}/>);
        }
        else {
            group.push(<Menu.MenuItem id="vc-analyze-invite" label="Analyze Discord invite" icon={OpenExternalIcon}>
                    {inviteUrls.map((url, i) => (<Menu.MenuItem id={`vc-analyze-invite-${i}`} key={url} label={truncateUrl(url)} action={() => analyzeInvite(url)}/>))}
                </Menu.MenuItem>);
        }
    }
    const modularModules = getModulesSync();
    if (modularModules.length > 0) {
        const analyzeModular = async (module, fileUrl, fileName) => {
            const result = await runModularScan(module, fileUrl, fileName);
            if (result)
                handleAnalysis(message.id, result, fileUrl);
        };
        group.push(<Menu.MenuItem id="vc-analyze-modular-group" label="Modular Scan" icon={SafetyIcon}>
                {modularModules.map(module => {
                const isUrlMatch = module.type === "url" && hasUrls;
                const isFileMatch = module.type === "file" && hasAttachments;
                if (!isUrlMatch && !isFileMatch)
                    return null;
                return (<Menu.MenuItem id={`vc-analyze-modular-${module.id}`} key={module.id} label={module.name} action={() => {
                        if (isUrlMatch) {
                            ...
                            {isUrlMatch && urls.length > 1 && urls.map((url, i) => (<Menu.MenuItem id={`vc-analyze-modular-${module.id}-${i}`} key={url} label={truncateUrl(url)} action={() => analyzeModular(module, url, "")}/>))}
                            {isFileMatch && message.attachments.length > 1 && message.attachments.map((attachment, i) => (<Menu.MenuItem id={`vc-analyze-modular-${module.id}-${i}`} key={attachment.id} label={attachment.filename} action={() => analyzeModular(module, attachment.url, attachment.filename)}/>))}
                        </Menu.MenuItem>);
            })}
            </Menu.MenuItem>);
    }
};
const userContextPatch = (children, { user, id }) => {
    const userId = user?.id ?? id;
    if (!user && !userId)
        return;
    if (user && settings.store.enableOsintSearchShortcuts) {
        const links = getUserSearchLinks(user.id);
        children.push(<Menu.MenuItem id="vc-analyze-search-user" label="Search User" icon={OpenExternalIcon}>
                {links.map(link => (<Menu.MenuItem id={`vc-analyze-search-user-${link.id}`} key={link.id} label={link.label} action={() => openExternal(link.url)}/>))}
            </Menu.MenuItem>);
    }
    if (settings.store.enableCordCat && userId) {
        const username = user?.username || userId;
        children.push(<Menu.MenuItem id="vc-analyze-user-cordcat" label="Analyze User with CordCat" icon={SafetyIcon} action={() => analyzeUserWithCordCat(userId, username)}/>);
    }
};
const devContextPatch = (children, context) => {
    if (!settings.store.enableCordCat)
        return;
    const userId = extractUserIdFromContext(context);
    if (userId) {
        children.push(<Menu.MenuItem id="vc-analyze-dev-context-cordcat" label="Analyze User with CordCat" icon={SafetyIcon} action={() => analyzeUserWithCordCat(userId, userId)}/>);
    }
    else {
        children.push(<Menu.MenuItem id="vc-analyze-dev-context-cordcat" label="Find User by ID (CordCat)" icon={SafetyIcon} action={openFindUserByIdModal}/>);
    }
};
const guildContextPatch = (children, { guild }) => {
    if (!guild || !settings.store.enableOsintSearchShortcuts)
        return;
    const group = findGroupChildrenByChildId("privacy", children) ?? children;
    const links = getServerSearchLinks(guild.id);
    group.push(<Menu.MenuItem id="vc-analyze-search-server" label="Search Server" icon={OpenExternalIcon}>
            {links.map(link => (<Menu.MenuItem id={`vc-analyze-search-server-${link.id}`} key={link.id} label={link.label} action={() => openExternal(link.url)}/>))}
        </Menu.MenuItem>);
};
export default definePlugin({
    ...
    async start() {
        ...
        if (wl)
            setCustomWhitelist(wl.split(",").map(s => s.trim()).filter(Boolean));
        const bl = settings.store.customBlocklist;
        if (bl)
            setCustomBlocklist(bl.split(",").map(s => s.trim()).filter(Boolean));
    },
    handleLinkClick(data) {
        if (!data?.href || !settings.store.warnOnLinkClick)
            return false;
            ...
            Alerts.show({
                ...
                body: (<div>
                        ...
                        <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>
                            {threat.reasons.map((r, i) => (<div key={i}>• {r}</div>))}
                        </div>
                    </div>),
    ...
    handleFileDownload(url) {
        if (!url || !settings.store.warnOnFileDownload)
            return false;
    ...
    flux: {
        MESSAGE_CREATE({ message, optimistic }) {
            if (optimistic)
                return;
            if (message.__vaAnalyzed)
                return;
            message.__vaAnalyzed = true;
    ...
    renderMessageAccessory: props => {
        if (!props.message.__vaAnalyzed) {
            props.message.__vaAnalyzed = true;
