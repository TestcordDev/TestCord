...
function EncModal(props) {
    ...
    return (<Modal {...props} size="sm" title="Encrypt Message" actions={[
            {
                text: "Send",
                variant: "primary",
                disabled: !isValid,
            ...
            <TextInput disabled={noCover} onChange={(e) => {
            setCover(e);
        }}/>
            <Heading style={{ marginTop: "10px" }}>Password</Heading>
            <TextInput style={{ marginBottom: "20px" }} defaultValue={"password"} onChange={(e) => {
            setPassword(e);
        }}/>
            <FormSwitch title="Don't use a Cover" value={noCover} onChange={(e) => {
            setNoCover(e);
        }}/>
        </Modal>);
}
