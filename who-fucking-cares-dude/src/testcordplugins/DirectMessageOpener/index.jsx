...
const UserIcon = (props) => (<svg x="0" y="0" className={props.className} aria-hidden="true" width={props.width ?? 24} height={props.height ?? 24} viewBox="0 0 24 24" fill="currentColor">
        <path fillRule="evenodd" clipRule="evenodd" d="M20 2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14l4 4V4c0-1.1-.9-2-2-2zm-8 4a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zm0 6c-2.2 0-4.5.9-4.5 2.2v0.8h9v-0.8c0-1.3-2.3-2.2-4.5-2.2z"/>
    </svg>);
...
function DirectMessageModal(props) {
    ...
    return (<ModalRoot {...props} title="">
            ...
            <ModalContent>
                ...
                <FormText type={"description"} style={{ marginBottom: "10px" }}>
                    Enter the Discord User ID of the person you want to message
                </FormText>
                <TextInput placeholder="123456789012345678" value={userId} onChange={setUserId} disabled={loading}/>
                {error && (<FormText type={"description"} style={{ color: "var(--text-danger)", marginTop: "10px" }}>
                        {error}
                    </FormText>)}
            </ModalContent>

            <ModalFooter style={{ display: "flex", flexDirection: "row-reverse", gap: "16px", alignItems: "center" }}>
                <Button color={Button.Colors.BRAND} disabled={loading || !userId.trim()} onClick={openDM}>
                    {loading ? "Opening..." : "Open DM"}
                </Button>
                <Button color={Button.Colors.TRANSPARENT} look={Button.Looks.LINK} onClick={props.onClose} style={{ marginRight: "8px" }}>
                    Cancel
                </Button>
            </ModalFooter>
        </ModalRoot>);
...
function ToolBarHeader() {
    return (<ErrorBoundary noop={true}>
            <HeaderBarIcon tooltip="Open DM by User ID" position="bottom" className="vc-plugin-icon-button vc-dm-opener" iconClassName="vc-plugin-icon-button" icon={UserIcon} onClick={openDirectMessageModal}/>
        </ErrorBoundary>);
}
export default definePlugin({
    ...
    renderDMButton() {
        return (<HeaderBarButton icon={UserIcon} tooltip="Open DM by User ID" className="vc-dm-opener-icon" onClick={openDirectMessageModal}/>);
    },
    commands: [
        {
            name: "opendm",
