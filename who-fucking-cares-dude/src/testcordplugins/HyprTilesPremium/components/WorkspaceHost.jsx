...
function WorkspaceHostComponent({ routeTarget, routeElement }) {
    ...
    const renderTileShell = (tile, contentStyle, options) => {
        ...
        return (<section key={tile.id} className={classes(cl("tile"), options.floating && cl("tile-floating"), options.focused && cl("tile-focused"), !options.focused && cl("tile-unfocused"), dragState?.tileId === tile.id && cl("tile-dragging"), liveBoundsTileId === tile.id && cl("tile-live-bounds"))} style={contentStyle} data-hypr-tile-id={tile.id}>
                {showTileHeaders && (<header className={cl("tile-header")} onMouseDown={options.onHeaderMouseDown}>
                        {options.header ?? (<>
                ...
                {options.onResizeStart && resizeCorners.map(corner => (<button key={`${tile.id}-${corner}`} type="button" className={classes(cl("resize-handle"), cl(`resize-${corner}`))} aria-label={`Resize ${info.title}`} onMouseDown={event => options.onResizeStart?.(corner, event)}/>))}
            </section>);
    };
    return (<div className={classes(cl("root"), enableAnimations && cl("animated"))}>
            ...
            return renderTileShell(tile, toStyle(renderedBounds), {
                ...
                {scratchpads.map(({ scratchpad, tile }) => renderTileShell(tile, toStyle(getRenderedFloatBounds(tile.id, tile.floatBounds ?? scratchpad.bounds)), {
            focused: workspace.focusedTileId === tile.id,
            floating: true,
            onHeaderMouseDown: event => {
                if (event.button !== 0)
