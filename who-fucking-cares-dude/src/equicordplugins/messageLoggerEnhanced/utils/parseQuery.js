...
const validIdSearchTypes = ["server", "guild", "channel", "in", "user", "from", "message", "has", "before", "after", "around", "near", "during"];
export const parseQuery = memoize((query = "") => {
    ...
    switch (type) {
        ...
        case "user":
            return message.author.id === value
                ...
                || message.author?.globalName?.toLowerCase()?.includes(value.toLowerCase());
        case "guild":
        case "server": {
