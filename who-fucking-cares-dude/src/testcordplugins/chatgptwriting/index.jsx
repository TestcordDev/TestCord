...
function applyAutocorrect(content) {
...
const ChatGPTWritingIcon = ({ width = 20, height = 20 }) => (<svg xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const getPresend = () => {
    return (_, msg) => {
        ...
        if (msg.content.indexOf(backticks) !== -1)
            return;
...
export default definePlugin({
    ...
    settings,
    _presendListener: null,
    chatBarButton: {
        icon: ChatGPTWritingIcon,
        render: (({ isMainChat }) => {
            if (!isMainChat || settings.store.location !== "chatbar")
                return null;
            return (<ChatBarButton tooltip={settings.store.enabled ? "ChatGPT Writing (ON)" : "ChatGPT Writing (OFF)"} onClick={() => {
                    settings.store.enabled = !settings.store.enabled;
                }}>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={settings.store.enabled ? "#5865F2" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5"/>
                    <path d="M8.5 8.5v.01"/>
                    <path d="M16 15.5v.01"/>
                    <path d="M12 12v.01"/>
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("ChatGPTWriting", () => (<HeaderBarButton icon={() => <ChatGPTWritingIcon />} tooltip={settings.store.enabled ? "ChatGPT Writing (ON)" : "ChatGPT Writing (OFF)"} onClick={() => { settings.store.enabled = !settings.store.enabled; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("ChatGPTWriting", () => (<ChannelToolbarButton icon={() => <ChatGPTWritingIcon />} tooltip={settings.store.enabled ? "ChatGPT Writing (ON)" : "ChatGPT Writing (OFF)"} onClick={() => { settings.store.enabled = !settings.store.enabled; }}/>), 5);
        }
    },
    stop() {
