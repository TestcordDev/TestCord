...
function InfoRow({ label, value }) {
    return (<div className="vc-voice-server-info-row">
            <span>{label}</span>
            <strong>{value}</strong>
        </div>);
}
export function VoiceServerModal({ rootProps, loadInfo }) {
    ...
    const coordinates = server?.info.latitude != null && server.info.longitude != null
        ...
        : "Unknown";
    return (<Modal {...rootProps} size="md" title="Voice Server Info" subtitle="Details about the Discord relay currently carrying your voice connection.">
            <div className="vc-voice-server-info-body">
                <div className="vc-voice-server-info-hero">
                    ...
                    <span className={classes("vc-voice-server-info-status", connectionState === "RTC_CONNECTED" && "vc-voice-server-info-status-connected")}>
                ...
                {server && (<div className="vc-voice-server-info-grid">
                ...
                <div className="vc-voice-server-info-actions">
                    ...
                    <Button size="small" variant="primary" disabled={!server} onClick={() => server && void copyWithToast(server.info.ip, "Voice server IP copied.")}>
                        Copy IP
                    </Button>
                </div>
            </div>
...
export function VoiceServerIcon({ width = 20, height = 20, className }) {
    return (<svg className={className} width={width} height={height} viewBox="0 0 24 24" fill="none" aria-hidden="true">
