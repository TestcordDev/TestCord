...
import { actionLabels, defaultKeybinds, getKeybindSettingKey, hotkeySections } from "../utils/keybinds";
...
export function KeybindEditor() {
    const [selectedAction, setSelectedAction] = useState("openCurrent");
    const [recording, setRecording] = useState(false);
    const settingKey = getKeybindSettingKey(selectedAction);
    const [currentBind, setCurrentBind] = useState(() => settings.store[settingKey] || defaultKeybinds[selectedAction]);
    useEffect(() => {
    ...
    return (<div className={cl("keybind-editor")}>
            <div className={cl("keybind-action-select")}>
                <Select options={actionOptions} isSelected={value => value === selectedAction} select={value => setSelectedAction(value)} serialize={value => value}/>
            </div>
            ...
            <div className={cl("keybind-controls")}>
                ...
                <button type="button" className={cl("keybind-record-btn", recording && "keybind-record-active")} onClick={() => setRecording(v => !v)}>
                    {recording ? "Cancel" : "Record Keybind"}
                </button>
                {isModified && (<button type="button" className={cl("keybind-reset-btn")} onClick={() => {
