...
function validateColor(value, key, fallback) {
...
function drawOscilloscope(ctx, w, h, dataArray, bufferLength) {
...
function drawSpectrograph(ctx, w, h, frequencyData, bufferLength) {
...
function Visualizer({ playerRef, src }) {
    ...
    React.useEffect(() => {
        ...
        const draw = () => {
            ...
            if (settings.store.oscilloscope)
                drawOscilloscope(canvasCtx, width, height, dataArray, dataArray.length);
            if (settings.store.spectrograph)
                drawSpectrograph(canvasCtx, width, height, frequencyData, frequencyData.length);
        };
        const onPlay = () => {
    ...
    return (<canvas className={cl("canvas")} ref={canvasRef}/>);
}
...
export default definePlugin({
    ...
    renderVisualizer(player) {
        if (player.props.type !== "AUDIO")
            return null;
