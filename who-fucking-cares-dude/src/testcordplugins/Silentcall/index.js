 ...
 */
function shouldSilence(channelId, channel) {
    if (!channel) {
        debug(`unknown channel ${channelId} — passing through`);
        return false;
    }
    if (channel.type === CHANNEL_TYPE_GROUP_DM)
        return settings.store.silenceGroupCalls;
    if (channel.type === CHANNEL_TYPE_DM)
        return settings.store.silenceDMCalls;
    return false;
}
export default definePlugin({
    ...
    start() {
        ...
        originalRing = ring;
        CallActions.ring = function (channelId, ...rest) {
            let channel;
            try {
