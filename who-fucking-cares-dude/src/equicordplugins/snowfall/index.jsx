...
class CopleSnow {
    ...
    static winHeight = window.innerHeight;
    ...
    static cssPrefixedNames = {
        transform: this.cssPrefix("transform"),
        transition: this.cssPrefix("transition")
    };
    static transitionEndEvent = { WebkitTransition: "webkitTransitionEnd", OTransition: "oTransitionEnd", Moztransition: "transitionend", transition: "transitionend" }[this.cssPrefixedNames.transition ?? "transition"] ?? "transitionend";
    static random(min, max, deviation) {
    ...
    static setStyle(element, rules) {
        for (const [name, value] of Object.entries(rules)) {
            const cssName = CopleSnow.cssPrefixedNames[name] || name;
            element.style[cssName] = value;
        }
    }
    options = { ...CopleSnow.defaultOptions };
    queue = [];
    $snowfield;
    timer = null;
    playing = false;
    ...
    animateSnowflake() {
        ...
        if (this.queue.length > 0) {
            ...
            if (reused.dataset.type !== this.options.type) {
                ...
                if (this.options.type === "text") {
                    ...
                    const { content } = this.options;
                    if (Array.isArray(content)) {
                        snowflake.src = content[Math.floor(Math.random() * content.length)];
                    }
                }
            }
        }
...
const SnowfallManager = () => {
    ...
    React.useEffect(() => {
        ...
        const snowType = settings.store.typeOfSnow;
        const snowOptions = {
            autoplay: false,
            type: snowType,
        ...
        let lastSettings = {
            type: settings.store.typeOfSnow,
            maxSize: settings.store.maxSize,
        ...
        const settingsInterval = setInterval(() => {
            ...
            if (Object.keys(newSettings).some(k => newSettings[k] !== lastSettings[k])) {
                ...
                if (newSettings.type === "image")
                    updateOptions.content = SNOWFLAKE_SVGS;
                else if (newSettings.type === "text")
