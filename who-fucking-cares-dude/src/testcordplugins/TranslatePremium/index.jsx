...
async function googleTranslate(text) {
    ...
    try {
        ...
        if (data.sourceLanguage === skipLang)
            return null;
...
async function translateAndReplace(message) {
    if (!message?.content || !message?.author?.id)
        return;
    if (!isTracked(message.author.id))
        return;
    ...
    if (originalMessages.size >= MAX_ORIGINAL_MESSAGES) {
        ...
        if (oldest !== undefined)
            originalMessages.delete(oldest);
    }
...
function handleMessageCreate({ message, optimistic }) {
    if (optimistic)
        return;
...
function handleMessageDelete({ channelId, id }) {
...
const userContextPatch = (children, props) => {
    ...
    children.push(<Menu.MenuSeparator key="tp-sep"/>, <Menu.MenuItem key="tp-toggle" id="translate-premium-toggle" label={tracked ? "Stop Auto-Translating" : `Auto-Translate to ${targetName}`} action={() => {
            if (tracked) {
...
const messageContextPatch = (children, props) => {
    if (!props?.message)
        return;
    ...
    if (originalMessages.has(props.message.id)) {
        group.splice(group.findIndex(c => c?.props?.id === "copy-text") + 1, 0, (<Menu.MenuItem key="tp-view-original" id="translate-premium-original" label="View Original Message" action={() => {
