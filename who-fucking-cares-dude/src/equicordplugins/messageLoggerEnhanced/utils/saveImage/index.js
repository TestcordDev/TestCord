...
export function isAttachmentGoodToCache(attachment, fileExtension) {
...
export async function cacheMessageImages(message) {
    try {
...
export async function deleteMessageImages(message) {
    for (let i = 0; i < message.attachments.length; i++) {
        const attachment = message.attachments[i];
        await deleteImage(attachment.id);
    }
}
export const getAttachmentBlobUrl = memoize(async (attachment) => {
