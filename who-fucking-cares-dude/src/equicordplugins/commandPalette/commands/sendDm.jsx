...
export const sendDmCommand = {
    ...
    page: () => ({
        ...
        spec: {
            ...
            validate(values, extras) {
                if (!UserStore.getUser(values.recipient))
                    return "Pick a recipient from the suggestions.";
                const files = extras?.files?.message ?? [];
                if (!values.message.trim() && files.length === 0)
                    return "Add a message or a file.";
                return null;
            },
            async submit(values, ctx, extras) {
                ...
                const files = extras?.files?.message ?? [];
                const uploads = files.map(file => new CloudUploader({ file, platform: 1 /* CloudUploadPlatform.WEB */ }, channelId));
