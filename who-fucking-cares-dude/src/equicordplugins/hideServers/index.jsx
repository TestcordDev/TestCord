...
const Patch = (children, { guild }) => {
    const group = findGroupChildrenByChildId("privacy", children);
    if (!group)
        return;
    const isHidden = HiddenServersStore.hiddenGuilds.has(guild.id.toString());
    group.push(<Menu.MenuItem id="vc-hide-server" label={isHidden ? "Unhide Server" : "Hide Server"} action={() => {
            if (isHidden) {
...
export function addIndicator() {
    addServerListElement(2 /* ServerListRenderPosition.Below */, hiddenServersButton);
}
export function removeIndicator() {
    removeServerListElement(2 /* ServerListRenderPosition.Below */, hiddenServersButton);
}
export default definePlugin({
    ...
    contextMenus: {
        ...
        "guild-context": (menuItems, props) => {
            ...
            if ("folderId" in props) {
                ...
                menuItems.push(<Menu.MenuItem id="vc-hide-folder" label={isHidden ? "Unhide Folder" : "Hide Folder"} action={() => {
                        if (isHidden) {
    ...
    useFilteredGuilds(guilds) {
        ...
        return guilds.flatMap(guild => {
            if (!(hiddenGuilds instanceof Set))
                return [guild];
            ...
            newGuild.children = guild.children.filter(child => !hiddenGuilds.has(child.id.toString()) &&
                !(child.type === "folder" && hiddenGuilds.has("folder-" + child.id.toString())));
            return [newGuild];
        });
    },
    filteredGuildResults(results) {
