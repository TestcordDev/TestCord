...
import { ExpressionPickerView, FavouriteItemFormat } from "./types";
...
export default definePlugin({
    ...
    renderTabs(Tab, activeView) {
        return (<>
                <Tab id="gif-picker-tab" key="gif-picker-tab" aria-controls="gif-picker-tab-panel" aria-selected={activeView === ExpressionPickerView.GIF} isActive={activeView === ExpressionPickerView.GIF} viewType={ExpressionPickerView.GIF}>
                    {getIntlMessage("QUICKSEARCH_MEDIA")}
                </Tab>
                <Tab id="files-picker-tab" key="files-picker-tab" aria-controls="files-picker-tab-panel" aria-selected={activeView === ExpressionPickerView.FILES} isActive={activeView === ExpressionPickerView.FILES} viewType={ExpressionPickerView.FILES}>
                    {getIntlMessage("QUICKSEARCH_FILES")}
                </Tab>
            </>);
    },
    renderFilePicker(activeView, onSelectGIF) {
        return activeView === ExpressionPickerView.FILES ? <FilePicker onSelectItem={onSelectGIF}/> : null;
    },
    renderAttachment(children, { item }) {
        if (!hasFavourites())
            return children;
        return <AttachmentContextProvider attachment={item}>{children}</AttachmentContextProvider>;
    },
    renderCV2File(children, key, component) {
        if (!hasFavourites())
            return children;
    ...
    renderEmbedMosaicItem(children, index) {
    ...
    filterGifs: (item) => item.format !== FavouriteItemFormat.NONE,
    interceptAddToFavourites: async (item) => {
        if (item.format !== FavouriteItemFormat.NONE)
            return item;
