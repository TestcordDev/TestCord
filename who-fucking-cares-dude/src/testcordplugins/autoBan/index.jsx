...
function isVoiceChannelOwner(guildId, channelId) {
...
function forceCheckVcOwnership(guildId, channelId) {
    const currentUserId = UserStore.getCurrentUser().id;
    if (!guildId || !channelId) {
...
function banAllUsersInCurrentVc() {
...
function makeUserContextMenuPatch() {
    return (children, props) => {
        ...
        children.splice(-1, 0, <Menu.MenuGroup>
                <Menu.MenuCheckboxItem id="auto-ban" label="Auto-Ban" checked={isChecked} action={async () => {
...
function makeChannelContextMenuPatch() {
    return (children, props) => {
        if (!props || !props.channel || props.channel.type !== 2)
            return;
...
function BulkAutoBanSubmenu() {
    ...
    return (<Menu.MenuItem id="bulk-autoban-submenu" label="Bulk Autoban">
            <Menu.MenuItem id="bulk-autoban-all-vc" label={`Ban All in VC (${usersInVc.length} users)`} color="danger" action={() => banAllUsersInCurrentVc()}/>
            <Menu.MenuSeparator />
            <Menu.MenuItem id="bulk-autoban-status" label={vcOwnerStatus} disabled={true} color={actuallyIsOwner ? "brand" : "default"}/>
        </Menu.MenuItem>);
}
...
function checkExistingUsersInVC(channelId) {
    ...
    if (!Object.keys(voiceStates).includes(currentUserId))
        return;
    const channel = ChannelStore.getChannel(channelId);
    if (!channel?.guild_id || !forceCheckVcOwnership(channel.guild_id, channelId))
        return;
    Object.keys(voiceStates).forEach((userId, index) => {
        if (userId === currentUserId || !bannedUsers.includes(userId))
            return;
        Toasts.show({
...
function EncModals(props) {
    ...
    return (<ModalRoot {...props} size={"dynamic" /* ModalSize.DYNAMIC */}>
            ...
            <ModalContent>
                <TextInput style={{ marginBottom: "10px", minWidth: "600px" }} value={reason} placeholder="Enter the reason for banning this user" onChange={setReason}/>
            </ModalContent>
            <ModalFooter>
...
const cb = async (e) => {
    ...
    if (state?.channelId === state?.oldChannelId)
        return;
    if (!Object.keys(Cvcstates).includes(currentUserId))
        return;
