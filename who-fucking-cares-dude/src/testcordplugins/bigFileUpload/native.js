...
let currentLoggingLevel = "errors";
...
function getEffectiveUploader(fileName, selectedUploader) {
 ...
 */
function navigateJsonPath(obj, pathParts) {
    let current = obj;
 ...
 */
function extractUrlFromResponse(responseText, responseType, urlPath, baseUrl) {
 ...
 */
async function raceToFirstSuccess(promises) {
...
function cancelUploadById(uploadId) {
...
function cancelBackgroundRetries(retries, keepUploadId) {
    for (const retry of retries) {
        if (retry.uploadId !== keepUploadId)
            cancelUploadById(retry.uploadId);
    }
}
/**
...
async function getGoFileUploadUrl() {
 ...
 */
function streamFilePutUpload(url, filePath, fileName, customHeaders = {}, webContents, uploadId, timeout = 300000) {
 ...
 */
function streamFileBinaryCustom(url, filePath, fileName, fileType, method, customHeaders = {}, webContents, uploadId, timeout = 300000) {
    let isCancelled = false;
    return new Promise((resolve, reject) => {
        const urlObj = new URL(url);
        const isHttps = urlObj.protocol === "https:";
 ...
 */
function streamFileUpload(url, filePath, fileName, fileType, fileFieldName, fields, customHeaders = {}, webContents, uploadId, timeout = 300000) {
    ...
    return new Promise((resolve, reject) => {
        ...
        for (const [key, value] of Object.entries(fields)) {
            if (key && key.trim() !== "") {
                preludeChunks.push(Buffer.from(`--${boundary}\r\n` +
 ...
 */
export async function uploadFileToGofileNative(event, filePathOrBuffer, fileName, token, uploadId, timeout = 300000) {
    let filePath;
    ...
    try {
        ...
        const responseText = await streamFileUpload(uploadUrl, filePath, fileName, "application/octet-stream", "file", fields, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
 ...
 */
export async function uploadFileToCatboxNative(event, url, filePathOrBuffer, fileName, fileType, userHash, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const result = await streamFileUpload(url, filePath, fileName, fileType, "fileToUpload", fields, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
 ...
 */
export async function uploadFileToLitterboxNative(event, filePathOrBuffer, fileName, fileType, time, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const result = await streamFileUpload("https://litterbox.catbox.moe/resources/internals/api.php", filePath, fileName, fileType, "fileToUpload", fields, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
 ...
 */
export async function uploadFileToTransferShNative(event, filePathOrBuffer, fileName, maxDays, maxDownloads, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const result = await streamFilePutUpload(`https://transfer.sh/${encodeURIComponent(fileName)}`, filePath, fileName, customHeaders, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
        }
        return result.trim();
 ...
 */
export async function uploadFileToTempShNative(event, filePathOrBuffer, fileName, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const result = await streamFileUpload("https://temp.sh/upload", filePath, fileName, "application/octet-stream", "file", {}, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
 ...
 */
export async function uploadFileToTmpFilesNative(event, filePathOrBuffer, fileName, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const responseText = await streamFileUpload("https://tmpfiles.org/api/v1/upload", filePath, fileName, "application/octet-stream", "file", {}, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        // Parse JSON response
        const result = JSON.parse(responseText);
 ...
 */
export async function uploadFileToFilebinNative(event, filePathOrBuffer, fileName, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        // Upload file to bin
        await streamFileUpload(`https://filebin.net/${binId}/${encodeURIComponent(fileName)}`, filePath, fileName, "application/octet-stream", "file", {}, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
 ...
 * Uses PUT request
 */
export async function uploadFileToBuzzheavierNative(event, filePathOrBuffer, fileName, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const responseText = await streamFilePutUpload(`https://w.buzzheavier.com/${encodeURIComponent(fileName)}`, filePath, fileName, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        if (isTempFile) {
            await deleteTempFile(filePath);
 ...
 */
export async function uploadFileToFileIoNative(event, filePathOrBuffer, fileName, expires, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const responseText = await streamFileUpload(uploadUrl, filePath, fileName, "application/octet-stream", "file", {}, {
            "User-Agent": "VencordBigFileUpload/1.0"
        }, event.sender, uploadId, timeout);
        const result = JSON.parse(responseText);
 ...
 */
export async function uploadFileTo0x0StNative(event, filePathOrBuffer, fileName, expires, uploadId, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    try {
        if (typeof filePathOrBuffer === "string") {
        ...
        const result = await streamFileUpload("https://0x0.st", filePath, fileName, "application/octet-stream", "file", fields, {
 ...
 */
export async function uploadFileCustomNative(event, url, filePathOrBuffer, fileName, fileType, fileFormName, customArgs, customHeaders, responseType, urlPath, uploadId, requestMethod, bodyType, timeout = 300000) {
    let filePath;
    let isTempFile = false;
    ...
    try {
        ...
        if (body === "Binary") {
            ...
            responseText = await streamFileBinaryCustom(url, filePath, fileName, fileType, method, headers, event.sender, uploadId, timeout);
        }
        else {
            ...
            delete headers["content-type"];
            responseText = await streamFileUpload(url, filePath, fileName, fileType, fileFormName, fields, {
                ...headers,
                "User-Agent": "Vencord-BigFileUpload/1.0"
            }, event.sender, uploadId, timeout);
        }
 ...
 */
export async function uploadFileBuffer(event, buffer, fileName, mimeType, uploaderSettings) {
    ...
    try {
        ...
        if (fileSize > MAX_BUFFER_UPLOAD_BYTES)
            throw new Error("File is too large to upload from memory.");
        ...
        const tryUploader = async (uploader, isBackgroundRetry = false, attemptUploadId = uploadId) => {
            let uploadResult;
            switch (uploader) {
                case "GoFile": {
                    const gofileResult = await uploadFileToGofileNative(event, buffer, fileName, uploaderSettings.gofileToken, attemptUploadId, uploaderSettings.uploadTimeout);
                ...
                case "Catbox": {
                    uploadResult = await uploadFileToCatboxNative(event, "https://catbox.moe/user/api.php", buffer, fileName, mimeType || "application/octet-stream", uploaderSettings.catboxUserHash || "", attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "Litterbox": {
                    uploadResult = await uploadFileToLitterboxNative(event, buffer, fileName, mimeType || "application/octet-stream", uploaderSettings.litterboxTime || "1h", attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "temp.sh": {
                    uploadResult = await uploadFileToTempShNative(event, buffer, fileName, attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "0x0.st": {
                    uploadResult = await uploadFileTo0x0StNative(event, buffer, fileName, uploaderSettings.zeroX0Expires, attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "tmpfiles.org": {
                    uploadResult = await uploadFileToTmpFilesNative(event, buffer, fileName, attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "filebin.net": {
                    uploadResult = await uploadFileToFilebinNative(event, buffer, fileName, attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "buzzheavier.com": {
                    uploadResult = await uploadFileToBuzzheavierNative(event, buffer, fileName, attemptUploadId, uploaderSettings.uploadTimeout);
                    break;
                }
                case "Custom": {
                    ...
                    uploadResult = await uploadFileCustomNative(event, uploaderSettings.customUploaderRequestURL, buffer, fileName, mimeType || "application/octet-stream", uploaderSettings.customUploaderFileFormName || "file", customArgs, customHeaders, uploaderSettings.customUploaderResponseType || "Text", urlPath, uploadId, uploaderSettings.customUploaderRequestMethod, uploaderSettings.customUploaderBodyType, uploaderSettings.uploadTimeout);
                    break;
                }
                default:
 ...
 */
export async function pickAndUploadFile(event, uploaderSettings) {
    updateLoggingLevel(uploaderSettings.loggingLevel);
        ...
        try {
            ...
            switch (uploader) {
                case "GoFile": {
                    ...
                    const gofileResult = await uploadFileToGofileNative(event, filePath, fileName, uploaderSettings.gofileToken, uploadId, uploaderSettings.uploadTimeout);
                    if (gofileResult.status === "ok" && gofileResult.data) {
                ...
                case "Catbox": {
                    ...
                    const catboxResult = await uploadFileToCatboxNative(event, "https://catbox.moe/user/api.php", filePath, fileName, "", uploaderSettings.catboxUserHash || "", uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = catboxResult;
                    break;
                }
                case "Litterbox": {
                    nativeLog.debug("[BigFileUpload] Trying Litterbox");
                    const litterboxResult = await uploadFileToLitterboxNative(event, filePath, fileName, "", uploaderSettings.litterboxTime || "1h", uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = litterboxResult;
                    break;
                }
                case "temp.sh": {
                    nativeLog.debug("[BigFileUpload] Trying temp.sh");
                    const tempShResult = await uploadFileToTempShNative(event, filePath, fileName, uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = tempShResult;
                    break;
                }
                case "0x0.st": {
                    nativeLog.debug("[BigFileUpload] Trying 0x0.st");
                    const zeroX0StResult = await uploadFileTo0x0StNative(event, filePath, fileName, uploaderSettings.zeroX0Expires, uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = zeroX0StResult;
                    break;
                }
                case "tmpfiles.org": {
                    nativeLog.debug("[BigFileUpload] Trying tmpfiles.org");
                    const tmpFilesResult = await uploadFileToTmpFilesNative(event, filePath, fileName, uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = tmpFilesResult;
                    break;
                }
                case "filebin.net": {
                    nativeLog.debug("[BigFileUpload] Trying filebin.net");
                    const filebinResult = await uploadFileToFilebinNative(event, filePath, fileName, uploadId, uploaderSettings.uploadTimeout);
                    uploadResult = filebinResult;
                    break;
                }
                case "buzzheavier.com": {
                    nativeLog.debug("[BigFileUpload] Trying buzzheavier.com");
                    const buzzheavierResult = await uploadFileToBuzzheavierNative(event, filePath, fileName, uploadId, uploaderSettings.uploadTimeout);
                ...
                case "Custom": {
                    ...
                    const customHeaders = parseJsonSafe(uploaderSettings.customUploaderHeaders, {});
                    const urlPath = (uploaderSettings.customUploaderURL || "").split(".").filter(s => s);
                    const customResult = await uploadFileCustomNative(event, uploaderSettings.customUploaderRequestURL, filePath, fileName, "", uploaderSettings.customUploaderFileFormName || "file", customArgs, customHeaders, uploaderSettings.customUploaderResponseType || "Text", urlPath, uploadId, uploaderSettings.customUploaderRequestMethod, uploaderSettings.customUploaderBodyType, uploaderSettings.uploadTimeout);
 ...
 */
function sendNotification(_event, notification) {
 ...
 */
export function cancelUpload(_event, uploadId) {
