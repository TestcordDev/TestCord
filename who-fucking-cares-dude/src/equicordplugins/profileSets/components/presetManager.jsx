...
export function PresetManager({ section, guildId }) {
    ...
    const lastRandomIndexRef = React.useRef(-1);
    const resolvedSection = section ?? "main";
    const isServerSection = resolvedSection === "server";
    const lastSelectedGuildId = useStateFromStores([SelectedGuildStore], () => SelectedGuildStore.getLastSelectedGuildId() ?? SelectedGuildStore.getGuildId());
    ...
    const handleLoadPreset = (index) => {
        if (!canUseGuild)
            return;
        applyPreset(index);
    };
    const selectRandomPreset = (sectionType) => {
    ...
    const showImportPrompt = (existingCount) => {
        return new Promise(resolve => {
            openModal(props => (<ImportProfilesModal {...props} title="Import Profiles" message={`You have ${existingCount} existing profiles in this section. Do you want to override them or merge with imported profiles?`} onOverride={() => resolve("override")} onMerge={() => resolve("merge")} onCancel={() => resolve("cancel")}/>));
        });
    };
    const { avatarSize } = settings.store;
    const hasPresets = presets.length > 0;
    const shouldShowPagination = filteredPresets.length > PRESETS_PER_PAGE;
    return (<div className={classes(cl("section"), isServerSection ? cl("section-server") : "")}>
            <Heading tag="h3" className={cl("heading")}>
                Saved Profiles
            </Heading>

            <div className={cl("text")}>
                <TextInput placeholder={searchMode ? "Search profiles..." : "Profile Name"} value={presetName} onChange={setPresetName} className={cl("text-input")}/>
            </div>

            <div className={cl("search")}>
                {!searchMode && (<Button size="small" disabled={isSaving || !presetName.trim() || !canUseGuild} onClick={handleSavePreset} className={cl("search-button")}>
                        {isSaving ? "Saving..." : "Save Profile"}
                    </Button>)}
                {hasPresets && (<Button size="small" variant={searchMode ? "primary" : "secondary"} onClick={() => {
                setSearchMode(!searchMode);
                handlePageChange(1);
            }}>
                        {searchMode ? "Cancel Search" : "Search"}
                    </Button>)}
                <Button size="small" variant="secondary" onClick={handleRandomPreset} disabled={!presets.length || !canUseGuild}>
                    Random
                </Button>
                <Button size="small" variant="secondary" onClick={() => importPresets(forceUpdate, showImportPrompt, resolvedSection, resolvedGuildId)} disabled={!canUseGuild}>
                    Import
                </Button>
            ...
            {hasPresets && (<>
                    <PresetList presets={currentPresets} allPresets={presets} avatarSize={avatarSize} selectedPreset={selectedPreset} onLoad={handleLoadPreset} onUpdate={() => {
                ...
                forceUpdate();
            }} guildId={resolvedGuildId} section={resolvedSection} currentPage={currentPage} onPageChange={handlePageChange}/>

                    {shouldShowPagination && (<div className={cl("pagination")}>
                            <Button size="small" variant="secondary" disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)}>
                                ←
                            </Button>
                            <div className={cl("page")}>
                                <input type="text" value={pageInput} onChange={e => {
                    ...
                    if (!isNaN(num) && num >= 1 && num <= totalPages) {
                        setCurrentPage(num);
                            ...
                            <Button size="small" variant="secondary" disabled={currentPage === totalPages} onClick={() => handlePageChange(currentPage + 1)}>
                                →
                            </Button>
                        </div>)}
