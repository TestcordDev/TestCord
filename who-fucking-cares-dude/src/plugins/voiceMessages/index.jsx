...
const ctxMenuPatch = (children, props) => {
    if (props.channel.guild_id && !(PermissionStore.can(PermissionsBits.SEND_VOICE_MESSAGES, props.channel) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)))
        return;
    children.push(<Menu.MenuItem id="vc-send-vmsg" iconLeft={Microphone} leadingAccessory={{
            type: "icon",
            icon: Microphone
        }} label="Send Voice Message" action={() => openModal(modalProps => <VoiceMessageModal modalProps={modalProps}/>)}/>);
};
...
function generateWaveform(audioBuffer) {
    ...
    const binCount = lodash.clamp(Math.floor(audioBuffer.duration * WAVEFORM_BINS_PER_SECOND), Math.min(WAVEFORM_MIN_BINS, channelData.length), WAVEFORM_MAX_BINS);
...
function VoiceMessageModal({ modalProps }) {
    ...
    return (<Modal {...modalProps} title="Record Voice Message" actions={[{
                text: "Send",
                variant: "primary",
                onClick: () => {
            ...
            <div className={cl("buttons")}>
                <VoiceRecorder setAudioBlob={blob => {
            setBlob(blob);
            setBlobUrl(blob);
            ...
            {isUnsupportedFormat && (<Card variant="warning" className={Margins.top16} defaultPadding>
