...
import { buttons, UserAreaButton } from "@api/UserArea";
...
function NitroBadgeTooltip({ icon, tierName, dateStr, premiumType }) {
    const accentColor = premiumType === 1 ? "#2dc770" : "#a970ff";
    return (<Tooltip text={<div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "12px 16px", gap: 2, position: "relative", overflow: "hidden", minWidth: 120 }}>
            ...
            {(tooltipProps) => (<img {...tooltipProps} src={icon} alt="Nitro" style={{ borderRadius: "50%", width: "22px", height: "22px", cursor: "pointer" }}/>)}
        </Tooltip>);
}
/**
...
function getTargetUser(guildId) {
...
function buildOverrides(active) {
    ...
    if (manualData?.overlaySelf) {
        const ov = {};
        if (target.avatarDecorationData !== undefined)
            ov.avatarDecorationData = target.avatarDecorationData;
        if (target.collectibles !== undefined)
            ov.collectibles = target.collectibles;
        ...
        ov.premiumType = (() => {
            ...
            if (fromProfile != null && fromProfile !== 0)
                return fromProfile;
            if (fromTarget != null && fromTarget !== 0)
                return fromTarget;
            return 2;
        })();
        if (target.accentColor != null)
            ov.accentColor = target.accentColor;
        ov.verified = true;
        ov.isClaimed = () => true;
    ...
    if (target.primaryGuild !== undefined)
        overrides.primaryGuild = target.primaryGuild;
    if (target.avatarDecorationData !== undefined)
        overrides.avatarDecorationData = target.avatarDecorationData;
    ...
    if (target.collectibles !== undefined)
        overrides.collectibles = target.collectibles;
    if (target.displayNameStyles !== undefined)
        overrides.displayNameStyles = target.displayNameStyles;
    ...
    if (isManual) {
        ...
        if (tag && tag.trim() !== "") {
            ...
            const badgeVal = resolveBadge(manualData?.manualClanBadge || settings.store.manualClanBadge || "", manualData?.manualClanBadgeCustom || settings.store.manualClanBadgeCustom || "");
            const manualClan = {
                tag: tag.trim(),
...
function safeMergeUserProfile(original, overrides, userId) {
    if (!original)
...
function mergeUser(base, overrides) {
    if (!base)
        return overrides;
    let proto = Object.prototype;
    try {
...
function cloneWithPremium(user, months) {
    if (!user)
        return user;
    let proto = Object.prototype;
    try {
...
function wrapUser(base) {
    ...
    if (!active || !isActive()) {
        if (isCurrentUser(base.id)) {
            ...
            if (!unclaimed && !needPremium)
                return base;
            let wrapped = wrappedUsers.get(base);
            if (!wrapped) {
                wrapped = base;
                if (unclaimed)
                    wrapped = mergeUser(wrapped, CLAIM_OVERRIDES);
                if (needPremium)
...
let cachedManualWrap = null;
function wrapUserForManualId(base, manualId, active) {
...
function clearWrapCache() {
    wrappedUsers = new WeakMap();
    cachedManualWrap = null;
}
let originalGetUser = null;
let originalGetCurrentUser = null;
let originalSet = null;
let originalGetUserAvatarURL = null;
let originalGetUserBannerURL = null;
let originalGetName = null;
let originalGetGlobalName = null;
let originalGetFormattedName = null;
let originalGetUserTag = null;
let originalGetStatus = null;
let originalGetClientStatus = null;
let originalGetActivities = null;
let originalGetPrimaryActivity = null;
let originalGetUnfilteredActivities = null;
let originalFindActivity = null;
let originalGetApplicationActivity = null;
let originalGetMember = null;
let originalGetNick = null;
...
let switcherDropdownObserver = null;
...
function readSwitcherDropdownOpen() {
    try {
...
function startSwitcherDropdownObserver() {
    if (switcherDropdownObserver || typeof document === "undefined")
        return;
    const { body } = document;
    if (!body) {
    ...
    switcherDropdownObserver = setInterval(() => {
        if (switcherDropdownCheckQueued)
            return;
...
function stopSwitcherDropdownObserver() {
    if (switcherDropdownObserver) {
        clearInterval(switcherDropdownObserver);
...
function isAccountSwitcherCall() {
...
function patchStore() {
    ...
    try {
        _setOriginalGetCurrentUser(originalGetCurrentUser);
    }
    catch { /* ignore */ }
            ...
            if (active) {
                if (userId === originalMeId) {
                    ...
                    if (active.isManual) {
                        const me = originalGetCurrentUser.call(UserStore);
                        if (me)
                            return wrapUserForManualId(me, active.user.id, active);
                    }
                    else {
                ...
                if (typeof url === "string") {
                    ...
                    if (profileMatch) {
                        ...
                        if (userId && isCurrentUser(userId)) {
                            ...
                            return originalGet.call(this, nextOptions).then((res) => {
                                if (res && res.body) {
...
function unpatchStore() {
    ...
    if (memberPatched && GuildMemberStore) {
        ...
        if (originalGetNick)
            GuildMemberStore.getNick = originalGetNick;
        memberPatched = false;
    }
...
function patchUtils() {
    ...
    IconUtils.getUserAvatarURL = function (user, animated, size, format) {
        if (isActive() && user && (isCurrentUser(user.id) || user.id === "0")) {
            ...
            if (active) {
                ...
                const t = active.user;
                if (t)
                    return originalGetUserAvatarURL.call(this, t, animated, size, format);
            }
        }
        return originalGetUserAvatarURL.call(this, user, animated, size, format);
    };
    IconUtils.getUserBannerURL = function (params) {
        if (isActive(params?.guildId ?? params?.guild_id) && params && isCurrentUser(params.id)) {
            ...
            if (active) {
                ...
                if (active.manualData?.overlaySelf) {
                    ...
                    if (ovBanner && !ovBanner.startsWith("#"))
                        return ovBanner;
    ...
    function getManualDisplayName() {
        ...
        return (md?.manualDisplayName
            || md?.manualUsername
            || md?.name
            || active.user?.globalName
            || active.user?.global_name
            || active.user?.username
            || "FakeUser");
    }
    function getManualUsername() {
        const active = getActiveTargetForGuild(undefined);
        if (!active?.isManual)
            return undefined;
        if (active.manualData?.overlaySelf)
            return undefined;
        const md = active.manualData;
        return md?.manualUsername || md?.name || active.user?.username || "FakeUser";
    }
    ...
    UsernameUtils.getName = function (user) {
        if (isActive() && !isOverlayActive() && user && isCurrentUser(user.id)) {
            ...
            const t = getTargetUser();
            if (t)
                return originalGetName.call(this, t);
        }
        return originalGetName.call(this, user);
    };
    UsernameUtils.getGlobalName = function (user) {
        if (isActive() && !isOverlayActive() && user && isCurrentUser(user.id)) {
            const manualName = getManualDisplayName();
            if (manualName != null)
                return manualName;
            const t = getTargetUser();
            if (t)
                return originalGetGlobalName.call(this, t);
        }
        return originalGetGlobalName.call(this, user);
    };
    UsernameUtils.getFormattedName = function (user, useTag) {
        if (isActive() && !isOverlayActive() && user && isCurrentUser(user.id)) {
            const manualName = getManualDisplayName();
            if (manualName != null) {
                if (useTag) {
            ...
            const t = getTargetUser();
            if (t)
                return originalGetFormattedName.call(this, t, useTag);
        }
        return originalGetFormattedName.call(this, user, useTag);
    };
    UsernameUtils.getUserTag = function (user, options) {
        if (isActive() && !isOverlayActive() && user && isCurrentUser(user.id)) {
            const t = getTargetUser();
            if (t) {
                ...
                if (manualName != null || manualUname != null) {
                    const synth = {
                        ...t,
                        username: manualUname ?? t.username,
                        globalName: manualName ?? t.globalName,
                        global_name: manualName ?? t.global_name,
                    };
                    return originalGetUserTag.call(this, synth, options);
                }
...
function unpatchUtils() {
    if (!utilsPatched)
        return;
    if (originalGetUserAvatarURL)
        IconUtils.getUserAvatarURL = originalGetUserAvatarURL;
    if (originalGetUserBannerURL)
        IconUtils.getUserBannerURL = originalGetUserBannerURL;
    if (originalGetName)
        UsernameUtils.getName = originalGetName;
    if (originalGetGlobalName)
        UsernameUtils.getGlobalName = originalGetGlobalName;
    if (originalGetFormattedName)
        UsernameUtils.getFormattedName = originalGetFormattedName;
    if (originalGetUserTag)
        UsernameUtils.getUserTag = originalGetUserTag;
    utilsPatched = false;
}
function getManualActivityListFor(manualData) {
    if (!manualData)
        return [];
...
let _origExtractTimestamp = null;
let _snowflakePatchTimer = null;
function patchSnowflake() {
    if (_origExtractTimestamp)
        return;
    let su = SnowflakeUtils;
    if (!su?.extractTimestamp) {
        try {
            su = window.Vencord?.Webpack?.findByProps?.("extractTimestamp", "fromTimestamp");
        }
        catch { /* ignore */ }
    }
    if (!su?.extractTimestamp) {
    ...
    try {
        ...
        try {
            su.extractTimestamp = wrapped;
        }
        catch { /* ignore */ }
        try {
            Object.defineProperty(su, "extractTimestamp", { value: wrapped, writable: true, configurable: true, enumerable: true });
        }
        catch { /* ignore */ }
    }
    catch { /* ignore */ }
...
function unpatchSnowflake() {
    ...
    if (_origExtractTimestamp) {
        const su = SnowflakeUtils ?? (typeof window !== "undefined" ? window.Vencord?.Webpack?.findByProps?.("extractTimestamp", "fromTimestamp") : null);
        try {
            if (su)
                su.extractTimestamp = _origExtractTimestamp;
        }
        catch { /* ignore */ }
...
function patchPresence() {
    ...
    PresenceStore.getStatus = function (userId, guildId, defaultStatus) {
    ...
    PresenceStore.getClientStatus = function (userId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            if (settings.store.spoofedStatus && settings.store.spoofedStatus !== "none") {
                const status = settings.store.spoofedStatus;
                return { desktop: status, web: status, mobile: status };
            }
            const active = getActiveTargetForGuild(undefined);
            if (active) {
                if (active.isManual) {
    ...
    function getCurrentActivityList(active, guildId) {
    ...
    PresenceStore.getActivities = function (userId, guildId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            const active = getActiveTargetForGuild(guildId);
            if (active) {
    ...
    PresenceStore.getPrimaryActivity = function (userId, guildId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            const active = getActiveTargetForGuild(guildId);
            if (active) {
    ...
    PresenceStore.getUnfilteredActivities = function (userId, guildId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            const active = getActiveTargetForGuild(guildId);
            if (active) {
    ...
    PresenceStore.findActivity = function (userId, predicate, guildId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            const active = getActiveTargetForGuild(guildId);
            if (active) {
    ...
    PresenceStore.getApplicationActivity = function (userId, applicationId, guildId) {
        const id = userId ?? UserStore.getCurrentUser()?.id;
        if (isActive() && id && isCurrentUser(id)) {
            const active = getActiveTargetForGuild(guildId);
            if (active) {
...
function unpatchPresence() {
    ...
    if (originalGetActivities)
        PresenceStore.getActivities = originalGetActivities;
    if (originalGetPrimaryActivity)
        PresenceStore.getPrimaryActivity = originalGetPrimaryActivity;
    if (originalGetUnfilteredActivities)
        PresenceStore.getUnfilteredActivities = originalGetUnfilteredActivities;
    if (originalFindActivity)
        PresenceStore.findActivity = originalFindActivity;
    if (originalGetApplicationActivity)
        PresenceStore.getApplicationActivity = originalGetApplicationActivity;
    presencePatched = false;
}
let originalGetTestcordCustomBadges = null;
let badgesPatched = false;
function getSpoofedCreatedAtMs() {
    const active = getActiveTargetForGuild(undefined);
    if (!active)
        return null;
    if (active.isManual) {
...
function patchBadges() {
    if (badgesPatched)
        return;
    if (typeof BadgeAPIPlugin?.getTestCordCustomBadges !== "function")
        return;
    badgesPatched = true;
    originalGetTestcordCustomBadges = BadgeAPIPlugin.getTestCordCustomBadges.bind(BadgeAPIPlugin);
    BadgeAPIPlugin.getTestCordCustomBadges = function (userId) {
        if (settings.store.spoofBadges && isActive() && isCurrentUser(userId)) {
            const target = getTargetUser();
            if (target && target.id !== "0")
                return originalGetTestcordCustomBadges(target.id);
        }
...
function unpatchBadges() {
    if (!badgesPatched)
        return;
    if (originalGetTestcordCustomBadges)
        BadgeAPIPlugin.getTestCordCustomBadges = originalGetTestcordCustomBadges;
    badgesPatched = false;
}
...
function syncSpoofState() {
    ...
    resolveManualAssets().then(() => {
        if (generation !== switcherGeneration)
            return;
        notifyUpdate();
    });
}
function FakeUserSwitcherIcon({ className, style }) {
    ...
    return (<svg className={className} style={style} width="20" height="20" viewBox="0 0 24 24" fill="none">
...
function FakeUserSwitcherButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    const tooltip = hideTooltips
        ...
        : active
            ...
            : "Fake User Switcher";
    return (<UserAreaButton tooltipText={tooltip} icon={<FakeUserSwitcherIcon className={iconForeground}/>} role="button" plated={nameplate != null} redGlow={active} onClick={() => {
...
const dynamicBadge = {
    ...
    getBadges: () => {
        ...
        if (!target)
            return [];
        const flags = target.publicFlags ?? target.flags ?? 0;
        const badges = [];
        ...
        const premium = target.premiumType ?? 0;
        if (premium >= 1) {
            const nitroLevel = manual?.manualNitroLevel ?? -1;
            ...
            if (nitroLevel != null && nitroLevel >= 1) {
                ...
                const nitroDate = manualSince ?? makeDateInRange(target.id, NITRO_TIER_MONTHS[nitroLevel] ?? 1, NITRO_TIER_MONTHS[nitroLevel + 1] ?? (NITRO_TIER_MONTHS[nitroLevel] ?? 1) + 12);
            ...
            badges.push({
                ...
                component: () => (<NitroBadgeTooltip icon={icon} tierName={tierName} dateStr={dateStr} premiumType={premium}/>),
            });
        }
        // Boost tier badge (manual mode only — derived from the chosen boost tier)
        const boostMonths = manual?.manualBoostMonths ?? -1;
        if (boostMonths >= 0 && boostMonths < BOOST_BADGE_ICONS.length) {
            ...
            const boostDate = manualBoostSince ?? makeDateInRange(target.id, BOOST_MONTHS[boostMonths] ?? 1, BOOST_MONTHS[boostMonths + 1] ?? (BOOST_MONTHS[boostMonths] ?? 1) + 6);
...
function buildFakeMessage(channelId, content, replyMessageReference) {
...
const userContextMenuPatch = (children, { user }) => {
    if (!user || isCurrentUser(user.id))
        return;
    children.push(<Menu.MenuSeparator />, <Menu.MenuItem id="fake-user-switcher-clone" label="Clone User to Fake User Switcher" action={async () => {
...
let multiAccountStore = null;
let switcherRunning = false;
let switcherGeneration = 0;
let originalMultiGetUsers = null;
let originalMultiGetValidUsers = null;
let originalMultiGetHasLoggedInAccounts = null;
let multiAuthModule = null;
let originalSwitchAccount = null;
let switcherClickInterceptor = null;
function isHttpUrl(value) {
    return typeof value === "string" && /^(https?:|data:)/.test(value);
}
function normalizeSwitcherAvatar(user) {
...
function getAvatarUrlForSwitcher(user, size = 80) {
...
function buildSwitcherUser(user, extra) {
    ...
    return {
        ...
        avatarUrl: getAvatarUrlForSwitcher(user),
        getAvatarURL: (_guildId, size) => getAvatarUrlForSwitcher(user, size) ?? undefined,
        tokenStatus: user?.tokenStatus ?? 1,
        pushSyncToken: user?.pushSyncToken ?? null,
        ...CLAIM_OVERRIDES,
    };
}
function getSwitcherActionId(action) {
...
function looksLikeMultiAccountStore(mod) {
    try {
        if (typeof mod?.getUsers !== "function")
            return false;
        if (typeof mod.getValidUsers !== "function" && typeof mod.getHasLoggedInAccounts !== "function")
            return false;
        const users = mod.getUsers();
        if (!Array.isArray(users))
            return false;
        if (typeof mod.getFrequentlyUsedEmojis === "function")
            return false;
        if (!users.length)
            return true;
        const first = users[0];
        ...
        if (!("tokenStatus" in first) && !("pushSyncToken" in first)) {
            if ("type" in first || "permissions" in first || "parentId" in first)
                return false;
        }
        return true;
    }
    ...
    const displayReal = real.map(user => {
        if (realMe && originalId && user?.id === originalId) {
            ...
            return buildSwitcherUser({
                ...
                globalName: realMe.globalName ?? realMe.username,
                global_name: realMe.global_name ?? realMe.globalName ?? realMe.username,
...
function patchInternalAccountSwitcher() {
    if (originalMultiGetUsers)
        return;
    waitFor(["getUsers", "getValidUsers", "getHasLoggedInAccounts"], (store) => {
        if (originalMultiGetUsers || !looksLikeMultiAccountStore(store))
            return;
        ...
        if (store.getHasLoggedInAccounts) {
            store.getHasLoggedInAccounts = () => {
                ...
                if (settings.store.patchInternalAccountSwitcher && getSwitcherAccounts().length)
                    return true;
    ...
    waitFor(["switchAccount"], (module) => {
        multiAuthModule = module;
        originalSwitchAccount = module.switchAccount;
        module.switchAccount = function (id, ...args) {
            if (settings.store.patchInternalAccountSwitcher) {
...
function unpatchInternalAccountSwitcher() {
    if (!multiAccountStore || !originalMultiGetUsers)
        return;
    multiAccountStore.getUsers = originalMultiGetUsers;
    if (originalMultiGetValidUsers)
        multiAccountStore.getValidUsers = originalMultiGetValidUsers;
    if (originalMultiGetHasLoggedInAccounts)
        multiAccountStore.getHasLoggedInAccounts = originalMultiGetHasLoggedInAccounts;
...
function switchRealAccount(id) {
    if (id === getOriginalMeId())
        return false;
    if (getSavedIdentityForSwitcherId(id))
        return false;
    try {
...
async function activateSwitcherIdentity(action) {
    if (!settings.store.patchInternalAccountSwitcher)
        return;
...
function removeSwitcherIdentity(action) {
    const id = getSwitcherActionId(action);
    if (!id)
        return;
    const saved = getSavedIdentityForSwitcherId(id);
...
function getClickedSwitcherIdentity(event) {
    ...
    if (originalId) {
        ...
        if (realMe)
            accounts.unshift(buildSwitcherUser(realMe, { id: originalId, userId: originalId }));
    }
    ...
    for (const account of accounts) {
        ...
        if (optionText.includes(id) || names.some(name => optionText.includes(name)))
            return id;
    }
...
function startSwitcherClickInterceptor() {
    if (switcherClickInterceptor || typeof document === "undefined")
        return;
    switcherClickInterceptor = event => {
        if (!settings.store.patchInternalAccountSwitcher)
            return;
        const id = getClickedSwitcherIdentity(event);
        if (!id || !getSavedIdentityForSwitcherId(id))
            return;
...
let _connectionsListCache = null;
let _connectionsListRaw = null;
function getFakeConnections() {
    const raw = settings.store.fakeConnectionsList || "[]";
    if (raw === _connectionsListRaw && _connectionsListCache)
...
const plugin = definePlugin({
    ...
    userAreaButton: {
        ...
        render: (props) => <FakeUserSwitcherButton {...props}/>,
        priority: -1,
    },
    async start() {
        const generation = ++switcherGeneration;
        ...
        if (settings.store.patchInternalAccountSwitcher) {
            ...
            waitFor(["getUsers", "getValidUsers", "getHasLoggedInAccounts"], (mod) => {
                if (!switcherRunning)
                    return;
    ...
    flux: {
        ...
        MULTI_ACCOUNT_SWITCH_ATTEMPT(action) {
    ...
    handleSwitch(originalFn, userId) {
        if (settings.store.patchInternalAccountSwitcher) {
    ...
    spoofMemberSinceTimestamp(userId) {
        try {
            if (!isActive() || !userId || typeof userId !== "string")
                return undefined;
        ...
        return (user, animated, size) => {
            if (isActive() && (isCurrentUser(user?.id) || user?.id === "0")) {
                ...
                if (active) {
                    ...
                    const t = active.user;
                    if (t)
                        return original(t, animated, size);
                }
            }
    ...
    getAvatarDecorationURL({ user, canAnimate }) {
        if (!isActive())
            return undefined;
    ...
    profileHook(userId, original) {
        ...
        if (activeSpoof && isManual) {
            ...
            if (bannerColor && bannerColor.startsWith("#")) {
                try {
                    ...
                    if (!isNaN(colorVal))
                        overrides.accentColor = colorVal;
                }
                catch { /* ignore */ }
            }
            ...
            if (settings.store.spoofProfileEffect && targetProfile?.profileEffect) {
                ...
                if (targetProfile.profileEffectId != null)
                    overrides.profileEffectId = targetProfile.profileEffectId;
                if (targetProfile.profileEffectExpiresAt !== undefined)
                    overrides.profileEffectExpiresAt = targetProfile.profileEffectExpiresAt;
            }
            ...
            // Cloner mode
            if (targetProfile && targetProfile.bio != null)
                overrides.bio = targetProfile.bio;
            if (targetProfile && targetProfile.pronouns != null)
                overrides.pronouns = targetProfile.pronouns;
            if (targetProfile && targetProfile.themeColors)
                overrides.themeColors = targetProfile.themeColors;
            overrides.banner = targetProfile?.banner ?? targetUser.banner ?? null;
            overrides.accentColor = targetProfile?.accentColor ?? targetUser.accentColor ?? null;
            if (targetProfile && targetProfile.profileEffect)
                overrides.profileEffect = targetProfile.profileEffect;
            if (targetProfile && targetProfile.popoutAnimationParticleType != null)
                overrides.popoutAnimationParticleType = targetProfile.popoutAnimationParticleType;
            if (targetProfile && targetProfile.profileEffectExpiresAt != null)
                overrides.profileEffectExpiresAt = targetProfile.profileEffectExpiresAt;
            const premiumType = targetProfile?.premiumType ?? targetUser.premiumType;
            if (premiumType != null)
                overrides.premiumType = premiumType;
            if (targetProfile && targetProfile.premiumSince != null)
                overrides.premiumSince = targetProfile.premiumSince;
            if (targetProfile && targetProfile.premiumGuildSince != null)
                overrides.premiumGuildSince = targetProfile.premiumGuildSince;
            ...
            if (original) {
                if (original.bio != null)
                    overrides.bio = original.bio;
                if (original.pronouns != null)
                    overrides.pronouns = original.pronouns;
                if (original.themeColors)
                    overrides.themeColors = original.themeColors;
        ...
        if (settings.store.fakeConnectionsEnabled) {
            ...
            if (parsed.length)
                overrides.connectedAccounts = parsed;
        }
        ...
        if (!Array.isArray(overrides.connectedAccounts))
            overrides.connectedAccounts = Array.isArray(original?.connectedAccounts) ? original.connectedAccounts : [];
        if (!Array.isArray(overrides.legacyApplications))
            overrides.legacyApplications = Array.isArray(original?.legacyApplications) ? original.legacyApplications : [];
        if (!Array.isArray(overrides.applicationRoleConnections))
            overrides.applicationRoleConnections = Array.isArray(original?.applicationRoleConnections) ? original.applicationRoleConnections : [];
        if (!Array.isArray(overrides.widgets))
            overrides.widgets = Array.isArray(original?.widgets) ? original.widgets : [];
        if (overrides.badges !== undefined && !Array.isArray(overrides.badges))
            overrides.badges = [];
        // Mirror the userProfile sub-object so the popout's display-name section reflects the target.
        const targetUserProfile = (activeSpoof && !isManual) ? (targetProfile?.userProfile ?? {}) : {};
        ...
        if (settings.store.spoofBadges || selfNitro) {
            ...
            if (Array.isArray(baseBadges) && (spoofingNitroTier || spoofingBoostTier)) {
                baseBadges = baseBadges.filter((b) => {
                    ...
                    if (spoofingNitroTier && isNativeNitro)
                        return false;
                    if (spoofingBoostTier && isNativeBoost)
                        return false;
                    return true;
                });
                overrides.badges = baseBadges;
            }
            const hasNitroBadge = baseBadges.some((b) => b.id === "nitro" || b.id === "fakeuserswitcher-nitro" || b.description === "Discord Nitro" || b.icon === "2ba85e8026a8614b640c2837bcdfe21b");
    ...
    bannerHook({ displayProfile, user }) {
    ...
    onBeforeMessageSend(channelId, msg, options) {
        if (!isActive() || !settings.store.fakeMessages)
            return;
