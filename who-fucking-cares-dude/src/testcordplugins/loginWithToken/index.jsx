...
function LoginWithTokenModal({ rootProps }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent style={{ padding: "16px" }}>
                ...
                <input type="password" autoFocus value={token} onChange={e => setToken(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} placeholder="Paste your token here..." style={{
            ...
            <ModalFooter>
                <button onClick={rootProps.onClose} style={{
...
function tryInject() {
    if (document.getElementById(MOUNT_ID))
        return;
    const emailInput = document.querySelector("input[name='email'], input[type='email'], input[autocomplete='username']");
    if (!emailInput)
        return;
    // Find the "Forgot password" link to inject just below
    let container = emailInput.closest("form");
...
function startObserver() {
    ...
    observer = new MutationObserver(() => {
        ...
        requestAnimationFrame(() => {
            queued = false;
            if (!observer || document.getElementById(MOUNT_ID))
                return;
            tryInject();
            if (document.getElementById(MOUNT_ID)) {
                observer.disconnect();
