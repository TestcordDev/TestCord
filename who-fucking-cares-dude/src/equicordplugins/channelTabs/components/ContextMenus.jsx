...
const normalizeFolderColor = (color) => {
    if (!color)
        return bookmarkFolderColors.Black;
    return legacyFolderColors[color] ?? color;
};
const colorToInt = (color) => {
    const normalized = normalizeFolderColor(color);
    if (!/^#([0-9a-f]{6})$/i.test(normalized))
        return parseInt(bookmarkFolderColors.Black.slice(1), 16);
    return parseInt(normalized.slice(1), 16);
};
const intToColor = (value) => value == null
    ? bookmarkFolderColors.Black
    : `#${value.toString(16).padStart(6, "0")}`;
export function BasicContextMenu() {
    const { showBookmarkBar } = settings.use(["showBookmarkBar"]);
    return (<Menu.Menu navId="channeltabs-context" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="ChannelTabs Context Menu">
            <Menu.MenuGroup>
                <Menu.MenuCheckboxItem checked={showBookmarkBar} id="show-bookmark-bar" label="Bookmark Bar" action={() => {
            settings.store.showBookmarkBar = !settings.store.showBookmarkBar;
        }}/>
            </Menu.MenuGroup>
        </Menu.Menu>);
}
function FolderGlyph({ color, iconName }) {
    ...
    return (<div style={{
            ...
            {IconComponent
            ? <IconComponent height={customIconSize} width={customIconSize} color={resolvedColor} fill={fill} style={{ transform: "scale(0.6666667)", transformOrigin: "center" }}/>
...
function FolderChipPreview({ name, color, iconName }) {
    return (<div style={{
            alignItems: "center",
...
function FolderIconPickerModal({ modalProps, modalKey, name, color, iconName, onSelect, onColorChange }) {
    ...
    return (<Modal {...modalProps} size="sm" title={<BaseText size="lg" weight="semibold">Choose Folder Icon</BaseText>} actions={[
            {
                text: "Close",
                variant: "secondary",
            ...
            <div className={Margins.top8}>
                <ColorPicker color={colorToInt(localColor)} onChange={value => {
            ...
            overflowY: "auto"
                ...
                {filteredIconNames.map(name => (<button key={name} onClick={() => {
                onSelect(name);
                closeModal(modalKey);
            }} style={{
                alignItems: "center",
...
function FolderAppearanceFields({ name, setName, color, setColor, iconName, setIconName, placeholder = "Folder" }) {
    return <>
        ...
        <div className={Margins.top8}>
            <ColorPicker color={colorToInt(color)} onChange={value => setColor(intToColor(value))} showEyeDropper={false}/>
        </div>
        <Heading className={Margins.top16}>Folder Icon</Heading>
        <FolderChipPreview name={name} color={color} iconName={iconName}/>
        <Button className={Margins.top8} onClick={() => {
            const key = openModal(modalProps => (<FolderIconPickerModal modalProps={modalProps} modalKey={key} name={name} color={color} iconName={iconName} onSelect={setIconName} onColorChange={setColor}/>));
        }}>Choose Icon</Button>
        {iconName && <Button className={Margins.top8} color={Button.Colors.TRANSPARENT} look={Button.Looks.FILLED} onClick={() => setIconName(undefined)}>Use Default Icon</Button>}
    </>;
}
export function EditModal({ modalProps, modalKey, bookmark, onSave }) {
    ...
    return (<Modal {...modalProps} size="sm" title={<BaseText size="lg" weight="semibold">Edit Bookmark</BaseText>} actions={[
            {
                text: "Save",
                variant: "primary",
            ...
            ? <FolderAppearanceFields name={name} setName={setName} color={color} setColor={setColor} iconName={iconName} setIconName={setIconName} placeholder={placeholder}/>
            : <>
...
function AddToFolderModal({ modalProps, modalKey, bookmarks, onSave }) {
    ...
    const [folderColor, setFolderColor] = useState(bookmarkFolderColors.Black);
    const [iconName, setIconName] = useState();
    return (<Modal {...modalProps} size="sm" title={<BaseText size="lg" weight="semibold">Add Bookmark to Folder</BaseText>} actions={[
            {
                text: "Save",
                variant: "primary",
            ...
            <Select options={[...Object.entries(bookmarks)
            ...
            })),
            {
                ...
                value: -1,
                default: true
            }]} isSelected={v => v === folderIndex} select={setIndex} serialize={String}/>
            {folderIndex === -1 && <FolderAppearanceFields name={folderName} setName={setFolderName} color={folderColor} setColor={setFolderColor} iconName={iconName} setIconName={setIconName}/>}
        </Modal>);
}
function DeleteFolderConfirmationModal({ modalProps, modalKey, onConfirm }) {
    return (<Modal {...modalProps} size="sm" title={<BaseText size="lg" weight="semibold">Are you sure?</BaseText>} actions={[
            {
                text: "Delete",
...
export function BookmarkContextMenu({ bookmarks, index, methods }) {
    ...
    return (<Menu.Menu navId="channeltabs-bookmark-context" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="ChannelTabs Bookmark Context Menu">
            <Menu.MenuGroup>
                {bookmarkNotificationDot && !isFolder &&
            <Menu.MenuItem id="mark-as-read" label={getIntlMessage("MARK_AS_READ")} disabled={!ReadStateStore.hasUnread(bookmark.channelId)} action={() => ReadStateUtils.ackChannel(ChannelStore.getChannel(bookmark.channelId))}/>}
                {isFolder
            ? <Menu.MenuItem id="open-all-in-folder" label={"Open All Bookmarks"} action={() => bookmark.bookmarks.forEach(b => createTab(b))}/>
            : <Menu.MenuItem id="open-in-tab" label={"Open in New Tab"} action={() => createTab(bookmark)}/>}
            </Menu.MenuGroup>
            <Menu.MenuGroup>
                <Menu.MenuItem id="edit-bookmark" label="Edit Bookmark" action={() => {
            const key = openModal(modalProps => <EditModal modalProps={modalProps} modalKey={key} bookmark={bookmark} onSave={(name, color, iconName) => {
                ...
                <Menu.MenuItem id="delete-bookmark" label="Delete Bookmark" action={() => {
            if (isFolder) {
                const key = openModal(modalProps => <DeleteFolderConfirmationModal modalProps={modalProps} modalKey={key} onConfirm={() => {
                ...
                <Menu.MenuItem id="add-to-folder" label="Add Bookmark to Folder" disabled={isFolder} action={() => {
            const key = openModal(modalProps => <AddToFolderModal modalProps={modalProps} modalKey={key} bookmarks={bookmarks} onSave={(index, folderName, folderColor, iconName) => {
                    if (index === -1) {
                        const folderIndex = methods.addFolder(folderName, normalizeFolderColor(folderColor), iconName);
                        methods.addBookmark(bookmark, folderIndex);
                    }
                    else
                        methods.addBookmark(bookmark, index);
...
export function TabContextMenu({ tab }) {
    const channel = ChannelStore.getChannel(tab.channelId);
    const [compact, setCompact] = useState(tab.compact);
    const { showBookmarkBar } = settings.use(["showBookmarkBar"]);
    return (<Menu.Menu navId="channeltabs-tab-context" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="ChannelTabs Tab Context Menu">
            <Menu.MenuGroup>
                {channel &&
            <Menu.MenuItem id="mark-as-read" label={getIntlMessage("MARK_AS_READ")} disabled={!ReadStateStore.hasUnread(channel.id)} action={() => ReadStateUtils.ackChannel(channel)}/>}
                <Menu.MenuCheckboxItem checked={compact} id="toggle-compact-tab" label="Compact" action={() => {
            ...
            {openedTabs.length !== 1 && <Menu.MenuGroup>
                ...
                <Menu.MenuItem id="close-other-tabs" label="Close Other Tabs" action={() => closeOtherTabs(tab.id)}/>
                <Menu.MenuItem id="close-right-tabs" label="Close Tabs to the Right" disabled={openedTabs.indexOf(tab) === openedTabs.length - 1} action={() => closeTabsToTheRight(tab.id)}/>
                <Menu.MenuItem id="close-left-tabs" label="Close Tabs to the Left" disabled={openedTabs.indexOf(tab) === 0} action={() => closeTabsToTheLeft(tab.id)}/>
                <Menu.MenuItem id="reopen-closed-tab" label="Reopen Closed Tab" disabled={!hasClosedTabs()} action={() => reopenClosedTab()}/>
            </Menu.MenuGroup>}
            <Menu.MenuGroup>
                <Menu.MenuCheckboxItem checked={showBookmarkBar} id="show-bookmark-bar" label="Bookmark Bar" action={() => {
            settings.store.showBookmarkBar = !settings.store.showBookmarkBar;
