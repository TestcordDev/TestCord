...
export function setNavigationSource(guildId, channelId, source) {
...
export function isNavigationFromSource(guildId, channelId, source) {
    if (!lastNavigationContext)
        return false;
    ...
    const matches = normalizedContextGuildId === normalizedGuildId &&
...
export function createTab(props, switchToTab, messageId, save = true, bypassOneTabPerServer = false) {
...
export function closeTab(id) {
    ...
    if (i === -1)
        return logger.error("Couldn't find channel tab with ID " + id, openTabs);
...
export function closeOtherTabs(id) {
    ...
    if (tab.channelId !== lastTab.channelId)
        moveToTab(id);
    else
        update();
}
export function closeTabsToTheRight(id) {
    ...
    if (!tabsToTheLeft.some(v => v.id === currentlyOpenTab))
        moveToTab(openTabs.at(-1).id);
    else
        update();
}
export function closeTabsToTheLeft(id) {
    const i = openTabs.findIndex(v => v.id === id);
    if (i === -1)
        return logger.error("Couldn't find channel tab with ID " + id, openTabs);
    const tabsToTheLeft = openTabs.filter((_, ind) => ind < i);
    ...
    if (!tabsToTheRight.some(v => v.id === currentlyOpenTab))
        moveToTab(openTabs[0].id);
    else
        update();
}
let lastNavigationTime = 0;
export function handleChannelSwitch(ch) {
...
export function isTabSelected(id) {
...
export function moveDraggedTabs(index1, index2) {
    ...
    if (!firstItem)
        return logger.error(`Tab at index ${index1} is undefined`, openTabs);
...
function cacheCurrentTabState() {
    if (!settings.store.renderAllTabs)
        return;
...
export async function openStartupTabs(props, setUserId) {
    const { userId } = props;
    ...
    if (settings.store.onStartup === "remember" && !keepCurrentChannel) {
        try {
            const persistedTabs = await DataStore.get("ChannelTabs_openChannels_v2");
            if (generation !== hydrationGeneration)
                return;
...
export function saveTabs(userId) {
    if (!userId)
    ...
    saveQueue = saveQueue
        .then(() => DataStore.update("ChannelTabs_openChannels_v2", old => ({
        ...(old ?? {}),
        [userId]: snapshot
    })))
        .catch(error => {
...
export function setUpdaterFunction(fn) {
    update = fn;
    return () => {
        if (update === fn)
            update = unsetUpdate;
...
export function toggleCompactTab(id) {
    const i = openTabs.findIndex(v => v.id === id);
    if (i === -1)
        return logger.error("Couldn't find channel tab with ID " + id, openTabs);
    openTabs[i] = {
...
export function useGhostTabs() {
    ...
    return new Array(count).fill(<div className={cl("tab", "ghost-tab")}/>);
}
