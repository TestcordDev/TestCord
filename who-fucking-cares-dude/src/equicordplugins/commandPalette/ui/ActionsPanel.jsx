...
function PanelRow({ action, selected, index, onSelect, onRun }) {
    ...
    return (<div ref={ref} className={cl("panel-row", { "panel-row-selected": selected })} onMouseMove={() => { if (!selected)
        onSelect(index); }} onClick={() => onRun(action)}>
...
export function ActionsPanel({ actions, selectedIndex, onSelect, onRun }) {
    return (<div className={cl("panel")}>
            <div className={cl("panel-title")}>Actions</div>
            <div className={cl("panel-list")}>
                {actions.map((action, i) => (<PanelRow key={action.id} action={action} index={i} selected={i === selectedIndex} onSelect={onSelect} onRun={onRun}/>))}
            </div>
        </div>);
}
