...
const NativeHelper = VencordNative.pluginHelpers.TestcordHelper;
import plugins, { ExcludedPlugins, PluginMeta } from "~plugins";
import { hexToInt, ICON_COLOR_FALLBACK, IconColorSettings, intToHex, isIconColorInputValid } from "./iconColors";
const logger = new Logger("TestcordHelper");
let prevOnerror = null;
let crashGuardsActive = false;
function crashRejectionHandler(e) {
    e.preventDefault();
    logger.warn("Suppressed unhandled promise rejection:", e.reason);
}
function installCrashGuards() {
    if (crashGuardsActive)
        return;
...
let origSubscribe = null;
let disposeDispatch = null;
let disposeSubscribe = null;
const dispatchStats = new Map();
const pluginDispatchStats = new Map();
let channelSwitchStart = 0;
const SLOW_DISPATCH_MS = 16;
const RECENT_EVENTS_LIMIT = 200;
const recentSlowEvents = [];
let pluginFluxMap;
...
function getPluginsForEvent(type) {
    ...
    for (const name in plugins) {
        ...
        for (const eventType in flux) {
            ...
            if (typeof handler !== "function")
                continue;
            handlerPluginMap.set(handler, name);
        }
    }
}
/** Per-plugin milliseconds spent inside the dispatch currently on the stack. */
const currentDispatchCost = new Map();
function wrapHandlerTiming(handler) {
 ...
 */
function describeDispatchBlame(type, totalMs) {
...
function dumpPatchDiagnostics() {
    ...
    if (all.size === 0) {
        ...
        for (const [plugin, entry] of all) {
            ...
            for (const f of entry.patchFailures) {
                ...
                if (f.moduleId !== undefined)
                    parts.push(`moduleId: ${String(f.moduleId)}`);
                if (f.error)
                    parts.push(`error: ${f.error}`);
                console.log(parts.join(" | "));
                if (f.sourceContext)
                    console.log(`sourceContext: ${f.sourceContext}`);
            }
    ...
    if (allPatches.length) {
        ...
        for (const patch of allPatches) {
            console.log(`${patch.plugin} | find: ${String(patch.find)} | replacements: ${patch.replacement.map(r => String(r.match)).join(" ; ")}`);
        }
        console.groupEnd();
    }
    console.groupEnd();
...
function dumpFullReport() {
    ...
    const snapshot = {
        ...
        pendingPatches: allPatches.map(patch => ({
            ...
            matches: patch.replacement.map(replacement => String(replacement.match))
        })),
        health,
        ...
        wrap: next => {
            ...
            return function (payload) {
                ...
                if (dt > SLOW_DISPATCH_MS) {
                    ...
                    if (recentSlowEvents.length > RECENT_EVENTS_LIMIT)
                        recentSlowEvents.shift();
...
const RenderEmbeds = getUserSettingLazy("textAndImages", "renderEmbeds");
...
const pluginResolveCache = new Map();
...
function IconColorRow({ settingKey }) {
    ...
    return (<div style={{ display: "grid", gap: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", gap: 12, alignItems: "center" }}>
                <TextInput value={value} placeholder="Theme default" onChange={newValue => settings.store[settingKey] = newValue}/>
                <ColorPicker color={hexToInt(value) ?? hexToInt(ICON_COLOR_FALLBACK) ?? 0xb5bac1} onChange={color => {
            if (color != null)
                settings.store[settingKey] = intToHex(color);
        }} showEyeDropper={true}/>
            </div>
        </div>);
}
function IconColorSettingsComponent() {
    return (<div style={{ display: "grid", gap: 16 }}>
            ...
            {Object.keys(IconColorSettings).map(settingKey => (<IconColorRow key={settingKey} settingKey={settingKey}/>))}
        </div>);
}
let pluginSearchData;
function round2(n) {
...
function getClient() {
    ...
    if (IS_VESKTOP)
        return `Vesktop v${VesktopNative.app.getVersion()}`;
    if (IS_EQUIBOP) {
    ...
    if ("legcord" in window)
        return `LegCord v${window.legcord.version}`;
    if ("goofcord" in window)
        return `GoofCord v${window.goofcord.version}`;
    return typeof window.unsafeWindow !== "undefined" ? "UserScript" : "Web";
}
async function buildDebugReport() {
    ...
    const info = {
        Testcord: `v${globalThis.VERSION} • ${gitHashShort} — ${Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(globalThis.BUILD_TIMESTAMP)}`,
    ...
    const flags = {
        ...
        ">2 Weeks Outdated": globalThis.BUILD_TIMESTAMP < Date.now() - 12096e5,
    };
...
async function sendDebugReport() {
    ...
    const isApi = (name) => name.endsWith("API") || plugins[name]?.required;
...
function ChatPluginCard({ pluginName, description }) {
    ...
    if (excludedPlugin || !p) {
        ...
        const card = (<AddonCard name={pluginName} description={description || toolTipText} enabled={false} setEnabled={() => { }} disabled={true} infoButton={<WarningIcon />}/>);
        return description
    ...
    if (required) {
        ...
        return (<Tooltip text={tooltipText} key={p.name}>
                {({ onMouseLeave, onMouseEnter }) => <PluginCard key={p.name} onMouseLeave={onMouseLeave} onMouseEnter={onMouseEnter} onRestartNeeded={onRestartNeeded} plugin={p} disabled/>}
            </Tooltip>);
    }
    return (<PluginCard key={p.name} onRestartNeeded={onRestartNeeded} plugin={p}/>);
}
function resolvePluginName(search) {
    if (isPluginCardCacheEnabled()) {
        const cacheKey = search.toLowerCase();
        if (pluginResolveCache.has(cacheKey))
            return pluginResolveCache.get(cacheKey) ?? undefined;
        ...
        if (pluginResolveCache.size > PLUGIN_RESOLVE_CACHE_LIMIT) {
            ...
            if (oldest !== undefined)
                pluginResolveCache.delete(oldest);
        }
        return pluginName;
    }
    return resolvePluginNameOriginal(search);
}
function resolvePluginNameOriginal(search) {
...
function resolvePluginNameCached(search) {
...
function getPluginLink(pluginName) {
...
function resolveUser(search) {
    ...
    if (userId)
        return UserStore.getUser(userId) ?? undefined;
    ...
    if (userResolveCache.size > USER_RESOLVE_CACHE_LIMIT) {
        ...
        if (oldest !== undefined)
            userResolveCache.delete(oldest);
    }
    return user;
}
...
function getProfileCardTheme(profile) {
...
function ChatProfileCard({ user }) {
    ...
    return (<div style={{
            ...
            <Avatar src={user.getAvatarURL(null, 80, true)} size="SIZE_56"/>
            ...
            <Button size={Button.Sizes.SMALL} onClick={() => openUserProfile(user.id)}>
                Profile
            </Button>
        </div>);
}
function replacePluginAliases(content) {
...
function replaceUserAliases(content) {
    if (settings.store.disableProfilePopoutEmbeds)
        return content;
...
const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }) {
    if (isPerformanceEnabled() && settings.store.performanceDisablePluginCards)
        return null;
    if (!PLUGIN_CARD_MARKER_PATTERN.test(message.content))
        return null;
    ...
    while ((match = PLUGIN_PATTERN.exec(message.content)) !== null) {
        ...
        pluginCards.push(<ChatPluginCard key={pluginName} pluginName={pluginName}/>);
    }
    ...
    if (pluginCards.length === 0)
        return null;
    return (<div className="vc-plugins-management-cards vc-plugins-grid" style={{ marginTop: "0px" }}>
            {pluginCards}
        </div>);
}, { noop: true });
const ProfileCards = ErrorBoundary.wrap(function ProfileCards({ message }) {
    if (settings.store.disableProfilePopoutEmbeds)
        return null;
    if (!USER_CARD_MARKER_PATTERN.test(message.content))
        return null;
    ...
    while ((match = USER_PATTERN.exec(message.content)) !== null) {
        ...
        profileCards.push(<ChatProfileCard key={user.id} user={user}/>);
    }
    ...
    if (profileCards.length === 0)
        return null;
    return (<div style={{ display: "grid", gap: 8, marginTop: 0 }}>
            {profileCards}
        </div>);
}, { noop: true });
let liveFixInterval = null;
...
function installConsoleCapture() {
    ...
    for (const level of levels) {
        ...
        console[level] = function (...args) {
            ...
            if (consoleBuf.length > CONSOLE_BUF_MAX)
                consoleBuf.shift();
...
function handleLiveFixRequest(req) {
    const { id, action } = req;
    try {
        switch (action) {
            case "search": {
                if (!req.query)
                    return { id, error: "Missing query" };
                const factories = wreq?.m;
                if (!factories)
                    return { id, error: "Webpack factories not available" };
                const results = [];
            ...
            case "readModule": {
                if (req.moduleId === undefined)
                    return { id, error: "Missing moduleId" };
                const factory = wreq?.m?.[req.moduleId];
                if (!factory)
                    return { id, error: `Module ${req.moduleId} not found` };
                return { id, source: String(factory) };
            }
            case "eval": {
                if (!req.code)
                    return { id, error: "Missing code" };
                const result = (0, eval)(req.code);
                return { id, result: typeof result === "object" ? JSON.stringify(result, null, 2) : String(result) };
            }
            case "testPattern": {
                if (!req.pattern || !req.code)
                    return { id, error: "Missing pattern or code" };
            ...
            case "listPending": {
                const pending = allPatches.map(patch => ({
                    plugin: patch.plugin,
                    find: String(patch.find),
                    matches: patch.replacement.map(r => String(r.match))
                }));
                return { id, pending };
            }
            case "patchHealth": {
            ...
            case "dispatchStats": {
                if (!origDispatch)
                    return { id, error: "Dispatch profiling is off. Send {action:'profile'} first, then interact with the client." };
            ...
            case "slowEvents": {
                if (!origDispatch)
                    return { id, error: "Dispatch profiling is off. Send {action:'profile'} first." };
            ...
            case "memory": {
                const mem = window.performance?.memory;
                if (!mem)
                    return { id, memory: null, note: "performance.memory unavailable" };
                return {
                    id,
                    memory: {
            ...
            case "profile": {
                if (origDispatch)
                    return { id, profiling: true, note: "Already profiling." };
                installDebugInstrumentation();
...
export default definePlugin({
    ...
    renderMessageAccessory(props) {
        ...
        if (!showPluginCards && !showProfileCards)
            return null;
        return (<>
    ...
    start() {
        if (settings.store.preventCrashes)
            installCrashGuards();
