...
function createServerBoostEmbed(boostTier = 1, boosterId) {
...
function createClydeEmbed(message) {
    return {
        type: "rich",
        description: message,
        color: 0x2F3136,
...
function createDiscordSystemEmbed(title, message, systemType = "announcement") {
    const color = {
...
function createAutoModEmbed(rule, action, username) {
    return {
        type: "rich",
...
function createPurchaseNotificationEmbed(item, price, username) {
    return {
        type: "rich",
    ...
    commands: [
        {
            ...
            execute: async (args, ctx) => {
                try {
                    ...
                    const boostTier = parseInt(args.find(x => x.name === "tier")?.value) || 1;
