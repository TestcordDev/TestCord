...
const Native = VencordNative.pluginHelpers.ClipUpload;
const defaultFileName = "clip.mp4";
const compatibleExtensions = [".mp4", ".m4v"];
const uploadAbortControllers = new Set();
...
export function getClipTitleFromName(name) {
    const extensionStart = name.lastIndexOf(".");
    return extensionStart > 0 ? name.slice(0, extensionStart) : name;
}
export function getDefaultClipTitle(clip) {
    return typeof clip?.name === "string" ? clip.name.trim() : "";
}
export function getClipCreatedAt(clip) {
...
export function getParticipantIds(clip) {
    if (!Array.isArray(clip?.users))
        return [];
...
function getTempVideoFileName(fileName) {
    if (compatibleExtensions.includes(getFileExtension(fileName)))
        return fileName;
    return `${getClipTitleFromName(fileName) || defaultFileName}.mp4`;
}
async function readStampedVideoFile(token, name, type) {
    const tmpToken = await Native.createTempVideoFile(token);
    if (!tmpToken)
        throw new Error("Couldn't prepare the selected file.");
    try {
...
async function updateTempVideoMetadata(tmpToken) {
    ...
    if (!isClipMetadataUpdater(mediaEngine))
        throw new Error("Couldn't update clip metadata.");
    await mediaEngine.updateClipMetadata(tmpPath, "{}");
}
async function stampVideoFile(file, fileName) {
...
function isClipMetadataUpdater(value) {
    return isObject(value) && typeof value.updateClipMetadata === "function";
}
export async function pickClipFile(parseFileMetadata) {
...
function parseAttachmentUploadResponse(response) {
    if (isObject(response.body))
        return response.body;
    return parseJSON(response.text) ?? {};
}
async function reserveClipUpload(options, file) {
...
async function uploadReservedClip(attachment, uploadFile) {
...
async function sendClipUpload(uploadFile, options) {
...
function getSearchableErrorText(error) {
...
function shouldRetryWithFFmpeg(error) {
...
export async function uploadClipFile(file, options) {
    try {
        ...
        try {
            ...
            if (!shouldRetryWithFFmpeg(error))
                throw error;
