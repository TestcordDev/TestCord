...
export let colors = {};
(async () => {
    colors = await get(DATASTORE_KEY) || {};
})();
...
const userContextMenuPatch = (children, { user }) => {
    if (user?.id == null)
        return;
    const setCustomColorItem = (<Menu.MenuItem label="Set Color" id="set-color" icon={ColorIcon} action={async () => {
...
export function getCustomColorString(id, withHash) {
    if (!id)
        return;
    if (!colors[id] || !Settings.plugins.CustomUserColors.enabled)
        return;
    if (withHash)
        return `#${colors[id]}`;
    return colors[id];
...
export default definePlugin({
    ...
    wrapMessageColorProps(colorProps, context) {
        try {
            ...
            if (colorString === colorProps.colorString)
                return colorProps;
            if (!settings.store.colorInServers && !isDM)
                return colorProps;
            return {
                ...colorProps,
                colorString,
