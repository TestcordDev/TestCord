...
let overlayCtxHandler = null;
let overlayMouseHandler = null;
// tracks what's already been unlocked this session
const unlockedGuilds = new Set();
...
function onActivity() {
    ...
    if (mins > 0)
        inactiveTimer = setTimeout(lockFull, mins * 60_000);
}
...
function lock() {
    unbindActivity();
    if (!document.getElementById("vcl-overlay"))
        createOverlay();
}
function unlock() {
    ...
    if (pendingGuildId) {
        unlockedGuilds.add(pendingGuildId);
        pendingGuildId = null;
    }
    if (pendingChannelId) {
        unlockedChannels.add(pendingChannelId);
        pendingChannelId = null;
    }
    if (pendingUserId) {
        unlockedUsers.add(pendingUserId);
        pendingUserId = null;
    }
    if (overlay) {
...
// ─── Helpers ──────────────────────────────────────────────────────────
let cachedParsedIds = new Set();
let cachedParsedIdsRaw;
function parseIds(raw) {
    if (raw === cachedParsedIdsRaw)
...
function onChannelSelect({ guildId, channelId }) {
    ...
    if (guildId && parseIds(settings.store.lockedGuilds).has(guildId)) {
        if (once && unlockedGuilds.has(guildId))
            return;
        ...
        try {
            ...
            if (channel?.isDM?.() || channel?.isGroupDM?.()) {
                ...
                if (recipientId && parseIds(settings.store.lockedUsers).has(recipientId)) {
                    if (once && unlockedUsers.has(recipientId))
                        return;
...
function createOverlay() {
    ...
    const input = overlay.querySelector("#vcl-input");
    const submitBtn = overlay.querySelector("#vcl-submit");
    const errorBox = overlay.querySelector("#vcl-error");
    const errorMsg = overlay.querySelector("#vcl-error-msg");
    let attempts = 0;
    let lockUntil = 0;
    ...
    function tryUnlock() {
        ...
        if (input.value === settings.store.password) {
            unlock();
            return;
        }
        attempts++;
    ...
    focusGuard = (e) => {
        if (!overlay || e.target === input)
            return;
        e.stopImmediatePropagation();
        requestAnimationFrame(() => input.focus());
    };
    document.addEventListener("focusin", focusGuard, true);
    overlayCtxHandler = (e) => { if (e.target !== input)
        e.preventDefault(); };
    overlayMouseHandler = (e) => { if (e.target === overlay) {
        e.preventDefault();
        input.focus();
    } };
...
export default definePlugin({
    ...
    start() {
        ...
        if (document.readyState === "complete")
            ...
            window.addEventListener("load", lockFull, { once: true });
    },
    stop() {
