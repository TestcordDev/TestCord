...
export function stripTransientRenderState(message) {
...
export function sanitizeForIDB(obj, seen = new WeakSet()) {
...
export function cleanupMessage(message, removeDetails = true) {
    const ret = typeof message.toJS === "function" ? message.toJS() : { ...message };
...
export function cleanUpCachedMessage(message) {
...
export function cleanupEmbed(embed) {
    ...
    if (!embed.id)
        return embed; /* already cleaned */
    const retEmbed = {};
    if (typeof embed.rawTitle === "string")
        retEmbed.title = embed.rawTitle;
    if (typeof embed.rawDescription === "string")
        retEmbed.description = embed.rawDescription;
    if (typeof embed.referenceId !== "undefined")
        retEmbed.reference_id = embed.referenceId;
    // if (typeof embed.color === "string") retEmbed.color = ZeresPluginLibrary.ColorConverter.hex2int(embed.color);
    if (typeof embed.type !== "undefined")
        retEmbed.type = embed.type;
    if (typeof embed.url !== "undefined")
        retEmbed.url = embed.url;
    ...
    if (typeof embed.footer === "object")
        retEmbed.footer = { text: embed.footer.text, icon_url: embed.footer.iconURL, proxy_icon_url: embed.footer.iconProxyURL };
    if (typeof embed.author === "object")
        retEmbed.author = { name: embed.author.name, url: embed.author.url, icon_url: embed.author.iconURL, proxy_icon_url: embed.author.iconProxyURL };
...
// stolen from mlv2
export function cleanupUserObject(user) {
