...
export default definePlugin({
    ...
    handleVoiceConnect(...args) {
        if (!settings.store.bypassWhenJoining)
            return;
    ...
    handleBlockedOrIgnoredUserVoiceChannelJoin(...args) {
        if (!settings.store.bypassWhenUserJoins)
            return;
        const userId = args[1];
    ...
    shouldShowBlockedProfilesFor(userId) {
        ...
        if (shouldShow)
            this.fetchBlockedProfile(userId);
        return shouldShow;
    },
    fetchBlockedProfile(userId) {
        if (pendingProfileFetches.has(userId) || UserProfileStore.getUserProfile(userId) != null)
            return;
