...
class BlazingOpossumCipher {
    key;
    static BLOCK_SIZE = 16; // 128-bit blocks
    static KEY_SIZE = 32; // 256-bit key
    static IV_SIZE = 16; // 128-bit IV
    static TAG_SIZE = 16; // 128-bit Poly-hash Tag
    static ROUNDS = 20; // Increased rounds for Quantum resistance
    roundKeys;
    constructor(key) {
        this.key = key;
        if (key.length !== 32) {
    ...
    readUint32LE(arr, offset) {
        return (arr[offset] |
            (arr[offset + 1] << 8) |
            (arr[offset + 2] << 16) |
            (arr[offset + 3] << 24)) >>> 0;
    }
    writeUint32LE(arr, offset, value) {
        arr[offset] = value & 0xFF;
        arr[offset + 1] = (value >>> 8) & 0xFF;
        arr[offset + 2] = (value >>> 16) & 0xFF;
        arr[offset + 3] = (value >>> 24) & 0xFF;
    }
    generateKeystreamBlock(ivLow, ivHigh, counter) {
...
let cipher = null;
const EncryptionEnabledIcon = ({ height = 20, width = 20, className }) => {
...
const EncryptionDisabledIcon = ({ height = 20, width = 20, className }) => {
    return (<svg width={width} height={height} viewBox="0 0 24 24" className={className}>
            <path fill="currentColor" d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"/>
        </svg>);
};
// Chatbar button
const EncryptionToggleButton = ({ channel, type }) => {
    ...
    if (!validChat || settings.store.location !== "chatbar")
        return null;
    // Only show button when plugin is activated
    if (!pluginActivated) {
        return (<ChatBarButton tooltip="Activate Securecord Opossum Plugin" onClick={() => {
    ...
    return (<ChatBarButton tooltip={encryptionEnabled ? "Disable Encryption" : "Enable Encryption"} onClick={() => {
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("SecurecordOpossum", () => (<HeaderBarButton icon={() => settings.store.encryptionEnabled ? <EncryptionEnabledIcon /> : <EncryptionDisabledIcon />} tooltip={settings.store.encryptionEnabled ? "Disable Encryption" : "Enable Encryption"} onClick={() => { settings.store.encryptionEnabled = !settings.store.encryptionEnabled; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("SecurecordOpossum", () => (<ChannelToolbarButton icon={() => settings.store.encryptionEnabled ? <EncryptionEnabledIcon /> : <EncryptionDisabledIcon />} tooltip={settings.store.encryptionEnabled ? "Disable Encryption" : "Enable Encryption"} onClick={() => { settings.store.encryptionEnabled = !settings.store.encryptionEnabled; }}/>), 5);
        }
    ...
    stop() {
        ...
        if (this._listener) {
            removeMessagePreSendListener(this._listener);
        }
        // Clean up cipher
        cipher = null;
        ...
        async MESSAGE_CREATE({ optimistic, type, message, channelId }) {
            ...
            if (message.content.startsWith("🔒ENCRYPTED:") && message.content.endsWith(":ENDLOCK")) {
                ...
                try {
                    ...
                    // Show error message
                    sendBotMessage(channelId, {
                        content: `🔒 Decryption error for message from ${message.author.username}. Details: ${error.message}`
                    });
                }
