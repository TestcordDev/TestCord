...
import { clearLogs, getLogs, loadLogs, subscribe } from "../store";
const statusOptions = [
    { label: "All", value: "all" },
    ...
    return (<SettingsTab>
            <div style={{ display: "flex", gap: "12px", marginBottom: "16px", alignItems: "center" }}>
                <div style={{ width: "160px" }}>
                    <Select options={statusOptions} isSelected={v => v === statusFilter} select={v => setStatusFilter(v)} serialize={v => v} placeholder="Status"/>
                </div>
                <div style={{ width: "160px" }}>
                    <Select options={typeOptions} isSelected={v => v === typeFilter} select={v => setTypeFilter(v)} serialize={v => v} placeholder="Type"/>
                </div>
                <button className="vc-btn-base vc-btn-small vc-settings-theme-links-reset" style={{ marginLeft: "auto" }} onClick={() => { if (confirm("Clear all logs?"))
        void clearLogs(); }}>
                    Clear Logs
                </button>
            </div>

            {filtered.length === 0 ? (<div style={{ textAlign: "center", color: "var(--text-muted)", padding: "32px" }}>
                    No logs yet.
                </div>) : (<div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    {filtered.map(log => (<div key={log.id} style={{
                    ...
                    if (!log.channelId || !log.messageId)
                        return;
