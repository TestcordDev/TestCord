...
export const Code = ({ theme, useHljs, lang, content, tokens, }) => {
    let lines;
    if (useHljs) {
        try {
        ...
        const renderTokens = tokens ??
            content
                .split("\n")
                .map(line => [{ color: theme.plainColor, content: line }]);
        lines = renderTokens.map((line, idx) => {
            ...
            return (<>
                    {line.map(({ content, color, fontStyle }, i) => (<span key={i} style={{
                        color,
    ...
    const codeTableRows = lines.map((line, i) => (<tr className={cl("table-row")} key={i}>
