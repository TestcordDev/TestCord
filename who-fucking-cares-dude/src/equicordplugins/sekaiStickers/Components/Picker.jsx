...
export default function CharSelectModal({ modalProps, setCharacter }) {
    ...
    const memoedSearchChar = React.useMemo(() => {
        ...
        return characters.map((c, index) => {
            if (s === c.id ||
                ...
                return (<img key={index} onClick={() => { modalProps.onClose(); setCharacter(index); }} src={`https://st.ayaka.one/img/${c.img}`} srcSet={`https://st.ayaka.one/img/${c.img}`} loading="lazy"/>);
            }
            return null;
        });
    }, [search, characters]);
    return (<Modal {...modalProps} size="lg" title="Select character menu">
            <Flex flexDirection="column" style={{ paddingTop: 12 }}>
                <TextInput content="mafuyu" placeholder="Mafuyu" onChange={(e) => setSearch(e)}/>
