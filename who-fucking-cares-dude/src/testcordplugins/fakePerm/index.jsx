...
function ensureDomObserver() {
    if (domInterval || !isEnabled || typeof document === "undefined")
        return;
    domInterval = setInterval(() => {
        if (!isEnabled)
            return;
        if (fakeNicks.size === 0 && disconnectedUsers.size === 0 && kickedUsers.size === 0)
            return;
        if (domTimer)
            return;
        domTimer = setTimeout(() => { domTimer = null; if (isEnabled)
            pluginSelf?.applyDomOverrides(); }, 150);
    }, 3000);
}
...
const deafenedUsers = new Map();
const fakeNicks = new Map();
...
function getCurrentGuildId() {
    try {
        return findByProps("getGuildId", "getLastSelectedGuildId")?.getGuildId?.() ?? null;
    }
    catch {
        return null;
    }
...
function notifyMemberListChange() {
    ...
    try {
        ...
        const voiceStates = findByProps("getVoiceStatesForChannel", "getVoiceStateForUser");
        const myId = (() => { try {
...
function hideMessageInDOM(messageId) {
    let msgEl = document.querySelector(`[id$="-${messageId}"]`) ??
    ...
    setTimeout(() => {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (item.getAttribute("data-fp-hidden") === "true")
                continue;
            if (!isDateSeparatorEl(item))
                continue;
            let hasVisible = false;
            for (let j = i + 1; j < items.length; j++) {
                if (isDateSeparatorEl(items[j]))
                    break;
                if (items[j].getAttribute("data-fp-hidden") !== "true") {
                    hasVisible = true;
                    break;
                }
            }
...
function getMember(guildId, userId) {
    if (!guildId)
        return null;
    try {
        return findByProps("getMember", "getMembers")?.getMember?.(guildId, userId) ?? null;
    }
    catch {
        return null;
    }
}
function isUserInVoice(userId, guildId) {
    if (!guildId)
        return false;
    try {
        const vs = findByProps("getVoiceStateForUser")?.getVoiceStateForUser?.(userId);
...
function getVoiceChannelId(userId, guildId) {
    if (!guildId)
        return null;
    try {
        return findByProps("getVoiceStateForUser")?.getVoiceStateForUser?.(userId)?.channelId ?? null;
    }
    catch {
        return null;
    }
...
function getGuildRoles(guildId) {
    ...
    try {
        ...
        try {
            ...
            if (!guild?.roles)
                return [];
            const vals = Object.values(guild.roles);
            const filtered = [];
            for (const r of vals) {
                if (r.id !== guildId)
                    filtered.push(r);
...
function getMemberRoleIds(guildId, userId) {
    if (!guildId)
        return [];
    try {
        return GuildMemberStore?.getMember?.(guildId, userId)?.roles ?? [];
    }
    catch {
        return getMember(guildId, userId)?.roles ?? [];
    }
}
function toast(msg) {
    try {
        const T = findByProps("show", "pop");
        T?.show?.({ message: msg, type: 1, id: T.genId?.() ?? String(Date.now()) });
    }
    catch { }
}
function findVoiceUserElement(userId) {
    const byAttr = document.querySelector(`[data-user-id="${userId}"]`);
    if (byAttr)
        return byAttr;
    const voiceUsers = document.querySelectorAll("[class*='voiceUser'], [class*='VoiceUser']");
    for (const el of voiceUsers) {
        for (const img of el.querySelectorAll("img")) {
            if (img.src.includes(userId))
                return el;
        }
    }
    return null;
...
// ─── Common Styles ───────────────────────────────────────────────────────────
const modalTitle = { fontFamily: "var(--font-display)", fontSize: "20px", fontWeight: 600, lineHeight: "24px", color: "#ffffff", margin: 0, padding: 0 };
const footerStyle = { display: "flex", gap: "24px", padding: "16px" };
function footerBtn(bg) { return { flex: 1, fontFamily: "var(--font-primary)", fontSize: "14px", fontWeight: 500, height: "38px", background: bg, color: "#ffffff", border: "none", borderRadius: "8px", cursor: "pointer" }; }
function textareaStyle(height = "100px") { return { fontFamily: "var(--font-primary)", width: "100%", height, background: "#383a40", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "8px", padding: "10px 12px", color: "#ffffff", fontSize: "16px", fontWeight: 400, lineHeight: "22px", outline: "none", resize: "none", boxSizing: "border-box", overflowY: "auto", scrollbarWidth: "thin" }; }
function sectionLabel(mb = "8px") { return { fontFamily: "var(--font-primary)", fontSize: "16px", fontWeight: 600, color: "#ffffff", marginBottom: mb }; }
// ─── Modals ───────────────────────────────────────────────────────────────────
function RenameModal({ rootProps, user, guildId }) {
    const member = getMember(guildId, user.id);
    const [nick, setNick] = React.useState(fakeNicks.get(user.id) ?? member?.nick ?? user.username ?? "");
    function applyNick() {
        const trimmed = nick.trim();
        if (trimmed)
            fakeNicks.set(user.id, trimmed);
        else
            fakeNicks.delete(user.id);
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent style={{ padding: "0 16px 20px" }}>
                ...
                <input value={nick} onChange={e => setNick(e.target.value)} autoFocus maxLength={32} onKeyDown={e => { if (e.key === "Enter")
        applyNick(); }} style={{ width: "100%", background: "#383a40", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "8px", padding: "10px 12px", color: "#ffffff", fontFamily: "var(--font-primary)", fontSize: "16px", outline: "none", boxSizing: "border-box" }}/>
            </ModalContent>
            <ModalFooter>
                <div style={{ display: "flex", gap: "12px", width: "100%", padding: "16px" }}>
                    <button onClick={rootProps.onClose} style={footerBtn("#4e5058")}>Cancel</button>
                    <button onClick={applyNick} style={footerBtn("#5865f2")}>Apply</button>
                </div>
            </ModalFooter>
        </ModalRoot>);
}
function KickModal({ rootProps, user, guildId }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent style={{ padding: "8px 16px 20px" }}>
                ...
                <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="" style={{ width: "100%", height: "120px", background: "var(--input-background, #1e1f22)", border: "1px solid var(--background-tertiary, #1e1f22)", borderRadius: "4px", padding: "10px", color: "#ffffff", fontFamily: "var(--font-primary)", fontSize: "14px", lineHeight: "20px", resize: "none", outline: "none", boxSizing: "border-box" }}/>
            </ModalContent>
            <ModalFooter>
                <div style={{ display: "flex", gap: "8px", width: "100%", padding: "16px" }}>
                    <button onClick={rootProps.onClose} style={footerBtn("#4e5058")}>Cancel</button>
                    <button onClick={() => { kickedUsers.add(user.id); disconnectedUsers.add(user.id); notifyMemberListChange(); toast(`@${tag} kicked (local)`); rootProps.onClose(); }} style={footerBtn("#da373c")}>
                        Kick
                    </button>
                </div>
            </ModalFooter>
...
function BanModal({ rootProps, user }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent style={{ padding: "0 16px 20px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
                    {BAN_REASONS.map(opt => (<label key={opt.value} style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer", fontFamily: "var(--font-primary)", fontSize: "16px", color: "#ffffff", userSelect: "none" }} onClick={() => setReason(opt.value)}>
                            <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, border: reason === opt.value ? "6px solid #5865f2" : "2px solid #4e5058", background: reason === opt.value ? "#fff" : "transparent", boxSizing: "border-box" }}/>
                            {opt.label}
                        </label>))}
                </div>
                {reason === "other" && <TextArea value={customReason} onChange={(v) => setCustomReason(v)} rows={3} style={{ marginBottom: "16px" }}/>}
                <div style={sectionLabel()}>Delete messages</div>
                <Select options={DELETE_OPTIONS} select={(v) => setDeleteValue(v)} isSelected={(v) => v === deleteValue} serialize={(v) => v} maxVisibleItems={5} closeOnSelect={true}/>
            </ModalContent>
            <ModalFooter>
                <div style={{ display: "flex", gap: "12px", width: "100%", padding: "16px", justifyContent: "flex-end" }}>
                    ...
                    <Button look={Button.Looks.FILLED} color={Button.Colors.RED} onClick={() => { if (!reason)
        return toast("Select a reason"); bannedUsers.add(user.id); kickedUsers.add(user.id); disconnectedUsers.add(user.id); notifyMemberListChange(); toast(`@${user.username} banned (local)`); rootProps.onClose(); }}>Ban</Button>
                </div>
            </ModalFooter>
        </ModalRoot>);
}
...
function TimeoutModal({ rootProps, user }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            <ModalContent style={{ padding: "8px 16px 20px" }}>
                ...
                <div style={{ display: "flex", marginBottom: "20px", borderRadius: "4px", overflow: "hidden", border: "1px solid var(--background-modifier-accent, rgba(255,255,255,0.1))" }}>
                    {TIMEOUT_DURATIONS.map((d, i) => (<button key={i} onClick={() => setSelectedIdx(i)} style={{
                flex: 1,
                ...
                <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Enter a reason. It will only be visible in server logs and this member won't be able to see it." style={{ width: "100%", height: "100px", background: "var(--input-background, #1e1f22)", border: "1px solid var(--background-tertiary, #1e1f22)", borderRadius: "4px", padding: "10px", color: "#ffffff", fontFamily: "var(--font-primary)", fontSize: "14px", lineHeight: "20px", resize: "none", outline: "none", boxSizing: "border-box" }}/>
            </ModalContent>
            <ModalFooter>
                <div style={{ display: "flex", gap: "8px", width: "100%", padding: "16px" }}>
                    <button onClick={rootProps.onClose} style={footerBtn("#4e5058")}>Cancel</button>
...
function AddRoleModal({ rootProps, user, guildId }) {
    ...
    return (<ModalRoot {...rootProps} size="small">
            ...
            return (<div key={role.id} onClick={() => { toast(`Role ${role.name} — simulation`); rootProps.onClose(); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", cursor: "pointer", opacity: memberRoleIds.includes(role.id) ? 0.5 : 1 }} onMouseEnter={e => (e.currentTarget.style.background = "var(--background-modifier-hover)")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
...
function findGroupWithItem(children, itemIds) {
    for (let i = 0; i < children.length; i++) {
        ...
        for (const child of sub) {
            if (child?.props?.id && itemIds.includes(child.props.id))
                return i;
        }
    }
    return -1;
}
const messageContextPatch = (children, { message }) => {
    if (!children || !Array.isArray(children) || !isEnabled || !message?.id)
        return;
    try {
        ...
        if (!guildId)
            return;
        const hasDelete = children.some((g) => {
            const sub = Array.isArray(g?.props?.children) ? g.props.children : [];
            return sub.some((c) => c?.props?.id === "delete-message");
        });
        children.splice(-1, 0, (<Menu.MenuGroup key="fp-msg-group">
                <Menu.MenuItem key="fp-delete-msg" id="fp-delete-msg" label={hasDelete ? "Delete for me (fake)" : "Delete message"} color="danger" action={() => { deletedMessages.add(message.id); hideMessageInDOM(message.id); toast("Message deleted (local)"); }}/>
            </Menu.MenuGroup>));
    }
    catch (e) {
...
const userContextPatch = (children, { user }) => {
    if (!children || !Array.isArray(children) || !isEnabled || !user)
        return;
    try {
        const guildId = getCurrentGuildId();
        if (!guildId)
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (!group?.props?.children)
                continue;
            const sub = Array.isArray(group.props.children)
                ? group.props.children
                : [group.props.children];
            const filtered = sub.filter((child) => !HIDDEN_IDS.has(child?.props?.id ?? ""));
        ...
        const groupA = (<Menu.MenuGroup key="fp-group-a">
                ...
                : [...allRoles.map(role => {
                        ...
                        const color = role.color ? `#${role.color.toString(16).padStart(6, "0")}` : "#80848e";
                        return (<Menu.MenuItem key={`fp-role-${role.id}`} id={`fp-role-${role.id}`} label={role.name} action={() => { }} render={() => (<div style={{ display: "flex", alignItems: "center", padding: "8px 10px", gap: 8, width: "100%", boxSizing: "border-box", cursor: "pointer" }}>
                    ...
                    <Menu.MenuItem key="fp-role-add" id="fp-role-add" label="+ Add a role" action={() => openModal(p => <AddRoleModal rootProps={p} user={user} guildId={guildId}/>)} render={() => <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderTop: "1px solid rgba(79,84,92,0.48)", color: "#b9bbbe", fontSize: 13, cursor: "pointer" }}><span>+</span><span>Add a role</span></div>}/>,
                ]}
                </Menu.MenuItem>
                ...
                const allChannels = [];
                try {
                    const gc = GuildChannelStore?.getChannels?.(guildId) ?? {};
                    const va = [...(gc.VOCAL ?? []), ...(gc[2] ?? []), ...(gc[13] ?? [])];
                    if (va.length === 0)
                        for (const arr of Object.values(gc)) {
                            if (Array.isArray(arr))
                                for (const item of arr) {
                                    const ch = item.channel ?? item;
                                    if ((ch?.type === 2 || ch?.type === 13) && ch.id && ch.name)
                                        va.push(item);
                                }
                        }
                    const seen = new Set();
                    for (const item of va) {
                        const ch = item.channel ?? item;
                        if (ch?.id && ch?.name && !seen.has(ch.id)) {
                            seen.add(ch.id);
                            allChannels.push({ id: ch.id, name: ch.name, position: ch.position ?? 0 });
                        }
                    }
                }
                catch { }
                ...
                if (allChannels.length === 0)
                    return <Menu.MenuItem key="fp-move-empty" id="fp-move-empty" label="No voice channels" disabled/>;
        ...
        if (idxBlock >= 0)
            children.splice(idxBlock + 1, 0, groupA);
        else
            children.splice(-1, 0, groupA);
    }
    catch (e) {
...
export default definePlugin({
            ...
            onChange(v) {
                ...
                if (!isEnabled) {
                    ...
                    document.querySelectorAll("[data-fp-hidden='true']").forEach(el => {
                        el.style.display = "";
                        el.removeAttribute("data-fp-hidden");
                    });
                    mutedUsers.clear();
                    deafenedUsers.clear();
                    fakeNicks.clear();
    ...
    applyDomOverrides() {
        ...
        for (const [userId, fakeNick] of fakeNicks) {
            document.querySelectorAll(`[data-user-id="${userId}"]`).forEach(el => {
                const nickEl = el.querySelector("[class*='nick'], [class*='Nick'], [class*='username'], [class*='Username']");
                if (nickEl && nickEl.dataset.fpOriginal === undefined)
                    nickEl.dataset.fpOriginal = nickEl.textContent ?? "";
                if (nickEl && nickEl.dataset.fpNick !== fakeNick) {
                    nickEl.dataset.fpNick = fakeNick;
                    nickEl.textContent = fakeNick;
                }
            });
        }
        for (const userId of disconnectedUsers) {
            document.querySelectorAll(`[class*='voiceUser'] [data-user-id="${userId}"], [class*='VoiceUser'] [data-user-id="${userId}"]`).forEach(el => {
                const voiceEl = el.closest("li, [class*='voiceUser'], [class*='VoiceUser']");
                if (voiceEl && voiceEl.getAttribute("data-fp-hidden") !== "true")
                    fpHide(voiceEl);
            });
        }
        for (const userId of kickedUsers) {
            document.querySelectorAll(`[data-user-id="${userId}"]`).forEach(el => {
                const memberEl = el.closest("li, [class*='member'], [class*='Member']");
                if (memberEl && memberEl.getAttribute("data-fp-hidden") !== "true")
                    fpHide(memberEl);
            });
        }
    },
    async start() {
        ...
        try {
            const S = Vencord?.Settings?.plugins?.FakePerm;
