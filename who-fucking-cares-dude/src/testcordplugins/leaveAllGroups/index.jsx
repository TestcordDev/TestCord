...
function confirmLeaveAll(groupCount) {
    if (!settings.store.confirmBeforeLeave)
        return true;
    return confirm(`⚠️ Are you sure you want to leave all ${groupCount} groups?\n\n` +
...
const GroupContextMenuPatch = (children, { channel }) => {
    ...
    if (container) {
        container.push(<Menu.MenuItem id="vc-leave-all-groups" label="🚪 Leave all groups" action={leaveAllGroups} color="danger"/>);
    }
};
...
const ServerContextMenuPatch = (children, props) => {
    ...
    if (group) {
        group.push(<Menu.MenuItem id="vc-leave-all-groups-server" label="🚪 Leave all groups" action={leaveAllGroups} color="danger"/>);
    }
};
...
const UserContextMenuPatch = (children, props) => {
    ...
    if (container) {
        container.push(<Menu.MenuItem id="vc-leave-all-groups-user" label="🚪 Leave all groups" action={leaveAllGroups} color="danger"/>);
    }
};
