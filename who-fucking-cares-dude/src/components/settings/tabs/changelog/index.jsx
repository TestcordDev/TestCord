...
import { clearChangelogHistory, clearIndividualLog, formatTimestamp, getChangelogHistory, getCommitsSinceLastSeen, getLastRepositoryCheckHash, getNewPlugins, getNewSettings, getNewSettingsEntries, getNewSettingsSize, getUpdatedPlugins, initializeChangelog, saveUpdateSession, } from "./changelogManager";
import { NewPluginsCompact, NewPluginsSection } from "./NewPluginsSection";
function ChangelogCard({ entry, repo, repoPending, }) {
    return (<Card className="vc-changelog-entry">
            <div style={{
            display: "flex",
...
function UpdateLogCard({ log, repo, repoPending, isExpanded, onToggleExpand, onClearLog, }) {
    const isRepositoryFetch = log.type === "repository_fetch" ||
        (log.type === undefined &&
            log.fromHash === log.toHash &&
            log.commits.length === 0);
    const isUpToDate = log.fromHash === log.toHash;
    return (<Card className="vc-changelog-log">
            ...
            : `Update: ${log.fromHash.slice(0, 7)} → ${log.toHash.slice(0, 7)}`}
                        ...
                        <Button size="min" variant="secondary" className="vc-changelog-delete-button" style={{
            padding: "4px",
            ...
            ` • ${getNewSettingsEntries(log.newSettings).reduce((sum, [, arr]) => sum + arr.length, 0)} new settings`}
                ...
                <div className={`vc-changelog-log-toggle ${isExpanded ? "expanded" : ""}`}>
                    ▼
                </div>
            </div>

            {isExpanded && (<div className="vc-changelog-log-content">
                    {log.newPlugins.length > 0 && (<div className="vc-changelog-log-plugins">
                            <NewPluginsCompact newPlugins={log.newPlugins} maxDisplay={50}/>
                        </div>)}

                    {log.updatedPlugins.length > 0 && (<div className="vc-changelog-log-plugins">
                    ...
                    {log.newSettings &&
                getNewSettingsSize(log.newSettings) > 0 && (<div className="vc-changelog-log-plugins">
                                <Heading className={Margins.bottom8}>
                                    New Settings
                                </Heading>
                                <div className="vc-changelog-new-plugins-list">
                                    {getNewSettingsEntries(log.newSettings).map(([pluginName, settings]) => settings.map(setting => (<span key={`${pluginName}-${setting}`} className="vc-changelog-new-plugin-tag" title={`New setting in ${pluginName}`}>
                                                    {pluginName}.{setting}
                                                </span>)))}
                                </div>
                            </div>)}

                    {log.commits.length > 0 && (<div className="vc-changelog-log-commits">
                            <div className="vc-changelog-log-commits-list">
                                {log.commits.map(entry => (<ChangelogCard key={entry.hash} entry={entry} repo={repo} repoPending={repoPending}/>))}
                            </div>
                        </div>)}
                </div>)}
        </Card>);
...
function ChangelogContent() {
    ...
    const [changelogHistory, setChangelogHistory] = React.useState([]);
    const [newPlugins, setNewPlugins] = React.useState([]);
    const [updatedPlugins, setUpdatedPlugins] = React.useState([]);
    ...
    React.useEffect(() => {
        const checkRecentStatus = async () => {
            try {
                ...
                if (updates.ok) {
                    const currentRepoHash = updates.value.length > 0
    ...
    const fetchChangelog = React.useCallback(async () => {
        ...
        try {
            ...
            const updates = await VencordNative.updater.getUpdates();
            const lastRepoCheck = await getLastRepositoryCheckHash();
            const currentRepoHash = updates.ok && updates.value.length > 0
                ? updates.value[0].hash
                : gitHash;
    ...
    const hasCurrentChanges = changelog.length > 0 ||
        newPlugins.length > 0 ||
    ...
    return (<>
            ...
            <div className="vc-changelog-controls">
                <Button size="small" disabled={isLoading || repoPending || !!repoErr} onClick={fetchChangelog} variant={recentlyChecked ? "positive" : "primary"}>
                    {isLoading
            ? "Loading..."
            : recentlyChecked
                ...
                {changelogHistory.length > 0 && (<>
                        <Button size="small" variant={showHistory ? "primary" : "secondary"} onClick={() => setShowHistory(!showHistory)} style={{ marginLeft: "8px" }}>
                            {showHistory ? "Hide Logs" : "Show Logs"}
                        </Button>
                        <Button size="small" variant="dangerPrimary" onClick={() => {
                Alerts.show({
            ...
            {error && (<ErrorCard style={{ padding: "1em", marginTop: "1em" }}>
            ...
            <Paragraph color="text-subtle">
                {repoPending ? (repo) : repoErr ? ("Failed to retrieve - check console") : (<Link href={repo}>
            ...
            {hasCurrentChanges && (<>
                    ...
                    {newPlugins.length > 0 && (<div className={Margins.bottom16}>
                            <NewPluginsSection newPlugins={newPlugins} onPluginToggle={() => { }}/>
                        </div>)}

                    {updatedPlugins.length > 0 && (<div className={Margins.bottom16}>
                            <Heading className={Margins.bottom8}>
                    ...
                    {changelog.length > 0 && (<div>
                            ...
                            <div className="vc-changelog-commits-list">
                                {changelog.map(entry => (<ChangelogCard key={entry.hash} entry={entry} repo={repo} repoPending={repoPending}/>))}
                            </div>
                        </div>)}
                </>)}
            ...
            {showHistory && changelogHistory.length > 0 && (<>
                    ...
                    <div className="vc-changelog-history-list">
                        {changelogHistory.map(log => (<UpdateLogCard key={log.id} log={log} repo={repo} repoPending={repoPending} isExpanded={expandedLogs.has(log.id)} onToggleExpand={() => toggleLogExpanded(log.id)} onClearLog={logId => {
                    Alerts.show({
                        title: "Clear Log",
                        ...
                        onConfirm: async () => {
                            ...
                            await loadChangelogHistory();
                            setExpandedLogs(new Set(Array.from(expandedLogs).filter(id => id !== logId)));
                            Toasts.show({
