...
const store = createPersistedValue("customCommands", []);
const KIND_OPTIONS = [
...
function runCustomCommand(data) {
    switch (data.kind) {
        case "url":
...
function commandForm(existing) {
    const spec = {
        ...
        validate(values) {
            ...
            if (kind === "url" && !parseUrl(values.url))
            ...
            if (kind === "message" && !values.text.trim())
