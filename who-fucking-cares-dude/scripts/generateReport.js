...
const logStderr = (...data) => console.error(`${CANARY ? "CANARY" : "STABLE"} ---`, ...data);
...
async function maybeGetError(handle) {
    return await handle?.getProperty("message")
        .then(m => m?.jsonValue())
        .catch(() => undefined);
}
;
...
function toCodeBlock(s, indentation = 0, isDiscord = false) {
    s = s.replace(/```/g, "`\u200B`\u200B`");
...
async function printReport() {
    ...
    if (process.env.WEBHOOK_URL) {
        const patchesToEmbed = (title, patches, color) => ({
            title,
            color,
            description: patches.map(p => {
                ...
                if (p.error)
                    lines.push(`Error: ${toCodeBlock(p.error, "Error: ".length, true)}`);
                return lines.join("\n");
            }).join("\n\n"),
        });
        const embeds = [
        ...
        await fetch(process.env.WEBHOOK_URL, {
            ...
            if (!res.ok)
                logStderr(`Webhook failed with status ${res.status}`);
            else
                logStderr("Posted to Webhook successfully");
        });
    }
}
page.on("console", async (e) => {
    ...
    async function getText(skipFirst = true) {
        ...
        if (skipFirst)
            args = args.slice(1);
        try {
            return await Promise.all(args.map(async (a) => {
    ...
    if (isReporterMeta) {
        metaData = await rawArgs[1].jsonValue();
        return;
    }
    outer: if (isEquicord) {
        try {
...
page.on("pageerror", (e) => {
    if (e.message.includes("Sentry successfully disabled"))
        return;
    ...
    if (e.message.includes("fetchTrackData"))
        return;
