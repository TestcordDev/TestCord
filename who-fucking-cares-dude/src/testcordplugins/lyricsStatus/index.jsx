...
async function fetchLyrics(track, artist, id, signal) {
    if (lyricsCache.has(id))
        return lyricsCache.get(id) ?? null;
...
function getCurrentLine(lines, posMs) {
    let current = null;
...
function tick() {
    if (!settings.store.active || !isPlaying || !currentLines)
        return;
...
function onSpotifyPlayerState(e) {
    ...
    if (trackChanged) {
        ...
        if (currentTrackId) {
            ...
            fetchLyrics(currentTrackName, currentArtist, currentTrackId, abortController.signal)
                .then(lines => {
                if (settings.store.active && generation === lyricGeneration && currentTrackId === newId)
                    currentLines = lines;
            })
                .finally(() => {
                if (lyricsAbortController === abortController)
...
function Icon({ className, active }) {
    const lineLength = 30;
    const lineStyle = {
    ...
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path fill={!active ? "var(--status-danger)" : "currentColor"} mask="url(#lyricsStatusLine)" d="M8.65 1.51A2 2 0 0 0 6 3.41v9.88A3.98 3.98 0 0 0 4.5 13C2.57 13 1 14.34 1 16s1.57 3 3.5 3S8 17.66 8 16V5.4l11 3.81v7.08a3.98 3.98 0 0 0-1.5-.29c-1.93 0-3.5 1.34-3.5 3s1.57 3 3.5 3 3.5-1.34 3.5-3V7.03c0-.74-.47-1.4-1.18-1.65L8.65 1.51Z"/>

            <line x1="22" y1="2" x2="2" y2="22" stroke="var(--status-danger, currentColor)" strokeWidth="2" strokeLinecap="round" style={lineStyle}/>
        </svg>);
}
function LyricsStatusToggleButton({ iconForeground, hideTooltips, nameplate }) {
    const { active, showPanelButton } = settings.use(["active", "showPanelButton"]);
    if (!showPanelButton)
        return null;
    return (<UserAreaButton className="button__201d5 wrapper__201d5" tooltipText={hideTooltips ? void 0 : active ? "Disable Lyrics Status" : "Enable Lyrics Status"} aria-label="Lyrics Status" icon={<Icon className={iconForeground} active={active}/>} role="switch" aria-checked={active} redGlow={!active} plated={nameplate != null} onClick={() => {
...
export default definePlugin({
    ...
    settings,
    userAreaButton: {
        icon: (props) => <Icon {...props} active={settings.store.active}/>,
