...
function generateRandomSentence(wordList) {
...
async function sendMessageEntry(entry) {
    if (!isRepeating)
        return;
...
function scheduleNextMessage(entry) {
    if (!isRepeating)
        return;
...
async function addMessageEntry(forceUpdate) {
    try {
        messageEntries.push({
            id: generateId(),
            message: "Hello!",
...
async function removeMessageEntry(id, forceUpdate) {
    try {
...
async function resetAllMessages(forceUpdate) {
    try {
...
function MessageEntries() {
    ...
    const elements = messageEntries.map((entry, i) => {
        return (<div key={entry.id} style={{ marginBottom: "16px", padding: "12px", border: "1px solid var(--background-modifier-accent)", borderRadius: "4px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    <Heading>Message {i + 1}</Heading>
                    <Button onClick={() => removeMessageEntry(entry.id, update)} look={Button.Looks.FILLED} color={Button.Colors.RED} size={Button.Sizes.SMALL}>
                        Delete
                    </Button>
                </div>
                ...
                <div style={{ marginBottom: "8px" }}>
                    ...
                    <TextInput placeholder="Enter message or command" value={entry.message} onChange={e => setMessage(entry.id, e)} onBlur={saveEntries}/>
                </div>

                <div>
                    <Paragraph>Delay (e.g., 1ms, 1s, 1m, 1h)</Paragraph>
                    <TextInput placeholder="1s" value={entry.delay} onChange={e => setDelay(entry.id, e)} onBlur={saveEntries}/>
                </div>
            </div>);
    });
    return (<>
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
                ...
                <Button onClick={() => resetAllMessages(update)} look={Button.Looks.FILLED} color={Button.Colors.RED}>
                    Reset All
                </Button>
            </div>
        </>);
...
const StartRepeaterIcon = ({ className }) => (<svg width="20" height="20" viewBox="0 0 24 24" className={className}>
        <path fill="currentColor" d="M8 5v14l11-7z"/>
    </svg>);
const StopRepeaterIcon = ({ className }) => (<svg width="20" height="20" viewBox="0 0 24 24" className={className}>
...
export default definePlugin({
    ...
    chatBarButton: {
        ...
        render: (({ isMainChat }) => {
            ...
            if (!isMainChat || settings.store.location !== "chatbar")
                return null;
            return (<ChatBarButton tooltip={isRunning ? "Stop Auto Repeating" : "Start Auto Repeating"} onClick={() => {
                    if (isRunning) {
                        stopRepeating();
                    }
                    else {
    ...
    async start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("AutoMessageRepeater", () => (<HeaderBarButton icon={StartRepeaterIcon} tooltip="Auto Message Repeater" onClick={() => { isRepeating ? stopRepeating() : startRepeating(); }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("AutoMessageRepeater", () => (<ChannelToolbarButton icon={StartRepeaterIcon} tooltip="Auto Message Repeater" onClick={() => { isRepeating ? stopRepeating() : startRepeating(); }}/>), 5);
        }
    },
    stop() {
        stopRepeating();
