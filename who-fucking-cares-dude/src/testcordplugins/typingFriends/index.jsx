...
const TYPING_REL = 69;
const TYPING_GUILDS = new Map();
const TYPING_USERS = new Map();
const TYPING_LOCATIONS = new Map();
const ORIGINAL_TYPES = new Map();
const TIMERS = new Map();
const GUILD_ICON_LISTENERS = new Set();
const PLUS_ICON = "data:image/svg+xml;utf8," + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16">
...
function TypingBadge({ src, offsetY, z }) {
...
function getTypingBadgeOffsets(count) {
    switch (count) {
        case 1: return [0];
        case 2: return [-8, 8];
...
function notifyGuildIconListeners() {
    for (const listener of GUILD_ICON_LISTENERS)
        listener();
}
const GuildTypingIcons = ErrorBoundary.wrap(function GuildTypingIcons({ guildId }) {
    const forceUpdate = useForceUpdater();
    React.useEffect(() => {
        GUILD_ICON_LISTENERS.add(forceUpdate);
        return () => void GUILD_ICON_LISTENERS.delete(forceUpdate);
    }, [forceUpdate]);
    if (!settings.store.showGuildIcons)
        return null;
...
function cleanupTyping(gid, userId) {
    ...
    if (set?.size === 0)
        TYPING_GUILDS.delete(previousGuildId);
    notifyGuildIconListeners();
}
function onTypingStart(e) {
    if (!RelationshipStore.isFriend(e.userId))
        return;
    const relationships = RelationshipStore.getMutableRelationships();
    ...
    if (!channel.guild_id) {
        ...
        relationships.set(e.userId, TYPING_REL);
        RelationshipStore.emitChange();
        clearTimeout(TIMERS.get(e.userId));
        TIMERS.set(e.userId, window.setTimeout(() => cleanupTyping(null, e.userId), settings.store.typingTimeout));
        notifyGuildIconListeners();
        return;
    }
    ...
    if (guild)
        TYPING_USERS.set(String(e.userId), guild.name);
    TYPING_LOCATIONS.set(String(e.userId), gid);
    if (!TYPING_GUILDS.has(gid))
        TYPING_GUILDS.set(gid, new Set());
    TYPING_GUILDS.get(gid).add(String(e.userId));
    relationships.set(e.userId, TYPING_REL);
    RelationshipStore.emitChange();
    clearTimeout(TIMERS.get(e.userId));
    TIMERS.set(e.userId, window.setTimeout(() => cleanupTyping(gid, e.userId), settings.store.typingTimeout));
    notifyGuildIconListeners();
}
function onTypingStop(e) {
...
function syncShowFriendsSection() {
    window.__friendsTypingShowSection = settings.store.showFriendsSection;
}
...
export default definePlugin({
    ...
    renderTypingEmptyState(jsxRuntime, emptyStateComponent) {
    ...
    start() {
        ...
        window.__typingUsers = TYPING_USERS;
    ...
    stop() {
        delete window.__typingUsers;
        delete window.__friendsTypingShowSection;
        FluxDispatcher.unsubscribe("TYPING_START", onTypingStart);
        FluxDispatcher.unsubscribe("TYPING_STOP", onTypingStop);
        const relationships = RelationshipStore.getMutableRelationships();
        for (const [id, original] of ORIGINAL_TYPES)
            relationships.set(id, original);
