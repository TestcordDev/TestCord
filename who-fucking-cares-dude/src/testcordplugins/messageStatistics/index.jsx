...
function streak(stats, goal) {
    ...
    while (true) {
        ...
        if (hit)
            count++;
        else if (day < startOfDay(Date.now()))
            break;
        day -= DAY;
        if (count > 0 && !hit)
            break;
        if (day < startOfDay(Date.now()) - 400 * DAY)
            break;
    }
    return count;
}
const row = (label, value, color) => (<tr style={{ borderBottom: "1px solid var(--background-modifier-accent)" }}>
...
function StatsPanel() {
    ...
    const reload = () => DataStore.get(KEY).then(v => setStats(normalizeStats(v)));
    useEffect(() => { reload(); }, []);
    const now = Date.now();
    const since = (ms) => {
        const cutoff = now - ms;
    ...
    return (<div style={{ color: "var(--text-normal)" }}>
            ...
            <button onClick={async () => { await DataStore.set(KEY, emptyStats()); reload(); }} style={{ marginTop: 15, background: "var(--background-tertiary)", color: "var(--text-normal)", border: "1px solid var(--background-modifier-accent)", padding: "8px 12px", borderRadius: 4, cursor: "pointer" }}>
                Reset stats
            </button>
        </div>);
}
    ...
    commands: [
        {
            ...
            options: [],
            execute: async (_, ctx) => {
                const stats = normalizeStats(await DataStore.get(KEY));
                const now = Date.now();
                const since = (ms) => {
                    const cutoff = now - ms;
                    let count = 0;
