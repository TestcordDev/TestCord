    ...
    commands: [
        {
            ...
            options: [
                { name: "user", description: "Whose avatar (defaults to you)", type: 6 /* ApplicationCommandOptionType.USER */ }
            ],
            execute: (opts, ctx) => {
                const userId = findOption(opts, "user") ?? UserStore.getCurrentUser()?.id;
