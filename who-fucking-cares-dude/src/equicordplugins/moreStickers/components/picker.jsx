 ...
 */
import { PickerContentHeader } from "@equicordplugins/moreStickers/types";
...
const debounceQueryChange = debounce((cb, ...args) => cb(...args), 150);
export const RecentPack = {
    id: RECENT_STICKERS_ID,
    name: RECENT_STICKERS_TITLE,
};
export const PickerSidebar = ({ packMetas, onPackSelect }) => {
    const [activePack, setActivePack] = React.useState(RecentPack);
    const [hovering, setHovering] = React.useState(false);
    return (<CategoryWrapper>
            <CategoryScroller categoryLength={packMetas.length}>
                <StickerCategory style={{ padding: "4px", boxSizing: "border-box", width: "32px" }} isActive={activePack === RecentPack} onClick={() => {
            if (activePack === RecentPack)
                return;
            onPackSelect(RecentPack);
            setActivePack(RecentPack);
        }}>
                    <RecentlyUsedIcon width={24} height={24} color={activePack === RecentPack ? " var(--interactive-icon-active)" : "var(--interactive-icon-default)"}/>
                </StickerCategory>
                {...packMetas.map(pack => {
            return (<StickerCategory key={pack.id} onClick={() => {
            ...
            <div className={clPicker("settings-cog-container")}>
                <button className={clPicker("settings-cog") + (hovering ? ` ${clPicker("settings-cog-active")}` : "")} onClick={() => {
            openModal(modalProps => {
                return (<Modal size="lg" title="Stickers+" {...modalProps}>
                                    <Packs />
                                </Modal>);
            });
...
function PickerContentRowGrid({ rowIndex, colIndex, sticker, onHover, channelId, onSend = () => { }, isHovered = false }) {
    ...
    return (<div role="gridcell" aria-rowindex={rowIndex} aria-colindex={colIndex} id={clPicker(`content-row-grid-${rowIndex}-${colIndex}`)} onMouseEnter={() => onHover(sticker)} onClick={e => {
            if (!channelId)
                return;
            ...
            width: "96px"
                            ...
                            <img alt={sticker.title} src={sticker.image} draggable="false" data-id={sticker.id} className={clPicker("content-row-grid-img")} loading="lazy"/>
                        </div>
                    </div>
                </div>
            </div>
...
function PickerContentRow({ rowIndex, grid1, grid2, grid3, channelId }) {
    return (<div className={clPicker("content-row")} role="row" aria-rowindex={rowIndex}>
...
function HeaderCollapseIcon({ isExpanded }) {
    return (<svg className={clPicker("content-header-collapse-icon")} width={16} height={16} viewBox="0 0 24 24" style={{
...
export function PickerContentHeader({ image, title, children, isSelected = false, afterScroll = () => { }, beforeScroll = () => { } }) {
    ...
    return (<span>
            <div className={clPicker("content-header-wrapper")}>
                <div className={clPicker("content-header-header")} ref={headerElem} aria-expanded={isExpand} aria-label={`Category, ${title}`} role="button" tabIndex={0} onClick={() => {
            setIsExpand(e => !e);
        }}>
                    <div className={clPicker("content-header-header-icon")}>
                        <div>
                            {typeof image === "string" ? <svg className={clPicker("content-header-svg")} width={16} height={16} viewBox="0 0 16 16">
                                <foreignObject x={0} y={0} width={16} height={16} overflow="visible" mask="url(#svg-mask-squircle)">
                                    <img alt={title} src={image} className={clPicker("content-header-guild-icon")} loading="lazy"></img>
                                </foreignObject>
                            </svg>
            : image}
                        </div>
                    </div>
                    <span className={clPicker("content-header-header-label")}>
                        {title}
                    </span>
...
export function PickerContent({ stickerPacks, selectedStickerPackId, setSelectedStickerPackId, channelId, closePopout, query }) {
    const [currentSticker, setCurrentSticker] = (React.useState((stickerPacks.length && stickerPacks[0].stickers.length) ?
        stickerPacks[0].stickers[0] :
        null));
    const [currentStickerPack, setCurrentStickerPack] = React.useState(stickerPacks.length ? stickerPacks[0] : null);
    const [recentStickers, setRecentStickers] = React.useState([]);
    const stickerPacksElemRef = React.useRef(null);
    const scrollerRef = React.useRef(null);
    function queryFilter(stickers) {
        if (!query)
            return stickers;
    ...
    const stickersToRows = (stickers) => stickers
        ...
        .map((stickers, i) => (<PickerContentRow key={i} rowIndex={i} channelId={channelId} grid1={{
            rowIndex: i,
            colIndex: 1,
            sticker: stickers[0],
        ...
        } : undefined} grid3={stickers.length > 2 ? {
            rowIndex: i,
            colIndex: 3,
            sticker: stickers[2],
            onHover: setCurrentSticker,
    ...
    return (<div className={clPicker("content-list-wrapper")}>
                ...
                <div className={clPicker("content-scroller")} ref={scrollerRef}>
                    <div className={clPicker("content-list-items")} role="none presentation">
                        <div ref={stickerPacksElemRef}>
                            <PickerContentHeader image={<RecentlyUsedIcon width={16} height={16} color="currentColor"/>} title={RECENT_STICKERS_TITLE} isSelected={RECENT_STICKERS_ID === selectedStickerPackId} beforeScroll={() => {
            ...
            return (<PickerContentHeader key={sp.id} image={sp.logo.image} title={sp.title} isSelected={sp.id === selectedStickerPackId} beforeScroll={() => {
                    scrollerRef.current?.scrollTo({
                        top: 0,
                    });
                }} afterScroll={() => { setSelectedStickerPackId(null); }}>
            ...
            height: `${stickerPacksElemRef.current?.clientHeight ?? 0}px`
                ...
                <div className={clPicker("content-inspector")} style={{
            ...
            width: "28px"
                                ...
                                <img alt={currentSticker?.title ?? ""} src={currentSticker?.image} draggable="false" data-id={currentSticker?.id ?? ""} className={clPicker("content-inspector-img")}/>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            <svg width={32} height={32} viewBox="0 0 32 32">
                                <foreignObject x={0} y={0} width={32} height={32} overflow="visible" mask="url(#svg-mask-squircle)">
                                    <img alt={currentStickerPack?.title ?? ""} src={currentStickerPack?.logo?.image}></img>
                                </foreignObject>
                            </svg>
                        </div>
                    </div>
...
export const PickerHeader = ({ onQueryChange }) => {
    const [query, setQuery] = React.useState();
    const setQueryDebounced = (value, immediate = false) => {
        setQuery(value);
        if (immediate)
            onQueryChange(value);
        else
            ...
            <div className={clPicker("container")}>
                <div>
                    <div className={clPicker("search-box")}>
                        <TextInput style={{ height: "30px", border: "none" }} placeholder="Search stickers" autoFocus={true} value={query} onChange={(value) => setQueryDebounced(value)}/>
                    </div>
