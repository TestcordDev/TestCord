...
export const ThemeCard = ({ theme, themeLinks, likedThemes, setThemeLinks, removeButtons, removePreview }) => {
    ...
    const handleThemeAttributesCheck = () => {
        ...
        if (requiresThemeAttributes && !Settings.plugins.ThemeAttributes.enabled) {
            openModal(modalProps => (<Modal {...modalProps} size="sm" title="Hold on!" actions={[
                    {
                        text: "Enable Plugin",
                        variant: "primary",
                        onClick: () => {
    ...
    return (<Card style={{ padding: ".5rem", marginBottom: ".5em", marginTop: ".5em", display: "flex", flexDirection: "column", backgroundColor: "var(--background-base-lower-alt)" }} key={theme.id}>
            ...
            {!removePreview && (<img role="presentation" src={theme.thumbnail_url} loading="lazy" alt={theme.name} className="vce-theme-info-preview"/>)}
            <div className="vce-theme-info">
                <div style={{ justifyContent: "flex-start", flexDirection: "column" }}>
                    {theme.tags && (<Paragraph>
                            {theme.tags.map(tag => (<span className="vce-theme-info-tag" key={tag}>
                                    {tag}
                                </span>))}
                        </Paragraph>)}
                    {!removeButtons && (<div style={{ marginTop: "8px", display: "flex", flexDirection: "row" }}>
                            <Button onClick={handleThemeAttributesCheck} size="medium" variant={themeLinks.includes(`${apiUrl}/${theme.id}`) ? "dangerPrimary" : "positive"} className={Margins.right8} disabled={!theme.content || theme.id === "preview"}>
            ...
            }} size="medium" className={Margins.right8}>
                                Theme Info
                            </Button>
                            <LikesComponent themeId={theme.id} likedThemes={likedThemes}/>
                            <Button onClick={handleViewSource} size="medium" variant="link" disabled={!theme.content || theme.id === "preview"} style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                                View Source
                            </Button>
                        </div>)}
                </div>
