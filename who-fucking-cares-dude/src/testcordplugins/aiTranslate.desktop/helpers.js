...
function applyTargetLanguage(systemPrompt, targetLanguage) {
...
function normalizeScriptLanguage(language) {
    ...
    if (normalized.includes("korean"))
        return "Korean";
    if (normalized.includes("cyrillic") || normalized.includes("russian") || normalized.includes("ukrainian") || normalized.includes("bulgarian") || normalized.includes("serbian"))
        return "Cyrillic";
    if (normalized.includes("arabic") || normalized.includes("persian") || normalized.includes("urdu"))
        return "Arabic";
...
function isLetterInTargetScript(char, targetScript) {
    if (targetScript === "Chinese")
        return /\p{Script=Han}/u.test(char);
    if (targetScript === "Japanese")
        return /\p{Script=Hiragana}|\p{Script=Katakana}|\p{Script=Han}/u.test(char);
    if (targetScript === "Korean")
    ...
    if (targetScript === "Hebrew")
        return /\p{Script=Hebrew}/u.test(char);
    if (targetScript === "Thai")
        return /\p{Script=Thai}/u.test(char);
    if (targetScript === "Devanagari")
        return /\p{Script=Devanagari}/u.test(char);
    if (targetScript === "Bengali")
        return /\p{Script=Bengali}/u.test(char);
    if (targetScript === "Greek")
        return /\p{Script=Greek}/u.test(char);
    return false;
}
function hasLetterOutsideTargetScript(content, targetScript) {
    for (const char of content) {
        if (/\p{L}/u.test(char) && !isLetterInTargetScript(char, targetScript))
            return true;
    }
    return false;
}
function getResponseDetail(responseBody) {
    let parsed;
    try {
...
export function isOpenCodeFreeModel(model) {
    const normalized = model.trim().toLowerCase();
    if (!normalized || NOT_FREE_MODEL_TOKEN.test(normalized))
        return false;
    return normalized === DEFAULT_OPENCODE_ZEN_MODEL || FREE_MODEL_TOKEN.test(normalized);
}
export function parseOpenCodeFreeModelsResponse(responseBody) {
    let parsed;
    try {
        parsed = JSON.parse(responseBody);
    }
    ...
    const models = new Set([DEFAULT_OPENCODE_ZEN_MODEL]);
    for (const model of rawModels) {
        const id = isRecord(model) && typeof model.id === "string" ? model.id.trim() : "";
        if (id && isOpenCodeFreeModel(id))
            models.add(id);
    }
    return [...models].sort((a, b) => {
        if (a === DEFAULT_OPENCODE_ZEN_MODEL)
            return -1;
        if (b === DEFAULT_OPENCODE_ZEN_MODEL)
            return 1;
        return a.localeCompare(b);
    });
}
export function shouldTranslateOutgoingContent(content, targetLanguage, enabled = true) {
    const text = stripIgnoredContent(content);
    if (!enabled || !/\p{L}/u.test(text))
        return false;
    const targetScript = normalizeScriptLanguage(targetLanguage);
    if (targetScript === "Unknown")
        return true;
    return hasLetterOutsideTargetScript(text, targetScript);
}
export function getAutoTranslateToggleLabel(enabled) {
    return enabled ? "AI outgoing translate is on. Messages will be sent to OpenCode Zen." : "AI outgoing translate is off.";
}
export function getTranslationStatusMessage(status, retryMs = 0) {
    if (status === "sent")
        return "Translated message sent.";
    if (status === "failed")
        return "Translation failed, so the message was not sent.";
    return `AI Translate is cooling down. Try again in ${Math.ceil(retryMs / 1000)} seconds.`;
}
export function getContentCacheKey(content, settings) {
...
export function buildTranslationRequest(text, targetLanguage, model, systemPrompt = DEFAULT_SYSTEM_PROMPT, useGenerationControls = true) {
...
export function getGenerationControlsRetryPayload(responseBody, payload) {
    ...
    if (!mentionsControl || !rejectsControl)
        return null;
    let parsed;
    try {
    ...
    if (!isRecord(parsed))
        return null;
    if (!("temperature" in parsed) && !("reasoning_effort" in parsed) && !("extra_body" in parsed))
        return null;
    const retryPayload = { ...parsed };
...
export function parseTranslationResponse(responseBody) {
    if (!responseBody)
        return null;
    let parsed;
    try {
