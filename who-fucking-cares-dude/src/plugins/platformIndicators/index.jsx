...
function Icon(path, opts) {
    return ({ color, tooltip, small, style }) => (<Tooltip text={tooltip}>
            {tooltipProps => (<svg {...tooltipProps} style={style} height={(opts?.height ?? 20) - (small ? 3 : 0)} width={(opts?.width ?? 20) - (small ? 3 : 0)} viewBox={opts?.viewBox ?? "0 0 24 24"} fill={color}>
                    <path d={path}/>
                </svg>)}
        </Tooltip>);
}
...
function useEnsureOwnStatus(user) {
    ...
    if (!isOwnUser || typeof sessions !== "object")
        return;
...
const PlatformIndicator = ({ user, isProfile, isMessage, isMemberList }) => {
    useEnsureOwnStatus(user);
    const status = useStateFromStores([PresenceStore], () => user == null ? null : PresenceStore.getClientStatus(user.id), [user?.id]);
    if (user == null || (user.bot && !settings.store.showBots))
        return null;
    if (!status)
        return null;
    const icons = Array.from(Object.entries(status), ([platform, status]) => (<PlatformIcon key={platform} platform={platform} status={status} small={isProfile || isMemberList}/>));
    if (!icons.length) {
        return null;
    }
    if (isMemberList && icons.length === 1) {
        const [[platform, platformStatus]] = Object.entries(status);
        return <PlatformIcon platform={platform} status={platformStatus} small={isProfile || isMemberList} style={{ marginLeft: "4px" }}/>;
    }
    return (<div className={classes("vc-platform-indicator", isProfile && "vc-platform-indicator-profile", isMessage && "vc-platform-indicator-message")} style={{ marginLeft: isMemberList ? "4px" : undefined }}>
            {icons}
        </div>);
};
