...
function useDiscordPronouns(id, useGlobalProfile = false) {
    ...
    if (useGlobalProfile)
        return globalPronouns;
    return guildPronouns || globalPronouns;
}
export function useFormattedPronouns(id, useGlobalProfile = false) {
    const pronouns = useDiscordPronouns(id, useGlobalProfile)?.trim().replace(/\n+/g, "");
    return settings.store.pronounsFormat === "LOWERCASE" /* PronounsFormat.Lowercase */ ? pronouns?.toLowerCase() : pronouns;
}
