...
const HOUSE_FLAGS: Record<number, number> = { 1: 64, 2: 128, 3: 256 };
    ...
    try {
        ...
        showToast(
            houseId === 0
                ...
                : t(`تم تحديث دار HypeSquad: ${name}!`, `HypeSquad House updated: ${name}!`),
            Toasts.Type.SUCCESS
        );
...
function currentHouse(): number {
    const flags = UserStore.getCurrentUser()?.flags ?? 0;
    for (const [id, flag] of Object.entries(HOUSE_FLAGS)) if (flags & flag) return +id;
    return 0;
}

export function HypeSquadSelectComponent() {
    const [selectedHouse, setSelectedHouse] = React.useState<number>(currentHouse);
        ...
        <div style={{ marginTop: 14, marginBottom: 14, width: "100%" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                ...
                <button
                    ...
                    onMouseEnter={e => { e.currentTarget.style.background = "#ed4245"; e.currentTarget.style.color = "#ffffff"; }}
                    onMouseLeave={e => {
