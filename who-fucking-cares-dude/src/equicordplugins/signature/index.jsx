...
const SignatureToggle = ({ isMainChat }) => {
    const { isEnabled, showIcon } = settings.use(["isEnabled", "showIcon"]);
    const toggle = () => settings.store.isEnabled = !settings.store.isEnabled;
    if (!isMainChat || !showIcon)
        return null;
    return (<ChatBarButton tooltip={isEnabled ? "Disable Signature" : "Enable Signature"} onClick={toggle}>
...
function SignatureIcon() {
    return (<svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 21.333">
...
const ChatBarContextCheckbox = children => {
    ...
    group.splice(idx + 1, 0, <Menu.MenuCheckboxItem id="vc-Signature" label="Enable Signature" checked={isEnabled} action={() => settings.store.isEnabled = !settings.store.isEnabled}/>);
};
