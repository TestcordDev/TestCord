...
const VoiceChannelActions = ChannelActions;
const PrivateChannelActions = ChannelActionCreators;
const warnedVoiceKeys = new Map();
const warnedGroupChannels = new Map();
const pendingVoiceJoins = new Map();
const pendingGroupWarnings = new Map();
const pendingVoiceWarnings = new Map();
const seenVoiceChannelMembers = new Map();
...
function getVoiceCandidateUserIds(channelId) {
...
async function getBlockedUsers(userIds) {
...
function shouldWarnForVoiceChannel(channel) {
    return channel != null && (channel.type === 2 /* ChannelType.GUILD_VOICE */ ||
        channel.type === 13 /* ChannelType.GUILD_STAGE_VOICE */ ||
        channel.type === 1 /* ChannelType.DM */ ||
        channel.type === 3 /* ChannelType.GROUP_DM */);
}
async function maybeWarnBeforeVoiceJoin(channelId, proceed) {
    ...
    const pendingPromise = (async () => {
        ...
        if (generation !== activeGeneration || attemptId !== latestVoiceJoinAttempt)
            return;
...
async function maybeWarnForCurrentVoiceChannel(channelId) {
    ...
    pendingState.promise = (async () => {
        ...
        if (!sameUserIds(candidateUserIds, latestCandidateUserIds))
            return;
        ...
        if (warnedVoiceKeys.get(channelId) === warningKey)
            return;
...
async function maybeWarnForGroupChannel(channelId) {
    const channel = ChannelStore.getChannel(channelId);
    ...
    if (warnedGroupChannels.get(channelId) === currentRecipientKey)
        return;
    const generation = activeGeneration;
    const pendingPromise = (async () => {
        ...
        if (latestChannel?.type !== 3 /* ChannelType.GROUP_DM */)
            return;
        const latestRecipientKey = [...latestChannel.recipients].sort().join(",");
        if (latestRecipientKey !== currentRecipientKey)
            return;
        if (!blockedUsers.length) {
...
export default definePlugin({
    ...
    flux: {
        USER_PROFILE_FETCH_SUCCESS({ userProfile }) {
        ...
        VOICE_STATE_UPDATES({ voiceStates }) {
            ...
            const changedCurrentChannel = voiceStates.some(state => state.userId !== currentUserId &&
                (state.channelId === myState.channelId || seenMembers?.has(state.userId)));
            if (!changedCurrentChannel)
                return;
    ...
    start() {
        ...
        VoiceChannelActions.selectVoiceChannel = (function selectVoiceChannel(channelId) {
