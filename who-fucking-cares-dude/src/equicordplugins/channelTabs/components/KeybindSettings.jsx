...
function KeybindInput({ label, description, settingKey, enabledKey }) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event) => {
            ...
            const conflictKey = Object.entries(allKeybinds).find(([key, value]) => key !== settingKey && value === keybindString);
            if (conflictKey) {
    ...
    return (<div className="channelTabs-keybind-input">
            <div className="channelTabs-keybind-info">
                <Text variant="text-md/semibold">{label}</Text>
                <Text variant="text-sm/normal" style={{ color: "var(--text-muted)" }}>
                    {description}
                </Text>
                {!isEnabled && (<Text variant="text-xs/normal" style={{ color: "var(--text-feedback-critical)" }}>
                        This shortcut is currently disabled
                    </Text>)}
                {error && (<Text variant="text-xs/normal" className="channelTabs-keybind-conflict">
                        {error}
                    </Text>)}
            </div>
            <div className="channelTabs-keybind-controls">
                <button ref={buttonRef} className={`channelTabs-keybind-button ${isListening ? "listening" : ""}`} onClick={() => setIsListening(true)} disabled={!isEnabled}>
                    {isListening ? "Press any key..." : formatKeybind(currentKeybind)}
                </button>
                <Button size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} onClick={handleReset} disabled={!isEnabled}>
                    Reset
                </Button>
            </div>
        </div>);
...
export function KeybindSettings() {
    ...
    return (<ErrorBoundary>
            ...
            <div className="channelTabs-keybind-settings">
                ...
                <KeybindInput label="Close Tab" description="Close the currently active tab" settingKey="closeTabKeybind" enabledKey="enableCloseTabShortcut"/>

                <KeybindInput label="New Tab" description="Open a new tab with the current channel" settingKey="newTabKeybind" enabledKey="enableNewTabShortcut"/>

                <KeybindInput label="Cycle Tabs Forward" description="Switch to the next tab (wraps around to first)" settingKey="cycleTabForwardKeybind" enabledKey="enableTabCycleShortcut"/>

                <KeybindInput label="Cycle Tabs Backward" description="Switch to the previous tab (wraps around to last)" settingKey="cycleTabBackwardKeybind" enabledKey="enableTabCycleShortcut"/>
