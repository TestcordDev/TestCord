...
export function connect(_, rawToken) {
    const nextToken = sanitizeToken(rawToken);
    return queueConnectionOperation(async () => {
        if (nextToken === state.token && state.socket?.isOpen)
...
export function getStatus(_) {
    return statusSnapshot();
}
export function command(_, name, payload = {}) {
    if (!PLAYER_COMMANDS.has(name))
        return false;
...
export function connectStations(_, rawToken) {
    return startStations(sanitizeToken(rawToken));
}
export function rescanStations(_) {
    return refreshStations();
}
export function getCoverDataUrl(_, rawUrl) {
    return resolveCoverDataUrl(rawUrl);
}
export function waitForEvents(_, timeout = 30_000) {
