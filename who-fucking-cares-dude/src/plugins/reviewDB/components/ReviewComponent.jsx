...
export default function ReviewComponent({ review, refetch, profileId }) {
    ...
    const [localVote, setLocalVote] = useState(review.userVote ?? null);
    ...
    function delReview() {
        openVencordModal(props => (<ConfirmModal {...props} title="Are you sure?" subtitle="Do you really want to delete this review?" confirmText="Delete" cancelText="Nevermind" onConfirm={async () => {
                if (!(await getToken())) {
    ...
    function reportRev() {
        openVencordModal(props => (<ConfirmModal {...props} title="Are you sure?" subtitle="Do you really want to report this review?" confirmText="Report" cancelText="Nevermind" onConfirm={async () => {
                if (!(await getToken())) {
    ...
    function blockReviewSender() {
        ...
        openVencordModal(props => (<ConfirmModal {...props} title="Are you sure?" subtitle="Do you really want to block this user? They will be unable to leave further reviews on your profile. You can unblock users in the plugin settings." confirmText="Block" cancelText="Nevermind" onConfirm={async () => {
                if (!(await getToken())) {
    ...
    return (<div className={classes(cl("review"), MessageClasses.cozyMessage, AvatarClasses.wrapper, MessageClasses.message, MessageClasses.groupStart, AvatarClasses.cozy)} style={{
            marginLeft: "0px",
            paddingLeft: "52px",
            // nobody knows anymore
        }}>

            {review.id !== 0 && (<div className={cl("vote-column")}>
                    ...
                    <div className={cl("vote-column-buttons")}>
                        <button className={classes(cl("vote-column-button"), !!localVote && cl("vote-column-up-selected"))} disabled={isVoting} onClick={() => submitVote(true)} type="button">
                        ...
                        <button className={classes(cl("vote-column-button"), localVote === false && cl("vote-column-down-selected"))} disabled={isVoting} onClick={() => submitVote(false)} type="button">
                            <svg height="20" viewBox="0 0 12 12" width="20" fill="none">
            ...
            <img className={classes(AvatarClasses.avatar, AvatarClasses.clickable)} onClick={openModal} src={review.sender.profilePhoto ?? IconUtils.getDefaultAvatarURL(review.sender.discordID)} style={{ left: "0px", zIndex: 0 }} onError={e => e.currentTarget.src = IconUtils.getDefaultAvatarURL(review.sender.discordID)}/>
            <div style={{ display: "inline-flex", justifyContent: "center", alignItems: "center" }}>
                <span className={classes(AvatarClasses.clickable, AvatarClasses.username)} style={{ color: "var(--channels-default)", fontSize: "14px" }} onClick={() => openModal()}>
                    {review.sender.username}
                </span>

                {review.type === 3 /* ReviewType.System */ && (<span className={classes(BotTagClasses.botTagVerified, BotTagClasses.botTagRegular, BotTagClasses.px, BotTagClasses.rem)} style={{ marginLeft: "4px" }}>
                        <span className={BotTagClasses.botText}>
                            System
                        </span>
                    </span>)}
            </div>
            {isAuthorBlocked && (<ReviewBadge name="You have blocked this user" description="You have blocked this user" icon="/assets/aaee57e0090991557b66.svg" type={0} onClick={() => openBlockModal()}/>)}
            {review.sender.badges.map((badge, idx) => <ReviewBadge key={idx} {...badge}/>)}

            {!settings.store.hideTimestamps && review.type !== 3 /* ReviewType.System */ && (<Timestamp timestamp={new Date(review.timestamp * 1000)}>
            ...
            {review.id !== 0 && (<div className={classes(ContainerClasses.container, ContainerClasses.isHeader, MessageClasses.buttons)} style={{
                padding: "0px",
            }}>
