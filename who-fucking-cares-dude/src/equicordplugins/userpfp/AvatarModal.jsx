...
export function SetAvatarModal({ userId, modalProps }) {
    ...
    return (<Modal {...modalProps} size="sm" title="Custom Avatar" actions={actions}>
            <div onKeyDown={handleKey}>
                ...
                <div className={cl("preview-row")}>
                    ...
                    <div className={cl("preview-box")}>
                        ...
                        <img src={preview || originalAvatar} className={`${cl("avatar")} ${preview ? cl("avatar-active") : ""}`} alt="local"/>
                    </div>
                </div>
                ...
                <section className={Margins.bottom8}>
                    ...
                    <TextInput placeholder="https://example.com/image.png" value={url.startsWith("data:") ? "(uploaded file)" : url} onChange={handleUrlChange} autoFocus/>
                </section>

                {/* Drag & drop */}
                <div className={`${cl("dropzone")} ${isDragging ? cl("dropzone-active") : ""}`} onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={e => {
            e.preventDefault();
            setIsDragging(false);
