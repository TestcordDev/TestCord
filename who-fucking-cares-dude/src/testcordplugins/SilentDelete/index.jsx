...
const SilentDeleteIcon = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill={getAccentColor()}>
...
async function silentDeleteMessage(channelId, messageId, deleteOriginal = true) {
    try {
...
const messageContextMenuPatch = (children, { message }) => {
    if (!message || message.author.id !== UserStore.getCurrentUser().id || !message.deleted)
        return;
    const group = findGroupChildrenByChildId("remove-message-history", children) ?? children;
    group.push(<Menu.MenuItem id="silent-delete-history" label={<span style={{ color: getAccentColor() }}>Silent Delete History</span>} action={() => silentDeleteMessage(message.channel_id, message.id, false)} icon={SilentDeleteIcon}/>);
};
