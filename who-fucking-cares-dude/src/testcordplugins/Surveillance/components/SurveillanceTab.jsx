...
import { analyzeSurveillanceEvents } from "../osintAnalysis";
import { clearEvents, getEvents, loadEvents, subscribe } from "../store";
const EVENT_PAGE_SIZE = 250;
const SECTION_NAV = ["timeline", "targets", "stats"];
...
const eventMatchesFilter = (event, filter) => {
    ...
    if (filter === "profile")
        return event.type === "profile_update";
    if (filter === "server")
        return event.scope === "server" || ["channel_", "thread_", "guild_", "role_"].some(prefix => event.type.startsWith(prefix));
...
const formatLabel = (label) => label.replace(/[A-Z]/g, match => ` ${match}`).replace(/^./, match => match.toUpperCase());
const getEventAvatarUrl = (event) => {
    const user = UserStore.getUser(event.userId);
    return user ? IconUtils.getUserAvatarURL(user, true, 64) : event.avatarUrl;
};
const getStringMetadata = (event, key) => {
    const value = event.metadata?.[key];
    return typeof value === "string" ? value : undefined;
};
const getNumberMetadata = (event, key) => {
    const value = event.metadata?.[key];
    return typeof value === "number" ? value : undefined;
};
const getBooleanMetadata = (event, key) => event.metadata?.[key] === true;
const formatCount = (value) => value.toLocaleString();
const formatDuration = (value) => value > 0 ? formatDurationMs(value, true) : "0s";
...
const isImageUrl = (url) => IMAGE_URL_REGEX.test(url);
const isImageAttachment = (contentType) => contentType?.startsWith("image/") === true;
const scrollToSection = (section) => {
    document.getElementById(`vc-surveillance-${section}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
};
const toast = (message, type = Toasts.Type.SUCCESS) => Toasts.show({
    type,
    message,
    id: Toasts.genId(),
});
function DetailField({ label, value, wide, preserve }) {
    if (value == null || value === "")
        return null;
    const text = String(value);
    return (<div className={classes(cl("modal-field"), wide && cl("modal-wide"))}>
            <strong>{label}</strong>
            {preserve ? <pre>{text}</pre> : <span>{text}</span>}
        </div>);
}
function MessageMedia({ event }) {
    if (!event.attachments?.length && !event.links?.length)
        return null;
    return (<div className={cl("message-media")}>
            {event.attachments?.length ? (<section>
                    <HeadingTertiary>Attachments</HeadingTertiary>
                    <div className={cl("attachment-grid")}>
                        {event.attachments.map(attachment => (<a key={attachment.id} className={cl("attachment")} href={attachment.url} target="_blank" rel="noreferrer">
                                {isImageAttachment(attachment.contentType) || isImageUrl(attachment.url) ? (<img src={attachment.proxyUrl ?? attachment.url} alt={attachment.filename}/>) : null}
            ...
            {event.links?.length ? (<section>
                    <HeadingTertiary>Links</HeadingTertiary>
                    <div className={cl("link-list")}>
                        {event.links.map(link => (<a key={link} className={cl("link-preview", { image: isImageUrl(link) })} href={link} target="_blank" rel="noreferrer">
...
function IdentityHistory({ history }) {
    if (!history?.length)
        return null;
    return (<section className={cl("identity-history")}>
            <HeadingTertiary>Identity History</HeadingTertiary>
            <div className={cl("identity-list")}>
                {history.slice().reverse().map(entry => (<div key={`${entry.timestamp}-${entry.field}-${entry.guildId ?? ""}-${entry.value ?? ""}`} className={cl("identity-entry")}>
...
function VoiceStateIcon({ enabled, label, tone, children }) {
    if (!enabled)
        return null;
    return (<span className={classes(cl("voice-state"), cl(`voice-state-${tone}`))} title={label}>
            {children}
        </span>);
}
function VoiceParticipants({ event }) {
    if (!event.voiceParticipants?.length)
        return null;
    return (<div className={cl("voice-members")}>
            <HeadingTertiary>Voice Channel Members</HeadingTertiary>
            <div className={cl("voice-member-grid")}>
                {event.voiceParticipants.map(member => (<div key={member.userId} className={cl("voice-member")}>
...
const getChannelRoute = (event) => event.channelId ? `/channels/${event.guildId ?? "@me"}/${event.channelId}` : undefined;
...
const EventDetailsModal = ErrorBoundary.wrap(function EventDetailsModal({ event, modalProps }) {
    ...
    const copyEvent = () => {
        try {
            void Promise.resolve(copyToClipboard(JSON.stringify(event, null, 2))).then(() => toast("Event copied."), () => toast("Failed to copy event.", Toasts.Type.FAILURE));
        }
        catch {
            toast("Failed to copy event.", Toasts.Type.FAILURE);
        }
    };
    return (<ModalRoot {...modalProps} title="" size={"medium" /* ModalSize.MEDIUM */}>
            <ModalHeader>
            ...
            <ModalContent className={cl("modal-content")}>
                ...
                {metadata.length ? (<div className={cl("modal-grid")}>
                        {metadata.map(([key, value]) => (<DetailField key={key} label={formatLabel(key)} value={value}/>))}
                    </div>) : null}
...
function TargetPill({ userId }) {
    const user = UserStore.getUser(userId);
    const avatarUrl = user ? IconUtils.getUserAvatarURL(user, true, 32) : undefined;
    return (<button className={cl("target-pill")} onClick={() => removeTarget(userId)}>
...
function ServerPill({ guildId }) {
    const guild = GuildStore.getGuild(guildId);
    return (<button className={cl("target-pill")} onClick={() => removeServerTarget(guildId)}>
            <span>{guild?.name ?? guildId}</span>
            <span className={cl("target-id")}>{guildId}</span>
        </button>);
}
function UserStat({ label, value }) {
    return (<div className={cl("user-stat")}>
            <span>{value ?? "-"}</span>
            <small>{label}</small>
        </div>);
}
function UserStatGroup({ title, children }) {
    return (<section className={cl("user-stat-group")}>
...
const getUserStats = (events, userId) => {
    ...
    const openVoice = new Map();
    const activeVoiceFlags = new Map();
    ...
    const closeFlag = (flag, timestamp) => {
        const startedAt = activeVoiceFlags.get(flag);
        if (startedAt == null)
            return;
        const duration = timestamp - startedAt;
        if (flag === "camera")
            stats.cameraMs += duration;
        else if (flag === "deaf")
            stats.deafMs += duration;
        else if (flag === "mute")
            stats.muteMs += duration;
        else
            stats.streamMs += duration;
        activeVoiceFlags.delete(flag);
    };
    let activeStatus;
    let activeStatusStartedAt;
    const addStatusDuration = (status, duration) => {
        if (status === "online")
            stats.onlineMs += duration;
        else if (status === "idle")
            stats.idleMs += duration;
        else if (status === "dnd")
            stats.dndMs += duration;
        else
            stats.offlineMs += duration;
    };
    const syncFlag = (flag, enabled, timestamp) => {
        if (enabled) {
            if (!activeVoiceFlags.has(flag))
                activeVoiceFlags.set(flag, timestamp);
            return;
        }
    ...
    for (const event of userEvents) {
        ...
        switch (event.type) {
            ...
            case "voice_join":
                stats.voiceJoins++;
                if (event.channelId)
                    openVoice.set(event.channelId, event);
            ...
            case "voice_leave":
                ...
                if (event.channelId)
                    openVoice.delete(event.channelId);
            ...
            case "voice_move":
                stats.voiceMoves++;
                stats.totalVoiceMs += getNumberMetadata(event, "durationMs") ?? 0;
                for (const channelId of openVoice.keys())
                    openVoice.delete(channelId);
                if (event.channelId)
                    openVoice.set(event.channelId, event);
                syncFlag("mute", getBooleanMetadata(event, "mute") || getBooleanMetadata(event, "selfMute"), event.timestamp);
                syncFlag("deaf", getBooleanMetadata(event, "deaf") || getBooleanMetadata(event, "selfDeaf"), event.timestamp);
            ...
            case "status":
                ...
                if (activeStatus && activeStatusStartedAt != null)
                    addStatusDuration(activeStatus, event.timestamp - activeStatusStartedAt);
            ...
            case "activity_stop": {
                ...
                if (startedAt != null)
                    stats.activityMs += event.timestamp - startedAt;
            ...
            case "profile_update":
                ...
                if (event.identityHistory?.length)
                    stats.identityHistory = event.identityHistory;
                break;
        }
    }
    for (const event of openVoice.values()) {
        if (event.channelId)
            stats.totalVoiceMs += now - event.timestamp;
    }
    ...
    if (activeStatus && activeStatusStartedAt != null)
        addStatusDuration(activeStatus, now - activeStatusStartedAt);
...
function UserStatsPanel({ events, selectedUserId, setSelectedUserId }) {
    const [showAllStats, setShowAllStats] = useState(false);
    const userOptions = useMemo(() => {
        const options = new Map();
        for (const event of events) {
            if (!options.has(event.userId))
                options.set(event.userId, `${event.username} (${event.userId})`);
        }
    ...
    return (<section className={cl("panel")}>
            <div className={cl("section-head")}>
                ...
                <Select placeholder="Select a user..." options={userOptions} maxVisibleItems={8} closeOnSelect={true} select={setSelectedUserId} isSelected={value => value === userId} serialize={value => value}/>
            </div>
            {stats ? (<>
                    ...
                    {showAllStats ? (<div className={cl("user-stat-groups")}>
...
function EventRow({ event }) {
    ...
    return (<div className={cl("event-row")} onClick={openDetails} role="button" tabIndex={0} onKeyDown={event => {
            ...
            <div className={cl("event-main")}>
                ...
                {event.before || event.after ? (<div className={cl("event-diff")}>
                        {event.before ? <span>Before: {event.before}</span> : null}
                        {event.after ? <span>After: {event.after}</span> : null}
                    </div>) : null}
            </div>
            {action ? (<button className={cl("event-action")} onClick={mouseEvent => {
...
function OsintAnalysisPanel({ events, selectedUserId }) {
    ...
    if (!selectedUserId || !result || result.sections.length === 0)
        return null;
    return (<section className={cl("panel")}>
            <div className={cl("section-head")}>
            ...
            <div className={cl("osint-sections")}>
                {(expanded ? result.sections : result.sections.slice(0, 3)).map((section, i) => (<div key={i} className={cl("osint-section")}>
                        <div className={cl("osint-section-title")}>{section.title}</div>
                        <div className={cl("osint-section-body")}>
                            {section.content.split("\n").map((line, j) => (<div key={j} className={cl("osint-line", { indent: line.startsWith("  -") })}>
                                    {line.replace(/^ {2}- /, "")}
                                </div>))}
                        </div>
                    </div>))}
            </div>
            {!expanded && result.sections.length > 3 && (<button className={cl("text-action")} onClick={() => setExpanded(true)}>
...
function SurveillanceTab() {
    ...
    const filteredEvents = useMemo(() => events.filter(event => eventMatchesFilter(event, filter) && eventMatchesQuery(event, query)), [events, filter, query]);
    ...
    const guildOptions = useMemo(() => guilds
        .filter(guild => !serverTargets.includes(guild.id))
        .map(guild => ({ label: guild.name, value: guild.id }))
        .sort((a, b) => a.label.localeCompare(b.label)), [guilds, serverTargets]);
    ...
    const copyEvents = () => {
        try {
            void Promise.resolve(copyToClipboard(JSON.stringify(filteredEvents, null, 2))).then(() => toast("Surveillance events copied."), () => toast("Failed to copy surveillance events.", Toasts.Type.FAILURE));
        }
        catch {
            ...
            <div className={cl("root")}>
                <div className={cl("header")}>
                    <div className={cl("header-top")}>
                        ...
                        <Tooltip text="Surveillance Settings">
                            {props => (<button {...props} className={cl("settings-btn")} onClick={() => {
                if (plugins.Surveillance)
                    openPluginModal(plugins.Surveillance);
                    ...
                    <div className={cl("actions")}>
                        ...
                        {SECTION_NAV.map(section => (<button key={section} className={cl("nav-action")} onClick={() => scrollToSection(section)}>
                                {formatLabel(section)}
                            </button>))}
                ...
                <section id="vc-surveillance-timeline" className={classes(cl("panel"), cl("timeline-panel"))}>
                    ...
                    <div className={cl("filters")}>
                        {filterOptions.map(option => (<button key={option.value} className={classes(cl("filter"), filter === option.value && cl("filter-active"))} onClick={() => setFilter(option.value)}>
                                {option.label}
                            </button>))}
                    </div>
                    <div className={cl("timeline-scroll")}>
                        <div className={cl("timeline")}>
                        {visibleEvents.length ? visibleEvents.map(event => (<EventRow key={event.id} event={event}/>)) : <div className={cl("empty")}>No events.</div>}
                        </div>
                    </div>
                    {filteredEvents.length > visibleEvents.length ? (<div className={cl("timeline-footer")}>
                            <span>Showing {visibleEvents.length} of {filteredEvents.length}</span>
                            <button className={cl("action")} onClick={() => setVisibleEventCount(count => count + EVENT_PAGE_SIZE)}>
                                Show more
                            </button>
                        </div>) : null}
                </section>
                ...
                <div id="vc-surveillance-targets" className={cl("target-grid")}>
                    <section className={cl("panel")}>
                        ...
                        <div className={cl("target-list")}>
                            {targets.length ? targets.map(userId => (<TargetPill key={userId} userId={userId}/>)) : <span className={cl("empty")}>No person targets.</span>}
                        </div>
                    </section>
                    ...
                    <section className={cl("panel")}>
                        ...
                        <div className={cl("server-select")}>
                            <Select placeholder="Select a server..." options={guildOptions} maxVisibleItems={8} closeOnSelect={true} select={addServerTarget} isSelected={value => serverTargets.includes(value)} serialize={value => value}/>
                        </div>
                        <div className={cl("target-list")}>
                            {serverTargets.length ? serverTargets.map(guildId => (<ServerPill key={guildId} guildId={guildId}/>)) : <span className={cl("empty")}>No server targets.</span>}
                        </div>
                    </section>
                </div>
                ...
                {settings.store.enableOsintAnalysis && (<div id="vc-surveillance-osint">
