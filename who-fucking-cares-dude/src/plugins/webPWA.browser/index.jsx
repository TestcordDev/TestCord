...
function colorToHex(color) {
    ...
    if (!ctx)
        return "#000000"; // no GPU, you'll have bigger issues than a miscolored title bar
    ctx.fillStyle = color;
...
export default definePlugin({
    ...
    flux: {
        KEYBINDS_REGISTER_GLOBAL_KEYBIND_ACTIONS: ({ keybinds }) => {
            _keybinds = keybinds;
        }
    },
        ...
        if (!IS_USERSCRIPT) {
            ...
            window.addEventListener("message", ({ data }) => {
                if (data?.type === "vencord:keybinds" && _keybinds) {
                    const { meta } = data;
                    if (meta in _keybinds)
                        _keybinds[meta].onTrigger();
                }
            }, this.ctrl);
        }
    },
