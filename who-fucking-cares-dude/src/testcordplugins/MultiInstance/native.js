...
const DISCORD_DOMAINS = ["discord.com", "ptb.discord.com", "canary.discord.com"];
const DISCORD_HOSTS = new Set(DISCORD_DOMAINS);
...
function normalizeInstanceMode(value) {
...
function isDiscordPopoutUrl(url) {
    try {
        const parsed = new URL(url);
...
function removeBlockingHeaders(responseHeaders, resourceType, url) {
...
function handleCustomProtocol(request, scheme) {
...
function registerProtocols(ses) {
...
function configureSession(partition, ses) {
...
function registerWindowControls(win) {
    const localIpc = win.webContents.ipc;
    if (!localIpc)
        return () => undefined;
    const handlers = {
...
export async function openInstance(_event, rawProfileId, rawDisplayName, rawSaveSession = true, rawDomain = "discord.com", rawBlockExternalTokenAccess = false, rawPerformanceMode = false, rawMode = "detached", rawToken = null) {
    const profileId = normalizeProfileId(rawProfileId);
    if (!profileId)
        return { ok: false, error: "Invalid instance profile." };
...
export async function getOpenInstances(_event) {
    return [...openWindows.entries()]
        .filter(([, { win }]) => !win.isDestroyed())
        .map(([id, { mode, user }]) => ({ id, mode, user }));
}
export async function reportInstanceUser(event, rawUser) {
...
export async function closeInstance(_event, rawProfileId) {
    const profileId = normalizeProfileId(rawProfileId);
    if (!profileId)
        return { ok: false, error: "Invalid instance profile." };
...
export async function closeAllInstances(_event) {
...
export async function clearSavedSession(_event, rawProfileId) {
    const profileId = normalizeProfileId(rawProfileId);
    if (!profileId)
        return { ok: false, error: "Invalid instance profile." };
    const entry = openWindows.get(profileId);
