...
export default definePlugin({
    ...
    shouldKeepMessage(message) {
        ...
        if (message.type === 24 && settings.store.allowAutoModMessages)
            return [true, suppressed];
        if (suppressed.suppressed)
            return [!suppressed.hide, true];
        if (replyToSuppressed.suppressed)
            return [!replyToSuppressed.hide, true];
        // [Message Visible, Author Blocked/Ignored]
        return [true, false];
    },
    disableNotification(message) {
        if (!message)
            return false;
    ...
    filterStream(channelStream) {
        ...
        channelStream.forEach(item => {
            ...
            const threadCreatorMessage = (isThreadStarter && item.content) || (hasThreadStarter && item.content?.[0]?.content) || null;
            ...
            if (isBlockedGroup || (isIgnoredGroup && alsoHideIgnoredUsers)) {
                const filteredContent = item.content.filter((subItem, index) => {
    ...
    isReplyToSuppressed(message) {
        if (!settings.store.hideBlockedUserReplies)
            return { suppressed: false, hide: false };
        try {
            // Messages received from the non-focused channel may have a referenced_message property.
            let repliedMessage = message.referenced_message || null;
            if (!repliedMessage) {
                const messageReference = message.messageReference || message.message_reference;
    ...
    isSuppressed(message) {
        try {
            ...
            const interactionUserRelationship = message.interaction && this.getRelationshipStatus(message.interaction.user);
            const prefixCMDReferenceData = message.author.bot && (message.messageReference || message.message_reference);
            ...
            const suppressed = (blockedContent || authorRelationship.blocked || (authorRelationship.ignored && settings.store.alsoHideIgnoredUsers) || (interactionUserRelationship && (interactionUserRelationship.blocked || (interactionUserRelationship.ignored && settings.store.alsoHideIgnoredUsers))) || (prefixCMDUserRelationship && (prefixCMDUserRelationship.blocked || (prefixCMDUserRelationship.ignored && settings.store.alsoHideIgnoredUsers))));
            const hide = !this.keepSuppressedMessage(message.author.id) || (message.interaction && !this.keepSuppressedMessage(message.interaction.user.id)) || (prefixCMDReference && !this.keepSuppressedMessage(prefixCMDReference.author.id));
