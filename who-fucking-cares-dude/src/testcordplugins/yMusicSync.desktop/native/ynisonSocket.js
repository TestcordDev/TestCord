...
export class YnisonSocket {
    ...
    constructor(socket, handlers) {
        ...
        this.socket = socket;
        socket.on("data", (chunk) => this.parseFrames(chunk));
    ...
    static connect(url, protocols, handlers, options = {}) {
            ...
            const onData = (chunk) => {
                ...
                if (separator === -1) {
                    if (head.length > 64 * 1024)
                        fail(new Error("Handshake response is too large"));
                    return;
                }
    ...
    parseFrames(chunk) {
        ...
        while (this.buffer.length >= 2) {
            ...
            if (!this.processFrame(fin, opcode, payload))
                return;
        }
    }
