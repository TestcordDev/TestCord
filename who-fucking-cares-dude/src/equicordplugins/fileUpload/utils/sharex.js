...
function parseDestinationTypes(destinationType) {
    if (!destinationType)
        return [];
...
export function parseShareXConfig(configText) {
    let parsed;
...
export function isSupportedShareXDestination(config) {
...
export function resolveShareXTemplate(template, responseText, responseJson) {
    if (!template)
        return undefined;
    ...
    output = output.replace(JSON_TEMPLATE_REGEX, (_, pathA, pathB) => {
