...
const isApiPlugin = (plugin) => plugin.name?.endsWith("API") || plugin.required;
...
function forkOf(name) {
    const folder = PluginMeta[name]?.folderName ?? "";
    if (PluginMeta[name]?.userPlugin || folder.startsWith("src/userplugins/"))
        return "user";
...
function computeStats() {
    ...
    for (const plugin of Object.values(Vencord.Plugins.plugins)) {
        if (!plugin?.name || isApiPlugin(plugin))
            continue;
        ...
        if (on) {
            ...
            if (plugin.requiresRestart || (plugin.patches?.length && plugin.requiresRestart !== false))
                restartNeeded++;
            if (plugin.patches?.length) {
                caps.Patches++;
                totalPatches += plugin.patches.length;
            }
            if (plugin.commands?.length)
                caps.Commands++;
            if (plugin.contextMenus && Object.keys(plugin.contextMenus).length)
                caps["Context menus"]++;
            if (plugin.flux && Object.keys(plugin.flux).length)
                caps["Flux events"]++;
            if (plugin.userProfileBadge || plugin.userProfileBadges?.length)
                caps["Profile badges"]++;
            if (plugin.chatBarButton || plugin.renderChatBarButton)
                caps["Chat bar buttons"]++;
            if (plugin.onBeforeMessageSend || plugin.onBeforeMessageEdit || plugin.onMessageClick)
                caps["Message hooks"]++;
            if (plugin.settings)
                caps.Settings++;
        }
...
function StatRow({ label, value }) {
    ...
    return (<div className={`vc-stats-row${canCopy ? " vc-stats-row-copyable" : ""}`} role={canCopy ? "button" : undefined} tabIndex={canCopy ? 0 : undefined} title={canCopy ? "Click to copy" : undefined} onClick={copy} onKeyDown={e => {
...
function StatCard({ title, rows }) {
    if (rows.length === 0)
        return null;
    return (<Card className={`vc-stats-card ${Margins.bottom16}`}>
...
function HeroStat({ value, label, tone }) {
    return (<Card className={`vc-stats-card vc-stats-hero vc-stats-hero-${tone}`}>
...
function StatsTab() {
    ...
    return (<SettingsTab>
            ...
            ...(s.forks.user.total > 0 ? [["User plugins", fork("user")]] : []),
        ]}/>

            <StatCard title="Capabilities (enabled plugins)" rows={Object.entries(s.caps).filter(([, n]) => n > 0).sort((a, b) => b[1] - a[1])}/>

            <StatCard title="Top authors (by enabled plugins)" rows={s.authors.map(([name, n]) => [name, n])}/>
