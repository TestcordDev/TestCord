...
function makeBoostDate(userId, boostMonthsIndex) {
...
function makeNitroDate(userId, nitroMonthsIndex) {
...
function getDecorationUrl(assetId, animated = false) {
...
let cachedOriginalUser = null;
let cachedFakeUser = null;
let cachedDataHash = 0;
let _trueOriginalUser = null;
let _dataVersion = 0;
let allAccountsData = {};
let allAccountsEnabled = {};
const _patchRestores = [];
...
function loadDataSync() {
    try {
        ...
        if (rawAll) {
            try {
                allAccountsData = JSON.parse(rawAll);
            }
            catch {
                allAccountsData = {};
            }
            ...
            try {
                allAccountsEnabled = rawEnabled ? JSON.parse(rawEnabled) : {};
            }
            catch {
                allAccountsEnabled = {};
            }
...
function applyAvatarPatchEarly() {
    if (_avatarPatchApplied || !isEnabled || !storedData.avatar)
        return;
    try {
        const IU = window.Vencord?.Webpack?.findByProps?.("getUserAvatarURL");
        if (!IU?.getUserAvatarURL)
            return;
        const orig = IU.getUserAvatarURL;
        IU.getUserAvatarURL = function (user, ...args) {
...
async function loadData() {
    try {
        const allData = await DataStore.get(DS_ALL_DATA);
        const allEnabled = await DataStore.get(DS_ALL_ENABLED);
...
async function copyUserProfile(userId) {
    try {
        const user = UserStore.getUser(userId);
        if (!user)
            return;
        const { findByProps } = await import("@webpack");
        const UserProfileStore = findByProps("getUserProfile", "getGuildMemberProfile");
        const IU = IconUtils;
        ...
        if (hasNitro) {
            ...
            if (!isNaN(pTs)) {
                ...
                if (months >= 72)
                    newData.nitroLevel = 8;
                else if (months >= 36)
                    newData.nitroLevel = 7;
                else if (months >= 24)
                    newData.nitroLevel = 6;
                else if (months >= 12)
                    newData.nitroLevel = 5;
                else if (months >= 6)
                    newData.nitroLevel = 4;
                else if (months >= 3)
                    newData.nitroLevel = 3;
                else if (months >= 2)
                    newData.nitroLevel = 2;
                else if (months >= 1)
                    newData.nitroLevel = 1;
                else
        ...
        if (boostSince) {
            ...
            if (!isNaN(bTs)) {
                ...
                if (bMonths >= 24)
                    newData.boostMonths = 8;
                else if (bMonths >= 18)
                    newData.boostMonths = 7;
                else if (bMonths >= 15)
                    newData.boostMonths = 6;
                else if (bMonths >= 12)
                    newData.boostMonths = 5;
                else if (bMonths >= 9)
                    newData.boostMonths = 4;
                else if (bMonths >= 6)
                    newData.boostMonths = 3;
                else if (bMonths >= 3)
                    newData.boostMonths = 2;
                else if (bMonths >= 2)
                    newData.boostMonths = 1;
                else
                    newData.boostMonths = 0;
            }
        }
        const bannerId = profile.banner ?? user.banner ?? null;
        if (bannerId)
            newData.banner = `https://cdn.discordapp.com/banners/${userId}/${bannerId}.${bannerId.startsWith("a_") ? "gif" : "png"}?size=512`;
        if (profile.accentColor !== undefined)
        ...
        try {
            ...
            if (user.avatarDecorationData?.asset)
                newData.decorationAsset = user.avatarDecorationData.asset;
        }
        catch { }
...
const userContextMenuPatch = (children, { user }) => {
    if (!children || !Array.isArray(children) || !user || !user.id)
        return;
    try {
        ...
        children.push(<Menu.MenuGroup>
                {isCopied ? (<Menu.MenuItem id="remove-copy-profile" label={t("Remove copy profile")} color="danger" action={() => {
                    try {
                        const myId = AuthenticationStore?.getId?.();
                        if (myId) {
                ...
                }}/>) : (<Menu.MenuItem id="copy-user-profile" label={t("Copy this profile")} action={() => copyUserProfile(user.id)}/>)}
            </Menu.MenuGroup>);
    }
    catch (err) {
...
function getRealNames() {
    try {
        ...
        return { username: null, globalName: null };
    }
}
let _realDateKey = null;
let _realDateVariants = [];
function getRealDateVariants() {
    try {
        const u = UserStore.getCurrentUser();
        if (!u?.id)
            return [];
        if (u.id === _realDateKey)
            return _realDateVariants;
        _realDateKey = u.id;
        ...
        const mS = monthsShort[d.getMonth()];
        const mL = monthsLong[d.getMonth()];
        ...
        _realDateVariants = [...variants].filter(v => v.length >= 4);
        return _realDateVariants;
    }
    catch {
        return [];
...
let _fakeDateKey = null;
let _fakeDateVariants = [];
function getFakeDateVariants(isoDate) {
    try {
        if (isoDate === _fakeDateKey)
        ...
        for (const fmt of fmtSpecs) {
            try {
                variants.add(new Intl.DateTimeFormat(navigator.language, fmt).format(d));
            }
            catch { }
        }
...
function scanTextNode(node) {
    if (!isEnabled || !node.nodeValue)
        return;
    const val = node.__cp_orig || node.nodeValue;
    let result = val;
    try {
    ...
    if (_realUsername && storedData.username && result.includes(_realUsername)) {
        result = result.split(_realUsername).join(storedData.username);
        replaced = true;
    }
    if (_realGlobalName && storedData.globalName && result.includes(_realGlobalName)) {
        result = result.split(_realGlobalName).join(storedData.globalName);
        replaced = true;
    }
    if (replaced && result !== node.nodeValue) {
        if (node.__cp_orig === undefined)
            node.__cp_orig = val;
...
function hasTextReplacements() {
    if (storedData.createdAt)
        return true;
    if (storedData.username && _realUsername)
        return true;
    if (storedData.globalName && _realGlobalName)
        return true;
    return false;
}
function startDomObserver() {
    ...
    domObserver = setInterval(() => {
        if (!isEnabled || !hasTextReplacements())
            return;
        scanNode(document.body);
    }, 5000);
}
function stopDomObserver() {
    if (domObserver) {
        clearInterval(domObserver);
        domObserver = null;
    }
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
    let n;
    while ((n = walker.nextNode())) {
        if (n.__cp_orig !== undefined) {
            n.nodeValue = n.__cp_orig;
            delete n.__cp_orig;
        }
    }
}
...
function NitroBadgeTooltip({ icon, tierName, dateStr }) {
    const title = tierName ? `NITRO ${tierName.toUpperCase()}` : "NITRO";
    return (<Tooltip text={<div style={{
                display: "flex",
                ...
                gap: 2,
                    ...
                    <img src={icon} alt="" style={{ width: 60, height: 60, objectFit: "contain", marginBottom: 4 }}/>
                    <div style={{
                fontWeight: 700,
                fontSize: 14,
            ...
            {(tooltipProps) => (<img {...tooltipProps} src={icon} alt={title} style={{ borderRadius: "50%", width: "22px", height: "22px" }}/>)}
        </Tooltip>);
}
...
function BadgeBtn({ label, icon, active, onClick }) {
...
function BadgePicker({ selected, onChange, nitroType, onNitroType, boostLevel, onBoostLevel, customIds, onCustomIds, oldName, onOldName }) {
    ...
    return (<div className="cp-field">
            ...
            <div className="cp-badges">
                {BADGES.map(b => (<BadgeBtn key={b.flag} label={b.label} icon={b.icon} active={!!(selected & b.flag)} onClick={() => onChange(selected ^ b.flag)}/>))}
            </div>
            ...
            <div className="cp-badges">
                ...
                {NITRO_LEVELS.map((n, i) => (<BadgeBtn key={i} label={n.label} icon={n.icon} active={nitroType === i} onClick={() => {
                onNitroType(i);
            ...
            <div className="cp-badges">
                <BadgeBtn label={t("Completed a quest")} icon="https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png" active={customIds.includes("quest")} onClick={() => onCustomIds(customIds.includes("quest") ? customIds.filter(x => x !== "quest") : [...customIds, "quest"])}/>
                <BadgeBtn label={t("Orbs — Apprentice")} icon="https://cdn.discordapp.com/badge-icons/83d8a1eb09a8d64e59233eec5d4d5c2d.png" active={customIds.includes("orbs")} onClick={() => onCustomIds(customIds.includes("orbs") ? customIds.filter(x => x !== "orbs") : [...customIds, "orbs"])}/>
                <BadgeBtn label={t("Old username")} icon={OLD_NAME_BADGE_ICON} active={hasOldName} onClick={() => onCustomIds(hasOldName ? customIds.filter(x => x !== "oldname") : [...customIds, "oldname"])}/>
            </div>
            {hasOldName && (<div className="cp-field" style={{ marginTop: 6 }}>
...
function CustomProfileModal({ rootProps }) {
    const myId = AuthenticationStore?.getId?.() || "";
    const [selectedAccountId, setSelectedAccountId] = React.useState(myId);
    const [data, setData] = React.useState(() => ({ ...(allAccountsData[myId] || storedData || {}) }));
    ...
    const accounts = React.useMemo(() => {
        try {
            ...
            const MAS = window.Vencord?.Webpack?.findByProps?.("getUsers", "getValidUsers");
            if (MAS?.getUsers) {
            ...
            const internalStore = window.Vencord?.Webpack?.findStore?.("MultiAccountStore");
            ...
            console.error("[CustomProfile] Failed to fetch accounts:", e);
        }
        const me = UserStore.getCurrentUser();
    ...
    return (<ModalRoot {...rootProps} size="medium">
            <ModalHeader separator={false}>
                ...
                <div style={{ marginLeft: "auto", marginRight: 8, minWidth: 200 }}>
                    <Select options={accounts.map((acc) => ({
            value: acc.id,
            label: acc.globalName || acc.username,
        }))} isSelected={(v) => v === selectedAccountId} select={(v) => setSelectedAccountId(v)} serialize={(v) => v} renderOptionLabel={(o) => (<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <img src={IconUtils.getUserAvatarURL(accounts.find((a) => a.id === o.value), false, 20)} style={{ borderRadius: "50%", width: 20, height: 20 }}/>
                                {o.label}
                            </div>)} renderOptionValue={(selected) => {
            const option = selected[0];
            if (!option)
                return "Select Account";
            return (<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    <img src={IconUtils.getUserAvatarURL(accounts.find((a) => a.id === option.value), false, 20)} style={{ borderRadius: "50%", width: 20, height: 20 }}/>
                                    {option.label}
                                </div>);
        }}/>
                </div>
            ...
            <ModalContent className="cp-content">
                ...
                <div className="cp-field">
                    ...
                    <div className="cp-color-row" style={{ marginBottom: 6 }}>
                        ...
                        <input type="color" value={accentHex || "#5865f2"} onChange={e => { const n = parseInt(e.target.value.replace("#", ""), 16); if (!isNaN(n))
        set("accentColor", n); }} className="cp-color-swatch"/>
                        <input value={accentHex} placeholder="#5865f2" onChange={e => { const h = e.target.value.replace("#", ""); const n = parseInt(h, 16); if (!isNaN(n) && h.length === 6)
        set("accentColor", n);
    else if (!e.target.value || e.target.value === "#")
        set("accentColor", undefined); }} className="cp-input cp-color-input"/>
            ...
            return (<>
                                <input type="color" value={hex2 || "#eb459e"} onChange={e => { const n = parseInt(e.target.value.replace("#", ""), 16); if (!isNaN(n))
                set("accentColor2", n); }} className="cp-color-swatch"/>
                                <input value={hex2} placeholder="#eb459e (optional)" onChange={e => { const h = e.target.value.replace("#", ""); const n = parseInt(h, 16); if (!isNaN(n) && h.length === 6)
                set("accentColor2", n);
            else if (!e.target.value || e.target.value === "#")
                set("accentColor2", undefined); }} className="cp-input cp-color-input"/>
                ...
                <div className="cp-divider"/>
                <BadgePicker selected={data.badgeFlags ?? 0} onChange={v => set("badgeFlags", v)} nitroType={nitroLevel} onNitroType={v => {
            set("nitroLevel", v);
            // Levels 1+ (Bronze to Opal) automatically enable Simulate Nitro
            // Level 0 (Nitro without months) and None (-1) do not enable it
            if (v >= 1)
                set("nitro", true);
        }} boostLevel={boostLevel} onBoostLevel={v => set("boostMonths", v)} customIds={customIds} onCustomIds={v => set("customBadgeIds", v)} oldName={oldName} onOldName={v => set("oldName", v)}/>
                <div className="cp-divider"/>
                ...
                <div className="cp-badges" style={{ flexWrap: "wrap", gap: 6 }}>
                    ...
                    {AVATAR_DECORATIONS.map(dec => (<button key={dec.id} onClick={() => set("decorationAsset", data.decorationAsset === dec.id ? undefined : dec.id)} className={`cp-badge ${data.decorationAsset === dec.id ? "cp-badge--on" : ""}`} title={dec.label} style={{ padding: 3, lineHeight: 0, width: 52, height: 52, borderRadius: 6 }}>
...
export default definePlugin({
    ...
    _copiedUserId: null,
    isCopiedUser(userId) {
        if (!isEnabled || !userId || !this._copiedUserId)
            return false;
        return userId === this._copiedUserId;
    },
    fakeCurrentUser(user) {
        if (!user || (!isEnabled && this._forceNative !== true) || !isMe(user.id))
            return user;
        ...
        const realUser = user.__cp_isClone ? _trueOriginalUser || user : user;
        if (!realUser.__cp_isClone)
            _trueOriginalUser = realUser;
        ...
        for (const key of Reflect.ownKeys(realUser)) {
            if (key === "username" || key === "globalName" || key === "displayName" || key === "__cp_isClone")
                continue;
        ...
        if (storedData.email)
            clone.email = storedData.email;
        if (storedData.phone)
            clone.phone = storedData.phone;
    ...
    _cachedProfile: null,
    _cachedProfileInput: null,
    _cachedProfileVersion: 0,
    hookUserProfile(profile) {
        ...
        try {
            const merged = {};
            if (storedData.bio)
                merged.bio = storedData.bio;
            if (storedData.pronouns)
                merged.pronouns = storedData.pronouns;
            if (storedData.accentColor != null)
                merged.accentColor = storedData.accentColor;
            if (storedData.banner)
                merged.banner = storedData.banner;
            if (storedData.decorationAsset) {
                const decoData = {
                    asset: storedData.decorationAsset,
                    skuId: storedData.decorationAsset
            ...
            if (isEnabled && (storedData.nitro || storedData.badgeFlags != null)) {
                ...
                if (profile.premiumType)
                    merged.premiumType = profile.premiumType;
    ...
    fakeObfuscatedEmail(real) {
        if (!isEnabled || !storedData.email || !real)
            return real;
    ...
    fakeObfuscatedPhone(real) {
        if (!isEnabled || !storedData.phone || !real)
            return real;
    ...
    patchBannerUrl({ displayProfile }) {
        if (!isEnabled || !storedData.nitro || !storedData.banner)
            return null;
        try {
            return isMe(displayProfile?.userId) ? storedData.banner : null;
        }
        catch {
            return null;
        }
    ...
    async start() {
        ...
        try {
            ...
            if (US && !US._cp_perfect_hook) {
                ...
                let _lastRealUser = null;
                let _lastFakeResult = null;
                    ...
                    if (realUser) {
                        ...
                        if (realUser !== _lastRealUser) {
                            if (realUser.username)
                                _realUsername = realUser.username;
                            if (realUser.globalName)
                                _realGlobalName = realUser.globalName;
                        }
                ...
                US.getUser = (id) => isMe(id) ? this.fakeCurrentUser(origGet(id)) : origGet(id);
        ...
        try {
            const UPS = Vencord.Webpack?.findByProps?.("getUserProfile", "getGuildMemberProfile");
            if (UPS && !UPS._cp_profile_hook) {
                const origGetProfile = UPS.getUserProfile.bind(UPS);
                UPS.getUserProfile = (userId) => {
                    try {
                        const profile = origGetProfile(userId);
                        if (!isEnabled || !userId || !isMe(userId) || !profile)
                            return profile;
                ...
                UPS.getGuildMemberProfile = (userId, guildId) => {
                    try {
        ...
        try {
            if (SnowflakeUtils?.extractTimestamp && !this._origExtractTimestamp) {
                ...
                SnowflakeUtils.extractTimestamp = (snowflake) => {
        ...
        loadData().then(() => {
            if (generation !== customProfileGeneration)
                return;
            updateCachedRealData();
            applyAvatarPatchEarly();
            if (isEnabled) {
        ...
        try {
            const decoMod = Vencord.Webpack?.findByProps?.("getAvatarDecorationURL");
            if (decoMod?.getAvatarDecorationURL && !decoMod._cp_deco_hook) {
                ...
                decoMod.getAvatarDecorationURL = (opts) => {
                    try {
        ...
        if (IconUtils?.getUserAvatarURL && !_avatarPatchApplied) {
            ...
            IconUtils.getUserAvatarURL = (user, ...args) => {
                if (isEnabled && storedData.avatar) {
                    const uid = user?.id ?? user?.userId;
                    if (uid && isMe(uid))
                        return storedData.avatar;
    ...
    userProfileBadges: [
        {
            getBadges({ userId, badges: nativeBadges }) {
                if (!isEnabled)
                    return nativeBadges || [];
                if (userId !== UserStore.getCurrentUser()?.id)
                    return nativeBadges || [];
                let badges = [...(nativeBadges || [])];
                ...
                badges = badges.filter(b => {
                    ...
                    if (isEnabled) { // ALWAYS filter native nitro/boost if plugin is enabled for this user
                        ...
                        if (nitroKeywords.some(k => desc.includes(k)))
                            return false;
                        if (icon.includes("nitro") || icon.includes("premium"))
                            return false;
                        const boostKeywords = ["booster", "boost"];
                        if (boostKeywords.some(k => desc.includes(k)))
                            return false;
                        if (icon.includes("boost") || icon.includes("leveling"))
                            return false;
                    }
                    ...
                    for (const badge of BADGES) {
                        if (wantedFlags & badge.flag) {
                            ...
                            if (badgeKeywords.some(k => k.length > 3 && desc.includes(k)))
                                return false;
                ...
                if (hasNitroFake) {
                    ...
                    badgeList.push({
                        ...
                        key: "cp-nitro-badge",
                        props: { style },
                        component: () => (<NitroBadgeTooltip icon={icon} tierName={tierName} dateStr={dateStr}/>),
                    });
                }
                // 4. HypeSquad Events
    ...
    stop() {
        ...
        if (this._origExtractTimestamp && SnowflakeUtils) {
            SnowflakeUtils.extractTimestamp = this._origExtractTimestamp;
            this._origExtractTimestamp = null;
        }
        if (this._origGetUserAvatarURL && IconUtils) {
            IconUtils.getUserAvatarURL = this._origGetUserAvatarURL;
        ...
        try {
            ...
            if (myUser) {
                try {
                    delete myUser.avatarDecoration;
                }
                catch { }
                try {
                    delete myUser.avatarDecorationData;
                }
                catch { }
            }
        }
