...
import { ContextMenuApi, FluxDispatcher, Menu, React, showToast, useCallback, useMemo, useRef, useState, } from "@webpack/common";
function EditableSong({ index, song, insert, setSongRef, onDrag, onDrop, onRemove }) {
    const { render, failed } = useRender(song);
    const [dragging, setDragging] = useState(false);
    const refCallback = useCallback((div) => setSongRef(index, div), [index, setSongRef]);
    const dragCallback = useCallback((event) => {
        setDragging(true);
        onDrag(index, event);
    }, [index, onDrag]);
    ...
    return (<Link href={render?.link} onContextMenu={e => ContextMenuApi.openContextMenu(e, () => (<Menu.Menu navId="vc-songspotlight-editable-song" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label={render?.label}>
                        <Menu.MenuItem id="copy-link" label="Copy link" icon={LinkIcon} action={() => render && copyWithToast(render.link)} disabled={!render}/>
                        <Menu.MenuItem id="remove-song" color="danger" label="Remove song" icon={TrashIcon} action={() => onRemove(song)}/>
                    </Menu.Menu>))} draggable="true" data-dragging={dragging} data-insert={insert} onDragStart={dragCallback} onDrag={dragCallback} onDragEnd={dragEndCallback} ref={refCallback} className={cl("editable-song-container")}>
...
export default function SongList({ localData, setLocalData }) {
    const editable = useMemo(() => {
        ...
        return new Array(apiConstants.songLimit).fill(null).map((_, i) => {
            ...
            else if (localData[i - 1] || i === 0)
                return { slot: "add" };
            else
                return { slot: "empty" };
        });
    }, [localData]);
    const songs = useRef(new Map());
    const [insert, setInsert] = useState();
    const handleRef = useCallback((index, element) => {
        if (element)
            songs.current.set(index, element);
        else
            songs.current.delete(index);
    }, []);
    const handleDrag = useCallback((index, event) => {
    ...
    const handleDrop = useCallback((song) => {
        if (insert === undefined)
            return;
        setInsert(undefined);
    ...
    const handleAdd = useCallback((song) => {
        if (localData.length >= apiConstants.songLimit)
            return "Not enough space";
        if (localData.some(x => sid(x) === sid(song)))
            return "You've already added this song";
    ...
    return (<Flex flexDirection="column" gap="6px">
            {editable.map(({ slot, song, last }, i) => {
            if (slot === "song") {
                return (<EditableSong index={i} song={song} insert={last && insert === i + 1 ? "bottom" : insert === i ? "top" : undefined} setSongRef={handleRef} onDrag={handleDrag} onDrop={handleDrop} onRemove={handleRemove} key={sid(song)}/>);
            }
            else if (slot === "add") {
                return <AddSong onAdd={handleAdd} key={slot}/>;
            }
            else
                return <div className={cl("empty-song")} key={`${slot}-${i}`}/>;
        })}
        </Flex>);
}
