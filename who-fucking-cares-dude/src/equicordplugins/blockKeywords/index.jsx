...
function RegexHelper() {
    ...
    return (<Card className={cl("regex")}>
            <HeadingTertiary className={Margins.bottom8}>Regex Helper</HeadingTertiary>
            <TextInput type="text" placeholder="Input to test..." value={testInput} onChange={setTestInput} maxLength={null}/>
            {results.length === 0 ?
            <Card key="vc-no-patterns-regex" variant="warning" className={classes(cl("card"), Margins.top8)}>
                    <code>No patterns configured</code>
                </Card> : (results.map(({ pattern, matches, error }, i) => (<Card key={`vc-pattern-card-${i}`} variant={error ? "danger" : matches ? "success" : "primary"} className={classes(cl("card"), Margins.top8)}>
                            <code>{pattern}</code>
...
export function containsBlockedKeywords(message) {
    if (!blockedKeywords)
        return false;
    if (!message.content && !message.embeds?.length)
        return false;
    const testField = (text) => text != null && blockedKeywords.some(regex => regex.test(text));
    return blockedKeywords.some(regex => regex.test(message.content)) || message.embeds.some(embed => testField(embed.rawDescription) || testField(embed.rawTitle));
}
export default definePlugin({
    ...
    blockMessagesWithKeywords(messageList) {
        return messageList.reset(messageList.map(message => message.set("blocked", message.blocked || this.containsBlockedKeywords(message))));
    }
});
