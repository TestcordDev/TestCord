...
function createThumbnailUrl(url, isVideo = false) {
    if (!url || url.endsWith("#"))
        return url;
    try {
...
export async function loadAllMediaFromAPI(channelId, apiRequestDelay, signal) {
    const allMessages = [];
    let before = null;
    const limit = 100;
    ...
    while (page <= maxPages) {
        if (signal?.aborted || !channelId || channelId === "undefined")
            break;
        try {
            ...
            const mediaMessages = response.body.filter((msg) => {
...
export async function searchMediaMessages(channelId, query, cacheOnly, apiRequestDelay, signal) {
    ...
    let cached = null;
    try {
        const cachedData = await DataStore.get(cacheKey);
            ...
            if (contentMatch) {
                ...
                for (const media of mediaUrls) {
                    mediaItems.push({
                        ...
                        channelId: channelId,
                        userId: userId,
                        timestamp: msg.timestamp?.valueOf() || msg.timestamp || Date.now()
                    });
                }
            }
        }
    ...
    if (mediaItems.length > 0 && !cacheOnly) {
        ...
        try {
            const existingCache = await DataStore.get(mediaItemsCacheKey);
...
function getAvatarURL(user, channel) {
    if (!user)
        return null;
    try {
...
export function MediaGrid({ displayedResults, navigateToMessage, setSelectedIndex, selectedIndex }) {
    ...
    const mediaItems = useMemo(() => {
        ...
        for (const result of displayedResults) {
            ...
            const user = result.user || UserStore.getUser(message.author?.id || message.author_id);
    ...
    return (<div className={cl("media-grid")} style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", width: "100%", gap: "2px" }}>
            ...
            return (<div key={`${channelId}-${messageId}-${index}`} className={cl("media-item")} style={{
                    aspectRatio: "1 / 1",
                    width: "100%",
                    paddingBottom: 0
                }} onClick={() => navigateToMessage({ message: item.message, channel: item.channel, matchType: "attachment" })} onMouseEnter={() => setSelectedIndex(index)}>
                        <div className={cl("media-thumbnail")}>
                            {item.type === "video" ? (<>
                                    <img src={item.thumbnailUrl || item.url} alt="" className={cl("media-image")} style={{
                        width: "100%",
                        height: "100%",
                        objectFit: "cover",
                        ...
                        {avatarUrl && (<div className={cl("media-user-avatar")}>
                                <Avatar src={avatarUrl} size="SIZE_24" className={cl("media-avatar")}/>
                            </div>)}
                    </div>);
        })}
        </div>);
