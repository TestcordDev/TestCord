...
function walkLeafIds(workspace, nodeId, leaves) {
    if (!nodeId)
        return;
...
function buildRects(workspace, nodeId, rect, out) {
    if (!nodeId)
        return;
    const node = workspace.nodesById[nodeId];
    if (!node)
...
function findPathToTile(workspace, nodeId, tileId, path) {
    if (!nodeId)
        return null;
    const node = workspace.nodesById[nodeId];
    if (!node)
...
export function getLeafIds(workspace) {
    const leaves = [];
    walkLeafIds(workspace, workspace.rootNodeId, leaves);
    return leaves;
}
export function findLeafByTileId(workspace, tileId) {
...
export function computeLayoutRects(workspace) {
...
export function findDirectionalCandidate(workspace, fromTileId, direction, wrap) {
...
export function findResizableSplit(workspace, tileId, edge) {
    ...
    for (let i = path.length - 1; i >= 0; i--) {
        ...
        if (!isSplit(split))
            continue;
        if (edge === "left" && split.axis === "x" && entry.side === "secondId")
            return split.id;
        if (edge === "right" && split.axis === "x" && entry.side === "firstId")
            return split.id;
        if (edge === "up" && split.axis === "y" && entry.side === "secondId")
            return split.id;
        if (edge === "down" && split.axis === "y" && entry.side === "firstId")
            return split.id;
    }
    return null;
}
