...
async function setAllGuildsMuted(muted) {
...
const guildContextPatch = (children, { guild }) => {
    if (!children || !Array.isArray(children) || !guild)
        return;
    try {
        children.splice(-1, 0, (<Menu.MenuGroup key="nc-muteunmute-group">
                <Menu.MenuItem id="nc-mute-all-servers" label="Mute all servers" icon={() => <MuteIcon />} action={() => setAllGuildsMuted(true)}/>
                <Menu.MenuItem id="nc-unmute-all-servers" label="Unmute all servers" icon={() => <UnmuteIcon />} action={() => setAllGuildsMuted(false)}/>
            </Menu.MenuGroup>));
    }
    catch (e) {
