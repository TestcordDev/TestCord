...
import { closeSpecificTileAndNavigate, focusTileById, getTileDisplayInfo, isHyprTilesRunning, moveTileByDropAndNavigate, switchWorkspaceAndNavigate, syncRouteTarget, } from "../controller";
...
function toPreviewStyle(bounds, preview) {
    if (!preview)
        return toStyle(bounds);
    return {
        ...toStyle(bounds),
        width: `${preview.w * 100}%`,
...
function resizeFloatingBounds(startBounds, corner, dx, dy) {
...
function WorkspaceHostComponent({ routeTarget, routeElement }) {
    ...
    const [floatDragState, setFloatDragState] = useState(null);
    const [resizeState, setResizeState] = useState(null);
    const dragSessionRef = useRef(null);
    const floatDragSessionRef = useRef(null);
    const resizeSessionRef = useRef(null);
    const dragFrameRef = useRef(null);
    const resizeFrameRef = useRef(null);
    const [floatPreview, setFloatPreview] = useState(null);
    const floatPreviewRef = useRef(null);
    const floatPreviewFrameRef = useRef(null);
    ...
    const allTileIds = [
        ...
        ...Object.values(state.scratchpadsById).map(scratchpad => scratchpad.tileId).filter(Boolean)
    ];
    ...
    function queueFloatPreview(tileId, bounds) {
    ...
    function consumeFloatPreview(tileId, fallback) {
    ...
    function getRenderedFloatBounds(tileId, bounds) {
    ...
    useEffect(() => {
        function onPointerMove(event) {
            ...
            dragFrameRef.current = requestAnimationFrame(() => {
                ...
                const hovered = document.elementFromPoint(event.clientX, event.clientY)?.closest("[data-hypr-tile-id]");
    ...
    useEffect(() => {
        function onPointerMove(event) {
            ...
            if (!activeFloatDrag || event.pointerId !== activeFloatDrag.pointerId)
                return;
        ...
        function finishFloatDrag(event) {
            const activeFloatDrag = floatDragSessionRef.current;
            if (!activeFloatDrag || event && event.pointerId !== activeFloatDrag.pointerId)
                return;
            floatDragSessionRef.current = null;
            activeFloatDrag.endInteraction();
            if (activeFloatDrag.target.hasPointerCapture(activeFloatDrag.pointerId))
                activeFloatDrag.target.releasePointerCapture(activeFloatDrag.pointerId);
            if (event?.type === "pointerup")
                updateTileFloatBounds(activeFloatDrag.tileId, consumeFloatPreview(activeFloatDrag.tileId, activeFloatDrag.startBounds));
            else
                consumeFloatPreview(activeFloatDrag.tileId, activeFloatDrag.startBounds);
    ...
    useEffect(() => {
        function onPointerMove(event) {
            ...
            if (activeResize.kind === "floating") {
                ...
                queueFloatPreview(activeResize.tileId, resizeFloatingBounds(activeResize.startBounds, activeResize.corner, dx, dy));
                return;
            }
            if (!workspace)
                return;
            ...
            resizeFrameRef.current = requestAnimationFrame(() => {
                ...
                for (const splitId of [activeResize.horizontalSplitId, activeResize.verticalSplitId]) {
                    ...
                    if (!splitRect || !split || split.kind !== "split")
                        continue;
        ...
        function finishResize(event) {
            ...
            if (activeResize.kind === "floating") {
                ...
                if (event?.type === "pointerup")
                    updateTileFloatBounds(activeResize.tileId, bounds);
            }
            setResizeState(null);
        }
        function onVisibilityChange() {
    ...
    if (!pluginRunning || !routeTarget || !workspace)
        return <>{routeElement}</>;
    const renderTileShell = (tile, contentStyle, options) => {
        ...
        return (<section key={tile.id} className={classes(cl("tile"), options.floating && cl("tile-floating"), options.focused && cl("tile-focused"), !options.focused && cl("tile-unfocused"), dragState?.tileId === tile.id && cl("tile-dragging"), liveBoundsTileId === tile.id && cl("tile-live-bounds"))} style={contentStyle} data-hypr-tile-id={tile.id}>
                {showTileHeaders && (<header className={cl("tile-header")} onPointerDown={options.onHeaderMouseDown}>
                        {options.header ?? (<>
                                <button type="button" className={cl("tile-title-button")} onClick={() => {
                        if (!options.focused)
                            void focusTileById(tile.id);
                    }}>
                                    <span className={cl("tile-badge")}>{info.badge}</span>
                                    {info.title !== info.badge && <span className={cl("tile-title")}>{info.title}</span>}
                                </button>
                                <button type="button" className={cl("tile-close")} onClick={event => {
                ...
                <div className={cl("tile-body")}>
                    {options.body ?? (isPrimary
                ? routeElement
                : <TileContent tile={tile} active={options.focused}/>)}
                </div>

                {!options.focused && (<button type="button" className={cl("tile-overlay")} aria-label={`Focus ${info.title}`} onClick={() => void focusTileById(tile.id)}/>)}

                {options.onResizeStart && resizeCorners.map(corner => (<button key={`${tile.id}-${corner}`} type="button" className={classes(cl("resize-handle"), cl(`resize-${corner}`))} aria-label={`Resize ${info.title}`} onPointerDown={event => options.onResizeStart?.(corner, event)}/>))}
            </section>);
    };
    return (<div className={classes(cl("root"), enableAnimations && cl("animated"))}>
            ...
            const header = leaf.tileIds.length > 1 && (<div className={cl("tab-strip")}>
                            {leaf.tileIds.map(tileId => {
                    const tileInfo = tileDisplayMap[tileId] ?? { title: "Tile", badge: "CHAN" };
                    return (<button key={tileId} type="button" className={classes(cl("tab"), leaf.activeTileId === tileId && cl("tab-active"))} onClick={() => void focusTileById(tileId)}>
                                        {tileInfo.title}
                                    </button>);
                })}
                        </div>);
            const body = (<div className={cl("leaf-body")}>
                            {leaf.tileIds.map(tileId => {
                    ...
                    return (<div key={tileId} className={classes(cl("tab-panel"), !active && cl("tab-panel-hidden"))}>
                                        {primary && active
                            ? routeElement
            ...
            if (!activeTile)
                return null;
            return (<React.Fragment key={leafId}>
                            {renderTileShell(activeTile, style, {
                    focused,
                    header: (<>
                                        <button type="button" className={cl("tile-title-button")} onClick={() => {
                            if (!focused)
                                void focusTileById(leaf.activeTileId);
                        }}>
                    ...
                    onResizeStart: (corner, event) => {
                        ...
                        if (!horizontalSplitId && !verticalSplitId)
                            return;
                        event.preventDefault();
                        event.stopPropagation();
                        const resize = { kind: "tiled", horizontalSplitId, verticalSplitId };
                        event.currentTarget.setPointerCapture(event.pointerId);
            ...
            return renderTileShell(tile, toPreviewStyle(tile.floatBounds, floatPreview?.tileId === tile.id ? renderedBounds : undefined), {
                ...
                {scratchpads.map(({ scratchpad, tile }) => renderTileShell(tile, toPreviewStyle(tile.floatBounds ?? scratchpad.bounds, floatPreview?.tileId === tile.id ? floatPreview.bounds : undefined), {
            focused: workspace.focusedTileId === tile.id,
            floating: true,
            onHeaderMouseDown: event => {
                if (event.button !== 0)
            ...
            return renderTileShell(tile, {
                ...
                {dragState?.targetTileId && dragState.zone && (<div className={classes(cl("drop-overlay"), cl(`drop-${dragState.zone}`), dragState.groupOnCenter && cl("drop-group"))} style={layoutRects.rects[dragState.targetTileId] ? {
                ...
                {overviewOpen && (<div className={cl("overview")}>
                ...
                return (<button key={workspaceId} type="button" className={classes(cl("overview-card"), Number(workspaceId) === state.activeWorkspace && cl("overview-card-active"))} onClick={() => {
                        switchWorkspaceAndNavigate(Number(workspaceId));
                        toggleWorkspaceOverview();
                    }}>
                                        <span className={cl("overview-card-title")}>Workspace {workspaceId}</span>
                                        <span className={cl("overview-mini")}>
                                            {miniRects.orderedIds.map(tileId => (<span key={tileId} className={cl("overview-mini-tile")} style={{
