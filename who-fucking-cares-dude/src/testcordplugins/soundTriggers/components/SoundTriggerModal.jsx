...
import { classFactory, EMPTY_TRIGGER, settings } from "../index";
import { failToast, successToast, triggersAreUnique, triggersEqual, validateAndFormatTrigger } from "../util";
import { EmojiTextInput } from "./EmojiTextInput";
import { EmptyState } from "./EmptyState";
export const openTriggerModal = (opts) => (openModal((modalProps) => (<SoundTriggerModal {...modalProps} mode={opts.mode} data={opts.data} onSubmit={opts.onSubmit}/>)));
export function SoundTriggerModal(props) {
    ...
    return (<ModalRoot {...props}>
            ...
            <ModalContent className={classFactory("modal-content")}>
                <Flex flexDirection="column" style={{ gap: "10px" }}>
                    <div>
                        ...
                        <EmojiTextInput value={emojiTextInput} onChange={setEmojiTextInput} onSubmit={() => {
            ...
            ? (<Card className={classFactory("modal-body-text-card")}>
                            {trigger.patterns.map((t, i) => (<Flex key={t} flexDirection="row" className={classFactory("modal-body-text-entry")}>
                                    <Text style={{ overflowWrap: "anywhere" }}>{t}</Text>
                                    <Button color={Button.Colors.RED} size={Button.Sizes.SMALL} onClick={() => setTrigger({ ...trigger, patterns: trigger.patterns.filter((_, idx) => i !== idx) })}>
            ...
            children: "Case Sensitive"
                    ...
                    <div>
                        ...
                        <TextInput type="text" placeholder="Enter URL" label="URL" width={"100%"} value={trigger.sound} onChange={v => setTrigger({ ...trigger, sound: v })} style={{ margin: 0 }}/>
                    </div>
                    <div>
                        <Forms.FormTitle tag="h5">Volume</Forms.FormTitle>
                        <Slider minValue={0} maxValue={1} onValueRender={v => `${Math.round(v * 100)}%`} initialValue={trigger.volume} onValueChange={v => setTrigger({ ...trigger, volume: v })}/>
                    </div>
                </Flex>
            ...
            <ModalFooter className={classFactory("modal-footer")}>
                <Button disabled={trigger.patterns.length === 0 || trigger.sound.trim() === ""} color={Button.Colors.GREEN} onClick={() => {
            const validationResult = validateAndFormatTrigger(trigger);
            if (validationResult.error) {
            ...
            successToast(props.mode === "create"
            ...
            props.onClose();
                ...
                <Button look={"OUTLINED"} onClick={props.onClose}>
                    Cancel
                </Button>
            </ModalFooter>
        </ModalRoot>);
