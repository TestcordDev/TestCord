...
import { pluginLogger as log, setLoggingLevelProvider } from "./logging";
...
const Native = VencordNative.pluginHelpers.BigFileUpload;
const OptionClasses = findByPropsLazy("optionName", "optionIcon", "optionLabel");
// Progress tracking - use centralized module
function generateUploadId() {
    return `upload-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
}
function createCloneableStore(initialState) {
...
function notifyFallbackInfo(result) {
    const attempted = result?.attemptedUploaders;
    const uploader = result?.actualUploader;
    if (!attempted || attempted.length <= 1 || !uploader)
        return;
...
let nativeNotificationInterval = null;
...
function SettingsComponent(props) {
    ...
    function handleShareXConfigUpload(event) {
    ...
    const handleFileUploaderChange = (v) => {
        if (!v || v.trim() === "") {
    ...
    const handleArgChange = (oldKey, newKey, value) => {
        const state = customUploaderStore.get();
    ...
    return (<Flex flexDirection="column" style={{ gap: "12px" }}>
            ...
            <Select className={Margins.bottom20} options={[
        ...
        ]} select={handleFileUploaderChange} isSelected={v => v === fileUploader} serialize={v => v} closeOnSelect={true} clearable={false}/>
            ...
            <FormSwitch title="Enable Paste" description="Intercept file paste events. Disable to let Discord handle pastes natively." value={settings.store.pasteEnabled !== "No"} onChange={(enabled) => {
            ...
            <FormSwitch title="Enable Drag and Drop" description="Intercept drag and drop file uploads (up to 1GB). Disable to let Discord handle drag and drop natively." value={settings.store.dragAndDropEnabled !== "No"} onChange={(enabled) => {
            ...
            <FormSwitch title="Respect Nitro Upload Limit" description="Let Discord handle files under your Nitro limit natively. Only intercept files that exceed Discord's limit." value={settings.store.respectNitroLimit === "Yes"} onChange={(enabled) => updateSetting("respectNitroLimit", enabled ? "Yes" : "No")} hideBorder={settings.store.respectNitroLimit === "Yes"}/>
            ...
            ]} placeholder="Select your Nitro tier..." select={value => updateSetting("nitroType", value)} isSelected={value => value === (settings.store.nitroType || "none")} serialize={value => value} closeOnSelect={true} clearable={false}/>
                    <Divider />
                </>)}
            <FormSwitch title="Embed Video Files" description="Wrap uploaded video file links with https://embeds.video/ to embed videos that Discord might not embed. Only applies to video files (mp4, webm, mkv, etc.)." value={settings.store.useEmbedsVideo === "Yes"} onChange={(enabled) => updateSetting("useEmbedsVideo", enabled ? "Yes" : "No")}/>
            <FormSwitch title="Display Original Filename" description="Format upload links as clickable text showing the original filename. Example: [vacation_video.mp4](link) instead of the raw link." value={settings.store.autoFormat === "Yes"} onChange={(enabled) => updateSetting("autoFormat", enabled ? "Yes" : "No")}/>
            <FormSwitch title="Auto-Send Links" description="Automatically send uploaded file links to chat immediately after upload completes." value={settings.store.autoSend === "Yes"} onChange={(enabled) => updateSetting("autoSend", enabled ? "Yes" : "No")}/>
            <FormSwitch title="Use Notifications Instead of Toasts" description="Show Vencord notifications instead of inline toasts." value={settings.store.useNotifications === "Yes"} onChange={(enabled) => updateSetting("useNotifications", enabled ? "Yes" : "No")}/>
            <FormSwitch title="Confirm Before Upload" description="Show a confirmation dialog with the file size and destination host before each upload." value={settings.store.confirmBeforeUpload} onChange={(enabled) => updateSetting("confirmBeforeUpload", enabled)}/>
        ...
        ]} placeholder="Choose how chatty the logs should be..." select={value => updateSetting("loggingLevel", value)} isSelected={value => value === (settings.store.loggingLevel || "errors")} serialize={value => value} closeOnSelect={true} clearable={false}/>
            ...
            {fileUploader === "GoFile" && (<>
                    ...
                    <TextInput className={Margins.bottom20} type="text" value={settings.store.gofileToken || ""} placeholder="Enter your GoFile token here..." onChange={newValue => updateSetting("gofileToken", newValue)}/>
                </>)}
            ...
            {fileUploader === "Catbox" && (<>
                    ...
                    <TextInput className={Margins.bottom20} type="text" value={settings.store.catboxUserHash || ""} placeholder="Enter your Catbox user hash..." onChange={newValue => updateSetting("catboxUserHash", newValue)}/>
                </>)}
            ...
            ]} placeholder="Select expiration time..." select={newValue => updateSetting("litterboxTime", newValue)} isSelected={v => v === settings.store.litterboxTime} serialize={v => v}/>
                </>)}
            ...
            {fileUploader === "0x0.st" && (<>
                    ...
                    <TextInput className={Margins.bottom20} type="text" value={settings.store.zeroX0Expires || ""} placeholder="e.g., 7d or 1y2w or 168h" onChange={newValue => updateSetting("zeroX0Expires", newValue)}/>
                </>)}
            ...
            {fileUploader === "Custom" && (<>
                    ...
                    <TextInput className={Margins.bottom20} type="text" value={customUploaderStore.get().name} placeholder="e.g., My Custom Uploader" onChange={(newValue) => customUploaderStore.set({ name: newValue })}/>

                    <Heading tag="h5">API Endpoint</Heading>
                    <TextInput className={Margins.bottom20} type="text" value={customUploaderStore.get().requestURL} placeholder="https://example.com/api/upload" onChange={(newValue) => customUploaderStore.set({ requestURL: newValue })}/>

                    <Heading tag="h5">File Form Field Name</Heading>
                    <TextInput className={Margins.bottom20} type="text" value={customUploaderStore.get().fileFormName} placeholder="e.g., file, image, upload" onChange={(newValue) => customUploaderStore.set({ fileFormName: newValue })}/>
            ...
            ]} placeholder="Select HTTP method..." select={(newValue) => customUploaderStore.set({ requestMethod: newValue })} isSelected={(v) => v === customUploaderStore.get().requestMethod} serialize={(v) => v}/>
            ...
            ]} placeholder="Select body type..." select={(newValue) => customUploaderStore.set({ bodyType: newValue })} isSelected={(v) => v === customUploaderStore.get().bodyType} serialize={(v) => v}/>

                    <Heading tag="h5">Response Format</Heading>
                    <Select className={Margins.bottom20} options={[
                { label: "Plain Text", value: "Text" },
                { label: "JSON", value: "JSON" },
            ]} placeholder="Select response type..." select={(newValue) => customUploaderStore.set({ responseType: newValue })} isSelected={(v) => v === customUploaderStore.get().responseType} serialize={(v) => v}/>
                    ...
                    <TextInput className={Margins.bottom20} type="text" value={customUploaderStore.get().url} placeholder="url" onChange={(newValue) => customUploaderStore.set({ url: newValue })}/>

                    <Heading tag="h5">Thumbnail Path (Optional)</Heading>
                    <TextInput className={Margins.bottom20} type="text" value={customUploaderStore.get().thumbnailURL} placeholder="thumbnail_url" onChange={(newValue) => customUploaderStore.set({ thumbnailURL: newValue })}/>

                    <Divider />

                    <Heading tag="h5">Request Arguments</Heading>
                    <div className={Margins.bottom20}>
                        {Object.entries(customUploaderStore.get().args).map(([key, value], index) => (<div key={index} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "8px" }}>
                                <TextInput type="text" value={key} placeholder="Key" onChange={(newKey) => handleArgChange(key, newKey, value)}/>
                                <TextInput type="text" value={value} placeholder="Value" onChange={(newValue) => handleArgChange(key, key, newValue)}/>
                            </div>))}
                    </div>
                    ...
                    <div className={Margins.bottom20}>
                        {Object.entries(customUploaderStore.get().headers).map(([key, value], index) => (<div key={index} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "8px" }}>
                                <TextInput type="text" value={key} placeholder="Header Key" onChange={(newKey) => handleHeaderChange(key, newKey, value)}/>
                                <TextInput type="text" value={value} placeholder="Header Value" onChange={(newValue) => handleHeaderChange(key, key, newValue)}/>
                            </div>))}
                    </div>
                    ...
                    <Button onClick={triggerFileUpload} color={Button.Colors.BRAND} size={Button.Sizes.MEDIUM}>
...
setLoggingLevelProvider(() => {
    try {
        return settings.store.loggingLevel ?? "errors";
    }
    catch {
...
function getDestinationHost(uploader) {
    if (uploader === "Custom") {
 ...
 */
function confirmUploadIfNeeded(uploader, fileName, fileSize) {
...
function inferExtFromBytes(bytes) {
    if (bytes.length < 12)
        return null;
...
function ensureFileNameExtension(name, bytes) {
    ...
    if (!inferredExt)
        return name;
    if (ext === "bin")
        return name.slice(0, lastDot) + "." + inferredExt;
 ...
 */
async function handleSmallFileUpload(file, skipBatchStart = false) {
    try {
        const channelId = SelectedChannelStore.getChannelId();
        ...
        if (file.size > DRAG_DROP_MAX_SIZE) {
            showUploadNotification(`File too large (${formatFileSize(file.size)}). Drag-and-drop limit is ${formatFileSize(DRAG_DROP_MAX_SIZE)}. Use the Upload button for larger files.`, Toasts.Type.FAILURE);
            return;
        }
        ...
        const result = await Native.uploadFileBuffer(buffer, uploadFileName, mimeType, {
            ...
            loggingLevel: settings.store.loggingLevel ?? "errors",
...
const ctxMenuPatch = (children, props) => {
    if (props.channel.guild_id && !PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel))
        return;
    children.splice(1, 0, <Menu.MenuItem id="upload-big-file" label={<div className={OptionClasses.optionLabel}>
