...
export async function unzipBlob(blob) {
    ...
    async function readEntry(e) {
        ...
        const LFH_SIG = 0x04034b50;
        const off = e.offset;
        if (readLE(buf, off, 4) !== LFH_SIG)
            throw new Error("Invalid local header at offset " + off + ". Buffer length: " + buf.length);
