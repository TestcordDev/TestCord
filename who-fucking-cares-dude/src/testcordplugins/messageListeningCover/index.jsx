...
function ListeningCover({ message }) {
    ...
    const activity = useStateFromStores([PresenceStore], () => PresenceStore.getActivities(message.author.id)
        .find(a => a.type === 2 /* ActivityType.LISTENING */ && a.assets?.large_image) ?? null, [message.author.id]);
    if (!activity)
        return null;
    ...
    return <Popout position="top" renderPopout={() => <div style={{ width: 267, height: 110 }}>
            <ActivityCard activity={activity} currentUser={UserStore.getCurrentUser()} user={message.author}/>
        </div>} targetElementRef={ref}>
...
export default definePlugin({
    ...
    renderMessageDecoration: props => {
        ...
        if (!me || props.message.author.id === me)
            return null;
