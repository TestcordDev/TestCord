...
function isImageAttachment(att, includeGifs) {
    if (!att)
        return false;
    ...
    if (ct.startsWith("image/")) {
        if (!includeGifs && (ct === "image/gif" || url.toLowerCase().includes(".gif")))
            return false;
        return true;
    }
...
function isImageUrl(url, includeGifs, isExplicitEmbedImage = false) {
    ...
    if (isExplicitEmbedImage) {
        if (/\.(mp4|webm|mov|avi|mkv)$/i.test(cleanUrl))
            return false;
        return true;
    }
    return false;
}
export function extractImages(messages, channelId, opts) {
    const items = [];
    for (const m of messages ?? []) {
        ...
        for (const a of m?.attachments ?? []) {
            if (!isImageAttachment(a, opts.includeGifs))
                continue;
        ...
        if (opts.includeEmbeds) {
            for (const e of m?.embeds ?? []) {
                ...
                for (const [source, sourceKind] of [[image, "img"], [thumb, "thumb"]]) {
                    if (!source?.url)
                        continue;
                    const url = String(source.url);
                    if (!isImageUrl(url, opts.includeGifs, true))
                        continue;
