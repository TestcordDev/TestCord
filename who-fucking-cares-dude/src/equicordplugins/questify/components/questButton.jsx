...
export function QuestIcon(height, width, className) {
    return (<svg viewBox="0 0 24 24" height={height} width={width} fill="none" className={className}>
...
function suffixClassNames(classNames, suffix) {
    return classNames.map(className => `${className}-${suffix}`).join(" ");
}
function QuestButtonLowerBadge(props) {
    return <ServerListItemLowerBadgeComponent {...props}/>;
}
function QuestButtonView({ id, className, badgeProps, hasUnread, selected, visible, onPress, }) {
    ...
    const lowerBadge = badgeProps.count === 0
        ? null
        : (<QuestButtonLowerBadge {...badgeProps} className={lowerBadgeClass} style={{ ...(badgeProps.style ?? {}), ...lowerBadgeSize }}/>);
    const icon = (<div className={iconContainerClass}>
            {QuestIcon(26, 26)}
        </div>);
    return (<ErrorBoundary>
            {visible && (<div id={id} className={mainContainerClass}>
                    <div className={pillContainerClass}>
                        <ServerListItemPillComponent unread={hasUnread} selected={selected} hovered={hovered} className={`${pillClass}${selected ? " selected" : hovered ? " hovered" : ""}`}/>
                    </div>
                    <div className={buttonContainerClass}>
                        <GuildlessServerListItemComponent icon={() => icon} tooltip="Quests" showPill={false} selected={selected} className={buttonClass} onClick={onPress} onMouseDown={onPress} onContextMenu={onPress} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} {...(lowerBadge ? { lowerBadge, lowerBadgeSize } : {})}/>
                    </div>
                </div>)}
        </ErrorBoundary>);
}
export function showQuestButton(displayMode, badgeCount, isOnQuestsPage) {
    return displayMode !== "never" && (displayMode === "always" || badgeCount > 0 || isOnQuestsPage);
}
export function disguiseHomeButton(pathname) {
...
function resolveQuestButtonAction(event, leftAction, middleAction, rightAction) {
...
function getQuestButtonLowerBadgeProps(badgeCount, indicatorMode, badgeColor) {
...
export function QuestButtonContextMenu({ dummy = false }) {
    ...
    return (<Menu.Menu navId={navId} onClose={ContextMenuApi.closeContextMenu} aria-label="Quest Button Menu">
            <Menu.MenuItem id={q(`${navId}-mark-all-ignored`)} label="Mark All Ignored" action={ignoreAllQuests} disabled={markAllIgnoredDisabled}/>
            <Menu.MenuItem id={q(`${navId}-reset-ignored-list`)} label="Reset Ignored List" action={resetIgnoredQuests} disabled={resetIgnoredDisabled}/>
            {queueAllQuestsVisible && (<Menu.MenuItem id={q(`${navId}-queue-all-quests`)} label={"Queue All Quests" + labelSuffix} action={() => {
            ...
            {clearQueueVisible && (<Menu.MenuItem id={q(`${navId}-clear-queue`)} label="Clear Quest Queue" action={() => {
            ...
            <Menu.MenuItem id={q(`${navId}-fetch-quests`)} label="Fetch Quests" action={() => { void fetchAndAlertQuests("MANUAL_FETCH"); }} disabled={fetchQuestsDisabled}/>
        </Menu.Menu>);
}
export function QuestButton() {
    const { isOnQuestsPage, questButtonBadgeColor, questButtonDisplay, questButtonBadgeCount, questButtonLeftClickAction, questButtonMiddleClickAction, questButtonRightClickAction, questButtonIndicator, } = useQuestifySettings([
        "isOnQuestsPage",
    ...
    const displayMode = questButtonDisplay;
    const leftClickAction = questButtonLeftClickAction;
    const middleClickAction = questButtonMiddleClickAction;
    const rightClickAction = questButtonRightClickAction;
    const indicatorMode = questButtonIndicator;
    ...
    function handlePress(event) {
        ...
        const action = resolveQuestButtonAction(event, leftClickAction, middleClickAction, rightClickAction);
    ...
    return (<QuestButtonView id={q("quest-button")} className={q("quest-button")} selected={onQuestsPage} visible={isVisible} hasUnread={badgeCount > 0 && hasUnread} badgeProps={lowerBadgeProps} onPress={handlePress}/>);
}
export function DummyQuestButton({ badgeColor, leftClickAction, middleClickAction, rightClickAction, showBadge, showPill, visible, }) {
    ...
    return (<QuestButtonView id={q("dummy-quest-button")} className={q("dummy-quest-button", "quest-button")} selected={dummySelected} visible={visible} hasUnread={showPill} badgeProps={lowerBadgeProps} onPress={handlePress}/>);
}
