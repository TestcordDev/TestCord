...
import { addTag, getTag, removeTag } from "./settings";
export function openCreateTagModal(initialValue = { name: "", message: "" }) {
...
function CreateTagDialog({ initialValue, modalProps }) {
    ...
    const notice = hasReservedEphemeral
        ...
        : nameAlreadyExists
            ...
            : undefined;
    return (<Modal {...modalProps} title={isEdit ? "Edit Tag" : "Create New Tag"} subtitle={isEdit ? "Edit your custom command." : "Create a new tag which will be registered as a slash command."} actions={[
            {
                text: "Cancel",
                variant: "secondary",
            ...
            <Flex flexDirection="column" gap={12}>
                ...
                {detectedArguments.length > 0 && (<section>
                        <HeadingSecondary>Detected Arguments</HeadingSecondary>
                        <Paragraph>
                            <ul>
                                {detectedArguments.map(arg => (<li key={arg.name}>
                ...
                <ExpandableSection renderContent={() => (<Flex flexDirection="column" gap={12}>
                            <Paragraph>
