...
const isLegal = (word) => {
    ...
    if (["a", "e", "o", "u", "y"].includes(lastChar))
        return false;
    return true;
};
const getWords = (string) => {
    const linkRegex = /https?:\/\/[^\s]+\.[^\s]+/g;
    const linkRanges = new Set();
...
const chooseRandomWord = (message) => {
    const words = getWords(message);
    while (words.length > 0) {
        const index = Math.floor(Math.random() * words.length);
        const wordMatch = words[index];
...
const ington = (word) => {
    if (word.endsWith("INGTON"))
        return "";
    if (word.endsWith("INGTO"))
        return "N";
    if (word.endsWith("INGT"))
        return "ON";
...
const handleMessage = ((channelId, message) => {
    ...
    if (!msg || !msg.trim())
        return;
    const wordMatch = chooseRandomWord(msg);
    if (wordMatch === null)
        return;
    ...
    const idx = wordMatch.startIndex + word.length;
    message.content = msg.slice(0, idx) + insertion + msg.slice(idx);
});
const IngtoninatorButton = ({ isMainChat }) => {
    const { isEnabled, showIcon } = settings.use(["isEnabled", "showIcon"]);
    const toggle = () => settings.store.isEnabled = !settings.store.isEnabled;
    if (!isMainChat || !showIcon)
        return null;
    return (<ChatBarButton tooltip={isEnabled ? "Ingtoninator Enabled" : "Ingtoninator Disabled"} onClick={toggle}>
            {isEnabled ? (enabledIcon()) : (disabledIcon())}
        </ChatBarButton>);
};
function enabledIcon() {
    return (<svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
