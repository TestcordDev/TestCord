...
function formatDiscordTimestamp(ts, discordTag) {
    return `<t:${Math.floor(ts / 1000)}:${discordTag}>`;
}
function CalendarIcon(props) {
    return (<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
function ChevronLeft(props) {
    return (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
function PlusIcon(props) {
    return (<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
function CalendarGrid({ year, month, selectedDay, notesMap, onDayClick }) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        cells.push(<div key={d} className={cl("day-cell", {
                today: isToday,
                selected: isSelected,
                "has-notes": hasNotes,
...
function NoteCard({ note, onDelete }) {
    ...
    const displayTime = fmt
        ...
        : mmt.format("MM/DD/YYYY h:mm A");
    return (<div className={cl("note-card")}>
            <div className={cl("note-header")}>
                <span className={cl("note-time")}>{displayTime}</span>
                <div className={cl("note-actions")}>
                    <button className={cl("note-action-btn")} onClick={() => copyWithToast(formatDiscordTimestamp(note.timestamp, note.format))} title="Copy Discord timestamp">
                        <CopyIcon />
                    </button>
                    <button className={cl("note-action-btn", "delete")} onClick={() => onDelete(note.id)} title="Delete note">
                        <TrashIcon />
                    </button>
                </div>
            </div>
...
function AddNoteForm({ onAdd, defaultTimestamp }) {
    ...
    const previewText = selectedFmt
        ...
        : "";
    return (<div className={cl("add-form")}>
            <input ref={inputRef} className={cl("add-input")} placeholder="Add a note..." value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()}/>
            <div className={cl("add-row")}>
                ...
                <button className={cl("add-btn")} onClick={handleSubmit} disabled={!text.trim()}>
                    <PlusIcon /> Add
                </button>
            </div>
            {selectedFmt && (<div className={cl("preview")}>
...
function NotesPanel({ dateKey, notesMap, onUpdate }) {
    ...
    const handleAdd = async (text, format) => {
        const newNote = {
    ...
    return (<div className={cl("notes-panel")}>
            ...
            {notes.length > 0 ? (<div className={cl("notes-list")}>
                    {notes.map(note => (<NoteCard key={note.id} note={note} onDelete={handleDelete}/>))}
                </div>) : (<div className={cl("notes-empty")}>No notes for this day.</div>)}
        </div>);
}
function CalendarModal(props) {
    ...
    const dateKey = selectedDay != null
        ...
        : null;
    return (<ModalRoot {...props} size={"large" /* ModalSize.LARGE */} className={cl("modal-root")}>
                ...
                <div className={cl("layout")}>
                    <div className={cl("calendar-side")}>
                        ...
                        <div className={cl("weekday-row")}>
                            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(d => (<div key={d} className={cl("weekday")}>{d}</div>))}
                        </div>
                        {loaded && (<CalendarGrid year={year} month={month} selectedDay={selectedDay} notesMap={notesMap} onDayClick={setSelectedDay}/>)}
                    </div>
                    <div className={cl("notes-side")}>
                        {dateKey ? (<NotesPanel dateKey={dateKey} notesMap={notesMap} onUpdate={setNotesMap}/>) : (<div className={cl("notes-placeholder")}>
...
function toggleCalendar() {
    ...
    modalKey = openModal((props) => <CalendarModal {...props}/>, { onCloseCallback: () => { modalKey = null; } });
}
