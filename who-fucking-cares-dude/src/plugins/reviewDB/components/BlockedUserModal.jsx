...
function UnblockButton(props) {
    return (<Tooltip text="Unblock user">
            {tooltipProps => (<div {...tooltipProps} role="button" onClick={props.onClick} className={cl("block-modal-unblock")}>
...
function BlockedUser({ user, isBusy, setIsBusy }) {
    const [gone, setGone] = useState(false);
    if (gone)
        return null;
    return (<div className={cl("block-modal-row")}>
            ...
            <UnblockButton onClick={isBusy ? undefined : async () => {
            setIsBusy(true);
            try {
...
function BlockedUsersList() {
    ...
    return (<>
            {blocks.map(b => (<BlockedUser key={b.discordID} user={b} isBusy={isBusy} setIsBusy={setIsBusy}/>))}
        </>);
}
export function openBlockModal() {
    openModal(modalProps => (<Modal {...modalProps} title="Blocked Users">
