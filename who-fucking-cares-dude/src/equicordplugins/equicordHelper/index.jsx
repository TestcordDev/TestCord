...
import { Devs, EQUICORD_GUILD_ID, EQUICORD_SUPPORT_CHANNEL_ID, EquicordDevs, SUPPORT_CHANNEL_IDS, VC_SUPPORT_CHANNEL_IDS } from "@utils/constants";
...
const StandingConfig = {
    [100 /* StandingState.ALL_GOOD */]: { label: "All good!", hoverColor: "var(--status-positive)", Icon: ShieldIcon },
    [200 /* StandingState.LIMITED */]: { label: "Limited", hoverColor: "var(--status-warning)", Icon: WarningIcon },
    [300 /* StandingState.VERY_LIMITED */]: { label: "Very limited", hoverColor: "var(--orange-345)", Icon: WarningIcon },
    [400 /* StandingState.AT_RISK */]: { label: "At risk", hoverColor: "var(--status-danger)", Icon: WarningIcon },
    [500 /* StandingState.SUSPENDED */]: { label: "Suspended", hoverColor: "var(--interactive-muted)", Icon: WarningIcon },
};
function StandingButton() {
    ...
    React.useEffect(() => {
        if (!isInitialized)
            fetchSafetyHub().catch(() => { });
    }, [isInitialized]);
    const config = StandingConfig[standing?.state] ?? StandingConfig[100 /* StandingState.ALL_GOOD */];
    return (<div style={{ display: "contents" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <HeaderBarButton tooltip={config.label} position="bottom" icon={props => <config.Icon {...props} color={hovered ? config.hoverColor : "currentColor"}/>} onClick={() => SettingsRouter.openUserSettings("account_standing_panel")}/>
        </div>);
}
...
export default definePlugin({
    ...
    renderMessageAccessory(props) {
        if (!isEquicordGuild(props.message.channel_id))
            return null;
        return (<>
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!selfId || isAnyPluginDev(selfId))
                return;
            if (VC_SUPPORT_CHANNEL_IDS.includes(channelId) && !clicked) {
                return Alerts.show({
                    ...
                    onConfirm() {
                        NavigationRouter.transitionTo(`/channels/${EQUICORD_GUILD_ID}/${EQUICORD_SUPPORT_CHANNEL_ID}`);
                    },
