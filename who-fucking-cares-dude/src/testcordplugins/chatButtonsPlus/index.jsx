...
function CustomButtonIcon({ svg }) {
    return (<svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24">
...
function HeaderChatButtons() {
    ...
    for (const entry of buttonEntries) {
        ...
        enabledButtons.push(<HeaderBarButton key={entry.id} icon={Icon} tooltip={entry.label} onClick={() => handleButtonClick(entry.message)}/>);
    }
...
function ChannelChatButtons() {
    ...
    for (const entry of buttonEntries) {
        if (entry.enabled === false)
            continue;
        const Icon = () => <CustomButtonIcon svg={entry.svg}/>;
        enabledButtons.push(<ChannelToolbarButton key={entry.id} icon={Icon} tooltip={entry.label} onClick={() => handleButtonClick(entry.message)}/>);
    }
    return <>{enabledButtons}</>;
}
async function addButtonEntry(forceUpdate) {
    try {
        buttonEntries.push({
            id: generateId(),
            label: "New Button",
...
async function removeButtonEntry(id, forceUpdate) {
    try {
...
async function resetAllButtons(forceUpdate) {
    try {
...
function ButtonEntries() {
    ...
    async function setLabel(id, value) {
        try {
            const index = buttonEntries.findIndex(entry => entry.id === id);
            if (index !== -1) {
    ...
    async function setMessage(id, value) {
        try {
            const index = buttonEntries.findIndex(entry => entry.id === id);
            if (index !== -1) {
    ...
    async function setSvg(id, value) {
        try {
            const index = buttonEntries.findIndex(entry => entry.id === id);
            if (index !== -1) {
    ...
    function isValidSvg(svg) {
        ...
        return svg.includes("<") && (svg.includes("path") ||
            svg.includes("svg") ||
            svg.includes("circle") ||
            svg.includes("rect") ||
    ...
    const elements = buttonEntries.map((entry, i) => {
        return (<div key={entry.id} className="chatButtonsPlus-card">
                <div className="chatButtonsPlus-header">
                    ...
                    <div className="chatButtonsPlus-controls">
                        <FormSwitch title="Enabled" value={entry.enabled ?? true} onChange={value => setEnabled(entry.id, value)} className="chatButtonsPlus-toggle"/>
                        <Button onClick={() => removeButtonEntry(entry.id, update)} look={Button.Looks.FILLED} color={Button.Colors.RED} size={Button.Sizes.SMALL}>
                            Delete
                        </Button>
                    </div>
                </div>
                ...
                <div className="chatButtonsPlus-field">
                    ...
                    <TextInput placeholder="Button label/tooltip" value={entry.label} onChange={e => setLabel(entry.id, e)}/>
                </div>

                <div className="chatButtonsPlus-field">
                    <Paragraph className="chatButtonsPlus-label">Message to Send</Paragraph>
                    <textarea className="chatButtonsPlus-textarea" placeholder="Message to send when clicked" value={entry.message} onChange={e => setMessage(entry.id, e.target.value)} rows={2}/>
                </div>

                <div className="chatButtonsPlus-field">
                    <Paragraph className="chatButtonsPlus-label">Custom SVG Path (24x24 viewBox)</Paragraph>
                    <textarea className="chatButtonsPlus-textarea" placeholder='<path fill="currentColor" d="..."/>' value={entry.svg} onChange={e => setSvg(entry.id, e.target.value)} rows={3}/>
    ...
    return (<>
            ...
            <div className="chatButtonsPlus-actions">
                ...
                <Button onClick={() => resetAllButtons(update)} look={Button.Looks.FILLED} color={Button.Colors.RED}>
                    Reset All
                </Button>
            </div>
        </>);
    ...
    chatBarButton: {
        ...
        render: (({ isMainChat }) => {
            ...
            for (const entry of buttonEntries) {
                if (entry.enabled === false)
                    continue;
                enabledButtons.push(<ChatBarButton key={entry.id} tooltip={entry.label} onClick={() => handleButtonClick(entry.message)}>
