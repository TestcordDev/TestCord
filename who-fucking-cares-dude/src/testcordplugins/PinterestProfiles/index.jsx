...
export default definePlugin({
    ...
    renderTabs(existingTabs, Tab, activeView) {
        return (<>
                {existingTabs}
                <Tab id="pinterest-picker-tab" key="pinterest-picker-tab" aria-controls="pinterest-picker-tab-panel" aria-selected={activeView === PINTEREST_VIEW} isActive={activeView === PINTEREST_VIEW} viewType={PINTEREST_VIEW}>
                    Pinterest
                </Tab>
            </>);
    },
    renderPinterestPickerComponent({ onSelectGIF }) {
        return <WrappedPinterestPicker onSelectItem={onSelectGIF}/>;
    },
    renderEditProfileButton({ guildId }) {
