...
async function fetchMessages(channelId, limit, timeframeHours, authorId) {
    const messages = [];
    ...
    while (messages.length < limit) {
        ...
        for (const msg of batch) {
            if (cutoffSnowflake && msg.id < cutoffSnowflake)
                return messages;
...
async function callNvidia(messages) {
    ...
    if (!apiKey)
        throw new Error("Nvidia NIM API key not configured in plugin settings.");
...
function buildSummaryPrompt(formattedMessages, style, channelName, personName) {
    ...
    commands: [
        {
            ...
            execute: async (opts, ctx) => {
                const channelId = findOption(opts, "channel", ctx.channel?.id);
                ...
                const style = findOption(opts, "style", "mid");
                const personId = findOption(opts, "person", "");
                const person = personId ? UserStore.getUser(personId) : null;
                const timeframe = findOption(opts, "timeframe", undefined);
