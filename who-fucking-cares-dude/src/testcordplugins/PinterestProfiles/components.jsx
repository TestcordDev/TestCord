...
import { cl, Native, settings } from "./shared";
const ManaSearchBar = findComponentByCodeLazy("#{intl::SEARCH}),ref");
...
function getSearchKinds(target) {
    return target === "ALL" ? ["AVATAR", "BANNER"] : [target];
}
function getPrimaryKind(target) {
    if (target === "ALL")
        return "AVATAR";
    return target;
}
...
function setPendingProfileChanges(payload, guildId) {
...
function getPendingImageAsset(image, description) {
    return {
...
function applyImageData(image, target, filename, guildId) {
    const payload = target === "BANNER"
        ? { pendingBanner: image }
        : { pendingAvatar: getPendingImageAsset(image, `pinterest-${filename || "image"}`) };
    setPendingProfileChanges(payload, guildId);
}
async function applyProfileResult(result, target, guildId) {
    try {
        const media = await Native.fetchMedia(result.url);
...
function PillButton({ children, compact = false, onClick, disabled = false, type = "button" }) {
    return (<button type={type} className={classes(cl("button"), compact && cl("button-compact"))} onClick={onClick} disabled={disabled}>
            {children}
        </button>);
}
function SelectionDropdown({ target, open, onToggle, onSelect }) {
    const options = ["ALL", "AVATAR", "BANNER"];
    return (<div className={cl("selection-wrap")}>
            ...
            {open ? (<div className={cl("selection-menu")}>
                    {options.map(option => (<button key={option} type="button" className={classes(cl("selection-item"), option === target && cl("selection-item-active"))} onClick={() => onSelect(option)}>
                            {option === "ALL" ? "All" : option === "AVATAR" ? "Avatar" : "Banner"}
                        </button>))}
                </div>) : null}
        </div>);
}
function ResultMenu({ result, open, onToggle }) {
    function closeMenu() {
        onToggle();
    }
    return (<div className={cl("menu-wrap")}>
            <button type="button" className={cl("menu-button")} onClick={event => {
            event.stopPropagation();
            onToggle();
        }}>
                <span className={cl("menu-button-dots")} aria-hidden="true">⋯</span>
            </button>
            {open ? (<div className={cl("menu")} onClick={event => event.stopPropagation()}>
                ...
                closeMenu();
                    ...
                    {result.pinterestUrl ? (<button type="button" className={cl("menu-item")} onClick={() => {
...
function ResultsSection({ kind, bucket, menuId, gifsOnly, slotCount, setMenuId, setBuckets, onLoadNextPage, onSelectResult }) {
    ...
    if (!bucket.error && bucket.data == null)
        return null;
    return (<section className={cl("section")}>
            ...
            {pagedResults.length ? (<div className={cl("section-body")}>
                    <button type="button" className={classes(cl("page-button"), cl("page-button-side"), cl("page-button-left"))} disabled={bucket.page === 0} onClick={() => setBuckets(current => ({
                ...current,
                [kind]: {
                    ...current[kind],
                    ...
                    <div className={classes(cl("grid"), kind === "BANNER" && cl("grid-banner"))}>
                        {pagedResults.map(result => (<button key={`${kind}-${result.id}`} type="button" className={cl("card")} onClick={() => onSelectResult(result, kind)}>
                                <div className={cl("card-top")}>
                                    <ResultMenu result={result} open={menuId === `${kind}:${result.id}`} onToggle={() => setMenuId(current => current === `${kind}:${result.id}` ? "" : `${kind}:${result.id}`)}/>
                                </div>
                    ...
                    <button type="button" className={classes(cl("page-button"), cl("page-button-side"), cl("page-button-right"))} disabled={bucket.loadingNextPage || (bucket.page >= totalPages - 1 && !bucket.bookmark?.length)} onClick={() => {
...
function PinterestBrowser({ query, setQuery, clearQuery, onSelectResult, rootClassName, initialTarget, showTargetSelector, panelProps }) {
    ...
    async function runSearch(nextQuery = query) {
        ...
        try {
            const responses = await Promise.all(kinds.map(async (kind) => {
                const response = await Native.search(trimmed, resultsPerRequest, gifsOnly ? "GIFS" : "ALL", [], kind);
    ...
    async function loadNextPage(kind) {
        const bucket = buckets[kind];
        if (!bucket.data || !bucket.bookmark?.length || bucket.loadingNextPage)
            return;
        setBuckets(current => ({
            ...current,
            [kind]: {
        ...
        try {
            const response = await Native.search(bucket.activeQuery || lastSearchQuery || query, resultsPerRequest, gifsOnly ? "GIFS" : "ALL", bucket.bookmark, kind);
            setBuckets(current => ({
                ...current,
                [kind]: {
                    ...current[kind],
    ...
    function getSlotCount(kind) {
        if (kind === "IMAGE")
            return Math.max(avatarSlots, bannerSlots);
        return kind === "AVATAR" ? avatarSlots : bannerSlots;
    }
    const guideSource = getGuideSource();
    return (<div {...panelProps} className={rootClassName}>
            ...
            void runSearch();
                    ...
                    <div className={cl("search-row")}>
                        <div className={cl("search-field")}>
                            <ManaSearchBar placeholder={getPlaceholder()} query={query} onChange={setQuery} onClear={clearQuery}/>
                        </div>
                        <PillButton compact type="submit" disabled={!query.trim() || loading}>
                            {loading ? "..." : "Search"}
                        </PillButton>
                        <button type="button" className={classes(cl("toggle"), gifsOnly && cl("toggle-active"))} onClick={() => setGifsOnly(current => !current)}>
                            <span className={cl("toggle-icon")}>GIF</span>
                        </button>
                        {showTargetSelector ? (<SelectionDropdown target={target} open={selectionOpen} onToggle={() => setSelectionOpen(current => !current)} onSelect={value => {
                setTarget(value);
                setSelectionOpen(false);
            }}/>) : null}
                    </div>
                    {guideSource?.guides.length ? (<div className={cl("guides-row")}>
                            {guideSource.guides.slice(0, 6).map(guide => (<button key={guide.query} type="button" className={classes(cl("guide"), guide.query === guideSource.query && cl("guide-active"))} onClick={() => {
            ...
            <div ref={scrollRef} className={cl("container-body")}>
                {getSearchKinds(target).map(kind => (<ResultsSection key={kind} kind={kind} bucket={buckets[kind]} menuId={menuId} gifsOnly={gifsOnly} slotCount={getSlotCount(kind)} setMenuId={setMenuId} setBuckets={setBuckets} onLoadNextPage={loadNextPage} onSelectResult={onSelectResult}/>))}
            </div>
        </div>);
}
export function PinterestPicker({ onSelectItem }) {
    const query = ExpressionPickerStore.useExpressionPickerStore(store => store.searchQuery);
    return (<PinterestBrowser query={query} setQuery={value => ExpressionPickerStore.setSearchQuery(value)} clearQuery={() => ExpressionPickerStore.setSearchQuery("")} onSelectResult={result => {
            onSelectItem({ url: result.url });
            ExpressionPickerStore.closeExpressionPicker();
        }} rootClassName={cl("container")} initialTarget="IMAGE" showTargetSelector={false} panelProps={{
            id: "pinterest-picker-tab-panel",
            role: "tabpanel",
            "aria-labelledby": "pinterest-picker-tab"
        }}/>);
}
export function PinterestProfilePanel({ guildId }) {
    const [query, setQuery] = useState("");
    return (<PinterestBrowser query={query} setQuery={setQuery} clearQuery={() => setQuery("")} onSelectResult={(result, kind) => {
            void applyProfileResult(result, kind, guildId);
        }} rootClassName={classes(cl("container"), cl("inline-wrap"))} initialTarget="ALL" showTargetSelector={true}/>);
}
