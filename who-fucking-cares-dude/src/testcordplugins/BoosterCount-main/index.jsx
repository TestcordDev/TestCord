...
async function openViewBoosters(guild) {
    ...
    const key = openModal(props => (<ErrorBoundary>
            <ModalRoot {...props} size={"large" /* ModalSize.LARGE */}>
                <ModalHeader>
...
function MakeContextCallback() {
    return (children, props) => {
        ...
        children.splice(-1, 0, <Menu.MenuItem id={"vc-view-boosts"} label="View Boosters" action={async () => await openViewBoosters(guild.id)} icon={BoostIcon}/>);
    };
}
...
async function fetchBoosters(guildId) {
...
function UserList({ guildId, boosters }) {
    const missing = [];
    const members = [];
    ...
    useStateFromStores([GuildMemberStore], () => GuildMemberStore.getMemberIds(guildId), null, (old, curr) => old?.length === curr?.length);
    useEffect(() => {
        if (missing.length > 0) {
    ...
    return (<ScrollerThin fade className={cl("scroller")}>
            ...
            return (<Flex key={id} style={{ alignItems: "center", marginBottom: "10px", gap: "12px" }}>
                        {/* Boost Count */}
                        ...
                        {/* Custom User Row */}
                        <Flex style={{ alignItems: "center", cursor: "pointer", gap: "10px" }} onClick={() => openUserProfile(id)}>
                            <img src={avatarUrl} width={32} height={32} style={{ borderRadius: "50%" }} alt={`${user.username}'s avatar`}/>
...
function BoostIcon() {
    return (<svg xmlns="http://www.w3.org/2000/svg" fill="white" stroke="white" width="18" height="18" viewBox="0 0 24 24" id="level-2-discord-boost">
