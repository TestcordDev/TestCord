...
const messageCtxPatch = (children, { message }) => {
    ...
    if (!group)
        return;
    group.splice(group.findIndex(c => c?.props?.id === "copy-text") + 1, 0, (<Menu.MenuItem id="vc-hidemessages" label="Hide" icon={EyeIcon} action={() => hideMessage(message.id, message.channel_id)}/>));
};
const userCtxPatch = (children, { channel }) => {
    if (!channel?.isDM())
        return;
    if (isPluginEnabled(pinDms.name) && isPinned(channel.id))
        return;
    const group = findGroupChildrenByChildId("close-dm", children);
    if (!group)
        return;
    const hidden = hiddenDmIds.has(channel.id);
    group.splice(group.findIndex(c => c?.props?.id === "close-dm"), 0, (<Menu.MenuItem id="vc-hidemessages-dm" label={hidden ? "Unhide DM" : "Hide DM"} icon={EyeIcon} action={() => toggleDm(channel.id)}/>));
};
...
export default definePlugin({
    ...
    filterPrivateChannelIds(privateChannelIds, instance) {
    ...
    renderHiddenMessagesToggle: ErrorBoundary.wrap(() => {
        ...
        return (<Tooltip text={label}>
                {tooltipProps => (<Clickable {...tooltipProps} className={cl("eye")} role="button" tabIndex={0} aria-label={label} aria-disabled={!hasHiddenDms} onClick={event => {
    ...
    messagePopoverButton: {
        icon: EyeIcon,
        render(message) {
            if (settings.store.hidePopoverButton)
                return null;
            return {
                label: "Hide",
                icon: EyeIcon,
