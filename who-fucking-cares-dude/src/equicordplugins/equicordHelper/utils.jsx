...
export async function toggleEnabled(name) {
    let restartNeeded = false;
    function onRestartNeeded() {
        restartNeeded = true;
    }
    async function beforeReturn(settings, wasEnabled) {
        if (restartNeeded) {
...
export function getWindowsName(release) {
...
export function getMacOSName(release) {
    const major = parseInt(release.split(".")[0]);
    if (major === 25)
        return "MacOS 26 (Tahoe)";
    if (major === 24)
...
export function platformName() {
    ...
    if (DiscordNative.process.platform === "win32")
        return `${getWindowsName(DiscordNative.os.release)}`;
    ...
    if (DiscordNative.process.platform === "linux")
        return `${navigator.platform} (${DiscordNative.os.release})`;
