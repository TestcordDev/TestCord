...
export const NoticeVariants = ["info", "warning", "danger", "positive"];
function InfoIcon(props) {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" {...props}>
...
export function Notice({ children, variant = "info", icon, action, hidden = false, className, ...restProps }) {
    const Icon = icon ?? DefaultIcons[variant];
    return (<div className={classes(cl("container", variant), hidden && cl("hidden"), className)} {...restProps}>
            <div className={cl("inner")}>
                ...
                {action && (<div className={cl("action")}>
                        {action}
                    </div>)}
            </div>
        </div>);
}
Notice.Info = (props) => <Notice variant="info" {...props}/>;
Notice.Warning = (props) => <Notice variant="warning" {...props}/>;
Notice.Danger = (props) => <Notice variant="danger" {...props}/>;
Notice.Positive = (props) => <Notice variant="positive" {...props}/>;
