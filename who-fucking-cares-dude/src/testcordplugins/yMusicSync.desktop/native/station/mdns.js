...
function encodeQuery(questions, unicast) {
...
function readTxt(data) {
    ...
    while (cursor < data.length) {
        ...
        if (separator > 0)
            entries.set(text.slice(0, separator), text.slice(separator + 1));
        cursor += 1 + length;
    }
    return entries;
}
function parse(buffer, records, onInstance) {
    if (buffer.length < 12)
        return;
    ...
    for (let index = 0; index < answers; index++) {
        ...
        switch (type) {
            case TYPE_PTR: {
                if (normalize(name) !== normalize(MDNS_SERVICE))
                    break;
            ...
            case TYPE_A:
                if (data.length === 4)
                    records.addresses.set(normalize(name), Array.from(data).join("."));
                break;
        }
    }
}
function privateInterfaces() {
    ...
    for (const items of Object.values(networkInterfaces())) {
        for (const item of items ?? []) {
            if (item.internal || (item.family !== "IPv4" && Number(item.family) !== 4))
                continue;
...
export function discoverStations() {
    return new Promise(resolve => {
        const records = { instances: new Set(), srv: new Map(), txt: new Map(), addresses: new Map() };
        const timers = [];
        let socket = null;
        let settled = false;
        const finish = () => {
        ...
        const send = (query) => {
            socket?.send(query, MDNS_PORT, MDNS_ADDRESS, error => {
                if (error)
                    log(`Station discovery query failed: ${error.message}`);
            });
        };
        const askInstance = (instance) => send(encodeQuery([
