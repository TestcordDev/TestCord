...
function extractBase64(body) {
    try {
        ...
        if (parsed && typeof parsed.data === "string")
            return parsed.data; // legacy JSON shape
    }
    catch {
...
async function readOutTextChunk(text) {
...
function sliceByBytes(text, limit) {
    const parts = [];
    let start = 0;
...
function chunkTextByBytes(text) {
...
const messageCtxPatch = (children, { message }) => {
    ...
    if (children.some(c => c?.props?.id === "tiktok-tts-play"))
        return;
...
export default definePlugin({
    ...
    authors: [EquicordDevs.VillainsRule, TestcordDevs.examplegit],
