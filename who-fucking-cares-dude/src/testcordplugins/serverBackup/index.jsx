...
async function backupServer(guildId) {
    try {
        ...
        // Save to DataStore
        const backups = (await DataStore.get(BACKUP_STORE_KEY)) ||
            {};
...
async function restoreServer(backupKey, targetGuildId) {
    try {
        ...
        for (const [channelId, channel] of Object.entries(existingChannels)) {
            try {
                ...
                console.error(`[ServerBackup] Error deleting channel ${channel.name}:`, error);
            }
        }
        ...
        for (const role of existingRoles) {
            ...
            try {
                ...
                console.error(`[ServerBackup] Error deleting role ${role.name}:`, error);
            }
        }
        ...
        for (const role of backup.roles) {
            try {
                ...
                console.error(`[ServerBackup] Error creating role ${role.name}:`, error);
            }
        }
        ...
        for (const channel of categories) {
            try {
                ...
                console.error(`[ServerBackup] Error creating category ${channel.name}:`, error);
            }
        }
        ...
        for (const channel of otherChannels) {
            try {
                ...
                console.error(`[ServerBackup] Error creating channel ${channel.name}:`, error);
            }
        }
...
export default definePlugin({
    name: "ServerBackup",
    description: "Backs up and restores complete Discord server configuration (roles, channels, permissions) - No permissions required",
            ...
            options: [
                {
                    ...
                    options: [
                        {
                            name: "server_id",
                            description: "Server ID to backup (leave empty for current server)",
                ...
                },
                {
                    ...
                    options: [
                        {
                            name: "backup_name",
                            description: "Name of backup to restore (use /backup list to see backups)",
                            type: 3 /* ApplicationCommandOptionType.STRING */,
                            required: true,
                        },
                        {
            ...
            execute: async (opts, ctx) => {
                try {
                    ...
                    if (subcommandName === "save") {
                        ...
                        const serverId = serverIdOpt?.value || ctx.guild?.id;
                        if (!serverId) {
                            sendBotMessage(ctx.channel.id, {
                                content: "❌ You must specify a server ID or use this command in a server.",
                            });
                            return;
                        }
                        sendBotMessage(ctx.channel.id, {
                        ...
                        const backupName = backupNameOpt?.value;
                        const targetServerId = targetServerIdOpt?.value || ctx.guild?.id;
                        if (!backupName) {
                            sendBotMessage(ctx.channel.id, {
                        ...
                        if (!targetServerId) {
                            sendBotMessage(ctx.channel.id, {
                                content: "❌ You must specify a target server ID or use this command in a server.",
                            });
                            return;
                        }
                        sendBotMessage(ctx.channel.id, {
                            content: "⏳ Restoration in progress... This may take several minutes depending on server size.\n⚠️ **WARNING:** This action will DELETE all existing roles and channels and replace them with those from the backup!",
                        });
