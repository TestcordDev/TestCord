...
const THROTTLED_CLASS_TOKENS = ["activity", "subText", "botText", "clanTag"];
...
function orSel(...clses) {
    const out = [];
    for (const c of clses) {
        if (!c)
            continue;
        try {
            const s = classNameToSelector(c);
            if (s)
                out.push(s);
        }
        catch { }
...
function joinSel(...selectors) {
    return selectors.filter((c) => !!c).join(", ");
}
function ruleFor(selectors, body) {
    const valid = selectors.split(",").map(s => s.trim()).filter(Boolean);
    if (!valid.length)
        return null;
    return `${valid.join(", ")} { ${body} }`;
}
function hasElementDescendants(node) {
...
/* -------------------------------------------------------------------------- */
let disposeBackgroundRAF = null;
let disposeBackgroundCancelRAF = null;
...
function installRafThrottle() {
    if (disposeBackgroundRAF || !bgFpsActive)
        return;
    let lastT = 0;
    disposeBackgroundRAF = RuntimeInterposition.register({
        ...
        wrap: next => cb => {
            if (RuntimeInteractions.isActive())
                return next(cb);
            const id = ++rafSeq;
...
let disposePresenceDispatch = null;
...
if (migrateOptimizerSettings(SettingsStore.plain.plugins))
...
export default definePlugin({
    ...
    networkCacheOrder: [],
    pendingRafReduction: new Map(),
    nextRafReductionId: 1,
    cacheCleanupTimer: null,
    memoryTimer: null,
    intersectionObserver: null,
    lazyIframeObserver: null,
    lazyIframeMutationObserver: null,
    mediaMutationObserver: null,
    pausedMedia: new Set(),
    optimizerStyleEl: null,
    extraStyleEl: null,
    domThrottleStyleEl: null,
    domThrottleObserver: null,
    domThrottleTimers: new Map(),
    gifMutationObserver: null,
    gifManagedImages: new WeakSet(),
    gifBlobUrls: new Set(),
    lazyImageObserver: null,
    consolidatedObserver: null,
    consolidatedFlushReset: null,
    observerCallbacks: new Map(),
    animatedEmojiObserver: null,
    imageDecodingObserver: null,
    gifAutoplayObserver: null,
    gifAutoplayCleanups: new WeakMap(),
    avatarObserver: null,
    avatarQualityObserver: null,
    preconnectLink: null,
    preconnectLink2: null,
    hoverTransitionStyleEl: null,
    compositingStyleEl: null,
    cacheTrimTimer: null,
    originalRaf: null,
    originalCancelRaf: null,
    originalPassiveListener: null,
    originalIdleCallback: null,
    originalCancelIdleCallback: null,
    originalResizeObserver: null,
    disposeRafHook: null,
    disposeCancelRafHook: null,
    disposeNetworkFetchHook: null,
    disposePassiveListenerHook: null,
    disposeResizeObserverHook: null,
    disposeIdleCallbackHook: null,
    disposeCancelIdleCallbackHook: null,
    disposeConcurrentFetchHook: null,
    disposeFluxDispatchHook: null,
    originalConsoleWarn: null,
    originalConsoleGroup: null,
    originalConsoleGroupEnd: null,
    originalConsoleGroupCollapsed: null,
    originalConsoleCount: null,
    originalConsoleCountReset: null,
    originalConsoleAssert: null,
    originalConsoleDir: null,
    originalConsoleDirxml: null,
    fetchQueue: [],
    rICMessagePort: null,
    rICMessagePort1: null,
    rICCallbacks: null,
    cacheTrimActivityHandler: null,
    suppressConsoleWarnEl: null,
    reactionStyleEl: null,
    canvasSuppressEl: null,
    unreadBadgeEl: null,
    channelTopicEl: null,
    folderAnimEl: null,
    invitePreviewEl: null,
    memberListGradientEl: null,
    memberFreezeEl: null,
    memberFreezeTimer: null,
    memberFreezeRefreshTimer: null,
    websocketPatchEl: null,
    spellcheckObserver: null,
    unfocusedFreezeStyleEl: null,
    unfocusedVisibilityHandler: null,
    deferCssTimer: null,
    fluxThrottleState: null,
    start() {
        if (settings.store.verboseLogging)
            logger.info("Starting optimizer suite");
        if (settings.store.domThrottle
            ...
            try {
                this.installConsolidatedObserver();
            }
            catch (e) {
                logger.warn("installConsolidatedObserver failed", e);
            }
        }
        try {
            if (settings.store.domThrottle)
                this.installDomThrottle();
        }
        catch (e) {
            logger.warn("installDomThrottle failed", e);
        }
        try {
            if (settings.store.fastNetwork || settings.store.networkCache || settings.store.forceLowImageQuality)
                this.installNetworkLayer();
        }
        catch (e) {
            logger.warn("installNetworkLayer failed", e);
        }
        try {
            if (settings.store.disableSpringAnimations)
                this.installSpringSkip();
        }
        catch (e) {
            logger.warn("installSpringSkip failed", e);
        }
        try {
            if (settings.store.memoryManagement)
                this.installMemoryManager();
        }
        catch (e) {
            logger.warn("installMemoryManager failed", e);
        }
        try {
            if (settings.store.pauseOffscreenMedia)
                this.installOffscreenMediaPause();
        }
        catch (e) {
            logger.warn("installOffscreenMediaPause failed", e);
        }
        try {
            if (settings.store.virtualizeMessages || settings.store.optimizeTextRendering)
                this.installCSSOptimizations();
        }
        catch (e) {
            logger.warn("installCSSOptimizations failed", e);
        }
        try {
            if (settings.store.suppressConsoleSpam)
                this.installConsoleSuppression();
        }
        catch (e) {
            logger.warn("installConsoleSuppression failed", e);
        }
        try {
            if (settings.store.freezeGifsUntilHover && settings.store.gifFreezeMethod !== "css")
                this.installGifFreezer();
        }
        catch (e) {
            logger.warn("installGifFreezer failed", e);
        }
        try {
            if (settings.store.lazyEmbedImages)
                this.installLazyImages();
        }
        catch (e) {
            logger.warn("installLazyImages failed", e);
        }
        try {
            if (settings.store.lazyIframes)
                this.installLazyIframes();
        }
        catch (e) {
            logger.warn("installLazyIframes failed", e);
        }
        try {
            if (settings.store.optimizeImageDecoding)
                this.installImageDecodingOptimization();
        }
        catch (e) {
            logger.warn("installImageDecodingOptimization failed", e);
        }
        try {
            if (settings.store.disableAnimatedEmoji)
                this.installDisableAnimatedEmoji();
        }
        catch (e) {
            logger.warn("installDisableAnimatedEmoji failed", e);
        }
        try {
            if (settings.store.suppressGifAutoplay)
                this.installSuppressGifAutoplay();
        }
        catch (e) {
            logger.warn("installSuppressGifAutoplay failed", e);
        }
        try {
            if (settings.store.killPerformanceMetrics)
                this.installPerfMetricsBlocker();
        }
        catch (e) {
            logger.warn("installPerfMetricsBlocker failed", e);
        }
        try {
            if (settings.store.suppressConsoleTimers)
                this.installConsoleTimerBlocker();
        }
        catch (e) {
            logger.warn("installConsoleTimerBlocker failed", e);
        }
        try {
            if (settings.store.killHoverTransitions)
                this.installHoverTransitionKiller();
        }
        catch (e) {
            logger.warn("installHoverTransitionKiller failed", e);
        }
        try {
            if (settings.store.preconnectDiscordCdn)
                this.installPreconnect();
        }
        catch (e) {
            logger.warn("installPreconnect failed", e);
        }
        try {
            if (settings.store.forceCompositingLayers)
                this.installCompositingLayers();
        }
        catch (e) {
            logger.warn("installCompositingLayers failed", e);
        }
        try {
            if (settings.store.freezeAnimatedAvatars)
                this.installAnimatedAvatarOptimizer();
        }
        catch (e) {
            logger.warn("installAnimatedAvatarOptimizer failed", e);
        }
        try {
            if (settings.store.reduceAvatarQuality)
                this.installAvatarQualityReducer();
        }
        catch (e) {
            logger.warn("installAvatarQualityReducer failed", e);
        }
        try {
            if (settings.store.animationFrameReduction)
                this.installRafReduction();
        }
        catch (e) {
            logger.warn("installRafReduction failed", e);
        }
        try {
            if (settings.store.forcePassiveListeners)
                this.installPassiveListeners();
        }
        catch (e) {
            logger.warn("installPassiveListeners failed", e);
        }
        try {
            if (settings.store.throttleResizeObservers)
                this.installResizeObserverThrottle();
        }
        catch (e) {
            logger.warn("installResizeObserverThrottle failed", e);
        }
        try {
            if (settings.store.limitMessageCache)
                this.installMessageCacheTrimmer();
        }
        catch (e) {
            logger.warn("installMessageCacheTrimmer failed", e);
        }
        try {
            if (settings.store.limitConcurrentRequests)
                this.installConcurrentRequestLimiter();
        }
        catch (e) {
            logger.warn("installConcurrentRequestLimiter failed", e);
        }
        try {
            if (settings.store.suppressConsoleWarn)
                this.installConsoleWarnSuppression();
        }
        catch (e) {
            logger.warn("installConsoleWarnSuppression failed", e);
        }
        try {
            if (settings.store.suppressConsoleGroup)
                this.installConsoleGroupSuppression();
        }
        catch (e) {
            logger.warn("installConsoleGroupSuppression failed", e);
        }
        try {
            if (settings.store.suppressConsoleCount)
                this.installConsoleCountSuppression();
        }
        catch (e) {
            logger.warn("installConsoleCountSuppression failed", e);
        }
        try {
            if (settings.store.suppressConsoleAssert)
                this.installConsoleAssertSuppression();
        }
        catch (e) {
            logger.warn("installConsoleAssertSuppression failed", e);
        }
        try {
            if (settings.store.suppressConsoleDir)
                this.installConsoleDirSuppression();
        }
        catch (e) {
            logger.warn("installConsoleDirSuppression failed", e);
        }
        try {
            if (settings.store.suppressIdleCallback)
                this.installIdleCallbackOptimizer();
        }
        catch (e) {
            logger.warn("installIdleCallbackOptimizer failed", e);
        }
        try {
            if (settings.store.disableDragAndDrop)
                this.installDragAndDropSuppression();
        }
        catch (e) {
            logger.warn("installDragAndDropSuppression failed", e);
        }
        try {
            if (settings.store.disableSpellcheck)
                this.installSpellcheckOpt();
        }
        catch (e) {
            logger.warn("installSpellcheckOpt failed", e);
        }
        try {
            if (settings.store.throttleFluxDispatches)
                this.installFluxThrottle();
        }
        catch (e) {
            logger.warn("installFluxThrottle failed", e);
        }
        try {
            if (settings.store.killReactionRendering)
                this.installReactionSimplifier();
        }
        catch (e) {
            logger.warn("installReactionSimplifier failed", e);
        }
        try {
            if (settings.store.disableUnreadBadges)
                this.installUnreadBadgeKiller();
        }
        catch (e) {
            logger.warn("installUnreadBadgeKiller failed", e);
        }
        try {
            if (settings.store.suppressAllCanvas)
                this.installCanvasSuppressor();
        }
        catch (e) {
            logger.warn("installCanvasSuppressor failed", e);
        }
        try {
            if (settings.store.disableChannelTopic)
                this.installChannelTopicKiller();
        }
        catch (e) {
            logger.warn("installChannelTopicKiller failed", e);
        }
        try {
            if (settings.store.disableFolderAnimations)
                this.installFolderAnimationKiller();
        }
        catch (e) {
            logger.warn("installFolderAnimationKiller failed", e);
        }
        try {
            if (settings.store.disableInvitePreviews)
                this.installInvitePreviewKiller();
        }
        catch (e) {
            logger.warn("installInvitePreviewKiller failed", e);
        }
        try {
            if (settings.store.unifiedMemberListGradient)
                this.installMemberListGradient();
        }
        catch (e) {
            logger.warn("installMemberListGradient failed", e);
        }
        try {
            if (settings.store.freezeMemberList)
                this.installMemberFreezer();
        }
        catch (e) {
            logger.warn("installMemberFreezer failed", e);
        }
        try {
            if (settings.store.freezeWhenUnfocused)
                this.installUnfocusedFreezer();
        }
        catch (e) {
            logger.warn("installUnfocusedFreezer failed", e);
        }
        try {
            if (settings.store.killVoiceVideo)
                this.installVoiceVideoKiller();
        }
        catch (e) {
            logger.warn("installVoiceVideoKiller failed", e);
        }
        try {
            if (settings.store.preventWebSocketFlood)
                this.installWebSocketFloodPreventer();
        }
        catch (e) {
            logger.warn("installWebSocketFloodPreventer failed", e);
        }
        this.deferCssTimer = (typeof requestIdleCallback === "function"
            ? requestIdleCallback(() => { try {
                this.installExtraCSS();
            }
            catch (e) {
                logger.warn("installExtraCSS failed", e);
            } }, { timeout: 3000 })
            : setTimeout(() => { try {
                this.installExtraCSS();
            }
        ...
        if (settings.store.cacheLimitsEnabled) {
            resetCacheLimits();
            if (settings.store.verboseLogging)
                logger.info("Plugin cache limits active");
        }
        if (settings.store.verboseLogging)
            logger.info("Started");
    },
    stop() {
        if (settings.store.verboseLogging)
            logger.info("Stopping, restoring originals");
        ...
        if (this.deferCssTimer !== null) {
            if (typeof cancelIdleCallback === "function")
                cancelIdleCallback(this.deferCssTimer);
            else
                clearTimeout(this.deferCssTimer);
    ...
    installConsolidatedObserver() {
        ...
        const flush = () => {
            ...
            for (const cb of callbacks.values()) {
                try {
                    ...
                    if (settings.store.verboseLogging)
                        logger.warn("Consolidated observer callback error", err);
                }
            }
        };
        ...
        try {
            this.consolidatedObserver = new MutationObserver(records => {
                ...
                if (throttle) {
                    ...
                    for (const rec of records)
                        queued.push(rec);
                    if (queued.length > MAX_QUEUED_RECORDS)
                        queued.splice(0, queued.length - MAX_QUEUED_RECORDS);
                    if (!frame)
            ...
            if (settings.store.verboseLogging)
                logger.info("Installed consolidated MutationObserver");
        }
        catch (err) {
            if (settings.store.verboseLogging)
                logger.warn("Failed to install consolidated observer", err);
        ...
        const callback = (records) => {
            for (const r of records) {
                ...
                for (const node of r.removedNodes) {
                    ...
                    release(node);
                    if (timers.size)
                        node.querySelectorAll?.("[data-op-throttled]").forEach(release);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    teardownDomThrottle() {
        ...
        for (const t of this.domThrottleTimers.values())
            clearTimeout(t);
    ...
    installRafReduction() {
        ...
        this.disposeRafHook = RuntimeInterposition.register({
            ...
            wrap: next => {
                ...
                return (cb) => {
                    ...
                    if (!originalRaf)
                        return 0;
                    if (RuntimeInteractions.isActive() || isEditorFocused())
                    ...
                    pending.raf = originalRaf(time => {
                        ...
                        pending.timeout = setTimeout(() => {
                            ...
                            if (!raf || !this.pendingRafReduction.has(id))
                                return;
        ...
        if (settings.store.verboseLogging)
            logger.info(`rAF reduction active: target ${Math.round(1000 / minInterval)}fps`);
    },
    restoreRafReduction() {
        for (const pending of this.pendingRafReduction.values()) {
            if (pending.raf !== undefined)
                this.originalCancelRaf?.(pending.raf);
            if (pending.timeout !== undefined)
                clearTimeout(pending.timeout);
        }
    ...
    installNetworkLayer() {
        ...
        const isDiscordCdn = (url) => /(?:cdn|media)\.discord(?:app)?\.(?:com|net)/.test(url);
        const blockedPaths = [
            /\/api\/v\d+\/science\b/,
            /\/api\/v\d+\/tracing\b/,
            /\/api\/v\d+\/logging\b/,
        ...
        const rewriteSize = (url) => {
            if (!lowQuality || !isDiscordCdn(url))
                return url;
            try {
                const u = new URL(url, window.location.origin);
                const size = u.searchParams.get("size");
                if (size && Number(size) > 96)
                    u.searchParams.set("size", "96");
                if (!size && /avatars|emojis|icons|banners/.test(u.pathname))
                    u.searchParams.set("size", "96");
                return u.toString();
            }
            catch {
        ...
        this.disposeNetworkFetchHook = RuntimeInterposition.register({
            ...
            hook: "fetch",
            priority: RuntimeInterpositionPriority.BEHAVIOR,
            wrap: originalFetch => function patched(input, init) {
    ...
    installMemoryManager() {
        ...
        if (!perf.memory) {
            if (settings.store.verboseLogging)
                logger.info("performance.memory unavailable; memory manager idle");
            return;
        }
        this.memoryTimer = setInterval(() => {
            try {
                ...
                if (settings.store.verboseLogging)
                    logger.warn("Memory pressure check failed", err);
            }
        }, intervalMs);
    },
            ...
            for (const r of records) {
                ...
                for (const node of r.removedNodes) {
                    if (node instanceof HTMLMediaElement)
                        ...
                        node.querySelectorAll("video, audio").forEach(release);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    installGifFreezer() {
        ...
        const isAnimated = (img) => /\.gif(?:$|[?#])/i.test(img.src);
        const freeze = (img) => {
            if (!ctx)
                return;
            if (!isAnimated(img))
                return;
            if (this.gifManagedImages.has(img))
                return;
            ...
            const buildFrozen = () => {
                ...
                try {
                    ...
                    sharedCanvas.toBlob(b => {
                        ...
                        if (img.dataset.opGifState !== "playing")
                            img.src = frozenUrl;
                    }, "image/png");
                    return null;
                }
            ...
            img.__opCleanup = cleanup;
            if (img.complete)
                buildFrozen();
            else
                img.addEventListener("load", onLoad, { once: true });
        };
        document.querySelectorAll("img").forEach(freeze);
        const release = (img) => {
            const cleanup = img.__opCleanup;
            ...
            for (const r of records) {
                for (const node of r.addedNodes) {
                    if (node instanceof HTMLImageElement)
                        ...
                        node.querySelectorAll("img").forEach(freeze);
                }
                for (const node of r.removedNodes) {
                    if (node instanceof HTMLImageElement)
                        release(node);
                    else if (node instanceof Element && node.querySelector?.("img"))
                        node.querySelectorAll("img").forEach(release);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    teardownGifFreezer() {
        ...
        for (const url of this.gifBlobUrls)
            URL.revokeObjectURL(url);
        this.gifBlobUrls.clear();
    },
    installLazyImages() {
        const chatSel = chatImageSelector();
        const isChatImage = (img) => chatSel ? img.closest(chatSel) !== null : false;
        const apply = (img) => {
            if (img.dataset.opLazy === "1")
                return;
            img.dataset.opLazy = "1";
            if (!isChatImage(img) && !img.hasAttribute("loading"))
                img.loading = "lazy";
            if (!isChatImage(img) && !img.hasAttribute("decoding"))
                img.decoding = "async";
        };
    ...
    installLazyIframes() {
        ...
        const observeIframe = (iframe) => {
            if (iframe.dataset.opLazyLoad)
                return;
            const src = iframe.src || "";
            if (/\.hcaptcha\.com/i.test(src))
                return;
            if (/discord\.com|\.youtube\.com|\.youtu\.be|\.spotify\.com/i.test(src))
                return;
    ...
    installExtraCSS() {
        ...
        if (settings.store.reduceMotion) {
            rules.push("*:not(#vc-smoothtype-caret), *:not(#vc-smoothtype-caret)::before, *:not(#vc-smoothtype-caret)::after { animation-duration: 0.001ms !important; animation-delay: 0ms !important; transition-duration: 0.001ms !important; transition-delay: 0ms !important; }");
        }
        ...
        if (settings.store.suppressEmbedPreviews) {
            ...
            if (embedSel)
                push(ruleFor(`article${embedSel}`, "display: none !important"));
            push(ruleFor(orSel(EmbedClasses.embedWrapper, EmbedClasses.embedFull, EmbedClasses.embedInner), "display: none !important"));
        }
        // --- Advanced CSS optimizations ---
        if (settings.store.freezeGifsUntilHover && settings.store.gifFreezeMethod === "css") {
            rules.push("img[src*=\".gif\"]:not(.emoji):not([data-op-gif-suppressed=\"1\"]) { content-visibility: hidden; }", "img[src*=\".gif\"]:not(.emoji):hover { content-visibility: visible; }");
        }
    ...
    installDisableAnimatedEmoji() {
        const isDiscordEmoji = (url) => /(?:cdn|media)\.discord(?:app)?\.(?:com|net)\/emojis\//.test(url);
        const rewrite = (img) => {
            const src = img.src || img.currentSrc;
            if (!src)
                return;
            if (!/\/(?:a_|[0-9]+\.gif)/.test(src))
                return;
            if (!isDiscordEmoji(src))
                return;
            if (img.dataset.opEmojiStatic === "1")
                return;
        ...
        document.querySelectorAll("img.emoji, img[src*=\"emojis\"]").forEach(rewrite);
        const callback = (records) => {
            for (const r of records) {
                for (const node of r.addedNodes) {
                    if (!(node instanceof Element))
                        continue;
                    if (node instanceof HTMLImageElement) {
                        if (node.classList.contains("emoji") || node.src.includes("emojis"))
                            rewrite(node);
                    }
                    else if (node.querySelector("img.emoji, img")) {
                        node.querySelectorAll("img.emoji, img[src*=\"emojis\"]").forEach(rewrite);
                    }
                }
            }
        };
    ...
    installSuppressGifAutoplay() {
        ...
        const pause = (el) => {
            if (el.dataset.opGifSuppressed === "1")
                return;
            const src = el.src || el.currentSrc;
            if (!/\.gif|giphy|tenor|media\.discord/i.test(src) && !(el instanceof HTMLVideoElement))
                return;
        ...
        document.querySelectorAll("video[src*=\"gif\"], video[src*=\"media.discord\"]").forEach(pause);
        const callback = (records) => {
            for (const r of records) {
                for (const node of r.addedNodes) {
                    if (!(node instanceof Element))
                        continue;
                    if (node instanceof HTMLVideoElement && /gif|media\.discord/i.test(node.src))
                        pause(node);
                    else if (node.querySelector("video"))
                        node.querySelectorAll("video[src*=\"gif\"], video[src*=\"media.discord\"]").forEach(pause);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    teardownSuppressGifAutoplay() {
        ...
        document.querySelectorAll("video[data-op-gif-suppressed]").forEach(video => {
    ...
    installCompositingLayers() {
        ...
        if (sc)
            rules.push(`${sc}{contain:content}`);
        if (GuildClasses.guilds)
            rules.push(`${sel(GuildClasses.guilds)}{contain:layout}`);
        if (MemberClasses.membersWrap)
            rules.push(`${sel(MemberClasses.membersWrap)}{contain:layout}`);
        if (!rules.length)
            return;
    ...
    installIdleCallbackOptimizer() {
        if (typeof MessageChannel === "undefined") {
            if (settings.store.verboseLogging)
                logger.info("MessageChannel unavailable, skipping idle callback optimizer");
            return;
        }
        ...
        channel.port1.onmessage = () => {
            ...
            for (const [, entry] of snapshot) {
                try {
                    ...
                    if (settings.store.verboseLogging)
                        logger.warn("Idle callback error", err);
                }
            }
        };
    ...
    installMessageCacheTrimmer() {
        ...
        this.cacheTrimTimer = setInterval(() => {
            try {
                ...
                if (!channels || typeof channels !== "function")
                    return;
                const store = MessageStore;
                const allChannels = store._messagesByChannel || store.getMutableAllMessages?.() || {};
                ...
                for (const [chId, last] of lastActivity) {
                    if (last <= cutoff && !allChannels[chId])
                        lastActivity.delete(chId);
                }
                ...
                if (settings.store.verboseLogging)
                    logger.warn("Message cache trim failed", err);
            }
        }, intervalMs);
    },
    ...
    installConcurrentRequestLimiter() {
        ...
        this.disposeConcurrentFetchHook = RuntimeInterposition.register({
            ...
            wrap: next => {
                origFetch = next;
                return function limitedFetch(input, init) {
    ...
    teardownConcurrentRequestLimiter() {
        ...
        for (const item of this.fetchQueue)
            item.reject(new Error("TestcordOptimizer stopped"));
        this.fetchQueue = [];
    },
    ...
    installAnimatedAvatarOptimizer() {
        ...
        const isAvatar = (img) => (avatarSel ? img.matches(avatarSel) : false) || (closestSel ? img.closest(closestSel) !== null : false);
        const freeze = (img) => {
            if (img.dataset.opAvFrozen === "1")
                return;
            const src = img.src || img.currentSrc;
            if (!src || !/\/(?:a_|[0-9]+\.gif)/.test(src))
                return;
            if (!isAvatar(img))
                return;
        ...
        if (avatarSel)
            document.querySelectorAll(avatarSel).forEach(freeze);
        const callback = (records) => {
            for (const r of records) {
                for (const node of r.addedNodes) {
                    if (!(node instanceof Element))
                        continue;
                    if (node instanceof HTMLImageElement && isAvatar(node))
                        freeze(node);
                    else if (avatarSel && node.querySelector("img"))
                        node.querySelectorAll(avatarSel).forEach(freeze);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    teardownAnimatedAvatarOptimizer() {
        ...
        document.querySelectorAll("img[data-op-av-frozen]").forEach(img => {
            const cleanup = img.__opAvCleanup;
            if (typeof cleanup === "function") {
                cleanup();
                delete img.__opAvCleanup;
            }
    ...
    installAvatarQualityReducer() {
        ...
        const isAvatar = (img) => (avatarSel ? img.matches(avatarSel) : false) || (closestSel ? img.closest(closestSel) !== null : false);
        const rewrite = (img) => {
            if (img.dataset.opAvQuality === "1")
                return;
            const src = img.src || img.currentSrc;
            if (!src.includes("cdn.discord") && !src.includes("media.discord"))
                return;
            if (!isAvatar(img))
                return;
        ...
        if (avatarSel)
            document.querySelectorAll(avatarSel).forEach(rewrite);
        const callback = (records) => {
            for (const r of records) {
                for (const node of r.addedNodes) {
                    if (!(node instanceof Element))
                    ...
                    if (node instanceof HTMLImageElement && isAvatar(node))
                        ...
                        node.querySelectorAll(avatarSel).forEach(rewrite);
                }
            }
        };
        if (this.consolidatedObserver) {
    ...
    installDragAndDropSuppression() {
        ...
        this.__dndHandler = handler;
        if (settings.store.verboseLogging)
            logger.info("Drag-and-drop events suppressed");
    },
    teardownDragAndDrop() {
        const handler = this.__dndHandler;
        if (handler) {
            ...
            this.__dndHandler = undefined;
        }
    },
    installSpellcheckOpt() {
        const set = (el) => {
            if (el.getAttribute("data-op-nospell") === "1")
                return;
    ...
    teardownSpellcheckOpt() {
        ...
        document.querySelectorAll("[data-op-nospell]").forEach(el => {
    ...
    installVoiceVideoKiller() {
        if (typeof window.RTCPeerConnection === "undefined")
            return;
        const noop = function () { return noopProto; };
        const noopProto = {
            close: () => { },
        ...
        window.__op_origRtc = window.RTCPeerConnection;
        window.__op_origWebkitRtc = webkitWindow.webkitRTCPeerConnection;
        window.RTCPeerConnection = noop;
        webkitWindow.webkitRTCPeerConnection = noop;
        if (settings.store.verboseLogging)
            logger.info("Voice/video WebRTC neutered");
    },
    teardownVoiceVideoKiller() {
        const orig = window.__op_origRtc;
        if (orig) {
            window.RTCPeerConnection = orig;
            delete window.__op_origRtc;
        }
        const webkitWindow = window;
        const webkitOrig = window.__op_origWebkitRtc;
        if (webkitOrig) {
        ...
        delete window.__op_origWebkitRtc;
    },
    installWebSocketFloodPreventer() {
        const origSend = WebSocket.prototype.send;
        window.__op_origWsSend = origSend;
        ...
        WebSocket.prototype.send = function (data) {
            if (typeof data === "string" && !GATEWAY_CONTROL.test(data)) {
                ...
                if (prev && prev.data === data && now - prev.at < DEDUPE_WINDOW_MS)
                    return;
        ...
        if (settings.store.verboseLogging)
            logger.info("WebSocket duplicate-frame dedupe active");
    },
    teardownWebSocketFloodPreventer() {
        const orig = window.__op_origWsSend;
        if (orig) {
            WebSocket.prototype.send = orig;
            delete window.__op_origWsSend;
        }
    },
    installFluxThrottle() {
        if (this.fluxThrottleState)
            return;
        ...
        const wrappedDispatch = function (payload) {
...
`;
    ...
    installUnreadBadgeKiller() {
        ...
        const rules = [];
        if (sidebar && unread)
            rules.push(`${sidebar} ${unread}{display:none!important}`);
        if (chat && unread)
            rules.push(`${chat} ${unread}{display:none!important}`);
        if (sidebar && badge && number)
            rules.push(`${sidebar} ${badge}${number}{display:none!important}`);
        if (sidebar && mention && badge)
            rules.push(`${sidebar} ${mention}${badge}{display:none!important}`);
        if (badgePulse)
            rules.push(`${badgePulse}{display:none!important}`);
        if (sidebar && unreadPill)
            rules.push(`${sidebar} ${unreadPill}{display:none!important}`);
        if (sidebar && unreadBar)
            rules.push(`${sidebar} ${unreadBar}{display:none!important}`);
        if (!rules.length)
            return;
    ...
    installCanvasSuppressor() {
        const rules = [];
        if (CanvasEffectClasses.spriteCanvas)
            rules.push(`canvas${sel(CanvasEffectClasses.spriteCanvas)}{display:none!important}`);
        if (EffectsClasses.effects)
            rules.push(`canvas${sel(EffectsClasses.effects)}{display:none!important}`);
    ...
    installChannelTopicKiller() {
        ...
        if (chatContent && topic)
            rules.push(`${chatContent}>${topic}{display:none!important}`);
        if (chat && title && topic)
            rules.push(`${chat}>${title}>${topic}{display:none!important}`);
        if (chat && channelTopic)
            rules.push(`${chat} ${channelTopic}{display:none!important}`);
        if (!rules.length)
            return;
    ...
    installFolderAnimationKiller() {
        ...
        const rules = [];
        if (folder && expandedFolder)
            rules.push(`${folder}${expandedFolder}{animation:none!important;transition:none!important}`);
        if (folderIcon)
            rules.push(`${folderIcon}{animation:none!important;transition:none!important}`);
        if (!rules.length)
            return;
    ...
    installInvitePreviewKiller() {
        ...
        const rules = [];
        if (invite)
            rules.push(`${chat} ${invite}{display:none!important}`);
        if (inviteCard)
            rules.push(`${chat} ${inviteCard}{display:none!important}`);
        if (wrapper && invite)
            rules.push(`${chat} ${wrapper}${invite}{display:none!important}`);
        if (!rules.length)
            return;
    ...
    installMemberListGradient() {
        ...
        if (selected)
            rules.push(`${members}>${member}${selected}{background:none!important}`);
        if (focused)
            rules.push(`${members}>${member}${focused}{background:none!important}`);
        rules.push(`${membersWrap}{background:linear-gradient(180deg,var(--background-primary),var(--background-secondary-alt),var(--background-primary))!important;position:relative}`, `${membersWrap}::before{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent 0%,var(--background-modifier-hover) 50%,transparent 100%);pointer-events:none;z-index:0}`, `${members}{position:relative;z-index:1}`);
    ...
    installMemberFreezer() {
        ...
        if (membersGroup)
            rules.push(`${members}>${membersGroup}{contain:paint layout style}`);
    ...
    useThrottledMemberList(guildId) {
