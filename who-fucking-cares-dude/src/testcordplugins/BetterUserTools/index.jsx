...
const VoiceActions = findByPropsLazy("setLoopback", "toggleSelfDeaf");
...
let otrState = { enabled: false };
function notifyMic(msg, type) {
    showToast(msg, type);
}
...
async function disableLoopback(silent = false) {
    ...
    try {
        ...
        selfDeafenedByPlugin = false;
        if (!silent)
            notifyMic("Mic test loopback disabled", Toasts.Type.SUCCESS);
    }
    catch (err) {
        log.error("Failed to disable mic test loopback", err);
        if (!silent)
            notifyMic("Failed to stop mic test loopback (see console)", Toasts.Type.FAILURE);
    }
}
function MicLoopbackIcon({ active, className }) {
    const lineLength = 30;
    const lineStyle = {
    ...
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden>
...
function MicLoopbackButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : "Mic Test Loopback"} icon={<MicLoopbackIcon active={loopbackActive} className={iconForeground}/>} role="switch" redGlow={!loopbackActive} aria-checked={loopbackActive} onClick={handleToggle} plated={nameplate != null} className={`button__201d5 wrapper__201d5 vc-betterusertools-btn${loopbackActive ? " danger" : ""}`}/>);
}
function RadarIcon({ active, className }) {
    const lineLength = 30;
    const lineStyle = {
        strokeDasharray: lineLength,
        strokeDashoffset: active ? lineLength : 0,
    ...
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
async function loadOtrState() {
    otrState = await DataStore.get(OTR_DATA_KEY) || { enabled: false };
}
function persistOtrState() {
    return DataStore.set(OTR_DATA_KEY, otrState);
}
async function applyOtrEnable() {
    if (!ShowCurrentGame || !StatusSetting)
        return;
    const currentStatus = StatusSetting.getSetting?.();
    const currentShow = ShowCurrentGame.getSetting?.();
    if (otrState.prevStatus === undefined)
        otrState.prevStatus = currentStatus;
    if (otrState.prevShowCurrentGame === undefined)
        otrState.prevShowCurrentGame = currentShow;
    await Promise.all([
...
async function applyOtrDisable() {
    ...
    if (otrState.prevStatus)
        tasks.push(StatusSetting.updateSetting(otrState.prevStatus));
    otrState.enabled = false;
    await Promise.all(tasks);
    await persistOtrState();
}
function OffTheRadarButton({ iconForeground, hideTooltips, nameplate }) {
    const [, forceUpdate] = React.useReducer(x => x + 1, 0);
    const toggle = async () => {
        try {
            if (otrState.enabled) {
    ...
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : otrState.enabled ? "Off The Radar (on)" : "Off The Radar (off)"} icon={<RadarIcon active={otrState.enabled} className={iconForeground}/>} role="switch" redGlow={!otrState.enabled} aria-checked={otrState.enabled} plated={nameplate != null} onClick={toggle} className={`button__201d5 wrapper__201d5 vc-betterusertools-btn${otrState.enabled ? " danger" : ""}`}/>);
}
function renderMicLoopbackButton({ iconForeground, hideTooltips, nameplate }) {
...
function renderOffTheRadarButton({ iconForeground, hideTooltips, nameplate }) {
