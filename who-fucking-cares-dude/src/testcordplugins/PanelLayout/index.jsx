...
const settings = definePluginSettings({
    ...
    // Sizing
    iconSize: { type: 5 /* OptionType.SLIDER */, description: "Icon size (px)", default: 20, markers: makeRange(12, 28, 2), stickToMarkers: false, onChange: () => apply() },
    buttonContainerSize: { type: 5 /* OptionType.SLIDER */, description: "Button overall size (px)", default: 36, markers: makeRange(24, 48, 4), stickToMarkers: false, onChange: () => apply() },
    buttonGap: { type: 5 /* OptionType.SLIDER */, description: "Gap between buttons (px)", default: 6, markers: makeRange(0, 12, 2), stickToMarkers: true, onChange: () => apply() },
    panelOpacity: { type: 5 /* OptionType.SLIDER */, description: "Panel buttons opacity (0-100)", default: 100, markers: makeRange(10, 100, 10), stickToMarkers: false, onChange: () => apply() },
    // Button styling
    buttonStyle: {
        type: 4 /* OptionType.SELECT */,
    ...
    panelBackgroundColor: { type: 0 /* OptionType.STRING */, description: "Panel background color", default: "#0e1852", onChange: () => apply() },
    glowColor: { type: 0 /* OptionType.STRING */, description: "Glow hover color", default: "#ffffff", onChange: () => apply() },
    colorfulActiveButtons: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Gives enabled plugins customizable colored rounded backgrounds", onChange: () => apply() },
    forceNativeButtonColor: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Force the icon color on Discord's native buttons (Mute, Deafen, Settings) even when no custom icon color is set", onChange: () => apply() },
    // Chevrons & Lock
    hideChevrons: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide dropdown chevrons next to Mute and Deafen", onChange: () => apply() },
    lockButtonPosition: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Lock Button Position (prevents buttons dropping down on long status)", onChange: () => apply() },
    // Call controls
    callCompact: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Compact mode for call control buttons", onChange: () => apply() },
    hideDisconnect: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide the disconnect button", onChange: () => apply() },
    hideVoiceStatus: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide the 'Voice Connected' status text and channel name", onChange: () => apply() },
    hidePingIcon: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide the ping/connection quality icon", onChange: () => apply() },
    // Per-button visibility
    hideMute: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide Mute button", onChange: () => apply() },
    hideDeafen: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide Deafen button", onChange: () => apply() },
    hideSettings: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide User Settings button", onChange: () => apply() },
    hideCamera: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide camera button in call controls", onChange: () => apply() },
    hideScreenShare: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide screen share button in call controls", onChange: () => apply() },
    hideActivity: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Hide activity button in call controls", onChange: () => apply() },
    // Line
    hideLine: { type: 3 /* OptionType.BOOLEAN */, default: true, description: "Hide the line between user and buttons", onChange: () => apply() },
    // Profile Nameplate
    fixProfileNameplate: { type: 3 /* OptionType.BOOLEAN */, default: false, description: "Fixes the rounding of the profile nameplate", onChange: () => apply() },
});
// ─── Selectors & Constants ────────────────────────────────────────────────────
const S = {
...
async function loadConfigs() {
    buttonConfigs = (await DataStore.get(BUTTON_CONFIG_KEY)) ?? {};
    configsLoaded = true;
}
function saveConfigs() {
    DataStore.set(BUTTON_CONFIG_KEY, buttonConfigs);
}
function getBtnCfg(id) {
    return buttonConfigs[id] ?? { label: id };
}
function setBtnCfg(id, patch) {
    buttonConfigs[id] = { ...getBtnCfg(id), label: id, ...patch };
    saveConfigs();
}
function getAllButtons() {
    const out = [];
    const pBtns = document.querySelector(S.panelButtons);
    const cBtns = document.querySelector(S.callControls);
    if (pBtns)
        out.push(...Array.from(pBtns.children));
    if (cBtns)
        out.push(...Array.from(cBtns.children));
    return out;
}
...
function getBtnSelector(canonical) {
...
function onGlobalKeydown(e) {
    ...
    for (const cfg of Object.values(buttonConfigs)) {
        if (cfg.keybind && cfg.keybind === combo) {
            const el = document.querySelector(getBtnSelector(cfg.label));
...
function gridCSS(selector, cols, gap) {
    return `
        ${selector} {
...
function verticalCSS(selector, gap, audioParent, button) {
    return `
        ${selector} {
            display: flex !important;
...
function buildCSS() {
    ...
    switch (st.userPanelLayout) {
        case "grid2":
            lines.push(gridCSS(S.panelButtons, 2, gap));
            break;
        case "grid3":
            lines.push(gridCSS(S.panelButtons, 3, gap));
            break;
        case "vertical":
        ...
        case "split_grid4": {
            ...
            if (st.userPanelLayout === "split_grid2")
                flexSize = `0 0 calc(50% - (${gap}px / 2))`;
            if (st.userPanelLayout === "split_grid3")
                flexSize = `0 0 calc(33.333% - (${gap}px * 2 / 3))`;
            if (st.userPanelLayout === "split_grid4")
                flexSize = `0 0 calc(25% - (${gap}px * 3 / 4))`;
        ...
        case "all_top":
            ...
            break;
        case "hidden":
            lines.push(`${S.panelButtons} { display: none !important; }`);
            break;
        default:
            if (gap !== 4)
                lines.push(`${S.panelButtons} { gap: ${gap}px !important; }`);
            break;
    }
    // Call controls layout
    switch (st.callControlsLayout) {
        case "grid2":
            lines.push(gridCSS(S.callControls, 2, gap));
            break;
        case "vertical":
            lines.push(`
            ...
            break;
        case "hidden":
            lines.push(`${S.callControls} { display: none !important; }`);
            break;
        default:
            if (gap !== 4)
                lines.push(`${S.callControls} { gap: ${gap}px !important; }`);
            break;
    }
    // Icon & Button size
    if (st.iconSize !== 20) {
    ...
    switch (st.hoverEffect) {
        case "scale":
            ...
            break;
        case "glow":
            lines.push(`${S.panelButtons} ${S.panelButton}:hover { filter: drop-shadow(0 0 6px ${st.glowColor}) !important; transition: filter 0.15s ease !important; }`);
            break;
        case "bright":
    ...
    // Visibility toggles
    if (st.hideChevrons)
        lines.push(`${S.panelButtons} ${S.chevron} { display: none !important; }`);
    if (st.hideDisconnect)
        lines.push(`${S.disconnect} { display: none !important; }`);
    if (st.hideVoiceStatus)
        lines.push(`${S.voiceStatus} { display: none !important; }`);
    if (st.hidePingIcon)
        lines.push(`${S.pingIcon} { display: none !important; }`);
    if (st.callCompact) {
    ...
    if (st.hideMute)
        lines.push(`${getBtnSelector("Mute")} { display: none !important; }`);
    if (st.hideDeafen)
        lines.push(`${getBtnSelector("Deafen")} { display: none !important; }`);
    if (st.hideSettings)
        lines.push(`${getBtnSelector("User Settings")} { display: none !important; }`);
    if (st.hideCamera)
        lines.push(`${getBtnSelector("Camera")} { display: none !important; }`);
    if (st.hideScreenShare)
        lines.push(`${getBtnSelector("Screen Share")} { display: none !important; }`);
    if (st.hideActivity)
        lines.push(`${getBtnSelector("Activity")} { display: none !important; }`);
...
function buildCustomCSS() {
    ...
    for (const cfg of Object.values(buttonConfigs)) {
        ...
        if (cfg.hidden)
            lines.push(`${sel} { display: none !important; }`);
        if (cfg.order != null) {
...
function SliderRow({ label, value, min, max, unit = "px", onChange, resetKey, gap }) {
    ...
    return (<Flex flexDirection="column" gap={gap ? 8 : 0} style={{ width: "100%" }}>
            ...
            <Slider key={`${label}-${resetKey}`} minValue={min} maxValue={max} initialValue={value} markers={stepMarkers} stickToMarkers renderMarker={() => null} 
    // asValueChanges fires continuously while dragging (not just on release),
    // so the panel updates live as the handle moves.
    asValueChanges={v => onChange(Math.round(v))} onValueRender={v => `${Math.round(v)}${unit}`}/>
        </Flex>);
}
function Dropdown({ label, options, value, onChange }) {
    return (<Flex flexDirection="column" gap={8} style={{ width: "100%" }}>
            <BaseText size="md" weight="medium" color="text-default">{label}</BaseText>
...
function ColorRow({ label, value, onChange }) {
    return (<Flex flexDirection="column" gap={8} style={{ width: "100%" }}>
            <BaseText size="md" weight="medium" color="text-default">{label}</BaseText>
            <input type="color" value={value} onChange={e => onChange(e.target.value)} style={{ width: "100%", height: "40px", border: "none", borderRadius: "6px", cursor: "pointer", background: "transparent" }}/>
        </Flex>);
}
...
function getBtnItems() {
    ...
    for (const el of getAllButtons()) {
        ...
        if (svg) {
            ...
            if (lottie) {
                const clone = lottie.cloneNode(true);
                clone.removeAttribute("style");
                iconHTML = clone.outerHTML;
            }
        }
...
function ButtonsDragTab() {
    ...
    const [items, setItems] = React.useState(getBtnItems());
    const [listeningId, setListeningId] = React.useState(null);
    ...
    const dragFromIndex = React.useRef(null);
    ...
    return (<Flex flexDirection="column" gap={16} style={{ paddingBottom: "12px" }}>
            ...
            {items.length === 0 ? (<BaseText size="sm" color="text-muted">No buttons detected. Open this tab again once buttons load.</BaseText>) : (<div className="deracul-scrollbar" style={{
                ...
                return (<div key={item.id} draggable onDragStart={e => handleDragStart(e, index)} onDragOver={e => handleDragOver(e, index)} onDragLeave={() => { if (dragOverIndex === index)
                    setDragOverIndex(null); }} onDrop={e => handleDrop(e, index)} onDragEnd={handleDragEnd} style={{
                        ...
                        transition: "border 0.1s ease, padding 0.1s ease, opacity 0.1s ease, transform 0.1s ease",
                                ...
                                <div className="deracul-btn-preview" dangerouslySetInnerHTML={{ __html: item.iconHTML }} style={{
                        ...
                        boxShadow: "0 2px 4px rgba(0,0,0,0.15)", pointerEvents: "none"
                                ...
                                <MiniToggle value={!cfg.hidden} onChange={v => {
            ...
            {items.length > 0 && (<Flex flexDirection="column" gap={8} style={{ marginTop: "4px" }}>
                ...
                return (<div key={item.id} style={{ padding: "8px 12px", backgroundColor: "var(--background-secondary, var(--background-surface-higher))", borderRadius: "6px" }}>
                                    ...
                                    <Flex gap={8} alignItems="center" style={{ padding: "30px 0 0 0" }}>
                                        {!isUserSettings && !isPanelLayout && settings.store.colorfulActiveButtons && (<>
                                                {((cfg.color !== undefined && cfg.color !== "#5865f2") || (cfg.opacity !== undefined && cfg.opacity !== 100) || ((cfg.background !== undefined && cfg.background !== true) && (settings.store.buttonStyle === "outlined" || settings.store.buttonStyle === "outlineold"))) && (<Button size="small" variant="secondary" title="Reset custom color & opacity" onClick={() => {
                                ...
                                forceUpdate();
                                                ...
                                                <SliderRow label="Opacity" min={0} max={100} value={cfg.opacity ?? 100} onChange={v => {
                            setBtnCfg(item.id, { opacity: Number(Math.round(v)) });
                            apply();
                            forceUpdate();
                        }} unit="%" gap={false}/>
                                                <div>
                                                    <input type="color" title="Active blob background color" value={cfg.color ?? "#5865f2"} onChange={e => {
                            ...
                            width: "100%",
                                                    ...
                                                    <Flex gap={8} alignItems="center">
                                                        ...
                                                        {cfg.keybind && (<Button size="small" variant="secondary" onClick={() => {
...
function PanelLayoutIcon({ style, className }) {
    return (<svg style={style} className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            {/* Top-Left Block */}
...
function PanelLayoutModal({ modalProps }) {
    const [, forceUpdate] = React.useReducer(x => x + 1, 0);
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            background: "var(--brand-experiment, var(--background-brand))", color: "white"
                ...
                <Flex gap={24} style={{ marginTop: "24px", borderBottom: "1px solid var(--background-modifier-accent, var(--border-muted))", width: "100%" }}>
                    {tabsList.map(t => (<div key={t.id} onClick={() => setTab(t.id)} style={{
                paddingBottom: "12px",
                cursor: "pointer",
...
// ─── Panel Button ─────────────────────────────────────────────────────────────
function PanelLayoutButton({ iconForeground, hideTooltips, nameplate }) {
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : "Panel Layout"} icon={<PanelLayoutIcon style={{ color: iconForeground }}/>} role="button" plated={nameplate != null} onClick={() => openModal(modalProps => <PanelLayoutModal modalProps={modalProps}/>)}/>);
}
// ─── Plugin ───────────────────────────────────────────────────────────────────
