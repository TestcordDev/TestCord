...
export default definePlugin({
    ...
    RenderTip(expires_at) {
        ...
        return (<Tooltip text={tooltipText}>
                {({ onMouseEnter, onMouseLeave }) => (<div className="vc-bi-tooltip" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
                        <InfoIcon className="vc-bi-tooltip-icon"/>
                    </div>)}
            </Tooltip>);
    },
    Header(inviter, guildName) {
        if (!inviter)
            return null;
        const userId = UserStore.getCurrentUser().id;
        const isSelf = userId === inviter.id;
        return (<div className="vc-bi-header-inner">
                <img alt="" className={classes(AvatarStyles.avatar, AvatarStyles.clickable) + " vc-bi-inviter-avatar"} onClick={() => openUserProfile(inviter.id)} src={inviter.avatar
