...
const ShowEmbeds = getUserSettingLazy("textAndImages", "renderEmbeds");
...
async function generateDebugInfoMessage() {
    ...
    const platformDisplay = spoofInfo?.spoofed
        ...
        : platformName();
    const info = {
        Testcord: `v${VERSION} • [${gitHashShort}](<https://github.com/TestcordDev/Testcord/commit/${gitHash}>)` +
...
async function uploadPluginListFile(channelId, fileContent, filename, pluginCount) {
...
function DevBuildConfirmModal(props) {
    const s = settings.use(["dismissedDevBuildWarning"]);
    return (<ConfirmModal {...props} title="Hold on!" confirmText="Understood" variant="primary" checkboxProps={{
...
export default definePlugin({
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (<ConfirmModal {...props} variant="primary" title="Hold on!" confirmText="Update & Restart Now" cancelText="View Updates" onConfirm={forceUpdate} onCancel={() => openSettingsTabModal(UpdaterTab)}>
                            <div>
    ...
    renderMessageAccessory(props) {
        ...
        const shouldAddUpdateButton = !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && testCordSupport))
            && props.message.content?.toLowerCase().includes("update");
        if (shouldAddUpdateButton) {
            buttons.push(<Button key="vc-update" variant="positive" onClick={async () => {
                    try {
        ...
        if (testCordSupport && isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/testcord-debug") || props.message.content.includes("/testcord-plugins")) {
                buttons.push(<Button key="vc-dbg" variant="secondary" onClick={async () => sendMessage(props.channel.id, { content: await generateDebugInfoMessage() })}>
                        Run /testcord-debug
                    </Button>, <Button key="vc-plg-list" variant="secondary" onClick={async () => {
        ...
        if (testCordSupport || isSupportChannel(props.channel.id) || isKnownIssuesCategory(props.channel.parent_id)) {
            ...
            if (match) {
                buttons.push(<Button key="vc-run-snippet" onClick={async () => {
                        try {
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        if (!isAnyPluginDev(userId))
            return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id))
            return null;
        return (<Card variant="warning" className={Margins.top8} defaultPadding>
