...
export function NewPluginsSection({ newPlugins, onPluginToggle, }) {
    ...
    const makeDependencyList = (deps) => {
        if (!deps)
            return null;
        return (<React.Fragment>
                <Paragraph>This plugin is required by:</Paragraph>
                {deps.map((dep) => (<Paragraph key={dep} className="vc-changelog-dep-text">
                        {dep}
                    </Paragraph>))}
            </React.Fragment>);
    };
    return (<div className={cl("new-plugins-section")}>
            ...
            if (isRequired) {
                return (<Tooltip text={tooltipText} key={plugin.name}>
                                {({ onMouseLeave, onMouseEnter }) => (<Card className={cl("new-plugin-card", "required")}>
                                        <PluginCard onMouseLeave={onMouseLeave} onMouseEnter={onMouseEnter} onRestartNeeded={name => {
                            changes.handleChange(name);
                            forceUpdate();
                        }} disabled={true} plugin={plugin} isNew={true}/>
                                    </Card>)}
                            </Tooltip>);
            }
            return (<Card key={plugin.name} className={cl("new-plugin-card")}>
                            <PluginCard onRestartNeeded={name => {
                    changes.handleChange(name);
                    forceUpdate();
                }} disabled={false} plugin={plugin} isNew={true}/>
                        </Card>);
        })}
            </div>

            {changes.hasChanges && (<div className={cl("restart-notice")}>
                    <Tooltip text={<>
                        ...
                        {tooltipProps => (<Button {...tooltipProps} variant="link" size="small" onClick={() => location.reload()} className={Margins.top16}>
                                Restart Required
                            </Button>)}
                    </Tooltip>
                </div>)}
...
function CompactPluginCard({ pluginName, depMap, settings, }) {
    const plugin = Plugins[pluginName];
    if (!plugin || plugin.hidden || typeof plugin.name !== "string")
        return null;
    const isRequired = plugin.required ||
        depMap[plugin.name]?.some(d => settings.plugins[d].enabled);
    const tooltipText = plugin.required
        ...
        : depMap[plugin.name]?.length > 0
            ? `This plugin is required by: ${depMap[plugin.name]
                ...
                .join(", ")}`
            : null;
    return (<div className={`vc-changelog-entry ${isRequired ? "required" : ""}`}>
...
export function NewPluginsCompact({ newPlugins, maxDisplay = 20, }) {
    const settings = useSettings();
    const depMap = React.useMemo(() => {
        const o = {};
        for (const plugin in Plugins) {
    ...
    return (<div className={cl("new-plugins-compact")}>
            <div className="vc-changelog-plugins-list">
                {displayPlugins.map(pluginName => (<CompactPluginCard key={pluginName} pluginName={pluginName} depMap={depMap} settings={settings}/>))}

                {hasMore && (<div className="vc-changelog-entry">
                        <div className="vc-changelog-entry-message">
