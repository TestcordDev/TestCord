...
let _modalConnectionsCache = null;
let _modalConnectionsRaw = null;
function getModalConnections() {
    const raw = settings.store.fakeConnectionsList || "[]";
    if (raw === _modalConnectionsRaw && _modalConnectionsCache)
...
function setModalConnections(list) {
 ...
 */
function extractNameplateAsset(src) {
...
function maxSinceDateForMonths(minMonths) {
    const d = new Date();
...
function MfColorPicker({ label, value, onChange }) {
    const hex = value ? "#" + Number(value).toString(16).padStart(6, "0") : "";
    return (<div className="fus-mf-field">
            <MfSectionLabel>{label}</MfSectionLabel>
            <div className="fus-mf-color-row">
                <input type="color" className="fus-mf-color-swatch" value={hex || "#5865f2"} onChange={e => {
            const n = parseInt(e.target.value.replace("#", ""), 16);
            if (!isNaN(n))
                onChange(String(n));
        }}/>
                <input className="fus-mf-input fus-mf-color-input" placeholder="#5865f2 (decimal)" value={value} onChange={e => onChange(e.target.value)}/>
                {value && (<button className="fus-mf-clear-btn" onClick={() => onChange("")} title="Clear">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
                    </button>)}
            </div>
        </div>);
}
function MfImageUpload({ label, value, onChange }) {
...
function MfBadgeBtn({ label, icon, active, onClick }) {
    return (<button onClick={onClick} className={`fus-mf-badge ${active ? "fus-mf-badge--on" : ""}`} style={{ display: "flex", alignItems: "center", gap: 5 }}>
 ...
 */
function MfCollapse({ label, count, open, onToggle, children }) {
    return (<div className="fus-mf-field">
...
export function FakeUserSwitcherModal({ modalProps }) {
    const [, forceUpdate] = React.useReducer(x => x + 1, 0);
    React.useEffect(() => { return subscribe(() => forceUpdate()); }, []);
    const [inputId, setInputId] = React.useState("");
    const [loading, setLoading] = React.useState(false);
    const [previewUser, setPreviewUser] = React.useState(getCachedTarget()?.user ?? null);
    const [previewProfile, setPreviewProfile] = React.useState(getCachedTarget()?.profile ?? null);
    ...
    const [nameplates, setNameplates] = React.useState([]);
    ...
    const Label = ({ children, style }) => (<div title={typeof children === "string" ? children : undefined} style={{
            fontSize: "11px",
            fontWeight: 700,
    ...
    const Pfp = ({ user, size, status }) => {
        ...
        return (<div style={{ position: "relative", width: size, height: size, flexShrink: 0, transition: settings.store.disableAnimations ? "none" : "width 0.25s cubic-bezier(0.4, 0, 0.2, 1), height 0.25s cubic-bezier(0.4, 0, 0.2, 1)" }}>
                <img src={src} onError={e => { e.target.src = "https://cdn.discordapp.com/embed/avatars/0.png"; }} style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }}/>
                {status && (<div style={{
                    position: "absolute",
                    bottom: 0,
                    right: 0,
    ...
    async function doActivate(userId) {
    ...
    function uploadManualImage(target) {
    ...
    function doRemoveSaved(id) {
        const list = getSavedUsers().filter(s => s.id !== id);
        setSavedUsers(list);
        setSaved(list);
    }
    const renderCompactButton = (onClick, title, defaultBg, hoverBg, defaultColor, hoverColor, defaultBorder, hoverBorder, children) => (<div onClick={onClick} title={title} style={{
            width: "22px",
            height: "22px",
            borderRadius: "4px",
            display: "flex",
        ...
        }} onMouseDown={e => { if (!settings.store.disableAnimations)
        e.currentTarget.style.transform = "scale(0.95)"; }} onMouseUp={e => { if (!settings.store.disableAnimations)
        e.currentTarget.style.transform = "scale(1)"; }}>
            {children}
        </div>);
    // Get the profile from Discord's UserProfileStore if we have a real user preview
    const getBannerUrl = (user, profile) => {
        if (!user)
            return null;
    ...
    const isCurrentlyActive = (id, name, avatar) => {
        if (!active)
            return false;
        if (id.startsWith("manual_")) {
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "16px", paddingBottom: "8px" }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                            <input type="text" value={inputId} onChange={e => setInputId(e.target.value)} onKeyDown={e => { if (e.key === "Enter")
        doPreview(); }} placeholder="Enter a Discord User ID" style={{ flex: 1, padding: "10px 14px", backgroundColor: "var(--background-tertiary)", border: `1px solid ${isIdInvalid ? "var(--status-danger)" : "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))"}`, borderRadius: "8px", color: "#dbdee1", fontSize: "14px", outline: "none", fontFamily: "var(--font-code, monospace)" }}/>
                ...
                position: "relative"
                                    ...
                                    {active && (activeManualUser || currentTargetId === displayUser.id) && (<div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: "4px", backgroundColor: "var(--status-danger)" }}/>)}
                                </div>
                                ...
                                <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", padding: "16px" }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: "18px", fontWeight: 700, color: "#e0e1e5", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "6px" }}>
                                            ...
                                            {displayUser.clan?.tag && (<span style={{
                    fontSize: "10px",
                    fontWeight: 700,
                    ...
                    lineHeight: "1.2"
                                        ...
                                        {displayUser.id && displayUser.id !== "0" && (<div onClick={() => {
                    ...
                    lineHeight: 1
                                        ...
                                        {displayUser.pronouns && (<div style={{ fontSize: "12px", color: Col.muted, marginTop: "2px", fontStyle: "italic" }}>
                                                {displayUser.pronouns}
                                            </div>)}
                                        {displayUser.bio && (<div style={{ fontSize: "12px", color: Col.muted, marginTop: "4px", whiteSpace: "pre-wrap", borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "4px" }}>
                                                {displayUser.bio}
                                            </div>)}
                                        {settings.store.manualMode && settings.store.manualActivityName && (<div style={{ fontSize: "12px", color: Col.primary, marginTop: "6px", display: "flex", gap: "4px", alignItems: "center", backgroundColor: "rgba(255,255,255,0.03)", padding: "4px 8px", borderRadius: "4px" }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0 }}>
                                        {!(active && (activeManualUser || currentTargetId === displayUser.id)) ? (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.GREEN} disabled={loading} onClick={() => doActivate(displayUser.id)}>
                                                {loading ? "..." : "Activate"}
                                            </Button>) : (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.RED} onClick={doDeactivate}>
                                                Deactivate
                                            </Button>)}
                                        {displayUser && displayUser.id !== "0" && !saved.find(s => s.id === displayUser.id) && (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.PRIMARY} onClick={() => {
            ...
            : `Saved Identities (${saved.length})`}
                                ...
                                <div role="button" title="Export saved identities" className="fus-io-btn" onClick={() => {
            if (saved.length === 0) {
            ...
            showToast("Exported presets successfully!", Toasts.Type.SUCCESS);
                                ...
                                <div role="button" title="Import saved identities" className="fus-io-btn" onClick={() => {
            const input = document.createElement("input");
            input.type = "file";
            input.accept = ".json";
            ...
            input.click();
                            ...
                            {saved.length > 0 && (<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                padding: "2px 0"
                                    ...
                                    <div onClick={() => setCompact(!compact)} title={compact ? "Show detailed list" : "Show compact list"} style={{
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
            ...
            }} onMouseEnter={() => setCompactHovered(true)} onMouseLeave={() => setCompactHovered(false)}>
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                            {compact ? (<path d="M3 6h18M3 12h18M3 18h18"/>) : (<path d="M4 6h16M4 10h16M4 14h16M4 18h16"/>)}
                                        </svg>
                                    </div>
                                </div>)}
                        </div>
                        {saved.length > 0 ? (<div style={{
                display: "grid",
                ...
                transition: "grid-template-columns 0.25s cubic-bezier(0.4, 0, 0.2, 1), gap 0.25s ease"
                                ...
                                {filteredSaved.length > 0 ? (filteredSaved.map(s => {
                ...
                return (<div key={s.id} style={{
                        ...
                        showToast("Copied ID to clipboard", Toasts.Type.SUCCESS);
                    } : undefined} title={compact ? `ID: ${s.id}\n(Click to copy)` : undefined} style={{
                        width: "fit-content",
                        maxWidth: "100%",
                        ...
                        transition: "gap 0.25s ease"
                                                            ...
                                                            {displayUsername && (<span style={{ fontSize: "11px", color: Col.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "100%" }}>
                                                                    @{displayUsername}
                                                                </span>)}
                        ...
                        transition: "max-height 0.25s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.2s ease, margin-top 0.25s ease"
                                                ...
                                                <div style={{ display: "flex", gap: compact ? "4px" : "6px", flexShrink: 0 }}>
                                                    {compact ? (<>
                                                            {!isCurrent ? (renderCompactButton(() => doActivate(s.id), "Use identity", "var(--background-tertiary)", "var(--status-positive)", "var(--text-muted)", "#ffffff", "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", "var(--status-positive)", <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>)) : (renderCompactButton(doDeactivate, "Stop spoofing", "var(--background-tertiary)", "var(--status-danger)", "var(--text-muted)", "#ffffff", "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", "var(--status-danger)", <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><rect x="5" y="5" width="14" height="14" rx="2" ry="2"/></svg>))}
                                                            {renderCompactButton(() => doEditSaved(s), "Edit identity", "var(--background-tertiary)", "var(--brand-experiment)", "var(--text-muted)", "#ffffff", "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", "var(--brand-experiment)", <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>)}
                                                            {renderCompactButton(() => doRemoveSaved(s.id), "Remove", "var(--background-tertiary)", "var(--status-danger)", "var(--text-muted)", "#ffffff", "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", "var(--status-danger)", <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>)}
                                                        </>) : (<>
                                                            {!isCurrent && (<Button size={Button.Sizes.SMALL} color={Button.Colors.GREEN} disabled={loading} onClick={() => doActivate(s.id)}>
                                                                    Use
                                                                </Button>)}
                                                            {isCurrent && (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={doDeactivate}>
                                                                    Stop
                                                                </Button>)}
                                                            <Button size={Button.Sizes.SMALL} color={Button.Colors.BRAND} onClick={() => doEditSaved(s)}>
                                                                Edit
                                                            </Button>
                                                            <Button size={Button.Sizes.SMALL} color={Button.Colors.PRIMARY} onClick={() => doRemoveSaved(s.id)}>
                                                                ✕
                                                            </Button>
                                                        </>)}
                                                </div>
                            ...
                            </div>) : (<div style={{ padding: "14px", backgroundColor: "var(--background-secondary)", borderRadius: "8px", border: "1px dashed var(--background-modifier-accent)", color: Col.muted, fontSize: "13px", textAlign: "center" }}>
        ...
        ]} isSelected={v => v === (settings.store.manualStatus || "online")} serialize={v => String(v)} select={v => {
        ...
        })()} isSelected={v => v === settings.store.manualClanGuildId} serialize={v => String(v)} select={v => {
        ...
        ]} isSelected={v => v === settings.store.manualClanBadge} serialize={v => String(v)} select={v => {
            ...
            notify();
                                    ...
                                    {settings.store.manualClanBadge === "custom" && (<div>
            ...
            list.push({
                ...
                manualCustomBadgeIds: (() => { try {
                    const p = JSON.parse(settings.store.manualCustomBadgeIds || "[]");
            ...
            showToast("Saved custom profile preset!", Toasts.Type.SUCCESS);
                                        ...
                                        {active && settings.store.manualMode ? (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.RED} onClick={() => { settings.store.manualMode = false; setEnabled(false); showToast("Manual spoof disabled.", Toasts.Type.SUCCESS); forceUpdate(); }}>Deactivate</Button>) : (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.GREEN} onClick={() => { settings.store.manualMode = true; setEnabled(true); showToast("Manual spoof activated.", Toasts.Type.SUCCESS); forceUpdate(); }}>Activate Spoof</Button>)}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div onClick={() => setProfileExpanded(!profileExpanded)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", userSelect: "none", padding: "12px 16px", backgroundColor: "var(--background-secondary-alt)", borderRadius: "8px", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.08))", transition: "background-color 0.2s, border-color 0.2s", marginBottom: "6px" }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = "var(--background-modifier-hover)"; e.currentTarget.style.borderColor = "var(--border-neutral-medium, rgba(255, 255, 255, 0.15))"; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = "var(--background-secondary-alt)"; e.currentTarget.style.borderColor = "var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.08))"; }}>
                        ...
                        <div style={{ maxHeight: profileExpanded ? "none" : "0px", overflow: profileExpanded ? "visible" : "hidden", transition: settings.store.disableAnimations ? "none" : "opacity 0.35s cubic-bezier(0.4, 0, 0.2, 1)", opacity: profileExpanded ? 1 : 0 }}>
                            <div style={{ padding: "8px 12px 16px 12px", backgroundColor: "var(--background-secondary)", borderRadius: "8px", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", display: "flex", flexDirection: "column", gap: "4px", marginBottom: "8px" }}>
                                ...
                                <FormSwitch value={settings.store.overlaySelf} onChange={v => {
            ...
            notify();
            forceUpdate();
        }} description="Apply the cosmetics below (decoration, nameplate, effect, badges, accent, bio, pronouns) onto your OWN real account, keeping your real name and avatar." title="Apply cosmetics to my account"/>
                                ...
                                <MfCollapse label="Badges" count={FLAG_BADGE_OPTIONS.length} open={flagGridOpen} onToggle={() => setFlagGridOpen(o => !o)}>
                                    <div className="fus-mf-badges fus-mf-grid-scroll" style={{ marginBottom: 14 }}>
                                        {FLAG_BADGE_OPTIONS.map(b => (<MfBadgeBtn key={b.flag} label={b.label} icon={b.icon} active={!!((settings.store.manualPublicFlags || 0) & b.flag)} onClick={() => { settings.store.manualPublicFlags = (settings.store.manualPublicFlags || 0) ^ b.flag; notify(); forceUpdate(); }}/>))}
                                    </div>
                                </MfCollapse>

                                <MfSectionLabel>Nitro badge tier</MfSectionLabel>
                                <div className="fus-mf-badges" style={{ marginBottom: 14 }}>
                                    {NITRO_LEVELS.map((n, i) => (<MfBadgeBtn key={i} label={n.label} icon={n.icon || undefined} active={(settings.store.manualNitroLevel ?? -1) === (i - 1)} onClick={() => {
                const level = i - 1;
                settings.store.manualNitroLevel = level;
                if (level >= 0) {
                    settings.store.manualPremiumType = 2;
                }
                else {
                    settings.store.manualPremiumType = 0;
                }
                notify();
                forceUpdate();
            }}/>))}
            ...
            return (<input className="fus-mf-input" type="date" min={SINCE_MIN_DATE} max={maxDate} value={settings.store.manualNitroSince} onChange={e => { settings.store.manualNitroSince = e.target.value; notify(); forceUpdate(); }}/>);
        })()}
                                </div>

                                <MfSectionLabel>Boost badge tier</MfSectionLabel>
                                <div className="fus-mf-badges" style={{ marginBottom: 14 }}>
                                    {BOOST_LEVELS.map((b, i) => (<MfBadgeBtn key={i} label={b.label} icon={b.icon || undefined} active={(settings.store.manualBoostMonths ?? -1) === (i - 1)} onClick={() => { settings.store.manualBoostMonths = i - 1; notify(); forceUpdate(); }}/>))}
                                </div>
                                <div className="fus-mf-field">
            ...
            const maxDate = minMonths > 0 ? maxSinceDateForMonths(minMonths) : maxSinceDateForMonths(0);
            return (<input className="fus-mf-input" type="date" min={SINCE_MIN_DATE} max={maxDate} value={settings.store.manualBoostSince} onChange={e => { settings.store.manualBoostSince = e.target.value; notify(); forceUpdate(); }}/>);
        })()}
                                </div>
                                ...
                                <MfCollapse label="Avatar decoration" count={decorations.length} open={decoGridOpen} onToggle={() => setDecoGridOpen(o => !o)}>
                                    <div className="fus-mf-decoration-grid fus-mf-grid-scroll">
                                        ...
                                        {decorations.map(d => (<div key={d.asset} className={`fus-mf-decoration-item ${settings.store.manualAvatarDecoration === d.asset ? "fus-mf-decoration-item--selected" : ""}`} onClick={() => { const nv = settings.store.manualAvatarDecoration === d.asset ? "" : d.asset; settings.store.manualAvatarDecoration = nv; settings.store.manualDecorationAsset = nv; notify(); forceUpdate(); }}>
                                                <div className="fus-mf-decoration-preview"><img src={`https://cdn.discordapp.com/avatar-decoration-presets/${d.asset}.png`} alt={d.name || d.asset} className="fus-mf-decoration-img" onError={e => { e.target.style.display = "none"; }}/></div>
                                ...
                                <MfCollapse label="Nameplate" count={nameplates.length} open={nameplateGridOpen} onToggle={() => setNameplateGridOpen(o => !o)}>
                                    <div className="fus-mf-nameplate-grid fus-mf-grid-scroll">
                                        ...
                                        {nameplates.map((n, i) => (<div key={n.asset || n.src || i} className={`fus-mf-nameplate-item ${n.asset && settings.store.manualNameplateAsset === n.asset ? "fus-mf-nameplate-item--selected" : ""} ${!n.asset ? "fus-mf-nameplate-item--disabled" : ""}`} title={n.asset ? n.name : `${n.name} (preview only — not spoofable)`} onClick={() => { if (!n.asset) {
            showToast("That nameplate has no spoofable asset ID.", Toasts.Type.FAILURE);
            return;
        } const nv = settings.store.manualNameplateAsset === n.asset ? "" : n.asset; settings.store.manualNameplateAsset = nv; settings.store.manualNameplateSkuId = nv; notify(); forceUpdate(); }}>
                                            <div className="fus-mf-nameplate-preview"><img src={n.src} alt={n.name} className="fus-mf-nameplate-img" onError={e => { e.target.style.display = "none"; }}/></div>
                                ...
                                <div className="fus-mf-field">
                                    ...
                                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                        ...
                                        {NAMEPLATE_PALETTES.map(p => (<button key={p} className={`fus-mf-badge ${settings.store.manualNameplatePalette === p ? "fus-mf-badge--on" : ""}`} onClick={() => { settings.store.manualNameplatePalette = p; notify(); forceUpdate(); }}>{p}</button>))}
                                    </div>
                                </div>
                                <div className="fus-mf-field">
                                ...
                                <MfCollapse label="Profile effect" count={effects.length} open={effectGridOpen} onToggle={() => setEffectGridOpen(o => !o)}>
                                    <div className="fus-mf-effect-grid fus-mf-grid-scroll">
                                        ...
                                        {effects.map(ef => (<div key={ef.skuId} className={`fus-mf-effect-item ${settings.store.manualProfileEffectId === ef.skuId ? "fus-mf-effect-item--selected" : ""}`} onClick={() => { const nv = settings.store.manualProfileEffectId === ef.skuId ? "" : ef.skuId; settings.store.manualProfileEffectId = nv; settings.store.manualProfileEffectAsset = nv; notify(); forceUpdate(); }}>
                                                <div className="fus-mf-effect-preview"><img src={ef.config.thumbnailPreviewSrc} alt={ef.config.title || ef.skuId} className="fus-mf-effect-img" onError={ev => { ev.target.style.display = "none"; }}/></div>
            ...
            return CUSTOM_BADGE_OPTIONS.map(b => (<MfBadgeBtn key={b.id} label={b.label} icon={b.icon} active={ids.includes(b.id)} onClick={() => {
                    ...
                    forceUpdate();
                                ...
                                {(() => { try {
            const p = JSON.parse(settings.store.manualCustomBadgeIds || "[]");
            return Array.isArray(p) && p.includes("oldname");
        }
        catch {
            return false;
        } })() && (<div className="fus-mf-field">
            ...
            opacity: customRpcExpanded ? 1 : 0
                            ...
                            <div style={{ padding: "8px 12px 16px 12px", backgroundColor: "var(--background-secondary)", borderRadius: "8px", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", display: "flex", flexDirection: "column", gap: "12px", marginBottom: "8px" }}>
                                <FormSwitch value={settings.store.customRpcEnabled} onChange={v => {
            settings.store.customRpcEnabled = v;
            notify();
        }} description="Override client-side Rich Presence activity with your custom configuration." title="Enable Custom Rich Presence"/>
        ...
        }} isSelected={v => false} serialize={v => v}/>
                                    </div>
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                                        <div>
        ...
        ]} select={v => { settings.store.manualStatus = v; notify(); }} isSelected={v => v === settings.store.manualStatus} serialize={v => v}/>
                                        </div>
                                        <div>
            ...
            { label: "Competing in", value: 5 }
        ]} select={v => { settings.store.manualActivityType = Number(v); notify(); }} isSelected={v => v === settings.store.manualActivityType} serialize={v => String(v)}/>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginTop: "10px", alignItems: "center" }}>
                                        <FormSwitch value={settings.store.manualActivityStartTimer} onChange={v => { settings.store.manualActivityStartTimer = v; notify(); }} description="Display elapsed time since you started the activity." title="Enable Activity Timer"/>
                                    </div>
            ...
            opacity: connectionsExpanded ? 1 : 0
                            ...
                            <div style={{ padding: "8px 12px 16px 12px", backgroundColor: "var(--background-secondary)", borderRadius: "8px", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", display: "flex", flexDirection: "column", gap: "12px", marginBottom: "8px" }}>
                                <FormSwitch value={settings.store.fakeConnectionsEnabled} onChange={v => {
            settings.store.fakeConnectionsEnabled = v;
            notify();
        }} description="Display fake connected social accounts on your profile." title="Enable Fake Connections"/>
            ...
            return (<div style={{ display: "flex", flexDirection: "column", gap: "8px", margin: "8px 0" }}>
                                                {list.map((c, index) => (<div key={c.id || index} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", backgroundColor: "var(--background-tertiary)", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.05)" }}>
                                                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                                            ...
                                                            {c.verified && (<span style={{ fontSize: "10px", fontWeight: 700, color: "var(--status-positive)", backgroundColor: "rgba(35,165,90,0.15)", padding: "1px 6px", borderRadius: "4px" }}>Verified</span>)}
                                                        </div>
                                                        <Button size={Button.Sizes.NONE} color={Button.Colors.RED} style={{ padding: "4px 8px", fontSize: "12px" }} onClick={() => {
        ...
        ]} select={v => setConnType(v)} isSelected={v => v === connType} serialize={v => v}/>
                                            </div>
                                            <div>
                                                <Label>Account Name</Label>
                                                <input type="text" value={connName} onChange={e => setConnName(e.target.value)} placeholder="e.g. Octocat" style={{ width: "100%", padding: "10px", backgroundColor: "var(--background-tertiary)", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", borderRadius: "8px", color: "#dbdee1", fontSize: "14px", outline: "none", boxSizing: "border-box" }}/>
                                            </div>
                                        </div>

                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "4px" }}>
                                            <FormSwitch value={connVerified} onChange={v => setConnVerified(v)} description="Show verified badge next to connection." title="Verified Connection"/>
                                            <Button size={Button.Sizes.MEDIUM} color={Button.Colors.BRAND} disabled={!connName.trim()} onClick={() => {
            ...
            showToast(`Added fake ${connType} connection!`, Toasts.Type.SUCCESS);
                    ...
                    <div>
                        <div onClick={() => setServerExpanded(!serverExpanded)} style={{
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
            cursor: "pointer",
            ...
            if (!currentGuildId) {
                return (<div style={{ color: Col.muted, fontSize: "13px" }}>
            ...
            const options = [
                ...
                ...saved.map(s => ({ label: `${s.name} (${s.isManual ? "Manual" : "Target"})`, value: s.id }))
            ];
            return (<>
                                                ...
                                                <div>
                                                    ...
                                                    <Select options={options} select={v => {
                    ...
                    if (v === "") {
                        ...
                        if (savedItem && !savedItem.isManual) {
                ...
                }} isSelected={v => v === activeIdentityId} serialize={v => v}/>
                                                </div>
                                            </>);
        })()}
                                </div>
            ...
            { label: "3 Yr", val: 36 },
            { label: "6 Yr", val: 72 }
        ].map(preset => (<Button key={preset.val} size={Button.Sizes.MIN} color={settings.store.fakeNitroMonths === preset.val ? Button.Colors.BRAND : Button.Colors.PRIMARY} onClick={() => {
                ...
                notify();
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                                            <input type="number" min="0" value={settings.store.fakeNitroMonths} onChange={e => {
            ...
            notify();
                                            ...
                                            <Button size={Button.Sizes.MEDIUM} color={Button.Colors.RED} disabled={settings.store.fakeNitroMonths === 0} onClick={() => {
            ...
            opacity: configExpanded ? 1 : 0
                            ...
                            <div style={{ padding: "8px 12px 16px 12px", backgroundColor: "var(--background-secondary)", borderRadius: "8px", border: "1px solid var(--border-neutral-semi-weak, rgba(255, 255, 255, 0.15))", display: "flex", flexDirection: "column", gap: "16px", marginBottom: "8px" }}>
                                    <FormSwitch value={settings.store.fakeMessages} onChange={v => { settings.store.fakeMessages = v; notify(); }} description="Post local fake messages as the spoofed target user instead of actually sending them." title="Fake outgoing messages"/>
                                    <FormSwitch value={settings.store.sendRealToo} onChange={v => { settings.store.sendRealToo = v; notify(); }} description="Also transmit the real message to the server (visible to others) in addition to displaying the fake one." disabled={!settings.store.fakeMessages} title="Send real message too"/>
                                    <FormSwitch value={settings.store.spoofBadges} onChange={v => { settings.store.spoofBadges = v; notify(); }} description="Replicate the target's badges onto your client-side profile." title="Spoof profile badges"/>
                                    <FormSwitch value={settings.store.spoofActivities} onChange={v => { settings.store.spoofActivities = v; notify(); }} description="Replicate the target's connected accounts and game library (in cloned mode)." title="Spoof activities and connections"/>
                                    <FormSwitch value={settings.store.disableAnimations} onChange={v => { settings.store.disableAnimations = v; notify(); }} description="Disable all layout transitions and sliding animations inside this settings panel." title="Disable animations"/>
                                    <FormSwitch value={settings.store.patchInternalAccountSwitcher} onChange={v => { settings.store.patchInternalAccountSwitcher = v; notify(); }} description="Integrate spoofed profiles directly into Discord's account switcher menu." title="Account switcher integration"/>
            ...
            { label: "Do Not Disturb", value: "dnd" },
            { label: "Invisible", value: "offline" }
        ]} select={v => { settings.store.spoofedStatus = v; notify(); forceUpdate(); }} isSelected={v => v === settings.store.spoofedStatus} serialize={v => v}/>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="vc-form-switch-container vc-form-switch-no-border" style={{ marginBottom: 0, paddingBottom: 0 }}>
        ...
        ]} select={v => { settings.store.uiMode = v; notify(); }} isSelected={v => v === settings.store.uiMode} serialize={v => v}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
