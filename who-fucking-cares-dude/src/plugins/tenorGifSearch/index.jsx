...
function mapToDiscordGifs(items) {
...
async function fetchTenorResults(path, limit, extra = {}) {
    ...
    while (items.length < limit) {
        ...
        if (pos)
            params.pos = pos;
        const { next, results: page } = await tenorFetch(path, params);
        if (!page.length)
            break;
...
async function fetchCategories() {
    return tenorFetch("/categories", { type: "featured" })
        .then(({ tags }) => {
        if (!tags?.length)
            return null;
        return {
...
export default definePlugin({
    ...
    async handleSuggestionsFetch(query) {
        if (!query)
            return;
        const { results } = await tenorFetch("/search_suggestions", { q: query, limit: "5" });
    ...
    tenorIntegrationSearch(integration, query) {
