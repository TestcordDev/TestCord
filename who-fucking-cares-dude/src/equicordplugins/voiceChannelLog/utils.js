...
export async function downloadSound(id) {
...
export function formatElapsedTime(startTime, eventTime) {
    if (!startTime)
        return null;
    ...
    if (hours > 0)
        return `${hours}h ${minutes}m into the call`;
    if (minutes > 0)
        return `${minutes}m into the call`;
