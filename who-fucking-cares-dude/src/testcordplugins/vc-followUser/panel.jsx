...
import { Avatar, Button, ChannelStore, GuildStore, React, RelationshipStore, UserStore, VoiceStateStore } from "@webpack/common";
import { getAllUsersInVoice, getPinnedUserIds, settings, toggleFollow, togglePin, triggerFollow } from "./index";
function FollowIcon({ className, active }) {
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
            <path fill={active ? "var(--status-positive)" : "currentColor"} d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12Zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8Z"/>
...
function FollowUserModal({ modalProps }) {
    ...
    const nonFriendsInVoice = React.useMemo(() => {
        if (!showNonFriendsInVoice)
            return [];
        const result = [];
    ...
    function doUnfollow() {
        if (settings.store.followUserId)
            toggleFollow(settings.store.followUserId);
        forceUpdate();
    }
    function doJoin(channelId) {
        triggerFollow(channelId);
        modalProps.onClose();
    }
    function doUnpin(userId) {
        togglePin(userId);
        forceUpdate();
    }
    return (<ModalRoot {...modalProps} title="Follow User" size={"medium" /* ModalSize.MEDIUM */}>
            <ModalHeader>
            ...
            <ModalContent>
                ...
                : "Not in voice right now"}
                                ...
                                <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                                    {followedChannel && (<Button size={Button.Sizes.MEDIUM} color={Button.Colors.GREEN} onClick={() => doJoin(followedChannel.id)}>
                                            Join
                                        </Button>)}
                ...
                textAlign: "center"
                    ...
                    <div>
                        ...
                        {pinnedUsers.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                {pinnedUsers.map(person => {
                const isFollowingThis = followUserId === person.userId;
                return (<div key={person.userId} style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "12px",
                        padding: "12px 14px",
                        ...
                        : "Not in voice right now"}
                                            ...
                                            <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                                                {person.channelId && (<Button size={Button.Sizes.SMALL} color={Button.Colors.BRAND} onClick={() => doJoin(person.channelId)}>
                                                        Join
                                                    </Button>)}
                                                {isFollowingThis ? (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={doUnfollow}>
                                                        Unfollow
                                                    </Button>) : (<Button size={Button.Sizes.SMALL} color={Button.Colors.GREEN} onClick={() => doFollow(person.userId)}>
                                                        Follow
                                                    </Button>)}
                ...
                textAlign: "center"
                    ...
                    <div>
                        ...
                        {friendsInVoice.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                {friendsInVoice.map(friend => {
                const isFollowingThis = followUserId === friend.userId;
                return (<div key={friend.userId} style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "12px",
                        padding: "12px 14px",
                        ...
                        border: `1px solid ${isFollowingThis ? "var(--status-positive)" : "var(--background-modifier-accent)"}`
                                            ...
                                            <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                                                ...
                                                {isFollowingThis ? (<Button size={Button.Sizes.SMALL} color={Button.Colors.RED} onClick={doUnfollow}>
                                                        Unfollow
                                                    </Button>) : (<Button size={Button.Sizes.SMALL} color={Button.Colors.GREEN} onClick={() => doFollow(friend.userId)}>
                                                        Follow
                                                    </Button>)}
                                            </div>
                                        </div>);
                ...
                textAlign: "center"
                    ...
                    {showNonFriendsInVoice && (<div>
                            <SectionLabel>Non-Friends in Voice {nonFriendsInVoice.length > 0 ? `(${nonFriendsInVoice.length})` : null}</SectionLabel>
                            {nonFriendsInVoice.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        ...
        ].map(({ label, value }) => (<div key={label} style={{ display: "flex", alignItems: "center", gap: "7px" }}>
                                    <div style={{
                width: 8,
                height: 8,
                borderRadius: "50%",
...
function PanelButton({ iconForeground, hideTooltips, nameplate }) {
    ...
    return (<UserAreaButton tooltipText={hideTooltips ? void 0 : isFollowing ? `Following ${name}` : "Follow User"} icon={<FollowIcon className={iconForeground} active={isFollowing}/>} role="button" aria-checked={isFollowing} plated={nameplate != null} onClick={() => openModal(props => <FollowUserModal modalProps={props}/>)}/>);
}
const PanelButtonBoundary = ErrorBoundary.wrap(PanelButton, { noop: true });
export const FollowPanelButton = props => <PanelButtonBoundary {...props}/>;
export { FollowIcon };
