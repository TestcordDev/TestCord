    ...
    flux: {
        async MESSAGE_CREATE({ optimistic, type, message, channelId }) {
            ...
            if (settings.store.ignoreBots && message.author?.bot)
                return;
            if (settings.store.ignoreBlocked && RelationshipStore.isBlocked(message.author?.id))
                return;
            if (!message.content)
                return;
        ...
        MESSAGE_REACTION_ADD({ optimistic, type, channelId, userId, messageAuthorId, emoji }) {
            if (optimistic || type !== "MESSAGE_REACTION_ADD")
                return;
            if (settings.store.ignoreBots && UserStore.getUser(userId)?.bot)
                return;
            if (settings.store.ignoreBlocked && RelationshipStore.isBlocked(messageAuthorId))
                return;
            if (channelId !== SelectedChannelStore.getChannelId())
                return;
        ...
        VOICE_CHANNEL_EFFECT_SEND({ emoji }) {
            if (!emoji?.name)
                return;
            const name = emoji.name.toLowerCase();
            if (name !== MOYAI && !name.includes("moyai") && !name.includes("moai"))
...
function countOccurrences(sourceString, subString) {
    let i = 0;
    let lastIdx = 0;
...
function countMatches(sourceString, pattern) {
    if (!pattern.global)
...
function getMoyaiCount(message) {
    const count = countOccurrences(message, MOYAI)
        + countMatches(message, customMoyaiRe);
    return Math.min(count, 10);
}
function boom() {
    if (!settings.store.triggerWhenUnfocused && !document.hasFocus())
        return;
