...
const QuickMsgIcon = ({ height = 20, width = 20, className }) => (<svg aria-hidden="true" role="img" width={width} height={height} className={className} viewBox="0 0 24 24" fill="currentColor">
        <path d="M13 2L3 14h7l-1 8 10-12h-7l1-8z"/>
    </svg>);
async function quickSend(channelId) {
...
const QuickMsgButton = ({ isAnyChat, channel }) => {
    if (!isAnyChat)
        return null;
    return (<ChatBarButton tooltip="Quick Send & Delete" onClick={() => quickSend(channel.id)}>
            <QuickMsgIcon />
        </ChatBarButton>);
};
