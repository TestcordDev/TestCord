...
async function openRoleIconModal(roleId, roleIcon, roleName) {
...
export function buildExtraRoleContextMenuItems(role, guild, popoutRef) {
    if (!role)
        return { before: [], after: [] };
    const before = [
        PermissionStore.getGuildPermissionProps(guild).canManageRoles && (<Menu.MenuItem key="vc-edit-role" id="vc-edit-role" label="Edit Role" action={async () => {
                await GuildSettingsActions.open(guild.id, "ROLES");
                GuildSettingsActions.selectRole(role.id);
            }} icon={PencilIcon}/>),
        role.colorString && (<Menu.MenuItem key="vc-copy-role-color" id="vc-copy-role-color" label="Copy Role Color" action={() => copyToClipboard(role.colorString)} icon={AppearanceIcon}/>)
    ].filter(isTruthy);
    const after = [
        role.icon && (<Menu.MenuItem key="vc-view-role-icon" id="vc-view-role-icon" label="View Role Icon" action={() => {
                openImageModal({
        ...
        popoutRef && (<Menu.MenuItem key="vc-view-role-members" id="vc-view-role-members" label="View Role Members" render={() => (<Popout position="right" align="center" targetElementRef={popoutRef} preload={() => loadRoleMembers(guild.id, role.id)} renderPopout={popoutProps => (<RoleMemberPopout popoutProps={popoutProps} guildId={guild.id} channelId={getCurrentChannel().id} roleId={role.id}/>)}>
                        {popoutProps => (<div className={classes(MenuItemClasses.item, MenuItemClasses.labelContainer, MenuItemClasses.colorDefault)} ref={popoutRef} role="menuitem" {...popoutProps}>
...
export function openRoleContextMenu(event, { guildId, id: roleId }) {
    ...
    ContextMenuApi.openContextMenu(event, () => {
        ...
        return (<Menu.Menu navId="vc-better-role-context-member-list" onClose={ContextMenuApi.closeContextMenu} aria-label="Role Actions">
                {before}
                {after}
                <Menu.MenuItem key="vc-better-role-context-copy-role-id" id="vc-better-role-context-copy-role-id" label={getIntlMessage("COPY_ID_ROLE")} icon={CopyIdIcon} action={() => copyToClipboard(role.id)}/>
            </Menu.Menu>);
    });
}
    ...
    contextMenus: {
        "dev-context"(children, { id }) {
            ...
            if (!role)
                return;
            if (role.colorString) {
                children.unshift(<Menu.MenuItem id="vc-copy-role-color" label="Copy Role Color" action={() => copyToClipboard(role.colorString)} icon={AppearanceIcon}/>);
            }
            if (PermissionStore.getGuildPermissionProps(guild).canManageRoles) {
                children.unshift(<Menu.MenuItem id="vc-edit-role" label="Edit Role" action={async () => {
                        await GuildSettingsActions.open(guild.id, "ROLES");
            ...
            if (role.icon) {
                ...
                children.push(<Menu.MenuItem id="vc-view-role-icon" label="View Role Icon" action={() => openRoleIconModal(role.id, roleIcon, role.name)} icon={ImageIcon}/>);
            }
        }
    }
});
