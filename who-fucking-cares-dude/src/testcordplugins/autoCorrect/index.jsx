...
async function correctText(text) {
    ...
    try {
        ...
        if (!corrected || corrected.trim() === "" || corrected === text)
            return text;
        if (corrected.toLowerCase().includes("correction:") || corrected.toLowerCase().includes("text:"))
            return text;
        if (corrected.length > text.length * 1.5 || corrected.length < text.length * 0.4)
            return text;
        if (aggr === "low") {
...
function AutoCorrectIcon({ enabled }) {
    return (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill="currentColor" d="M8.87 2.31A.5.5 0 0 1 9.34 2h10.92c.36 0 .6.36.47.69l-.6 1.5a.5.5 0 0 1-.47.31h-4.28l-4.17 15h4.05c.36 0 .6.36.47.69l-.6 1.5a.5.5 0 0 1-.47.31H3.74a.5.5 0 0 1-.47-.69l.6-1.5a.5.5 0 0 1 .47-.31h4.28l4.17-15H8.74a.5.5 0 0 1-.47-.69l.6-1.5Z" opacity={enabled ? 1 : 0.35}/>
            {!enabled && (<path fill="var(--status-danger)" d="M21.178 1.707 22.592 3.12 4.12 21.593l-1.414-1.415L21.178 1.707Z"/>)}
        </svg>);
}
const AutoCorrectChatBarButton = ({ type }) => {
...
export default definePlugin({
    ...
    start() {
        ...
        if (location === "headerbar") {
            addHeaderBarButton("AutoCorrect", () => (<HeaderBarButton icon={() => <AutoCorrectIcon enabled={settings.store.isActive}/>} tooltip="AutoCorrect" onClick={() => { settings.store.isActive = !settings.store.isActive; }}/>), 5);
        }
        else if (location === "channeltoolbar") {
            addChannelToolbarButton("AutoCorrect", () => (<ChannelToolbarButton icon={() => <AutoCorrectIcon enabled={settings.store.isActive}/>} tooltip="AutoCorrect" onClick={() => { settings.store.isActive = !settings.store.isActive; }}/>), 5);
        }
    },
    stop() {
    ...
    async onBeforeMessageSend(_channelId, message) {
        ...
        if (!message.content || message.content.trim().length < 3)
            return;
        if (message.content.startsWith("/") || message.content.length > 2000)
            return;
