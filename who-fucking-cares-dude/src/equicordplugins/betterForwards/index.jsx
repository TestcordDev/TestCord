...
export const ForwardOptionsContext = proxyLazyWebpack(() => React.createContext({
    opts: {},
    setOpts: () => { },
...
const getId = ({ id, type }) => {
    ...
    return (ChannelStore.getDMFromUserId(id) ??
        ChannelActionCreators.getOrEnsurePrivateChannel(id));
};
...
export default definePlugin({
            ...
            replacement: [
                {
                    ...
                    replace: (_, additionalMessage, channels, message) => `if(${channels}.length>0)return await $self.sendForward(${additionalMessage},${channels},${message},__state.opts);`,
    ...
    async sendForward(additionalMessage, channels, message, options) {
        ...
        const ids = (await Promise.all(channels.map(getId))).filter(Boolean);
        const chunkSize = 5;
        ids.forEach(id => {
    ...
    shouldTransition(origCond) {
        return ignore ? origCond : false;
    },
    setShift(event) {
        ignore = !!event?.shiftKey;
    },
    renderForwardFooter({ message }) {
    ...
    useProps(props) {
        ...
        const [opts, setOpts] = useState(() => {
            if (!props.forwardOptions || !props.forwardOptions.onlyEmbedIndices)
                return props.forwardOptions ?? {};
            let id = 0;
            const embedsIds = new Set(props.forwardOptions.onlyEmbedIndices);
        ...
        const state = useMemo(() => ({ message, opts, setOpts, defaultOpts, hasOpts }), [message, opts, defaultOpts, hasOpts]);
        return { ...props, forwardOptions, __state: state };
    },
    renderWrapper(state, children) {
