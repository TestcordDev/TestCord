...
export function getActivityApplication(activity) {
    if (!activity)
        return undefined;
...
export function getApplicationIcons(activities, preferSmall = false) {
    ...
    for (const activity of applications) {
        ...
        if (!application_id && !platform && !id?.startsWith("spotify:"))
            continue;
        if (assets) {
