...
import { AccessibilityStore, ChannelStore, GuildMemberStore, GuildStore, Menu, MessageStore, Modal, openModal, RelationshipStore, StreamerModeStore, TextInput, useEffect, useMemo, UserStore, useState } from "@webpack/common";
const SMYNC = classNameFactory();
const wrapEmojis = findByCodeLazy("lastIndex;return");
const UserNameWithEffects = findComponentByCodeLazy("UserNameWithEffects", "--custom-display-name-styles-prism-cycle");
const DisplayNameEffectDisplayTypes = findByPropsLazy("PLAIN", "STATIC", "ANIMATED");
const DisplayNameEffects = findByPropsLazy("SOLID", "GRADIENT", "NEON", "TOON", "POP", "GLOW", "PRISM", "GUMMY");
const DisplayNameFonts = findByPropsLazy("DEFAULT", "CHERRY_BOMB", "CHICLE", "MUSEO_MODERNO");
...
let convertToRGBCanvas = null;
let convertToRGBCtx = null;
let convertToRGBCache = null;
function convertToRGB(color) {
    const cached = convertToRGBCache?.get(color);
    if (cached !== undefined)
        return cached ?? null;
    if (!convertToRGBCanvas || !convertToRGBCtx)
        return null;
...
function validColor(color) {
    ...
    if (!color)
        return true;
    if (trimmedColor.length !== color.length)
        return false;
    if (trimmedColor.toLowerCase() === "role")
        return true;
...
const baseNormalStyle = { "isolation": "isolate" };
...
let cachedDefaultColor = null;
function resolveColor(colorStrings, displayNameStyles, savedColor, canUseGradient, inGuild, ircColorsEnabled, shouldShowEffects) {
    ...
    let primaryAdjusted = null;
    let secondaryAdjusted = null;
    let tertiaryAdjusted = null;
    if (isRoleColor) {
...
const parseTemplateCache = new Map();
function parseTemplateItem(entry) {
...
function getProcessedNames(author, truncateAllNamesWithStreamerMode, discriminators, inGuild, friendNameOnlyInDirectMessages, customNameOnlyInDirectMessages) {
    let discriminator = null;
    if (discriminators) {
        const userAuthor = author;
        if (userAuthor?.bot && !isNaN(userAuthor?.discriminator) && Number(userAuthor?.discriminator) !== 0) {
    ...
    const username = !author?.username ? null
        : StreamerModeStore.enabled
            ? author.username[0] + "..."
            : author.username + (discriminator ? `#${discriminator}` : "");
    const display = !author?.globalName ? null
        : StreamerModeStore.enabled && (truncateAllNamesWithStreamerMode || author.globalName.toLowerCase() === author.username.toLowerCase())
            ? author.globalName[0] + "..."
            : author.globalName;
    const nick = !author?.nick ? null
        : StreamerModeStore.enabled && (truncateAllNamesWithStreamerMode || author.nick.toLowerCase() === author.username.toLowerCase())
            ? author.nick[0] + "..."
            : author.nick;
    const friendName = (author && !(inGuild && friendNameOnlyInDirectMessages)) ? RelationshipStore.getNickname(author.id) || null : null;
    const friend = !friendName ? null
        : StreamerModeStore.enabled && (truncateAllNamesWithStreamerMode || friendName.toLowerCase() === author.username.toLowerCase())
            ? friendName[0] + "..."
            : friendName;
    const customName = (author && !(inGuild && customNameOnlyInDirectMessages)) ? customNicknames[author.id] || null : null;
...
function getTypingMemberListProfilesReactionsVoiceName(props, forceHookless = false) {
...
function getTypingMemberListProfilesReactionsVoiceNameText(props) {
    return getTypingMemberListProfilesReactionsVoiceName(props, /* forceHookless */ true)[2];
}
function getTypingMemberListProfilesReactionsVoiceNameElement(props) {
    return getTypingMemberListProfilesReactionsVoiceName(props)[1];
}
function getActiveNowNameElement({ user, guildId, isHovered }) {
    const name = getTypingMemberListProfilesReactionsVoiceNameText({ user, guildId, type: "membersList" });
    if (!name)
        return null;
    const displayNameStyles = (AccessibilityStore.displayNameStylesEnabled
        && user?.displayNameStyles) || null;
    return displayNameStyles
        ? <DisplayNameEffectName name={name} styles={displayNameStyles} animate={shouldAnimateNameEffects(isHovered, settings.store.styleActiveNow)} showStaticEffect={settings.store.styleActiveNow}/>
        : name;
}
function getMessageName(props) {
...
function getMessageNameElement(props) {
    return getMessageName(props)[1];
}
function getMessageNameText(props) {
    return getMessageName(props)[0];
}
function getMentionNameElement(props) {
...
function getDisplayNameStyles(styles, ignoreFont) {
    return ignoreFont
        ? { ...styles, fontId: DisplayNameFonts.DEFAULT }
        : styles;
}
function getDisplayNameEffectDisplayType(isHovered, showStaticEffect = true) {
    if (shouldAnimateNameEffects(isHovered, showStaticEffect))
        return DisplayNameEffectDisplayTypes.ANIMATED;
    return showStaticEffect
        ? DisplayNameEffectDisplayTypes.STATIC
        : DisplayNameEffectDisplayTypes.PLAIN;
}
function shouldAnimateNameEffects(isHovered, isEffectVisible = true) {
    return isHovered || (isEffectVisible && settings.store.alwaysAnimateEffects);
}
function getDisplayNameEffectClassName(styles, effectDisplayType) {
...
function DisplayNameEffectName({ name, styles, animate, ignoreFont = false, showStaticEffect = true, useMessageLayout = false }) {
    const effectDisplayType = animate
        ...
        : showStaticEffect
            ? DisplayNameEffectDisplayTypes.STATIC
            : DisplayNameEffectDisplayTypes.PLAIN;
    return (<UserNameWithEffects userName={name} displayNameStyles={getDisplayNameStyles(styles, ignoreFont)} effectDisplayType={effectDisplayType} textClassName={SMYNC(getDisplayNameEffectClassName(styles, effectDisplayType), {
            "smyn-native-message-effect": useMessageLayout,
        })} loop/>);
}
function needsGradientAnimationOverride(styles) {
    return styles?.effectId === DisplayNameEffects.GRADIENT
        || styles?.effectId === DisplayNameEffects.GLOW;
}
function getSecondaryNameStyle(style, useNativeEffect, ignoreEffects, animateGradient, showGradientGlow) {
    if (!style)
        return {};
    if (useNativeEffect) {
        if (!showGradientGlow || !style.gradient)
            return {};
        return animateGradient ? style.gradient.animated : style.gradient.static.original;
    }
    if (ignoreEffects)
        return style.normal.adjusted;
    if (animateGradient && style.gradient)
        return style.gradient.animated;
    return style.gradient?.static.original ?? style.normal.adjusted;
}
function renderUsername(author, channelId, messageId, type, mentionSymbol, hookless, inGuild, colorString, colorStrings, isHoveredOverride) {
    ...
    const groupId = message?.showMeYourNameGroupId || null;
    ...
    const authorColorStrings = colorStrings || author?.colorStrings || null;
    const authorDisplayNameStyles = (!inGuild
        && !ircColorsEnabled
        && AccessibilityStore.displayNameStylesEnabled
        && author?.displayNameStyles) || null;
    ...
    const canUseGradient = (author?.guildId ? (GuildStore.getGuild(author.guildId) ?? {}).premiumFeatures?.features.includes("ENHANCED_ROLE_COLORS") : !inGuild);
    ...
    const _r = (c) => author ? resolveColor(authorColorStrings, authorDisplayNameStyles, c.trim(), canUseGradient, inGuild, ircColorsEnabled, shouldShowHoverEffects) : null;
    const _colorCache = new Map();
    const _rc = (c) => { const k = c.trim(); if (!_colorCache.has(k))
        _colorCache.set(k, _r(k)); return _colorCache.get(k); };
    ...
    const nameElement = (<span style={{
            ...topLevelStyle,
            ...(shouldShowDisplayNameEffect ? {} : topRoleStyle?.normal.original || {})
        }} className="smyn-container">
            {mentionSymbol && <span>{mentionSymbol}</span>}
            {(<span className={SMYNC(firstGroupClasses, { [gradientClasses]: shouldShowGradientGlow })} data-text={shouldShowGradientGlow ? firstDataText : undefined} style={(shouldShowGradientGlow && useTopRoleStyle && topRoleStyle?.gradient
                ? shouldAnimatePrimaryGradient
                    ? topRoleStyle.gradient.animated
                    : topRoleStyle.gradient.static.original
                : undefined)}>
                    <span className={SMYNC(firstNameClasses)} style={shouldShowDisplayNameEffect
                ? undefined
                : topRoleStyle ?
                ...
                ? <DisplayNameEffectName name={first.name} styles={authorDisplayNameStyles} animate={shouldShowHoverEffects} showStaticEffect={showStaticDisplayNameEffect} useMessageLayout={isMessage}/>
                : first.wrapped}
                    </span>
                </span>)}
            {[
        ...
        ].map(({ name, dataText, groupClass, nameClass, position }) => name && (<span key={position} className={SMYNC(groupClass)}>
                    <span style={affixColor} className={prefixClasses}>
        ...
        className={SMYNC(nameClass, {
            ...
            })} data-text={shouldShowGradientGlow && dataText ? dataText : undefined} style={{
                ...(ignoreFonts ? { fontFamily: "var(--font-primary)", letterSpacing: "normal" } : {}),
                ...getSecondaryNameStyle(shouldShowDisplayNameStyle ? name.style : null, shouldShowSecondaryDisplayNameEffect, ignoreEffects, shouldAnimatePrimaryGradient && shouldAnimateSecondaryEffects, shouldShowGradientGlow && shouldAnimateSecondaryEffects)
            }}>
                        {shouldShowSecondaryDisplayNameEffect && authorDisplayNameStyles
                ? <DisplayNameEffectName name={name.name} styles={authorDisplayNameStyles} animate={shouldAnimateSecondaryEffects} ignoreFont={ignoreFonts} showStaticEffect={showStaticDisplayNameEffect} useMessageLayout={isMessage}/>
                : name.wrapped}
                    </span>
                    <span style={affixColor} className={suffixClasses}>
                        {name.suffix}</span>
                </span>))}
        </span>);
    return [allDataText, nameElement, first.name];
}
const hoveringMessageMap = new Map();
const hoveringRepliesMap = new Map();
function handleHoveringMessage(message, isHovered) {
...
function addHoveringMessage(id) {
    if (!id)
        return;
...
function removeHoveringMessage(id) {
    if (!id)
        return;
    const currentCount = hoveringMessageMap.get(id) || 0;
    if (currentCount <= 1) {
...
function removeHoveringReply(id) {
    if (!id)
        return;
    const currentCount = hoveringRepliesMap.get(id) || 0;
    if (currentCount <= 1) {
...
function CustomNicknameModal({ modalProps, user }) {
    const [value, setValue] = useState(customNicknames[user.id] ?? "");
    return (<Modal {...modalProps} size="sm" title={customNicknames[user.id] ? "Change SMYN Nickname" : "Add SMYN Nickname"} actions={[
            {
                text: "Save",
                variant: "primary",
                onClick: async () => {
            ...
            <TextInput value={value} maxLength={32} onChange={setValue} placeholder={user.globalName ?? user.username} style={{ width: "100%" }}/>
            <TextButton className="smyn-reset-button" onClick={async () => {
            setValue("");
            delete customNicknames[user.id];
            await DataStore.set("SMYNCustomNicknames", customNicknames);
            triggerNameRerender();
...
const userContextPatch = (children, { user }) => {
    ...
    (group || children).push(<Menu.MenuItem id="smyn-custom-nickname" label={customNicknames[user.id] ? "Change SMYN Nickname" : "Add SMYN Nickname"} action={() => openModal(props => (<ErrorBoundary>
...
export default definePlugin({
    ...
    async start() {
        ...
        const data = await DataStore.get("SMYNCustomNicknames");
