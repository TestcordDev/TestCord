...
export const buildSeveralUsers = ErrorBoundary.wrap(function buildSeveralUsers({ users, count, guildId }) {
    return (<>
            {users.slice(0, count).map(user => (<React.Fragment key={user.id}>
...
function typingUserColor(guildId, userId) {
    if (!settings.store.showRoleColors)
        return;
...
const TypingUser = ErrorBoundary.wrap(function TypingUser({ user, guildId }) {
    return (<strong className={cl("user")} role="button" onClick={() => {
            openUserProfile(user.id);
        }} style={{
            color: settings.store.showRoleColors ? typingUserColor(guildId, user.id) : undefined,
        }}>
            {settings.store.showAvatars && (<Avatar className={cl("avatar")} size="SIZE_16" src={user.getAvatarURL(guildId, 128)}/>)}
...
export default definePlugin({
    ...
    useTypingUsers(channel) {
        try {
            ...
            return Object.keys(typingUsers)
                .filter(id => {
                if (!id || RelationshipStore.isBlockedOrIgnored(id))
                    return false;
                if (id === myId)
                    return settings.store.amITyping;
                return true;
            })
    ...
    buildSeveralUsers,
    renderTypingUsers: ErrorBoundary.wrap(({ guildId, users, children }) => {
        try {
