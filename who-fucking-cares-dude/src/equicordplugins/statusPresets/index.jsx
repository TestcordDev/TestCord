...
const StatusSubMenuComponent = () => {
    ...
    return (<Menu.Menu navId="sp-custom-status-submenu" onClose={() => { }}>
            {Object.entries(settings.store.StatusPresets).map(([index, status]) => status != null ? (<Menu.MenuItem key={"status-presets-" + index} id={"status-presets-" + index} label={status.text} action={() => (status.emojiInfo?.id == null || premiumType > 0) && setStatus(status)} icon={status.emojiInfo != null
                ? () => <EmojiComponent emoji={status.emojiInfo} animate={false} hideTooltip={false}/>
                : undefined} disabled={status.emojiInfo?.id != null && premiumType === 0}>
                        <Menu.MenuItem id={"status-presets-delete-" + index} label="Delete Preset" action={() => {
...
export default definePlugin({
    ...
    render() {
        ...
        return (<ErrorBoundary>
                <div />
                {status == null ?
                <PMenu id="sp-custom/presets-status" action="PRESS_SET_STATUS" onClick={openCustomStatusModalLazy} icon={() => <div />} label="Set Custom Status" renderSubmenu={StatusSubMenuComponent}/>
                :
                    <PMenu id="sp-edit/presets-status" action="PRESS_EDIT_CUSTOM_STATUS" onClick={openCustomStatusModalLazy} icon={() => status.emoji != null ? (<EmojiComponent emoji={status.emoji} animate={false} hideTooltip={false}/>) : null} label="Edit Custom Status" renderSubmenu={StatusSubMenuComponent}/>}
            </ErrorBoundary>);
    },
    renderRememberButton(status) {
        return {
            text: "Keep",
