...
export function CarouselControls({ activities, currentActivity, onActivityChange }) {
    const currentIndex = activities.findIndex(a => a.id === currentActivity.id);
    return (<div className={cl("controls")} style={{
            display: "flex",
            flexDirection: "row",
            ...
            <div className={cl("controls-carousel")}>
                {activities.map((activity, index) => (<div key={"dot--" + index} onClick={() => onActivityChange(activity)} className={cl("controls-dot", currentActivity.id === activity.id && "controls-selected")}/>))}
            </div>
