...
const Native = VencordNative?.pluginHelpers?.AnonLi;
...
function AnonLiIcon({ className, height = 20, width = 20 }) {
    return (<svg aria-hidden="true" className={className} fill="currentColor" height={height} viewBox="0 0 74.4482 80.1979" width={width}>
...
async function uploadAnonLiDrop() {
    ...
    const result = await Native.uploadDrop(apiKey, file.name, file.type || "application/octet-stream", await file.arrayBuffer(), settings.store.expiryDays ?? 3, settings.store.maxDownloads ?? 0);
    if (!result.success) {
        showToast(result.error, Toasts.Type.FAILURE);
        return;
    }
    if (settings.store.copyAfterUpload)
        await copyWithToast(result.url, "Anon.li link copied.");
...
const channelAttachContextMenuPatch = children => {
    if (children.find(child => child?.props?.id === "vc-anonli-drop"))
        return;
    children.push(<Menu.MenuItem id="vc-anonli-drop" label={(<Flex alignItems="center" gap="8px">
                    <AnonLiIcon height={18} width={18}/>
                    <span>Anon.li Drop</span>
                </Flex>)} action={() => void uploadAnonLiDrop()}/>);
};
