...
var FolderIconDisplay;
(function (FolderIconDisplay) {
    FolderIconDisplay[FolderIconDisplay["Never"] = 0] = "Never";
    FolderIconDisplay[FolderIconDisplay["Always"] = 1] = "Always";
    FolderIconDisplay[FolderIconDisplay["MoreThanOneFolderExpanded"] = 2] = "MoreThanOneFolderExpanded";
})(FolderIconDisplay || (FolderIconDisplay = {}));
...
function injectStyles() {
    if (document.getElementById("betterfolders-foldersize-styles"))
        return;
...
function filterTreeWithTargetNode(children, predicate) {
    if (children == null) {
        return false;
    }
...
export default definePlugin({
    ...
    settings,
    start() { if (settings.store.consistentFolderSize) {
        injectStyles();
    } },
    stop() { if (settings.store.consistentFolderSize) {
        removeStyles();
    } },
    patches: [
        {
    ...
    FolderSideBar,
    closeFolders,
    wrapGuildNodeComponent(node, originalComponent, isBetterFolders, expandedFolderIds) {
        if (!isBetterFolders ||
    ...
    makeGuildsBarSidebarFilter(isBetterFolders) {
        return (child) => {
            if (!isBetterFolders) {
                return true;
            }
    ...
    shouldShowFolderIconAndBackground(isBetterFolders, expandedFolderIds) {
        if (!isBetterFolders) {
            return true;
        }
    ...
    shouldShowTransition(props) {
        // Pending guilds
        if (props?.folderNode?.id === 1)
            return true;
        return !!props?.isBetterFolders;
    },
    shouldRenderContents(props, isExpanded) {
        // Pending guilds
        if (props?.folderNode?.id === 1)
            return false;
