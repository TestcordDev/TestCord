...
function getThemeActivationMode(themeId) {
    return Settings.themeActivationModes?.[themeId] ?? "always";
}
function shouldApplyTheme(mode, activeTheme) {
    if (mode === "always")
        return true;
    if (!activeTheme)
        return false;
...
async function initThemes() {
    ...
    if (IS_WEB) {
        ...
        const themesToApply = enabledThemes.filter(theme => shouldApplyTheme(getThemeActivationMode(theme), activeTheme));
...
function applyToPopout(popoutWindow, key) {
    if (!popoutWindow?.document)
        return;
    const doc = popoutWindow.document;
    doc.querySelector("vencord-root")?.remove();
    const clonedRoot = vencordRootNode.cloneNode(false);
...
export function initQuickCssThemeStore(themeStore) {
    if (IS_USERSCRIPT)
        return;
    initThemes();
