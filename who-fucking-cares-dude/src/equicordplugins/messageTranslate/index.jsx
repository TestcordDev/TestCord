...
function shouldTranslate(message) {
    if (!message.content || typeof message.content !== "string")
        return false;
    if (hasFailed(message.id, message.content))
        return false;
    ...
    if (settings.store.skipBotMessages && message.author?.bot)
        return false;
    if (message.author && getIgnoredUsers().has(message.author.id))
        return false;
    if (message.channel_id && getIgnoredChannels().has(message.channel_id))
        return false;
    const guildId = message.channel_id ? ChannelStore.getChannel(message.channel_id)?.guild_id : null;
    if (guildId && getIgnoredGuilds().has(guildId))
        return false;
    return true;
}
function triggerReRender(message) {
...
export default definePlugin({
    ...
    wrapContent(content, messageId, channelId) {
        ...
        if (!sourceLang || !cached)
            return content;
        const indicatorElement = settings.store.showIndicator && (<div className={cl("indicator")}>translated from {sourceLang}</div>);
