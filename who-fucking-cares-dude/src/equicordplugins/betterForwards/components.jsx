...
import { cl, ForwardOptionsContext } from ".";
...
export function GuildName({ guildId }) {
    const currentGuildId = useStateFromStores([SelectedGuildStore], () => SelectedGuildStore.getGuildId(), []);
    const guild = useStateFromStores([GuildStore, BasicGuildStore], () => GuildStore.getGuild(guildId) ?? BasicGuildStore.getGuild(guildId), [guildId]);
    const icon = useMemo(() => {
        if (!guild)
            return null;
        return guild.icon ? (<img src={IconUtils.getGuildIconURL({ ...guild, canAnimate: true, size: 16 })} alt={`Server icon for ${guild.name}`} className={cl("guild-icon")}/>) : (<div className={cl("guild-acronym")}>{getGuildAcronym(guild)}</div>);
    }, [guild]);
    const guildDivRef = useRef(null);
    useEffect(() => void fetchBasicGuild(guildId), [guildId]);
    return (currentGuildId !== guildId && (<Popout position="top" renderPopout={() => <ServerProfileComponent guildId={guildId}/>} targetElementRef={guildDivRef}>
...
export function ChannelName({ guildId, channelId, messageId }) {
    const channel = useStateFromStores([ChannelStore], () => ChannelStore.getChannel(channelId), [channelId]);
    const name = useStateFromStores([UserStore, RelationshipStore], () => channel ? (formatChannelName(channel, UserStore, RelationshipStore, false, false)) : (<i>{guildId ? getIntlMessage("UNKNOWN_CHANNEL").toLowerCase() : getIntlMessage("UNKNOWN_USER")}</i>), [channel, guildId]);
    const icon = useMemo(() => {
        if (channel?.isDM()) {
            const [user] = channel.recipients.map(UserStore.getUser).filter(Boolean);
            if (user)
                return (<img src={getUserAvatarUrl(user, guildId, true, 16)} alt={`DM icon for ${name}`} className={cl("user-icon")}/>);
        }
        if (channel?.isGroupDM()) {
            return (<img src={IconUtils.getChannelIconURL({
                    ...channel,
                    applicationId: channel.getApplicationId(),
                    size: 16
                })} alt={`Group DM icon for ${name}`} className={cl("user-icon")}/>);
        }
    ...
    return (<div className={cl("footer-element")} onClick={() => navigateTo(guildId ?? "@me", channelId, messageId)} onMouseEnter={() => ChannelActionCreators.preload(guildId ?? "@me", channelId)}>
            {icon}
            <BaseText size="sm" weight="medium" className={cl("footer-text")}>
                {name}
            </BaseText>
...
export function Timestamp({ snowflake }) {
    const formatted = useMemo(() => DateUtils.calendarFormat(new Date(SnowflakeUtils.extractTimestamp(snowflake))), [snowflake]);
    return (<div className={cl("footer-element")} style={{ pointerEvents: "none" }}>
            <BaseText size="sm" weight="medium" className={cl("footer-text")}>
                {formatted}
            </BaseText>
        </div>);
...
export function ForwardPicker() {
    ...
    const { message } = state;
    if (!message || message.embeds.length + message.attachments.length === 0)
        return null;
    return (<Flex gap={12} flexDirection="column">
...
export function EmbedPicker({ message, opts, setOpts, hasOpts, defaultOpts }) {
    ...
    return embeds?.map(({ title, subEmbeds }) => (<Flex gap={4} flexDirection="column" key={subEmbeds[0].id}>
            <BaseText size="sm" color="text-subtle" className={cl("embed-name")} style={{ opacity: !hasOpts ? 0.5 : undefined }}>
                {title}
            </BaseText>
            <TagContainer>
                {subEmbeds.map(({ id, name, isMainEmbed }) => {
            const Icon = isMainEmbed ? EmbedIcon : ImageIcon;
            return (<Tag key={id} id={id} source={hasOpts ? (opts.onlyEmbedIndices ?? []) : defaultOpts.onlyEmbedIndices} onChange={onlyEmbedIndices => setOpts(prev => ({ ...prev, onlyEmbedIndices }))} disabled={!hasOpts}>
...
export function AttachmentPicker({ message, opts, setOpts, hasOpts, defaultOpts }) {
    return (<TagContainer>
            {message.attachments.map(attachment => (<Tag key={attachment.id} id={attachment.id} source={hasOpts ? opts.onlyAttachmentIds ?? [] : defaultOpts.onlyAttachmentIds} onChange={onlyAttachmentIds => setOpts(prev => ({ ...prev, onlyAttachmentIds }))} disabled={!hasOpts}>
...
function Tag({ id, children, source, onChange, disabled }) {
    const selected = useMemo(() => source.includes(id), [source, id]);
    return (<div className={tagClasses.tag} data-selection-mode="multiple" data-selected={!disabled && selected ? "true" : undefined} onClick={() => onChange(selected ? source.filter(x => x !== id) : [...source, id])} style={{ textWrap: "wrap", opacity: disabled ? .5 : undefined }} inert={disabled}>
            {children}
        </div>);
}
