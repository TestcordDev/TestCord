...
const SearchIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
const EditIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="white">
        <path d="M3 17.25V21H6.75L17.81 9.94L14.06 6.19L3 17.25ZM20.71 7.04C21.1 6.65 21.1 6.02 20.71 5.63L18.37 3.29C17.98 2.9 17.35 2.9 16.96 3.29L15.13 5.12L18.88 8.87L20.71 7.04Z"/>
    </svg>);
const DownloadIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/>
    </svg>);
const UploadIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/>
    </svg>);
const TrashIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/>
    </svg>);
const SyncIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z"/>
    </svg>);
async function uploadToSync(data) {
    try {
...
function getCurrentBanListState() {
    ...
    try {
        ...
        if (plugin.settings.store.store) {
            ...
            reasonEntries.forEach(entry => {
                const [id, reason] = entry.split("/");
                if (id && reason)
                    existingReasons[id] = reason;
            });
        }
        // Merge data
        const mergedUsers = [...existingUsers];
        const mergedReasons = { ...existingReasons };
        syncResult.users.forEach((userData) => {
        ...
        const newCount = syncResult.users.filter((u) => !existingUsers.includes(u.id)).length;
...
function exportBanList(pluginName, usersKey, reasonsKey) {
    const plugin = Vencord.Plugins.plugins[pluginName];
    if (!plugin?.settings?.store)
        return;
    // Get users
...
function importBanList(pluginName, usersKey, reasonsKey, onUpdate) {
    ...
    input.onchange = e => {
        ...
        reader.onload = e => {
            try {
                ...
                const newCount = importData.users.filter((u) => !existingUsers.includes(u.id)).length;
...
function clearBanList(pluginName, usersKey, reasonsKey, onUpdate) {
...
function BanList({ pluginName, usersKey, reasonsKey, title, onUpdate }) {
    ...
    return (<div style={{
            ...
            borderBottom: "2px solid var(--brand-experiment)"
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <Text variant="heading-lg/semibold" style={{
            color: "white",
            fontSize: "24px",
            fontWeight: "700"
        }}>
            ...
            gap: "6px"
                    ...
                    <Button size="small" color="green" onClick={() => exportBanList(pluginName, usersKey, reasonsKey)} style={{
            ...
            gap: "6px"
                    ...
                    <Button size="small" color="blurple" onClick={() => importBanList(pluginName, usersKey, reasonsKey, triggerUpdate)} style={{
            backgroundColor: "var(--brand-experiment)",
            color: "white",
            fontWeight: "600",
            display: "flex",
            ...
            gap: "6px"
                    ...
                    <Button size="small" color="red" onClick={() => clearBanList(pluginName, usersKey, reasonsKey, triggerUpdate)} style={{
            ...
            <div style={{ marginBottom: "24px", position: "relative" }}>
                <TextInput value={searchTerm} onChange={setSearchTerm} placeholder="🔍 Search by username, display name, or reason..." style={{
            width: "100%",
            paddingRight: "40px",
            ...
            padding: "12px"
                ...
                {filteredUsers.length === 0 ? (<div style={{
                textAlign: "center",
                padding: "40px 20px",
                color: "var(--text-muted)",
                fontSize: "16px",
                ...
                `No users matching "${searchTerm}"`}
                    </div>) : (filteredUsers.map((id, index) => {
            const user = userMap[id];
            ...
            return (<div key={id} style={{
                    ...
                    fontWeight: "700"
                                ...
                                {/* Avatar */}
                                <img onClick={() => openUserProfile(id)} src={user?.getAvatarURL?.() ?? "https://cdn.discordapp.com/embed/avatars/0.png"} style={{
                    width: "48px",
                    height: "48px",
                    borderRadius: "50%",
                    cursor: "pointer",
                    ...
                    marginBottom: "2px"
                                    ...
                                    {user?.globalName && user.globalName !== user.username && (<div style={{
                        fontSize: "14px",
                        color: "var(--text-muted)",
                        fontStyle: "italic"
                    }}>
                    ...
                    fontFamily: "monospace"
                                ...
                                {reasonsKey && (<div style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "8px",
                        minWidth: "250px"
                    }}>
                                        <TextInput value={isEditing ? reasons[id] || "" : reason} placeholder="Enter ban reason..." disabled={!isEditing} onChange={value => setReasons(prev => ({ ...prev, [id]: value }))} style={{
                        flex: 1,
                        ...
                        fontSize: "14px"
                                        ...
                                        {isEditing ? (<Button size="small" color="green" onClick={() => saveReason(id, reasons[id] || "")} style={{
                            backgroundColor: "#43b581",
                            color: "white",
                            fontWeight: "600"
                        }}>
                                                ✓ Save
                                            </Button>) : (<div onClick={() => setEditingReasons(prev => ({ ...prev, [id]: true }))} style={{
                            cursor: "pointer",
                            padding: "8px",
                            borderRadius: "6px",
                            backgroundColor: "var(--background-secondary)",
                            ...
                            e.currentTarget.style.color = "var(--text-normal)";
                                ...
                                {/* Remove button */}
                                <Button onClick={() => removeUser(id)} size="small" color="red" style={{
                    backgroundColor: "#f04747",
                    color: "white",
                    fontWeight: "600",
                    minWidth: "80px"
...
function UserList({ modalProps }) {
    ...
    return (<ModalRoot {...modalProps} size={"large" /* ModalSize.LARGE */}>
            ...
            marginTop: "4px"
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                        <Button size="small" color="blurple" onClick={handleSyncDownload} style={{
            backgroundColor: "var(--brand-experiment)",
            color: "white",
            fontWeight: "600",
            display: "flex",
            ...
            gap: "6px"
                        ...
                        <Button size="small" color="green" onClick={handleSyncUpload} style={{
            backgroundColor: "#43b581",
            color: "white",
            fontWeight: "600",
            display: "flex",
            ...
            backgroundColor: "var(--background-secondary)"
                    ...
                    <div style={{ display: "flex", borderBottom: "2px solid var(--background-modifier-accent)" }}>
                        <button style={tabStyle(activeTab === "single")} onClick={() => setActiveTab("single")} onMouseEnter={e => {
            if (activeTab !== "single") {
            ...
            fontWeight: "600"
                        ...
                        <button style={tabStyle(activeTab === "multi")} onClick={() => setActiveTab("multi")} onMouseEnter={e => {
            if (activeTab !== "multi") {
                e.currentTarget.style.backgroundColor = "var(--background-modifier-hover)";
                e.currentTarget.style.transform = "translateY(-1px)";
            }
            ...
            fontWeight: "600"
                ...
                <div style={{ padding: "0 20px 20px 20px" }} key={refreshKey}>
                    {activeTab === "single" ? (<BanList pluginName="autoBan" usersKey="users" reasonsKey="store" title="Dadscord Bans" onUpdate={handleUpdate}/>) : (<BanList pluginName="MultiServerAutoban" usersKey="users" reasonsKey="reasons" title="Multi Server Bans" onUpdate={handleUpdate}/>)}
                </div>
            </ModalContent>
            ...
            fontStyle: "italic"
                    ...
                    <Button color="red" onClick={modalProps.onClose} style={{
            backgroundColor: "#f04747",
            color: "white",
            fontWeight: "600",
            padding: "10px 20px",
