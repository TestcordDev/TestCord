...
function SaveModal({ modalProps, initialName, onSave }) {
    const [name, setName] = React.useState(initialName);
    const [scope, setScope] = React.useState({
        plugins: true, themes: true, quickCss: false, dataStore: false,
    });
    const toggle = (key, v) => setScope(s => ({ ...s, [key]: v }));
    ...
    return (<ModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */}>
            ...
            <ModalContent className="vc-presets-modal-content">
                ...
                {SCOPES.map(s => (<FormSwitch key={s.key} value={scope[s.key]} onChange={v => toggle(s.key, v)} title={s.label} description={s.warn && scope[s.key] ? `⚠ ${s.description}` : s.description}/>))}
...
export function openSaveModal(initialName, onSave) {
