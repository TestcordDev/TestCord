...
async function fetchBatch(channelId, before) {
...
async function runPurge(userId, cb) {
...
function PurgeModal({ rootProps, user }) {
    ...
    return (<ModalRoot {...rootProps} size={"medium" /* ModalSize.MEDIUM */}>
            <ModalHeader>
            ...
            <ModalContent>
                <div className={cl("modal")}>
                    {!armed ? (<Text variant="text-md/normal">
                            ...
                            <div className={cl("feed")}>
                                {lines.length === 0 && state === "running" && (<Text variant="text-sm/normal" style={{ color: "var(--text-muted)" }}>
                                        scanning history…
                                    </Text>)}
                                {lines.map(line => (<div key={line.id} className={cl("line") + " " + cl(line.ok ? "line-killed" : "line-failed")}>
            ...
            <ModalFooter>
                {!armed ? (<>
                    ...
                    </>) : state === "running" ? (<Button color={Button.Colors.RED} onClick={stop}>
                        Stop
                    </Button>) : (<Button color={Button.Colors.BRAND} onClick={rootProps.onClose}>
                        Close
                    </Button>)}
            </ModalFooter>
        </ModalRoot>);
}
function openPurgeModal(user) {
...
const userContextPatch = (children, props) => {
    ...
    children.push(<Menu.MenuGroup>
            <Menu.MenuItem id="vc-purgedms" label="Purge DMs" color="danger" action={() => openPurgeModal(user)}/>
        </Menu.MenuGroup>);
};
