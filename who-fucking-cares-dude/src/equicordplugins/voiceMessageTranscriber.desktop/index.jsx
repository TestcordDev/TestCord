...
const Native = VencordNative.pluginHelpers.VoiceMessageTranscriber;
...
const settings = definePluginSettings({
    ...
    delete: {
        ...
        component: () => {
            ...
            return <Button variant="dangerPrimary" onClick={() => {
...
function LanguageSelectionModal(props) {
    ...
    return (<Modal {...modalProps} size="md" title="Transcription Options" actions={[
            {
                text: "Start",
                variant: "primary",
                onClick: start
            ...
            <Flex flexDirection="column" gap={20} style={{ padding: "16px" }}>
                <div>
                    ...
                    <SearchableSelect options={languageOptions} value={languageOptions.find(o => o.value === language)?.value} onChange={setLanguage}/>
                </div>
                ...
                <div>
                    ...
                    <ManaBaseRadioGroup options={[{
                name: "Transcribe",
                value: "transcribe"
            }, {
                name: "Translate to English",
                value: "translate"
            }]} value={task} onChange={v => setTask(v)}/>
                </div>
            </Flex>
        </Modal>);
}
function TranscriptionModal(props) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!active)
                    return;
                workerRef.current = new TranscriptionWorker(s => {
                    if (active)
                        setStatus(s);
                }, out => {
                    if (active) {
                ...
                workerRef.current.run(audioData, selectedModel, quantized, options.language === "auto" ? undefined : options.language, options.task);
            }
            catch (err) {
                if (active)
                    setError(String(err));
    ...
    const displayText = result ? (showTimestamps
    ...
    return (<Modal {...modalProps} size="lg" title="Transcription" actions={actions}>
            <div className={cl("content")}>
                {error ? (<Flex flexDirection="column" gap={16} style={{ padding: "20px 0" }}>
                        <Heading tag="h3" style={{ color: "var(--red-360)" }}>Error</Heading>
                        <Span style={{ whiteSpace: "pre-wrap" }}>{error}</Span>
                    </Flex>) : (status === "complete" || (status === "transcribing" && result)) ? (<Flex flexDirection="column" gap={16} style={{ paddingBottom: "20px" }}>
                        {status === "transcribing" && (<Span size="sm" color="text-muted">Transcribing in progress...</Span>)}
                        <ScrollerAuto className={cl("result")}>
                            <Span>{displayText}</Span>
                        </ScrollerAuto>
                        <Flex flexDirection="row" gap={12} alignItems="center">
                            <div style={{ flexGrow: 1 }}>
                                <FormSwitch title="Show Timestamps" value={showTimestamps} onChange={setShowTimestamps}/>
                            </div>
                        </Flex>
                    </Flex>) : (<Flex flexDirection="column" gap={16} style={{ padding: "20px 0", alignItems: "center" }}>
                        <Heading tag="h3">
...
export default definePlugin({
    ...
    set ManaBaseRadioGroup(value) {
        ManaBaseRadioGroup = value;
    },
    settings,
    button(src) {
        return <button className={cl("button")} style={{ backgroundColor: "transparent" }} onClick={e => {
                e.stopPropagation();
