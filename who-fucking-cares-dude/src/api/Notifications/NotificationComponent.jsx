...
export default ErrorBoundary.wrap(function NotificationComponent({ title, body, richBody, color, icon, onClick, onClose, image, permanent, className, dismissOnClick }) {
    ...
    return (<button className={classes("vc-notification-root", className)} style={position === "bottom-right" ? { bottom: "1rem" } : { top: "3rem" }} onClick={() => {
            onClick?.();
            ...
            <div className="vc-notification">
                ...
                <div className="vc-notification-content">
                    <div className="vc-notification-header">
                        ...
                        <button className="vc-notification-close-btn" onClick={e => {
            e.preventDefault();
            e.stopPropagation();
            onClose();
        }}>
                            <svg width="24" height="24" viewBox="0 0 24 24" role="img" aria-labelledby="vc-notification-dismiss-title">
            ...
            {timeout !== 0 && !permanent && (<div className="vc-notification-progressbar" style={{ width: `${(1 - timeoutProgress) * 100}%`, backgroundColor: color || "var(--brand-500)" }}/>)}
        </button>);
}, {
