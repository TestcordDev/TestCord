...
const getUserId = () => {
    ...
    if (!id)
        throw new Error("User not yet logged in");
    return id;
};
...
async function setAuthorization(secret) {
    await DataStore.update("Vencord_cloudSecret", secrets => {
        secrets ??= {};
...
export async function authorizeCloud() {
    ...
    openModal((props) => <OAuth2AuthorizeModal {...props} scopes={["identify"]} responseType="code" redirectUri={redirectUri} permissions={0n} clientId={clientId} cancelCompletesFlow={false} callback={async ({ location }) => {
            if (!location) {
                Settings.cloud.authenticated = false;
                return;
            }
