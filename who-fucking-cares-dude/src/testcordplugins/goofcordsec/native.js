...
const log = (...args) => console.log("[GoofcordSecurity]", ...args);
function getStore() {
    return RendererSettings.store.plugins?.GoofcordSecurity ?? {};
}
function isEnabled() { return getStore().enabled === true; }
...
function generateUserAgentString(platform, majorVersion) {
...
function generateClientHints(platform, arch, osVersion, chromeVersion) {
...
function getProfile() {
    ...
    const targetPlatform = isWindowsSpoof ? "win32" : process.platform;
...
async function spoofChrome(win) {
    if (!getStore().spoofChrome)
        return;
        ...
        try {
            if (!win.webContents.debugger.isAttached()) {
                try {
                    ...
                    console.warn("[GoofcordSecurity] Debugger attach warning:", err);
                }
            }
...
function injectInvidiousReplacer(win) {
    const store = getStore();
    if (!store.invidiousEmbeds)
        return;
...
function injectRequestInterceptor(win) {
    void win.webContents.once("dom-ready", () => {
        void win.webContents.executeJavaScript(REQUEST_INTERCEPTOR_SCRIPT).catch(() => { });
    });
}
// ───────────────────────────────────────────────────────────────── bootstrap
...
function bootstrap() {
    ...
    bootstrapped = true;
    if (!isEnabled()) {
        log("Plugin disabled — skipping init");
        return;
    }
    initFirewall();
    initCspUnstricter();
