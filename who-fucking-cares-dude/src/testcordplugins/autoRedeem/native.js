...
function validatePageUrl(pageUrl) {
    ...
    try {
        ...
        if (url.protocol !== "https:" || url.hostname !== "discord.com" && !url.hostname.endsWith(".discord.com"))
            return null;
        return url.toString();
    }
    catch {
...
async function requestNoneCap(url, apiKey, signal, body) {
...
function parseSolve(data) {
    if (!isRecord(data))
        throw new Error("NoneCap returned an invalid response.");
    return {
...
export async function solveCaptcha(_, apiKey, sitekey, rqdata, pageUrl, userAgent) {
    const key = typeof apiKey === "string" ? apiKey.trim() : "";
    const url = typeof pageUrl === "string" ? validatePageUrl(pageUrl) : null;
    if (!key || key.length > 512 || /[\r\n]/.test(key))
        return { success: false, error: "NoneCap API key is invalid." };
    if (typeof sitekey !== "string" || !sitekey || sitekey.length > 256)
        return { success: false, error: "CAPTCHA site key is invalid." };
    if (rqdata !== undefined && (typeof rqdata !== "string" || rqdata.length > 20_000))
        return { success: false, error: "CAPTCHA request data is invalid." };
    if (!url)
        return { success: false, error: "Discord page URL is invalid." };
    if (typeof userAgent !== "string" || userAgent.length > 512 || /[\r\n]/.test(userAgent))
        return { success: false, error: "User agent is invalid." };
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), SOLVE_TIMEOUT_MS);
    activeSolves.add(controller);
    try {
        let solve = parseSolve(await requestNoneCap(`${NONECAP_SOLVES_URL}?wait=90`, key, controller.signal, JSON.stringify({
        ...
        while (solve.status === "pending" || solve.status === "solving") {
            ...
            solve = parseSolve(await requestNoneCap(`${NONECAP_SOLVES_URL}/${encodeURIComponent(solve.id)}`, key, controller.signal));
        }
...
export function cancelCaptchaSolves() {
    for (const controller of activeSolves)
        controller.abort();
    activeSolves.clear();
}
