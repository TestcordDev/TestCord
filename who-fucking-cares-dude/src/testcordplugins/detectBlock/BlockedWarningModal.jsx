...
const DetectBlockModalRoot = ModalRoot;
const DetectBlockModalHeader = ModalHeader;
const DetectBlockModalContent = ModalContent;
const DetectBlockModalFooter = ModalFooter;
const DetectBlockModalCloseButton = ModalCloseButton;
const WARNING_RECIPES = {
    voiceJoin: {
        title: "Join voice?",
...
function getPrimaryUser(userIds) {
    return userIds
        .map(id => UserStore.getUser(id))
        .find((user) => Boolean(user));
}
function getAvatar(user, fallbackId) {
    const fallbackUser = UserStore.getUser(fallbackId);
    return (<Avatar src={user?.getAvatarURL(undefined, 24, true) ?? fallbackUser?.getAvatarURL(undefined, 24, true) ?? fallbackUser?.getAvatarURL(undefined, 24, false) ?? undefined} size="SIZE_32"/>);
}
...
function getBlockedSubjectText(names, userIds) {
...
function WarningIcon() {
    return (<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
...
function InfoRow({ icon, children }) {
    return (<div className={cl("info-row")}>
...
export function openBlockedWarningModal({ blockedNames, blockedUserIds, onConfirm, variant, }) {
    ...
    return openModal(modalProps => (<DetectBlockModalRoot {...modalProps} size={"medium" /* ModalSize.MEDIUM */} className={cl("modal")}>
            ...
            <DetectBlockModalFooter className={cl("footer-shell")}>
                <Flex className={cl("footer")}>
                    {recipe.actions.map(action => (<Button key={action.text} variant={action.variant} className={cl("button")} onClick={() => {
                if (action.confirms)
                    void onConfirm();
                modalProps.onClose();
            }}>
