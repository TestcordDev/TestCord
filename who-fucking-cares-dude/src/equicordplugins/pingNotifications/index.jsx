...
import { ChannelStore, NavigationRouter, PresenceStore, RelationshipStore, SelectedChannelStore, UserGuildSettingsStore, UserStore } from "@webpack/common";
...
function checkIfMuted(channel) {
    ...
    if (channel.guild_id) {
        if (UserGuildSettingsStore.isMuted(channel.guild_id))
            return true;
        if (UserGuildSettingsStore.isMuted(channel.guild_id))
            return true;
        if (UserGuildSettingsStore.isChannelMuted(channel.guild_id, channel.id))
            return true;
        if (UserGuildSettingsStore.isCategoryMuted(channel.guild_id, channel.id))
            return true;
    }
    return false;
}
...
export default definePlugin({
    ...
    flux: {
        MESSAGE_CREATE({ message }) {
            try {
                if (!message?.channel_id || message.state === "SENDING")
                    return;
                ...
                if (isUserBlocked(message.author.id))
                    return;
                if (!settings.store.showInActive && channel.id === SelectedChannelStore.getChannelId())
                    return;
                if (PresenceStore.getStatus(currentUser.id) === "dnd")
                    return;
                ...
                if (shouldNotify) {
                    showNotification({
                        ...
                        onClick: () => NavigationRouter.transitionTo(`/channels/${channel.guild_id || "@me"}/${channel.id}/${message.id}`)
                    });
                }
            }
            catch (err) {
