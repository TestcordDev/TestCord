...
import { currentShape, setShape, setShapeFill, ShapeTool } from "@equicordplugins/remix/editor/tools/shape";
...
export const Toolbar = () => {
    ...
    function onChangeTool(old, newTool) {
    ...
    return (<div className="vc-remix-toolbar">
            ...
            <Slider minValue={1} maxValue={500} initialValue={size} onValueChange={setSize} markers={[1, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500]} hideBubble/>}
                </div>
                {(tool === "crop") && <Button onClick={resetBounds}>Reset</Button>}
                <div className="vc-remix-setting-section">
                    {(tool === "shape") && (<>
                        <Select select={setShape} isSelected={v => v === currentShape} serialize={v => String(v)} placeholder="Shape" options={["Rectangle", "Ellipse", "Line", "Arrow"].map(v => ({
                label: v,
                value: v.toLowerCase(),
            }))}/>
