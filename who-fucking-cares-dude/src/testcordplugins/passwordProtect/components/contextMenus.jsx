...
function createPasswordItem(channelId) {
    const isProtected = isPasswordProtected(channelId);
    return (<Menu.MenuItem id="password-protect" label="Password Protect">
            {!isProtected && (<>
                    <Menu.MenuItem id="vc-password-protect-lock" label="Lock" color="brand" action={() => openLockModal(channelId)}/>
                </>)}

            {isProtected && (<>
                    <Menu.MenuItem id="vc-password-protect-unlock" label="Unlock" color="danger" action={() => openUnlockModal(channelId)}/>
                </>)}
...
const ChannelContect = (children, props) => {
