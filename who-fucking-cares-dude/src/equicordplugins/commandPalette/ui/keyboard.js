...
export function comboEquals(a, b) {
    if (a.length !== b.length)
        return false;
    const normalize = (combo) => [...combo].map(k => k.toLowerCase()).sort().join("+");
...
export function formatComboKeys(combo) {
    return combo.map(key => {
        const lower = key.toLowerCase();
        if (IS_MAC && MAC_SYMBOLS[lower])
            return MAC_SYMBOLS[lower];
        if (!IS_MAC && MODIFIERS.includes(lower)) {
...
export function setPaletteKeyHandler(handler) {
    paletteHandler = handler;
}
export function setGlobalKeyHandler(handler) {
